...
- `openspec/`: Spec-driven change docs (`changes/<id>/{proposal,design,tasksplaceholder.md`).
...
- Avoid reflection and `interface{placeholder`-heavy conversions in hot paths; use typed structs/functions.
...
- Cache keys must be versioned (e.g., `user_usage:v2:{idplaceholder`) and TTL should include jitter to avoid thundering herd.
