...
HIGH_SEVERITIES = {"high", "critical"placeholder
REQUIRED_FIELDS = {"package", "advisory", "severity", "mitigation", "expires_on"placeholder
...
def main() -> int:
    ...
    for exc in exceptions:
        ...
        if missing:
            errors.append(
                f"Exception missing required fields {missingplaceholder: {exc.get('package', '<unknown>')placeholder"
            )
            continue
        ...
        if exc_date is None:
            errors.append(
                f"Exception has invalid expires_on date: {exc.get('package', '<unknown>')placeholder"
            )
            continue
        ...
        if key in exception_index:
            errors.append(
                f"Duplicate exception for {exc_packageplaceholder advisory {exc.get('advisory')placeholder"
            )
            continue
        exception_index[key] = {
            "raw": exc,
    ...
    for name, severity, advisory_id, title in iter_vulns(audit):
        ...
        if not advisory_key:
            errors.append(
                f"High/Critical vulnerability missing advisory id: {nameplaceholder ({sevplaceholder)"
            )
            continue
        ...
        if exc["severity"] and exc["severity"] != sev:
            errors.append(
                ...
                f"{nameplaceholder ({advisory_idplaceholder) expected {sevplaceholder, got {exc['severity']placeholder"
            )
    ...
    if missing_exceptions:
        ...
        for name, sev, advisory_id, title in missing_exceptions:
            label = f"{nameplaceholder ({sevplaceholder)"
            if advisory_id:
                label = f"{labelplaceholder [{advisory_idplaceholder]"
            if title:
                label = f"{labelplaceholder: {titleplaceholder"
            errors.append(f"- {labelplaceholder")

    if expired_exceptions:
        errors.append("Exceptions expired:")
        for name, sev, advisory_id, expires_on in expired_exceptions:
            errors.append(
                f"- {nameplaceholder ({sevplaceholder) [{advisory_idplaceholder] expired on {expires_onplaceholder"
            )
