...
RULES: list[Rule] = [
    Rule(
        ...
        pattern=re.compile(r"GOCSPX-[0-9A-Za-z_-]{24,placeholder"),
        allowlist=(
            re.compile(r"GOCSPX-your-"),
            re.compile(r"GOCSPX-placeholder"),
        ),
    ),
    Rule(
        name="google_api_key",
        ...
        pattern=re.compile(r"AIza[0-9A-Za-z_-]{35placeholder"),
        allowlist=(
            re.compile(r"AIza\.{3placeholder"),
            re.compile(r"AIza-your-"),
            re.compile(r"AIza-placeholder"),
        ),
    ),
]
    ...
    for idx, line in enumerate(lines, start=1):
        for rule in RULES:
            ...
            if any(allow.search(line) for allow in rule.allowlist):
                continue
            rel = path.relative_to(repo_root).as_posix()
            findings.append((f"{relplaceholder:{idxplaceholder ({rule.nameplaceholder)", idx))
    return findings
...
def main(argv: Sequence[str]) -> int:
    ...
    if problems:
        ...
        for p in problems:
            sys.stderr.write(f"- {pplaceholder\n")
