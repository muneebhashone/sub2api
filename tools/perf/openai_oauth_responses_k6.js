...
export const options = {
  scenarios: {
    non_stream: {
      ...
      maxVUs,
      exec: 'runNonStream',
      tags: { request_type: 'non_stream' placeholder,
    placeholder,
    stream: {
      executor: 'constant-arrival-rate',
      rate: streamRPS,
      ...
      maxVUs,
      exec: 'runStream',
      tags: { request_type: 'stream' placeholder,
    placeholder,
  placeholder,
  thresholds: {
...
function recordMetrics(res, stream) {
  reqDurationMs.add(res.timings.duration, { request_type: stream ? 'stream' : 'non_stream' placeholder);
  ttftMs.add(res.timings.waiting, { request_type: stream ? 'stream' : 'non_stream' placeholder);
  non2xxRate.add(res.status < 200 || res.status >= 300, { request_type: stream ? 'stream' : 'non_stream' placeholder);

  if (stream) {
    const done = !!res.body && res.body.indexOf('[DONE]') >= 0;
    streamDoneRate.add(done, { request_type: 'stream' placeholder);
  placeholder
placeholder

function postResponses(stream) {
  const url = `${baseURLplaceholder/v1/responses`;
  const res = http.post(url, buildBody(stream), {
    headers: buildHeaders(),
    timeout,
    tags: { endpoint: '/v1/responses', request_type: stream ? 'stream' : 'non_stream' placeholder,
  placeholder);
