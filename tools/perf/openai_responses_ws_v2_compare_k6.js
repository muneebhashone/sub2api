...
export const options = {
  scenarios: {
    http_baseline: {
      ...
      preAllocatedVUs,
      maxVUs,
      tags: { path: 'http_baseline' placeholder,
    placeholder,
    ws_baseline: {
      executor: 'constant-arrival-rate',
      exec: 'runWSBaseline',
      ...
      preAllocatedVUs,
      maxVUs,
      tags: { path: 'ws_baseline' placeholder,
    placeholder,
    ws_chain_20_rounds: {
      executor: 'constant-arrival-rate',
      ...
      tags: { path: 'ws_chain_20_rounds' placeholder,
    placeholder,
  placeholder,
  thresholds: {
...
function buildBody(previousResponseID) {
  const body = {
    ...
    input: [
      {
        role: 'user',
        content: [{ type: 'input_text', text: '请回复一个单词: pong' placeholder],
      placeholder,
    ],
...
export function runHTTPBaseline() {
  const res = postResponses(httpAPIKey, buildBody(''), { transport: 'http' placeholder);
  httpDurationMs.add(res.timings.duration, { transport: 'http' placeholder);
  httpNon2xxRate.add(res.status < 200 || res.status >= 300, { transport: 'http' placeholder);
placeholder

export function runWSBaseline() {
  const res = postResponses(wsAPIKey, buildBody(''), { transport: 'ws_v2' placeholder);
  wsDurationMs.add(res.timings.duration, { transport: 'ws_v2' placeholder);
  wsNon2xxRate.add(res.status < 200 || res.status >= 300, { transport: 'ws_v2' placeholder);
placeholder
...
export function runWSChain20Rounds() {
  ...
  for (let round = 1; round <= chainRounds; round += 1) {
    ...
    const res = postResponses(wsAPIKey, buildBody(previousResponseID), { transport: 'ws_v2_chain' placeholder);
    const ok = res.status >= 200 && res.status < 300;
    wsChainRoundSuccessRate.add(ok, { round: `${roundplaceholder` placeholder);
    wsChainDurationMs.add(Date.now() - roundStart, { round: `${roundplaceholder` placeholder);
    wsChainTTFTMs.add(res.timings.waiting, { round: `${roundplaceholder` placeholder);
    wsNon2xxRate.add(!ok, { transport: 'ws_v2_chain' placeholder);
    if (!ok) {
      return;
    placeholder
    const respID = parseResponseID(res);
    if (!respID) {
      wsChainRoundSuccessRate.add(false, { round: `${roundplaceholder`, reason: 'missing_response_id' placeholder);
      return;
    placeholder
