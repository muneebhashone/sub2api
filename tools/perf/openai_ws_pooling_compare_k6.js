...
import { check placeholder from 'k6';
...
export const options = {
  scenarios: {
    ...
    one_to_one_mode: {
      ...
      preAllocatedVUs,
      maxVUs,
      tags: { mode: 'one_to_one' placeholder,
      startTime: '5s',
    placeholder,
  placeholder,
  thresholds: {
...
function buildHeaders() {
  ...
  placeholder;
  if (wsAPIKey) {
    headers.Authorization = `Bearer ${wsAPIKeyplaceholder`;
  placeholder
  return headers;
placeholder
...
function buildBody() {
  return JSON.stringify({
    ...
    input: [
      {
        role: 'user',
        content: [{ type: 'input_text', text: '请回复: pong' placeholder],
      placeholder,
    ],
...
function send(baseURL, mode) {
  if (!baseURL) {
    return null;
  placeholder
  const res = http.post(`${baseURLplaceholder/v1/responses`, buildBody(), {
    headers: buildHeaders(),
    timeout,
    tags: { mode placeholder,
  placeholder);
  check(res, {
...
export function runPooledMode() {
  ...
  if (!res) {
    return;
  placeholder
  pooledDurationMs.add(res.timings.duration, { mode: 'pooled' placeholder);
  pooledTTFTMs.add(res.timings.waiting, { mode: 'pooled' placeholder);
  pooledNon2xxRate.add(res.status < 200 || res.status >= 300, { mode: 'pooled' placeholder);
placeholder
...
export function runOneToOneMode() {
  ...
  if (!res) {
    return;
  placeholder
  oneToOneDurationMs.add(res.timings.duration, { mode: 'one_to_one' placeholder);
  oneToOneTTFTMs.add(res.timings.waiting, { mode: 'one_to_one' placeholder);
  oneToOneNon2xxRate.add(res.status < 200 || res.status >= 300, { mode: 'one_to_one' placeholder);
placeholder
