...
if [ -z "${BASH_VERSION:-placeholder" ]; then
...
BASH_MAJOR_VERSION="${BASH_VERSION%%.*placeholder"
...
msg() {
    ...
    if [ "$LANG_CHOICE" = "en" ]; then
        echo "${MSG_EN[$key]placeholder"
    else
        echo "${MSG_ZH[$key]placeholder"
    fi
placeholder

# Print functions
print_info() {
    echo -e "${BLUEplaceholder[$(msg 'info')]${NCplaceholder $1"
placeholder

print_success() {
    echo -e "${GREENplaceholder[$(msg 'success')]${NCplaceholder $1"
placeholder

print_warning() {
    echo -e "${YELLOWplaceholder[$(msg 'warning')]${NCplaceholder $1"
placeholder

print_error() {
    echo -e "${REDplaceholder[$(msg 'error')]${NCplaceholder $1"
placeholder
...
configure_server() {
    ...
    if ! is_interactive; then
        print_info "$(msg 'server_config_summary'): ${SERVER_HOSTplaceholder:${SERVER_PORTplaceholder (default)"
        return
    fi
    ...
    echo "==============================================${NCplaceholder"
    echo ""
    echo -e "${BLUEplaceholder$(msg 'server_config_desc')${NCplaceholder"
    echo ""

    # Server host
    echo -e "${YELLOWplaceholder$(msg 'server_host_hint')${NCplaceholder"
    read -p "$(msg 'server_host_prompt') [${SERVER_HOSTplaceholder]: " input_host < /dev/tty
    ...
    # Server port
    echo -e "${YELLOWplaceholder$(msg 'server_port_hint')${NCplaceholder"
    while true; do
        read -p "$(msg 'server_port_prompt') [${SERVER_PORTplaceholder]: " input_port < /dev/tty
    ...
    echo ""
    print_info "$(msg 'server_config_summary'): ${SERVER_HOSTplaceholder:${SERVER_PORTplaceholder"
    echo ""
placeholder
...
detect_platform() {
    ...
    print_info "$(msg 'detected_platform'): ${OSplaceholder_${ARCHplaceholder"
placeholder
...
check_dependencies() {
    ...
    if [ ${#missing[@]placeholder -gt 0 ]; then
        print_error "$(msg 'missing_deps'): ${missing[*]placeholder"
...
github_api_curl() {
    ...
    if [ -n "${UPDATE_GITHUB_TOKEN:-placeholder" ]; then
...
get_latest_version() {
    ...
    LATEST_VERSION=$(github_api_curl -s --connect-timeout 10 --max-time 30 "https://api.github.com/repos/${GITHUB_REPOplaceholder/releases/latest" 2>/dev/null | grep '"tag_name"' | sed -E 's/.*"([^"]+)".*/\1/')
...
list_versions() {
    ...
    local versions
    versions=$(github_api_curl -s --connect-timeout 10 --max-time 30 "https://api.github.com/repos/${GITHUB_REPOplaceholder/releases" 2>/dev/null | grep '"tag_name"' | sed -E 's/.*"([^"]+)".*/\1/' | head -20)
...
validate_version() {
    ...
    local http_code
    http_code=$(github_api_curl -s -o /dev/null -w "%{http_codeplaceholder" --connect-timeout 10 --max-time 30 "https://api.github.com/repos/${GITHUB_REPOplaceholder/releases/tags/${versionplaceholder" 2>/dev/null)
...
download_and_extract() {
    local version_num=${LATEST_VERSION#vplaceholder
    local archive_name="sub2api_${version_numplaceholder_${OSplaceholder_${ARCHplaceholder.tar.gz"
    local download_url="https://github.com/${GITHUB_REPOplaceholder/releases/download/${LATEST_VERSIONplaceholder/${archive_nameplaceholder"
    local checksum_url="https://github.com/${GITHUB_REPOplaceholder/releases/download/${LATEST_VERSIONplaceholder/checksums.txt"

    print_info "$(msg 'downloading') ${archive_nameplaceholder..."
    ...
    if curl -sL "$checksum_url" -o "$TEMP_DIR/checksums.txt" 2>/dev/null; then
        local expected_checksum=$(grep "$archive_name" "$TEMP_DIR/checksums.txt" | awk '{print $1placeholder')
        local actual_checksum=$(sha256sum "$TEMP_DIR/$archive_name" | awk '{print $1placeholder')
...
Environment=SERVER_HOST=${SERVER_HOSTplaceholder
Environment=SERVER_PORT=${SERVER_PORTplaceholder
...
get_public_ip() {
    ...
    PUBLIC_IP=$(hostname -I 2>/dev/null | awk '{print $1placeholder' || echo "YOUR_SERVER_IP")
    return 1
placeholder
...
print_completion() {
    ...
    local display_host="${PUBLIC_IP:-YOUR_SERVER_IPplaceholder"
    ...
    echo "$(msg 'server_config_summary'): ${SERVER_HOSTplaceholder:${SERVER_PORTplaceholder"
    echo ""
    echo "=============================================="
    echo "  $(msg 'step4_open_wizard')"
    echo "=============================================="
    echo ""
    print_info "     http://${display_hostplaceholder:${SERVER_PORTplaceholder"
    echo ""
...
install_version() {
    ...
    if [ "$current_version" = "$target_version" ] || [ "$current_version" = "${target_version#vplaceholder" ]; then
    ...
    if [ -f "$INSTALL_DIR/sub2api" ]; then
        ...
        if [ "$current_version" != "unknown" ] && [ "$current_version" != "not_installed" ]; then
            backup_name="sub2api.backup.${current_versionplaceholder"
        else
...
uninstall() {
    ...
    if ! is_interactive; then
        if [ "${FORCE_YES:-placeholder" != "true" ]; then
    ...
    if [ "${PURGE:-placeholder" = "true" ]; then
        remove_config=true
...
main() {
    ...
    set -- "${positional_args[@]placeholder"
    ...
    case "${1:-placeholder" in
        ...
        rollback)
            ...
            if [ -z "$target_version" ] && [ -n "${2:-placeholder" ]; then
                target_version="$2"
            fi
            if [ -z "$target_version" ]; then
