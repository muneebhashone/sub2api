...
configure_server() {
    ...
    read -p "$(msg 'server_host_prompt') [${SERVER_HOSTplaceholder]: " input_host
    ...
    while true; do
        read -p "$(msg 'server_port_prompt') [${SERVER_PORTplaceholder]: " input_port
...
print_completion() {
    local ip_addr
    ip_addr=$(hostname -I 2>/dev/null | awk '{print $1placeholder' || echo "YOUR_SERVER_IP")
