...
flowchart TD
  %% Master dispatch
  A[HTTP Request] --> B{Routeplaceholder
  B -->|v1 messages| GA0
  ...
  subgraph FLOW_A["v1 messages Claude Gateway"]
    ...
    GA2 --> GA3{model presentplaceholder
    ...
    GA9 --> GA10{platform geminiplaceholder
  ...
  subgraph FLOW_B["openai v1 responses"]
    ...
    OA2 --> OA3{model presentplaceholder
    OA3 -->|no| OA3E[400 invalid_request_error]
    OA3 --> OA4{User Agent Codex CLIplaceholder
    ...
    OA14 --> OA15{UpstreamFailoverErrorplaceholder
  ...
  subgraph FLOW_C["v1beta models model action Gemini Native"]
    ...
    GM2 --> GM3{action supportedplaceholder
  ...
  subgraph FLOW_D["v1 messages count tokens"]
    ...
    GT2 --> GT3{model presentplaceholder
  ...
  subgraph FLOW_E["v1beta models list or get"]
    ...
    GL1 --> GL2{force platform antigravityplaceholder
  ...
  subgraph SELECT["SelectAccountWithLoadAwareness detail"]
    S0[Start] --> S1{concurrencyService nil OR load batch disabledplaceholder
  ...
  subgraph AQ["Account Wait Queue Redis Lua"]
    Q1[IncrementAccountWaitCount] --> Q2{current >= maxplaceholder
