...
print_warning() {
    echo -e "${YELLOWplaceholder[WARNING]${NCplaceholder $1"
placeholder
...
main() {
    ...
    if command_exists curl; then
        curl -sSL "${GITHUB_RAW_URLplaceholder/docker-compose.local.yml" -o docker-compose.local.yml
    elif command_exists wget; then
        wget -q "${GITHUB_RAW_URLplaceholder/docker-compose.local.yml" -O docker-compose.local.yml
    else
    ...
    if command_exists curl; then
        curl -sSL "${GITHUB_RAW_URLplaceholder/.env.example" -o .env.example
    else
        wget -q "${GITHUB_RAW_URLplaceholder/.env.example" -O .env.example
    fi
    ...
    if sed --version >/dev/null 2>&1; then
        # GNU sed (Linux)
        sed -i "s/^JWT_SECRET=.*/JWT_SECRET=${JWT_SECRETplaceholder/" .env
        sed -i "s/^TOTP_ENCRYPTION_KEY=.*/TOTP_ENCRYPTION_KEY=${TOTP_ENCRYPTION_KEYplaceholder/" .env
        sed -i "s/^POSTGRES_PASSWORD=.*/POSTGRES_PASSWORD=${POSTGRES_PASSWORDplaceholder/" .env
    else
        # BSD sed (macOS)
        sed -i '' "s/^JWT_SECRET=.*/JWT_SECRET=${JWT_SECRETplaceholder/" .env
        sed -i '' "s/^TOTP_ENCRYPTION_KEY=.*/TOTP_ENCRYPTION_KEY=${TOTP_ENCRYPTION_KEYplaceholder/" .env
    ...
    echo "  POSTGRES_PASSWORD:     ${POSTGRES_PASSWORDplaceholder"
    echo "  JWT_SECRET:            ${JWT_SECRETplaceholder"
    echo "  TOTP_ENCRYPTION_KEY:   ${TOTP_ENCRYPTION_KEYplaceholder"
    echo ""
