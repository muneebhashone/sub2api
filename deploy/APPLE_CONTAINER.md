...
The env file uses literal `KEY=value` syntax. Do not use Compose expressions such as `${VALUE:-defaultplaceholder`, and do not quote values unless the quote characters are part of the intended value. `BIND_HOST` must be an IPv4 address, and `SERVER_PORT` must be between 1025 and 65535.
