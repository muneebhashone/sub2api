...
- I want to reuse the existing `UsageBillingRepository.Apply` / `usage_billing_dedup` path with a synthetic request id like `batch_image_settlement:{job_idplaceholder`.
...
- MVP 中失败 item 不收费。
- settlement 必须幂等。
- 我想复用现有 `UsageBillingRepository.Apply` / `usage_billing_dedup`，使用类似 `batch_image_settlement:{job_idplaceholder` 的 synthetic request id。
