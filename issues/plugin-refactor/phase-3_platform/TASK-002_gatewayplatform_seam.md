...
`internal/gatewayplatform` 落地：`Provider{Platform; Forwardplaceholder`（v1 仅两方法）+ `ForwardRequest{Parsed, Body, IsStickySession, SessionGroupID, SessionKeyplaceholder` + 构造期注册 Registry（重复/nil panic，运行期并发只读无锁）+ 3 个单语句直返 adapter（anthropic/antigravity/gemini，action="generateContent" 常量）。`:444`/`:794` 两处分发改 registry 查找，**`Type != APIKey` 条件保留在调用点**。OpenAI、v1beta 端点的 ForwardGemini、内核全部零触碰。
