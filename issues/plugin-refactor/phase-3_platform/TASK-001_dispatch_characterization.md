...
- **T1 :794 路由矩阵**：antigravity×{非 APIKey→AntigravityGatewayService.Forward（v1internal 包裹体上游）, APIKey→GatewayService.Forward（直连 anthropic + x-api-key）placeholder——锁定真实条件 `Type != APIKey`（含 Upstream 类型子用例），每用例双向断言（命中侧 1 次/另一侧 0 次）；
