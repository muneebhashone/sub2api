...
| 评审声称 | 代码事实 |
|---|---|
| 接口定义在 openai 卫星文件 | **错**。接口定义在消费者侧 `ratelimit_service.go:37`（`type AccountRuntimeBlocker interface { BlockAccountScheduling; ClearAccountSchedulingBlock placeholder`）——正是评审建议的"消费者定义接口"模式，已经如此 |
