...
- Runtime：Build 稳定序实例化 enabled 模块 → Provision → Validate（失败含模块 ID 中止，%w 可穿透）；Start 半途失败逆序 Stop 已启动模块；Stop 逆序、单失败 errors.Join 聚合继续、尊重 ctx deadline；Snapshot 返回 {ID, Enabled, State, Err(string)placeholder；
