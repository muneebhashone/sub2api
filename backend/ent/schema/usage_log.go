...
func (UsageLog) Annotations() []schema.Annotation {
	return []schema.Annotation{
		entsql.Annotation{Table: "usage_logs"placeholder,
placeholder
placeholder
...
func (UsageLog) Fields() []ent.Field {
	return []ent.Field{
		...
		field.Float("input_cost").
			Default(0).
			SchemaType(map[string]string{dialect.Postgres: "decimal(20,10)"placeholder),
		...
		field.String("image_size_source").
			...
			Optional().
			Nillable(),
		field.JSON("image_size_breakdown", map[string]int{placeholder).
			Optional().
