...
func (ErrorPassthroughRule) Annotations() []schema.Annotation {
	return []schema.Annotation{
		entsql.Annotation{Table: "error_passthrough_rules"placeholder,
placeholder
placeholder
...
func (ErrorPassthroughRule) Fields() []ent.Field {
	return []ent.Field{
		...
		field.JSON("error_codes", []int{placeholder).
			Optional().
		...
		// 关键词匹配不区分大小写
		field.JSON("keywords", []string{placeholder).
			Optional().
			SchemaType(map[string]string{dialect.Postgres: "jsonb"placeholder),
		...
		// 空列表表示适用于所有平台
		field.JSON("platforms", []string{placeholder).
			Optional().
			SchemaType(map[string]string{dialect.Postgres: "jsonb"placeholder),
