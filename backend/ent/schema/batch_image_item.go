...
func (BatchImageItem) Annotations() []schema.Annotation {
	return []schema.Annotation{
		entsql.Annotation{Table: "batch_image_items"placeholder,
placeholder
placeholder
...
func (BatchImageItem) Fields() []ent.Field {
	return []ent.Field{
		...
		field.String("prompt_preview").Optional().Nillable().SchemaType(map[string]string{dialect.Postgres: "text"placeholder),
		...
		field.String("error_message").Optional().Nillable().SchemaType(map[string]string{dialect.Postgres: "text"placeholder),
		field.Float("billed_amount").Optional().Nillable().SchemaType(map[string]string{dialect.Postgres: "decimal(20,10)"placeholder),
		field.Time("created_at").Immutable().Default(time.Now).SchemaType(map[string]string{dialect.Postgres: "timestamptz"placeholder),
		field.Time("indexed_at").Optional().Nillable().SchemaType(map[string]string{dialect.Postgres: "timestamptz"placeholder),
placeholder
placeholder
