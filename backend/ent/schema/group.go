...
func (Group) Annotations() []schema.Annotation {
	return []schema.Annotation{
		entsql.Annotation{Table: "groups"placeholder,
placeholder
placeholder
...
func (Group) Fields() []ent.Field {
	return []ent.Field{
		...
		field.Float("video_price_1080p").
			...
			Nillable().
			SchemaType(map[string]string{dialect.Postgres: "decimal(20,8)"placeholder),
		field.JSON("video_model_prices", map[string]map[string]float64{placeholder).
			Optional().
		...
		field.JSON("model_pricing", json.RawMessage{placeholder).
			Optional().
			SchemaType(map[string]string{dialect.Postgres: "jsonb"placeholder).
		...
		field.JSON("model_routing", map[string][]int64{placeholder).
			Optional().
			SchemaType(map[string]string{dialect.Postgres: "jsonb"placeholder).
		...
		field.JSON("supported_model_scopes", []string{placeholder).
			Default([]string{"claude", "gemini_text", "gemini_image"placeholder).
			SchemaType(map[string]string{dialect.Postgres: "jsonb"placeholder).
		...
		field.JSON("messages_dispatch_model_config", domain.OpenAIMessagesDispatchModelConfig{placeholder).
			Default(domain.OpenAIMessagesDispatchModelConfig{placeholder).
			SchemaType(map[string]string{dialect.Postgres: "jsonb"placeholder).
			Comment("OpenAI Messages 调度模型配置：按 Claude 系列/精确模型映射到目标 GPT 模型"),
		field.JSON("models_list_config", domain.GroupModelsListConfig{placeholder).
			Default(domain.GroupModelsListConfig{placeholder).
			SchemaType(map[string]string{dialect.Postgres: "jsonb"placeholder).
		...
		field.JSON("reasoning_effort_mappings", []domain.ReasoningEffortMapping{placeholder).
			Default([]domain.ReasoningEffortMapping{placeholder).
			SchemaType(map[string]string{dialect.Postgres: "jsonb"placeholder).
