...
func (TLSFingerprintProfile) Annotations() []schema.Annotation {
	return []schema.Annotation{
		entsql.Annotation{Table: "tls_fingerprint_profiles"placeholder,
placeholder
placeholder
...
func (TLSFingerprintProfile) Fields() []ent.Field {
	return []ent.Field{
		...
		field.JSON("cipher_suites", []uint16{placeholder).
			Optional().
			SchemaType(map[string]string{dialect.Postgres: "jsonb"placeholder),

		// curves: 椭圆曲线/支持的组列表
		field.JSON("curves", []uint16{placeholder).
			Optional().
			SchemaType(map[string]string{dialect.Postgres: "jsonb"placeholder),

		// point_formats: EC 点格式列表
		field.JSON("point_formats", []uint16{placeholder).
			Optional().
			SchemaType(map[string]string{dialect.Postgres: "jsonb"placeholder),

		// signature_algorithms: 签名算法列表
		field.JSON("signature_algorithms", []uint16{placeholder).
			Optional().
			SchemaType(map[string]string{dialect.Postgres: "jsonb"placeholder),

		// alpn_protocols: ALPN 协议列表（如 ["http/1.1"]）
		field.JSON("alpn_protocols", []string{placeholder).
			Optional().
			SchemaType(map[string]string{dialect.Postgres: "jsonb"placeholder),

		// supported_versions: 支持的 TLS 版本列表（如 [0x0304, 0x0303]）
		field.JSON("supported_versions", []uint16{placeholder).
			Optional().
			SchemaType(map[string]string{dialect.Postgres: "jsonb"placeholder),

		// key_share_groups: Key Share 中发送的曲线组（如 [29] 即 X25519）
		field.JSON("key_share_groups", []uint16{placeholder).
			Optional().
			SchemaType(map[string]string{dialect.Postgres: "jsonb"placeholder),

		// psk_modes: PSK 密钥交换模式（如 [1] 即 psk_dhe_ke）
		field.JSON("psk_modes", []uint16{placeholder).
			Optional().
			SchemaType(map[string]string{dialect.Postgres: "jsonb"placeholder),

		// extensions: TLS 扩展类型 ID 列表，按发送顺序排列
		field.JSON("extensions", []uint16{placeholder).
			Optional().
			SchemaType(map[string]string{dialect.Postgres: "jsonb"placeholder),
placeholder
placeholder
