...
placeholder
	if len(nodes) == 0 {
		return nil, &NotFoundError{promocode.Labelplaceholder
placeholder
	return nodes[0], nil
placeholder
...
placeholder
	...
	case 0:
		return nil, &NotFoundError{promocode.Labelplaceholder
	default:
		return nil, &NotSingularError{promocode.Labelplaceholder
placeholder
placeholder
...
placeholder
	...
	case 0:
		err = &NotFoundError{promocode.Labelplaceholder
	default:
		err = &NotSingularError{promocode.Labelplaceholder
placeholder
	return
placeholder
...
placeholder
	return &PromoCodeQuery{
		config:           _q.config,
		ctx:              _q.ctx.Clone(),
		order:            append([]promocode.OrderOption{placeholder, _q.order...),
		inters:           append([]Interceptor{placeholder, _q.inters...),
		predicates:       append([]predicate.PromoCode{placeholder, _q.predicates...),
...
func (_q *PromoCodeQuery) GroupBy(field string, fields ...string) *PromoCodeGroupBy {
	...
	grbuild := &PromoCodeGroupBy{build: _qplaceholder
...
func (_q *PromoCodeQuery) Select(fields ...string) *PromoCodeSelect {
	...
	sbuild := &PromoCodeSelect{PromoCodeQuery: _qplaceholder
...
placeholder
	_spec.Assign = func(columns []string, values []any) error {
		node := &PromoCode{config: _q.configplaceholder
...
placeholder
	if query := _q.withUsageRecords; query != nil {
		if err := _q.loadUsageRecords(ctx, query, nodes,
			func(n *PromoCode) { n.Edges.UsageRecords = []*PromoCodeUsage{placeholder placeholder,
			func(n *PromoCode, e *PromoCodeUsage) { n.Edges.UsageRecords = append(n.Edges.UsageRecords, e) placeholder); err != nil {
			return nil, err
	placeholder
placeholder
	return nodes, nil
