...
placeholder
	if len(nodes) == 0 {
		return nil, &NotFoundError{paymentorder.Labelplaceholder
placeholder
	return nodes[0], nil
placeholder
...
placeholder
	...
	case 0:
		return nil, &NotFoundError{paymentorder.Labelplaceholder
	default:
		return nil, &NotSingularError{paymentorder.Labelplaceholder
placeholder
placeholder
...
placeholder
	...
	case 0:
		err = &NotFoundError{paymentorder.Labelplaceholder
	default:
		err = &NotSingularError{paymentorder.Labelplaceholder
placeholder
	return
placeholder
...
placeholder
	return &PaymentOrderQuery{
		config:     _q.config,
		ctx:        _q.ctx.Clone(),
		order:      append([]paymentorder.OrderOption{placeholder, _q.order...),
		inters:     append([]Interceptor{placeholder, _q.inters...),
		predicates: append([]predicate.PaymentOrder{placeholder, _q.predicates...),
...
func (_q *PaymentOrderQuery) GroupBy(field string, fields ...string) *PaymentOrderGroupBy {
	...
	grbuild := &PaymentOrderGroupBy{build: _qplaceholder
...
func (_q *PaymentOrderQuery) Select(fields ...string) *PaymentOrderSelect {
	...
	sbuild := &PaymentOrderSelect{PaymentOrderQuery: _qplaceholder
...
func (_q *PaymentOrderQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*PaymentOrder, error) {
	var (
		nodes       = []*PaymentOrder{placeholder
		_spec       = _q.querySpec()
		loadedTypes = [1]bool{
			_q.withUser != nil,
	placeholder
...
placeholder
	_spec.Assign = func(columns []string, values []any) error {
		node := &PaymentOrder{config: _q.configplaceholder
...
placeholder
	if query := _q.withUser; query != nil {
		if err := _q.loadUser(ctx, query, nodes, nil,
			func(n *PaymentOrder, e *User) { n.Edges.User = e placeholder); err != nil {
			return nil, err
	placeholder
placeholder
	return nodes, nil
