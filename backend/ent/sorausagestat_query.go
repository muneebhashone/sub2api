...
placeholder
	if len(nodes) == 0 {
		return nil, &NotFoundError{sorausagestat.Labelplaceholder
placeholder
	return nodes[0], nil
placeholder
...
placeholder
	if len(ids) == 0 {
		err = &NotFoundError{sorausagestat.Labelplaceholder
		return
placeholder
	return ids[0], nil
placeholder
...
placeholder
	...
	case 0:
		return nil, &NotFoundError{sorausagestat.Labelplaceholder
	default:
		return nil, &NotSingularError{sorausagestat.Labelplaceholder
placeholder
placeholder
...
placeholder
	...
	case 0:
		err = &NotFoundError{sorausagestat.Labelplaceholder
	default:
		err = &NotSingularError{sorausagestat.Labelplaceholder
placeholder
	return
placeholder
...
placeholder
	return &SoraUsageStatQuery{
		config:     _q.config,
		ctx:        _q.ctx.Clone(),
		order:      append([]sorausagestat.OrderOption{placeholder, _q.order...),
		inters:     append([]Interceptor{placeholder, _q.inters...),
		predicates: append([]predicate.SoraUsageStat{placeholder, _q.predicates...),
...
func (_q *SoraUsageStatQuery) GroupBy(field string, fields ...string) *SoraUsageStatGroupBy {
	...
	grbuild := &SoraUsageStatGroupBy{build: _qplaceholder
...
func (_q *SoraUsageStatQuery) Select(fields ...string) *SoraUsageStatSelect {
	...
	sbuild := &SoraUsageStatSelect{SoraUsageStatQuery: _qplaceholder
...
func (_q *SoraUsageStatQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*SoraUsageStat, error) {
	var (
		nodes = []*SoraUsageStat{placeholder
		_spec = _q.querySpec()
	)
...
placeholder
	_spec.Assign = func(columns []string, values []any) error {
		node := &SoraUsageStat{config: _q.configplaceholder
