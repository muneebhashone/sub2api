...
placeholder
	if len(nodes) == 0 {
		return nil, &NotFoundError{batchimagejob.Labelplaceholder
placeholder
	return nodes[0], nil
placeholder
...
placeholder
	if len(ids) == 0 {
		err = &NotFoundError{batchimagejob.Labelplaceholder
		return
placeholder
	return ids[0], nil
placeholder
...
placeholder
	...
	case 0:
		return nil, &NotFoundError{batchimagejob.Labelplaceholder
	default:
		return nil, &NotSingularError{batchimagejob.Labelplaceholder
placeholder
placeholder
...
placeholder
	...
	case 0:
		err = &NotFoundError{batchimagejob.Labelplaceholder
	default:
		err = &NotSingularError{batchimagejob.Labelplaceholder
placeholder
	return
placeholder
...
placeholder
	return &BatchImageJobQuery{
		config:     _q.config,
		ctx:        _q.ctx.Clone(),
		order:      append([]batchimagejob.OrderOption{placeholder, _q.order...),
		inters:     append([]Interceptor{placeholder, _q.inters...),
		predicates: append([]predicate.BatchImageJob{placeholder, _q.predicates...),
...
func (_q *BatchImageJobQuery) GroupBy(field string, fields ...string) *BatchImageJobGroupBy {
	...
	grbuild := &BatchImageJobGroupBy{build: _qplaceholder
...
func (_q *BatchImageJobQuery) Select(fields ...string) *BatchImageJobSelect {
	...
	sbuild := &BatchImageJobSelect{BatchImageJobQuery: _qplaceholder
...
func (_q *BatchImageJobQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*BatchImageJob, error) {
	var (
		nodes = []*BatchImageJob{placeholder
		_spec = _q.querySpec()
	)
...
placeholder
	_spec.Assign = func(columns []string, values []any) error {
		node := &BatchImageJob{config: _q.configplaceholder
