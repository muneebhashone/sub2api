...
placeholder
	if _, ok := _c.mutation.FrozenBalance(); !ok {
		return &ValidationError{Name: "frozen_balance", err: errors.New(`ent: missing required field "User.frozen_balance"`)placeholder
placeholder
	if _, ok := _c.mutation.Concurrency(); !ok {
...
placeholder
	if _, ok := _c.mutation.BalanceNotifyThresholdType(); !ok {
		return &ValidationError{Name: "balance_notify_threshold_type", err: errors.New(`ent: missing required field "User.balance_notify_threshold_type"`)placeholder
placeholder
	if _, ok := _c.mutation.BalanceNotifyExtraEmails(); !ok {
...
placeholder
	if _, ok := _c.mutation.TotalRecharged(); !ok {
		return &ValidationError{Name: "total_recharged", err: errors.New(`ent: missing required field "User.total_recharged"`)placeholder
placeholder
	if _, ok := _c.mutation.RpmLimit(); !ok {
		return &ValidationError{Name: "rpm_limit", err: errors.New(`ent: missing required field "User.rpm_limit"`)placeholder
placeholder
	return nil
placeholder
...
placeholder
	if nodes := _c.mutation.PaymentOrdersIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			...
			Columns: []string{user.PaymentOrdersColumnplaceholder,
			Bidi:    false,
			Target: &sqlgraph.EdgeTarget{
...
placeholder
	if nodes := _c.mutation.PlatformQuotasIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			...
			Columns: []string{user.PlatformQuotasColumnplaceholder,
			Bidi:    false,
			Target: &sqlgraph.EdgeTarget{
