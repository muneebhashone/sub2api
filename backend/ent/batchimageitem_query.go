...
placeholder
	if len(nodes) == 0 {
		return nil, &NotFoundError{batchimageitem.Labelplaceholder
placeholder
	return nodes[0], nil
placeholder
...
placeholder
	if len(ids) == 0 {
		err = &NotFoundError{batchimageitem.Labelplaceholder
		return
placeholder
	return ids[0], nil
placeholder
...
placeholder
	...
	case 0:
		return nil, &NotFoundError{batchimageitem.Labelplaceholder
	default:
		return nil, &NotSingularError{batchimageitem.Labelplaceholder
placeholder
placeholder
...
placeholder
	...
	case 0:
		err = &NotFoundError{batchimageitem.Labelplaceholder
	default:
		err = &NotSingularError{batchimageitem.Labelplaceholder
placeholder
	return
placeholder
...
placeholder
	return &BatchImageItemQuery{
		config:     _q.config,
		ctx:        _q.ctx.Clone(),
		order:      append([]batchimageitem.OrderOption{placeholder, _q.order...),
		inters:     append([]Interceptor{placeholder, _q.inters...),
		predicates: append([]predicate.BatchImageItem{placeholder, _q.predicates...),
...
func (_q *BatchImageItemQuery) GroupBy(field string, fields ...string) *BatchImageItemGroupBy {
	...
	grbuild := &BatchImageItemGroupBy{build: _qplaceholder
...
func (_q *BatchImageItemQuery) Select(fields ...string) *BatchImageItemSelect {
	...
	sbuild := &BatchImageItemSelect{BatchImageItemQuery: _qplaceholder
...
func (_q *BatchImageItemQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*BatchImageItem, error) {
	var (
		nodes = []*BatchImageItem{placeholder
		_spec = _q.querySpec()
	)
...
placeholder
	_spec.Assign = func(columns []string, values []any) error {
		node := &BatchImageItem{config: _q.configplaceholder
