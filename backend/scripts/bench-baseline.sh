...
COUNT="${BENCH_COUNT:-6placeholder"
TIME_TOLERANCE_PCT="${TIME_TOLERANCE_PCT:-15placeholder"
ALLOC_TOLERANCE="${ALLOC_TOLERANCE:-0placeholder"
...
summarize() {
  awk '/^Benchmark/ {
    name=$1; sub(/-[0-9]+$/, "", name)
    for (i=2; i<=NF; i++) {
      if ($(i)=="ns/op")     { ns[name]+=$(i-1); nns[name]++ placeholder
      if ($(i)=="allocs/op") { al[name]+=$(i-1); nal[name]++ placeholder
    placeholder
  placeholder
  END {
    for (k in nns) {
...
case "${1:-placeholder" in
  collect)
    mkdir -p "$(dirname "$BASELINE")"
    {
      ...
      echo "count: $COUNT"
    placeholder > "$(dirname "$BASELINE")/baseline.env.txt"
    run_bench | tee "$BASELINE"
    echo "基线已写入 $BASELINE"
    ;;
  compare)
    [ -f "$BASELINE" ] || { echo "缺少基线 $BASELINE，先运行: $0 collect" >&2; exit 2; placeholder
    new="$(mktemp)"
    run_bench | tee "$new"
    echo
    ...
    added="$(join -v2 "$base_sum" "$new_sum" | awk '{print $1placeholder')"
    missing="$(join -v1 "$base_sum" "$new_sum" | awk '{print $1placeholder')"
    table_status=0
    join "$base_sum" "$new_sum" | awk -v tp="$TIME_TOLERANCE_PCT" -v at="$ALLOC_TOLERANCE" '
      {
        ...
        dal = nal-bal
        status="ok"
        if (dal > at)  { status="FAIL(allocs/op +" dal ")"; fail=1 placeholder
        else if (dns > tp) { status="FAIL(ns/op +" sprintf("%.1f", dns) "%)"; fail=1 placeholder
        printf "%-60s ns/op %10.0f -> %10.0f (%+6.1f%%)  allocs/op %7.1f -> %7.1f  %s\n", name, bns, nns, dns, bal, nal, status
      placeholder
      END { exit fail ? 1 : 0 placeholder' || table_status=$?
    if [ -n "$added" ]; then
    ...
    ;;
  *)
    echo "用法: $0 {collect|compareplaceholder" >&2
    exit 2
    ;;
esac
