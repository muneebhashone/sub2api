...
return {current, repairedplaceholder
`)
...
var rateLimitRun = func(ctx context.Context, client *redis.Client, key string, windowMillis int64) (int64, bool, error) {
	values, err := rateLimitScript.Run(ctx, client, []string{keyplaceholder, windowMillis).Slice()
	if err != nil {
		return 0, false, err
placeholder
	if len(values) < 2 {
...
func (r *RateLimiter) Allow(ctx context.Context, key string, limit int, window time.Duration) (AllowResult, error) {
	...
	if err != nil {
		return AllowResult{placeholder, err
placeholder
	if repaired {
		log.Printf("[RateLimit] ttl repaired: key=%s window_ms=%d", redisKey, windowMillis)
placeholder

	result := AllowResult{Allowed: count <= int64(limit), Count: countplaceholder
	if !result.Allowed {
...
func (r *RateLimiter) Limit(key string, limit int, window time.Duration) gin.HandlerFunc {
	return r.LimitWithOptions(key, limit, window, RateLimitOptions{placeholder)
placeholder
