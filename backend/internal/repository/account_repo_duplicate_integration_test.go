...
placeholder
	require.NoError(t, repo.CreateWithAccountGroups(ctx, success, []service.AccountGroup{{GroupID: group.ID, Priority: 37placeholderplaceholder))
	t.Cleanup(func() {
...
placeholder"api_key": "secret"placeholder,
		Extra:       map[string]any{placeholder,
placeholder
	err = repo.CreateWithAccountGroups(ctx, failure, []service.AccountGroup{{GroupID: int64(^uint64(0) >> 1), Priority: 1placeholderplaceholder)
placeholder
