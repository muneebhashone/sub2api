...
func (s *GroupRepoSuite) TestListWithAccountCountSort_AttachesActiveCount() {
	...
	gA := &service.Group{Name: "sort-count-a", Platform: service.PlatformAnthropic, RateMultiplier: 1, Status: service.StatusActive, SubscriptionType: service.SubscriptionTypeStandardplaceholder
	// Group B: 1 total, 1 active
	gB := &service.Group{Name: "sort-count-b", Platform: service.PlatformAnthropic, RateMultiplier: 1, Status: service.StatusActive, SubscriptionType: service.SubscriptionTypeStandardplaceholder
	...
	insertAccount := func(name, status string) int64 {
		...
		s.Require().NoError(scanSingleRow(s.ctx, s.tx,
			...
			[]any{name, service.PlatformAnthropic, service.AccountTypeOAuth, statusplaceholder,
			&id))
		return id
placeholder
...
placeholder, service.PlatformAnthropic, service.StatusActive, "", nil)
	s.Require().NoError(err)
