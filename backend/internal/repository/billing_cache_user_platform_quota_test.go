...
placeholder
	...
	return &billingCache{rdb: rdbplaceholder, mr
placeholder
...
func TestUserPlatformQuotaCache_Delete(t *testing.T) {
	c, _ := newMiniRedisCache(t)
	ctx := context.Background()
	_ = c.SetUserPlatformQuotaCache(ctx, 1, "openai", &service.UserPlatformQuotaCacheEntry{Version: 1placeholder, time.Minute)
