...
placeholder
	...
	placeholder,
		Extra: map[string]any{"large": strings.Repeat("x", 4096)placeholder,
placeholder
...
placeholder
	...
	require.NoError(t, cache.UpdateLastUsed(ctx, map[int64]time.Time{account.ID: latestplaceholder))
...
func TestSchedulerCacheLastUsedSideKeyIsMonotonicAndRequiresAccount(t *testing.T) {
	ctx := context.Background()
	cache := newSchedulerCacheUnit(t)
	account := service.Account{ID: 9202, Platform: service.PlatformGrok, Type: service.AccountTypeOAuthplaceholder
	require.NoError(t, cache.SetAccount(ctx, &account))

	newer := time.Now().UTC().Truncate(time.Millisecond)
	older := newer.Add(-time.Minute)
	require.NoError(t, cache.UpdateLastUsed(ctx, map[int64]time.Time{account.ID: newerplaceholder))
	require.NoError(t, cache.UpdateLastUsed(ctx, map[int64]time.Time{account.ID: olderplaceholder))
...
placeholder
	...
	require.NoError(t, cache.UpdateLastUsed(ctx, map[int64]time.Time{missingID: newerplaceholder))
...
func TestSchedulerCacheUpdateLastUsedChunksLargeBatches(t *testing.T) {
	...
	for i := 0; i < total; i++ {
		id := int64(9300 + i)
		accounts = append(accounts, service.Account{ID: id, Platform: service.PlatformGrokplaceholder)
