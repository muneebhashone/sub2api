...
func (s *UsageLogRepoSuite) TestListWithFilters_SortByModelAsc() {
	user := mustCreateUser(s.T(), s.client, &service.User{Email: "usage-sort@example.com"placeholder)
	apiKey := mustCreateApiKey(s.T(), s.client, &service.APIKey{UserID: user.ID, Key: "sk-usage-sort", Name: "k"placeholder)
	account := mustCreateAccount(s.T(), s.client, &service.Account{Name: "usage-sort-account"placeholder)
...
placeholder
	...
	logs, _, err := s.repo.ListWithFilters(s.ctx, pagination.PaginationParams{
		...
		SortBy:    "model",
		SortOrder: "asc",
placeholder, usagestats.UsageLogFilters{UserID: user.IDplaceholder)
	s.Require().NoError(err)
