...
func TestUserRepository_DeleteUser_AtomicWithAPIKeys(t *testing.T) {
	...
	user := mustCreateUser(t, client, &service.User{placeholder)
	key1 := mustCreateApiKey(t, client, &service.APIKey{UserID: user.ID, Key: fmt.Sprintf("sk-atomic-a-%d", user.ID)placeholder)
	key2 := mustCreateApiKey(t, client, &service.APIKey{UserID: user.ID, Key: fmt.Sprintf("sk-atomic-b-%d", user.ID)placeholder)
...
placeholder)
	...
	listParams := pagination.PaginationParams{Page: 1, PageSize: 10placeholder
	...
	keys, _, err := apiKeyRepo.ListByUserID(ctx, user.ID, listParams, service.APIKeyListFilters{placeholder)
	...
	keysAfter, _, err := apiKeyRepo.ListByUserID(ctx, user.ID, listParams, service.APIKeyListFilters{placeholder)
	require.NoError(t, err, "ListByUserID")
