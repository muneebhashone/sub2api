...
// 格式: umq:{accountIDplaceholder:lock / umq:{accountIDplaceholder:last
const (
	umqKeyPrefix  = "umq:"
...
// 返回 {是否获取成功, 锁预计过期时间毫秒placeholder，让 Go 侧用同一 Redis 时间源更新索引。
...
if cur ~= false then
    ...
    if pttl and pttl > 0 then
        return {0, ms + pttlplaceholder
    end
    return {0, msplaceholder
end
...
local pttl = redis.call('PTTL', KEYS[1])
if pttl == -2 then
    return {-2, 0placeholder
end
if pttl == -1 then
    redis.call('DEL', KEYS[1])
    return {-1, 0placeholder
end
return {1, pttlplaceholder
`)
...
func NewUserMsgQueueCache(rdb *redis.Client) service.UserMsgQueueCache {
	return &userMsgQueueCache{rdb: rdbplaceholder
placeholder

func umqLockKey(accountID int64) string {
	// 格式: umq:{placeholder:lock — 花括号确保 Redis Cluster hash tag 生效
	return umqKeyPrefix + "{" + strconv.FormatInt(accountID, 10) + "placeholder" + umqLockSuffix
placeholder

func umqLastKey(accountID int64) string {
	// 格式: umq:{placeholder:last — 与 lockKey 同一 hash slot
	return umqKeyPrefix + "{" + strconv.FormatInt(accountID, 10) + "placeholder" + umqLastSuffix
placeholder
...
func (c *userMsgQueueCache) AcquireLock(ctx context.Context, accountID int64, requestID string, lockTtlMs int) (bool, error) {
	...
	result, err := acquireLockScript.Run(ctx, c.rdb, []string{keyplaceholder, requestID, lockTtlMs).Result()
	if err != nil {
...
func (c *userMsgQueueCache) ReleaseLock(ctx context.Context, accountID int64, requestID string) (bool, error) {
	...
	result, err := releaseLockScript.Run(ctx, c.rdb, []string{lockKey, lastKeyplaceholder, requestID).Int()
	if err != nil {
...
placeholder
	...
	placeholder
		...
		result, err := reconcileLockScript.Run(ctx, c.rdb, []string{umqLockKey(accountID)placeholder).Result()
