...
func NewGatewayCache(rdb *redis.Client) service.GatewayCache {
	return &gatewayCache{rdb: rdbplaceholder
placeholder
...
// 格式: sticky_session:{groupIDplaceholder:{sessionHashplaceholder
...
// 格式: client_affinity:{groupIDplaceholder:{clientIDplaceholder
...
// 格式: client_affinity_rev:{groupIDplaceholder:{accountIDplaceholder
...
func (c *gatewayCache) GetClientAffinityAccounts(ctx context.Context, groupID int64, clientID string, ttl time.Duration) ([]int64, error) {
	...
	result, err := getAffinityScript.Run(ctx, c.rdb, []string{keyplaceholder, expireThreshold).StringSlice()
	if err != nil {
		if err == redis.Nil {
			return nil, nil
	placeholder
...
func (c *gatewayCache) UpdateClientAffinity(ctx context.Context, groupID int64, clientID string, accountID int64, ttl time.Duration) error {
	...
	return updateAffinityScript.Run(ctx, c.rdb, []string{fwdKey, revKeyplaceholder,
...
func (c *gatewayCache) GetAccountAffinityCountBatch(ctx context.Context, groupID int64, accountIDs []int64, ttl time.Duration) (map[int64]int64, error) {
	if len(accountIDs) == 0 {
		return map[int64]int64{placeholder, nil
placeholder
	...
	for i, accID := range accountIDs {
		...
		cmds[i] = getAffinityCountScript.Run(ctx, pipe, []string{keyplaceholder, expireThreshold)
placeholder
	_, err := pipe.Exec(ctx)
...
placeholder
	...
	for accID, groupIDs := range accountGroups {
		for _, gID := range groupIDs {
			queries = append(queries, queryItem{accountID: accID, groupID: gIDplaceholder)
	placeholder
placeholder
	...
	for i, q := range queries {
		...
		cmds[i] = getAffinityClientsScript.Run(ctx, pipe, []string{keyplaceholder, expireThreshold)
placeholder
	_, err := pipe.Exec(ctx)
	if err != nil && err != redis.Nil {
		return nil, err
placeholder
	...
	seen := make(map[int64]map[string]struct{placeholder, len(accountGroups))
	for i, q := range queries {
	...
	placeholder
		if seen[q.accountID] == nil {
			seen[q.accountID] = make(map[string]struct{placeholder)
	placeholder
		for _, clientID := range clients {
			if _, exists := seen[q.accountID][clientID]; !exists {
				seen[q.accountID][clientID] = struct{placeholder{placeholder
...
placeholder
	...
	for i, gID := range groupIDs {
		...
		cmds[i] = getAffinityClientsWithScoresScript.Run(ctx, pipe, []string{keyplaceholder, expireThreshold)
placeholder
	_, err := pipe.Exec(ctx)
	if err != nil && err != redis.Nil {
		return nil, err
...
placeholder
	...
	for _, gID := range groupIDs {
		...
		clearAccountAffinityScript.Run(ctx, pipe, []string{revKeyplaceholder, gID, accountID)
placeholder
	_, err := pipe.Exec(ctx)
	if err != nil && err != redis.Nil {
		return err
