...
func lockProxyProbeIdentity(ctx context.Context, client *dbent.Client, proxyID int64) (proxyProbeIdentity, error) {
	...
	`, proxyID)
	if err != nil {
		return proxyProbeIdentity{placeholder, err
placeholder
	...
	if !rows.Next() {
		if err := rows.Err(); err != nil {
			return proxyProbeIdentity{placeholder, err
	placeholder
		return proxyProbeIdentity{placeholder, service.ErrProxyNotFound
placeholder
...
func invalidateProxyProbeSnapshots(ctx context.Context, exec sqlExecutor, proxyID int64) ([]int64, error) {
	rows, err := exec.QueryContext(ctx, `
		UPDATE accounts
		SET extra = COALESCE(extra, '{placeholder'::jsonb) - 'upstream_billing_probe', updated_at = NOW()
		WHERE proxy_id = $1
			AND platform = 'openai'
			AND type = 'apikey'
...
func enqueueProxyProbeAccountChanges(ctx context.Context, exec sqlExecutor, accountIDs []int64) error {
	...
	placeholder
		payload := map[string]any{"account_ids": accountIDs[start:end]placeholder
...
func (r *proxyRepository) CountAccountsByProxyID(ctx context.Context, proxyID int64) (int64, error) {
	var count int64
	if err := scanSingleRow(ctx, r.sql, "SELECT COUNT(*) FROM accounts WHERE proxy_id = $1 AND deleted_at IS NULL", []any{proxyIDplaceholder, &count); err != nil {
		return 0, err
placeholder
	return count, nil
placeholder
...
placeholder
	...
	if len(changedAccountIDs) > 0 {
		...
		payload := map[string]any{"account_ids": changedAccountIDsplaceholder
...
func sortedUniqueAccountIDs(accountIDs []int64) []int64 {
	if len(accountIDs) < 2 {
		return accountIDs
placeholder
	sort.Slice(accountIDs, func(i, j int) bool { return accountIDs[i] < accountIDs[j] placeholder)
	write := 1
...
func (r *proxyRepository) CountExpired(ctx context.Context) (int64, error) {
	var c int64
	err := scanSingleRow(ctx, r.sql, `SELECT COUNT(*) FROM proxies WHERE status=$1 AND deleted_at IS NULL`, []any{service.StatusExpiredplaceholder, &c)
	return c, err
placeholder
...
func (r *proxyRepository) CountExpiringSoon(ctx context.Context, now time.Time) (int64, error) {
	...
	err := scanSingleRow(ctx, r.sql, `
		...
		[]any{service.StatusActive, nowplaceholder, &c)
	return c, err
placeholder
