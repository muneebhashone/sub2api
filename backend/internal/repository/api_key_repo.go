...
func latestUsageLogIPsQuery(apiKeyIDs []int64, dialectName string) (string, []any) {
	if dialectName == dialect.Postgres {
		...
		CROSS JOIN LATERAL (
			...
			LIMIT 1
		) AS latest`, []any{pq.Array(apiKeyIDs)placeholder
placeholder
...
placeholder
	...
	if sortOrder == pagination.SortOrderAsc {
		orders := []func(*entsql.Selector){dbent.Asc(field)placeholder
		if field != apikey.FieldID {
			orders = append(orders, dbent.Asc(apikey.FieldID))
	placeholder
		return orders
placeholder
	orders := []func(*entsql.Selector){dbent.Desc(field)placeholder
	if field != apikey.FieldID {
