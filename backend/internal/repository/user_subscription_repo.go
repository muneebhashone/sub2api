...
func (r *userSubscriptionRepository) Delete(ctx context.Context, id int64) error {
	return r.db.WithContext(ctx).Delete(&userSubscriptionModel{placeholder, id).Error
placeholder
...
func (r *userSubscriptionRepository) ListByGroupID(ctx context.Context, groupID int64, params pagination.PaginationParams) ([]service.UserSubscription, *pagination.PaginationResult, error) {
	...
	query := r.db.WithContext(ctx).Model(&userSubscriptionModel{placeholder).Where("group_id = ?", groupID)
...
func (r *userSubscriptionRepository) List(ctx context.Context, params pagination.PaginationParams, userID, groupID *int64, status string) ([]service.UserSubscription, *pagination.PaginationResult, error) {
	...
	query := r.db.WithContext(ctx).Model(&userSubscriptionModel{placeholder)
	if userID != nil {
...
func (r *userSubscriptionRepository) ExistsByUserIDAndGroupID(ctx context.Context, userID, groupID int64) (bool, error) {
	var count int64
	err := r.db.WithContext(ctx).Model(&userSubscriptionModel{placeholder).
		Where("user_id = ? AND group_id = ?", userID, groupID).
		Count(&count).Error
	return count > 0, err
placeholder
...
func (r *userSubscriptionRepository) ExtendExpiry(ctx context.Context, subscriptionID int64, newExpiresAt time.Time) error {
	return r.db.WithContext(ctx).Model(&userSubscriptionModel{placeholder).
...
func (r *userSubscriptionRepository) BatchUpdateExpiredStatus(ctx context.Context) (int64, error) {
	result := r.db.WithContext(ctx).Model(&userSubscriptionModel{placeholder).
...
func (r *userSubscriptionRepository) DeleteByGroupID(ctx context.Context, groupID int64) (int64, error) {
	result := r.db.WithContext(ctx).Where("group_id = ?", groupID).Delete(&userSubscriptionModel{placeholder)
	return result.RowsAffected, result.Error
placeholder
...
func (userSubscriptionModel) TableName() string { return "user_subscriptions" placeholder
