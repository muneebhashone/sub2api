...
func (s *ConcurrencyCacheSuite) TestOpenAIWSIngressAPIKeySlot_ReapsCrashedLeaseWithoutDeletingLiveOtherInstance() {
	...
	require.NoError(s.T(), s.rdb.ZAdd(s.ctx, key,
		redis.Z{Score: float64(now - openAIWSIngressLeaseTTLSeconds - 1), Member: "crashed-instance"placeholder,
		redis.Z{Score: float64(now), Member: "live-other-instance"placeholder,
	).Err())
...
func (s *ConcurrencyCacheSuite) TestAPIKeySlot_TrackReleaseAndBatchCount() {
	...
	counts, err := cache.GetAPIKeyConcurrencyBatch(s.ctx, []int64{apiKeyID, emptyAPIKeyIDplaceholder)
	require.NoError(s.T(), err)
	require.Equal(s.T(), map[int64]int{apiKeyID: 2, emptyAPIKeyID: 0placeholder, counts)
	...
	counts, err = cache.GetAPIKeyConcurrencyBatch(s.ctx, []int64{apiKeyIDplaceholder)
	require.NoError(s.T(), err)
...
func (s *ConcurrencyCacheSuite) TestCleanupStaleProcessSlots() {
	...
	require.NoError(s.T(), s.rdb.ZAdd(s.ctx, accountKey,
		redis.Z{Score: float64(now), Member: "oldproc-1"placeholder,
		redis.Z{Score: float64(now), Member: "keep-1"placeholder,
	).Err())
	require.NoError(s.T(), s.rdb.ZAdd(s.ctx, userKey,
		redis.Z{Score: float64(now), Member: "oldproc-2"placeholder,
		redis.Z{Score: float64(now), Member: "keep-2"placeholder,
	).Err())
	require.NoError(s.T(), s.rdb.ZAdd(s.ctx, unindexedAccountKey,
		redis.Z{Score: float64(now), Member: "oldproc-unindexed"placeholder,
	).Err())
	require.NoError(s.T(), s.rdb.ZAdd(s.ctx, apiKeyKey,
		redis.Z{Score: float64(now), Member: "oldproc-3"placeholder,
		redis.Z{Score: float64(now), Member: "keep-3"placeholder,
	).Err())
...
placeholder).Err())
	...
	require.NoError(s.T(), err)
	require.Equal(s.T(), []string{"keep-1"placeholder, accountMembers)

	userMembers, err := s.rdb.ZRange(s.ctx, userKey, 0, -1).Result()
	require.NoError(s.T(), err)
	require.Equal(s.T(), []string{"keep-2"placeholder, userMembers)

	// API Key 槽位（stats-only）不在启动清理范围内，靠分数裁剪与 key TTL 自愈。
	apiKeyMembers, err := s.rdb.ZRange(s.ctx, apiKeyKey, 0, -1).Result()
	require.NoError(s.T(), err)
	require.ElementsMatch(s.T(), []string{"keep-3", "oldproc-3"placeholder, apiKeyMembers)
	...
	require.NoError(s.T(), err)
	require.Equal(s.T(), []string{"oldproc-unindexed"placeholder, unindexedMembers)
...
func (s *ConcurrencyCacheSuite) TestGetAccountsLoadBatch() {
	...
	accounts := []service.AccountWithConcurrency{
		{ID: account1, MaxConcurrency: 3placeholder,
		{ID: account2, MaxConcurrency: 2placeholder,
		{ID: account3, MaxConcurrency: 1placeholder,
placeholder
...
func (s *ConcurrencyCacheSuite) TestGetAccountsLoadBatch_Empty() {
	...
	loadMap, err := s.cache.GetAccountsLoadBatch(s.ctx, []service.AccountWithConcurrency{placeholder)
	require.NoError(s.T(), err)
...
func (s *ConcurrencyCacheSuite) TestCleanupExpiredAccountSlots() {
	...
	err = s.rdb.ZAdd(s.ctx, slotKey, redis.Z{Score: float64(expiredTime), Member: "req1"placeholder).Err()
	require.NoError(s.T(), err)
	err = s.rdb.ZAdd(s.ctx, slotKey, redis.Z{Score: float64(expiredTime), Member: "req2"placeholder).Err()
	require.NoError(s.T(), err)
...
func (s *ConcurrencyCacheSuite) TestCleanupExpiredAccountSlotKeys() {
	...
	require.NoError(s.T(), s.rdb.ZAdd(s.ctx, accountKeyWithFresh,
		redis.Z{Score: float64(expiredTime), Member: "expired"placeholder,
		redis.Z{Score: float64(now), Member: "fresh"placeholder,
	).Err())
	require.NoError(s.T(), s.rdb.ZAdd(s.ctx, accountKeyExpiredOnly,
		redis.Z{Score: float64(expiredTime), Member: "expired-only"placeholder,
	).Err())
	require.NoError(s.T(), s.rdb.ZAdd(s.ctx, userKey,
		redis.Z{Score: float64(expiredTime), Member: "user-expired"placeholder,
	).Err())
	require.NoError(s.T(), s.rdb.ZAdd(s.ctx, unindexedAccountKey,
		redis.Z{Score: float64(expiredTime), Member: "unindexed-expired"placeholder,
	).Err())
	require.NoError(s.T(), s.rdb.ZAdd(s.ctx, accountActiveIndexKey,
		redis.Z{Score: float64(now), Member: "301"placeholder,
		redis.Z{Score: float64(now), Member: "302"placeholder,
	).Err())
	...
	require.NoError(s.T(), err)
	require.Equal(s.T(), []string{"fresh"placeholder, accountMembers)
	...
	userMembers, err := s.rdb.ZRange(s.ctx, userKey, 0, -1).Result()
	require.NoError(s.T(), err)
	require.Equal(s.T(), []string{"user-expired"placeholder, userMembers)

	unindexedMembers, err := s.rdb.ZRange(s.ctx, unindexedAccountKey, 0, -1).Result()
	require.NoError(s.T(), err)
	require.Equal(s.T(), []string{"unindexed-expired"placeholder, unindexedMembers)
...
func (s *ConcurrencyCacheSuite) TestCleanupExpiredAccountSlotKeys_ReapsUserIndex() {
	...
	require.NoError(s.T(), s.rdb.ZAdd(s.ctx, userActiveIndexKey,
		redis.Z{Score: expiredScore, Member: "401"placeholder,
		redis.Z{Score: expiredScore, Member: "402"placeholder,
		redis.Z{Score: expiredScore, Member: "not-a-user-id"placeholder,
	).Err())
...
placeholder).Err())
	...
	require.NoError(s.T(), err)
	require.Equal(s.T(), []string{"activeproc-1"placeholder, accountMembers)

	userMembers, err := s.rdb.ZRange(s.ctx, userSlotKey, 0, -1).Result()
	require.NoError(s.T(), err)
	require.Equal(s.T(), []string{"activeproc-2"placeholder, userMembers)
...
func (s *ConcurrencyCacheSuite) TestCleanupStaleProcessSlots_DeletesEmptySlotKeys() {
	...
	now, err := s.rawCache.redisUnixSeconds(s.ctx)
	require.NoError(s.T(), err)
	require.NoError(s.T(), s.rdb.ZAdd(s.ctx, accountSlotKey, redis.Z{Score: float64(now), Member: "oldproc-1"placeholder).Err())
	require.NoError(s.T(), s.rdb.Expire(s.ctx, accountSlotKey, testSlotTTL).Err())
	require.NoError(s.T(), s.rdb.ZAdd(s.ctx, accountActiveIndexKey, redis.Z{
		Score:  float64(now + 60),
		Member: strconv.FormatInt(accountID, 10),
