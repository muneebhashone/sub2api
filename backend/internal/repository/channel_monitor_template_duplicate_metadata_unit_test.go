...
placeholder
	...
	monitorIDs := []int64{41, 42placeholder
	...
	mock.ExpectExec(`(?s)UPDATE channel_monitors\s+SET extra_headers = \$1::jsonb \|\| CASE\s+WHEN COALESCE\(extra_headers, '\{\placeholder'::jsonb\) \? \(\$2::text\)\s+THEN jsonb_build_object\(\$2::text, COALESCE\(extra_headers, '\{\placeholder'::jsonb\) -> \(\$2::text\)\)\s+ELSE '\{\placeholder'::jsonb\s+END\s+WHERE template_id = \$3\s+AND id = ANY\(\$4\)\s+AND provider = \$5\s+AND api_mode = \$6`).
		WithArgs(
			`{"User-Agent":"template-client"placeholder`,
...
placeholder
	...
	repo := NewChannelMonitorRequestTemplateRepository(client, db)
	affected, err := repo.ApplyToMonitors(context.Background(), templateID, []int64{41, 42placeholder)
...
func expectChannelMonitorTemplateForApply(mock sqlmock.Sqlmock, templateID int64) {
	...
	placeholder).AddRow(
			...
			service.MonitorAPIModeResponses, "", []byte(`{"User-Agent":"template-client"placeholder`),
