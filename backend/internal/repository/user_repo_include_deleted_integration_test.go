...
func TestUserRepo_ListWithFilters_IncludeDeleted(t *testing.T) {
	...
	active := mustCreateUser(t, client, &service.User{Email: "shared-keyword-active@test.com"placeholder)
	deleted := mustCreateUser(t, client, &service.User{Email: "shared-keyword-deleted@test.com"placeholder)
	require.NoError(t, client.User.DeleteOneID(deleted.ID).Exec(ctx))

	params := pagination.PaginationParams{Page: 1, PageSize: 50, SortBy: "email", SortOrder: "asc"placeholder

	// 默认（不含已删）：只返回活跃用户。
	usersDefault, resDefault, err := repo.ListWithFilters(ctx, params,
		service.UserListFilters{Search: "shared-keyword-"placeholder)
placeholder
	...
	usersAll, resAll, err := repo.ListWithFilters(ctx, params,
		service.UserListFilters{Search: "shared-keyword-", IncludeDeleted: trueplaceholder)
placeholder
...
func TestUserRepo_GetByIDIncludeDeleted(t *testing.T) {
	...
	u := mustCreateUser(t, client, &service.User{Email: "getbyid-deleted@test.com"placeholder)
