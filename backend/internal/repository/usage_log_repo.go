...
func newUsageLogRepositoryWithSQL(client *dbent.Client, sqlq sqlExecutor) *usageLogRepository {
	...
	repo := &usageLogRepository{client: client, sql: sqlqplaceholder
...
placeholder
	...
	if err := scanSingleRow(ctx, sqlq, query, prepared.args, &log.ID, &log.CreatedAt); err != nil {
		if errors.Is(err, sql.ErrNoRows) && prepared.requestID != "" {
			...
			if err := scanSingleRow(ctx, sqlq, selectQuery, []any{prepared.requestID, log.APIKeyIDplaceholder, &log.ID, &log.CreatedAt); err != nil {
				return false, err
		placeholder
...
func (r *usageLogRepository) fillDashboardUsageStatsFromUsageLogs(ctx context.Context, stats *DashboardStats, startUTC, endUTC, todayUTC, now time.Time) error {
	...
	if err := scanSingleRow(
		...
		r.sql,
		combinedStatsQuery,
		[]any{startUTC, endUTC, todayUTC, todayEndplaceholder,
		&stats.TotalRequests,
		&stats.TotalInputTokens,
		&stats.TotalOutputTokens,
		&stats.TotalCacheCreationTokens,
...
placeholder
	...
	activeUsersQuery := `
		...
		FROM scoped
	`
	if err := scanSingleRow(ctx, r.sql, activeUsersQuery, []any{todayUTC, todayEnd, hourStart, hourEndplaceholder, &stats.ActiveUsers, &stats.HourlyActiveUsers); err != nil {
		return err
placeholder
...
func (r *usageLogRepository) GetAccountStatsAggregated(ctx context.Context, accountID int64, startTime, endTime time.Time) (*usagestats.UsageStats, error) {
	...
	if err := scanSingleRow(
		...
		r.sql,
		query,
		[]any{accountID, startTime, endTimeplaceholder,
		&stats.TotalRequests,
		&stats.TotalInputTokens,
		&stats.TotalOutputTokens,
		&stats.TotalCacheTokens,
...
func (r *usageLogRepository) GetModelStatsAggregated(ctx context.Context, modelName string, startTime, endTime time.Time) (*usagestats.UsageStats, error) {
	...
	if err := scanSingleRow(
		...
		r.sql,
		query,
		[]any{modelName, startTime, endTimeplaceholder,
		&stats.TotalRequests,
		&stats.TotalInputTokens,
		&stats.TotalOutputTokens,
		&stats.TotalCacheTokens,
...
placeholder
	...
	for _, accountID := range accountIDs {
		if _, ok := result[accountID]; !ok {
			result[accountID] = &usagestats.AccountStats{placeholder
	placeholder
placeholder
	return result, nil
placeholder
...
placeholder
	...
	for _, accountID := range accountIDs {
		if _, ok := result[accountID]; !ok {
			result[accountID] = service.GeminiUsageTotals{placeholder
	placeholder
placeholder
	return result, nil
placeholder
...
func (r *usageLogRepository) getPerformanceStatsByAPIKey(ctx context.Context, apiKeyID int64) (rpm, tpm int64, err error) {
	...
	args := []any{fiveMinutesAgo, apiKeyIDplaceholder
...
placeholder
	...
	if r.db != nil {
		...
		g.Go(func() error { return runSummary(gctx) placeholder)
		g.Go(func() error { runEndpoints(gctx); return nil placeholder)
		g.Go(func() error { runUpstream(gctx); return nil placeholder)
		g.Go(func() error { runPaths(gctx); return nil placeholder)
...
placeholder
	...
	if endpointErr != nil {
		...
		endpoints = []EndpointStat{placeholder
placeholder
	...
	if upstreamEndpointErr != nil {
		...
		upstreamEndpoints = []EndpointStat{placeholder
placeholder
...
func buildRequestTypeFilterCondition(startArgIndex int, requestType int16) (string, []any) {
	...
	case service.RequestTypeSync:
		return fmt.Sprintf("(request_type = $%d OR (request_type = %d AND stream = FALSE AND openai_ws_mode = FALSE))", startArgIndex, int16(service.RequestTypeUnknown)), []any{requestTypeArgplaceholder
	case service.RequestTypeStream:
		return fmt.Sprintf("(request_type = $%d OR (request_type = %d AND stream = TRUE AND openai_ws_mode = FALSE))", startArgIndex, int16(service.RequestTypeUnknown)), []any{requestTypeArgplaceholder
	case service.RequestTypeWSV2:
		return fmt.Sprintf("(request_type = $%d OR (request_type = %d AND openai_ws_mode = TRUE))", startArgIndex, int16(service.RequestTypeUnknown)), []any{requestTypeArgplaceholder
	default:
		return fmt.Sprintf("request_type = $%d", startArgIndex), []any{requestTypeArgplaceholder
placeholder
placeholder
