...
func (r *usageLogRepository) fillDashboardUsageStatsFromUsageLogs(ctx context.Context, stats *DashboardStats, startUTC, endUTC, todayUTC, now time.Time) error {
	...
	if err := scanSingleRow(
		...
		r.sql,
		totalStatsQuery,
		[]any{startUTC, endUTCplaceholder,
		&stats.TotalRequests,
		&stats.TotalInputTokens,
		&stats.TotalOutputTokens,
		&stats.TotalCacheCreationTokens,
...
placeholder
	...
	if err := scanSingleRow(
		...
		r.sql,
		todayStatsQuery,
		[]any{todayUTC, todayEndplaceholder,
		&stats.TodayRequests,
		&stats.TodayInputTokens,
		&stats.TodayOutputTokens,
		&stats.TodayCacheCreationTokens,
...
placeholder
	...
	activeUsersQuery := `
		...
		WHERE created_at >= $1 AND created_at < $2
	`
	if err := scanSingleRow(ctx, r.sql, activeUsersQuery, []any{todayUTC, todayEndplaceholder, &stats.ActiveUsers); err != nil {
		return err
placeholder
	...
	hourlyActiveQuery := `
		...
		WHERE created_at >= $1 AND created_at < $2
	`
	if err := scanSingleRow(ctx, r.sql, hourlyActiveQuery, []any{hourStart, hourEndplaceholder, &stats.HourlyActiveUsers); err != nil {
		return err
placeholder
