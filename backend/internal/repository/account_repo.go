...
const codexFingerprintSeedCanonicalPattern = "^[0-9a-f]{8placeholder-[0-9a-f]{4placeholder-[0-9a-f]{4placeholder-[0-9a-f]{4placeholder-[0-9a-f]{12placeholder$"
...
func ensureCodexFingerprintSeedSQL(extraExpr string) string {
	return "CASE WHEN platform = 'openai' AND type = 'oauth' THEN " +
		"jsonb_set(" + extraExpr + ", '{codex_fingerprint_seedplaceholder', " +
...
placeholder
	...
	if probeAccount {
		if enabled, ok, err := decodeAccountExtraJSON(currentEnabled); err != nil {
			return nil, err
	placeholder else if value, isBool := enabled.(bool); ok && isBool {
			probeEnabled = value
...
placeholder
	...
	if contextTx != nil {
		client = contextTx.Client()
placeholder else if r.client != nil {
		var txErr error
	...
	result, err := client.ExecContext(ctx, `
		...
		SET
			...
			extra = CASE
				...
				THEN COALESCE(extra, '{placeholder'::jsonb)
					- 'upstream_billing_probe'
				...
				WHEN type = 'apikey'
					AND credentials IS DISTINCT FROM $1::jsonb
				THEN COALESCE(extra, '{placeholder'::jsonb) - 'upstream_billing_probe'
				ELSE extra
			END,
			updated_at = NOW()
...
placeholder
	...
	if sortOrder == pagination.SortOrderDesc {
		return []func(*entsql.Selector){dbent.Desc(field), dbent.Desc(dbaccount.FieldID)placeholder
placeholder
	if defaultOrder {
...
func (r *accountRepository) ListSchedulableCapacityByGroupIDs(ctx context.Context, groupIDs []int64) ([]service.GroupAccountCapacityRow, error) {
	...
	if len(groupIDs) == 0 {
		return []service.GroupAccountCapacityRow{placeholder, nil
placeholder
	if r.sql == nil {
...
placeholder
	...
	rows, err := r.sql.QueryContext(ctx, `
		SELECT
			...
			a.id AS account_id,
			a.concurrency,
			COALESCE(a.extra, '{placeholder'::jsonb)::text AS extra,
...
placeholder
	...
	if len(accountIDs) > 0 {
		...
		payload := map[string]any{"account_ids": accountIDsplaceholder
...
placeholder
	...
	placeholder
placeholder
	extraExpression := "COALESCE(extra, '{placeholder'::jsonb) || $1::jsonb"
...
func (r *accountRepository) updateUpstreamBillingProbeSnapshotInTx(
	...
	payload, err := json.Marshal(map[string]any{service.UpstreamBillingProbeExtraKey: snapshotplaceholder)
	if err != nil {
		return err
placeholder
...
placeholder
	result, err := client.ExecContext(ctx, `
		UPDATE accounts
		SET
			extra = COALESCE(extra, '{placeholder'::jsonb) || $1::jsonb,
			rate_multiplier = CASE
				WHEN $10::numeric IS NOT NULL
					AND extra @> '{"upstream_billing_probe_enabled": trueplaceholder'::jsonb
					AND extra @> '{"upstream_billing_rate_sync_enabled": trueplaceholder'::jsonb
				THEN $10::numeric
...
placeholder
	...
	placeholder
		...
		setClauses = append(setClauses, "credentials = COALESCE(credentials, '{placeholder'::jsonb) || "+credentialPlaceholder+"::jsonb")
...
placeholder
	...
	if len(updates.Extra) > 0 || len(ollamaGroupIdentityChanges) > 0 || ollamaProxyIdentityChanged != "" || updates.EnsureCodexFingerprintSeed {
		extraExpression := "COALESCE(extra, '{placeholder'::jsonb)"
	...
	placeholder else if snapshotIdentityChanged != "" {
...
placeholder
	...
	rows, err := r.sql.QueryContext(ctx, `
		WITH candidates AS (
			SELECT
				id,
				extra #>> '{upstream_billing_probe,statusplaceholder' AS probe_status,
				extra #>> '{upstream_billing_probe,next_probe_atplaceholder' AS next_probe_at
			FROM accounts
			WHERE deleted_at IS NULL
				AND status = 'active'
				AND type = 'apikey'
			...
			SELECT
				...
				probe_status,
				next_probe_at,
				next_probe_at ~ '^[0-9]{4placeholder-[0-9]{2placeholder-[0-9]{2placeholderT[0-9]{2placeholder:[0-9]{2placeholder:[0-9]{2placeholder(\.[0-9]+)?(Z|[+-][0-9]{2placeholder:[0-9]{2placeholder)$' AS rfc3339_shape,
				jsonb_path_query_first_tz(
					...
					'{placeholder'::jsonb,
					true
				) #>> '{placeholder' AS parsed_next_probe_at
			FROM candidates
		), normalized AS (
			SELECT
				id,
...
func (r *accountRepository) ResetQuotaUsed(ctx context.Context, id int64) error {
	_, err := r.sql.ExecContext(ctx,
		`UPDATE accounts SET extra = (
			COALESCE(extra, '{placeholder'::jsonb)
			|| '{"quota_used": 0, "quota_daily_used": 0, "quota_weekly_used": 0placeholder'::jsonb
