...
func (s *UsageLogRepoSuite) TestGetBatchUserUsageStats() {
	...
	stats, err := s.repo.GetBatchUserUsageStats(s.ctx, []int64{user1.ID, user2.IDplaceholder)
...
func (s *UsageLogRepoSuite) TestGetBatchUserUsageStats_Empty() {
	stats, err := s.repo.GetBatchUserUsageStats(s.ctx, []int64{placeholder)
	s.Require().NoError(err)
...
func (s *UsageLogRepoSuite) TestGetBatchApiKeyUsageStats() {
	...
	stats, err := s.repo.GetBatchAPIKeyUsageStats(s.ctx, []int64{apiKey1.ID, apiKey2.IDplaceholder)
	s.Require().NoError(err, "GetBatchAPIKeyUsageStats")
	s.Require().Len(stats, 2)
placeholder

func (s *UsageLogRepoSuite) TestGetBatchApiKeyUsageStats_Empty() {
	stats, err := s.repo.GetBatchAPIKeyUsageStats(s.ctx, []int64{placeholder)
	s.Require().NoError(err)
	s.Require().Empty(stats)
placeholder
