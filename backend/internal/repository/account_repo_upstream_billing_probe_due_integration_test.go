...
placeholder
	...
	placeholder`, nextProbeAt)
		err := scanSingleRow(ctx, tx, `
			...
			RETURNING id
		`, []any{name, platform, accountType, extraplaceholder, &id)
	placeholder
		return id
placeholder
	...
	err = scanSingleRow(ctx, tx, `
		INSERT INTO accounts (name, platform, type, status, extra)
		VALUES ('probe-grok-disabled', 'grok', $1, 'active', '{placeholder'::jsonb)
		RETURNING id
	`, []any{service.AccountTypeAPIKeyplaceholder, &disabledID)
placeholder
