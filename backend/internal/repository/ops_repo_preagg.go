...
usage_agg AS (
  SELECT
    ...
    percentile_cont(0.50) WITHIN GROUP (ORDER BY duration_ms) FILTER (WHERE duration_ms IS NOT NULL) AS placeholder,
...
SELECT
  ...
  ROUND(SUM(placeholder::double precision * success_count) FILTER (WHERE placeholder IS NOT NULL)
    / NULLIF(SUM(success_count) FILTER (WHERE placeholder IS NOT NULL), 0))::int AS placeholder,
...
func (r *opsRepository) GetLatestHourlyBucketStart(ctx context.Context) (time.Time, bool, error) {
	if r == nil || r.db == nil {
		return time.Time{placeholder, false, fmt.Errorf("nil ops repository")
placeholder
