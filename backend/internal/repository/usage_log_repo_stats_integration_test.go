...
func TestUsageLog_UpstreamModelMismatchFilterAndPartialIndex(t *testing.T) {
	...
	user := mustCreateUser(t, client, &service.User{Email: "model-audit@test.com"placeholder)
	apiKey := mustCreateApiKey(t, client, &service.APIKey{UserID: user.ID, Key: "sk-model-audit", Name: "model-audit"placeholder)
	account := mustCreateAccount(t, client, &service.Account{Name: "model-audit-account"placeholder)
	now := time.Now().UTC()
	responseModel := "gpt-5.4"
	for _, mismatch := range []bool{true, falseplaceholder {
...
placeholderplaceholder, stats.Endpoints)
	require.Equal(t, []usagestats.EndpointStat{{
		Endpoint: "unknown", Requests: 1, TotalTokens: 2,
placeholderplaceholder, stats.UpstreamEndpoints)
	require.Equal(t, []usagestats.EndpointStat{{
		Endpoint: "unknown -> unknown", Requests: 1, TotalTokens: 2,
placeholderplaceholder, stats.EndpointPaths)
...
func TestUsageLog_GetStatsWithFilters_AggregatesAndEndpoints(t *testing.T) {
	...
	user := mustCreateUser(t, client, &service.User{Email: "stats@test.com"placeholder)
	apiKey := mustCreateApiKey(t, client, &service.APIKey{UserID: user.ID, Key: "sk-stats-1", Name: "k"placeholder)
	account := mustCreateAccount(t, client, &service.Account{Name: "acc-stats"placeholder)
...
placeholder
	...
	stats, err := repo.GetStatsWithFilters(ctx, usagestats.UsageLogFilters{UserID: user.ID, StartTime: &start, EndTime: &endplaceholder)
placeholder
