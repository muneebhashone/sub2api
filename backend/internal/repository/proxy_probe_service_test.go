...
func (s *ProxyProbeServiceSuite) TestProbeProxy_Success_IPAPI() {
	s.setupProxyServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if strings.Contains(r.RequestURI, "ip-api.com") {
			...
			_, _ = io.WriteString(w, `{"status":"success","query":"1.2.3.4","city":"c","regionName":"r","country":"cc","countryCode":"CC"placeholder`)
			return
	placeholder
		// 其他请求返回错误
...
func (s *ProxyProbeServiceSuite) TestProbeProxy_Success_HTTPBinFallback() {
	...
	placeholder
		...
		if strings.Contains(r.RequestURI, "httpbin.org") {
			w.Header().Set("Content-Type", "application/json")
			_, _ = io.WriteString(w, `{"origin": "5.6.7.8"placeholder`)
			return
	placeholder
		w.WriteHeader(http.StatusServiceUnavailable)
placeholder))
...
func (s *ProxyProbeServiceSuite) TestProbeProxy_ProxyServerClosed() {
	s.setupProxyServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {placeholder))
	s.proxySrv.Close()
...
func (s *ProxyProbeServiceSuite) TestParseIPAPI_Success() {
	body := []byte(`{"status":"success","query":"1.2.3.4","city":"Beijing","regionName":"Beijing","country":"China","countryCode":"CN"placeholder`)
...
func (s *ProxyProbeServiceSuite) TestParseIPAPI_Failure() {
	body := []byte(`{"status":"fail","message":"rate limited"placeholder`)
...
func (s *ProxyProbeServiceSuite) TestParseHTTPBin_Success() {
	body := []byte(`{"origin": "9.8.7.6"placeholder`)
