...
const (
	...
	// Format: concurrency:account:{accountIDplaceholder:{requestIDplaceholder
	accountSlotKeyPrefix = "concurrency:account:"
	// Format: concurrency:user:{userIDplaceholder:{requestIDplaceholder
	userSlotKeyPrefix = "concurrency:user:"
	// Wait queue keeps counter format: concurrency:wait:{userIDplaceholder
...
func NewConcurrencyCache(rdb *redis.Client) ports.ConcurrencyCache {
	return &concurrencyCache{rdb: rdbplaceholder
placeholder
...
func (c *concurrencyCache) AcquireAccountSlot(ctx context.Context, accountID int64, maxConcurrency int, requestID string) (bool, error) {
	...
	result, err := acquireScript.Run(ctx, c.rdb, []string{pattern, slotKeyplaceholder, maxConcurrency, int(slotTTL.Seconds())).Int()
	if err != nil {
		return false, err
placeholder
	return result == 1, nil
...
func (c *concurrencyCache) GetAccountConcurrency(ctx context.Context, accountID int64) (int, error) {
	pattern := accountSlotPattern(accountID)
	result, err := getCountScript.Run(ctx, c.rdb, []string{patternplaceholder).Int()
	if err != nil {
		return 0, err
placeholder
	return result, nil
...
func (c *concurrencyCache) IncrementWaitCount(ctx context.Context, userID int64, maxWait int) (bool, error) {
	...
	result, err := incrementWaitScript.Run(ctx, c.rdb, []string{keyplaceholder, maxWait, int(slotTTL.Seconds())).Int()
	if err != nil {
		return false, err
placeholder
	return result == 1, nil
