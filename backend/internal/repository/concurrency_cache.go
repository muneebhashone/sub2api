...
var (
	...
	// KEYS[1] = concurrency:account:{accountIDplaceholder
	// ARGV[1] = TTL (seconds)
...
placeholder
	...
	args := []any{c.slotTTLSecondsplaceholder
	for _, acc := range accounts {
		args = append(args, acc.ID, acc.MaxConcurrency)
placeholder

	result, err := getAccountsLoadBatchScript.Run(ctx, c.rdb, []string{placeholder, args...).Slice()
	if err != nil {
		return nil, err
placeholder
...
placeholder
	...
	result, err := getUsersLoadBatchScript.Run(ctx, c.rdb, []string{placeholder, args...).Slice()
	if err != nil {
		return nil, err
placeholder
