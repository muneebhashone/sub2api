...
func (c *concurrencyCache) removeActiveIndexMembers(ctx context.Context, indexKey string, members []string) {
	if len(members) == 0 {
		return
placeholder
	args := make([]interface{placeholder, 0, len(members))
	for _, member := range members {
...
func (c *concurrencyCache) AcquireAccountSlot(ctx context.Context, accountID int64, maxConcurrency int, requestID string) (bool, error) {
	...
	result, err := acquireScript.Run(ctx, c.rdb, []string{keyplaceholder, maxConcurrency, c.slotTTLSeconds, requestID).Int()
	if err != nil {
		return false, err
placeholder
	if result == 1 {
...
func (c *concurrencyCache) IncrementWaitCount(ctx context.Context, userID int64, maxWait int) (bool, error) {
	...
	result, err := incrementWaitScript.Run(ctx, c.rdb, []string{keyplaceholder, maxWait, c.waitQueueTTLSeconds).Int()
	if err != nil {
		return false, err
placeholder
	if result == 1 {
...
func (c *concurrencyCache) IncrementAccountWaitCount(ctx context.Context, accountID int64, maxWait int) (bool, error) {
	...
	result, err := incrementAccountWaitScript.Run(ctx, c.rdb, []string{keyplaceholder, maxWait, c.waitQueueTTLSeconds).Int()
	if err != nil {
		return false, err
placeholder
	if result == 1 {
...
func (c *concurrencyCache) cleanupStaleProcessSlotsForIndex(
	...
	placeholder
		...
		if _, err := startupCleanupSlotScript.Run(ctx, c.rdb, []string{slotKeyForID(id)placeholder, activeRequestPrefix, c.slotTTLSeconds).Result(); err != nil {
