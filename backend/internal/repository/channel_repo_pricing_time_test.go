...
const channelModelPricingTimePricingJSON = `{"timezone":"Asia/Shanghai","periods":[{"start_time":"09:00","end_time":"12:00","multiplier":2placeholder]placeholder`
...
placeholder
	...
	return &channelRepository{db: dbplaceholder, mock
placeholder
...
placeholder
	...
	t.Run("create writes JSON", func(t *testing.T) {
		...
		mock.ExpectQuery(regexp.QuoteMeta("INSERT INTO channel_model_pricing (channel_id, platform, models, billing_mode, input_price, output_price, cache_write_price, cache_read_price, fast_multiplier, flex_multiplier, image_input_price, image_output_price, per_request_price, time_pricing)")).
			...
			WillReturnRows(sqlmock.NewRows([]string{"id", "created_at", "updated_at"placeholder).AddRow(int64(11), time.Time{placeholder, time.Time{placeholder))
...
func TestChannelModelPricingTimePricingCreateAndUpdateWriteNullWhenDisabled(t *testing.T) {
	tests := []struct {
		...
		timePricing *service.ChannelTimePricing
placeholder{
		{name: "nil", timePricing: nilplaceholder,
		{name: "empty periods", timePricing: &service.ChannelTimePricing{Timezone: "Asia/Shanghai"placeholderplaceholder,
placeholder
