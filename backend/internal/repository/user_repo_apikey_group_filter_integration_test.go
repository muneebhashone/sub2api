...
func (s *UserRepoAPIKeyGroupFilterSuite) listByAPIKeyGroup(groupID int64) []service.User {
	...
	users, _, err := s.repo.ListWithFilters(
		s.ctx,
		pagination.PaginationParams{Page: 1, PageSize: 50placeholder,
		service.UserListFilters{APIKeyGroupID: groupIDplaceholder,
	)
...
func (s *UserRepoAPIKeyGroupFilterSuite) TestFiltersUsersByAPIKeyGroup() {
	...
	s.Require().Equal([]int64{hit.IDplaceholder, s.ids(s.listByAPIKeyGroup(g.ID)))
placeholder
...
func (s *UserRepoAPIKeyGroupFilterSuite) TestMultipleKeysAnyMatchDedup() {
	...
	s.Require().Equal([]int64{u.IDplaceholder, s.ids(s.listByAPIKeyGroup(g.ID)))
placeholder
...
func (s *UserRepoAPIKeyGroupFilterSuite) TestAPIKeyGroupAndStatusFilter() {
	...
	s.Require().Equal([]int64{active.IDplaceholder, s.ids(users), "only active user with matching key group should match")
placeholder
...
func (s *UserRepoAPIKeyGroupFilterSuite) TestZeroGroupIDNoFilter() {
	...
	users, _, err := s.repo.ListWithFilters(
		s.ctx,
		pagination.PaginationParams{Page: 1, PageSize: 50placeholder,
		service.UserListFilters{APIKeyGroupID: 0placeholder,
	)
	s.Require().NoError(err)
	s.Require().ElementsMatch([]int64{u1.ID, u2.IDplaceholder, s.ids(users))
placeholder
