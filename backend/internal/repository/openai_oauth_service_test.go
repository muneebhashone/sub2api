...
func (s *OpenAIOAuthServiceSuite) TestRefreshToken_FormFields() {
	...
	placeholder
		...
		w.Header().Set("Content-Type", "application/json")
		_, _ = io.WriteString(w, `{"access_token":"at2","refresh_token":"rt2","token_type":"bearer","expires_in":3600placeholder`)
placeholder))
...
placeholder))
	...
	require.Equal(s.T(), []string{openai.ClientID, openai.SoraClientIDplaceholder, seenClientIDs)
placeholder
