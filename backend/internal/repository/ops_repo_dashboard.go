...
placeholder
	...
	if err != nil {
		if isQueryTimeoutErr(err) {
			degraded = true
			duration = service.OpsPercentiles{placeholder
			ttft = service.OpsPercentiles{placeholder
	placeholder else {
			return nil, err
	placeholder
placeholder
...
placeholder
	...
	head := opsDashboardPartial{placeholder
	tail := opsDashboardPartial{placeholder
...
placeholder
	...
	duration := combineApproxPercentiles([]opsPercentileSegment{
		{weight: preagg.successCount, p: preagg.durationplaceholder,
		{weight: head.successCount, p: head.durationplaceholder,
		{weight: tail.successCount, p: tail.durationplaceholder,
placeholder)
	...
	ttft := combineApproxPercentiles([]opsPercentileSegment{
		{weight: preagg.ttftSampleCount, p: preagg.ttftplaceholder,
		{weight: head.ttftSampleCount, p: head.ttftplaceholder,
		{weight: tail.ttftSampleCount, p: tail.ttftplaceholder,
placeholder)
...
placeholder
	if start.IsZero() || end.IsZero() || !start.Before(end) {
		return []opsHourlyMetricsRow{placeholder, nil
placeholder

	where := "bucket_start >= $1 AND bucket_start < $2"
	args := []any{start.UTC(), end.UTC()placeholder
	idx := 3
...
func aggregateHourlyRows(rows []opsHourlyMetricsRow) opsDashboardPartial {
	out := opsDashboardPartial{placeholder
	if len(rows) == 0 {
		return out
placeholder
...
placeholder
	...
	return service.OpsPercentiles{
		P50: weightedInt(func(p service.OpsPercentiles) *int { return p.P50 placeholder),
		P90: weightedInt(func(p service.OpsPercentiles) *int { return p.P90 placeholder),
		P95: maxInt(func(p service.OpsPercentiles) *int { return p.P95 placeholder),
		P99: maxInt(func(p service.OpsPercentiles) *int { return p.P99 placeholder),
		Avg: weightedInt(func(p service.OpsPercentiles) *int { return p.Avg placeholder),
		Max: maxInt(func(p service.OpsPercentiles) *int { return p.Max placeholder),
placeholder
placeholder
...
` + where
	...
	if err := r.db.QueryRowContext(ctx, q, args...).Scan(
		...
		return service.OpsPercentiles{placeholder, service.OpsPercentiles{placeholder, 0, err
placeholder
