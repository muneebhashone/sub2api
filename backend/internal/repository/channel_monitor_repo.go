...
func NewChannelMonitorRepository(client *dbent.Client, db *sql.DB) service.ChannelMonitorRepository {
	return &channelMonitorRepository{client: client, db: dbplaceholder
placeholder
...
placeholder
	...
	for rows.Next() {
		l := &service.ChannelMonitorLatest{placeholder
...
// 集中实现避免 latency / ping 两处重复 if latency.Valid { v := int(...) ... placeholder 模板。
...
func scanMonitorQuota(data []byte) *domain.MonitorQuotaSnapshot {
	if len(data) == 0 {
		return nil
placeholder
	snapshot := &domain.MonitorQuotaSnapshot{placeholder
...
func scanAvailabilityRow(rows interface{ Scan(...any) error placeholder, windowDays int) (*service.ChannelMonitorAvailability, error) {
	row := &service.ChannelMonitorAvailability{WindowDays: windowDaysplaceholder
...
placeholder
	...
	for rows.Next() {
		var monitorID int64
		entry := &service.ChannelMonitorHistoryEntry{placeholder
		var latency, ping sql.NullInt64
