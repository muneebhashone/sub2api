...
placeholder
	...
	stale.Name = "after"
	s.Require().NoError(
		s.repo.Update(s.ctx, stale, service.APIKeyUpdateFields{Name: trueplaceholder),
		"Update",
	)
...
placeholder
	...
	s.Require().NoError(
		s.repo.Update(s.ctx, current, service.APIKeyUpdateFields{QuotaUsed: true, RateLimitUsage: trueplaceholder),
		"Update",
	)
