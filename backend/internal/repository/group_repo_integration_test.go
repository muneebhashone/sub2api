...
placeholder
	...
	placeholder
		s.Require().NoError(scanSingleRow(
			...
			[]any{name, service.PlatformOpenAI, accountType, deletedAtplaceholder,
			&id,
		))
		return id
placeholder
	...
	for _, binding := range []struct {
		accountID int64
		priority  int
placeholder{{oauthID, 37placeholder, {apiKeyID, 8placeholder, {deletedID, 3placeholderplaceholder {
...
placeholder
	...
	err := s.repo.UpdateSortOrders(s.ctx, []service.GroupSortOrderUpdate{
		{ID: g1.ID, SortOrder: 30placeholder,
		{ID: g2.ID, SortOrder: 10placeholder,
		{ID: g3.ID, SortOrder: 20placeholder,
		{ID: g2.ID, SortOrder: 15placeholder, // 重复 ID 应以最后一次为准
placeholder)
	s.Require().NoError(err)
...
placeholder
	...
	err = s.repo.UpdateSortOrders(s.ctx, []service.GroupSortOrderUpdate{
		{ID: g1.ID, SortOrder: 99placeholder,
		{ID: 99999999, SortOrder: 1placeholder,
placeholder)
	s.Require().Error(err)
	s.Require().ErrorIs(err, service.ErrGroupNotFound)
...
placeholder
	...
	insertAccount := func(name, status string, schedulable bool) int64 {
		...
		s.Require().NoError(scanSingleRow(
			...
			[]any{name, service.PlatformAnthropic, service.AccountTypeOAuth, status, schedulableplaceholder,
			&id,
		))
		return id
placeholder
...
placeholder
	...
	groups, _, err := s.repo.ListWithFilters(s.ctx,
		pagination.PaginationParams{Page: 1, PageSize: 100placeholder,
...
placeholder
	...
	s.Require().NoError(scanSingleRow(s.ctx, s.tx,
		"INSERT INTO accounts (name, platform, type) VALUES ($1, $2, $3) RETURNING id",
		[]any{"acc-normal", service.PlatformAnthropic, service.AccountTypeOAuthplaceholder,
		&normalID))
	...
	s.Require().NoError(scanSingleRow(s.ctx, s.tx,
		...
		[]any{"acc-rate-limited", service.PlatformAnthropic, service.AccountTypeOAuthplaceholder,
		&rateLimitedID))
	...
	s.Require().NoError(scanSingleRow(s.ctx, s.tx,
		...
		[]any{"acc-overloaded", service.PlatformAnthropic, service.AccountTypeOAuthplaceholder,
		&overloadedID))
	...
	s.Require().NoError(scanSingleRow(s.ctx, s.tx,
		...
		[]any{"acc-temp-unschedulable", service.PlatformAnthropic, service.AccountTypeOAuthplaceholder,
		&tempUnschedulableID))
	...
	s.Require().NoError(scanSingleRow(s.ctx, s.tx,
		...
		[]any{"acc-expired", service.PlatformAnthropic, service.AccountTypeOAuthplaceholder,
		&expiredID))
