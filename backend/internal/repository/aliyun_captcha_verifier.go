...
placeholder
	...
	response, err := client.VerifyIntelligentCaptchaWithContext(ctx, request, &dara.RuntimeOptions{placeholder)
	if err != nil {
		return nil, normalizeAliyunCaptchaError(err)
placeholder

	result := &service.AliyunCaptchaVerifyResult{placeholder
