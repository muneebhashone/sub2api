...
func TestMaybeReset(t *testing.T) {
	...
	cases := []struct {
		...
		want      float64
placeholder{
		{"nil prev start resets", 10, nil, start, 1.5, placeholder,
		{"different start resets", 10, &other, start, 1.5, placeholder,
		{"same start accumulates", 10, &start, start, 1.5, 1placeholder,
placeholder
	for _, c := range cases {
