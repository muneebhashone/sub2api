...
placeholder"api_key": "sk-kimi", "account_mode": service.AccountModeCodingplaceholder,
placeholder)
	t.Cleanup(func() {
	...
	rows := []*service.ChannelMonitorHistoryRow{
		{
			...
			Quota: &domain.MonitorQuotaSnapshot{
				...
				Tiers: []domain.MonitorQuotaTier{
					{Window: "5h", UsedPercent: 42.5, Used: 17, Limit: 40, ResetAt: now.Add(time.Hour).Format(time.RFC3339)placeholder,
			placeholder,
				FetchedAt: now,
		placeholder,
	placeholder,
...
placeholder
	...
	byModel := map[string]*service.ChannelMonitorHistoryEntry{placeholder
...
placeholder
	...
	// 用户视图聚合：主模型最近一行带快照。
	latest, err := repo.ListLatestForMonitorIDs(ctx, []int64{monitor.IDplaceholder)
placeholder
