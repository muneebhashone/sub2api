...
placeholder
	...
	err := s.repo.Update(s.ctx, key, service.APIKeyUpdateFields{Name: true, Status: trueplaceholder)
...
placeholder
	...
	key.GroupID = nil
	err := s.repo.Update(s.ctx, key, service.APIKeyUpdateFields{GroupID: trueplaceholder)
	s.Require().NoError(err, "Update")
...
func (s *APIKeyRepoSuite) TestListByUserID() {
	...
	keys, page, err := s.repo.ListByUserID(s.ctx, user.ID, pagination.PaginationParams{Page: 1, PageSize: 10placeholder, service.APIKeyListFilters{placeholder)
...
placeholder
	...
	keys, page, err := s.repo.ListByUserID(s.ctx, user.ID, pagination.PaginationParams{Page: 1, PageSize: 2placeholder, service.APIKeyListFilters{placeholder)
	s.Require().NoError(err)
	s.Require().Len(keys, 2)
...
func (s *APIKeyRepoSuite) TestCRUD_Search_ClearGroupID() {
	...
	key.Status = service.StatusDisabled
	key.GroupID = nil
	s.Require().NoError(s.repo.Update(s.ctx, key, service.APIKeyUpdateFields{Name: true, Status: true, GroupID: trueplaceholder), "Update")
...
func (s *APIKeyRepoSuite) TestIncrementQuotaUsedAndGetState() {
	...
	key.Quota = 3
	key.QuotaUsed = 1
	s.Require().NoError(s.repo.Update(s.ctx, key, service.APIKeyUpdateFields{Quota: true, QuotaUsed: trueplaceholder), "Update quota")
...
func (s *APIKeyRepoSuite) TestDeleteWithAudit_RepeatIsIdempotent() {
	...
	key := &service.APIKey{UserID: user.ID, Key: "sk-del-audit-2", Name: "K", Status: service.StatusActiveplaceholder
	s.Require().NoError(s.repo.Create(s.ctx, key))
