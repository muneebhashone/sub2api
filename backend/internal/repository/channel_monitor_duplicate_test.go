...
placeholder
	...
	restored := entToServiceMonitor(&dbent.ChannelMonitor{ExtraHeaders: persistedplaceholder)
	require.Equal(t, "operation-digest", restored.DuplicateOperationID)
	require.Equal(t, map[string]string{"User-Agent": "Codex"placeholder, restored.ExtraHeaders)
