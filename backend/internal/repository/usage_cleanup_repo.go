...
func newUsageCleanupRepositoryWithSQL(client *dbent.Client, sqlq sqlExecutor) *usageCleanupRepository {
	return &usageCleanupRepository{client: client, sql: sqlqplaceholder
placeholder
...
placeholder
	if total == 0 {
		return []service.UsageCleanupTask{placeholder, paginationResultFromTotal(0, params), nil
placeholder
...
placeholder
	var status string
	if err := scanSingleRow(ctx, r.sql, "SELECT status FROM usage_cleanup_tasks WHERE id = $1", []any{taskIDplaceholder, &status); err != nil {
		return "", err
placeholder
	return status, nil
placeholder
...
placeholder
	...
	`
	if err := scanSingleRow(ctx, r.sql, query, []any{task.Status, filtersJSON, task.CreatedBy, task.DeletedRowsplaceholder, &task.ID, &task.CreatedAt, &task.UpdatedAt); err != nil {
		return err
placeholder
	return nil
placeholder
...
placeholder
	if len(row.Filters) > 0 {
		if err := json.Unmarshal(row.Filters, &task.Filters); err != nil {
			return service.UsageCleanupTask{placeholder, fmt.Errorf("parse cleanup filters: %w", err)
	placeholder
placeholder
