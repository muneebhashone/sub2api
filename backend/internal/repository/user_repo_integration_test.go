...
func (s *UserRepoSuite) TestGetByEmail_NormalizesSpacingAndCaseOnPostgres() {
	user := s.mustCreateUser(&service.User{Email: " Legacy@Example.com "placeholder)
...
func (s *UserRepoSuite) TestExistsByEmail_NormalizesSpacingAndCaseOnPostgres() {
	s.mustCreateUser(&service.User{Email: " Legacy@Example.com "placeholder)
...
func (s *UserRepoSuite) TestUpdate() {
	...
	s.Require().NoError(s.repo.Update(s.ctx, got, service.UserUpdateFields{Username: trueplaceholder), "Update")
...
placeholder)
	...
	affected, err := s.repo.BatchUpdateLimits(s.ctx, []int64{user.IDplaceholder, &concurrency, nil)
	s.Require().NoError(err)
	s.Equal(1, affected)
...
placeholder)
	...
	affected, err := s.repo.BatchUpdateLimits(s.ctx, []int64{user.IDplaceholder, &zero, &zero)
	s.Require().NoError(err)
	s.Equal(1, affected)
...
func (s *UserRepoSuite) TestBatchUpdateLimitsIgnoresDeletedUsersAndReturnsAffectedRows() {
	active := s.mustCreateUser(&service.User{Email: "batch-limits-active@test.com", RPMLimit: 10placeholder)
	deleted := s.mustCreateUser(&service.User{Email: "batch-limits-deleted@test.com", RPMLimit: 10placeholder)
	s.Require().NoError(s.client.User.DeleteOneID(deleted.ID).Exec(s.ctx))
	rpmLimit := 45

	affected, err := s.repo.BatchUpdateLimits(s.ctx, []int64{active.ID, deleted.IDplaceholder, nil, &rpmLimit)
	s.Require().NoError(err)
	s.Equal(1, affected)
...
func (s *UserRepoSuite) TestUpdateIgnoresNoRowsFromConflictingEmailIdentityUpsert() {
	user := s.mustCreateUser(&service.User{Email: "update-existing-identity@test.com", Username: "original"placeholder)
	...
	s.Require().NoError(err)
	got.Username = "updated"
	s.Require().NoError(s.repo.Update(s.ctx, got, service.UserUpdateFields{Username: trueplaceholder), "Update should tolerate ON CONFLICT DO NOTHING returning no rows")
...
func (s *UserRepoSuite) TestDeleteRemovesAuthIdentitiesAndChannels() {
	user := s.mustCreateUser(&service.User{Email: "delete-oauth@test.com"placeholder)
...
func (s *UserRepoSuite) TestApplyRedeemBalanceAdjustment_ConcurrentNeverNegative() {
	user := s.mustCreateUser(&service.User{Email: "redeem-bal-concurrent@test.com", Balance: 10placeholder)
...
func (s *UserRepoSuite) TestDeductBalance_AllowsOverdraft() {
	user := s.mustCreateUser(&service.User{Email: "overdraft@test.com", Balance: 5.0placeholder)
...
func (s *UserRepoSuite) TestApplyRedeemConcurrencyAdjustment_ConcurrentNeverNegative() {
	user := s.mustCreateUser(&service.User{Email: "redeem-concurrency-concurrent@test.com", Concurrency: 10placeholder)
