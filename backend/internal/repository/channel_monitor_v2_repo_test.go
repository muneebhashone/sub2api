...
func TestChannelMonitorV2DisplayModelIsPlatformScoped(t *testing.T) {
	cfg := service.ChannelMonitorV2Config{Platforms: []service.ChannelMonitorV2PlatformConfig{
		{Platform: "openai", Enabled: true, Models: []string{"shared", "gpt-5"placeholderplaceholder,
		{Platform: "grok", Enabled: true, Models: []string{"grok-4"placeholderplaceholder,
		// Empty models list must NOT collapse everything into __other__.
		{Platform: "anthropic", Enabled: true, Models: []string{placeholderplaceholder,
placeholderplaceholder
	...
	require.True(t, channelMonitorV2ModelSelected(service.ChannelMonitorV2Filter{Models: []string{service.ChannelMonitorV2OtherModelplaceholderplaceholder, cfg, "grok", "shared"))
placeholder

func TestChannelMonitorV2MatrixDimensionKey(t *testing.T) {
	cfg := service.ChannelMonitorV2Config{Platforms: []service.ChannelMonitorV2PlatformConfig{{Platform: "openai", Enabled: true, Models: []string{"gpt-5"placeholderplaceholderplaceholderplaceholder
	key := channelMonitorV2MatrixDimensionKey(service.ChannelMonitorV2GroupByPlatformGroupModel, cfg, "openai", 7, "gpt-5")
	require.Equal(t, channelMonitorV2MatrixKey{platform: "openai", groupID: 7, model: "gpt-5"placeholder, key)
	key = channelMonitorV2MatrixDimensionKey(service.ChannelMonitorV2GroupByPlatformModel, cfg, "openai", 7, "unlisted")
	require.Equal(t, channelMonitorV2MatrixKey{platform: "openai", model: service.ChannelMonitorV2OtherModelplaceholder, key)
	key = channelMonitorV2MatrixDimensionKey(service.ChannelMonitorV2GroupByPlatform, cfg, "openai", 7, "gpt-5")
	require.Equal(t, channelMonitorV2MatrixKey{platform: "openai"placeholder, key)
placeholder
...
func TestChannelMonitorV2HistogramPercentilesAreMergedFromCounts(t *testing.T) {
	...
	histogram := map[int64]int64{100: 50, 500: 40, 1000: 10placeholder
...
func TestChannelMonitorV2WhereUsesConfiguredScopeAndEmptyFilterMeansAllConfigured(t *testing.T) {
	filter := service.ChannelMonitorV2Filter{Start: time.Unix(1, 0), End: time.Unix(2, 0)placeholder
	cfg := service.ChannelMonitorV2Config{
		Platforms: []service.ChannelMonitorV2PlatformConfig{{Platform: "openai", Enabled: trueplaceholder, {Platform: "grok", Enabled: falseplaceholderplaceholder,
		GroupIDs:  []int64{3, 4placeholder,
placeholder
...
func TestChannelMonitorV2WhereRejectsGroupFilterOutsideConfiguredScope(t *testing.T) {
	filter := service.ChannelMonitorV2Filter{
		Start: time.Unix(1, 0), End: time.Unix(2, 0), GroupIDs: []int64{9placeholder,
placeholder
	cfg := service.ChannelMonitorV2Config{
		Platforms: []service.ChannelMonitorV2PlatformConfig{{Platform: "openai", Enabled: trueplaceholderplaceholder,
		GroupIDs:  []int64{3, 4placeholder,
placeholder
	where, args := channelMonitorV2Where(filter, cfg, "m")
...
func TestChannelMonitorV2UsageSuccessExcludesCyberBillingRows(t *testing.T) {
	for _, query := range []string{channelMonitorV2UsageMetricsSQL, channelMonitorV2UserMetricsSQLplaceholder {
...
func TestChannelMonitorV2RatesUseCoveredWindow(t *testing.T) {
	...
	filter := service.ChannelMonitorV2Filter{Start: start, End: start.Add(24 * time.Hour)placeholder
	coverage := service.ChannelMonitorV2Coverage{CoverageStart: start.Add(6 * time.Hour), DataThrough: start.Add(18 * time.Hour)placeholder
...
func TestChannelMonitorV2HistoryCoverageCompleteIgnoresTrailingLag(t *testing.T) {
	...
	require.False(t, channelMonitorV2HistoryCoverageComplete(time.Time{placeholder, start))
placeholder
...
placeholder
	seen := map[int]time.Duration{placeholder
...
placeholder
	...
	m2 := service.ChannelMonitorV2Metric{RequestCount: 10, ErrorRequests: 2, SuccessRequests: 8, ErrorRate: 0.2, SuccessRate: 0.8placeholder
	...
	m3 := service.ChannelMonitorV2Metric{RequestCount: 10, ErrorRequests: 2, SuccessRequests: 8, ErrorRate: 0.2, SuccessRate: 0.8placeholder
...
func TestChannelMonitorV2CatalogFilterClearsMultiSelectDimensions(t *testing.T) {
	...
	filter := service.ChannelMonitorV2Filter{
		...
		Platforms: []string{"openai"placeholder, GroupIDs: []int64{3placeholder, Models: []string{"gpt-5"placeholder,
placeholder
	...
	cfg := service.ChannelMonitorV2Config{
		Platforms: []service.ChannelMonitorV2PlatformConfig{
			{Platform: "openai", Enabled: trueplaceholder,
			{Platform: "grok", Enabled: trueplaceholder,
	placeholder,
		GroupIDs: []int64{3, 4placeholder,
placeholder
