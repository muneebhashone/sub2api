...
func (r *groupRepository) Delete(ctx context.Context, id int64) error {
	return r.db.WithContext(ctx).Delete(&groupModel{placeholder, id).Error
placeholder
...
func (r *groupRepository) ListWithFilters(ctx context.Context, params pagination.PaginationParams, platform, status string, isExclusive *bool) ([]service.Group, *pagination.PaginationResult, error) {
	...
	db := r.db.WithContext(ctx).Model(&groupModel{placeholder)
...
func (r *groupRepository) ExistsByName(ctx context.Context, name string) (bool, error) {
	var count int64
	err := r.db.WithContext(ctx).Model(&groupModel{placeholder).Where("name = ?", name).Count(&count).Error
	return count > 0, err
placeholder
...
placeholder
	...
	placeholder
		...
		// 5. 删除分组本身（带锁，避免并发写）
		if err := tx.Clauses(clause.Locking{Strength: "UPDATE"placeholder).Delete(&groupModel{placeholder, id).Error; err != nil {
			return err
	placeholder
...
func (groupModel) TableName() string { return "groups" placeholder
