...
func (r *groupRepository) GetByID(ctx context.Context, id int64) (*service.Group, error) {
	...
	if err != nil {
		return nil, err
placeholder
	counts, err := r.loadAccountCounts(ctx, []int64{out.IDplaceholder)
	if err == nil {
		c := counts[out.ID]
...
placeholder
	...
	for i, r := range rows {
		groupIDs[i] = r.ID
		entries = append(entries, sortEntry{id: r.ID, sortOrder: r.SortOrderplaceholder)
placeholder
...
placeholder
	...
	if sortOrder == pagination.SortOrderDesc && sortBy != "" {
		if tieField == "" {
			return []func(*entsql.Selector){dbent.Desc(field)placeholder
	placeholder
		return []func(*entsql.Selector){dbent.Desc(field), dbent.Desc(tieField)placeholder
placeholder
	if defaultOrder {
		return []func(*entsql.Selector){dbent.Asc(group.FieldSortOrder), dbent.Asc(group.FieldID)placeholder
placeholder
	if tieField == "" {
		return []func(*entsql.Selector){dbent.Asc(field)placeholder
placeholder
	return []func(*entsql.Selector){dbent.Asc(field), dbent.Asc(tieField)placeholder
placeholder
...
func (r *groupRepository) GetAccountCount(ctx context.Context, groupID int64) (total int64, active int64, err error) {
	...
	err = scanSingleRow(ctx, r.sql,
		...
		[]any{groupIDplaceholder, &total, &active, &rateLimited)
	return
placeholder
...
placeholder
	...
	if err := scanSingleRow(
		...
		[]any{pq.Array(groupIDs)placeholder,
		&existingCount,
	); err != nil {
		return err
placeholder
