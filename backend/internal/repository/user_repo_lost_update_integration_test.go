...
placeholder)
	...
	s.Require().NoError(
		s.repo.Update(s.ctx, stale, service.UserUpdateFields{Username: trueplaceholder),
		"Update",
	)
...
placeholder)
	...
	s.Require().NoError(
		s.repo.Update(s.ctx, banned, service.UserUpdateFields{Status: trueplaceholder),
		"ban",
	)
...
placeholder)
	...
	affected, err := s.repo.BatchUpdateLimits(s.ctx, []int64{user.IDplaceholder, &concurrency, &rpmLimit)
...
placeholder)
	...
	got, err := s.repo.GetByID(s.ctx, user.ID)
	s.Require().NoError(err, "GetByID after update")
	s.Require().Equal([]int64{group.IDplaceholder, got.AllowedGroups, "granted group must not be reverted")
placeholder

func (s *UserRepoSuite) TestAdjustBalance_AppliesDeltaAndReportsChange() {
	user := s.mustCreateUser(&service.User{Email: "adjust-balance@example.com", Balance: 10placeholder)
...
func (s *UserRepoSuite) TestAdjustBalance_RefusesNegativeResult() {
	user := s.mustCreateUser(&service.User{Email: "adjust-balance-negative@example.com", Balance: 3placeholder)
...
func (s *UserRepoSuite) TestSetBalance_ReplacesValueAndReportsPrevious() {
	user := s.mustCreateUser(&service.User{Email: "set-balance@example.com", Balance: 7placeholder)
...
func (s *UserRepoSuite) TestSetBalance_RejectsNegativeValue() {
	user := s.mustCreateUser(&service.User{Email: "set-balance-negative@example.com", Balance: 7placeholder)
