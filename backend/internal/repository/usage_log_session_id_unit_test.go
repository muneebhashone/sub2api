...
func TestUsageLogInsertQueries_IncludeSessionID(t *testing.T) {
	...
	batchQuery, batchArgs := buildUsageLogBatchInsertQuery([]string{keyplaceholder,
		map[string]usageLogInsertPrepared{key: preparedplaceholder)
	...
	bestEffortQuery, bestEffortArgs := buildUsageLogBestEffortInsertQuery([]usageLogInsertPrepared{preparedplaceholder)
