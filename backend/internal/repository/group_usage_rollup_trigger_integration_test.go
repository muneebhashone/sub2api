...
func TestGroupUsageRollupTriggerInvalidatesCascadedHistoricalDelete(t *testing.T) {
	for _, partitioned := range []bool{false, trueplaceholder {
		name := "ordinary"
		if partitioned {
			name = "partitioned"
	placeholder
...
placeholder
	...
	defer func() { _ = syncTx.Rollback() placeholder()
	var stateID int16
	...
	defer func() { _ = lateTx.Rollback() placeholder()
...
placeholder
	...
	defer func() { _ = insertTx.Rollback() placeholder()
...
placeholder{
		{name: "closed_rollup", closedBefore: "2026-08-14", includeYesterday: trueplaceholder,
		{name: "raw_tail", closedBefore: "2026-08-13", includeYesterday: falseplaceholder,
placeholder
