	...
	for _, tt := range tests {
		...
		placeholder
			...
			mock.ExpectQuery(`(?s)`+regexp.QuoteMeta("SELECT")+`.*`+regexp.QuoteMeta("FOR NO KEY UPDATE")).
				...
				WillReturnRows(sqlmock.NewRows([]string{"identity_unchanged", "enabled", "snapshot"placeholder).
...
placeholder
	...
	mock.ExpectExec(`(?s)UPDATE accounts SET extra = .* WHERE id = ANY\(\$2\) AND deleted_at IS NULL AND platform = \$3 AND type = \$4`).
		WithArgs(sqlmock.AnyArg(), `{27,28placeholder`, service.PlatformOpenAI, service.AccountTypeAPIKey).
		WillReturnResult(sqlmock.NewResult(0, 1))
	mock.ExpectRollback()
...
placeholder
	...
	mock.ExpectQuery(`(?s)`+regexp.QuoteMeta("SELECT")+`.*`+regexp.QuoteMeta("FOR NO KEY UPDATE")).
		WithArgs(int64(27), service.PlatformOpenAI, service.AccountTypeAPIKey, `{"api_key":"sk-test"placeholder`, nil).
		WillReturnRows(sqlmock.NewRows([]string{"identity_unchanged", "enabled", "snapshot"placeholder).
			AddRow(true, []byte(`true`), []byte(`{"status":"ok"placeholder`)))
