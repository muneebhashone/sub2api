...
func (s *UserProfileIdentityRepoSuite) mustCreatePendingAuthSession(key AuthIdentityKey) *dbent.PendingAuthSession {
	...
	session, err := s.client.PendingAuthSession.Create().
		...
		SetUpstreamIdentityClaims(map[string]any{"provider_subject": key.ProviderSubjectplaceholder).
...
func (s *UserProfileIdentityRepoSuite) TestCreateAndLookupCanonicalAndChannelIdentity() {
	...
	placeholder,
		...
		Metadata:        map[string]any{"unionid": "union-123"placeholder,
		ChannelMetadata: map[string]any{"openid": "openid-123"placeholder,
placeholder)
	s.Require().NoError(err)
...
func (s *UserProfileIdentityRepoSuite) TestBindAuthIdentityToUser_IsIdempotentAndRejectsOtherOwners() {
	...
	placeholder,
		Metadata:        map[string]any{"username": "first"placeholder,
		ChannelMetadata: map[string]any{"scope": "read"placeholder,
placeholder)
	s.Require().NoError(err)
	...
	placeholder,
		Channel: &AuthIdentityChannelKey{
			...
			ChannelSubject: "subject-1",
	placeholder,
		Metadata:        map[string]any{"username": "second"placeholder,
		ChannelMetadata: map[string]any{"scope": "write"placeholder,
placeholder)
	s.Require().NoError(err)
...
VALUES
	('wechat_openid_only_requires_remediation', 'u-1', '{"user_id":1placeholder'::jsonb, '2026-04-20T10:00:00Z'),
	('wechat_openid_only_requires_remediation', 'u-2', '{"user_id":2placeholder'::jsonb, '2026-04-20T11:00:00Z'),
	('oidc_synthetic_email_requires_manual_recovery', 'u-3', '{"user_id":3placeholder'::jsonb, '2026-04-20T12:00:00Z')`)
	s.Require().NoError(err)
