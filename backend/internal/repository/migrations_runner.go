...
type migrationChecksumCompatibilityRule struct {
	fileChecksum       string
	acceptedDBChecksum map[string]struct{placeholder
	acceptedChecksums  map[string]struct{placeholder
placeholder
...
var migrationChecksumCompatibilityRules = map[string]migrationChecksumCompatibilityRule{
	...
	"112_add_payment_order_provider_key_snapshot.sql":         newMigrationChecksumCompatibilityRule("b75f8f56d39455682787696a3d92ad25b055444ca328fb7fca9a460a15d68d99", "placeholder"),
...
placeholder
	defer func() { _ = lockConn.Close() placeholder()
...
func prepareNonTransactionalMigration(ctx context.Context, db migrationConnection, name string) error {
	...
	case usageLogsEffectiveModelIndexesMigration:
		for _, indexName := range []string{usageLogsEffectiveRequestedModelIndex, usageLogsEffectiveUpstreamModelIndexplaceholder {
...
func checksumSet(values ...string) map[string]struct{placeholder {
	out := make(map[string]struct{placeholder, len(values))
	for _, value := range values {
		out[value] = struct{placeholder{placeholder
placeholder
	return out
placeholder
...
func newMigrationChecksumCompatibilityRule(fileChecksum string, acceptedDBChecksums ...string) migrationChecksumCompatibilityRule {
	return migrationChecksumCompatibilityRule{
		...
		acceptedChecksums:  checksumSet(append([]string{fileChecksumplaceholder, acceptedDBChecksums...)...),
placeholder
placeholder
