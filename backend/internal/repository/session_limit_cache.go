...
// - Key: session_limit:account:{accountIDplaceholder
...
const (
	// 会话限制键前缀
	// 格式: session_limit:account:{accountIDplaceholder
	sessionLimitKeyPrefix = "session_limit:account:"

	// 窗口费用缓存键前缀
	// 格式: window_cost:account:{accountIDplaceholder
...
var (
	...
	// KEYS[1] = session_limit:account:{accountIDplaceholder
	// ARGV[1] = maxSessions
...
placeholder
	...
	result, err := registerSessionScript.Run(ctx, c.rdb, []string{keyplaceholder, maxSessions, idleTimeoutSeconds, sessionUUID).Int()
	if err != nil {
...
placeholder
	...
	_, err := refreshSessionScript.Run(ctx, c.rdb, []string{keyplaceholder, idleTimeoutSeconds, sessionUUID).Result()
	return err
placeholder
...
func (c *sessionLimitCache) GetActiveSessionCount(ctx context.Context, accountID int64) (int, error) {
	...
	result, err := getActiveSessionCountScript.Run(ctx, c.rdb, []string{keyplaceholder, idleTimeoutSeconds).Int()
	if err != nil {
		return 0, err
placeholder
	return result, nil
...
placeholder
	...
	for _, accountID := range accountIDs {
		key := sessionLimitKey(accountID)
		cmds[accountID] = getActiveSessionCountScript.Run(ctx, pipe, []string{keyplaceholder, idleTimeoutSeconds)
placeholder
...
placeholder
	...
	result, err := isSessionActiveScript.Run(ctx, c.rdb, []string{keyplaceholder, idleTimeoutSeconds, sessionUUID).Int()
	if err != nil {
		return false, err
placeholder
	return result == 1, nil
