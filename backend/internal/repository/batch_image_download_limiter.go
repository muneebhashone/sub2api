...
placeholder
	...
	ok, err := batchImageDownloadAcquireScript.Run(ctx, l.rdb, []string{keyplaceholder, l.maxActive, int(l.ttl.Seconds())).Int()
	if err != nil {
		return nil, err
placeholder
	if ok != 1 {
		return nil, service.ErrBatchImageDownloadLimited
placeholder
	return &batchImageDownloadPermit{rdb: l.rdb, key: keyplaceholder, nil
placeholder
...
placeholder
	p.once.Do(func() {
		_, p.err = batchImageDownloadReleaseScript.Run(ctx, p.rdb, []string{p.keyplaceholder).Result()
placeholder)
	return p.err
placeholder
