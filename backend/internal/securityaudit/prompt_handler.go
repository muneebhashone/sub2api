...
type PromptAdminHandler struct{ service PromptAdminService placeholder

func NewPromptAdminHandler(service PromptAdminService) *PromptAdminHandler {
	return &PromptAdminHandler{service: serviceplaceholder
placeholder

func (h *PromptAdminHandler) GetConfig(c *gin.Context) { response.Success(c, h.service.GetConfig()) placeholder
...
placeholder
	...
	if err != nil {
		setPromptAdminAudit(c, "failed", infraerrors.Reason(err), map[string]any{"event_id": idplaceholder)
		response.ErrorFrom(c, err)
		return
placeholder
	setPromptAdminAudit(c, "success", "", deleteAuditFields(result, map[string]any{"event_id": idplaceholder))
	LogWarn(EventEventDeleted, map[string]any{"user_id": adminID(c), "event_id": id, "status": "deleted"placeholder)
	response.Success(c, result)
placeholder
...
placeholder
	for _, id := range request.IDs {
		if id <= 0 {
			setPromptAdminAudit(c, "failed", "prompt_audit_invalid_event_id", map[string]any{"requested_count": len(request.IDs)placeholder)
			response.ErrorFrom(c, infraerrors.BadRequest("prompt_audit_invalid_event_id", "事件 ID 无效"))
			return
	placeholder
placeholder
	...
	if err != nil {
		setPromptAdminAudit(c, "failed", infraerrors.Reason(err), map[string]any{"requested_count": len(request.IDs)placeholder)
		response.ErrorFrom(c, err)
		return
placeholder
	setPromptAdminAudit(c, "success", "", deleteAuditFields(result, map[string]any{"requested_count": len(request.IDs)placeholder))
	LogWarn(EventEventsDeleted, map[string]any{"user_id": adminID(c), "status": "deleted"placeholder)
	response.Success(c, result)
placeholder
...
func eventFilterFromQuery(c *gin.Context) (EventFilter, error) {
	...
	if err != nil {
		return EventFilter{placeholder, err
placeholder
...
placeholder
	if value := strings.TrimSpace(c.Query("start_at")); value != "" {
		...
		if filter.StartAt == nil {
			return EventFilter{placeholder, infraerrors.BadRequest("prompt_audit_invalid_time", "开始时间无效")
	placeholder
placeholder
	if value := strings.TrimSpace(c.Query("end_at")); value != "" {
		filter.EndAt = parseTimeQuery(value)
		if filter.EndAt == nil {
			return EventFilter{placeholder, infraerrors.BadRequest("prompt_audit_invalid_time", "结束时间无效")
	placeholder
placeholder
	return filter, nil
placeholder
