...
func TestPromptAuditLogAllowlistAndErrorsDoNotLeakCanarySecrets(t *testing.T) {
	...
	t.Cleanup(func() { slog.SetDefault(previous) placeholder)
...
placeholder
	logGuardFailure(snapshot, ActiveConfig{ConfigVersion: 7placeholder, DecisionUnavailable, ErrorCodeUnavailable, "guard-1", 25*time.Millisecond)
