...
func NewPostgreSQLRepository(db *sql.DB) *PostgreSQLRepository {
	return &PostgreSQLRepository{db: db, clock: realClock{placeholderplaceholder
placeholder
...
placeholder
	tx, err := r.db.BeginTx(ctx, &sql.TxOptions{Isolation: sql.LevelReadCommittedplaceholder)
	if err != nil {
		return nil, err
placeholder
...
func (r *PostgreSQLRepository) QueueStats(ctx context.Context) (QueueStats, error) {
	...
	if err != nil {
		return QueueStats{placeholder, err
placeholder
...
type rowScanner interface{ Scan(...any) error placeholder
