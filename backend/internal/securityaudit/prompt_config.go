...
func DefaultStorageConfig() storageConfig {
	return storageConfig{
		...
		Endpoints:              []StorageEndpoint{placeholder,
		ConfigVersion:          1,
placeholder
placeholder
...
placeholder
	if err := json.Unmarshal([]byte(raw), &cfg); err != nil {
		return storageConfig{placeholder, fmt.Errorf("decode prompt audit config: %w", err)
placeholder
...
placeholder
	seen := make(map[string]struct{placeholder, len(cfg.Endpoints))
	enabled := 0
	...
	placeholder
		seen[ep.ID] = struct{placeholder{placeholder
...
placeholder
	if groupID == nil {
		return false
placeholder
	i := sort.Search(len(cfg.GroupIDs), func(i int) bool { return cfg.GroupIDs[i] >= *groupID placeholder)
...
placeholder
	...
	placeholder)
placeholder
	active := ActiveConfig{RiskControlEnabled: riskControlEnabled, Enabled: cfg.Enabled, BlockingEnabled: cfg.BlockingEnabledplaceholder
	return PublicConfig{
...
placeholder{cfg.Enabled, cfg.BlockingEnabled, cfg.BlockingLatestTurnOnly, cfg.StorePassEvents, len(cfg.Endpoints), len(cfg.Scanners), cfg.AllGroups, len(cfg.GroupIDs), ""placeholder
...
placeholder
	sort.Slice(result, func(i, j int) bool { return result[i] < result[j] placeholder)
	return result
placeholder
