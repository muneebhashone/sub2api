...
placeholder
	...
	placeholder
		...
		for _, text := range texts {
			result = append(result, promptSegment{text: text, user: role == "user", role: roleplaceholder)
	placeholder
placeholder
	return result
placeholder
...
func extractInstructions(value any) []promptSegment {
	...
	case string:
		if text := strings.TrimSpace(typed); text != "" {
			return []promptSegment{{text: text, role: "system"placeholderplaceholder
	placeholder
	case []any:
...
func extractResponses(value any) []promptSegment {
	switch typed := value.(type) {
	case string:
		return []promptSegment{{text: typed, user: true, role: "user"placeholderplaceholder
	case []any:
		result := make([]promptSegment, 0, len(typed))
		for _, item := range typed {
			switch entry := item.(type) {
			case string:
				result = append(result, promptSegment{text: entry, user: true, role: "user"placeholder)
			case map[string]any:
			...
			placeholder
				if content, exists := entry["content"]; exists {
					for _, text := range contentTexts(content) {
						result = append(result, promptSegment{text: text, user: role == "" || role == "user", role: roleplaceholder)
				placeholder
	...
	case map[string]any:
		if parts, ok := typed["parts"].([]any); ok {
			...
			for _, part := range parts {
				if object, ok := part.(map[string]any); ok {
					if text := stringValue(object["text"]); text != "" {
						result = append(result, promptSegment{text: text, role: "system"placeholder)
				placeholder
			placeholder
		placeholder
			return result
	...
	for _, item := range instances {
		if instance, ok := item.(map[string]any); ok {
			if prompt := stringValue(instance["prompt"]); prompt != "" {
				result = append(result, promptSegment{text: prompt, user: true, role: "user"placeholder)
		placeholder
	placeholder
placeholder
	return result
