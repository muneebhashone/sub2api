...
func TestAtomicMetricsExposeCountsLatencyDistributionAndAsyncDelivery(t *testing.T) {
	...
	latencies := []time.Duration{10, 20, 30, 40, 100placeholder
	kinds := []DecisionKind{DecisionAllow, DecisionFlag, DecisionBlock, DecisionUnavailable, DecisionInvalidplaceholder
...
placeholder
	...
	require.Equal(t, AuditMetricsSnapshot{Enqueued: 1, Dropped: 1placeholder, metrics.AuditSnapshot())
placeholder
