...
func TestRuntimeBuildFailureBlocksRetry(t *testing.T) {
	...
	okMod := &fakeModule{id: "job.a", enabledByDefault: true, log: logplaceholder
	badMod := &fakeModule{id: "job.b", enabledByDefault: true, log: log, provisionErr: errors.New("boom")placeholder
	rt := newTestRuntime(t, Config{placeholder, okMod, badMod)
...
func TestRuntimeBuildSuccessThenRebuildStillRejected(t *testing.T) {
	rt := newTestRuntime(t, Config{placeholder, &plainModule{id: "job.ok"placeholder)
...
func TestParseConfigEnabledNilTreatedAsUnset(t *testing.T) {
	cfg, err := ParseConfig(map[string]map[string]any{
		"job.hello": {"enabled": nil, "greeting": "hi"placeholder,
placeholder)
placeholder
	...
	require.True(t, cfg.enabledFor(ModuleInfo{ID: "job.hello", EnabledByDefault: trueplaceholder))
	require.False(t, cfg.enabledFor(ModuleInfo{ID: "job.hello", EnabledByDefault: falseplaceholder))
placeholder
