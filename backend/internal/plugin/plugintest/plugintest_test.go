...
func (m *fakeModule) ModuleInfo() plugin.ModuleInfo {
	return plugin.ModuleInfo{
		...
		New:              func() plugin.Module { return m placeholder,
placeholder
placeholder
...
func (m *fakeModule) Validate() error { m.validates++; return m.validateErr placeholder

func (m *fakeModule) Start(context.Context) error { m.starts++; return m.startErr placeholder

func (m *fakeModule) Stop(context.Context) error { m.stops++; return m.stopErr placeholder
...
func (r *recordingTB) Helper() {placeholder
...
func (r *recordingTB) Cleanup(f func()) { r.cleanups = append(r.cleanups, f) placeholder
...
func TestNewHostDefaults(t *testing.T) {
	...
	cfg := fixtureConfig{Greeting: "default"placeholder
...
func TestWithConfig(t *testing.T) {
	host := plugintest.NewHost(t, plugintest.WithConfig(map[string]map[string]any{
		"job.fixture": {"enabled": true, "greeting": "hi"placeholder,
placeholder))
...
func TestRunLifecycleSuccessAndAutoStop(t *testing.T) {
	m := &fakeModule{id: "job.fixture"placeholder
...
func TestRunLifecycleRawOverridesHostConfig(t *testing.T) {
	m := &fakeModule{id: "job.fixture"placeholder
	host := plugintest.NewHost(t, plugintest.WithConfig(map[string]map[string]any{
		"job.fixture": {"greeting": "from-with-config"placeholder,
placeholder))
	plugintest.RunLifecycle(t, m, host, map[string]map[string]any{
		"job.fixture": {"enabled": true, "greeting": "from-raw"placeholder,
placeholder)
...
func TestRunLifecycleNilHostUsesDefaultHost(t *testing.T) {
	...
	plugintest.RunLifecycle(t, m, nil, map[string]map[string]any{
		"job.fixture": {"enabled": trueplaceholder,
placeholder)
...
func TestRunLifecycleDisabledModuleIsNoop(t *testing.T) {
	m := &fakeModule{id: "job.fixture"placeholder // EnabledByDefault=false 且 raw 未显式 enabled
...
func TestRunLifecycleBuildFailureFatalsWithModuleID(t *testing.T) {
	rec := &recordingTB{TB: tplaceholder
	m := &fakeModule{id: "job.fixture", enabledByDefault: true, provisionErr: errors.New("provision boom")placeholder
	msg := expectFatal(t, rec, func() {
...
func TestRunLifecycleStartFailureFatalsWithModuleID(t *testing.T) {
	rec := &recordingTB{TB: tplaceholder
	m := &fakeModule{id: "job.fixture", enabledByDefault: true, startErr: errors.New("start boom")placeholder
	msg := expectFatal(t, rec, func() {
		plugintest.RunLifecycle(rec, m, nil, nil)
placeholder)
...
func TestCleanupStopFailureReportedWithModuleID(t *testing.T) {
	rec := &recordingTB{TB: tplaceholder
	m := &fakeModule{id: "job.fixture", enabledByDefault: true, stopErr: errors.New("stop boom")placeholder
	plugintest.RunLifecycle(rec, m, nil, nil)
...
func TestBuildExpectingErrorReturnsBuildError(t *testing.T) {
	m := &fakeModule{id: "job.fixture", enabledByDefault: true, validateErr: errors.New("validate boom")placeholder
...
func TestBuildExpectingErrorFatalsOnUnexpectedSuccess(t *testing.T) {
	rec := &recordingTB{TB: tplaceholder
	m := &fakeModule{id: "job.fixture", enabledByDefault: trueplaceholder
	msg := expectFatal(t, rec, func() {
