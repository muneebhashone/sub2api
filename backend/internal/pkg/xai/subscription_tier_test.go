...
func TestSubscriptionTierFromJWTUsesNumericClaim(t *testing.T) {
	...
	require.Equal(t, "supergrok_heavy", SubscriptionTierFromJWT(jwtWithClaims(t, map[string]any{"tier": 5placeholder)))
	require.Equal(t, "free", SubscriptionTierFromJWT(jwtWithClaims(t, map[string]any{"tier": 0placeholder)))
	require.Equal(t, "supergrok_lite", SubscriptionTierFromJWT(jwtWithClaims(t, map[string]any{"tier": 6placeholder)))
	require.Empty(t, SubscriptionTierFromJWT(jwtWithClaims(t, map[string]any{"sub": "user"placeholder)))
...
func TestCanonicalGrokPlanUsesOnlyGrok45ResponsesWindow(t *testing.T) {
	...
	require.Equal(t, "supergrok_heavy", CanonicalGrokPlan(&zero, "SuperGrokPro", &QuotaSnapshot{
		Model:             "grok-4.5",
		Requests:          &QuotaWindow{Limit: &heavyReqplaceholder,
		Tokens:            &QuotaWindow{Limit: &heavyTokplaceholder,
...
placeholder))
	require.Equal(t, "supergrok", CanonicalGrokPlan(&zero, "SuperGrokPro", &QuotaSnapshot{
		Model:             "grok-4.5",
		Requests:          &QuotaWindow{Limit: &superReqplaceholder,
		Tokens:            &QuotaWindow{Limit: &superTokplaceholder,
		LastHeadersSeenAt: fresh,
placeholder))
	require.Equal(t, "supergrok", CanonicalGrokPlan(&zero, "SuperGrokPro", &QuotaSnapshot{
		Model:             "grok-4.5",
...
placeholder
	...
	next := &QuotaSnapshot{
		Model:             "grok-4.6",
		Requests:          &QuotaWindow{Limit: int64Ptr(100)placeholder,
		LastHeadersSeenAt: fresh,
placeholder
