...
func TestValidateTrustedBaseURLAcceptsOfficialRegionalHosts(t *testing.T) {
	for _, raw := range []string{DefaultUSEast1BaseURL, DefaultUSWest2BaseURL, DefaultEUWest1BaseURLplaceholder {
...
func TestDefaultModelMappingIncludesGrokAliases(t *testing.T) {
	...
	t.Cleanup(func() { SetRuntimeModelMappingOptions(original) placeholder)
