...
func TestBuildBillingSummaryMonthlyOnlyKeepsWeeklyUsageEmpty(t *testing.T) {
	t.Parallel()
	payload, err := ParseBillingPayload([]byte(`{"config":{"monthlyLimit":{"val":15000placeholder,"used":{"val":7500placeholder,"billingPeriodStart":"2026-07-01T00:00:00Z","billingPeriodEnd":"2026-08-01T00:00:00Z"placeholderplaceholder`))
placeholder
...
placeholder
	...
	require.Equal(t, []string{"weekly"placeholder, merged.FailedWindows)
	require.Equal(t, []string{"monthly"placeholder, previous.FailedWindows)
placeholder
