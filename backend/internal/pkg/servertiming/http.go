...
type dependencyModuleKey struct{placeholder
...
placeholder
	...
	if module == "" {
		return ctx
placeholder
	return context.WithValue(ctx, dependencyModuleKey{placeholder, module)
placeholder
...
placeholder
	if _, ok := base.(*timingRoundTripper); ok {
		return base
placeholder
	return &timingRoundTripper{base: baseplaceholder
placeholder
...
func dependencyModule(req *http.Request) string {
	if req != nil {
		if module, ok := req.Context().Value(dependencyModuleKey{placeholder).(string); ok && module != "" {
			return module
	placeholder
placeholder
