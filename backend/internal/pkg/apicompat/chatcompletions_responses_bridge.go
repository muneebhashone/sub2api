...
func buildChatMessagesFromItems(messages []ChatMessage, rawItems []json.RawMessage, opts *ResponsesToChatOptions) ([]ChatMessage, toolOutputMediaByCallID, error) {
	...
	invalidFunctionCallIDs := make(map[string]struct{placeholder)
...
placeholder
	...
	placeholder
		...
		placeholder else if opts != nil && opts.ReasoningContentByID != nil {
		...
		placeholder
			...
			if !json.Valid([]byte(arguments)) {
				...
				if callID != "" {
					invalidFunctionCallIDs[callID] = struct{placeholder{placeholder
			placeholder
				pendingReasoning = ""
				continue
		placeholder
		...
		case "tool_search_call":
			// tool_search 调用的 arguments 是 JSON 对象（如 {"query": ...placeholder），
		...
		case "custom_tool_call":
			...
			// 的 {"input": ...placeholder 参数，与请求方向的工具降级（customToolInputSchema）
			// 保持一致，模型才能把历史与当前工具定义对上。
			arguments, _ := json.Marshal(map[string]string{"input": rawString(item["input"])placeholder)
			toolCall := ChatToolCall{
				ID:   rawString(item["call_id"]),
				Type: "function",
				Function: ChatFunctionCall{
	...
	placeholder
		...
		if err != nil {
			return nil, nil, err
	placeholder
		msg := ChatMessage{Role: role, Content: chatContentplaceholder
...
placeholder
	...
	if err := json.Unmarshal(outputRaw, &outputString); err == nil {
		if isToolOutputImageDataURL(outputString) {
			return toolOutputMediaMarker, []ChatContentPart{toolOutputImagePart(outputString)placeholder, true
	placeholder
...
func rewriteToolOutputMediaValue(value any) (any, []ChatContentPart, bool) {
	...
	case map[string]any:
		...
		placeholder, []ChatContentPart{toolOutputImagePart(imageURL)placeholder, true
	placeholder
	...
	for _, m := range messages {
		...
		placeholder
			if len(mediaParts) > 0 {
				...
				out = append(out, ChatMessage{Role: "user", Content: contentplaceholder)
		placeholder
		default:
			out = append(out, m)
	placeholder
...
const customToolInputSchema = `{"type":"object","properties":{"input":{"type":"string","description":"The raw input for this tool, passed through verbatim."placeholderplaceholder,"required":["input"]placeholder`
...
const toolSearchProxySchema = `{"type":"object","properties":{"query":{"type":"string","description":"Search query for tools or connectors to load."placeholder,"limit":{"type":"integer","description":"Maximum number of tool groups to return."placeholderplaceholder,"required":["query"]placeholder`
...
placeholder
	...
	placeholder
		...
		entry := NamespacedToolName{Namespace: tool.Name, Name: child.Nameplaceholder
		if topLevel[flat] {
...
placeholder
	...
	case "x_search":
		if !declared["x_search"] {
			return nil
	placeholder
		out, err := json.Marshal(map[string]any{"type": "x_search"placeholder)
		if err != nil {
			return raw
	placeholder
		return out
...
// 自由文本输入：优先取 {"input": "..."placeholder 的 input 字段；模型未按 schema 输出时原样
// 回传，交由客户端校验、模型重试。
...
func toolSearchCallArgumentsJSON(arguments string) json.RawMessage {
	trimmed := strings.TrimSpace(arguments)
	if trimmed == "" {
		return json.RawMessage(`{placeholder`)
placeholder
	...
	placeholder
		...
		placeholder
			...
			placeholder
				...
				// producing a doubled, invalid JSON like {"a":1placeholder{"a":1placeholder.
