...
func anthropicUserToChatMessages(raw json.RawMessage) ([]ChatMessage, error) {
	...
	if err := json.Unmarshal(raw, &s); err == nil {
		...
		return []ChatMessage{{Role: "user", Content: contentplaceholderplaceholder, nil
placeholder
	...
	placeholder)
		for _, ip := range imageParts {
			toolResultImageParts = append(toolResultImageParts, ChatContentPart{
				Type:     "image_url",
				ImageURL: &ChatImageURL{URL: ip.ImageURLplaceholder,
		placeholder)
	placeholder
placeholder
	...
	for _, b := range blocks {
		...
		case "text":
			if b.Text != "" {
				...
				parts = append(parts, ChatContentPart{Type: "text", Text: b.Textplaceholder)
		placeholder
		case "image":
			if uri := anthropicImageToDataURI(b.Source); uri != "" {
				...
				parts = append(parts, ChatContentPart{
					Type:     "image_url",
					ImageURL: &ChatImageURL{URL: uriplaceholder,
			placeholder)
		placeholder
	placeholder
placeholder
...
func anthropicAssistantToChatMessages(raw json.RawMessage) ([]ChatMessage, error) {
	...
	if err := json.Unmarshal(raw, &s); err == nil {
		content, _ := json.Marshal(s)
		return []ChatMessage{{Role: "assistant", Content: contentplaceholderplaceholder, nil
placeholder
...
placeholder
	...
	return []ChatMessage{msgplaceholder, nil
placeholder
...
//	{"type":"tool","name":"X"placeholder → {"type":"function","function":{"name":"X"placeholderplaceholder (X declared)
...
placeholder
	...
	if len(out.Content) == 0 {
		out.Content = []AnthropicContentBlock{{Type: "text", Text: ""placeholderplaceholder
placeholder
...
placeholder
	if text != "" || len(message.ToolCalls) == 0 {
		blocks = append(blocks, AnthropicContentBlock{Type: "text", Text: textplaceholder)
placeholder
...
placeholder
	...
	placeholder
placeholder else if !state.toolAnnounced[idx] && toolCall.Function.Name != "" {
...
// block that streamed no argument delta gets a final input_json_delta "{placeholder"
// first — the double-conversion path normalizes empty tool arguments to "{placeholder",
