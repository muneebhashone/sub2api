...
func TestResponsesToChatCompletions_PreservesUpstreamServiceTier(t *testing.T) {
	...
	placeholder
placeholder
		Usage: &ResponsesUsage{InputTokens: 1, OutputTokens: 1placeholder,
placeholder
...
func TestResponsesToChatCompletions_OmitsMissingServiceTier(t *testing.T) {
	resp := &ResponsesResponse{ID: "resp_1", Model: "gpt-5.5", Status: "completed"placeholder
	chat := ResponsesToChatCompletions(resp, "gpt-5.5")
...
func TestResponsesEventToChatChunks_PreservesUpstreamServiceTier(t *testing.T) {
	...
	created := &ResponsesStreamEvent{Type: "response.created"placeholder
	require.NoError(t, json.Unmarshal([]byte(`{"type":"response.created","response":{"id":"resp_s1","model":"gpt-5.5","service_tier":"priority","status":"in_progress"placeholderplaceholder`), created))
...
placeholder
	...
	delta := &ResponsesStreamEvent{Type: "response.output_text.delta", Delta: "hi"placeholder
...
func TestResponsesEventToChatChunks_NoServiceTierStaysClean(t *testing.T) {
	state := NewResponsesEventToChatState()
	created := &ResponsesStreamEvent{Type: "response.created", Response: &ResponsesResponse{ID: "resp", Model: "gpt-5.5"placeholderplaceholder
	chunks := ResponsesEventToChatChunks(created, state)
	require.NotEmpty(t, chunks)
...
func TestResponsesResponse_UnmarshalPreservesServiceTier(t *testing.T) {
	...
	require.NoError(t, json.Unmarshal([]byte(`{"id":"resp_1","object":"response","model":"gpt-5.5","status":"completed","service_tier":"flex","output":[]placeholder`), &resp))
...
func TestChatCompletionsResponseToResponses_PreservesServiceTier(t *testing.T) {
	cc := &ChatCompletionsResponse{
		...
		Choices: []ChatChoice{{
			...
			FinishReason: "stop",
placeholder
		Usage: &ChatUsage{PromptTokens: 1, CompletionTokens: 1, TotalTokens: 2placeholder,
placeholder
...
func TestChatCompletionsChunkToResponsesEvents_PreservesServiceTier(t *testing.T) {
	...
	chunk := &ChatCompletionsChunk{
		...
		Choices: []ChatChunkChoice{{
			Index: 0,
			Delta: ChatDelta{Content: strPtr("hi")placeholder,
placeholder
placeholder
...
func TestChatCompletionsChunkToResponsesEvents_NoTierStaysClean(t *testing.T) {
	state := NewChatCompletionsToResponsesStreamState("gpt-5.5")
	chunk := &ChatCompletionsChunk{ID: "chatcmpl-3", Model: "gpt-5.5"placeholder
	events := ChatCompletionsChunkToResponsesEvents(chunk, state)
	created := findEvent(events, "response.created")
	require.NotNil(t, created)
