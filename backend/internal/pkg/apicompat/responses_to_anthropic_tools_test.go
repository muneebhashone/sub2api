...
placeholder
	...
	assert.JSONEq(t, `{"type":"object","properties":{placeholderplaceholder`, string(tool.InputSchema))
...
func TestResponsesToAnthropic_CustomToolPreservesSchemaParameters(t *testing.T) {
	tools := convertResponsesToAnthropicTools([]ResponsesTool{{
		...
		Parameters:  json.RawMessage(`{"type":"object","properties":{"patch":{"type":"string"placeholderplaceholder,"required":["patch"]placeholder`),
placeholderplaceholder)
	...
	assert.JSONEq(t, `{"patch":{"type":"string"placeholderplaceholder`, string(schema["properties"]))
	...
	tools := convertResponsesToAnthropicTools([]ResponsesTool{
		{
			Type:       "function",
			Name:       "read_file",
			Parameters: json.RawMessage(`{"type":"object","properties":{"path":{"type":"string"placeholderplaceholderplaceholder`),
	placeholder,
		{
			Type: "custom",
			Name: "apply_patch",
...
placeholder)
	...
	assert.JSONEq(t, `{"type":"object","properties":{placeholderplaceholder`, string(tools[1].InputSchema))
...
placeholderplaceholder)
	...
	assert.JSONEq(t, `{"type":"object","properties":{placeholderplaceholder`, string(tools[0].InputSchema))
placeholder
