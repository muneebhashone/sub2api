...
func anthToResHandleContentBlockStop(evt *AnthropicStreamEvent, state *AnthropicEventToResponsesState) []ResponsesStreamEvent {
	...
	case "message":
		...
		state.TextAccum = ""
		state.CurrentContent = append(state.CurrentContent, ResponsesContentPart{Type: "output_text", Text: textplaceholder)
		...
		placeholder),
			makeResponsesEvent(state, "response.content_part.done", &ResponsesStreamEvent{
				...
				ContentIndex: state.ContentIndex,
				ItemID:       state.CurrentItemID,
				Part:         &ResponsesContentPart{Type: "output_text", Text: textplaceholder,
		placeholder),
	placeholder
placeholder
...
placeholder
	...
	// carry its content. Emitting only {type,id,statusplaceholder makes SDK-side
...
placeholder
	...
	case "reasoning":
		if state.CurrentSummary != "" {
			item.Summary = []ResponsesSummary{{Type: "summary_text", Text: state.CurrentSummaryplaceholderplaceholder
	placeholder
placeholder
...
placeholder
	...
	if outputs == nil {
		outputs = []ResponsesOutput{placeholder
placeholder
