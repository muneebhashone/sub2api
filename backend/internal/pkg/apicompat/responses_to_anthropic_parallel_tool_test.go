			...
			Delta: ChatDelta{
				ToolCalls: []ChatToolCall{{
					...
					Function: ChatFunctionCall{
						Name:      "get_weather",
						Arguments: `{"city":"Tokyo"placeholder`,
				placeholder,
		placeholder
		placeholder,
placeholder
...
placeholder
	...
	for _, chunk := range []*ChatCompletionsChunk{chatChunk1, chatChunk2, chatChunk3placeholder {
...
func TestStreamingParallelToolUseSecondToolPackedArgsDone(t *testing.T) {
	...
	ResponsesEventToAnthropicEvents(&ResponsesStreamEvent{
		...
		Response: &ResponsesResponse{ID: "resp_par", Model: "glm-5.2"placeholder,
placeholder, state)
	...
	ResponsesEventToAnthropicEvents(&ResponsesStreamEvent{
		...
		OutputIndex: 0,
		Item:        &ResponsesOutput{Type: "function_call", CallID: "call_a", Name: "tool_a"placeholder,
placeholder, state)
...
placeholder
	...
	ResponsesEventToAnthropicEvents(&ResponsesStreamEvent{
		Type:        "response.output_item.added",
		OutputIndex: 1,
		Item:        &ResponsesOutput{Type: "function_call", CallID: "call_b", Name: "tool_b"placeholder,
placeholder, state)
...
placeholder, state)
	...
	for _, e := range eventsTool2Done {
		if e.Type == "content_block_delta" && e.Index != nil {
			...
			assert.Equal(t, `{"y":2placeholder`, e.Delta.PartialJSON)
	placeholder
		if e.Type == "content_block_stop" && e.Index != nil {
			sawStop = true
			stopIndex = *e.Index
...
func TestStreamingThreeParallelToolsAllPackedDone(t *testing.T) {
	...
	ResponsesEventToAnthropicEvents(&ResponsesStreamEvent{
		Type:     "response.created",
		Response: &ResponsesResponse{ID: "resp_3par", Model: "glm-5.2"placeholder,
placeholder, state)

	// Open three tool blocks at indices 0, 1, 2.
	for i, name := range []string{"tool_a", "tool_b", "tool_c"placeholder {
		ResponsesEventToAnthropicEvents(&ResponsesStreamEvent{
			Type:        "response.output_item.added",
			OutputIndex: i,
			Item:        &ResponsesOutput{Type: "function_call", CallID: "call_" + name, Name: nameplaceholder,
	placeholder, state)
placeholder

	// Track started blocks.
	started := map[int]bool{0: true, 1: true, 2: trueplaceholder

	// All three .done events with packed arguments (no prior delta).
	for i, args := range []string{`{"a":1placeholder`, `{"b":2placeholder`, `{"c":3placeholder`placeholder {
