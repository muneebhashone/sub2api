...
placeholder{
		{
			...
			thinking:    &ThinkingConfig{Type: "enabled"placeholder,
			wantBudget:  -1,
			wantPresent: true,
	placeholder,
		{
			...
			model:       "claude-opus-4-6-thinking",
			thinking:    &ThinkingConfig{Type: "enabled", BudgetTokens: placeholder,
			wantBudget:  1024,
			wantPresent: true,
	placeholder,
		{
			...
			model:       "claude-opus-4-6-thinking",
			thinking:    &ThinkingConfig{Type: "enabled", BudgetTokens: -1placeholder,
			wantBudget:  -1,
			wantPresent: true,
	placeholder,
		{
			...
			model:       "claude-opus-4-6-thinking",
			thinking:    &ThinkingConfig{Type: "adaptive", BudgetTokens: 20000placeholder,
	...
	placeholder,
		{
			...
			thinking:    &ThinkingConfig{Type: "adaptive"placeholder,
			wantBudget:  -1,
			wantPresent: true,
	placeholder,
		{
			...
			model:       "claude-opus-4-6-thinking",
			thinking:    &ThinkingConfig{Type: "disabled", BudgetTokens: placeholder,
			wantBudget:  0,
			wantPresent: false,
	placeholder,
		{
...
placeholder{
		{
			name:   "system array",
			system: json.RawMessage(`[{"type":"text","text":"x-anthropic-billing-header keep"placeholder]`),
	placeholder,
		{
			name:   "system string",
		...
		t.Run(tt.name, func(t *testing.T) {
			claudeReq := &ClaudeRequest{
				...
				Messages: []ClaudeMessage{
					{
						Role:    "user",
						Content: json.RawMessage(`[{"type":"text","text":"hello"placeholder]`),
				placeholder,
			placeholder,
		placeholder
...
placeholder
	...
	placeholder
		...
		placeholder,
			Tools: tools,
	placeholder, "project-1", "gemini-2.5-flash", DefaultTransformOptions())
	placeholder
...
placeholder
	...
	t.Run("mixed builtin and function tools enable server-side tool invocations", func(t *testing.T) {
		req, raw := transform(t, []ClaudeTool{functionTool, webSearchToolplaceholder)
...
placeholder)
	...
	t.Run("function tools only leave the flag unset", func(t *testing.T) {
		req, raw := transform(t, []ClaudeTool{functionToolplaceholder)
...
placeholder)
	...
	t.Run("web search only leaves the flag unset", func(t *testing.T) {
		req, raw := transform(t, []ClaudeTool{webSearchToolplaceholder)
