...
var modelInfoMap = map[string]modelInfo{
	"claude-fable-5":    {DisplayName: "Claude Fable 5", CanonicalID: "claude-fable-5"placeholder,
	"claude-opus-4-8":   {DisplayName: "Claude Opus 4.8", CanonicalID: "claude-opus-4-8"placeholder,
	"claude-opus-4-7":   {DisplayName: "Claude Opus 4.7", CanonicalID: "claude-opus-4-7"placeholder,
...
1) 优先尝试 XML 格式调用：输出 ` + "`<mcp__tool_name>{\"arg\":\"value\"placeholder</mcp__tool_name>`" + `。
	...
	if len(system) > 0 {
		...
		if err := json.Unmarshal(system, &sysStr); err == nil {
			...
			placeholder
				...
				if filtered != "" {
					userSystemParts = append(userSystemParts, GeminiPart{Text: filteredplaceholder)
			placeholder
		placeholder
	placeholder else {
			// 尝试解析为数组
...
placeholder
	...
	if opts.EnableMCPXML && hasMCPTools(tools) {
		parts = append(parts, GeminiPart{Text: mcpXMLProtocolplaceholder)
placeholder
