...
placeholder
	...
	placeholder else if subArr, ok := v.([]any); ok {
...
placeholder else if items, ok := schemaMap["items"]; ok {
		...
		if itemsArr, ok := items.([]any); ok {
			...
			if best == nil {
				// 回退到通用字符串
				best = map[string]any{"type": "string"placeholder
		placeholder
			// 用处理后的对象替换原有数组
...
placeholder
	...
	if typeStr == "" || typeStr == "object" {
		if anyOf, ok := schemaMap["anyOf"].([]any); ok {
			unionArray = anyOf
	placeholder else if oneOf, ok := schemaMap["oneOf"].([]any); ok {
			unionArray = oneOf
	placeholder
placeholder
			...
			if bestMap, ok := bestBranch.(map[string]any); ok {
				...
				for k, v := range bestMap {
					...
					placeholder
						...
						placeholder
					placeholder
				placeholder else if k == "required" {
					...
					placeholder
				placeholder else if _, exists := schemaMap[k]; !exists {
	...
	placeholder
		...
		placeholder
			if !hasProps {
				...
				placeholder,
			placeholder
				schemaMap["required"] = []any{"reason"placeholder
		placeholder
	placeholder
			...
			case []any:
				...
				for _, t := range v {
					if ts, ok := t.(string); ok {
						...
						if lower == "null" {
							isEffectivelyNullable = true
					placeholder else if selectedType == "" {
							selectedType = lower
					placeholder
				placeholder
			placeholder
...
func migrateConstraints(m map[string]any) {
	constraints := []struct {
		...
		label string
placeholder{
		{"minLength", "minLen"placeholder,
		{"maxLength", "maxLen"placeholder,
		{"pattern", "pattern"placeholder,
		{"minimum", "min"placeholder,
		{"maximum", "max"placeholder,
		{"multipleOf", "multipleOf"placeholder,
		{"exclusiveMinimum", "exclMin"placeholder,
		{"exclusiveMaximum", "exclMax"placeholder,
		{"minItems", "minItems"placeholder,
		{"maxItems", "maxItems"placeholder,
		{"propertyNames", "propertyNames"placeholder,
