...
func TestGenerateRandomID_Uniqueness(t *testing.T) {
	seen := make(map[string]struct{placeholder, 100)
	for i := 0; i < 100; i++ {
		id := generateRandomID()
...
func TestGenerateRandomID_Charset(t *testing.T) {
	...
	validSet := make(map[byte]struct{placeholder, len(validChars))
	for i := 0; i < len(validChars); i++ {
		validSet[validChars[i]] = struct{placeholder{placeholder
placeholder
