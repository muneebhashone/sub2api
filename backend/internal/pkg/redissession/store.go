// Package redissession provides a multi-instance OAuth session backend.
package redissession

import (
	"context"
	"encoding/json"
	"errors"
	"strings"
	"time"

	"github.com/redis/go-redis/v9"
)

var ErrNotConfigured = errors.New("redis session store not configured")

// Store persists JSON sessions and single-use markers under one namespace.
type Store struct {
	rdb    *redis.Client
	prefix string
	ttl    time.Duration
placeholder

func New(rdb *redis.Client, prefix string, ttl time.Duration) *Store {
	if ttl <= 0 {
		ttl = 30 * time.Minute
placeholder
	prefix = strings.TrimSpace(prefix)
	if prefix == "" {
		prefix = "oauth:session"
placeholder
	if !strings.HasSuffix(prefix, ":") {
		prefix += ":"
placeholder
	return &Store{rdb: rdb, prefix: prefix, ttl: ttlplaceholder
placeholder

func (s *Store) dataKey(id string) string { return s.prefix + strings.TrimSpace(id) placeholder
func (s *Store) usedKey(id string) string { return s.prefix + "used:" + strings.TrimSpace(id) placeholder

func (s *Store) Set(ctx context.Context, id string, value any) error {
	if s == nil || s.rdb == nil {
		return ErrNotConfigured
placeholder
	id = strings.TrimSpace(id)
	if id == "" {
		return errors.New("session id is required")
placeholder
	if ctx == nil {
		ctx = context.Background()
placeholder
	raw, err := json.Marshal(value)
	if err != nil {
		return err
placeholder
	return s.rdb.Set(ctx, s.dataKey(id), raw, s.ttl).Err()
placeholder

func (s *Store) Get(ctx context.Context, id string, dest any) (bool, error) {
	if s == nil || s.rdb == nil {
		return false, ErrNotConfigured
placeholder
	id = strings.TrimSpace(id)
	if id == "" {
		return false, nil
placeholder
	if ctx == nil {
		ctx = context.Background()
placeholder
	raw, err := s.rdb.Get(ctx, s.dataKey(id)).Bytes()
	if errors.Is(err, redis.Nil) {
		return false, nil
placeholder
	if err != nil {
		return false, err
placeholder
	if err := json.Unmarshal(raw, dest); err != nil {
		return false, err
placeholder
	return true, nil
placeholder

func (s *Store) Delete(ctx context.Context, id string) error {
	if s == nil || s.rdb == nil {
		return ErrNotConfigured
placeholder
	id = strings.TrimSpace(id)
	if id == "" {
		return nil
placeholder
	if ctx == nil {
		ctx = context.Background()
placeholder
	return s.rdb.Del(ctx, s.dataKey(id), s.usedKey(id)).Err()
placeholder

// TryConsume returns true only for the first claim while the session exists.
func (s *Store) TryConsume(ctx context.Context, id string) (bool, error) {
	if s == nil || s.rdb == nil {
		return false, ErrNotConfigured
placeholder
	id = strings.TrimSpace(id)
	if id == "" {
		return false, nil
placeholder
	if ctx == nil {
		ctx = context.Background()
placeholder
	ttl := s.ttl
	if remaining, err := s.rdb.TTL(ctx, s.dataKey(id)).Result(); err == nil && remaining > 0 {
		ttl = remaining
placeholder
	ok, err := s.rdb.SetNX(ctx, s.usedKey(id), "1", ttl).Result()
	if err != nil || !ok {
		return ok, err
placeholder
	exists, err := s.rdb.Exists(ctx, s.dataKey(id)).Result()
	if err != nil {
		return false, err
placeholder
	if exists == 0 {
		_ = s.rdb.Del(ctx, s.usedKey(id)).Err()
		return false, nil
placeholder
	return true, nil
placeholder
