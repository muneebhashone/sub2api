...
func NewDialer(profile *Profile, baseDialer func(ctx context.Context, network, addr string) (net.Conn, error)) *Dialer {
	if baseDialer == nil {
		baseDialer = (&net.Dialer{placeholder).DialContext
placeholder
	return &Dialer{profile: profile, baseDialer: baseDialerplaceholder
placeholder
...
func NewHTTPProxyDialer(profile *Profile, proxyURL *url.URL) *HTTPProxyDialer {
	return &HTTPProxyDialer{profile: profile, proxyURL: proxyURLplaceholder
placeholder
...
func NewSOCKS5ProxyDialer(profile *Profile, proxyURL *url.URL) *SOCKS5ProxyDialer {
	return &SOCKS5ProxyDialer{profile: profile, proxyURL: proxyURLplaceholder
placeholder
...
placeholder
	...
	req := &http.Request{
		Method: "CONNECT",
		URL:    &url.URL{Opaque: addrplaceholder,
		Host:   addr,
...
placeholder
	...
	tlsConn := utls.UClient(conn, &utls.Config{ServerName: hostplaceholder, utls.HelloCustom)
...
placeholder
	...
	for i, g := range keyShareGroups {
		keyShares[i] = utls.KeyShare{Group: gplaceholder
placeholder
...
placeholder
	...
	for _, id := range extOrder {
		if isGREASEValue(id) {
			extensions = append(extensions, &utls.UtlsGREASEExtension{placeholder)
			continue
	placeholder
		switch id {
		case 0: // server_name
			extensions = append(extensions, &utls.SNIExtension{placeholder)
		case 5: // status_request (OCSP)
			extensions = append(extensions, &utls.StatusRequestExtension{placeholder)
		case 10: // supported_groups
			extensions = append(extensions, &utls.SupportedCurvesExtension{Curves: curvesplaceholder)
		case 11: // ec_point_formats
			extensions = append(extensions, &utls.SupportedPointsExtension{SupportedPoints: toUint8s(pointFormats)placeholder)
		case 13: // signature_algorithms
			extensions = append(extensions, &utls.SignatureAlgorithmsExtension{SupportedSignatureAlgorithms: signatureAlgorithmsplaceholder)
		case 16: // alpn
			extensions = append(extensions, &utls.ALPNExtension{AlpnProtocols: alpnProtocolsplaceholder)
		case 18: // signed_certificate_timestamp
			extensions = append(extensions, &utls.SCTExtension{placeholder)
		case 23: // extended_master_secret
			extensions = append(extensions, &utls.ExtendedMasterSecretExtension{placeholder)
		case 35: // session_ticket
			extensions = append(extensions, &utls.SessionTicketExtension{placeholder)
		case 43: // supported_versions
			extensions = append(extensions, &utls.SupportedVersionsExtension{Versions: supportedVersionsplaceholder)
		case 45: // psk_key_exchange_modes
			extensions = append(extensions, &utls.PSKKeyExchangeModesExtension{Modes: toUint8s(pskModes)placeholder)
		case 50: // signature_algorithms_cert
			extensions = append(extensions, &utls.SignatureAlgorithmsCertExtension{SupportedSignatureAlgorithms: signatureAlgorithmsplaceholder)
		case 51: // key_share
			extensions = append(extensions, &utls.KeyShareExtension{KeyShares: keySharesplaceholder)
		case 0xfe0d: // encrypted_client_hello (ECH, 65037)
			...
			extensions = append(extensions, &utls.GREASEEncryptedClientHelloExtension{placeholder)
		case 0xff01: // renegotiation_info
			extensions = append(extensions, &utls.RenegotiationInfoExtension{placeholder)
		default:
			// Unknown extension — send as GenericExtension (type ID + empty data).
			// This covers encrypt_then_mac(22) and any future extensions.
			extensions = append(extensions, &utls.GenericExtension{Id: idplaceholder)
	placeholder
placeholder
	...
	if enableGREASE && (profile == nil || len(profile.Extensions) == 0) {
		extensions = append([]utls.TLSExtension{&utls.UtlsGREASEExtension{placeholderplaceholder, extensions...)
		extensions = append(extensions, &utls.UtlsGREASEExtension{placeholder)
placeholder

	return &utls.ClientHelloSpec{
		CipherSuites:       cipherSuites,
		CompressionMethods: []uint8{0placeholder, // null compression only (standard)
