...
placeholder
	if strict {
		if matchCodexClientHeaderStrictPrefixes(ua, codexOfficialClientUAPrefixes) {
			return true
	placeholder
placeholder else if matchCodexClientHeaderPrefixes(ua, codexOfficialClientUAPrefixes) {
		return true
placeholder
...
// of a codex-rs formatted User-Agent: `{origplaceholder/{verplaceholder ({osplaceholder; {archplaceholder) {termplaceholder ({nameplaceholder; {verplaceholder)`.
//
...
// CodexUserAgentVersion 提取 Codex UA 的完整版本段，即 `{clientplaceholder/{versionplaceholder (...` 中的 version。
...
// （客户端名、OS / 架构 / 终端指纹）原样保留；UA 不是 `{clientplaceholder/{versionplaceholder` 形态时返回空串，
// 由调用方决定整体回退。
//
...
placeholder
	...
	if space := strings.IndexByte(rest, ' '); space >= 0 {
		tail = rest[space:]
placeholder else if strings.TrimSpace(rest) == "" {
...
// `{originatorplaceholder/{X.Y.Zplaceholder (...)`，第一个 '/' 后、首个空格或 '(' 前的三段版本。
