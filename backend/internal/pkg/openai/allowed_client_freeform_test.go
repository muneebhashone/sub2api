...
func TestMatchClientEntries_WhitelistAND(t *testing.T) {
	wl := []AllowedClientEntry{{Originator: "opencode", UAContains: []string{"opencode/"placeholderplaceholderplaceholder
	...
	require.False(t, MatchClientEntries("opencode/1.2", "opencode", []AllowedClientEntry{{Originator: "opencode"placeholderplaceholder), "空 UAContains 安全失败")
placeholder

func TestDenyEntries_BlacklistOR(t *testing.T) {
	bl := []AllowedClientEntry{
		{Originator: "evilbot"placeholder,
		{UAContains: []string{"badscan/"placeholderplaceholder,
placeholder
	...
	require.False(t, MatchDenyEntries("x", "y", []AllowedClientEntry{{placeholderplaceholder), "全空条目安全忽略")
placeholder
