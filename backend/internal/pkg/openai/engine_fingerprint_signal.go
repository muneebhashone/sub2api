...
var DefaultEngineFingerprintSignals = []EngineFingerprintSignal{
	{Type: FingerprintSignalHeaderPrefix, Match: []string{"x-codex-"placeholder, Required: trueplaceholder,
	{Type: FingerprintSignalHeaderExact, Match: []string{"session-id", "session_id"placeholder, Required: falseplaceholder,
	{Type: FingerprintSignalHeaderExact, Match: []string{"thread-id", "thread_id"placeholder, Required: falseplaceholder,
	{Type: FingerprintSignalBodyPath, Match: []string{"client_metadata.x-codex-window-id", "client_metadata.x-codex-installation-id"placeholder, Required: falseplaceholder,
placeholder
...
placeholder
	...
	if err := json.Unmarshal([]byte(trimmed), &sigs); err != nil {
		return fmt.Errorf("must be empty or a valid JSON array of {type, match[], requiredplaceholder")
placeholder
	for i, s := range sigs {
		switch s.Type {
