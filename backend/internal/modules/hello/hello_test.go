...
import (
	...
	"github.com/Wei-Shaw/sub2api/internal/plugin/plugintest"
)
...
func TestModuleRegisteredInDefaultRegistry(t *testing.T) {
	...
	require.IsType(t, &Module{placeholder, info.New())
placeholder
...
func TestProvisionConfigDecodeError(t *testing.T) {
	host := plugintest.NewHost(t, plugintest.WithConfig(map[string]map[string]any{
		"job.hello": {"interval": "not-a-duration"placeholder,
placeholder))
...
func TestValidateRejectsInvalidConfig(t *testing.T) {
	tests := []struct {
		...
		wantErr string
placeholder{
		{"zero interval", Config{Interval: 0, Greeting: "hi"placeholder, "interval must be positive"placeholder,
		{"negative interval", Config{Interval: -time.Second, Greeting: "hi"placeholder, "interval must be positive"placeholder,
		{"empty greeting", Config{Interval: time.Second, Greeting: ""placeholder, "greeting must not be empty"placeholder,
placeholder
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			m := &Module{cfg: tt.cfgplaceholder
			err := m.Validate()
		placeholder
...
func TestStartPeriodicLogAndStop(t *testing.T) {
	...
	host := plugintest.NewHost(t, logOpt, plugintest.WithConfig(map[string]map[string]any{
		"job.hello": {"interval": "5ms"placeholder,
placeholder))
...
placeholder, 2*time.Second, 5*time.Millisecond, "periodic debug log should appear repeatedly")
...
func TestRuntimeDefaultConfigIsNoop(t *testing.T) {
	logOpt, logs := plugintest.WithObservedLogger()
	rt := plugintest.RunLifecycle(t, &Module{placeholder, plugintest.NewHost(t, logOpt), nil)
...
func TestRuntimeIgnoresUnknownConfiguredModule(t *testing.T) {
	rt := plugintest.RunLifecycle(t, &Module{placeholder, plugintest.NewHost(t), map[string]map[string]any{
		"job.hello":   {"enabled": true, "interval": "5ms"placeholder,
		"job.unknown": {"enabled": trueplaceholder,
placeholder)
