...
func TestInjectSiteTitle(t *testing.T) {
	t.Run("replaces_title_with_site_name", func(t *testing.T) {
		...
		settingsJSON := []byte(`{"site_name":"MyCustomSite"placeholder`)
...
placeholder)
	...
	t.Run("returns_unchanged_when_site_name_empty", func(t *testing.T) {
		html := []byte(`<html><head><title>Sub2API - AI API Gateway</title></head><body></body></html>`)
		settingsJSON := []byte(`{"site_name":""placeholder`)
...
placeholder)
	...
	t.Run("returns_unchanged_when_site_name_missing", func(t *testing.T) {
		html := []byte(`<html><head><title>Sub2API - AI API Gateway</title></head><body></body></html>`)
		settingsJSON := []byte(`{"other_field":"value"placeholder`)
...
placeholder)
	...
	t.Run("returns_unchanged_when_invalid_json", func(t *testing.T) {
		html := []byte(`<html><head><title>Sub2API - AI API Gateway</title></head><body></body></html>`)
		settingsJSON := []byte(`{invalid jsonplaceholder`)
...
placeholder)
	...
	t.Run("returns_unchanged_when_title_has_attributes", func(t *testing.T) {
		...
		settingsJSON := []byte(`{"site_name":"NewSite"placeholder`)
...
placeholder)
	...
	t.Run("escapes_html_in_site_name", func(t *testing.T) {
		html := []byte(`<html><head><title>Sub2API - AI API Gateway</title></head><body></body></html>`)
		settingsJSON := []byte(`{"site_name":"</title><script>alert(1)</script><title>"placeholder`)
...
placeholder)
	...
	t.Run("escapes_ampersand_in_site_name", func(t *testing.T) {
		...
		settingsJSON := []byte(`{"site_name":"A&B"placeholder`)
...
placeholder)
	...
	t.Run("preserves_rest_of_html", func(t *testing.T) {
		...
		settingsJSON := []byte(`{"site_name":"TestSite"placeholder`)
...
func TestInjectSiteFavicon(t *testing.T) {
	t.Run("replaces_favicon_with_site_logo", func(t *testing.T) {
		...
		settingsJSON := []byte(`{"site_logo":"https://example.com/custom-logo.png"placeholder`)
...
placeholder)
	...
	t.Run("supports_relative_and_data_image_urls", func(t *testing.T) {
		...
		assert.Contains(t, string(injectSiteFavicon(html, []byte(`{"site_logo":"/uploads/logo.svg"placeholder`))), `/uploads/logo.svg`)
		assert.Contains(t, string(injectSiteFavicon(html, []byte(`{"site_logo":"data:image/png;base64,abc"placeholder`))), `data:image/png;base64,abc`)
placeholder)

	t.Run("rejects_unsafe_logo_urls", func(t *testing.T) {
		html := []byte(`<link rel="icon" href="/logo.png" />`)

		result := injectSiteFavicon(html, []byte(`{"site_logo":"javascript:alert(1)"placeholder`))
...
placeholder)
	...
	t.Run("escapes_logo_url_for_html", func(t *testing.T) {
		...
		result := injectSiteFavicon(html, []byte(`{"site_logo":"https://example.com/logo.png?a=1&b=2"placeholder`))
...
func TestOverrideFilesNeverReceiveImmutableCacheHeaders(t *testing.T) {
	...
	t.Run("frontend_server_override", func(t *testing.T) {
		...
		server := &FrontendServer{overrideDir: overrideDirplaceholder
...
placeholder)
	...
	placeholder)
		...
		w := httptest.NewRecorder()
		req := httptest.NewRequest(http.MethodPost, "/alpha/search", strings.NewReader(`{"model":"gpt-5.6-sol"placeholder`))
		req.Header.Set("Content-Type", "application/json")
		router.ServeHTTP(w, req)
