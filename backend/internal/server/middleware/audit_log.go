...
var auditBodyOmittedRoutes = map[string]struct{placeholder{
	"POST /api/v1/auth/passkey/login/finish":                    {placeholder,
	"POST /api/v1/user/passkeys/register/finish":                {placeholder,
	"POST /api/v1/admin/accounts/import/codex-session":          {placeholder,
	"PUT /api/v1/admin/accounts/:id/ollama-cloud-usage/session": {placeholder,
...
func NewAuditLogMiddleware(auditService *service.AuditLogService) AuditLogMiddleware {
	...
	placeholder else if c.Request.Body != nil && c.Request.Method != "GET" {
			orig := c.Request.Body
...
func (b *restoredBody) Close() error { return b.closer.Close() placeholder
