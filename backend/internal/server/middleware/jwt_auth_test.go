...
func newJWTTestEnv(users map[int64]*service.User) (*gin.Engine, *service.AuthService) {
	...
	userRepo := &stubJWTUserRepo{users: usersplaceholder
...
func TestJWTAuth_ValidToken(t *testing.T) {
	user := &service.User{
		...
		TokenVersion: 1,
placeholder
	router, authSvc := newJWTTestEnv(map[int64]*service.User{1: userplaceholder)
...
placeholder
	...
	userRepo := &stubJWTUserRepo{users: map[int64]*service.User{1: userplaceholderplaceholder
	authSvc := service.NewAuthService(nil, userRepo, nil, nil, cfg, nil, nil, nil, nil, nil, nil, nil, nil)
	userSvc := service.NewUserService(userRepo, nil, nil, nil)
	toucher := &recordingActivityToucher{placeholder
...
placeholder
	...
	require.Equal(t, http.StatusOK, w.Code)
	require.Equal(t, []int64{1placeholder, toucher.userIDs)
placeholder
...
func TestJWTAuth_InvalidHeaderFormat(t *testing.T) {
	tests := []struct {
		...
		header string
placeholder{
		{"无Bearer前缀", "Token abc123"placeholder,
		{"缺少空格分隔", "Bearerabc123"placeholder,
		{"仅有单词", "abc123"placeholder,
placeholder
	router, _ := newJWTTestEnv(nil)
...
placeholder
	...
	router, authSvc := newJWTTestEnv(map[int64]*service.User{placeholder)
...
placeholder
	router, authSvc := newJWTTestEnv(map[int64]*service.User{1: userInDBplaceholder)
