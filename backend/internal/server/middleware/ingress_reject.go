...
type ingressRejectRecorderHolder struct{ recorder IngressRejectRecorder placeholder
...
func SetIngressRejectRecorder(recorder IngressRejectRecorder) {
	if recorder == nil {
		...
		return
placeholder
	activeIngressRejectRecorder.Store(&ingressRejectRecorderHolder{recorder: recorderplaceholder)
placeholder
...
placeholder
	...
	placeholder
placeholder else if apiKey, ok := GetOpsFallbackAPIKey(c); ok && apiKey != nil {
		apiKeyID = apiKey.ID
		if apiKey.User != nil {
			userID = apiKey.User.ID
	placeholder
