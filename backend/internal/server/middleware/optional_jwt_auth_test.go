...
func newOptionalJWTTestEnv(users map[int64]*service.User) (*gin.Engine, *service.AuthService) {
	...
	r.GET("/plaza", func(c *gin.Context) {
		...
		c.JSON(http.StatusOK, gin.H{"authed": authed, "user_id": subject.UserIDplaceholder)
placeholder)
	return r, authSvc
placeholder
...
func TestOptionalJWTAuth_ValidTokenSetsSubject(t *testing.T) {
	user := &service.User{
		...
		TokenVersion: 1,
placeholder
	router, authSvc := newOptionalJWTTestEnv(map[int64]*service.User{7: userplaceholder)
