...
placeholder)
	...
	r.GET("/v1beta/models", func(c *gin.Context) { c.Status(http.StatusOK) placeholder)
...
func (f fakeAPIKeyRepo) GetRateLimitData(ctx context.Context, id int64) (*service.APIKeyRateLimitData, error) {
	return &service.APIKeyRateLimitData{placeholder, nil
placeholder
...
placeholder
	...
	placeholder,
		...
		nil,
		nil,
		&config.Config{RunMode: config.RunModeSimpleplaceholder,
	)
...
placeholder
	...
	placeholder,
		...
		activateWindow: func(ctx context.Context, id int64, dailyStart, periodicStart time.Time) error { return nil placeholder,
...
placeholder, nil, nil, &config.Config{RunMode: config.RunModeStandardplaceholder)

	r := gin.New()
	r.Use(APIKeyAuthWithSubscriptionGoogle(apiKeyService, subscriptionService, &config.Config{RunMode: config.RunModeStandardplaceholder))
	r.GET("/v1beta/test", func(c *gin.Context) { c.JSON(200, gin.H{"ok": trueplaceholder) placeholder)
