...
placeholder
	...
	placeholder
		service.MarkOpsClientBusinessLimited(c, service.OpsClientBusinessLimitedReasonLocalFeatureGate)
		c.JSON(http.StatusNotFound, gin.H{"error": gin.H{"type": "not_found_error", "message": "Videos API is not supported for this platform"placeholderplaceholder)
placeholder
...
placeholder
	...
	placeholder)
		...
		voiceHandler := func(endpoint string) gin.HandlerFunc {
			return func(c *gin.Context) {
				if getGroupPlatform(c) != service.PlatformGrok {
					service.MarkOpsClientBusinessLimited(c, service.OpsClientBusinessLimitedReasonLocalFeatureGate)
					c.JSON(http.StatusNotFound, gin.H{"error": gin.H{"type": "not_found_error", "message": "Voice API is not supported for this platform"placeholderplaceholder)
					return
			placeholder
	...
	placeholder
		...
		gateway.GET("/realtime", func(c *gin.Context) {
			if getGroupPlatform(c) != service.PlatformGrok {
				service.MarkOpsClientBusinessLimited(c, service.OpsClientBusinessLimitedReasonLocalFeatureGate)
				c.JSON(http.StatusNotFound, gin.H{"error": gin.H{"type": "not_found_error", "message": "Realtime API is not supported for this platform"placeholderplaceholder)
				return
		placeholder
	...
	placeholder)
		gateway.POST("/web_search", func(c *gin.Context) {
			if getGroupPlatform(c) != service.PlatformGrok {
				service.MarkOpsClientBusinessLimited(c, service.OpsClientBusinessLimitedReasonLocalFeatureGate)
				c.JSON(http.StatusNotFound, gin.H{"error": gin.H{"type": "not_found_error", "message": "Web Search API is not supported for this platform"placeholderplaceholder)
				return
		placeholder
	...
	placeholder)
		gateway.POST("/x_search", func(c *gin.Context) {
			if getGroupPlatform(c) != service.PlatformGrok {
				service.MarkOpsClientBusinessLimited(c, service.OpsClientBusinessLimitedReasonLocalFeatureGate)
				c.JSON(http.StatusNotFound, gin.H{"error": gin.H{"type": "not_found_error", "message": "X Search API is not supported for this platform"placeholderplaceholder)
				return
		placeholder
			h.Gateway.XSearch(c)
	placeholder)
placeholder
	...
	gemini.Use(requireGroupGoogle)
	{
		...
		// Gin treats ":" as a param marker, but Gemini uses "{modelplaceholder:{actionplaceholder" in the same segment.
...
placeholder
	...
	placeholder
		...
		placeholder
			c.JSON(status, gin.H{"error": gin.H{"type": "invalid_request_error", "message": messageplaceholderplaceholder)
			c.Abort()
			return
	placeholder
		...
		if model != "" {
			...
			if err != nil {
				c.JSON(http.StatusInternalServerError, gin.H{"error": gin.H{"type": "server_error", "message": "Failed to resolve composite model route"placeholderplaceholder)
				c.Abort()
				return
		placeholder
			if decision.Matched {
...
func compositeJSONRequestModel(body []byte) (string, string) {
	for _, path := range []string{"model", "session.model"placeholder {
