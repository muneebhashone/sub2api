...
func NormalizeForwardedClientIPHeaders(headers []string) ([]string, error) {
	...
	seen := make(map[string]struct{placeholder, len(headers))
...
func (c *Config) ForwardedClientIPSettings() ForwardedClientIPSettings {
	if c == nil {
		return ForwardedClientIPSettings{Headers: []string{placeholderplaceholder
placeholder
...
placeholder
	...
	if c.Security.forwardedClientIPSettingsLive == nil {
		c.Security.forwardedClientIPSettingsLive = &atomic.Pointer[ForwardedClientIPSettings]{placeholder
placeholder
...
type GatewayConfig struct {
	...
	// 模板渲染后会直接覆盖最终 instructions；若需要保留客户端 system 转换结果，请在模板中显式引用 {{ .ExistingInstructions placeholderplaceholder。
...
placeholder
	...
	if m := cfg.Gateway.UserMessageQueue.Mode; m != "" && m != UMQModeSerialize && m != UMQModeThrottle {
		slog.Warn("invalid user_message_queue mode, disabling",
			"mode", m,
			"valid_modes", []string{UMQModeSerialize, UMQModeThrottleplaceholder)
...
func setDefaults() {
	...
	// CORS
	viper.SetDefault("cors.allowed_origins", []string{placeholder)
	...
	viper.SetDefault("webauthn.rp_origins", []string{placeholder)
...
placeholder)
	...
	viper.SetDefault("batch_image.vertex_managed_gcs_prefix", "batch-image/{envplaceholder/{batch_idplaceholder")
	...
	viper.SetDefault("plugins.trusted_publishers", map[string]string{placeholder)
...
func setEnvReachableDefaults() {
	...
	for _, provider := range []string{"github_oauth", "google_oauth"placeholder {
...
placeholder
	...
	viper.SetDefault("dingtalk_connect.attribute_sync_fields", []string{placeholder)
...
placeholder
	...
	placeholder
		if !strings.Contains(c.BatchImage.VertexManagedGCSPrefix, "{batch_idplaceholder") {
			return fmt.Errorf("batch_image.vertex_managed_gcs_prefix must contain {batch_idplaceholder")
	placeholder
...
func isWeakJWTSecret(secret string) bool {
	...
	if lower == "" {
		return true
placeholder
	weak := map[string]struct{placeholder{
