...
placeholder
	...
	require.Equal(t, map[string]any{"enabled": true, "greeting": "hi"placeholder, cfg.Modules["job.hello"])
	require.Equal(t, map[string]any{"enabled": falseplaceholder, cfg.Modules["payment.provider.stripe"])
...
placeholder)
	t.Run("non-map top level", func(t *testing.T) {
		_, err := normalizeModulesSubtree([]any{"x"placeholder)
	placeholder
placeholder)
	t.Run("non-map entry", func(t *testing.T) {
		_, err := normalizeModulesSubtree(map[string]any{"job.hello": 42placeholder)
	placeholder
		require.Contains(t, err.Error(), "job.hello")
placeholder)
	t.Run("nil entry becomes empty map", func(t *testing.T) {
		out, err := normalizeModulesSubtree(map[string]any{"job.hello": nilplaceholder)
	placeholder
