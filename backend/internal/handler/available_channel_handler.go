...
placeholder
	...
	if !h.featureEnabled(c) {
		response.Success(c, []userAvailableChannel{placeholder)
		return
placeholder
	...
	if err != nil {
		...
		return
placeholder
	allowedGroupIDs := make(map[int64]struct{placeholder, len(userGroups))
	for i := range userGroups {
		allowedGroupIDs[userGroups[i].ID] = struct{placeholder{placeholder
placeholder
...
placeholder
	...
	if len(compositeGroups) > 0 {
		modelPlatforms := make(map[string]struct{placeholder, len(ch.SupportedModels))
		for i := range ch.SupportedModels {
			if platform := ch.SupportedModels[i].Platform; platform != "" {
				modelPlatforms[platform] = struct{placeholder{placeholder
		placeholder
	placeholder
...
placeholder
	...
	for _, platform := range platforms {
		platformSet := map[string]struct{placeholder{platform: {placeholderplaceholder
...
func filterUserVisibleGroups(
	...
	allowed map[int64]struct{placeholder,
) []userAvailableGroup {
...
func toUserSupportedModels(
	...
	allowedPlatforms map[string]struct{placeholder,
) []userSupportedModel {
...
placeholder
	...
	if intervals == nil {
		...
		intervals = []userPricingIntervalDTO{placeholder
placeholder
