...
func promptGuardDecision(kind securityaudit.DecisionKind) *securityaudit.Decision {
	decision := &securityaudit.Decision{Kind: kind, AllowNextStage: falseplaceholder
	switch kind {
...
func TestPromptGuardOpenAIAndClaudeErrorEnvelopesGolden(t *testing.T) {
	...
	placeholder)
		...
		t.Run("responses_"+string(kind), func(t *testing.T) {
			c, recorder := securityAuditErrorTestContext(t)
			(&GatewayHandler{placeholder).responsesSecurityAuditError(c, decision)
			require.Equal(t, decision.HTTPStatus, recorder.Code)
	...
	placeholder)
		...
		t.Run("claude_"+string(kind), func(t *testing.T) {
			c, recorder := securityAuditErrorTestContext(t)
			(&GatewayHandler{placeholder).anthropicSecurityAuditError(c, decision)
			require.Equal(t, decision.HTTPStatus, recorder.Code)
			payload := decodeErrorJSON(t, recorder)
...
func TestPromptGuardGeminiErrorEnvelopeGolden(t *testing.T) {
	...
	placeholder
		...
		require.Equal(t, map[string]any{"request_id": "request-error-golden"placeholder, metadata)
placeholder
placeholder
...
func TestLegacyModerationErrorKeepsExistingClientPriority(t *testing.T) {
	legacy := &securityaudit.Decision{
		...
		Legacy: &securityaudit.LegacyDecision{Blocked: true, StatusCode: http.StatusForbidden, ErrorCode: "content_policy_violation", Message: "legacy exact message"placeholder,
		Prompt: &securityaudit.PromptDecision{Kind: securityaudit.DecisionBlock, ErrorCode: securityaudit.ErrorCodeBlockedplaceholder,
placeholder
	c, recorder := securityAuditErrorTestContext(t)
	(&GatewayHandler{placeholder).openAISecurityAuditError(c, legacy)
