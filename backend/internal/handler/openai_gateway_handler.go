...
placeholder
	...
	wsPricingCtx, wsPricingAt := h.gatewayService.WithOpenAIRequestPricingContext(ctx, apiKey.GroupID)
	ctx = wsPricingCtx
	...
	placeholder
		...
		turnChannelMapping.Store(&openAIWSTurnChannelMappingSnapshot{turn: 1, mapping: channelMappingWSplaceholder)
		// turn 级定价：BeforeTurn 重新冻结 pricingAt 并按最新门复核当前账号，
		// AfterTurn 的计费读取所属 turn 的时刻；turn 在连接内串行推进，互斥锁
		// 只为跨用量提交 goroutine 的读取安全。
		var turnPricingMu sync.Mutex
		turnPricingAt := wsPricingAt
		...
		placeholder,
			...
			placeholder
				turnChannelMapping.Store(&openAIWSTurnChannelMappingSnapshot{turn: turn, mapping: mappingplaceholder)
		...
		placeholder,
			...
			placeholder
				turnPricingMu.Lock()
				turnPricingAt = turnAt
				turnPricingMu.Unlock()
				if turn == 1 {
					return nil
			placeholder
		...
		placeholder,
			...
			placeholder
				...
				sessionID := service.ExtractClientSessionID(c)
				turnPricingMu.Lock()
				turnRecordPricingAt := turnPricingAt
				turnPricingMu.Unlock()
				cyberBlocked := service.GetOpsCyberPolicy(c) != nil
...
placeholder
	...
	placeholder
		...
		flusher, ok := c.Writer.(http.Flusher)
		if ok {
			errorObject := gin.H{"type": errType, "message": messageplaceholder
			if code != "" {
				errorObject["code"] = code
		placeholder
			payload, err := json.Marshal(gin.H{"error": errorObjectplaceholder)
			if err != nil {
				payload = []byte(`{"error":{"type":"upstream_error","message":"Upstream request failed"placeholderplaceholder`)
		placeholder
...
placeholder)
	if err != nil {
		payload = []byte(`{"event_id":"evt_cyber_session_blocked","type":"error","error":{"type":"permission_error","code":"session_blocked_by_cyber_policy","message":"This session is blocked by cyber-security policy, please start a new session"placeholderplaceholder`)
placeholder
	writeCtx, cancel := context.WithTimeout(ctx, 2*time.Second)
	defer cancel()
	_ = conn.Write(writeCtx, coderws.MessageText, payload)
...
func (h *OpenAIGatewayHandler) enqueueCyberSessionBlockedOpsEntry(c *gin.Context, apiKey *service.APIKey, model string, sessionBlockKey string) {
	if h.opsService == nil {
		return
placeholder
	meta := cyberPolicyOpsErrorMeta{Model: model, InboundEndpoint: GetInboundEndpoint(c), CreatedAt: time.Now(), SessionBlockKey: sessionBlockKeyplaceholder
