...
placeholder
	return &http.Response{
		StatusCode: 520,
		Header:     http.Header{"Content-Type": []string{"text/html"placeholderplaceholder,
	...
	accounts := []service.Account{
		{
			...
			Concurrency: 0,
			Priority:    0,
	placeholder"access_token": "token-1"placeholder,
	placeholder,
		{
			ID:          2,
			...
			Concurrency: 0,
			Priority:    1,
	placeholder"access_token": "token-2"placeholder,
	placeholder,
placeholder
	accountRepo := openAIImagesFailoverAccountRepo{accounts: accountsplaceholder
...
placeholder
	groupID := int64(3131)
	body := []byte(`{"model":"gpt-5.1","stream":false,"input":"hello"placeholder`)
...
placeholder
	...
	c.Set(string(middleware2.ContextKeyAPIKey), &service.APIKey{
		...
		Group: &service.Group{
			...
			Platform: service.PlatformOpenAI,
	placeholder,
		User: &service.User{ID: 100placeholder,
placeholder)
	c.Set(string(middleware2.ContextKeyUser), middleware2.AuthSubject{UserID: 100, Concurrency: 0placeholder)
	return c, rec
placeholder
...
func TestOpenAIGatewayHandlerResponses_FailoverAbortsWhenClientDisconnected(t *testing.T) {
	...
	defer cancel()
	upstream := &openAIResponsesFailoverCancelUpstream{onFirstDo: cancelplaceholder
	handler := newOpenAIResponsesFailoverTestHandler(t, upstream)
	c, rec := newOpenAIResponsesFailoverTestContext(t, ctx)

	handler.Responses(c)

	require.Equal(t, []int64{1placeholder, upstream.calls(), "客户端断开后不应再切换到账号 2")
...
func TestOpenAIGatewayHandlerResponses_FailoverContinuesForConnectedClient(t *testing.T) {
	...
	upstream := &openAIResponsesFailoverCancelUpstream{placeholder
	handler := newOpenAIResponsesFailoverTestHandler(t, upstream)
	c, rec := newOpenAIResponsesFailoverTestContext(t, nil)

	handler.Responses(c)

	require.Equal(t, []int64{1, 2placeholder, upstream.calls(), "在线客户端应正常切换账号")
