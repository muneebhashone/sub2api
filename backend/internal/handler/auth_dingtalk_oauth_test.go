...
// B/C: Step 3/4 失败降级时 staff = &DingTalkStaffInfo{placeholder，claims 不应有 nil。
...
func TestCheckDingTalkCorpAllowed_CrossOrgPolicy(t *testing.T) {
	cfg := config.DingTalkConnectConfig{CorpRestrictionPolicy: "none"placeholder
...
func TestDecideDingTalkStep34Strategy_PolicyNone(t *testing.T) {
	step3Err := &DingTalkAPIError{Code: "60011", Message: "not in directory", HTTP: 403placeholder
...
func TestDecideDingTalkStep34Strategy_PolicyNoneEmpty(t *testing.T) {
	stepErr := &DingTalkAPIError{Code: "60011", Message: "not in directory", HTTP: 403placeholder
...
func TestDecideDingTalkStep34Strategy_NoError(t *testing.T) {
	for _, policy := range []string{"none", "internal_only", ""placeholder {
...
func TestBuildDingTalkUpstreamClaims_SubjectEqualsUnionID(t *testing.T) {
	staff := &DingTalkStaffInfo{UserID: "user123", Name: "张三", Email: "zhangsan@corp.com"placeholder
...
func TestBuildDingTalkUpstreamClaims_CrossOrgEmptyCorpUserID(t *testing.T) {
	// 跨组织降级路径：staff = &DingTalkStaffInfo{placeholder（所有字段为零值）
	staff := &DingTalkStaffInfo{placeholder
...
func TestBuildDingTalkUpstreamClaims_PrimaryDeptIDInClaims(t *testing.T) {
	staff := &DingTalkStaffInfo{UserID: "u1", Name: "张三", Email: "a@b.com", DeptIDs: []int64{42, 99placeholderplaceholder
...
func TestBuildDingTalkUpstreamClaims_NoDeptIDs(t *testing.T) {
	staff := &DingTalkStaffInfo{UserID: "u2", Name: "李四"placeholder
...
func TestDingTalkStaffFromClaims_RoundTrip(t *testing.T) {
	staff := &DingTalkStaffInfo{UserID: "u3", Name: "王五", Email: "ww@corp.com", DeptIDs: []int64{55placeholderplaceholder
	...
	require.Equal(t, []int64{55placeholder, recovered.DeptIDs)
placeholder
...
func TestResolveDingTalkDeptPath_SingleLevel(t *testing.T) {
	...
	responses := map[string]string{
		"42": `{"errcode":0,"result":{"dept_id":42,"name":"研发部","parent_id":1placeholderplaceholder`,
		"1":  `{"errcode":0,"result":{"dept_id":1,"name":"公司","parent_id":0placeholderplaceholder`,
placeholder
	...
	placeholder else {
			_, _ = w.Write([]byte(`{"errcode":60003,"errmsg":"not found"placeholder`))
	placeholder
placeholder))
	defer server.Close()
...
func TestResolveDingTalkDeptPath_MultiLevel(t *testing.T) {
	...
	responses := map[string]string{
		"42": `{"errcode":0,"result":{"dept_id":42,"name":"AI研发","parent_id":10placeholderplaceholder`,
		"10": `{"errcode":0,"result":{"dept_id":10,"name":"研发部","parent_id":1placeholderplaceholder`,
		"1":  `{"errcode":0,"result":{"dept_id":1,"name":"公司","parent_id":0placeholderplaceholder`,
placeholder
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
