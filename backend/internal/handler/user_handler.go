...
placeholder
	if h.userPlatformQuotaRepo == nil {
		response.Success(c, map[string]any{"platform_quotas": []any{placeholderplaceholder)
		return
placeholder
...
placeholder
	response.Success(c, map[string]any{"platform_quotas": outplaceholder)
placeholder
...
func (h *UserHandler) buildUserProfileResponse(ctx context.Context, userID int64, user *service.User) (userProfileResponse, error) {
	...
	if err != nil {
		return userProfileResponse{placeholder, err
placeholder
...
func thirdPartyIdentityProviders(identities service.UserIdentitySummarySet) []service.UserIdentitySummary {
	...
	for _, summary := range []service.UserIdentitySummary{identities.LinuxDo, identities.OIDC, identities.WeChat, identities.DingTalkplaceholder {
		if summary.Bound {
