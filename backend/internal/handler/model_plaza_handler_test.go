...
func plazaGroups() []service.PlazaGroup {
	return []service.PlazaGroup{
		{ID: 1, Name: "public-standard", Platform: "anthropic", SubscriptionType: "standard", RateMultiplier: 1placeholder,
		{ID: 2, Name: "exclusive-a", Platform: "anthropic", IsExclusive: true, RateMultiplier: 0.5placeholder,
		{ID: 3, Name: "public-subscription", Platform: "openai", SubscriptionType: "subscription", RateMultiplier: 1placeholder,
		{ID: 4, Name: "exclusive-b", Platform: "openai", IsExclusive: true, RateMultiplier: 0.8placeholder,
placeholder
placeholder
...
func TestFilterPlazaVisibleGroups_AuthedSeesGrantedExclusive(t *testing.T) {
	...
	allowed := map[int64]struct{placeholder{2: {placeholderplaceholder
	...
	for _, g := range visible {
		ids = append(ids, g.ID)
placeholder
	require.ElementsMatch(t, []int64{1, 2, 3placeholder, ids)
placeholder
...
func TestFilterPlazaVisibleGroups_AuthedEmptySetSeesNoExclusive(t *testing.T) {
	...
	visible := filterPlazaVisibleGroups(plazaGroups(), map[int64]struct{placeholder{placeholder)
	require.Len(t, visible, 2)
placeholder

func TestModelPlazaHandler_NilSettingServiceFailsClosed404(t *testing.T) {
	gin.SetMode(gin.TestMode)
	h := &ModelPlazaHandler{placeholder // settingService == nil → fail-closed
...
placeholder
	...
	dto := toModelPlazaGroupDTO(&g, map[int64]float64{2: 0.5placeholder)
		...
		Models: []service.PlazaModel{{
			...
			Pricing: &service.ChannelModelPricing{
				...
				Intervals: []service.PricingInterval{
					{MinTokens: 0, MaxTokens: &maxTokens, TierLabel: "≤272K", InputPrice: testPtr(2.5e-6)placeholder,
					{MinTokens: 272000, TierLabel: ">272K", InputPrice: testPtr(5e-6)placeholder,
			placeholder,
		placeholder,
			OfficialPricing: &service.PlazaOfficialPricing{
				InputPrice: testPtr(2.5e-6),
...
func testPtr(v float64) *float64 { return &v placeholder
...
func TestToModelPlazaGroupDTO_TimePricing(t *testing.T) {
	g := service.PlazaGroup{
		...
		Models: []service.PlazaModel{{
			Name:     "deepseek-chat",
			Platform: "deepseek",
			Pricing:  &service.ChannelModelPricing{BillingMode: service.BillingModeToken, InputPrice: testPtr(0.28e-6)placeholder,
	...
	placeholder, {
			...
			Platform: "deepseek",
			Pricing:  &service.ChannelModelPricing{BillingMode: service.BillingModeToken, InputPrice: testPtr(0.56e-6)placeholder,
