...
placeholder
	...
	h.enrichShadowParents(ctx, []AccountWithConcurrency{itemplaceholder)
...
func (h *AccountHandler) fetchOpenAIAccountLoadMap(ctx context.Context, openAIAccounts []*service.Account) map[int64]*service.AccountLoadInfo {
	loadMap := map[int64]*service.AccountLoadInfo{placeholder
	if h.concurrencyService == nil || len(openAIAccounts) == 0 {
		return loadMap
placeholder
	seen := make(map[int64]struct{placeholder, len(openAIAccounts))
	...
	placeholder
		if _, ok := seen[account.ID]; ok {
			continue
	placeholder
		seen[account.ID] = struct{placeholder{placeholder
...
placeholder else if batchLoad != nil {
		loadMap = batchLoad
placeholder
	return loadMap
placeholder
...
placeholder
	...
	pageOpenAIAccountIDs := make(map[int64]struct{placeholder)
	...
	for i := range accounts {
		...
		if account.Platform != service.PlatformOpenAI {
			continue
	placeholder
		pageOpenAIAccountIDs[account.ID] = struct{placeholder{placeholder
		if len(account.AccountGroups) == 0 && len(account.GroupIDs) == 0 {
			continue
	placeholder
		for _, accountGroup := range account.AccountGroups {
			if accountGroup.GroupID > 0 {
				groupIDs[accountGroup.GroupID] = struct{placeholder{placeholder
		placeholder
	placeholder
		for _, groupID := range account.GroupIDs {
			if groupID > 0 {
				groupIDs[groupID] = struct{placeholder{placeholder
		placeholder
	placeholder
placeholder
...
placeholder
	sort.Slice(groupIDList, func(i, j int) bool { return groupIDList[i] < groupIDList[j] placeholder)
...
placeholder
	if h.ollamaCloudUsage != nil {
		if err := h.ollamaCloudUsage.ResolveAccounts(c.Request.Context(), []*service.Account{accountplaceholder); err != nil {
			response.ErrorFrom(c, err)
			return
	placeholder
placeholder
...
placeholder else if account.Platform == service.PlatformAntigravity {
	...
	placeholder
		...
		placeholder
	placeholder
placeholder else if account.Platform == service.PlatformGrok {
...
placeholder else if cleared != nil {
