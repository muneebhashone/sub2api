...
func NewDataManagementHandler(dataManagementService *service.DataManagementService) *DataManagementHandler {
	return &DataManagementHandler{dataManagementService: dataManagementServiceplaceholder
placeholder
...
placeholder)
	if err != nil {
		...
		return
placeholder
	response.Success(c, gin.H{"ok": result.OK, "message": result.Messageplaceholder)
placeholder
...
placeholder)
	if err != nil {
		...
		return
placeholder
	response.Success(c, gin.H{"job_id": job.JobID, "status": job.Statusplaceholder)
placeholder
...
func (h *DataManagementHandler) requireAgentEnabled(c *gin.Context) bool {
	if h.dataManagementService == nil {
		...
		).WithMetadata(map[string]string{"socket_path": service.DefaultDataManagementAgentSocketPathplaceholder)
		response.ErrorFrom(c, err)
		return false
placeholder
