...
placeholder
	...
	if requestID == "" && clientRequestID == "" {
		response.Paginated(c, []*service.OpsErrorLog{placeholder, 0, 1, 10)
		return
placeholder
...
placeholder
	...
	req := opsRetryRequest{Mode: service.OpsRetryModeClientplaceholder
...
func parseOpsTimeRange(c *gin.Context, defaultRange string) (time.Time, time.Time, error) {
	...
	parseTS := func(s string) (time.Time, error) {
		if s == "" {
			return time.Time{placeholder, nil
	placeholder
...
placeholder
	...
	if err != nil {
		return time.Time{placeholder, time.Time{placeholder, err
placeholder
...
placeholder
	...
	placeholder
		if start.After(end) {
			return time.Time{placeholder, time.Time{placeholder, fmt.Errorf("invalid time range: start_time must be <= end_time")
	placeholder
		if end.Sub(start) > 30*24*time.Hour {
			return time.Time{placeholder, time.Time{placeholder, fmt.Errorf("invalid time range: max window is 30 days")
	placeholder
