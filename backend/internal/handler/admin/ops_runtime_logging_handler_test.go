...
func newTestSettingRepo() *testSettingRepo {
	return &testSettingRepo{values: map[string]string{placeholderplaceholder
placeholder
...
func newOpsRuntimeRouter(handler *OpsHandler, withUser bool) *gin.Engine {
	...
	if withUser {
		r.Use(func(c *gin.Context) {
			c.Set(string(middleware.ContextKeyUser), middleware.AuthSubject{UserID: 7placeholder)
			c.Next()
	placeholder)
placeholder
...
placeholder
	...
	cfg := &config.Config{
		Ops: config.OpsConfig{Enabled: trueplaceholder,
...
func TestOpsRuntimeLoggingHandler_UpdateUnauthorized(t *testing.T) {
	...
	body := `{"level":"debug","enable_sampling":false,"sampling_initial":100,"sampling_thereafter":100,"caller":true,"stacktrace_level":"error","retention_days":30placeholder`
	w := httptest.NewRecorder()
