...
func NewPluginHandler(manager *service.PluginManager) *PluginHandler {
	return &PluginHandler{manager: managerplaceholder
placeholder
...
func (h *PluginHandler) Enable(c *gin.Context) {
	...
	if !ok {
		return
placeholder
	request := pluginEnableRequest{RolloutPercent: 100placeholder
...
placeholder
	if err := h.manager.Delete(c.Request.Context(), id); err != nil {
		...
		return
placeholder
	response.Success(c, gin.H{"message": "插件已卸载"placeholder)
placeholder
