...
placeholder
	...
	router := gin.New()
	handler := &ModuleHandler{runtime: sourceplaceholder
...
func TestModuleHandlerListSerializesSnapshot(t *testing.T) {
	stub := &moduleStatusSourceStub{statuses: []plugin.ModuleStatus{
		{ID: "gateway.platform.anthropic", Enabled: true, State: plugin.StateRunningplaceholder,
		{ID: "job.hello", Enabled: false, State: plugin.StateRegisteredplaceholder,
...
func TestModuleHandlerListSortsByID(t *testing.T) {
	stub := &moduleStatusSourceStub{statuses: []plugin.ModuleStatus{
		{ID: "job.zeta", Enabled: true, State: plugin.StateRunningplaceholder,
		{ID: "gateway.alpha", Enabled: true, State: plugin.StateRunningplaceholder,
		{ID: "job.alpha", Enabled: false, State: plugin.StateRegisteredplaceholder,
placeholderplaceholder
	...
	for _, m := range envelope.Data.Modules {
		ids = append(ids, m.ID)
placeholder
	require.Equal(t, []string{"gateway.alpha", "job.alpha", "job.zeta"placeholder, ids)
placeholder

func TestModuleHandlerListRedactsErrorText(t *testing.T) {
	stub := &moduleStatusSourceStub{statuses: []plugin.ModuleStatus{
		{ID: "job.bad", Enabled: true, State: plugin.StateErrored, Err: "provision failed: access_token=ya29.secret-value"placeholder,
placeholderplaceholder
...
func TestModuleHandlerListEmptySnapshot(t *testing.T) {
	w, envelope := performModuleList(t, &moduleStatusSourceStub{placeholder)
...
func (m *moduleHandlerFakeModule) ModuleInfo() plugin.ModuleInfo {
	...
	return plugin.ModuleInfo{
		ID:               id,
		New:              func() plugin.Module { return &moduleHandlerFakeModule{id: id, enabledByDefault: enabledplaceholder placeholder,
...
func TestModuleHandlerListWithRuntimeSnapshot(t *testing.T) {
	...
	registry.RegisterModule(&moduleHandlerFakeModule{id: "job.enabled", enabledByDefault: trueplaceholder)
	registry.RegisterModule(&moduleHandlerFakeModule{id: "job.disabled", enabledByDefault: falseplaceholder)
	...
	t.Cleanup(func() { _ = runtime.Stop(context.Background()) placeholder)
