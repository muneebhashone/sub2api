...
func NewModuleHandler(runtime *plugin.Runtime) *ModuleHandler {
	return &ModuleHandler{runtime: runtimeplaceholder
placeholder
...
func (h *ModuleHandler) List(c *gin.Context) {
	...
	placeholder)
placeholder
	sort.Slice(modules, func(i, j int) bool { return modules[i].ID < modules[j].ID placeholder)
	response.Success(c, gin.H{"modules": modulesplaceholder)
placeholder
