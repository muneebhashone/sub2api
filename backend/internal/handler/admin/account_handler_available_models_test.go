...
func setupSyncUpstreamModelsRouter(adminSvc service.AdminService, upstream service.HTTPUpstream) *gin.Engine {
	...
	accountTestSvc := service.NewAccountTestService(
		...
		nil,
		upstream,
		&config.Config{Security: config.SecurityConfig{URLAllowlist: config.URLAllowlistConfig{Enabled: falseplaceholderplaceholderplaceholder,
		nil,
	)
...
placeholder
	require.ElementsMatch(t, []string{
		"gpt-5.3-codex-spark",
placeholder, ids, "影子可用模型由 model_mapping 派生（非写死）")
placeholder
...
placeholder
	require.ElementsMatch(t, []string{"gemini-2.0-flash", "gemini-2.5-flash", "gemini-2.5-pro"placeholder, ids)
...
func TestAccountHandlerSyncUpstreamModels_ConfigErrorReturnsBadRequest(t *testing.T) {
	...
	placeholder,
placeholder
	router := setupSyncUpstreamModelsRouter(svc, &syncUpstreamHTTPUpstream{placeholder)
