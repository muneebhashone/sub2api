...
placeholder
	...
	placeholder"token": "secret"placeholder,
			Extra:       map[string]any{"note": "x"placeholder,
			ProxyID:     &proxyID,
			Concurrency: 3,
			Priority:    50,
	...
	placeholder,
		{
			...
			Platform:        service.PlatformOpenAI,
			Type:            service.AccountTypeOAuth,
			Credentials:     map[string]any{placeholder, // 影子恒空凭据
...
func TestExportDataPassesAccountFiltersAndSort(t *testing.T) {
	router, adminSvc := setupAccountDataRouter()
	adminSvc.accounts = []service.Account{
		{ID: 1, Name: "acc-1", Status: service.StatusActiveplaceholder,
placeholder
			...
			"accounts": []map[string]any{
				{
					...
					"platform":    service.PlatformOpenAI,
					"type":        service.AccountTypeOAuth,
					"credentials": map[string]any{"token": "x"placeholder,
					"proxy_key":   "socks5|1.2.3.4|1080|u|p",
					"concurrency": 3,
					"priority":    50,
			placeholder,
