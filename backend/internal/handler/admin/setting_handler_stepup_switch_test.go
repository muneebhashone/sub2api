...
placeholder
	...
	repo := &settingHandlerRepoStub{values: storedplaceholder
...
func TestUpdateSettingsEnableStepUpRejectsWithoutSession(t *testing.T) {
	h, repo := newStepUpSwitchTestHandler(t, map[string]string{placeholder)

	rec := doUpdateSettings(t, h, map[string]any{"step_up_enabled": trueplaceholder, nil)
...
func TestUpdateSettingsEnableStepUpRejectsAdminAPIKey(t *testing.T) {
	h, _ := newStepUpSwitchTestHandler(t, map[string]string{placeholder)

	rec := doUpdateSettings(t, h, map[string]any{"step_up_enabled": trueplaceholder, func(c *gin.Context) {
...
func TestUpdateSettingsEnableStepUpFailsClosedWithoutUserService(t *testing.T) {
	...
	rec := doUpdateSettings(t, h, map[string]any{"step_up_enabled": trueplaceholder, func(c *gin.Context) {
		c.Set(string(middleware.ContextKeyUser), middleware.AuthSubject{UserID: 1placeholder)
placeholder)
...
placeholder)
	...
	rec := doUpdateSettings(t, h, map[string]any{"step_up_enabled": falseplaceholder, nil)
...
placeholder)
	...
	rec := doUpdateSettings(t, h, map[string]any{"step_up_enabled": falseplaceholder, func(c *gin.Context) {
		c.Set("auth_method", service.AuditAuthMethodAdminAPIKey)
placeholder)
...
placeholder)
	...
	rec := doUpdateSettings(t, h, map[string]any{"registration_enabled": trueplaceholder, nil)
...
placeholder)
	...
	preserved := doUpdateSettings(t, h, map[string]any{"registration_enabled": trueplaceholder, nil)
	...
	cleared := doUpdateSettings(t, h, map[string]any{"forwarded_client_ip_headers": []string{placeholderplaceholder, nil)
...
func TestUpdateSettingsMalformedForwardedClientIPHeadersRemainFailClosedWhenOmitted(t *testing.T) {
	cfg := &config.Config{Default: config.DefaultConfig{UserConcurrency: 5placeholderplaceholder
	repo := &settingHandlerRepoStub{values: map[string]string{
		service.SettingKeyAPIKeyACLTrustForwardedIP: "true",
		service.SettingKeyForwardedClientIPHeaders:  `{"not":"an array"placeholder`,
...
placeholder)
	...
	rec := doUpdateSettings(t, h, map[string]any{
		"forwarded_client_ip_headers": []string{"X Invalid"placeholder,
placeholder, nil)
