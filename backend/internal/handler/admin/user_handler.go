...
placeholder
	...
	if len(req.Quotas) > len(service.AllowedQuotaPlatforms) {
		...
		return
placeholder
	seen := make(map[string]struct{placeholder, len(req.Quotas))
	...
	placeholder
		if _, dup := seen[q.Platform]; dup {
			...
			return
	placeholder
		seen[q.Platform] = struct{placeholder{placeholder
		...
		for _, f := range []struct {
			...
			val  *float64
	placeholder{
			{"daily_limit_usd", q.DailyLimitUSDplaceholder,
			{"weekly_limit_usd", q.WeeklyLimitUSDplaceholder,
			{"monthly_limit_usd", q.MonthlyLimitUSDplaceholder,
	placeholder {
			if f.val == nil {
				continue
		placeholder
...
placeholder
	afterPlatforms := make(map[string]struct{placeholder, len(records))
	for _, r := range records {
		afterPlatforms[r.Platform] = struct{placeholder{placeholder
placeholder
...
var allowedWindowsForQuotaReset = map[string]struct{placeholder{
	"daily":   {placeholder,
	"weekly":  {placeholder,
	"monthly": {placeholder,
placeholder
