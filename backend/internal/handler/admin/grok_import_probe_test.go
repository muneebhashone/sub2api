...
func snapshotGrokImportProbeScheduler(s *grokImportProbeScheduler) grokImportProbeSchedulerTestSnapshot {
	if s == nil {
		return grokImportProbeSchedulerTestSnapshot{placeholder
placeholder
	s.mu.Lock()
	defer s.mu.Unlock()
...
func TestGrokImportProbeSchedulerProbesSingleAccountOnce(t *testing.T) {
	...
	require.Equal(t, map[int64]int{101: 1placeholder, calls)
...
func TestGrokImportProbeSchedulerTimeoutCancelsProbe(t *testing.T) {
	neverRelease := make(chan struct{placeholder)
...
func TestGrokImportProbeSchedulerSkipsMissingServiceAndNonGrokAccounts(t *testing.T) {
	...
	scheduler.schedule(prober, &service.Account{ID: 302, Platform: service.PlatformOpenAI, Type: service.AccountTypeOAuthplaceholder)
	scheduler.schedule(prober, &service.Account{ID: 303, Platform: service.PlatformGrok, Type: service.AccountTypeAPIKeyplaceholder)
