...
func TestOpenAIRefreshQuota_QueryFailureIsReported(t *testing.T) {
	quota := &openAIQuotaWorkflowStub{queryErr: errors.New("upstream query failed")placeholder
	handler := &OpenAIOAuthHandler{
		adminService: &openAIResetAdminServiceStub{placeholder,
		quotaService: quota,
placeholder
...
func TestNewOpenAIOAuthHandlerKeepsNilQuotaCapabilitiesGuarded(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		path   string
placeholder{
		{http.MethodGet, "/openai/accounts/42/quota"placeholder,
		{http.MethodPost, "/openai/accounts/42/quota/refresh"placeholder,
		{http.MethodPost, "/openai/accounts/42/reset-quota"placeholder,
placeholder {
		recorder := httptest.NewRecorder()
