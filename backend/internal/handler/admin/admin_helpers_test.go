...
func TestParseOpsOpenAITokenStatsDuration(t *testing.T) {
	tests := []struct {
		...
		ok    bool
placeholder{
		{input: "30m", want: 30 * time.Minute, ok: trueplaceholder,
		{input: "1h", want: time.Hour, ok: trueplaceholder,
		{input: "1d", want: 24 * time.Hour, ok: trueplaceholder,
		{input: "15d", want: 15 * 24 * time.Hour, ok: trueplaceholder,
		{input: "30d", want: 30 * 24 * time.Hour, ok: trueplaceholder,
		{input: "7d", want: 0, ok: falseplaceholder,
placeholder
...
placeholder
	...
	_, err = validateOpsAlertRulePayload(map[string]json.RawMessage{placeholder)
placeholder
...
placeholder)
	...
	t.Run("non-empty values pass through (lowercased)", func(t *testing.T) {
		in := &dto.OpenAIFastPolicySettings{
			Rules: []dto.OpenAIFastPolicyRule{
				{ServiceTier: "priority", Action: "filter", Scope: "all"placeholder,
				{ServiceTier: "flex", Action: "block", Scope: "oauth"placeholder,
				{ServiceTier: "all", Action: "pass", Scope: "apikey"placeholder,
		placeholder,
	placeholder
		out := openaiFastPolicySettingsFromDTO(in)
