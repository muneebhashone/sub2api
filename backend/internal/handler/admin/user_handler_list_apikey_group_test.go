...
func (s *listUsersFilterStub) ListUsers(_ context.Context, _, _ int, filters service.UserListFilters, _, _ string) ([]service.User, int64, error) {
	s.captured = filters
	return []service.User{placeholder, 0, nil
placeholder
...
func TestAdminUserList_ParsesAPIKeyGroupID(t *testing.T) {
	...
	cases := []struct {
		...
		want  int64
placeholder{
		{"valid id", "?api_key_group_id=42", 42placeholder,
		{"missing", "", 0placeholder,
		{"zero ignored", "?api_key_group_id=0", 0placeholder,
		{"negative ignored", "?api_key_group_id=-3", 0placeholder,
		{"non-numeric ignored", "?api_key_group_id=abc", 0placeholder,
placeholder
