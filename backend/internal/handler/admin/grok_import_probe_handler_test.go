...
type grokImportOAuthClientStub struct{placeholder
...
func TestGrokSSOBatchImportKeepsCreatedAccountsWhenOneAutomaticProbeFails(t *testing.T) {
	...
	oauthService := service.NewGrokOAuthService(nil, grokImportOAuthClientStub{placeholder)
	...
	request := httptest.NewRequest(
		...
		strings.NewReader(`{"sso_tokens":["sso-one","sso-two","sso-three"]placeholder`),
	)
...
placeholder
	...
	require.Equal(t, map[int64]int{501: 1, 502: 1, 503: 1placeholder, calls)
placeholder
...
func TestAccountCreateWithoutAutomaticGrokProbeServiceStillSucceeds(t *testing.T) {
	...
	request := httptest.NewRequest(
		...
		strings.NewReader(`{"name":"grok-rt","platform":"grok","type":"oauth","credentials":{"refresh_token":"secret"placeholderplaceholder`),
	)
	request.Header.Set("Content-Type", "application/json")
	router.ServeHTTP(recorder, request)
