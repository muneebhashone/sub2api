...
func (s *stubAdminService) DuplicateGroup(ctx context.Context, id int64, actorScope, operationKey string) (*service.Group, error) {
	group := service.Group{ID: 201, Name: "group (Copy)", Status: "inactive"placeholder
	return &group, nil
placeholder
...
placeholder
	...
	if start >= total {
		return []service.Account{placeholder, int64(total), nil
placeholder
	end := start + pageSize
	if end > total {
		end = total
...
func (s *stubAdminService) AdminUpdateAPIKeyGroupID(ctx context.Context, keyID int64, groupID *int64) (*service.AdminUpdateAPIKeyGroupIDResult, error) {
	for i := range s.apiKeys {
		if s.apiKeys[i].ID == keyID {
			...
			placeholder
		placeholder
			return &service.AdminUpdateAPIKeyGroupIDResult{APIKey: &kplaceholder, nil
	placeholder
placeholder
