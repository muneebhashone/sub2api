...
placeholder
	...
	soraMap := map[int64]*service.SoraAccount{placeholder
	if h.soraAccountRepo != nil {
		soraMap, _ = h.soraAccountRepo.GetByAccountIDs(c.Request.Context(), accountIDs)
placeholder

	usageMap := map[int64]*service.SoraUsageStat{placeholder
	if h.usageRepo != nil {
...
placeholder
	...
	for _, item := range req.Accounts {
		entry := SoraAccountBatchItemResult{AccountID: item.AccountIDplaceholder
...
func (h *SoraAccountHandler) ListUsage(c *gin.Context) {
	...
	if h.usageRepo == nil {
		response.Paginated(c, []dto.SoraUsageStat{placeholder, 0, page, pageSize)
		return
placeholder
