...
placeholder
	...
	if req.ClearRebateRate {
		if err := h.affiliateService.AdminSetUserRebateRate(c.Request.Context(), userID, nil); err != nil {
			...
			return
	placeholder
placeholder else if req.AffRebateRatePercent != nil {
...
placeholder
	...
	response.Success(c, gin.H{"user_id": userIDplaceholder)
placeholder
...
placeholder
	if err := h.affiliateService.AdminBatchSetUserRebateRate(c.Request.Context(), req.UserIDs, rate); err != nil {
		...
		return
placeholder
	response.Success(c, gin.H{"affected": len(req.UserIDs)placeholder)
placeholder
...
func (h *AffiliateHandler) LookupUsers(c *gin.Context) {
	keyword := c.Query("q")
	if keyword == "" {
		response.Success(c, []AffiliateUserSummary{placeholder)
		return
placeholder
	users, _, err := h.adminService.ListUsers(c.Request.Context(), 1, 20, service.UserListFilters{Search: keywordplaceholder, "email", "asc")
	if err != nil {
		response.ErrorFrom(c, err)
		return
placeholder
	...
	for i, u := range users {
		result[i] = AffiliateUserSummary{ID: u.ID, Email: u.Email, Username: u.Usernameplaceholder
placeholder
