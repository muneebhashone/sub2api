...
placeholder
	...
	var resp struct {
		...
		Data struct {
			...
			placeholder `json:"linuxdo"`
				OIDC struct {
					Bound       bool   `json:"bound"`
					DisplayName string `json:"display_name"`
					ProviderKey string `json:"provider_key"`
			placeholder `json:"oidc"`
				WeChat struct {
					Bound         bool   `json:"bound"`
					CanBind       bool   `json:"can_bind"`
					BindStartPath string `json:"bind_start_path"`
			placeholder `json:"wechat"`
		placeholder `json:"identities"`
	placeholder `json:"data"`
placeholder
	require.NoError(t, json.Unmarshal(recorder.Body.Bytes(), &resp))
	require.Equal(t, 0, resp.Code)
...
placeholder
	...
	c, _ := gin.CreateTestContext(recorder)
	c.Request = httptest.NewRequest(http.MethodGet, "/api/v1/user/profile", nil)
	c.Set(string(middleware2.ContextKeyUser), middleware2.AuthSubject{UserID: 21placeholder)
...
placeholder
	...
	c, _ := gin.CreateTestContext(recorder)
	c.Request = httptest.NewRequest(http.MethodGet, "/api/v1/user/profile", nil)
	c.Set(string(middleware2.ContextKeyUser), middleware2.AuthSubject{UserID: 22placeholder)
...
placeholder
	...
	body := []byte(`{"email":"new@example.com","verify_code":"123456","password":"new-password"placeholder`)
	recorder := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(recorder)
	c.Request = httptest.NewRequest(http.MethodPost, "/api/v1/user/account-bindings/email", bytes.NewReader(body))
	c.Request.Header.Set("Content-Type", "application/json")
	c.Params = gin.Params{{Key: "provider", Value: "email"placeholderplaceholder
	c.Set(string(middleware2.ContextKeyUser), middleware2.AuthSubject{UserID: 11placeholder)
...
placeholder
	...
	c.Set(string(middleware2.ContextKeyUser), middleware2.AuthSubject{UserID: 21placeholder)
	c.Params = gin.Params{{Key: "provider", Value: "linuxdo"placeholderplaceholder

	handler.UnbindIdentity(c)

	require.Equal(t, http.StatusOK, recorder.Code)
	require.Equal(t, []string{"linuxdo"placeholder, repo.unbound)
...
func TestUserHandlerUnbindIdentityRevokesAllUserSessionsWhenAuthServiceConfigured(t *testing.T) {
	...
	placeholder,
placeholder
	refreshTokenCache := &userHandlerRefreshTokenCacheStub{placeholder
	cfg := &config.Config{
		JWT: config.JWTConfig{
			Secret:     "test-secret",
			ExpireHour: 1,
...
placeholder
	...
	c, _ := gin.CreateTestContext(recorder)
	c.Request = httptest.NewRequest(http.MethodDelete, "/api/v1/user/account-bindings/linuxdo", nil)
	c.Set(string(middleware2.ContextKeyUser), middleware2.AuthSubject{UserID: 23placeholder)
	c.Params = gin.Params{{Key: "provider", Value: "linuxdo"placeholderplaceholder

	handler.UnbindIdentity(c)

	require.Equal(t, http.StatusOK, recorder.Code)
	require.Equal(t, []int64{23placeholder, refreshTokenCache.revokedUserIDs)
...
placeholder
	...
	c, _ := gin.CreateTestContext(recorder)
	c.Request = httptest.NewRequest(http.MethodDelete, "/api/v1/user/account-bindings/linuxdo", nil)
	c.Set(string(middleware2.ContextKeyUser), middleware2.AuthSubject{UserID: 24placeholder)
	c.Params = gin.Params{{Key: "provider", Value: "linuxdo"placeholderplaceholder
...
placeholder
	...
	repo := &userHandlerRepoStub{user: userplaceholder
	emailCache := &userHandlerEmailCacheStub{
		data: &service.VerificationCodeData{
			Code:      "123456",
			CreatedAt: time.Now().UTC(),
...
placeholder
	...
	body := []byte(`{"email":"new@example.com","verify_code":"123456","password":"wrong-password"placeholder`)
	recorder := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(recorder)
	c.Request = httptest.NewRequest(http.MethodPost, "/api/v1/user/account-bindings/email", bytes.NewReader(body))
	c.Request.Header.Set("Content-Type", "application/json")
...
placeholder
	...
	body := []byte(`{"provider":"wechat","redirect_to":"/settings/profile"placeholder`)
	recorder := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(recorder)
