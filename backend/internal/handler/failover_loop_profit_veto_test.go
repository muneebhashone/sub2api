...
placeholder
	res := profitVetoLoopResult{placeholder
...
func TestProfitVetoAfter503DoesNotLivelock(t *testing.T) {
	...
	fs.SwitchCount = 1
	fs.FailedAccountIDs[1] = struct{placeholder{placeholder

	start := time.Now()
	res := runProfitVetoLoop(t, fs, []int64{1placeholder, map[int64]bool{1: trueplaceholder, 50)
...
func TestProfitVetoKeepsBackoffUsefulForHealthyAccount(t *testing.T) {
	...
	res := runProfitVetoLoop(t, fs, []int64{2, 1placeholder, map[int64]bool{2: trueplaceholder, 50)
...
func TestHandleSelectionExhaustedUnaffectedWithoutProfitVeto(t *testing.T) {
	...
	fs.LastFailoverErr = newTestFailoverErr(503, false, false)
	fs.SwitchCount = 1
	fs.FailedAccountIDs[100] = struct{placeholder{placeholder
