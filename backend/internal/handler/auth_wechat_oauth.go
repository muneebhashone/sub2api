...
placeholder
	...
	placeholder else {
			return authErr
	placeholder
placeholder else if tokenPair != nil {
...
placeholder
	keys := []string{preferredplaceholder
...
placeholder
	...
	if h == nil || h.settingSvc == nil {
		return wechatOAuthConfig{placeholder, infraerrors.ServiceUnavailable("CONFIG_NOT_READY", "wechat oauth settings service not ready")
placeholder
...
placeholder
	if strings.TrimSpace(cfg.redirectURI) == "" {
		return wechatOAuthConfig{placeholder, infraerrors.InternalServer("OAUTH_CONFIG_INVALID", "wechat oauth redirect url not configured")
placeholder
...
func decodeWeChatPaymentOAuthContext(raw string) (wechatPaymentOAuthContext, error) {
	...
	if raw == "" {
		return wechatPaymentOAuthContext{placeholder, nil
placeholder
	var ctx wechatPaymentOAuthContext
	if err := json.Unmarshal([]byte(raw), &ctx); err != nil {
		return wechatPaymentOAuthContext{placeholder, err
placeholder
	return ctx, nil
placeholder
