...
placeholder
	return gatewayhook.NewChain(nil, &contentModerationPreFlightHook{svc: contentModerationServiceplaceholder)
placeholder

// preFlightLoggerKey 在钩子执行期间通过 ctx 传递请求级 logger（reqLog），
// 保留其已累积的 component/model/stream 等字段，保证审核日志字段等价。
type preFlightLoggerKey struct{placeholder

func withPreFlightLogger(ctx context.Context, log *zap.Logger) context.Context {
	if log == nil {
		return ctx
placeholder
	return context.WithValue(ctx, preFlightLoggerKey{placeholder, log)
placeholder
...
func preFlightLoggerFromContext(ctx context.Context) *zap.Logger {
	if ctx == nil {
		return nil
placeholder
	log, _ := ctx.Value(preFlightLoggerKey{placeholder).(*zap.Logger)
	return log
placeholder
...
func newGatewayHookRequest(c *gin.Context, apiKey *service.APIKey, subject middleware2.AuthSubject, protocol string, model string, body []byte) *gatewayhook.Request {
	caller := gatewayhook.CallerInfo{UserID: subject.UserIDplaceholder
	if apiKey != nil {
...
func (h *contentModerationPreFlightHook) HookID() string { return "content_moderation" placeholder
