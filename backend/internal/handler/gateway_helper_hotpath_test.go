...
func validClaudeCodeBodyJSON() []byte {
	return []byte(`{
		...
		"metadata":{"user_id":"user_aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa_account__session_aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa"placeholder
placeholder`)
placeholder
...
placeholder)
	...
	t.Run("cli_messages_path_invalid_body_sets_false", func(t *testing.T) {
		...
		SetClaudeCodeClientContext(c, []byte(`{"model":"x"placeholder`), nil)
		require.False(t, service.IsClaudeCodeClient(c.Request.Context()))
placeholder)
placeholder
...
placeholder
	...
	helper := NewConcurrencyHelper(concurrency, SSEPingFormatNone, 5*time.Millisecond)
	c, _ := newHelperTestContext(http.MethodPost, "/v1/messages")
	c.Set(string(middleware2.ContextKeyAPIKey), &service.APIKey{ID: 77placeholder)
	streamStarted := false
...
placeholder
	...
	require.Equal(t, []int64{77placeholder, cache.apiKeyTrackIDs)
...
func TestAcquireUserSlotWithWait_TimeoutDecrementsWaitQueue(t *testing.T) {
	cache := &helperConcurrencyCacheStub{
		userSeq:     []bool{false, false, falseplaceholder,
		waitAllowed: true,
placeholder
	concurrency := service.NewConcurrencyService(cache)
	helper := NewConcurrencyHelper(concurrency, SSEPingFormatNone, 5*time.Millisecond)
...
func TestAcquireUserSlotWithWait_RequestCancelDecrementsWaitQueue(t *testing.T) {
	cancelled := make(chan struct{placeholder)
	var cancel context.CancelFunc
	cache := &helperConcurrencyCacheStub{
		userSeq:     []bool{false, falseplaceholder,
		waitAllowed: true,
		waitIncrementHook: func() {
			cancel()
			close(cancelled)
