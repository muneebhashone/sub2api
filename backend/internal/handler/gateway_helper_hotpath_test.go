...
func TestSetClaudeCodeClientContext_ReuseParsedRequestAndContextCache(t *testing.T) {
	t.Run("reuse parsed request without body unmarshal", func(t *testing.T) {
		...
		parsedReq := &service.ParsedRequest{
			Model: "claude-3-5-sonnet-20241022",
			System: []any{
				map[string]any{"text": "You are Claude Code, Anthropic's official CLI for Claude."placeholder,
		placeholder,
	...
	t.Run("reuse context cache without body unmarshal", func(t *testing.T) {
		...
		c.Set(service.OpenAIParsedRequestBodyKey, map[string]any{
			...
			"system": []any{
				map[string]any{"text": "You are Claude Code, Anthropic's official CLI for Claude."placeholder,
		placeholder,
			"metadata": map[string]any{"user_id": "user_aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa_account__session_aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa"placeholder,
	placeholder)
