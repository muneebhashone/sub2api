...
placeholder
	...
	kid := "kid-1"
	jwks := oidcJWKSet{Keys: []oidcJWK{buildRSAJWK(kid, &priv.PublicKey)placeholderplaceholder
...
placeholder))
	...
	claims := oidcIDTokenClaims{
		...
		RegisteredClaims: jwt.RegisteredClaims{
			...
			Subject:   "subject-1",
			Audience:  jwt.ClaimStrings{"client-1", "another-aud"placeholder,
...
func TestOIDCParseUserInfoIncludesSuggestedProfile(t *testing.T) {
	cfg := config.OIDCConnectConfig{placeholder
...
placeholder)
	...
	c.Request = req
	c.Set(string(servermiddleware.ContextKeyUser), servermiddleware.AuthSubject{UserID: 84placeholder)
...
func TestOIDCOAuthCallbackAllowsOptionalPKCEAndIDTokenValidation(t *testing.T) {
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/token":
			...
			_, _ = w.Write([]byte(`{"access_token":"oidc-access","token_type":"Bearer","expires_in":3600placeholder`))
		case "/userinfo":
			w.Header().Set("Content-Type", "application/json")
			_, _ = w.Write([]byte(`{"sub":"oidc-subject-compat","preferred_username":"oidc_user","name":"OIDC Display","email":"oidc@example.com"placeholder`))
		default:
			http.NotFound(w, r)
	placeholder
placeholder))
...
placeholder
	_, err = client.AuthIdentity.Create().
		...
		SetMetadata(map[string]any{"username": "legacy-user"placeholder).
		Save(ctx)
placeholder
...
placeholder
	...
	req.AddCookie(&http.Cookie{Name: oauthPendingBrowserCookieName, Value: encodeCookieValue("oidc-browser")placeholder)
	c.Request = req
...
placeholder
	...
	req.Header.Set("Content-Type", "application/json")
	req.AddCookie(&http.Cookie{Name: oauthPendingSessionCookieName, Value: encodeCookieValue(session.SessionToken)placeholder)
	req.AddCookie(&http.Cookie{Name: oauthPendingBrowserCookieName, Value: encodeCookieValue("oidc-invalid-browser")placeholder)
	c.Request = req
...
placeholder
	...
	req.Header.Set("Content-Type", "application/json")
	req.AddCookie(&http.Cookie{Name: oauthPendingSessionCookieName, Value: encodeCookieValue(session.SessionToken)placeholder)
	req.AddCookie(&http.Cookie{Name: oauthPendingBrowserCookieName, Value: encodeCookieValue("oidc-choice-browser")placeholder)
	c.Request = req
...
placeholder
	...
	req.Header.Set("Content-Type", "application/json")
	req.AddCookie(&http.Cookie{Name: oauthPendingSessionCookieName, Value: encodeCookieValue(session.SessionToken)placeholder)
	req.AddCookie(&http.Cookie{Name: oauthPendingBrowserCookieName, Value: encodeCookieValue("oidc-browser-no-adoption")placeholder)
	c.Request = req
...
placeholder
	...
	req.Header.Set("Content-Type", "application/json")
	req.AddCookie(&http.Cookie{Name: oauthPendingSessionCookieName, Value: encodeCookieValue(session.SessionToken)placeholder)
	req.AddCookie(&http.Cookie{Name: oauthPendingBrowserCookieName, Value: encodeCookieValue("oidc-conflict-browser")placeholder)
	c.Request = req
...
placeholder)
	if handler.cfg == nil {
		handler.cfg = &config.Config{placeholder
placeholder
...
placeholder
	...
	kid := "test-kid"
	jwks := oidcJWKSet{Keys: []oidcJWK{buildRSAJWK(kid, &privateKey.PublicKey)placeholderplaceholder
...
placeholder))
	...
	claims := oidcIDTokenClaims{
		...
		RegisteredClaims: jwt.RegisteredClaims{
			...
			Audience:  jwt.ClaimStrings{"oidc-client"placeholder,
			IssuedAt:  jwt.NewNumericDate(now),
			NotBefore: jwt.NewNumericDate(now.Add(-30 * time.Second)),
			ExpiresAt: jwt.NewNumericDate(now.Add(5 * time.Minute)),
	placeholder,
