...
func TestOpenAIHandleStreamingAwareError_ResponsesStreamingReusesRequestID(t *testing.T) {
	...
	c.Request = c.Request.WithContext(
		context.WithValue(c.Request.Context(), ctxkey.RequestID, "placeholder"),
	)
...
func TestOpenAIHandleStreamingAwareError_ResponsesStreamingJSONEscaping(t *testing.T) {
	cases := []struct {
		...
		message string
placeholder{
		{"双引号", "server_error", `upstream returned "invalid" response`placeholder,
		{"反斜杠", "server_error", `path C:\Users\test\file.txt not found`placeholder,
		{"双引号+反斜杠", "upstream_error", `error parsing "key\value": unexpected token`placeholder,
		{"换行与制表", "server_error", "line1\nline2\ttab"placeholder,
		{"普通", "upstream_error", "Upstream service temporarily unavailable"placeholder,
placeholder
...
// Gateway handler: /v1/messages preserves the legacy data:{type:error,...placeholder format
...
func TestInboundIsResponses_CoversAllRoutes(t *testing.T) {
	cases := []struct {
		...
		want  bool
placeholder{
		{"/v1/responses", trueplaceholder,
		{"/v1/responses/compact", trueplaceholder,
		{"/responses", trueplaceholder,                       // <-- 用户 16 实际走这条
		{"/responses/compact", trueplaceholder,
		{"/backend-api/codex/responses", trueplaceholder,
		{"/backend-api/codex/responses/compact", trueplaceholder,
		{"/v1/chat/completions", falseplaceholder,
		{"/v1/messages", falseplaceholder,
		{"/", falseplaceholder,
		{"/responses-fake", falseplaceholder,
placeholder
	for _, tc := range cases {
...
func TestMapResponsesErrorCode(t *testing.T) {
	cases := []struct{ in, out string placeholder{
		{"rate_limit_error", "rate_limit_exceeded"placeholder,
		{"invalid_request_error", "invalid_request"placeholder,
		{"permission_error", "permission_denied"placeholder,
		{"authentication_error", "authentication_failed"placeholder,
		{"upstream_error", "upstream_error"placeholder,
		{"server_error", "server_error"placeholder,
		{"api_error", "server_error"placeholder,
		{"", "server_error"placeholder,
		{"custom_thing", "custom_thing"placeholder,
placeholder
	for _, tc := range cases {
