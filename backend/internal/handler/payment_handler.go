...
func (h *PaymentHandler) GetChannels(c *gin.Context) {
	channels, _, err := h.channelService.List(c.Request.Context(), pagination.PaginationParams{Page: 1, PageSize: 1000placeholder, "active", "")
	if err != nil {
		response.ErrorFrom(c, err)
		return
placeholder
...
func requireAuth(c *gin.Context) (middleware2.AuthSubject, bool) {
	...
	if !ok {
		...
		return middleware2.AuthSubject{placeholder, false
placeholder
	return subject, true
placeholder
