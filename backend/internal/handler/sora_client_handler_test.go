...
func newStubSoraGenRepo() *stubSoraGenRepo {
	return &stubSoraGenRepo{gens: make(map[int64]*service.SoraGeneration), nextID: 1placeholder
placeholder
...
func newTestSoraClientHandler(repo *stubSoraGenRepo) *SoraClientHandler {
	...
	return &SoraClientHandler{genService: genServiceplaceholder
placeholder
...
func TestParseMediaURLsFromBody_MediaURLs(t *testing.T) {
	urls := parseMediaURLsFromBody([]byte(`{"media_urls":["https://a.com/1.mp4","https://a.com/2.mp4"]placeholder`))
	require.Equal(t, []string{"https://a.com/1.mp4", "https://a.com/2.mp4"placeholder, urls)
placeholder

func TestParseMediaURLsFromBody_SingleMediaURL(t *testing.T) {
	urls := parseMediaURLsFromBody([]byte(`{"media_url":"https://a.com/video.mp4"placeholder`))
	require.Equal(t, []string{"https://a.com/video.mp4"placeholder, urls)
placeholder

func TestParseMediaURLsFromBody_EmptyBody(t *testing.T) {
	require.Nil(t, parseMediaURLsFromBody(nil))
	require.Nil(t, parseMediaURLsFromBody([]byte{placeholder))
placeholder
...
func TestParseMediaURLsFromBody_NoMediaFields(t *testing.T) {
	require.Nil(t, parseMediaURLsFromBody([]byte(`{"data":"something"placeholder`)))
placeholder

func TestParseMediaURLsFromBody_EmptyMediaURL(t *testing.T) {
	require.Nil(t, parseMediaURLsFromBody([]byte(`{"media_url":""placeholder`)))
placeholder

func TestParseMediaURLsFromBody_EmptyMediaURLs(t *testing.T) {
	require.Nil(t, parseMediaURLsFromBody([]byte(`{"media_urls":[]placeholder`)))
placeholder

func TestParseMediaURLsFromBody_MediaURLsPriority(t *testing.T) {
	body := `{"media_url":"https://single.com/1.mp4","media_urls":["https://multi.com/a.mp4","https://multi.com/b.mp4"]placeholder`
...
func TestParseMediaURLsFromBody_NonStringArray(t *testing.T) {
	...
	require.Nil(t, parseMediaURLsFromBody([]byte(`{"media_urls":"not-array"placeholder`)))
placeholder
...
func TestExtractMediaURLsFromResult_OAuthPath(t *testing.T) {
	result := &service.ForwardResult{MediaURL: "https://oauth.com/video.mp4"placeholder
	recorder := httptest.NewRecorder()
	url, urls := extractMediaURLsFromResult(result, recorder)
	require.Equal(t, "https://oauth.com/video.mp4", url)
	require.Equal(t, []string{"https://oauth.com/video.mp4"placeholder, urls)
placeholder

func TestExtractMediaURLsFromResult_OAuthWithBody(t *testing.T) {
	result := &service.ForwardResult{MediaURL: "https://oauth.com/video.mp4"placeholder
	recorder := httptest.NewRecorder()
	_, _ = recorder.Write([]byte(`{"media_urls":["https://body.com/1.mp4","https://body.com/2.mp4"]placeholder`))
	url, urls := extractMediaURLsFromResult(result, recorder)
...
func TestExtractMediaURLsFromResult_APIKeyPath(t *testing.T) {
	recorder := httptest.NewRecorder()
	_, _ = recorder.Write([]byte(`{"media_url":"https://upstream.com/video.mp4"placeholder`))
	url, urls := extractMediaURLsFromResult(nil, recorder)
	require.Equal(t, "https://upstream.com/video.mp4", url)
	require.Equal(t, []string{"https://upstream.com/video.mp4"placeholder, urls)
placeholder
...
func TestExtractMediaURLsFromResult_EmptyMediaURL(t *testing.T) {
	result := &service.ForwardResult{MediaURL: ""placeholder
	recorder := httptest.NewRecorder()
	url, urls := extractMediaURLsFromResult(result, recorder)
	require.Empty(t, url)
	require.Nil(t, urls)
...
func TestGetUserIDFromContext_AuthSubject(t *testing.T) {
	c, _ := gin.CreateTestContext(httptest.NewRecorder())
	c.Request = httptest.NewRequest("GET", "/", nil)
	c.Set(string(middleware2.ContextKeyUser), middleware2.AuthSubject{UserID: 777placeholder)
...
func TestGenerate_Unauthorized(t *testing.T) {
	...
	c, rec := makeGinContext("POST", "/api/v1/sora/generate", `{"model":"sora2-landscape-10s","prompt":"test"placeholder`, 0)
	h.Generate(c)
...
func TestGenerate_BadRequest_MissingModel(t *testing.T) {
	h := newTestSoraClientHandler(newStubSoraGenRepo())
	c, rec := makeGinContext("POST", "/api/v1/sora/generate", `{"prompt":"test"placeholder`, 1)
	h.Generate(c)
...
func TestGenerate_BadRequest_MissingPrompt(t *testing.T) {
	h := newTestSoraClientHandler(newStubSoraGenRepo())
	c, rec := makeGinContext("POST", "/api/v1/sora/generate", `{"model":"sora2-landscape-10s"placeholder`, 1)
	h.Generate(c)
	require.Equal(t, http.StatusBadRequest, rec.Code)
placeholder
...
func TestGenerate_TooManyRequests(t *testing.T) {
	...
	c, rec := makeGinContext("POST", "/api/v1/sora/generate", `{"model":"sora2-landscape-10s","prompt":"test"placeholder`, 1)
	h.Generate(c)
...
func TestGenerate_Success(t *testing.T) {
	repo := newStubSoraGenRepo()
	h := newTestSoraClientHandler(repo)
	c, rec := makeGinContext("POST", "/api/v1/sora/generate", `{"model":"sora2-landscape-10s","prompt":"测试生成"placeholder`, 1)
	h.Generate(c)
...
func TestGenerate_ImageMediaType(t *testing.T) {
	repo := newStubSoraGenRepo()
	h := newTestSoraClientHandler(repo)
	c, rec := makeGinContext("POST", "/api/v1/sora/generate", `{"model":"gpt-image","prompt":"test","media_type":"image"placeholder`, 1)
	h.Generate(c)
	require.Equal(t, http.StatusOK, rec.Code)
...
func TestListGenerations_Success(t *testing.T) {
	repo := newStubSoraGenRepo()
	repo.gens[1] = &service.SoraGeneration{ID: 1, UserID: 1, Model: "sora2-landscape-10s", Status: "completed", StorageType: "upstream"placeholder
	repo.gens[2] = &service.SoraGeneration{ID: 2, UserID: 1, Model: "gpt-image", Status: "pending", StorageType: "none"placeholder
	repo.nextID = 3
	h := newTestSoraClientHandler(repo)
...
func TestGetGeneration_InvalidID(t *testing.T) {
	...
	c.Params = gin.Params{{Key: "id", Value: "abc"placeholderplaceholder
	h.GetGeneration(c)
	require.Equal(t, http.StatusBadRequest, rec.Code)
placeholder
...
func TestGetGeneration_NotFound(t *testing.T) {
	...
	c.Params = gin.Params{{Key: "id", Value: "999"placeholderplaceholder
	h.GetGeneration(c)
...
func TestGetGeneration_WrongUser(t *testing.T) {
	repo := newStubSoraGenRepo()
	repo.gens[1] = &service.SoraGeneration{ID: 1, UserID: 2, Status: "completed"placeholder
	h := newTestSoraClientHandler(repo)
...
func TestGetGeneration_Success(t *testing.T) {
	repo := newStubSoraGenRepo()
	repo.gens[1] = &service.SoraGeneration{ID: 1, UserID: 1, Model: "sora2-landscape-10s", Status: "completed", StorageType: "upstream", MediaURL: "https://example.com/video.mp4"placeholder
	h := newTestSoraClientHandler(repo)
	c, rec := makeGinContext("GET", "/api/v1/sora/generations/1", "", 1)
	c.Params = gin.Params{{Key: "id", Value: "1"placeholderplaceholder
	h.GetGeneration(c)
...
func TestDeleteGeneration_Success(t *testing.T) {
	repo := newStubSoraGenRepo()
	repo.gens[1] = &service.SoraGeneration{ID: 1, UserID: 1, Status: "completed"placeholder
	h := newTestSoraClientHandler(repo)
	c, rec := makeGinContext("DELETE", "/api/v1/sora/generations/1", "", 1)
	c.Params = gin.Params{{Key: "id", Value: "1"placeholderplaceholder
	h.DeleteGeneration(c)
...
func TestCancelGeneration_WrongUser(t *testing.T) {
	repo := newStubSoraGenRepo()
	repo.gens[1] = &service.SoraGeneration{ID: 1, UserID: 2, Status: "pending"placeholder
	h := newTestSoraClientHandler(repo)
...
func TestCancelGeneration_Pending(t *testing.T) {
	repo := newStubSoraGenRepo()
	repo.gens[1] = &service.SoraGeneration{ID: 1, UserID: 1, Status: "pending"placeholder
	h := newTestSoraClientHandler(repo)
	c, rec := makeGinContext("POST", "/api/v1/sora/generations/1/cancel", "", 1)
	c.Params = gin.Params{{Key: "id", Value: "1"placeholderplaceholder
	h.CancelGeneration(c)
...
func TestCancelGeneration_Generating(t *testing.T) {
	repo := newStubSoraGenRepo()
	repo.gens[1] = &service.SoraGeneration{ID: 1, UserID: 1, Status: "generating"placeholder
	h := newTestSoraClientHandler(repo)
	c, rec := makeGinContext("POST", "/api/v1/sora/generations/1/cancel", "", 1)
	c.Params = gin.Params{{Key: "id", Value: "1"placeholderplaceholder
	h.CancelGeneration(c)
...
func TestCancelGeneration_Failed(t *testing.T) {
	repo := newStubSoraGenRepo()
	repo.gens[1] = &service.SoraGeneration{ID: 1, UserID: 1, Status: "failed"placeholder
	h := newTestSoraClientHandler(repo)
	c, rec := makeGinContext("POST", "/api/v1/sora/generations/1/cancel", "", 1)
	c.Params = gin.Params{{Key: "id", Value: "1"placeholderplaceholder
	h.CancelGeneration(c)
...
func TestCancelGeneration_Cancelled(t *testing.T) {
	repo := newStubSoraGenRepo()
	repo.gens[1] = &service.SoraGeneration{ID: 1, UserID: 1, Status: "cancelled"placeholder
	h := newTestSoraClientHandler(repo)
	c, rec := makeGinContext("POST", "/api/v1/sora/generations/1/cancel", "", 1)
	c.Params = gin.Params{{Key: "id", Value: "1"placeholderplaceholder
	h.CancelGeneration(c)
...
func TestGetModels(t *testing.T) {
	...
	for _, item := range data {
		...
		if m["type"] == "video" {
			videoCount++
	placeholder else if m["type"] == "image" {
			imageCount++
	placeholder
placeholder
...
placeholder
	...
	h := &SoraClientHandler{mediaStorage: mediaStorageplaceholder
...
func TestSaveToStorage_NotUpstream(t *testing.T) {
	repo := newStubSoraGenRepo()
	repo.gens[1] = &service.SoraGeneration{ID: 1, UserID: 1, Status: "completed", StorageType: "s3", MediaURL: "https://example.com/v.mp4"placeholder
	h := newTestSoraClientHandler(repo)
...
func TestSaveToStorage_EmptyMediaURL(t *testing.T) {
	repo := newStubSoraGenRepo()
	repo.gens[1] = &service.SoraGeneration{ID: 1, UserID: 1, Status: "completed", StorageType: "upstream", MediaURL: ""placeholder
	h := newTestSoraClientHandler(repo)
	c, rec := makeGinContext("POST", "/api/v1/sora/generations/1/save", "", 1)
	c.Params = gin.Params{{Key: "id", Value: "1"placeholderplaceholder
	h.SaveToStorage(c)
...
func TestSaveToStorage_S3Nil(t *testing.T) {
	repo := newStubSoraGenRepo()
	repo.gens[1] = &service.SoraGeneration{ID: 1, UserID: 1, Status: "completed", StorageType: "upstream", MediaURL: "https://example.com/video.mp4"placeholder
	h := newTestSoraClientHandler(repo)
	c, rec := makeGinContext("POST", "/api/v1/sora/generations/1/save", "", 1)
	c.Params = gin.Params{{Key: "id", Value: "1"placeholderplaceholder
	h.SaveToStorage(c)
...
func TestSaveToStorage_WrongUser(t *testing.T) {
	repo := newStubSoraGenRepo()
	repo.gens[1] = &service.SoraGeneration{ID: 1, UserID: 2, Status: "completed", StorageType: "upstream", MediaURL: "https://example.com/video.mp4"placeholder
	h := newTestSoraClientHandler(repo)
	c, rec := makeGinContext("POST", "/api/v1/sora/generations/1/save", "", 1)
	c.Params = gin.Params{{Key: "id", Value: "1"placeholderplaceholder
	h.SaveToStorage(c)
...
func TestStoreMediaWithDegradation_NilS3NilMedia(t *testing.T) {
	...
	require.Equal(t, []string{"https://upstream.com/v.mp4"placeholder, urls)
	require.Nil(t, keys)
...
func TestStoreMediaWithDegradation_NilGuardsMultiURL(t *testing.T) {
	h := &SoraClientHandler{placeholder
	url, urls, storageType, keys, size := h.storeMediaWithDegradation(
		context.Background(), 1, "video", "https://upstream.com/v.mp4", []string{"https://a.com/1.mp4", "https://a.com/2.mp4"placeholder,
	)
	require.Equal(t, service.SoraStorageTypeUpstream, storageType)
...
func TestStoreMediaWithDegradation_EmptyMediaURLsFallback(t *testing.T) {
	...
	url, _, storageType, _, _ := h.storeMediaWithDegradation(
		context.Background(), 1, "video", "https://upstream.com/v.mp4", []string{placeholder,
	)
	require.Equal(t, service.SoraStorageTypeUpstream, storageType)
	require.Equal(t, "https://upstream.com/v.mp4", url)
placeholder
...
func newStubUserRepoForHandler() *stubUserRepoForHandler {
	return &stubUserRepoForHandler{users: make(map[int64]*service.User)placeholder
placeholder
...
placeholder
	...
	return nil
placeholder
func (r *stubUserRepoForHandler) Create(context.Context, *service.User) error { return nil placeholder
...
func (r *stubUserRepoForHandler) GetFirstAdmin(context.Context) (*service.User, error) {
	return nil, nil
placeholder
func (r *stubUserRepoForHandler) Delete(context.Context, int64) error { return nil placeholder
...
func (r *stubUserRepoForHandler) ListWithFilters(context.Context, pagination.PaginationParams, service.UserListFilters) ([]service.User, *pagination.PaginationResult, error) {
	return nil, nil, nil
placeholder
func (r *stubUserRepoForHandler) UpdateBalance(context.Context, int64, float64) error { return nil placeholder
func (r *stubUserRepoForHandler) DeductBalance(context.Context, int64, float64) error { return nil placeholder
func (r *stubUserRepoForHandler) UpdateConcurrency(context.Context, int64, int) error { return nil placeholder
...
func newStubAPIKeyRepoForHandler() *stubAPIKeyRepoForHandler {
	return &stubAPIKeyRepoForHandler{keys: make(map[int64]*service.APIKey)placeholder
placeholder
...
placeholder
func (r *stubAPIKeyRepoForHandler) Create(context.Context, *service.APIKey) error { return nil placeholder
...
func (r *stubAPIKeyRepoForHandler) GetByKeyForAuth(context.Context, string) (*service.APIKey, error) {
	return nil, nil
placeholder
func (r *stubAPIKeyRepoForHandler) Update(context.Context, *service.APIKey) error { return nil placeholder
func (r *stubAPIKeyRepoForHandler) Delete(context.Context, int64) error           { return nil placeholder
...
func newTestAPIKeyService(repo *stubAPIKeyRepoForHandler) *service.APIKeyService {
	return service.NewAPIKeyService(repo, nil, nil, nil, nil, nil, &config.Config{placeholder)
placeholder
...
placeholder
	...
	h := &SoraClientHandler{genService: genService, apiKeyService: apiKeyServiceplaceholder
	c, rec := makeGinContext("POST", "/api/v1/sora/generate",
		`{"model":"sora2-landscape-10s","prompt":"test","api_key_id":42placeholder`, 1)
	h.Generate(c)
	require.Equal(t, http.StatusOK, rec.Code)
	resp := parseResponse(t, rec)
	data := resp["data"].(map[string]any)
...
func TestGenerate_WithAPIKeyID_NotFound(t *testing.T) {
	...
	h := &SoraClientHandler{genService: genService, apiKeyService: apiKeyServiceplaceholder
	c, rec := makeGinContext("POST", "/api/v1/sora/generate",
		`{"model":"sora2-landscape-10s","prompt":"test","api_key_id":999placeholder`, 1)
	h.Generate(c)
	require.Equal(t, http.StatusBadRequest, rec.Code)
	resp := parseResponse(t, rec)
...
func TestGenerate_WithAPIKeyID_NilAPIKeyService(t *testing.T) {
	...
	h := &SoraClientHandler{genService: genServiceplaceholder // apiKeyService = nil
	c, rec := makeGinContext("POST", "/api/v1/sora/generate",
		`{"model":"sora2-landscape-10s","prompt":"test","api_key_id":42placeholder`, 1)
	h.Generate(c)
	require.Equal(t, http.StatusOK, rec.Code)
...
func TestGenerate_NoAPIKeyID_NoContext_NilResult(t *testing.T) {
	...
	h := &SoraClientHandler{genService: genService, apiKeyService: apiKeyServiceplaceholder
	c, rec := makeGinContext("POST", "/api/v1/sora/generate",
		`{"model":"sora2-landscape-10s","prompt":"test"placeholder`, 1)
	h.Generate(c)
	require.Equal(t, http.StatusOK, rec.Code)
	require.Nil(t, repo.gens[1].APIKeyID)
placeholder
...
func TestGenerate_APIKeyID_Zero_IgnoredInJSON(t *testing.T) {
	...
	c, rec := makeGinContext("POST", "/api/v1/sora/generate",
		`{"model":"sora2-landscape-10s","prompt":"test","api_key_id":0placeholder`, 1)
	h.Generate(c)
	require.Equal(t, http.StatusBadRequest, rec.Code)
placeholder
...
func TestProcessGeneration_WithGroupID_NoForcePlatform(t *testing.T) {
	...
	repo.gens[1] = &service.SoraGeneration{ID: 1, UserID: 1, Status: "pending"placeholder
	genService := service.NewSoraGenerationService(repo, nil, nil)
	h := &SoraClientHandler{genService: genServiceplaceholder
...
func TestGenerateRequest_WithAPIKeyID_JSONParsing(t *testing.T) {
	...
	err := json.Unmarshal([]byte(`{"model":"sora2","prompt":"test","api_key_id":42placeholder`), &req)
placeholder
...
func TestGenerateRequest_WithoutAPIKeyID_JSONParsing(t *testing.T) {
	...
	var req GenerateRequest
	err := json.Unmarshal([]byte(`{"model":"sora2","prompt":"test"placeholder`), &req)
placeholder
...
func TestGenerateRequest_NullAPIKeyID_JSONParsing(t *testing.T) {
	...
	var req GenerateRequest
	err := json.Unmarshal([]byte(`{"model":"sora2","prompt":"test","api_key_id":nullplaceholder`), &req)
placeholder
	require.Nil(t, req.APIKeyID)
placeholder
...
func TestGenerateRequest_JSONSerialize_OmitsNilAPIKeyID(t *testing.T) {
	...
	req := GenerateRequest{Model: "sora2", Prompt: "test"placeholder
...
func TestGenerateRequest_JSONSerialize_IncludesAPIKeyID(t *testing.T) {
	...
	id := int64(42)
	req := GenerateRequest{Model: "sora2", Prompt: "test", APIKeyID: &idplaceholder
	b, err := json.Marshal(req)
placeholder
	var parsed map[string]any
	require.NoError(t, json.Unmarshal(b, &parsed))
...
placeholder
	return &stubSettingRepoForHandler{values: valuesplaceholder
placeholder

func (r *stubSettingRepoForHandler) Get(_ context.Context, key string) (*service.Setting, error) {
	if v, ok := r.values[key]; ok {
		return &service.Setting{Key: key, Value: vplaceholder, nil
placeholder
...
placeholder)
	settingService := service.NewSettingService(settingRepo, &config.Config{placeholder)
...
func (r *stubAccountRepoForHandler) Create(context.Context, *service.Account) error { return nil placeholder
...
func (r *stubAccountRepoForHandler) ListCRSAccountIDs(context.Context) (map[string]int64, error) {
	return nil, nil
placeholder
func (r *stubAccountRepoForHandler) Update(context.Context, *service.Account) error { return nil placeholder
func (r *stubAccountRepoForHandler) Delete(context.Context, int64) error            { return nil placeholder
...
func (r *stubAccountRepoForHandler) ListByPlatform(context.Context, string) ([]service.Account, error) {
	return nil, nil
placeholder
func (r *stubAccountRepoForHandler) UpdateLastUsed(context.Context, int64) error { return nil placeholder
func (r *stubAccountRepoForHandler) BatchUpdateLastUsed(context.Context, map[int64]time.Time) error {
	return nil
placeholder
func (r *stubAccountRepoForHandler) SetError(context.Context, int64, string) error { return nil placeholder
func (r *stubAccountRepoForHandler) ClearError(context.Context, int64) error       { return nil placeholder
...
func (r *stubAccountRepoForHandler) AutoPauseExpiredAccounts(context.Context, time.Time) (int64, error) {
	return 0, nil
placeholder
func (r *stubAccountRepoForHandler) BindGroups(context.Context, int64, []int64) error { return nil placeholder
...
func (r *stubAccountRepoForHandler) SetTempUnschedulable(context.Context, int64, time.Time, string) error {
	return nil
placeholder
func (r *stubAccountRepoForHandler) ClearTempUnschedulable(context.Context, int64) error { return nil placeholder
func (r *stubAccountRepoForHandler) ClearRateLimit(context.Context, int64) error         { return nil placeholder
func (r *stubAccountRepoForHandler) ClearAntigravityQuotaScopes(context.Context, int64) error {
	return nil
placeholder
func (r *stubAccountRepoForHandler) ClearModelRateLimits(context.Context, int64) error { return nil placeholder
...
func (s *stubSoraClientForHandler) Enabled() bool { return true placeholder
...
func TestProcessGeneration_SelectAccountError(t *testing.T) {
	...
	accountRepo := &stubAccountRepoForHandler{accounts: nilplaceholder
	gatewayService := newMinimalGatewayService(accountRepo)
	h := &SoraClientHandler{genService: genService, gatewayService: gatewayServiceplaceholder
...
func TestProcessGeneration_SoraGatewayServiceNil(t *testing.T) {
	...
	accountRepo := &stubAccountRepoForHandler{
		accounts: []service.Account{
			{ID: 1, Platform: service.PlatformSora, Status: service.StatusActive, Schedulable: trueplaceholder,
	placeholder,
placeholder
	gatewayService := newMinimalGatewayService(accountRepo)
...
placeholder
	gatewayService := newMinimalGatewayService(accountRepo)
	soraClient := &stubSoraClientForHandler{
		videoStatus: &service.SoraVideoTaskStatus{Status: "failed", ErrorMsg: "reject"placeholder,
placeholder
	soraGatewayService := newMinimalSoraGatewayService(soraClient)
	h := &SoraClientHandler{
		genService:         genService,
...
placeholder
	...
	soraClient := &stubSoraClientForHandler{
		videoStatus: &service.SoraVideoTaskStatus{
			Status: "completed",
			URLs:   []string{"https://example.com/video.mp4"placeholder,
	placeholder,
placeholder
	soraGatewayService := newMinimalSoraGatewayService(soraClient)
	h := &SoraClientHandler{
...
placeholder
	...
	soraClient := &stubSoraClientForHandler{
		videoStatus: &service.SoraVideoTaskStatus{
			Status: "completed",
			URLs:   []string{sourceServer.URL + "/video.mp4"placeholder,
	placeholder,
placeholder
	soraGatewayService := newMinimalSoraGatewayService(soraClient)
	s3Storage := newS3StorageForHandler(fakeS3.URL)
...
func TestCleanupStoredMedia_LocalPath(t *testing.T) {
	...
	h := &SoraClientHandler{placeholder
	h.cleanupStoredMedia(context.Background(), service.SoraStorageTypeLocal, nil, []string{"/tmp/test.mp4"placeholder)
placeholder
...
placeholder
	...
	repo.gens[1] = &service.SoraGeneration{
		...
		MediaURLs:   []string{"video/test.mp4"placeholder,
placeholder
	genService := service.NewSoraGenerationService(repo, nil, nil)
	h := &SoraClientHandler{genService: genService, mediaStorage: mediaStorageplaceholder
...
func TestDeleteGeneration_DeleteError(t *testing.T) {
	// repo.Delete 出错
	repo := newStubSoraGenRepo()
	repo.gens[1] = &service.SoraGeneration{ID: 1, UserID: 1, Status: "completed", StorageType: "upstream"placeholder
...
func TestFetchUpstreamModels_NoAccounts(t *testing.T) {
	...
	accountRepo := &stubAccountRepoForHandler{accounts: nilplaceholder
	gatewayService := newMinimalGatewayService(accountRepo)
	h := &SoraClientHandler{gatewayService: gatewayServiceplaceholder
	_, err := h.fetchUpstreamModels(context.Background())
placeholder
...
func TestFetchUpstreamModels_NonAPIKeyAccount(t *testing.T) {
	...
	accountRepo := &stubAccountRepoForHandler{
		accounts: []service.Account{
			{ID: 1, Type: "oauth", Platform: service.PlatformSora, Status: service.StatusActive, Schedulable: trueplaceholder,
	placeholder,
placeholder
	gatewayService := newMinimalGatewayService(accountRepo)
	h := &SoraClientHandler{gatewayService: gatewayServiceplaceholder
...
func TestFetchUpstreamModels_MissingAPIKey(t *testing.T) {
	...
	accountRepo := &stubAccountRepoForHandler{
		...
		placeholder"base_url": "https://sora.test"placeholderplaceholder,
	placeholder,
placeholder
	gatewayService := newMinimalGatewayService(accountRepo)
	h := &SoraClientHandler{gatewayService: gatewayServiceplaceholder
...
func TestFetchUpstreamModels_MissingBaseURL_FallsBackToDefault(t *testing.T) {
	...
	accountRepo := &stubAccountRepoForHandler{
		accounts: []service.Account{
			{ID: 1, Type: service.AccountTypeAPIKey, Platform: service.PlatformSora, Status: service.StatusActive, Schedulable: true,
		placeholder"api_key": "sk-test"placeholderplaceholder,
	placeholder,
placeholder
	gatewayService := newMinimalGatewayService(accountRepo)
	h := &SoraClientHandler{gatewayService: gatewayServiceplaceholder
...
placeholder))
	...
	accountRepo := &stubAccountRepoForHandler{
		accounts: []service.Account{
			{ID: 1, Type: service.AccountTypeAPIKey, Platform: service.PlatformSora, Status: service.StatusActive, Schedulable: true,
		placeholder"api_key": "sk-test", "base_url": ts.URLplaceholderplaceholder,
	placeholder,
placeholder
	gatewayService := newMinimalGatewayService(accountRepo)
	h := &SoraClientHandler{gatewayService: gatewayServiceplaceholder
...
func TestFetchUpstreamModels_UpstreamReturnsEmptyList(t *testing.T) {
	...
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusOK)
		_, _ = w.Write([]byte(`{"data":[]placeholder`))
placeholder))
	defer ts.Close()
...
func TestFetchUpstreamModels_Success(t *testing.T) {
	...
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.WriteHeader(http.StatusOK)
		_, _ = w.Write([]byte(`{"data":[{"id":"sora2-landscape-10s"placeholder,{"id":"sora2-portrait-10s"placeholder,{"id":"sora2-landscape-15s"placeholder,{"id":"gpt-image"placeholder]placeholder`))
placeholder))
	defer ts.Close()
...
func TestFetchUpstreamModels_UnrecognizedModels(t *testing.T) {
	...
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusOK)
		_, _ = w.Write([]byte(`{"data":[{"id":"unknown-model-1"placeholder,{"id":"unknown-model-2"placeholder]placeholder`))
placeholder))
	defer ts.Close()
...
func TestGetModelFamilies_CachesUpstreamResult(t *testing.T) {
	...
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusOK)
		_, _ = w.Write([]byte(`{"data":[{"id":"sora2-landscape-10s"placeholder,{"id":"gpt-image"placeholder]placeholder`))
placeholder))
	defer ts.Close()
...
func TestGetModelFamilies_ExpiredCacheRefreshes(t *testing.T) {
	...
	h := &SoraClientHandler{
		cachedFamilies:     []service.SoraModelFamily{{ID: "old"placeholderplaceholder,
...
func TestGenerate_CheckQuotaNonQuotaError(t *testing.T) {
	...
	body := `{"model":"sora2-landscape-10s","prompt":"test"placeholder`
...
placeholder
	...
	h.cleanupStoredMedia(context.Background(), service.SoraStorageTypeLocal, nil, []string{"nonexistent/file.mp4"placeholder)
placeholder
...
placeholder
	...
	repo.gens[1] = &service.SoraGeneration{
		...
		MediaURLs:   []string{"nonexistent/video.mp4"placeholder,
placeholder
	genService := service.NewSoraGenerationService(repo, nil, nil)
	h := &SoraClientHandler{genService: genService, mediaStorage: mediaStorageplaceholder
