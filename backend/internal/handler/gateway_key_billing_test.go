...
func TestGatewayHandlerKeyBillingInfoUsesUserOverride(t *testing.T) {
	...
	apiKey := &service.APIKey{
		UserID:  11,
		GroupID: &groupID,
		Group:   &service.Group{ID: groupID, RateMultiplier: 0.75placeholder,
placeholder
	c, w := newKeyBillingContext(apiKey)
	repo := &keyBillingUserGroupRateRepo{rate: &userRateplaceholder
...
placeholder)
	...
	t.Run("ungrouped API key", func(t *testing.T) {
		c, w := newKeyBillingContext(&service.APIKey{placeholder)
		newKeyBillingHandler(nil).KeyBillingInfo(c)
...
placeholder)
	...
	t.Run("missing billing service", func(t *testing.T) {
		...
		c, w := newKeyBillingContext(&service.APIKey{
			UserID:  11,
			GroupID: &groupID,
			Group:   &service.Group{ID: groupID, RateMultiplier: 1placeholder,
	placeholder)
		(&GatewayHandler{placeholder).KeyBillingInfo(c)
...
placeholder)
	...
	t.Run("rate lookup failure matches billing fallback", func(t *testing.T) {
		...
		c, w := newKeyBillingContext(&service.APIKey{
			...
			Group:   &service.Group{ID: groupID, RateMultiplier: 1placeholder,
	placeholder)
		newKeyBillingHandler(&keyBillingUserGroupRateRepo{err: errors.New("database password leaked")placeholder).KeyBillingInfo(c)
		require.Equal(t, http.StatusOK, w.Code)
		var got keyBillingInfoResponse
		require.NoError(t, json.Unmarshal(w.Body.Bytes(), &got))
...
func TestGatewayHandlerKeyBillingInfoSharesBillingResolverCacheByPlatform(t *testing.T) {
	for _, tc := range []struct {
		...
		openAI   bool
placeholder{
		{name: "anthropic", platform: service.PlatformAnthropicplaceholder,
		{name: "openai", platform: service.PlatformOpenAI, openAI: trueplaceholder,
		{name: "grok", platform: service.PlatformGrok, openAI: trueplaceholder,
placeholder {
		t.Run(tc.name, func(t *testing.T) {
			groupID := int64(7)
			oldRate, newRate := 0.5, 1.8
			repo := &keyBillingUserGroupRateRepo{rate: &oldRateplaceholder
			gatewayService := newKeyBillingGatewayService(repo)
			openAIGatewayService := newKeyBillingOpenAIGatewayService(repo)
			h := &GatewayHandler{
