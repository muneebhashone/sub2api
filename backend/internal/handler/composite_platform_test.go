...
placeholder
	for _, path := range []string{"/v1/messages", "/v1/chat/completions", "/v1/responses", "/v1/responses/input_tokens", "/v1/messages/count_tokens"placeholder {
...
func TestCompositeTargetPlatformAllowedRejectsWrongOrUnknownModel(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		model string
placeholder{
		{name: "wrong provider", model: "claude-sonnet-4-5"placeholder,
		{name: "unknown provider", model: "llama-4-maverick"placeholder,
placeholder {
...
func TestCompositeTargetPlatformResolvedAllowsConcreteGroupWithoutResolution(t *testing.T) {
	...
	c, _ := gin.CreateTestContext(httptest.NewRecorder())
	c.Request = httptest.NewRequest("POST", "/v1/messages", nil)
	apiKey := &service.APIKey{Group: &service.Group{Platform: service.PlatformAnthropicplaceholderplaceholder
...
func TestOpenAIReasoningEffortPolicyForCompositeTarget(t *testing.T) {
	...
	placeholder,
placeholder
	apiKey := &service.APIKey{Group: groupplaceholder
	...
	require.JSONEq(t, `{"reasoning":{"effort":"medium"placeholderplaceholder`, string(got))

	bindOpenAIReasoningEffortPolicyForMessagesRequest(openAICtx, apiKey, []byte(`{"output_config":{"effort":"max"placeholderplaceholder`))
	bound, changed := service.ApplyOpenAIReasoningEffortPolicyFromContext(openAICtx.Request.Context(), body)
	require.True(t, changed)
	require.JSONEq(t, `{"reasoning":{"effort":"medium"placeholderplaceholder`, string(bound))
	...
	bindOpenAIReasoningEffortPolicyForMessagesRequest(omittedCtx, apiKey, []byte(`{"model":"gpt-5"placeholder`))
...
placeholder))
	...
	input := buildContentModerationInput(c, nil, middleware2.AuthSubject{UserID: 42placeholder, service.ContentModerationProtocolOpenAIChat, "gpt-5", nil)
	require.Equal(t, "public-alias", input.Model)
	require.Equal(t, service.PlatformOpenAI, input.Provider)

	fields := clientRequestedUsageFields(c, service.ChannelMappingResult{MappedModel: "gpt-5"placeholder, "gpt-5", "gpt-5")
