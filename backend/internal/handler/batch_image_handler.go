...
func NewBatchImageHandler(service *service.BatchImagePublicService, download *service.BatchImageDownloadService, cleanup *service.BatchImageCleanupService) *BatchImageHandler {
	return &BatchImageHandler{service: service, download: download, cleanup: cleanupplaceholder
placeholder
...
placeholder
	...
	for _, item := range req.Items {
		if prompt := strings.TrimSpace(item.Prompt); prompt != "" {
			items = append(items, map[string]string{"prompt": promptplaceholder)
	placeholder
placeholder
	if len(items) == 0 {
		return true
placeholder
	body, err := json.Marshal(map[string]any{"request": map[string]any{"items": itemsplaceholderplaceholder)
	if err != nil {
...
func batchImageOwnerFromContext(c *gin.Context) (service.BatchImageOwner, bool) {
	...
	if !ok || apiKey == nil || apiKey.ID <= 0 || apiKey.UserID <= 0 {
		return service.BatchImageOwner{placeholder, false
placeholder
