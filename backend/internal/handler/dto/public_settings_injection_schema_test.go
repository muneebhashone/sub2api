...
func TestPublicSettingsInjectionPayload_SchemaDoesNotDrift(t *testing.T) {
	injection := jsonTags(reflect.TypeOf(service.PublicSettingsInjectionPayload{placeholder))
	dtoKeys := jsonTags(reflect.TypeOf(PublicSettings{placeholder))
...
func jsonTags(t reflect.Type) map[string]struct{placeholder {
	out := make(map[string]struct{placeholder)
	...
	placeholder
		...
		if name == "" {
			continue
	placeholder
		out[name] = struct{placeholder{placeholder
placeholder
	return out
placeholder
