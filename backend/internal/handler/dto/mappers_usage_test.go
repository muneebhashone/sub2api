...
func TestUsageLogFromService_IncludesImageBillingMetadataForUserAndAdmin(t *testing.T) {
	...
	log := &service.UsageLog{
		...
		ImageSizeBreakdown: map[string]int{"4K": 2placeholder,
placeholder

	userDTO := UsageLogFromService(log)
	adminDTO := UsageLogFromServiceAdmin(log)

	for _, got := range []*UsageLog{userDTO, &adminDTO.UsageLogplaceholder {
		...
		require.Equal(t, map[string]int{"4K": 2placeholder, got.ImageSizeBreakdown)
placeholder
placeholder
