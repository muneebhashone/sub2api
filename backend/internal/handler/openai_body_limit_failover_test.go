...
func TestOpenAIBodyLimitFailoverExhausted_ReturnsRedactedJSON413(t *testing.T) {
	...
	(&OpenAIGatewayHandler{placeholder).handleFailoverExhausted(c, bodyLimitFailoverTestError(), false)
...
func TestOpenAIBodyLimitFailoverExhausted_ReturnsRedactedResponsesSSE(t *testing.T) {
	...
	(&OpenAIGatewayHandler{placeholder).handleFailoverExhausted(c, bodyLimitFailoverTestError(), true)
...
func bodyLimitFailoverTestError() *service.UpstreamFailoverError {
	return &service.UpstreamFailoverError{
		...
		ResponseBody:      []byte(`{"error":{"message":"proxy limit secret=must-not-leak"placeholderplaceholder`),
