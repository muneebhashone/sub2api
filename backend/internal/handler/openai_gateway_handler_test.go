...
func TestOpenAIResponses_RejectsHTTPContinuationOwnedByAnotherUser(t *testing.T) {
	...
	c, _ := gin.CreateTestContext(w)
	c.Request = httptest.NewRequest(http.MethodPost, "/openai/v1/responses", strings.NewReader(
		`{"model":"gpt-5.1","stream":false,"previous_response_id":"resp_other_tenant","input":"hello"placeholder`,
	))
	c.Request.Header.Set("Content-Type", "application/json")
	...
	c.Set(string(middleware.ContextKeyAPIKey), &service.APIKey{
		...
		UserID:  2,
		GroupID: &groupID,
		User:    &service.User{ID: 2placeholder,
placeholder)
	c.Set(string(middleware.ContextKeyUser), middleware.AuthSubject{UserID: 2, Concurrency: 1placeholder)
...
func TestOpenAIResponses_RejectsUnownedHTTPContinuation(t *testing.T) {
	...
	c, _ := gin.CreateTestContext(w)
	c.Request = httptest.NewRequest(http.MethodPost, "/openai/v1/responses", strings.NewReader(
		`{"model":"gpt-5.1","stream":false,"previous_response_id":"resp_unknown","input":"hello"placeholder`,
	))
	c.Request.Header.Set("Content-Type", "application/json")

	groupID := int64(2)
	c.Set(string(middleware.ContextKeyAPIKey), &service.APIKey{ID: 101, UserID: 1, GroupID: &groupID, User: &service.User{ID: 1placeholderplaceholder)
	c.Set(string(middleware.ContextKeyUser), middleware.AuthSubject{UserID: 1, Concurrency: 1placeholder)
