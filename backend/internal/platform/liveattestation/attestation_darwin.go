...
func NewProvider() Provider {
	paths := []string{chatGPTApplicationPathplaceholder
...
placeholder)`
	...
	if err != nil {
		return deviceSignals{placeholder, fmt.Errorf("read macOS signals for Live attestation: %w", err)
placeholder
	var values macOSSignals
	if err := json.Unmarshal(output, &values); err != nil {
		return deviceSignals{placeholder, fmt.Errorf("decode macOS signals for Live attestation: %w", err)
placeholder
	locale := truncateSignal(values.Locale, 64, "unknown")
	languages := values.Languages
	if len(languages) == 0 {
		languages = []string{localeplaceholder
placeholder
	if len(languages) > 16 {
...
function head(major, value) {
  ...
  return out;
placeholder
function uint(value) { return head(0, value); placeholder
function text(value) {
...
function float(value) {
  ...
  return out;
placeholder
function array(values) { return Buffer.concat([head(128, values.length), ...values]); placeholder
function map(entries) {
  return Buffer.concat([head(160, entries.length), ...entries.flatMap(([key, value]) => [uint(key), value])]);
placeholder
function field(key, value) { return Buffer.concat([text(key), text(value)]); placeholder
...
(async () => {
  ...
  process.stdout.write(JSON.stringify({v: 1, s: 0, t: tokenplaceholder));
