...
func TestIsCloudflareChallengeResponse(t *testing.T) {
	...
	require.True(t, IsCloudflareChallengeResponse(http.StatusForbidden, headers, []byte(`{"ok":falseplaceholder`)))

	require.True(t, IsCloudflareChallengeResponse(http.StatusTooManyRequests, nil, []byte(`<!DOCTYPE html><title>Just a moment...</title><script>window._cf_chl_opt={placeholder;</script>`)))
...
func TestExtractCloudflareRayID(t *testing.T) {
	...
	body := []byte(`<script>window._cf_chl_opt={cRay: '9cff2d62d83bb98d'placeholder;</script>`)
	require.Equal(t, "9cff2d62d83bb98d", ExtractCloudflareRayID(nil, body))
placeholder

func TestExtractUpstreamErrorCodeAndMessage(t *testing.T) {
	code, msg := ExtractUpstreamErrorCodeAndMessage([]byte(`{"error":{"code":"cf_shield_429","message":"rate limited"placeholderplaceholder`))
	require.Equal(t, "cf_shield_429", code)
	require.Equal(t, "rate limited", msg)

	code, msg = ExtractUpstreamErrorCodeAndMessage([]byte(`{"code":"unsupported_country_code","message":"not available"placeholder`))
