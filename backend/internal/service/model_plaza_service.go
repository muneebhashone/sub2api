			...
			for j := range supported {
				...
				if pg.Platform == PlatformComposite {
					if !isConcreteRequestPlatform(m.Platform) {
						continue
				placeholder
			placeholder else if m.Platform != pg.Platform {
					continue
			placeholder
				key := modelKey{platform: m.Platform, name: m.Nameplaceholder
...
func plazaPricingFromSchedule(raw *ChannelModelPricing, sched *ContextPricingSchedule) *ChannelModelPricing {
	out := &ChannelModelPricing{BillingMode: BillingModeTokenplaceholder
	if raw != nil {
...
placeholder
	tiers := []struct {
		...
		groupPrice *float64
placeholder{
		{"1K", g.ImagePrice1Kplaceholder,
		{"2K", g.ImagePrice2Kplaceholder,
		{"4K", g.ImagePrice4Kplaceholder,
placeholder
	clone := *p
...
placeholder
	...
	placeholder
		if s.resolver != nil {
			sched, schedErr := s.billingService.ResolveContextPricingSchedule(ctx, s.resolver, ContextPricingScheduleInput{Model: modelNameplaceholder)
