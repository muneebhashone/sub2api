...
const openAIInvalidFunctionParametersBody = `{"error":{` +
	...
	`"code":"invalid_function_parameters"placeholderplaceholder`
...
placeholderID: 1, Platform: PlatformOpenAI, Type: AccountTypeOAuth, Name: "acct"placeholder
placeholder
...
func TestHandleErrorResponse_Deterministic400WithoutUpstreamMetadata(t *testing.T) {
	...
	_, err := svc.handleErrorResponse(
		context.Background(),
		newOpenAIUpstreamErrorResponse(http.StatusBadRequest, `{"error":{"message":"Invalid 'input': expected an array."placeholderplaceholder`),
		c, newOpenAIUpstreamErrorTestAccount(), nil,
	)
...
placeholder{
		...
		{"not_found", http.StatusNotFound, `{"error":{"message":"Unknown request URL"placeholderplaceholder`,
			http.StatusBadGateway, "upstream_error", "Upstream request failed"placeholder,
		{"unprocessable", http.StatusUnprocessableEntity, `{"error":{"message":"Invalid schema for field messages"placeholderplaceholder`,
			http.StatusBadGateway, "upstream_error", "Upstream request failed"placeholder,
		// 401/402/403 是网关运营方的凭据/账单问题，必须继续对客户端屏蔽上游账号状态。
		{"unauthorized", http.StatusUnauthorized, `{"error":{"message":"Incorrect API key provided: sk-abc"placeholderplaceholder`,
			http.StatusBadGateway, "upstream_error", "Upstream authentication failed, please contact administrator"placeholder,
		{"forbidden", http.StatusForbidden, `{"error":{"message":"Your account is deactivated"placeholderplaceholder`,
			http.StatusBadGateway, "upstream_error", "Upstream access forbidden, please contact administrator"placeholder,
		// 429 保持独立映射。
		{"rate_limited", http.StatusTooManyRequests, `{"error":{"message":"Rate limit reached"placeholderplaceholder`,
			http.StatusTooManyRequests, "rate_limit_error", "Upstream rate limit exceeded, please retry later"placeholder,
placeholder
	...
	placeholder,
		{
			...
			body:        `{"error":{"type":"invalid_prompt","message":"blocked"placeholderplaceholder`,
			upstreamMsg: "blocked",
	...
	placeholder,
		{
			...
			body:        `{"error":{"message":"failed for key=secret123"placeholderplaceholder`,
