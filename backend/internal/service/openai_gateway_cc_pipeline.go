...
func (s *OpenAIGatewayService) readCCUpstreamJSONResponse(
	...
	placeholder
		return nil, OpenAIUsage{placeholder, fmt.Errorf("read upstream body: %w", err)
placeholder

	var ccResp apicompat.ChatCompletionsResponse
	if err := json.Unmarshal(respBody, &ccResp); err != nil {
		writeError(c, http.StatusBadGateway, "api_error", "Failed to parse upstream response")
		return nil, OpenAIUsage{placeholder, fmt.Errorf("parse chat completions response: %w", err)
placeholder
