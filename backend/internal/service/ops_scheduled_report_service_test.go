...
placeholder
	overview := &OpsDashboardOverview{
		...
		Duration:                     OpsPercentiles{P50: &latencyP50, P99: &latencyP99placeholder,
		TTFT:                         OpsPercentiles{P50: &ttftP50, P99: &ttftP99placeholder,
		QPS:                          OpsRateSummary{Current: 0, Peak: 1.2, Avg: 0placeholder,
		TPS:                          OpsRateSummary{Current: 0, Peak: 133421.2, Avg: 1406.8placeholder,
placeholder
	...
	legacyTemplate, err := renderNotificationEmail(
		...
		"Report",
		`<section>{{report_htmlplaceholderplaceholder</section>`,
		variables,
		map[string]string{"report_html": `<h2>generated summary</h2>`placeholder,
	)
placeholder
...
func TestOpsScheduledReportLegacyTemplateReceivesSummaryHTML(t *testing.T) {
	...
	_, err := notificationService.UpdateTemplate(
		...
		NotificationEmailEventOpsScheduledReport,
		"en",
		"Legacy report {{report_nameplaceholderplaceholder",
		`<section data-template="legacy">{{report_htmlplaceholderplaceholder</section>`,
	)
placeholder

	svc := &OpsScheduledReportService{
		opsService:   &OpsService{opsRepo: &opsRepoMock{placeholderplaceholder,
...
placeholder
	report := &opsScheduledReport{
		...
		ReportType: "daily_summary",
		TimeRange:  24 * time.Hour,
		Recipients: []string{"ops@example.com"placeholder,
placeholder
