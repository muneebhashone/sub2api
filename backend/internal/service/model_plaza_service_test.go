...
placeholder
	return NewModelPlazaService(repo, &stubGroupRepoForAvailable{activeGroups: groupsplaceholder, pricing, nil, nil)
placeholder
...
func TestListPlazaGroups_GroupCentricAggregation(t *testing.T) {
	...
	channels := []Channel{
		plazaPricedChannel(1, "chA", []int64{10placeholder, "anthropic", "claude-sonnet"),
		plazaPricedChannel(2, "chB", []int64{10placeholder, "anthropic", "claude-opus"),
placeholder
	groups := []Group{
		{ID: 10, Name: "g-main", Description: "desc", Platform: "anthropic", RateMultiplier: 1placeholder,
		{ID: 20, Name: "g-empty", Platform: "anthropic", RateMultiplier: 0.5placeholder,
placeholder
...
func TestListPlazaGroups_DedupFirstWinsWithPricingUpgrade(t *testing.T) {
	...
	unpriced := Channel{
		ID: 1, Name: "alpha", Status: StatusActive, GroupIDs: []int64{10placeholder,
		// mapping-only → SupportedModels 产出无定价条目
		ModelMapping: map[string]map[string]string{
			"anthropic": {"claude-sonnet": "claude-sonnet"placeholder,
	placeholder,
placeholder
	priced := plazaPricedChannel(2, "beta", []int64{10placeholder, "anthropic", "claude-sonnet")
	groups := []Group{{ID: 10, Name: "g", Platform: "anthropic", RateMultiplier: 1placeholderplaceholder

	// alpha(无价)按名称序先于 beta(有价):先见者无价,应被有价条目升级。
	svc := newPlazaService([]Channel{priced, unpricedplaceholder, groups, nil)
	out, err := svc.ListGroups(context.Background())
placeholder
	require.Len(t, out, 1)
...
func TestListPlazaGroups_PlatformIsolation(t *testing.T) {
	...
	ch := Channel{
		ID: 1, Name: "multi", Status: StatusActive, GroupIDs: []int64{10, 20placeholder,
		ModelPricing: []ChannelModelPricing{
			{Platform: "anthropic", Models: []string{"claude-sonnet"placeholder, InputPrice: testPtrFloat64(3e-6)placeholder,
			{Platform: "openai", Models: []string{"gpt-5"placeholder, InputPrice: testPtrFloat64(2e-6)placeholder,
	placeholder,
placeholder
	groups := []Group{
		{ID: 10, Name: "g-claude", Platform: "anthropic", RateMultiplier: 1placeholder,
		{ID: 20, Name: "g-gpt", Platform: "openai", RateMultiplier: 1placeholder,
placeholder
	svc := newPlazaService([]Channel{chplaceholder, groups, nil)
	out, err := svc.ListGroups(context.Background())
placeholder
	require.Len(t, out, 2)
	byName := map[string][]PlazaModel{placeholder
	for _, g := range out {
...
func TestListPlazaGroups_CompositeIncludesConfiguredConcretePlatforms(t *testing.T) {
	...
	openAIPrice := 2e-6
	ch := Channel{
		ID: 1, Name: "multi", Status: StatusActive, GroupIDs: []int64{10placeholder,
		ModelPricing: []ChannelModelPricing{
			{Platform: PlatformAnthropic, Models: []string{"shared-model"placeholder, InputPrice: &anthropicPriceplaceholder,
			{Platform: PlatformOpenAI, Models: []string{"shared-model"placeholder, InputPrice: &openAIPriceplaceholder,
			{Platform: "", Models: []string{"empty-platform"placeholderplaceholder,
			{Platform: PlatformComposite, Models: []string{"nested-composite"placeholderplaceholder,
			{Platform: "unknown-platform", Models: []string{"unknown-platform"placeholderplaceholder,
	placeholder,
placeholder
	groups := []Group{{ID: 10, Name: "composite", Platform: PlatformComposite, RateMultiplier: 1placeholderplaceholder

	out, err := newPlazaService([]Channel{chplaceholder, groups, nil).ListGroups(context.Background())
...
func TestListPlazaGroups_CompositeAndOrdinaryGroupsDoNotLeakPlatforms(t *testing.T) {
	ch := Channel{
		ID: 1, Name: "multi", Status: StatusActive, GroupIDs: []int64{10, 20placeholder,
		ModelPricing: []ChannelModelPricing{
			{Platform: PlatformAnthropic, Models: []string{"claude-sonnet"placeholder, InputPrice: testPtrFloat64(3e-6)placeholder,
			{Platform: PlatformOpenAI, Models: []string{"gpt-5"placeholder, InputPrice: testPtrFloat64(2e-6)placeholder,
	placeholder,
placeholder
	groups := []Group{
		{ID: 10, Name: "anthropic-only", Platform: PlatformAnthropic, RateMultiplier: 1placeholder,
		{ID: 20, Name: "composite", Platform: PlatformComposite, RateMultiplier: 1placeholder,
placeholder
...
placeholder
	require.Len(t, out, 2)
	byName := map[string]PlazaGroup{placeholder
...
placeholderplaceholder, byName["anthropic-only"].Models)
	require.Len(t, byName["composite"].Models, 2)
	require.Equal(t, []string{"claude-sonnet", "gpt-5"placeholder, []string{
		byName["composite"].Models[0].Name,
		byName["composite"].Models[1].Name,
placeholder)
	require.Equal(t, []string{PlatformAnthropic, PlatformOpenAIplaceholder, []string{
...
func TestListPlazaGroups_InactiveChannelSkipped(t *testing.T) {
	inactive := plazaPricedChannel(1, "off", []int64{10placeholder, "anthropic", "claude-sonnet")
	inactive.Status = "inactive"
	groups := []Group{{ID: 10, Name: "g", Platform: "anthropic", RateMultiplier: 1placeholderplaceholder
	svc := newPlazaService([]Channel{inactiveplaceholder, groups, nil)
	out, err := svc.ListGroups(context.Background())
placeholder
	require.Empty(t, out)
placeholder
...
func TestListPlazaGroups_SortedByRateMultiplierAsc(t *testing.T) {
	channels := []Channel{
		plazaPricedChannel(1, "ch", []int64{10, 20, 30placeholder, "anthropic", "claude-sonnet"),
placeholder
	groups := []Group{
		{ID: 10, Name: "b-standard", Platform: "anthropic", RateMultiplier: 1placeholder,
		{ID: 20, Name: "a-standard", Platform: "anthropic", RateMultiplier: 1placeholder,
		{ID: 30, Name: "cheap", Platform: "anthropic", RateMultiplier: 0.5placeholder,
placeholder
	svc := newPlazaService(channels, groups, nil)
	out, err := svc.ListGroups(context.Background())
placeholder
...
func TestListPlazaGroups_OfficialPricingFill(t *testing.T) {
	...
	placeholder,
		"token-absent": {Mode: "image_generation", TokenPricingAbsent: true, OutputCostPerImage: 0.04placeholder,
placeholder)
	channels := []Channel{
		plazaPricedChannel(1, "ch", []int64{10placeholder, "anthropic", "claude-sonnet", "unknown-model", "token-absent"),
placeholder
	groups := []Group{{ID: 10, Name: "g", Platform: "anthropic", RateMultiplier: 1placeholderplaceholder
	svc := newPlazaService(channels, groups, pricingSvc)
	// 官方价与计费同源：需要计费服务与解析器（官方参考不查渠道，解析器无需渠道服务）。
	svc.billingService = NewBillingService(&config.Config{placeholder, pricingSvc)
...
func TestListPlazaGroups_GroupImagePriceOverridesChannelPricing(t *testing.T) {
	...
	imgPrice := 0.02
	channels := []Channel{{
		ID: 1, Name: "img-ch", Status: StatusActive, GroupIDs: []int64{10, 20placeholder,
		ModelPricing: []ChannelModelPricing{{
			Platform:        "openai",
			Models:          []string{"gpt-image-2"placeholder,
			BillingMode:     BillingModeImage,
			PerRequestPrice: &perReq,
			Intervals:       []PricingInterval{{TierLabel: "4K", PerRequestPrice: &tier4Kplaceholderplaceholder,
placeholder
placeholderplaceholder
	groups := []Group{
		{ID: 10, Name: "g-media", Platform: "openai", RateMultiplier: 1,
			ImagePrice1K: &imgPrice, ImageRateIndependent: true, ImageRateMultiplier: 1placeholder,
		{ID: 20, Name: "g-plain", Platform: "openai", RateMultiplier: 0.1placeholder,
placeholder
	svc := newPlazaService(channels, groups, nil)
	out, err := svc.ListGroups(context.Background())
placeholder
...
placeholder
	...
	tierPrices := map[string]float64{placeholder
...
func TestListPlazaGroups_GroupImagePriceIgnoredForNonImageModes(t *testing.T) {
	// token 模式定价不受分组图片价影响。
	imgPrice := 0.02
	channels := []Channel{plazaPricedChannel(1, "ch", []int64{10placeholder, "openai", "gpt-5")placeholder
	groups := []Group{{ID: 10, Name: "g", Platform: "openai", RateMultiplier: 1, ImagePrice1K: &imgPriceplaceholderplaceholder
	svc := newPlazaService(channels, groups, nil)
	out, err := svc.ListGroups(context.Background())
placeholder
	require.Len(t, out, 1)
...
placeholder
	svc := NewModelPlazaService(repo, &stubGroupRepoForAvailable{placeholder, nil, nil, nil)
	out, err := svc.ListGroups(context.Background())
	require.Nil(t, out)
	require.ErrorIs(t, err, sentinel)

	svc2 := NewModelPlazaService(
		&mockChannelRepository{listAllFn: func(ctx context.Context) ([]Channel, error) { return nil, nil placeholderplaceholder,
...
placeholder
	...
	return NewModelPlazaService(repo, &stubGroupRepoForAvailable{activeGroups: groupsplaceholder, catalog, bs, NewModelPricingResolver(cs, bs))
placeholder
...
func TestListGroups_TokenLadderFollowsGroupToggle(t *testing.T) {
	...
	channels := []Channel{{
		ID: 1, Name: "ch", Status: StatusActive, GroupIDs: []int64{10, 20placeholder,
		ModelPricing: []ChannelModelPricing{{Platform: PlatformOpenAI, Models: []string{"gpt-5.4"placeholder, BillingMode: BillingModeTokenplaceholderplaceholder,
placeholderplaceholder
	groups := []Group{
		{ID: 10, Name: "on", Platform: PlatformOpenAI, RateMultiplier: 1, LongContextPricingEnabled: trueplaceholder,
		{ID: 20, Name: "off", Platform: PlatformOpenAI, RateMultiplier: 2, LongContextPricingEnabled: falseplaceholder,
placeholder
	svc := newPlazaServiceWithBilling(channels, groups, map[int64]string{10: PlatformOpenAI, 20: PlatformOpenAIplaceholder, nil)
	out, err := svc.ListGroups(context.Background())
placeholder
	require.Len(t, out, 2)
	...
	for _, m := range []PlazaModel{onModel, offModelplaceholder {
...
func TestListGroups_GeminiLegacyRuleShownAsMarginal(t *testing.T) {
	channels := []Channel{{
		ID: 1, Name: "ch", Status: StatusActive, GroupIDs: []int64{10placeholder,
		ModelMapping: map[string]map[string]string{PlatformGemini: {"gemini-2.5-pro": "gemini-2.5-pro"placeholderplaceholder,
placeholderplaceholder
	groups := []Group{{ID: 10, Name: "g", Platform: PlatformGemini, RateMultiplier: 1, LongContextPricingEnabled: trueplaceholderplaceholder
	svc := newPlazaServiceWithBilling(channels, groups, map[int64]string{10: PlatformGeminiplaceholder, geminiCatalogStub())
	out, err := svc.ListGroups(context.Background())
placeholder
	require.Len(t, out, 1)
	m := out[0].Models[0]
...
func TestListGroups_GroupTokenCardOverridesChannelPricing(t *testing.T) {
	channels := []Channel{plazaPricedChannel(1, "ch", []int64{10placeholder, PlatformAnthropic, "claude-sonnet-4")placeholder
	groups := []Group{{
		ID: 10, Name: "g", Platform: PlatformAnthropic, RateMultiplier: 1, LongContextPricingEnabled: true,
		ModelPricing: []ChannelModelPricing{{Models: []string{"claude-sonnet-*"placeholder, BillingMode: BillingModeToken, InputPrice: testPtrFloat64(1e-6)placeholderplaceholder,
placeholderplaceholder
	svc := newPlazaServiceWithBilling(channels, groups, map[int64]string{10: PlatformAnthropicplaceholder, nil)
	out, err := svc.ListGroups(context.Background())
placeholder
	m := out[0].Models[0]
...
func TestListGroups_ImageModelKeepsTierSynthesisWithBilling(t *testing.T) {
	channels := []Channel{{
		ID: 1, Name: "ch", Status: StatusActive, GroupIDs: []int64{10placeholder,
		ModelPricing: []ChannelModelPricing{{
			Platform: PlatformOpenAI, Models: []string{"gpt-image-2"placeholder, BillingMode: BillingModeImage,
...
placeholderplaceholder
	svc := newPlazaServiceWithBilling(channels, groups, map[int64]string{10: PlatformOpenAIplaceholder, nil)
	out, err := svc.ListGroups(context.Background())
placeholder
	m := out[0].Models[0]
...
func TestListGroups_CatalogMissingStillShowsChannelFlatPricing(t *testing.T) {
	...
	channels := []Channel{plazaPricedChannel(1, "ch", []int64{10placeholder, PlatformAnthropic, "unknown-model-xyz")placeholder
	groups := []Group{{ID: 10, Name: "g", Platform: PlatformAnthropic, RateMultiplier: 1, LongContextPricingEnabled: trueplaceholderplaceholder
	svc := newPlazaServiceWithBilling(channels, groups, map[int64]string{10: PlatformAnthropicplaceholder, nil)
	out, err := svc.ListGroups(context.Background())
placeholder
	m := out[0].Models[0]
...
func TestListGroups_TimePricingPassthrough(t *testing.T) {
	channels := []Channel{{
		ID: 1, Name: "ch", Status: StatusActive, GroupIDs: []int64{10placeholder,
		ModelPricing: []ChannelModelPricing{{
			Platform: PlatformDeepseek, Models: []string{"deepseek-chat"placeholder, BillingMode: BillingModeToken,
...
placeholder
placeholderplaceholder
	groups := []Group{{ID: 10, Name: "cn", Platform: PlatformDeepseek, RateMultiplier: 1, LongContextPricingEnabled: trueplaceholderplaceholder
	svc := newPlazaServiceWithBilling(channels, groups, map[int64]string{10: PlatformDeepseekplaceholder, nil)
	out, err := svc.ListGroups(context.Background())
placeholder
	m := out[0].Models[0]
