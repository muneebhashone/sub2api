...
func TestOpenAIProfitControlVetoReason(t *testing.T) {
	...
	gateCtx := func(threshold float64) context.Context {
		return context.WithValue(context.Background(), openAIProfitControlGateCtxKey{placeholder, &openAIProfitControlGate{
			threshold: threshold,
			pricingAt: now,
	placeholder)
placeholder
...
placeholder)
	...
	t.Run("negative and non-finite rates are invalid", func(t *testing.T) {
		for _, rate := range []float64{-1, math.NaN(), math.Inf(1)placeholder {
...
func TestProfitControlSchedulerFiltersCandidates(t *testing.T) {
	...
	for _, account := range []*Account{cheap, expensive, oauthplaceholder {
...
placeholder
	cache := &upstreamCostTrackingConcurrencyCache{loadMap: map[int64]*AccountLoadInfo{
		...
		oauth.ID:     {AccountID: oauth.IDplaceholder,
placeholderplaceholder
	cfg := &config.Config{placeholder
	svc := &OpenAIGatewayService{
		accountRepo:        schedulerTestOpenAIAccountRepo{accounts: []Account{*cheap, *expensive, *oauthplaceholderplaceholder,
		cfg:                cfg,
...
placeholder)
	...
	t.Run("manually rated oauth account is admitted", func(t *testing.T) {
		...
		svc.accountRepo = schedulerTestOpenAIAccountRepo{accounts: []Account{*cheap, *expensive, *oauthplaceholderplaceholder
		ctx := profitControlTestCtx(profitControlTestGroup(groupID, 0.7, 0.1))
		selection, _, err := svc.SelectAccountWithScheduler(ctx, &groupID, "", "", "gpt-test", nil, OpenAIUpstreamTransportAny, false)
	placeholder
		require.NotNil(t, selection)
...
func TestValidateProfitControlConfig(t *testing.T) {
	...
	for _, platform := range []string{PlatformOpenAI, PlatformAnthropic, PlatformGemini, PlatformGrok, PlatformAntigravityplaceholder {
