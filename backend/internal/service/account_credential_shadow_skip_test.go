...
func TestOpenAITokenRefresherSkipsShadow(t *testing.T) {
	...
	require.False(t, r.CanRefresh(&Account{ID: 200, Platform: PlatformOpenAI, Type: AccountTypeOAuth, ParentAccountID: &pidplaceholder))
	// 普通账号：有 refresh_token → CanRefresh 应返回 true
	require.True(t, r.CanRefresh(&Account{ID: 100, Platform: PlatformOpenAI, Type: AccountTypeOAuth, Credentials: map[string]any{"refresh_token": "RT"placeholderplaceholder))
placeholder
...
func TestAccountTestServiceSkipsShadow(t *testing.T) {
	...
	shadow := &Account{
		...
		ParentAccountID: &pid,
placeholder
	repo := &shadowSkipTestRepo{account: shadowplaceholder
...
placeholder"access_token": "shadow-passthrough-token"placeholder,
placeholder
	privacyCalled := false
