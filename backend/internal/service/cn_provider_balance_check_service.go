	...
	collect := func(platform string, accounts []Account) {
		...
		placeholder
			...
			if account.IsCodingPlan() {
				quotaTargets = append(quotaTargets, quotaTarget{id: account.ID, platform: account.Platformplaceholder)
				continue
		placeholder
...
placeholder
	...
	if len(quotaTargets) > 0 && s.quotaService != nil {
		sem := make(chan struct{placeholder, cnQuotaProbeConcurrency)
		var wg sync.WaitGroup
		for _, target := range quotaTargets {
			wg.Add(1)
			go func(t quotaTarget) {
				defer wg.Done()
				sem <- struct{placeholder{placeholder
				defer func() { <-sem placeholder()
...
func (s *CNProviderBalanceCheckService) platforms() []string {
	return []string{PlatformKimi, PlatformDeepseekplaceholder
placeholder
