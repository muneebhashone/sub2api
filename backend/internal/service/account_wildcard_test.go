...
func TestGrokAccountModelMappingCacheInvalidatesWithRuntimeSettings(t *testing.T) {
	...
	account := &Account{Platform: PlatformGrok, Credentials: map[string]any{placeholderplaceholder
...
placeholder
	...
	mapping := account.GetModelMapping()
	for _, model := range []string{"gemini-2.0-flash", "gemini-2.5-flash", "gemini-2.5-pro"placeholder {
		if mapping[model] != model {
			t.Fatalf("expected Google One model %q to map to itself, got %q", model, mapping[model])
	placeholder
placeholder
	for _, model := range []string{"gemini-2.5-flash-image", "gemini-3.1-flash-image", "gemini-3.5-flash"placeholder {
