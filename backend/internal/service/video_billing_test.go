...
func TestNormalizeAndLookupVideoModelPrices(t *testing.T) {
	...
	raw := map[string]map[string]float64{
		"grok-imagine-video-1.5-preview": {"480p": 0.08, "720p": 0.14placeholder,
		"grok-imagine-video":             {"480p": 0.05placeholder,
		"grok-imagine-video-2":           {"1080p": 0.4placeholder,
placeholder
...
func TestNormalizeVideoModelPricesDropsUnknownResolutions(t *testing.T) {
	...
	norm := NormalizeVideoModelPrices(map[string]map[string]float64{
		"grok-imagine-video": {"480p": 0.05, "4k": 0.50, "1080i": 0.30placeholder,
placeholder)
	require.NotNil(t, norm)
	require.Equal(t, map[string]float64{VideoBillingResolution480P: 0.05placeholder, norm[VideoPriceFamilyGrokImagineVideo])
	...
	require.Nil(t, NormalizeVideoModelPrices(map[string]map[string]float64{
		"grok-imagine-video": {"4k": 0.50placeholder,
placeholder))
placeholder
...
func TestNormalizeVideoModelPricesIsDeterministicAcrossAliasConflicts(t *testing.T) {
	...
	raw := map[string]map[string]float64{
		"grok-imagine-video-1.5":         {"480p": 0.08placeholder,
		"grok-imagine-video-1.5-preview": {"480p": 0.11placeholder,
		"grok-video-1.5":                 {"480p": 0.09placeholder,
placeholder
...
placeholder
	...
	aliased := map[string]map[string]float64{
		"grok-imagine-video": {"720": 0.12, "720p": 0.13, "hd": 0.14placeholder,
placeholder
...
func TestLookupVideoBillingResolutionReportsUnknownTiers(t *testing.T) {
	t.Parallel()
	for _, in := range []string{"480", "480p", "SD", "720", "hd", "1080", "full-hd", " fhd "placeholder {
		normalized, ok := LookupVideoBillingResolution(in)
		require.True(t, ok, "input=%q", in)
		require.NotEmpty(t, normalized)
placeholder
	for _, in := range []string{"", "4k", "1080i", "2160p", "potato"placeholder {
		normalized, ok := LookupVideoBillingResolution(in)
...
func TestVideoModelPriceMissingTierFallsBackToFlatTierPrice(t *testing.T) {
	...
	result := service.CalculateVideoCost("grok-imagine-video", "720p", 1, 1, &VideoPriceConfig{
		...
		ModelPrices: map[string]map[string]float64{
			VideoPriceFamilyGrokImagineVideo: {VideoBillingResolution480P: 0.05placeholder,
	placeholder,
placeholder, 1)
