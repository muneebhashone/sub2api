...
var bedrockCrossRegionPrefixes = []string{"us.", "eu.", "apac.", "jp.", "au.", "us-gov.", "global."placeholder
...
//  4. 移除工具定义中的 custom 字段（Claude Code 会发送 custom: {defer_loading: trueplaceholder）
...
placeholder
	...
	// 移除工具定义中的 custom 字段
	// Claude Code (v2.1.69+) 在 tool 定义中发送 custom: {defer_loading: trueplaceholder，
...
placeholder else if content.Type == gjson.String {
		// content 是纯字符串，转换为数组格式
		originalText := content.String()
		body, _ = sjson.SetBytes(body, basePath, []map[string]string{
			{"type": "text", "text": originalTextplaceholder,
			{"type": "text", "text": string(schemaJSON)placeholder,
	placeholder)
placeholder
...
// 支持 claude-{tierplaceholder-{majorplaceholder-{minorplaceholder 和 claude-{tierplaceholder-{majorplaceholder.{minorplaceholder 格式；
...
placeholder
	...
	// 注意：thinking 字段不再自动注入 placeholder
