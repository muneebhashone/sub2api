...
func TestGatewayProfitControlAfterAdmissionBindSemantics(t *testing.T) {
	...
	t.Run("eager without gate", func(t *testing.T) {
		cache := &mockGatewayCacheForPlatform{sessionBindings: map[string]int64{"s": expensiveIDplaceholderplaceholder
...
placeholder)
	...
	t.Run("gated read failure is conservative", func(t *testing.T) {
		...
		cache := &mockGatewayCacheForPlatform{sessionBindings: map[string]int64{placeholderplaceholder
		svc := &GatewayService{cache: cacheplaceholder
		gate := &openAIProfitControlGate{groupID: groupID, platform: PlatformAnthropic, threshold: 0.5placeholder
		gateCtx := context.WithValue(context.Background(), openAIProfitControlGateCtxKey{placeholder, gate)
...
placeholder)
	...
	t.Run("gated sentinel miss binds", func(t *testing.T) {
		cache := &sentinelMissGatewayCache{mockGatewayCacheForPlatform: &mockGatewayCacheForPlatform{sessionBindings: map[string]int64{placeholderplaceholderplaceholder
		svc := &GatewayService{cache: cacheplaceholder
		gate := &openAIProfitControlGate{groupID: groupID, platform: PlatformAnthropic, threshold: 0.5placeholder
		gateCtx := context.WithValue(context.Background(), openAIProfitControlGateCtxKey{placeholder, gate)
