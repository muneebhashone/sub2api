...
type allowClaudeCodeSettingRepoStub struct{ values map[string]string placeholder
...
func TestSettingService_IsOpenAIAllowClaudeCodeCodexPluginEnabled(t *testing.T) {
	t.Run("默认关闭（设置缺失）", func(t *testing.T) {
		svc := NewSettingService(&allowClaudeCodeSettingRepoStub{values: map[string]string{placeholderplaceholder, &config.Config{placeholder)
