...
func adaptOpenAIResponsesClientTools(body []byte) ([]byte, apicompat.ResponsesClientToolMapping, error) {
	if !needsOpenAIResponsesClientToolAdaptation(body) {
		return body, apicompat.ResponsesClientToolMapping{placeholder, nil
placeholder
	...
	if err := decoder.Decode(&requestBody); err != nil {
		return body, apicompat.ResponsesClientToolMapping{placeholder, fmt.Errorf("decode OpenAI Responses client tools: %w", err)
placeholder
	...
	placeholder
		return body, apicompat.ResponsesClientToolMapping{placeholder, fmt.Errorf("decode OpenAI Responses client tools trailing data: %w", err)
placeholder
...
placeholder
	...
	if err != nil {
		return body, apicompat.ResponsesClientToolMapping{placeholder, fmt.Errorf("encode OpenAI Responses client tools: %w", err)
placeholder
...
placeholder
	...
	case "response.failed":
		return false
	case "error":
		// 上游降载/瞬时故障会先推 {"type":"error"placeholder 帧、再以 response.failed 收尾。
...
func openAIStreamItemHasVisibleOutput(item gjson.Result) bool {
	if item.Get("arguments").String() != "" || item.Get("input").String() != "" || item.Get("result").String() != "" {
		return true
placeholder
	for _, path := range []string{"content", "summary"placeholder {
...
placeholder
	updated := payload
	changed := false
	for _, path := range []string{"response.error.code", "error.code"placeholder {
...
placeholder
	...
	for _, path := range []string{"response.error.status_code", "error.status_code", "status_code"placeholder {
...
placeholder
	...
	placeholder
placeholder
	for _, path := range []string{"response.error.type", "error.type", "type"placeholder {
	...
	placeholder
placeholder
	for _, path := range []string{"response.error.code", "error.code", "code"placeholder {
