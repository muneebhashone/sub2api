...
func (i *BatchImageResultIndexer) listExpectedCustomIDs(ctx context.Context, batchID string) (map[string]struct{placeholder, error) {
	const pageSize = 500
	expected := make(map[string]struct{placeholder)
	offset := 0
	for {
		page, err := i.Repo.ListBatchImageItems(ctx, batchID, BatchImageItemFilter{Limit: pageSize, Offset: offsetplaceholder)
		if err != nil {
			return nil, err
	placeholder
		for _, item := range page {
			if item != nil {
				expected[item.CustomID] = struct{placeholder{placeholder
		placeholder
	placeholder
