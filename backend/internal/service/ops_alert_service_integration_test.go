...
func TestOpsAlertService_StartedViaWireProviders_RunsIndependentTicker(t *testing.T) {
	...
	t.Cleanup(func() { opsAlertEvalInterval = oldInterval placeholder)
...
placeholder, 1*time.Second, 5*time.Millisecond)
...
func newFakeOpsRepository() *fakeOpsRepository {
	return &fakeOpsRepository{
		eventCreatedCh: make(chan struct{placeholder),
placeholder
placeholder
...
type fakeOpsRepository struct {
	...
	nextEventID    int64
	eventCreatedCh chan struct{placeholder
	eventOnce      sync.Once
placeholder
...
func (r *fakeOpsRepository) GetLatestSystemMetric(ctx context.Context) (*OpsMetrics, error) {
	return &OpsMetrics{WindowMinutes: 1placeholder, sql.ErrNoRows
placeholder
...
placeholder
	...
	r.eventOnce.Do(func() { close(r.eventCreatedCh) placeholder)
	return nil
placeholder
...
func (r *fakeOpsRepository) GetOverviewStats(ctx context.Context, startTime, endTime time.Time) (*OverviewStats, error) {
	return &OverviewStats{placeholder, nil
placeholder
