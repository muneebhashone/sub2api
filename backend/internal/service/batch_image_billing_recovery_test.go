...
placeholder
	...
	svc := &BatchImageBillingRecoveryService{Repo: repo, Billing: billing, StaleAfter: time.Minute, Limit: 10placeholder
...
placeholder
	repo.jobs[stale.BatchID] = stale
	billing := &fakeBatchImageBillingRepo{releaseErr: errors.New("billing db down")placeholder
	queue := &recordingBatchImageQueue{fakeBatchImageQueue: newFakeBatchImageQueue("")placeholder
	svc := &BatchImageBillingRecoveryService{Repo: repo, Billing: billing, Queue: queue, StaleAfter: time.Minute, Limit: 10placeholder
...
placeholder
	require.Equal(t, 0, released)
	require.Equal(t, BatchImageJobStatusFailed, repo.jobs[stale.BatchID].Status)
	require.Equal(t, []string{stale.BatchIDplaceholder, queue.enqueued)
placeholder
