...
func TestApiKeyService_Delete_OwnerMismatch(t *testing.T) {
	repo := &apiKeyRepoStub{ownerID: 1placeholder
...
func TestApiKeyService_Delete_Success(t *testing.T) {
	repo := &apiKeyRepoStub{ownerID: 7placeholder
	cache := &apiKeyCacheStub{placeholder
	svc := &ApiKeyService{apiKeyRepo: repo, cache: cacheplaceholder
...
func TestApiKeyService_Delete_NotFound(t *testing.T) {
	repo := &apiKeyRepoStub{ownerErr: ErrApiKeyNotFoundplaceholder
	cache := &apiKeyCacheStub{placeholder
	svc := &ApiKeyService{apiKeyRepo: repo, cache: cacheplaceholder
...
placeholder
	...
	require.Equal(t, []int64{3placeholder, repo.deletedIDs)   // 验证删除操作被调用
	require.Equal(t, []int64{3placeholder, cache.invalidated) // 验证缓存已被清除（即使删除失败）
placeholder
