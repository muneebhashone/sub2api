...
func TestCalculateOpenAIRecordUsageCostWebSearchPerCall(t *testing.T) {
	...
	cost, err := svc.calculateOpenAIRecordUsageCost(context.Background(), result, apiKey, []string{"gpt-5.6-sol"placeholder, 3.0, 1.0, 1.0, 2.0, UsageTokens{placeholder, "", boolPtr(false), time.Time{placeholder)
placeholder
	require.Equal(t, string(BillingModePerRequest), cost.BillingMode)
	require.InDelta(t, 0.01, cost.TotalCost, 1e-12)
	...
	cost, err = svc.calculateOpenAIRecordUsageCost(context.Background(), result, apiKey, []string{"gpt-5.6-sol"placeholder, 1.0, 1.0, 1.0, 1.0, UsageTokens{placeholder, "", boolPtr(false), time.Time{placeholder)
placeholder
	...
	_, err = svc.calculateOpenAIRecordUsageCost(context.Background(), result, apiKey, []string{"gpt-5.6-sol"placeholder, 1.0, 1.0, 1.0, 1.0, UsageTokens{InputTokens: 10placeholder, "", boolPtr(false), time.Time{placeholder)
placeholder
placeholder
