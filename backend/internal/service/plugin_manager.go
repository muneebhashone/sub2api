...
type PluginManager struct {
	...
	reconcileDone      chan struct{placeholder
...
placeholder
	...
	m.reconcileDone = make(chan struct{placeholder)
...
placeholder
	...
	placeholder else if installation.State == PluginStateEnabled {
...
placeholder
	...
	if installation.RuntimeHealthy {
		installation.RuntimeMessage = "插件进程运行中"
placeholder else if route := m.route.Load(); route != nil && route.pluginID == id {
		installation.RuntimeMessage = route.unavailable
placeholder
...
placeholder
	...
	placeholder
		previous = existing
placeholder else if !errors.Is(getErr, sql.ErrNoRows) {
		cleanupErr := m.cleanupInstallationFiles(packageInfo)
...
placeholder
	...
	for _, path := range []string{installation.ArtifactPath, installation.InstallPathplaceholder {
...
func (m *PluginManager) reconcileLoop(ctx context.Context, done chan struct{placeholder) {
	defer close(done)
...
placeholder
	if enabled.State == PluginStateStarting && !m.startingStateExpired(enabled) {
		if current == nil {
			m.route.Store(&pluginRoute{pluginID: enabled.ID, rolloutPercent: rollout, unavailable: "插件正在其他实例中启动"placeholder)
	placeholder
		return nil
placeholder
...
placeholder
	...
	m.route.Store(&pluginRoute{pluginID: enabled.ID, runtime: runtime, rolloutPercent: rolloutplaceholder)
	m.mu.Unlock()
...
func (m *PluginManager) publishUnavailableRoute(pluginID int64, rollout int, message string) {
	...
	for id, runtime := range m.runtimes {
		...
		delete(m.runtimes, id)
placeholder
	m.route.Store(&pluginRoute{pluginID: pluginID, rolloutPercent: rollout, unavailable: messageplaceholder)
	m.mu.Unlock()
...
placeholder
	...
	if err != nil {
		...
		if hasEnabledOpenAIBinding(originalBindings) {
			m.route.Store(&pluginRoute{pluginID: id, rolloutPercent: bindingRollout(originalBindings), unavailable: err.Error()placeholder)
	placeholder
...
placeholder
	...
	if err := runtime.validateAndApplyConfig(testCtx, configJSON); err != nil {
		return nil, err
placeholder
	return runtime.api.TestConfig(testCtx, &pluginv1.TestConfigRequest{ConfigJson: configJSONplaceholder)
placeholder
...
func (m *PluginManager) CreateUIAssetToken(ctx context.Context, id int64, ttl time.Duration) (string, time.Time, error) {
	if ttl <= 0 || ttl > time.Hour {
		return "", time.Time{placeholder, errors.New("插件 UI 会话有效期无效")
placeholder
	if _, err := m.repo.GetByID(ctx, id); err != nil {
		return "", time.Time{placeholder, err
placeholder
	expires := time.Now().Add(ttl)
	raw, err := json.Marshal(pluginUIAssetClaims{Version: 1, PluginID: id, Expires: expires.Unix()placeholder)
	if err != nil {
		return "", time.Time{placeholder, err
placeholder
	...
	if err != nil {
		return "", time.Time{placeholder, fmt.Errorf("加密插件 UI 会话: %w", err)
placeholder
