...
placeholder
	...
	placeholder
		...
		if len(modelFiltered) == 0 && len(accounts) > 0 {
			return nil, 0, 0, 0, noAvailableOpenAISelectionError(req.RequestedModel, false, openAISelectionFilterStats{placeholder.summary("grok_model_quota_block"))
	placeholder
...
type openAIGroupPrivacyRequirementContextKey struct{placeholder
...
func (s *OpenAIGatewayService) withOpenAIGroupPrivacyRequirement(ctx context.Context, groupID *int64) context.Context {
	return context.WithValue(ctx, openAIGroupPrivacyRequirementContextKey{placeholder, openAIGroupPrivacyRequirement{
...
func (s *OpenAIGatewayService) openAIGroupRequiresPrivacySet(ctx context.Context, groupID *int64) bool {
	if cached, ok := ctx.Value(openAIGroupPrivacyRequirementContextKey{placeholder).(openAIGroupPrivacyRequirement); ok && cached.groupID == derefGroupID(groupID) {
...
placeholder
	...
	if scheduler == nil {
		...
		placeholder
			fallbackScheduler := &defaultOpenAIAccountScheduler{service: s, stats: newOpenAIAccountRuntimeStats()placeholder
...
placeholder
	...
	placeholder else if len(observedErr) > 0 && observedErr[0] != nil {
