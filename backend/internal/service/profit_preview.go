...
func PreviewProfitAdmission(inputs []ProfitPreviewGroupInput, evalAt time.Time) []ProfitPreviewGroupReport {
	...
	placeholder
		sort.Slice(report.Verdicts, func(i, j int) bool { return report.Verdicts[i].AccountID < report.Verdicts[j].AccountID placeholder)
...
placeholder else if account.RateMultiplier != nil && *account.RateMultiplier == 1 {
...
func profitPreviewProbeWarnings(account *Account, evalAt time.Time) []string {
	...
	if snapshot == nil {
		return []string{ProfitPreviewWarningProbeMissingplaceholder
placeholder
	switch snapshot.Status {
	case UpstreamBillingProbeStatusFailed:
		return []string{ProfitPreviewWarningProbeFailedplaceholder
	case UpstreamBillingProbeStatusUnsupported:
		return []string{ProfitPreviewWarningProbeUnsupportedplaceholder
	case UpstreamBillingProbeStatusOK:
		if snapshot.FreshUntil == nil || !evalAt.Before(*snapshot.FreshUntil) {
			return []string{ProfitPreviewWarningProbeStaleplaceholder
	placeholder
placeholder
	return nil
placeholder
