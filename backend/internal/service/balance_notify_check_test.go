...
func TestCheckQuotaDimCrossings_NoDimensions(t *testing.T) {
	s, _ := newBalanceNotifyServiceForTest()
	account := &Account{ID: 1, Name: "test", Platform: PlatformAnthropicplaceholder
	// Empty dims → no crossing, no panic.
	s.checkQuotaDimCrossings(account, nil, 10, []string{"admin@example.com"placeholder, "TestSite")
	s.checkQuotaDimCrossings(account, []quotaDim{placeholder, 10, []string{"admin@example.com"placeholder, "TestSite")
placeholder
...
placeholder
	...
	s.checkQuotaDimCrossings(account, dims, 50, []string{"admin@example.com"placeholder, "TestSite")
placeholder
