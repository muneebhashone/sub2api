...
placeholder)
	...
	t.Run("non owner returns not found", func(t *testing.T) {
		...
		_, err := svc.DeleteOutputsForOwner(ctx, BatchImageOwner{UserID: 11, APIKeyID: 999placeholder, "imgbatch_cleanup")
...
func TestBatchImageCleanupService_InputOutputAndWorker(t *testing.T) {
	...
	placeholder
		require.Equal(t, []CleanupTarget{CleanupTargetInputplaceholder, provider.cleanupTargets)
...
func TestBatchImageSettlementOutputExpiration(t *testing.T) {
	...
	svc := &BatchImageSettlementService{
		...
		Config:      &config.Config{BatchImage: config.BatchImageConfig{OutputRetentionAfterTerminalHours: 5placeholderplaceholder,
placeholder
...
func TestBatchImageDownloadAfterOutputDeletedReturnsGone(t *testing.T) {
	...
	result, err := svc.StreamZip(context.Background(), testBatchImageOwner(), "imgbatch_download", BatchImageZipOptions{placeholder, &out)
...
func newTestBatchImageCleanupService() (*BatchImageCleanupService, *fakeBatchImageRepository, *publicBatchImageProvider) {
	...
	provider := &publicBatchImageProvider{name: BatchImageProviderGeminiAPIplaceholder
	accountID := int64(101)
	svc := &BatchImageCleanupService{
		...
		Config:           &config.Config{BatchImage: config.BatchImageConfig{CleanupBatchSize: 10, InputRetentionAfterTerminalHours: 24placeholderplaceholder,
placeholder
