...
func (s *accountRepoStub) FindByExtraField(ctx context.Context, key string, value interface{placeholder) ([]Account, error) {
...
func TestAccountService_Delete_NotFound(t *testing.T) {
	repo := &accountRepoStub{exists: falseplaceholder
	svc := &AccountService{accountRepo: repoplaceholder
...
func TestAccountService_Delete_CheckError(t *testing.T) {
	repo := &accountRepoStub{existsErr: errors.New("db down")placeholder
	svc := &AccountService{accountRepo: repoplaceholder
...
placeholder
	...
	require.Equal(t, []int64{55placeholder, repo.deletedIDs) // 验证删除操作被调用
placeholder
...
func TestAccountService_Delete_Success(t *testing.T) {
	repo := &accountRepoStub{exists: trueplaceholder
	svc := &AccountService{accountRepo: repoplaceholder

	err := svc.Delete(context.Background(), 55)
placeholder
	require.Equal(t, []int64{55placeholder, repo.deletedIDs) // 验证正确的 ID 被删除
placeholder
