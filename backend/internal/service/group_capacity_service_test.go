...
func TestGetAllGroupCapacityBatchAggregatesRuntimeAndLimits(t *testing.T) {
	...
	placeholder,
placeholder
	groupRepo := &groupCapacityGroupRepoStub{groupIDs: []int64{10, 20placeholderplaceholder
	concurrencyCache := &groupCapacityConcurrencyCacheStub{counts: map[int64]int{1: 1, 2: 2placeholderplaceholder
	sessionCache := &groupCapacitySessionCacheStub{counts: map[int64]int{1: 2, 2: 1placeholderplaceholder
	rpmCache := &groupCapacityRPMCacheStub{counts: map[int64]int{1: 5, 2: 7placeholderplaceholder
...
placeholder
	...
	require.Equal(t, []int64{10, 20placeholder, accountRepo.requested)
	require.Equal(t, []int64{1, 2placeholder, concurrencyCache.requested)
	require.ElementsMatch(t, []int64{1, 2placeholder, sessionCache.requested)
	require.ElementsMatch(t, []int64{1, 2placeholder, rpmCache.requested)
...
func TestGetAllGroupCapacityBatchKeepsEmptyGroupRows(t *testing.T) {
	accountRepo := &groupCapacityAccountRepoStub{
		rows: []GroupAccountCapacityRow{
			{GroupID: 20, AccountID: 2, Concurrency: 4placeholder,
	placeholder,
placeholder
	groupRepo := &groupCapacityGroupRepoStub{groupIDs: []int64{10, 20placeholderplaceholder
...
placeholder
	...
	require.Equal(t, []GroupCapacitySummary{
		{GroupID: 10placeholder,
		{GroupID: 20, ConcurrencyMax: 4placeholder,
placeholder, results)
placeholder
