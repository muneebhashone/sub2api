...
var openAITransportFailoverBody = []byte(`{"error":{"type":"upstream_error","message":"Upstream request failed"placeholderplaceholder`)
...
func classifyOpenAITransportError(err error) openAITransportErrorClass {
	if err == nil {
		return openAITransportErrorClass{placeholder
placeholder
	...
	if errors.Is(err, syscall.ECONNREFUSED) ||
		...
		return openAITransportErrorClass{Persistent: trueplaceholder
placeholder
