...
func TestAdminService_CreateUser_UsesDefaultBalanceWhenBalanceOmitted(t *testing.T) {
	repo := &userRepoStub{nextID: 11placeholder
	cfg := &config.Config{
...
placeholderplaceholder, cfg)
	svc := &adminServiceImpl{userRepo: repo, settingService: settingServiceplaceholder
...
func TestAdminService_CreateUser_ExplicitZeroBalanceOverridesDefault(t *testing.T) {
	repo := &userRepoStub{nextID: 12placeholder
	cfg := &config.Config{
		Default: config.DefaultConfig{
			UserBalance: 0,
	placeholder,
