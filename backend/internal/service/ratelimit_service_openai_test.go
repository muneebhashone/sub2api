...
func TestCalculateOpenAI429ResetTime_7dExhausted(t *testing.T) {
	svc := &RateLimitService{placeholder
...
func TestHandle429_OpenAIPersistsCodexSnapshotImmediately(t *testing.T) {
	repo := &openAI429SnapshotRepo{placeholder
	svc := NewRateLimitService(repo, nil, nil, nil, nil)
	account := &Account{ID: 123, Platform: PlatformOpenAI, Type: AccountTypeOAuthplaceholder
...
placeholder"plan_type": "plus"placeholder,
placeholder
	body := []byte(`{"error":{"type":"usage_limit_reached","message":"limit reached","plan_type":"free","resets_at":1777283883placeholderplaceholder`)

	svc.handle429(context.Background(), account, http.Header{placeholder, body)
...
func TestHandle429_SkipsSparkShadow(t *testing.T) {
	...
	parentID := int64(900)
	shadowRepo := &openAI429SnapshotRepo{placeholder
...
placeholder
	...
	normalRepo := &openAI429SnapshotRepo{placeholder
	normalSvc := NewRateLimitService(normalRepo, nil, nil, nil, nil)
	normal := &Account{ID: 902, Platform: PlatformOpenAI, Type: AccountTypeOAuthplaceholder
