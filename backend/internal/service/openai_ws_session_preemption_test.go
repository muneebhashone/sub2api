...
func TestOpenAIWSSessionPreemptRegistryCancelsSameScopedSessionOnly(t *testing.T) {
	...
	key := openAIWSSessionPreemptKey{groupID: 7, apiKeyID: 11, sessionHash: "sess"placeholder
	other := openAIWSSessionPreemptKey{groupID: 7, apiKeyID: 12, sessionHash: "sess"placeholder
...
func TestOpenAIWSSessionPreemptContextEligibilityAndLocalCancellation(t *testing.T) {
	...
	svc := &OpenAIGatewayService{openaiWSStateStore: stateStoreplaceholder
	oauth := &Account{ID: 1, Platform: PlatformOpenAI, Type: AccountTypeOAuthplaceholder
	apiKey := &Account{ID: 2, Platform: PlatformOpenAI, Type: AccountTypeAPIKeyplaceholder
	grok := &Account{ID: 3, Platform: PlatformGrok, Type: AccountTypeOAuthplaceholder
...
func TestOpenAIWSIngressSessionPreemptionSurvivesNestedForwardCleanup(t *testing.T) {
	...
	newContext := func() *gin.Context {
		...
		c.Set("api_key", &APIKey{ID: 11, GroupID: &groupIDplaceholder)
		return c
placeholder

	svc := &OpenAIGatewayService{placeholder
	account := &Account{ID: 1, Platform: PlatformOpenAI, Type: AccountTypeOAuthplaceholder
	firstMessage := []byte(`{"type":"response.create","prompt_cache_key":"session-1","input":"hello"placeholder`)
...
func TestOpenAIWSSessionPreemptRemoteClaimAndStaleReleaseAreAtomic(t *testing.T) {
	cache := &openAIWSSessionPreemptCacheStub{placeholder
