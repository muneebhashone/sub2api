...
placeholder
	...
	placeholder
		if compatibilityBody, compatibilityChanged, compatibilityErr := normalizeOpenAIResponsesWebSocketCompatibilityBody(normalized, account); compatibilityErr != nil {
			return openAIWSClientPayload{placeholder, NewOpenAIWSClientCloseError(coderws.StatusPolicyViolation, "invalid websocket request payload", compatibilityErr)
	...
	placeholder
		if account.IsOpenAIOAuth() && !forceHTTPBridge {
			...
			if aliasErr != nil {
				return openAIWSClientPayload{placeholder, NewOpenAIWSClientCloseError(coderws.StatusPolicyViolation, aliasErr.Error(), aliasErr)
		placeholder
	...
	placeholder
		...
		if codexBridgeEnabled {
			...
			if err := json.Unmarshal(normalized, &payloadMap); err != nil {
				return openAIWSClientPayload{placeholder, NewOpenAIWSClientCloseError(coderws.StatusPolicyViolation, "invalid websocket request payload", err)
		placeholder
		...
		placeholder
			if bridgeModified {
				...
				if marshalErr != nil {
					return openAIWSClientPayload{placeholder, NewOpenAIWSClientCloseError(coderws.StatusPolicyViolation, "invalid websocket request payload", marshalErr)
			placeholder
				normalized = rebuilt
		placeholder
	placeholder
		...
		if hooks != nil && hooks.MapRequestModel != nil {
			...
			if mapErr != nil {
				return openAIWSClientPayload{placeholder, mapErr
		placeholder
	...
	placeholder
		...
		if policyErr != nil {
			return openAIWSClientPayload{placeholder, NewOpenAIWSClientCloseError(coderws.StatusPolicyViolation, "invalid websocket request payload", policyErr)
	placeholder
		if blocked != nil {
