...
func TestSelectContiguousMetrics_Contiguous(t *testing.T) {
	...
	metrics := []OpsMetrics{
		{UpdatedAt: nowplaceholder,
		{UpdatedAt: now.Add(-1 * time.Minute)placeholder,
		{UpdatedAt: now.Add(-2 * time.Minute)placeholder,
placeholder
...
func TestSelectContiguousMetrics_GapFails(t *testing.T) {
	...
	metrics := []OpsMetrics{
		...
		{UpdatedAt: now.Add(-2 * time.Minute)placeholder,
		{UpdatedAt: now.Add(-3 * time.Minute)placeholder,
placeholder
...
func TestSelectContiguousMetrics_StaleNewestFails(t *testing.T) {
	...
	metrics := []OpsMetrics{
		{UpdatedAt: now.Add(-10 * time.Minute)placeholder,
		{UpdatedAt: now.Add(-11 * time.Minute)placeholder,
placeholder
...
func TestOpsAlertService_StopWithoutStart_NoPanic(t *testing.T) {
	...
	require.NotPanics(t, func() { s.Stop() placeholder)
placeholder
...
func TestBuildWebhookHTTPClient_DefaultTimeout(t *testing.T) {
	...
	base := &http.Client{placeholder
	client = buildWebhookHTTPClient(base, nil)
	require.Equal(t, webhookHTTPClientTimeout, client.Timeout)
	require.NotNil(t, client.CheckRedirect)

	base = &http.Client{Timeout: 2 * time.Secondplaceholder
	client = buildWebhookHTTPClient(base, nil)
...
func TestValidateWebhookURL_RequiresHTTPS(t *testing.T) {
	...
	t.Cleanup(func() { lookupIPAddrs = oldLookup placeholder)
	lookupIPAddrs = func(ctx context.Context, host string) ([]net.IPAddr, error) {
		return []net.IPAddr{{IP: net.ParseIP("93.184.216.34")placeholderplaceholder, nil
placeholder
...
func TestValidateWebhookURL_RejectsPrivateIPViaDNS(t *testing.T) {
	...
	lookupIPAddrs = func(ctx context.Context, host string) ([]net.IPAddr, error) {
		...
		return []net.IPAddr{{IP: net.ParseIP("10.0.0.2")placeholderplaceholder, nil
placeholder
...
func TestValidateWebhookURL_RejectsLinkLocalIPViaDNS(t *testing.T) {
	...
	lookupIPAddrs = func(ctx context.Context, host string) ([]net.IPAddr, error) {
		...
		return []net.IPAddr{{IP: net.ParseIP("169.254.169.254")placeholderplaceholder, nil
placeholder
...
placeholder
	...
	lookupIPAddrs = func(ctx context.Context, host string) ([]net.IPAddr, error) {
		lookupCalls++
		return []net.IPAddr{{IP: net.ParseIP("10.0.0.1")placeholderplaceholder, nil
placeholder
...
placeholder
	require.Equal(t, []string{"93.184.216.34:443"placeholder, dialAddrs)
...
func TestRetryWithBackoff_SucceedsAfterRetries(t *testing.T) {
	...
	t.Cleanup(func() { opsAlertSleep = oldSleep placeholder)
...
placeholder
	...
	err := retryWithBackoff(
		context.Background(),
		3,
		[]time.Duration{time.Second, 2 * time.Second, 4 * time.Secondplaceholder,
		func() error {
			attempts++
			if attempts <= 3 {
...
placeholder
	...
	require.Equal(t, []time.Duration{time.Second, 2 * time.Second, 4 * time.Secondplaceholder, slept)
placeholder
...
placeholder
	...
	require.Equal(t, []time.Duration{time.Secondplaceholder, slept)
placeholder
