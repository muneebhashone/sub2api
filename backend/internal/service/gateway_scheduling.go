...
placeholder else if sessionHash != "" && s.cache != nil {
...
placeholder
	...
	if s.concurrencyService == nil || !cfg.LoadBatchEnabled {
		// 复制排除列表，用于会话限制拒绝时的重试
		localExcluded := make(map[int64]struct{placeholder)
	...
	placeholder
		...
		placeholder
			...
			if !s.checkAndRegisterSession(ctx, account, sessionHash) {
				localExcluded[account.ID] = struct{placeholder{placeholder
				continue
		placeholder
		...
		if len(routingCandidates) > 0 {
			...
			if sessionHash != "" && stickyAccountID > 0 {
				slog.Debug("sticky.layer1_5_checking",
					...
					"in_account_map", func() bool { _, ok := accountByID[stickyAccountID]; return ok placeholder(),
					"session", shortSessionHash(sessionHash),
				)
		...
		placeholder
			...
			placeholder
				if loadInfo.LoadRate < 100 {
					routingAvailable = append(routingAvailable, accountWithLoad{account: acc, loadInfo: loadInfoplaceholder)
			placeholder
		placeholder
...
placeholder
	...
	if len(routingAccountIDs) == 0 && sessionHash != "" && stickyAccountID > 0 && !isExcluded(stickyAccountID) {
		...
		if accountID > 0 && !isExcluded(accountID) {
			...
			placeholder
				...
				placeholder
					...
					placeholder
				placeholder
			placeholder else if !clearSticky {
					slog.Debug("sticky.layer1_5_no_routing_miss",
						"account_id", accountID,
		...
		placeholder
	placeholder
placeholder else if len(routingAccountIDs) == 0 && sessionHash != "" {
		slog.Debug("sticky.layer1_5_no_routing_skip",
			"sticky_account_id", stickyAccountID,
			"is_excluded", func() bool { return stickyAccountID > 0 && isExcluded(stickyAccountID) placeholder(),
			"session", shortSessionHash(sessionHash),
			"reason", func() string {
...
placeholder
	...
	placeholder else if s.cfg != nil && s.cfg.RunMode == config.RunModeSimple {
...
type windowCostPrefetchContextKeyType struct{placeholder

var windowCostPrefetchContextKey = windowCostPrefetchContextKeyType{placeholder
...
type rpmPrefetchContextKeyType struct{placeholder

var rpmPrefetchContextKey = rpmPrefetchContextKeyType{placeholder
...
placeholder
	...
	if count, ok := rpmFromPrefetchContext(ctx, account.ID); ok {
		currentRPM = count
placeholder else if s.rpmCache != nil {
...
placeholder
	...
	placeholder
		if j-i > 1 {
			...
			placeholder
				if len(oauth) > 1 {
					mathrand.Shuffle(len(oauth), func(a, b int) { oauth[a], oauth[b] = oauth[b], oauth[a] placeholder)
			placeholder
				if len(others) > 1 {
					mathrand.Shuffle(len(others), func(a, b int) { others[a], others[b] = others[b], others[a] placeholder)
			placeholder
...
placeholder
	...
	placeholder
		...
		routingSet := make(map[int64]struct{placeholder, len(routingAccountIDs))
		for _, id := range routingAccountIDs {
			if id > 0 {
				routingSet[id] = struct{placeholder{placeholder
		placeholder
	placeholder
...
placeholder
	if vetoed, reason := openAIProfitControlVetoReason(ctx, acc); vetoed {
		return selectionFailureDiagnosis{Category: reasonplaceholder
placeholder
