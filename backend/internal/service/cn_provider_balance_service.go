...
placeholder
	...
	case PlatformKimi:
		...
		entries = append(entries, CNProviderBalanceEntry{Currency: "CNY", Balance: balanceplaceholder)
	...
	placeholder
		...
		gjson.GetBytes(bodyBytes, "balance_infos").ForEach(func(_, info gjson.Result) bool {
			...
			if currency == "" {
				currency = "CNY"
		placeholder
			entries = append(entries, CNProviderBalanceEntry{Currency: currency, Balance: balanceplaceholder)
			return true
	placeholder)
		if len(entries) == 0 {
