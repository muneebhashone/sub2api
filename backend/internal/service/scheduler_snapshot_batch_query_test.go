...
placeholder
	...
	for _, id := range accountIDs {
		accounts = append(accounts, Account{ID: idplaceholder)
placeholder
...
func TestSchedulerRebuildBatchReusesAccountPayloadForSingleForced(t *testing.T) {
	...
	for run := 1; run <= 2; run++ {
		require.NoError(t, svc.rebuildBuckets(context.Background(), []SchedulerBucket{single, forcedplaceholder, "reuse"))
...
placeholder
	require.Equal(t, 2, repo.callCount(batchAccountQueryKey{groupID: groupID, platform: PlatformOpenAIplaceholder), "账号载荷不得跨重建批次复用")
placeholder
...
func TestSchedulerRebuildBatchDoesNotReuseAccountPayloadAfterFirstWriterFailure(t *testing.T) {
	...
	err := svc.rebuildBuckets(context.Background(), []SchedulerBucket{single, forcedplaceholder, "failure")
...
func TestSchedulerRebuildBatchDoesNotReuseAccountPayloadAfterLateFirstWriterFailure(t *testing.T) {
	...
	err := svc.rebuildBuckets(context.Background(), []SchedulerBucket{single, forcedplaceholder, "late-failure")
	require.ErrorIs(t, err, wantErr)
	full, idOnly := cache.reuseCounts(single)
	require.Equal(t, 1, full)
	require.Zero(t, idOnly)
...
func TestSchedulerRebuildBatchDoesNotReuseAccountPayloadAfterLockBusy(t *testing.T) {
	...
	require.NoError(t, svc.rebuildBuckets(context.Background(), []SchedulerBucket{single, forcedplaceholder, "busy"))
	full, idOnly := cache.reuseCounts(single)
	require.Zero(t, full)
	require.Zero(t, idOnly)
	full, idOnly = cache.reuseCounts(forced)
...
func TestSchedulerRebuildBatchKeepsMixedAndDifferentQueriesOnFullWrites(t *testing.T) {
	...
	openAISingle := SchedulerBucket{GroupID: groupID, Platform: PlatformOpenAI, Mode: SchedulerModeSingleplaceholder
	openAIForced := SchedulerBucket{GroupID: groupID, Platform: PlatformOpenAI, Mode: SchedulerModeForcedplaceholder
	anthropicSingle := SchedulerBucket{GroupID: groupID, Platform: PlatformAnthropic, Mode: SchedulerModeSingleplaceholder
	anthropicMixed := SchedulerBucket{GroupID: groupID, Platform: PlatformAnthropic, Mode: SchedulerModeMixedplaceholder
	cache := newBatchSnapshotAccountIDCache()
	svc := newBatchQueryTestService(cache, newBatchAccountQueryRepo(), config.RunModeStandard)

	require.NoError(t, svc.rebuildBuckets(context.Background(), []SchedulerBucket{openAISingle, openAIForced, anthropicSingle, anthropicMixedplaceholder, "scope"))
...
func TestSchedulerRebuildBatchPropagatesAccountIDOnlyWriteFailure(t *testing.T) {
	...
	err := svc.rebuildBuckets(context.Background(), []SchedulerBucket{single, forcedplaceholder, "id-error")
	require.ErrorIs(t, err, wantErr)
	full, idOnly := cache.reuseCounts(forced)
...
func TestSchedulerRebuildBatchReusesSuccessfulEmptyAccountIDs(t *testing.T) {
	...
	require.NoError(t, svc.rebuildBuckets(context.Background(), []SchedulerBucket{single, forcedplaceholder, "empty"))
	full, idOnly := cache.reuseCounts(single)
	require.Equal(t, 1, full)
	require.Zero(t, idOnly)
	full, idOnly = cache.reuseCounts(forced)
...
func TestSchedulerRebuildBatchReusesAccountPayloadForSimpleGroupZero(t *testing.T) {
	single := SchedulerBucket{GroupID: 0, Platform: PlatformOpenAI, Mode: SchedulerModeSingleplaceholder
	...
	require.NoError(t, svc.rebuildBuckets(context.Background(), []SchedulerBucket{single, forcedplaceholder, "simple"))
	full, idOnly := cache.reuseCounts(single)
	require.Equal(t, 1, full)
	require.Zero(t, idOnly)
	full, idOnly = cache.reuseCounts(forced)
...
func TestSchedulerAccountQueryCacheReleasesSnapshotAccountIDs(t *testing.T) {
	single := schedulerBucketWriteTask{bucket: SchedulerBucket{GroupID: 218, Platform: PlatformOpenAI, Mode: SchedulerModeSingleplaceholderplaceholder
	forced := schedulerBucketWriteTask{bucket: SchedulerBucket{GroupID: 218, Platform: PlatformOpenAI, Mode: SchedulerModeForcedplaceholderplaceholder
	queries := newSchedulerAccountQueryCache([]schedulerBucketWriteTask{single, forcedplaceholder)
	key, ok := schedulerAccountQueryKeyForBucket(single.bucket)
	require.True(t, ok)
	queries.snapshotAccountIDs[key] = []int64{1, 2placeholder
