...
func TestClaudeOpus5_FamilyFallbackDoesNotUseOpus4Rates(t *testing.T) {
	svc := NewBillingService(&config.Config{placeholder, &PricingService{
		...
		placeholder,
			...
			"claude-opus-4-1":        {InputCostPerToken: 15e-6, OutputCostPerToken: 75e-6placeholder,
			"claude-opus-4-20250514": {InputCostPerToken: 15e-6, OutputCostPerToken: 75e-6placeholder,
			"claude-3-opus-20240229": {InputCostPerToken: 15e-6, OutputCostPerToken: 75e-6placeholder,
	placeholder,
placeholder)

	for _, model := range []string{"claude-opus-5", "us.anthropic.claude-opus-5-v1"placeholder {
...
func TestClaudeOpus5_HardcodedFallbackPricing(t *testing.T) {
	...
	tests := []struct {
		...
		output float64
placeholder{
		{"claude-opus-5", opus5InputPricePerToken, opus5OutputPricePerTokenplaceholder,
		{"us.anthropic.claude-opus-5-v1", opus5InputPricePerToken, opus5OutputPricePerTokenplaceholder,
		// 4.8 与 5 同价；修复前兜底表缺失，会掉到 claude-3-opus 的 $15/$75
		{"claude-opus-4-8", opus5InputPricePerToken, opus5OutputPricePerTokenplaceholder,
		// 相邻型号不能被 "opus-5" 误匹配
		{"claude-opus-4-5-20251101", 5e-6, placeholder,
		{"claude-opus-4-1-20250805", 15e-6, 75e-6placeholder,
		{"claude-3-opus-20240229", 15e-6, 75e-6placeholder,
placeholder
...
func TestClaudeOpus5_BedrockCapabilityGates(t *testing.T) {
	tests := []struct {
		...
		opus47Newer   bool
placeholder{
		{"claude-opus-5", true, true, trueplaceholder,
		{"us.anthropic.claude-opus-5-v1", true, true, trueplaceholder,
		{"eu.anthropic.claude-opus-5-v1", true, true, trueplaceholder,
		{"claude-sonnet-5", true, true, falseplaceholder,
		{"us.anthropic.claude-sonnet-5-v1", true, true, falseplaceholder,
		// 回归保护：旧模型不能因为 minor 可选而被误判为新版本
		{"anthropic.claude-opus-4-1-v1", false, false, falseplaceholder,
		{"anthropic.claude-sonnet-4-0-v1", false, false, falseplaceholder,
		{"anthropic.claude-3-opus-20240229-v1:0", false, false, falseplaceholder,
		{"us.anthropic.claude-opus-4-8-v1", true, true, trueplaceholder,
		// Haiku 不支持 tool search
		{"us.anthropic.placeholder-v1:0", true, false, falseplaceholder,
placeholder
	for _, tt := range tests {
...
func TestClaudeOpus5_BedrockThinkingConvertedToAdaptive(t *testing.T) {
	body := []byte(`{"thinking":{"type":"enabled","budget_tokens":10000placeholderplaceholder`)
	got := sanitizeBedrockThinking(body, "us.anthropic.claude-opus-5-v1")

	assert.JSONEq(t, `{"thinking":{"type":"adaptive"placeholderplaceholder`, string(got))
placeholder
