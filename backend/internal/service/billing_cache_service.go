...
placeholder
	...
	placeholder else if s.userGroupRateRepo != nil {
	...
	placeholder else if group.RPMLimit > 0 {
			...
			if err != nil {
				...
				// fail-open
		placeholder else if count > group.RPMLimit {
				return ErrGroupRPMExceeded
		placeholder
	placeholder
placeholder
...
// 返回 nil = 允许；返回 ErrUserPlatform{Daily/Weekly/MonthlyplaceholderQuotaExhausted = 拒绝（带 window_resets_at metadata）。
