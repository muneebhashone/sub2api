...
func TestOpenAIImagesJSONKeepalive_DisabledIsNoop(t *testing.T) {
	...
	stop()
	c.JSON(http.StatusBadRequest, gin.H{"error": gin.H{"message": "invalid request"placeholderplaceholder)
...
func TestOpenAIImagesJSONKeepalive_KeepsOAuthNonStreamResponseValid(t *testing.T) {
	...
	go func() {
		...
		_, _ = io.WriteString(writer,
			"data: {\"type\":\"response.completed\",\"response\":{\"created_at\":1710000000,\"output\":[{\"type\":\"image_generation_call\",\"result\":\"aW1hZ2U=\",\"output_format\":\"png\"placeholder]placeholderplaceholder\n\n"+
				"data: [DONE]\n\n",
		)
		_ = writer.Close()
placeholder()
...
func TestOpenAIImagesJSONKeepaliveWriter_NilGuards(t *testing.T) {
	w := &openAIImagesJSONKeepaliveWriter{placeholder
		...
		httpUpstream: &httpUpstreamRecorder{
			resp: &http.Response{
				...
				Header: http.Header{
					...
					"X-Request-Id": []string{"req_img_heartbeat_failover"placeholder,
			placeholder,
				Body: io.NopCloser(strings.NewReader(
					"data: {\"type\":\"response.created\",\"response\":{\"created_at\":1710000021placeholderplaceholder\n\n" +
						"data: {\"type\":\"error\",\"error\":{\"type\":\"server_error\",\"code\":\"server_error\",\"message\":\"The image service is temporarily unavailable.\"placeholderplaceholder\n\n",
				)),
		placeholder,
	placeholder,
placeholder
