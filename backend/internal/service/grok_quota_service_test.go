...
placeholderplaceholder
	...
	cfg.Security.URLAllowlist.UpstreamHosts = []string{"allowed.example.test"placeholder
	svc := &GrokQuotaService{accountRepo: repo, httpUpstream: upstream, cfg: cfgplaceholder
...
placeholderplaceholder
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		...
		Body:       io.NopCloser(strings.NewReader(`{"data":[{"id":"grok-4.5"placeholder]placeholder`)),
placeholderplaceholder
	svc := &GrokQuotaService{accountRepo: repo, httpUpstream: upstream, cfg: &config.Config{placeholderplaceholder
...
type grokHybridUpstream struct {
	...
	billingStarted       chan struct{placeholder
	billingRelease       <-chan struct{placeholder
...
placeholder
	...
	placeholder
		...
		if headers == nil {
			headers = http.Header{
				"X-Ratelimit-Limit-Tokens":     []string{"2000000"placeholder,
				"X-Ratelimit-Remaining-Tokens": []string{"1500000"placeholder,
		placeholder
	placeholder
		return &http.Response{StatusCode: status, Header: headers, Body: io.NopCloser(strings.NewReader(`{"id":"resp_probe"placeholder`))placeholder, nil
placeholder
	if u.billingStarted != nil {
		u.billingStartOnce.Do(func() { close(u.billingStarted) placeholder)
placeholder
...
placeholder
	if billingStatus != 0 && billingStatus != http.StatusOK {
		return &http.Response{
			...
			Body:       io.NopCloser(strings.NewReader(`{"error":{"message":"billing limited"placeholderplaceholder`)),
	placeholder, nil
placeholder
	...
	placeholder
		payload := `{"config":{"currentPeriod":{"type":"WEEKLY","start":"2026-07-09T03:25:00Z","end":"2026-07-16T03:25:00Z"placeholder` + usage + `placeholderplaceholder`
...
placeholder
	...
	if u.monthlyLimitCents != nil {
		monthlyLimit = `,"monthlyLimit":{"val":` + strconv.FormatFloat(*u.monthlyLimitCents, 'f', -1, 64) + `placeholder`
placeholder
	monthlyPayload := `{"config":{"billingPeriodStart":"2026-07-01T00:00:00Z","billingPeriodEnd":"2026-08-01T00:00:00Z"` + monthlyLimit + `placeholderplaceholder`
	return &http.Response{
		StatusCode: http.StatusOK,
		Header:     make(http.Header),
...
func TestGrokQuotaServiceFetchBillingRetries502ThenSucceeds(t *testing.T) {
	...
	upstream := &grokQuotaSequenceUpstream{steps: []grokQuotaUpstreamStep{
		{status: http.StatusBadGateway, body: `The origin web server returned an invalid or incomplete response to Cloudflare.`placeholder,
		{status: http.StatusOK, body: `{"config":{"currentPeriod":{"type":"WEEKLY","start":"2026-07-09T03:25:00Z","end":"2026-07-16T03:25:00Z"placeholder,"creditUsagePercent":12placeholderplaceholder`placeholder,
placeholderplaceholder
	svc := &GrokQuotaService{httpUpstream: upstreamplaceholder
...
func TestGrokQuotaServiceFetchBillingRetriesTransportErrorThenSucceeds(t *testing.T) {
	...
	upstream := &grokQuotaSequenceUpstream{steps: []grokQuotaUpstreamStep{
		{err: errors.New("temporary transport failure")placeholder,
		{status: http.StatusOK, body: `{"config":{"currentPeriod":{"type":"WEEKLY"placeholder,"creditUsagePercent":8placeholderplaceholder`placeholder,
placeholderplaceholder
	svc := &GrokQuotaService{httpUpstream: upstreamplaceholder
...
func TestGrokQuotaServiceFetchBillingStopsAfterSingleTransientRetry(t *testing.T) {
	...
	upstream := &grokQuotaSequenceUpstream{steps: []grokQuotaUpstreamStep{
		{status: http.StatusBadGateway, body: `cloudflare failure`placeholder,
		{status: http.StatusBadGateway, body: `cloudflare failure`placeholder,
		{status: http.StatusOK, body: `{"config":{"currentPeriod":{"type":"WEEKLY"placeholderplaceholderplaceholder`placeholder,
placeholderplaceholder
	svc := &GrokQuotaService{httpUpstream: upstreamplaceholder
...
func TestGrokQuotaServiceFetchBillingDoesNotRetryNonTransientStatuses(t *testing.T) {
	tests := []struct {
		...
		wantErr bool
placeholder{
		{name: "unauthorized", status: http.StatusUnauthorized, wantErr: trueplaceholder,
		{name: "forbidden", status: http.StatusForbidden, wantErr: trueplaceholder,
		{name: "rate limited", status: http.StatusTooManyRequests, wantErr: falseplaceholder,
placeholder
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			upstream := &grokQuotaSequenceUpstream{steps: []grokQuotaUpstreamStep{
				{status: tt.status, body: `{"error":{"message":"rejected"placeholderplaceholder`placeholder,
				{status: http.StatusOK, body: `{"config":{"currentPeriod":{"type":"WEEKLY"placeholderplaceholderplaceholder`placeholder,
		placeholderplaceholder
			svc := &GrokQuotaService{httpUpstream: upstreamplaceholder
...
func TestIsRetryableGrokBillingStatus(t *testing.T) {
	tests := []struct {
		...
		want   bool
placeholder{
		{status: http.StatusBadGateway, want: trueplaceholder,
		{status: http.StatusServiceUnavailable, want: trueplaceholder,
		{status: http.StatusGatewayTimeout, want: trueplaceholder,
		{status: http.StatusUnauthorized, want: falseplaceholder,
		{status: http.StatusForbidden, want: falseplaceholder,
		{status: http.StatusTooManyRequests, want: falseplaceholder,
placeholder
...
placeholderplaceholder
	upstream := &grokQuotaSequenceUpstream{steps: []grokQuotaUpstreamStep{
		{status: http.StatusBadGateway, body: `cloudflare failure`placeholder,
		{status: http.StatusOK, body: `{"id":"unexpected_retry"placeholder`placeholder,
placeholderplaceholder
...
func TestGrokQuotaServiceProbeUsageRedactsUpstreamErrorBodyFromErrorAndLogs(t *testing.T) {
	...
	repo := &grokQuotaAccountRepo{
		mockAccountRepoForPlatform: &mockAccountRepoForPlatform{
			accountsByID: map[int64]*Account{49: accountplaceholder,
	placeholder,
placeholder
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusBadRequest,
		...
		Body: io.NopCloser(strings.NewReader(
			`{"error":"` + upstreamSecret + `","detail":"credential rejected"placeholder`,
		)),
placeholderplaceholder
...
func TestGrokQuotaServiceProbeUsageDoesNotOverwriteSnapshotOnUnauthorized(t *testing.T) {
	...
	previous := &xai.QuotaSnapshot{StatusCode: http.StatusOK, HeadersObserved: trueplaceholder
	account.Extra = map[string]any{grokQuotaSnapshotExtraKey: previousplaceholder
	repo := &grokQuotaAccountRepo{mockAccountRepoForPlatform: &mockAccountRepoForPlatform{
		accountsByID: map[int64]*Account{account.ID: accountplaceholder,
placeholderplaceholder
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		...
		Header:     http.Header{placeholder,
		Body:       io.NopCloser(strings.NewReader(`{"error":"unauthorized"placeholder`)),
placeholderplaceholder
	svc := NewGrokQuotaService(repo, nil, NewGrokTokenProvider(repo, nil), upstream, nil)
...
func TestGrokLocalUsage24hUsesRollingUTCWindow(t *testing.T) {
	...
	t.Run("returns usage from exact rolling window", func(t *testing.T) {
		repo := &grokQuotaUsageLogRepo{stats: &usagestats.AccountStats{Tokens: placeholder
		stats := grokLocalUsage24h(context.Background(), repo, 57, now)

		require.NotNil(t, stats)
		require.EqualValues(t, 1_250_000, stats.Tokens)
		require.Equal(t, []time.Time{now.UTC().Add(-24 * time.Hour)placeholder, repo.startTimes)
placeholder)

	t.Run("query failure returns no stats", func(t *testing.T) {
		repo := &grokQuotaUsageLogRepo{err: context.DeadlineExceededplaceholder
		stats := grokLocalUsage24h(context.Background(), repo, 57, now)
...
placeholder)
	...
	t.Run("invalid account returns no stats without query", func(t *testing.T) {
		repo := &grokQuotaUsageLogRepo{placeholder
...
placeholder
	...
	t.Run("free queries only rolling 24h", func(t *testing.T) {
		repo := &grokQuotaUsageLogRepo{stats: &usagestats.AccountStats{Tokens: 500_000placeholderplaceholder
		...
		require.Equal(t, []time.Time{now.Add(-24 * time.Hour)placeholder, repo.startTimes)
placeholder)
...
placeholder
	...
	go func() {
		...
		billingOutcomes <- probeOutcome{result: result, err: errplaceholder
placeholder()
	<-billingStarted
	secondStarted := make(chan struct{placeholder)
	go func() {
		close(secondStarted)
		result, err := svc.ProbeBilling(context.Background(), account.ID)
		billingOutcomes <- probeOutcome{result: result, err: errplaceholder
...
func TestGrokQuotaServiceBilling403PersistsMediaEligibilitySignal(t *testing.T) {
	...
	repo := &grokQuotaAccountRepo{mockAccountRepoForPlatform: &mockAccountRepoForPlatform{
		accountsByID: map[int64]*Account{account.ID: accountplaceholder,
placeholderplaceholder
	upstream := &grokHybridUpstream{billingStatus: http.StatusForbiddenplaceholder
	svc := NewGrokQuotaService(repo, nil, NewGrokTokenProvider(repo, nil), upstream, nil)
...
placeholder
	...
	account.Extra = map[string]any{grokBillingExtraKey: billingplaceholder
...
placeholder
	...
	account.Extra = map[string]any{grokBillingExtraKey: result.Billingplaceholder
	eligible, reason := account.GrokMediaGenerationEligibility()
	require.False(t, eligible)
	require.Equal(t, "billing_forbidden", reason)
placeholder
...
func TestGrokQuotaServiceProbeMediaEligibility(t *testing.T) {
	t.Run("positive paid evidence enables media", func(t *testing.T) {
		...
		repo := &grokQuotaAccountRepo{mockAccountRepoForPlatform: &mockAccountRepoForPlatform{
			accountsByID: map[int64]*Account{account.ID: accountplaceholder,
	placeholderplaceholder
		upstream := &grokHybridUpstream{weeklyUsagePercent: &usagePercent, monthlyLimitCents: &monthlyLimitplaceholder
		svc := NewGrokQuotaService(repo, nil, NewGrokTokenProvider(repo, nil), upstream, nil)
...
placeholder)
	...
	t.Run("successful empty billing identifies free account", func(t *testing.T) {
		...
		repo := &grokQuotaAccountRepo{mockAccountRepoForPlatform: &mockAccountRepoForPlatform{
			accountsByID: map[int64]*Account{account.ID: accountplaceholder,
	placeholderplaceholder
		svc := NewGrokQuotaService(repo, nil, NewGrokTokenProvider(repo, nil), &grokHybridUpstream{placeholder, nil)
...
placeholder)
	...
	t.Run("forbidden billing is deterministic ineligibility", func(t *testing.T) {
		...
		repo := &grokQuotaAccountRepo{mockAccountRepoForPlatform: &mockAccountRepoForPlatform{
			accountsByID: map[int64]*Account{account.ID: accountplaceholder,
	placeholderplaceholder
		svc := NewGrokQuotaService(repo, nil, NewGrokTokenProvider(repo, nil), &grokHybridUpstream{billingStatus: http.StatusForbiddenplaceholder, nil)
...
func TestPreferBillingObservationStatus(t *testing.T) {
	...
	tests := []struct {
		...
		want          int
placeholder{
		{name: "weekly forbidden wins", weeklyStatus: http.StatusForbidden, monthlyStatus: http.StatusBadGateway, want: http.StatusForbiddenplaceholder,
		{name: "monthly forbidden wins", weeklyStatus: http.StatusBadGateway, monthlyStatus: http.StatusForbidden, want: http.StatusForbiddenplaceholder,
		{name: "weekly observation otherwise wins", weeklyStatus: http.StatusTooManyRequests, monthlyStatus: http.StatusBadGateway, want: http.StatusTooManyRequestsplaceholder,
		{name: "monthly observation is fallback", monthlyStatus: http.StatusBadGateway, want: http.StatusBadGatewayplaceholder,
placeholder
