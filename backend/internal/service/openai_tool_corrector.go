...
placeholder
	...
	case "edit":
		...
		placeholder else if next, changed := moveJSONField(updated, "path", "filePath"); changed {
				updated = next
				corrected = true
				logger.LegacyPrintf("service.openai_tool_corrector", "[CodexToolCorrector] Renamed 'path' to 'filePath' in edit tool")
		placeholder else if next, changed := moveJSONField(updated, "file", "filePath"); changed {
				updated = next
				corrected = true
