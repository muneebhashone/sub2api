...
func TestResponseModelBillingDeclaration(t *testing.T) {
	...
	tests := []struct {
		...
		want        string
placeholder{
		{name: "opted_in_and_clean", source: BillingModelSourceResponse, model: " claude-sonnet-4 ", want: "claude-sonnet-4"placeholder,
		{name: "other_source_never_looks_at_response", source: BillingModelSourceChannelMapped, model: "claude-sonnet-4"placeholder,
		{name: "empty_source_never_looks_at_response", source: "", model: "claude-sonnet-4"placeholder,
		{name: "upstream_source_never_looks_at_response", source: BillingModelSourceUpstream, model: "claude-sonnet-4"placeholder,
		{name: "in_stream_conflict_rejected", source: BillingModelSourceResponse, model: "claude-sonnet-4", conflict: trueplaceholder,
		{name: "media_billed_request_rejected", source: BillingModelSourceResponse, model: "claude-sonnet-4", mediaBilled: trueplaceholder,
		{name: "blank_declaration_rejected", source: BillingModelSourceResponse, model: "   "placeholder,
placeholder
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
...
func TestBillingServiceHasIdentifiedTokenPricing_RejectsFamilyGuesses(t *testing.T) {
	...
	billing := newGatewayRecordUsageServiceForTest(
		&openAIRecordUsageLogRepoStub{placeholder, &openAIRecordUsageUserRepoStub{placeholder, &openAIRecordUsageSubRepoStub{placeholder,
	).billingService
...
func TestResponseModelBillingAdoptable(t *testing.T) {
	...
	cost := func(total float64) *CostBreakdown {
		return &CostBreakdown{TotalCost: total, ActualCost: totalplaceholder
placeholder
	tests := []struct {
		name                  string
...
placeholder{
		// 1. 不得更贵
		{name: "cheaper_adopted", baseline: cost(1), response: cost(0.5), want: trueplaceholder,
		{name: "equal_adopted", baseline: cost(1), response: cost(1), want: trueplaceholder,
		{name: "float_noise_within_epsilon_adopted", baseline: cost(1), response: cost(1 + 1e-13), want: trueplaceholder,
		{name: "pricier_rejected", baseline: cost(1), response: cost(1.0001)placeholder,

		// 2. 不得把一笔本应计费的请求归零（价格表里有显式写 0 的条目，能通过确定性识别）
		{name: "zeroing_a_billable_request_rejected", baseline: cost(1), response: cost(0)placeholder,
		{name: "negative_cost_rejected_as_zeroing", baseline: cost(1), response: cost(-1)placeholder,
		{name: "already_zero_baseline_unaffected", baseline: cost(0), response: cost(0), want: trueplaceholder,

		// 3. 不得从渠道定价跨到全局价格表（否则渠道加价被带日期的自报模型名绕过）
		{name: "channel_priced_baseline_to_global_rejected", baseline: cost(1), response: cost(0.5), baselineChannelPriced: trueplaceholder,
		{name: "channel_priced_on_both_sides_adopted", baseline: cost(1), response: cost(0.5), baselineChannelPriced: true, responseChannelPriced: true, want: trueplaceholder,
		{name: "global_baseline_to_channel_priced_adopted", baseline: cost(1), response: cost(0.5), responseChannelPriced: true, want: trueplaceholder,

		{name: "nil_baseline_rejected", response: cost(0.5)placeholder,
		{name: "nil_response_rejected", baseline: cost(1)placeholder,
placeholder
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
