...
func TestOpenAIWSConnPool_NextConnIDFormat(t *testing.T) {
	pool := newOpenAIWSConnPool(&config.Config{placeholder)
...
func TestNormalizeOpenAIWSRoutingAffinityPrefersCanonicalAndSortsVariants(t *testing.T) {
	headers := http.Header{
		"X-CODEX-ROUTING-HINT": []string{" variant-uppercase "placeholder,
		"X-Codex-Routing-Hint": []string{" ", " canonical "placeholder,
placeholder
...
func TestOpenAIWSConnLease_WriteJSONAndGuards(t *testing.T) {
	conn := newOpenAIWSConn("lease_write", 1, &openAIWSFakeConn{placeholder, nil)
	lease := &openAIWSConnLease{conn: connplaceholder
	require.NoError(t, lease.WriteJSON(map[string]any{"type": "response.create"placeholder, 0))

	var nilLease *openAIWSConnLease
	err := nilLease.WriteJSONWithContextTimeout(context.Background(), map[string]any{"type": "response.create"placeholder, time.Second)
	require.ErrorIs(t, err, errOpenAIWSConnClosed)

	err = (&openAIWSConnLease{placeholder).WriteJSONWithContextTimeout(context.Background(), map[string]any{"type": "response.create"placeholder, time.Second)
	require.ErrorIs(t, err, errOpenAIWSConnClosed)
placeholder

func TestOpenAIWSConn_WriteJSONWithTimeout_NilParentContextUsesBackground(t *testing.T) {
	probe := &openAIWSContextProbeConn{placeholder
	conn := newOpenAIWSConn("ctx_probe", 1, probe, nil)
	require.NoError(t, conn.writeJSONWithTimeout(context.Background(), map[string]any{"type": "response.create"placeholder, 0))
...
func TestOpenAIWSConnPool_EnsureTargetIdleAsync(t *testing.T) {
	...
	accountID := int64(77)
	account := &Account{ID: accountID, Platform: PlatformOpenAI, Type: AccountTypeAPIKeyplaceholder
	ap := pool.getOrCreateAccountPool(accountID)
	ap.mu.Lock()
...
func TestOpenAIWSConnPool_EnsureTargetIdleAsyncFailureSuppress(t *testing.T) {
	...
	pool := newOpenAIWSConnPool(cfg)
	dialer := &openAIWSAlwaysFailDialer{placeholder
	pool.setClientDialerForTest(dialer)
...
func TestOpenAIWSConnPool_AcquireQueueWaitMetrics(t *testing.T) {
	...
	accountID := int64(99)
	account := &Account{ID: accountID, Platform: PlatformOpenAI, Type: AccountTypeAPIKeyplaceholder
	conn := newOpenAIWSConn("busy", accountID, &openAIWSFakeConn{placeholder, nil)
...
func TestOpenAIWSConnPool_DialSuccessWakesTopologyWaiterAndCanceledWaiterDoesNotLoseLease(t *testing.T) {
	...
	pool.setClientDialerForTest(dialer)
	account := &Account{ID: 991, Platform: PlatformOpenAI, Type: AccountTypeOAuthplaceholder
	req := openAIWSAcquireRequest{Account: account, WSURL: "wss://example.com/v1/responses"placeholder
...
placeholder
	...
	go func() {
		...
		firstCh <- result{lease: lease, err: errplaceholder
placeholder()
	<-dialer.firstStarted
	...
	waitReq := req
	waitReq.Headers = http.Header{openAICodexRoutingHintHeader: {"model=gpt-5.6-codex;tier=priority"placeholderplaceholder
	go func() {
		lease, err := pool.Acquire(waitCtx, waitReq)
		secondCh <- result{lease: lease, err: errplaceholder
placeholder()
	...
	placeholder
		return false
placeholder, time.Second, 5*time.Millisecond)
...
func TestOpenAIWSConnPool_PrewarmHintChangeDoesNotInvalidateHealthyDial(t *testing.T) {
	...
	dialer := newOpenAIWSFirstDialBlockingCaptureDialer()
	pool.setClientDialerForTest(dialer)
	account := &Account{ID: 992, Platform: PlatformOpenAI, Type: AccountTypeOAuthplaceholder
...
func TestOpenAIWSConnPool_ClearAccountWakesIncompatibleTopologyWaiter(t *testing.T) {
	...
	dialer := &openAIWSCountingDialer{placeholder
	pool.setClientDialerForTest(dialer)
	account := &Account{ID: 993, Platform: PlatformOpenAI, Type: AccountTypeOAuthplaceholder
	baseReq := openAIWSAcquireRequest{Account: account, WSURL: "wss://example.com/v1/responses"placeholder
	betaAReq := baseReq
	betaAReq.Headers = http.Header{"X-Codex-Beta-Features": {"feature_a"placeholderplaceholder
	betaBReq := baseReq
	betaBReq.Headers = http.Header{"X-Codex-Beta-Features": {"feature_b"placeholderplaceholder
...
placeholder
	...
	go func() {
		...
		resultCh <- result{lease: lease, err: acquireErrplaceholder
placeholder()

	require.Never(t, func() bool { return dialer.DialCount() > 1 placeholder, 50*time.Millisecond, 5*time.Millisecond)
...
func TestOpenAIWSConnPool_ClearAccountDoesNotReviveInFlightDialGeneration(t *testing.T) {
	...
	dialer := newOpenAIWSFirstDialBlockingCaptureDialer()
	pool.setClientDialerForTest(dialer)
	account := &Account{ID: 994, Platform: PlatformOpenAI, Type: AccountTypeOAuthplaceholder
	req := openAIWSAcquireRequest{Account: account, WSURL: "wss://example.com/v1/responses"placeholder
...
placeholder
	...
	go func() {
		lease, err := pool.Acquire(context.Background(), req)
		resultCh <- result{lease: lease, err: errplaceholder
placeholder()
	<-dialer.firstStarted
...
func TestOpenAIWSConnPool_ForceNewConnSkipsReuse(t *testing.T) {
	...
	account := &Account{ID: 123, Platform: PlatformOpenAI, Type: AccountTypeAPIKeyplaceholder
...
func TestOpenAIWSConnPool_AcquireReusesOnlyMatchingBetaFeatures(t *testing.T) {
	...
	account := &Account{ID: 128, Platform: PlatformOpenAI, Type: AccountTypeAPIKeyplaceholder
...
placeholder
	...
	betaReq := baseReq
	betaReq.Headers = http.Header{"X-Codex-Beta-Features": {" remote_compaction_v2 ", " responses_websockets_v2 "placeholderplaceholder
...
placeholder
	...
	reorderedReq.Headers = http.Header{"X-Codex-Beta-Features": {"responses_websockets_v2,remote_compaction_v2"placeholderplaceholder
...
func TestOpenAIWSConnPool_AcquireReusesSameStableIdentityWithDifferentTurnMetadata(t *testing.T) {
	...
	headers.Set("x-codex-turn-metadata", `{"turn_id":"turn-a"placeholder`)
...
placeholder
	...
	nextHeaders.Set("x-codex-turn-metadata", `{"turn_id":"turn-b"placeholder`)
...
func TestOpenAIWSConnPool_AcquireDoesNotReuseDifferentStableIdentity(t *testing.T) {
	for _, tt := range []struct {
		...
		value  string
placeholder{
		{name: "installation", header: "x-codex-installation-id", value: "install-b"placeholder,
		{name: "session hyphen", header: "session-id", value: "session-hyphen-b"placeholder,
		{name: "session underscore", header: "session_id", value: "session-underscore-b"placeholder,
		{name: "thread", header: "thread-id", value: "thread-b"placeholder,
		{name: "client request", header: "x-client-request-id", value: "client-request-b"placeholder,
		{name: "window", header: "x-codex-window-id", value: "window-b"placeholder,
placeholder {
...
func TestOpenAIWSConnPool_AcquireReplacesIdleConnWithDifferentBetaFeatures(t *testing.T) {
	...
	account := &Account{ID: 129, Platform: PlatformOpenAI, Type: AccountTypeAPIKeyplaceholder
...
placeholder
	...
	betaLease, err := pool.Acquire(context.Background(), openAIWSAcquireRequest{
		Account: account,
		WSURL:   "wss://example.com/v1/responses",
		Headers: http.Header{"X-Codex-Beta-Features": {"remote_compaction_v2"placeholderplaceholder,
placeholder)
placeholder
	require.False(t, betaLease.Reused())
	require.NotEqual(t, plainConnID, betaLease.ConnID())
...
func TestOpenAIWSConnPool_AcquireWaitsForBusyIncompatibleConnection(t *testing.T) {
	...
	dialer := &openAIWSCountingDialer{placeholder
	pool.setClientDialerForTest(dialer)
	account := &Account{ID: 130, Platform: PlatformOpenAI, Type: AccountTypeAPIKeyplaceholder
	baseReq := openAIWSAcquireRequest{Account: account, WSURL: "wss://example.com/v1/responses"placeholder
...
placeholder
	...
	go func() {
		betaReq := baseReq
		betaReq.Headers = http.Header{"X-Codex-Beta-Features": {"remote_compaction_v2"placeholderplaceholder
		lease, acquireErr := pool.Acquire(ctx, betaReq)
		resultCh <- acquireResult{lease: lease, err: acquireErrplaceholder
		done.Store(true)
placeholder()
...
func TestOpenAIWSConnPool_AcquireReplacesIncompatibleIdleWhenMatchingBusy(t *testing.T) {
	...
	dialer := &openAIWSCountingDialer{placeholder
	pool.setClientDialerForTest(dialer)
	account := &Account{ID: 131, Platform: PlatformOpenAI, Type: AccountTypeAPIKeyplaceholder
	baseReq := openAIWSAcquireRequest{Account: account, WSURL: "wss://example.com/v1/responses"placeholder
...
func TestOpenAIWSConnPool_AcquireForcePreferredConnUnavailable(t *testing.T) {
	...
	pool := newOpenAIWSConnPool(cfg)
	account := &Account{ID: 124, Platform: PlatformOpenAI, Type: AccountTypeAPIKeyplaceholder
	ap := pool.getOrCreateAccountPool(account.ID)
	otherConn := newOpenAIWSConn("other_conn", account.ID, &openAIWSFakeConn{placeholder, nil)
	ap.mu.Lock()
...
func TestOpenAIWSConnPool_AcquireForcePreferredConnQueuesOnPreferredOnly(t *testing.T) {
	...
	pool := newOpenAIWSConnPool(cfg)
	account := &Account{ID: 125, Platform: PlatformOpenAI, Type: AccountTypeAPIKeyplaceholder
	ap := pool.getOrCreateAccountPool(account.ID)
	preferredConn := newOpenAIWSConn("preferred_conn", account.ID, &openAIWSFakeConn{placeholder, nil)
	otherConn := newOpenAIWSConn("other_conn_idle", account.ID, &openAIWSFakeConn{placeholder, nil)
...
func TestOpenAIWSConnPool_AcquireForcePreferredConnDirectAndQueueFull(t *testing.T) {
	...
	pool := newOpenAIWSConnPool(cfg)
	account := &Account{ID: 127, Platform: PlatformOpenAI, Type: AccountTypeAPIKeyplaceholder
	ap := pool.getOrCreateAccountPool(account.ID)
	preferredConn := newOpenAIWSConn("preferred_conn_direct", account.ID, &openAIWSFakeConn{placeholder, nil)
	otherConn := newOpenAIWSConn("other_conn_direct", account.ID, &openAIWSFakeConn{placeholder, nil)
	ap.mu.Lock()
	ap.conns[preferredConn.id] = preferredConn
	ap.conns[otherConn.id] = otherConn
	ap.lastCleanupAt = time.Now()
...
func TestOpenAIWSConnPool_CleanupSkipsPinnedConn(t *testing.T) {
	...
	accountID := int64(126)
	ap := pool.getOrCreateAccountPool(accountID)
	pinnedConn := newOpenAIWSConn("pinned_conn", accountID, &openAIWSFakeConn{placeholder, nil)
	idleConn := newOpenAIWSConn("idle_conn", accountID, &openAIWSFakeConn{placeholder, nil)
	ap.mu.Lock()
...
func TestOpenAIWSConnPool_PinUnpinConnBranches(t *testing.T) {
	...
	ap := &openAIWSAccountPool{
		conns: map[string]*openAIWSConn{placeholder,
placeholder
	...
	conn := newOpenAIWSConn("pin_refcount", accountID, &openAIWSFakeConn{placeholder, nil)
	ap.mu.Lock()
	ap.conns[conn.id] = conn
	ap.mu.Unlock()
...
func TestOpenAIWSConnPool_EffectiveMaxConnsByAccount(t *testing.T) {
	...
	oauthHigh := &Account{Platform: PlatformOpenAI, Type: AccountTypeOAuth, Concurrency: 10placeholder
	require.Equal(t, 8, pool.effectiveMaxConnsByAccount(oauthHigh), "应受全局硬上限约束")

	oauthLow := &Account{Platform: PlatformOpenAI, Type: AccountTypeOAuth, Concurrency: 3placeholder
	require.Equal(t, 3, pool.effectiveMaxConnsByAccount(oauthLow))

	apiKeyHigh := &Account{Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Concurrency: 10placeholder
	require.Equal(t, 6, pool.effectiveMaxConnsByAccount(apiKeyHigh), "API Key 应按系数缩放")

	apiKeyLow := &Account{Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Concurrency: 1placeholder
	require.Equal(t, 1, pool.effectiveMaxConnsByAccount(apiKeyLow), "最小值应保持为 1")

	unlimited := &Account{Platform: PlatformOpenAI, Type: AccountTypeOAuth, Concurrency: 0placeholder
...
func TestOpenAIWSConnPool_EffectiveMaxConnsDisabledFallbackHardCap(t *testing.T) {
	...
	pool := newOpenAIWSConnPool(cfg)
	account := &Account{Platform: PlatformOpenAI, Type: AccountTypeOAuth, Concurrency: 2placeholder
...
func TestOpenAIWSConnPool_EffectiveMaxConnsByAccount_ModeRouterV2RespectsHardCap(t *testing.T) {
	...
	high := &Account{Platform: PlatformOpenAI, Type: AccountTypeOAuth, Concurrency: 20placeholder
	require.Equal(t, 8, pool.effectiveMaxConnsByAccount(high), "v2 路径也必须受连接池硬上限约束")

	nonPositive := &Account{Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Concurrency: 0placeholder
...
func TestOpenAIWSConnPool_AcquireRejectsWhenEffectiveMaxConnsIsZero(t *testing.T) {
	...
	account := &Account{ID: 901, Platform: PlatformOpenAI, Type: AccountTypeOAuth, Concurrency: 0placeholder
	_, err := pool.Acquire(context.Background(), openAIWSAcquireRequest{
		Account: account,
		WSURL:   "wss://example.com/v1/responses",
placeholder)
...
func TestOpenAIWSConnLease_ReadMessageWithContextTimeout_PerRead(t *testing.T) {
	conn := newOpenAIWSConn("timeout", 1, &openAIWSBlockingConn{readDelay: 80 * time.Millisecondplaceholder, nil)
	lease := &openAIWSConnLease{conn: connplaceholder
...
func TestOpenAIWSConnLease_WriteJSONWithContextTimeout_RespectsParentContext(t *testing.T) {
	conn := newOpenAIWSConn("write_timeout_ctx", 1, &openAIWSWriteBlockingConn{placeholder, nil)
	lease := &openAIWSConnLease{conn: connplaceholder
...
placeholder()
	...
	start := time.Now()
	err := lease.WriteJSONWithContextTimeout(parentCtx, map[string]any{"type": "response.create"placeholder, 2*time.Minute)
...
func TestOpenAIWSConnLease_PingWithTimeout(t *testing.T) {
	conn := newOpenAIWSConn("ping_ok", 1, &openAIWSFakeConn{placeholder, nil)
	lease := &openAIWSConnLease{conn: connplaceholder
...
func TestOpenAIWSConn_ReadAndWriteCanProceedConcurrently(t *testing.T) {
	conn := newOpenAIWSConn("full_duplex", 1, &openAIWSBlockingConn{readDelay: 120 * time.Millisecondplaceholder, nil)
...
func TestOpenAIWSConnPool_BackgroundPingSweep_EvictsDeadIdleConn(t *testing.T) {
	...
	accountID := int64(301)
	ap := pool.getOrCreateAccountPool(accountID)
	conn := newOpenAIWSConn("dead_idle", accountID, &openAIWSPingFailConn{placeholder, nil)
	ap.mu.Lock()
	ap.conns[conn.id] = conn
	ap.mu.Unlock()
...
func TestOpenAIWSConnPool_BackgroundCleanupSweep_WithoutAcquire(t *testing.T) {
	...
	accountID := int64(302)
	ap := pool.getOrCreateAccountPool(accountID)
	stale := newOpenAIWSConn("stale_bg", accountID, &openAIWSFakeConn{placeholder, nil)
	stale.createdAtNano.Store(time.Now().Add(-2 * time.Hour).UnixNano())
	stale.lastUsedNano.Store(time.Now().Add(-2 * time.Hour).UnixNano())
	ap.mu.Lock()
	ap.conns[stale.id] = stale
...
placeholder)
	...
	poolNoStop := &openAIWSConnPool{placeholder
	require.NotPanics(t, func() {
		poolNoStop.startBackgroundWorkers()
placeholder)

	poolStopPing := &openAIWSConnPool{workerStopCh: make(chan struct{placeholder)placeholder
	pingDone := make(chan struct{placeholder)
	go func() {
...
placeholder
	...
	poolStopCleanup := &openAIWSConnPool{workerStopCh: make(chan struct{placeholder)placeholder
	cleanupDone := make(chan struct{placeholder)
	go func() {
...
func TestOpenAIWSConnPool_SnapshotIdleConnsForPing_SkipsInvalidEntries(t *testing.T) {
	pool := &openAIWSConnPool{placeholder
	pool.accounts.Store("invalid-key", &openAIWSAccountPool{placeholder)
...
placeholder
	...
	leased := newOpenAIWSConn("leased", accountID, &openAIWSFakeConn{placeholder, nil)
	require.True(t, leased.tryAcquire())
	ap.conns[leased.id] = leased

	waiting := newOpenAIWSConn("waiting", accountID, &openAIWSFakeConn{placeholder, nil)
	waiting.waiters.Store(1)
	ap.conns[waiting.id] = waiting

	idle := newOpenAIWSConn("idle", accountID, &openAIWSFakeConn{placeholder, nil)
...
func TestOpenAIWSConnPool_RunBackgroundCleanupSweep_SkipsInvalidAndUsesAccountCap(t *testing.T) {
	...
	pool := &openAIWSConnPool{cfg: cfgplaceholder
...
placeholder
	ap.conns["nil_conn"] = nil
	stale := newOpenAIWSConn("stale_bg_cleanup", accountID, &openAIWSFakeConn{placeholder, nil)
	stale.createdAtNano.Store(time.Now().Add(-2 * time.Hour).UnixNano())
	stale.lastUsedNano.Store(time.Now().Add(-2 * time.Hour).UnixNano())
	ap.conns[stale.id] = stale
	ap.lastAcquire = &openAIWSAcquireRequest{
...
func TestOpenAIWSConnPool_QueueLimitPerConn_DefaultAndConfigured(t *testing.T) {
	...
	pool := &openAIWSConnPool{cfg: &config.Config{placeholderplaceholder
...
func TestOpenAIWSDialError_ErrorAndUnwrap(t *testing.T) {
	...
	dialErr := &openAIWSDialError{StatusCode: 502, Err: baseErrplaceholder
	require.Contains(t, dialErr.Error(), "status=502")
	require.ErrorIs(t, dialErr.Unwrap(), baseErr)

	noStatus := &openAIWSDialError{Err: baseErrplaceholder
...
func TestOpenAIWSConnLease_ReadWriteHelpersAndConnStats(t *testing.T) {
	conn := newOpenAIWSConn("helper_conn", 1, &openAIWSFakeConn{placeholder, http.Header{
		"X-Test": []string{" value "placeholder,
placeholder)
	lease := &openAIWSConnLease{conn: connplaceholder

	require.NoError(t, lease.WriteJSONContext(context.Background(), map[string]any{"type": "response.create"placeholder))
...
placeholder
	...
	require.ErrorIs(t, nilConn.writeJSONWithTimeout(context.Background(), map[string]any{placeholder, time.Second), errOpenAIWSConnClosed)
...
func TestOpenAIWSConnPool_PickOldestIdleAndAccountPoolLoad(t *testing.T) {
	pool := &openAIWSConnPool{placeholder
	accountID := int64(404)
	ap := &openAIWSAccountPool{conns: map[string]*openAIWSConn{placeholderplaceholder

	idleOld := newOpenAIWSConn("idle_old", accountID, &openAIWSFakeConn{placeholder, nil)
	idleOld.lastUsedNano.Store(time.Now().Add(-10 * time.Minute).UnixNano())
	idleNew := newOpenAIWSConn("idle_new", accountID, &openAIWSFakeConn{placeholder, nil)
	idleNew.lastUsedNano.Store(time.Now().Add(-1 * time.Minute).UnixNano())
	leased := newOpenAIWSConn("leased", accountID, &openAIWSFakeConn{placeholder, nil)
	require.True(t, leased.tryAcquire())
...
placeholder
	...
	ap := &openAIWSAccountPool{
		conns: map[string]*openAIWSConn{placeholder,
placeholder
	idle := newOpenAIWSConn("idle_conn", accountID, &openAIWSFakeConn{placeholder, nil)
	leased := newOpenAIWSConn("leased_conn", accountID, &openAIWSFakeConn{placeholder, nil)
	require.True(t, leased.tryAcquire())
...
func TestOpenAIWSConnLease_BasicGetterBranches(t *testing.T) {
	...
	conn := newOpenAIWSConn("getter_conn", 1, &openAIWSFakeConn{placeholder, http.Header{"X-Test": []string{"ok"placeholderplaceholder)
	lease := &openAIWSConnLease{
		conn:      conn,
...
func TestOpenAIWSConnPool_UtilityBranches(t *testing.T) {
	var nilPool *openAIWSConnPool
	require.Equal(t, OpenAIWSPoolMetricsSnapshot{placeholder, nilPool.SnapshotMetrics())
	require.Equal(t, OpenAIWSTransportMetricsSnapshot{placeholder, nilPool.SnapshotTransportMetrics())
	...
	pool.clientDialer = &openAIWSFakeDialer{placeholder
	require.Equal(t, OpenAIWSTransportMetricsSnapshot{placeholder, pool.SnapshotTransportMetrics())
	...
	now := time.Now()
	apNilFail := &openAIWSAccountPool{prewarmFails: 1placeholder
	require.False(t, pool.shouldSuppressPrewarmLocked(apNilFail, now))
	apZeroTime := &openAIWSAccountPool{prewarmFails: 2placeholder
	require.False(t, pool.shouldSuppressPrewarmLocked(apZeroTime, now))
	require.Equal(t, 0, apZeroTime.prewarmFails)
	apOldFail := &openAIWSAccountPool{prewarmFails: 2, prewarmFailAt: now.Add(-openAIWSPrewarmFailureWindow - time.Second)placeholder
	require.False(t, pool.shouldSuppressPrewarmLocked(apOldFail, now))
	apRecentFail := &openAIWSAccountPool{prewarmFails: openAIWSPrewarmFailureSuppress, prewarmFailAt: nowplaceholder
	...
	conn := newOpenAIWSConn("health", 1, &openAIWSFakeConn{placeholder, nil)
	...
	unsafeConn := newOpenAIWSConn("unsafe_health", 1, &openAIWSIdlePingUnsupportedConn{placeholder, nil)
...
func TestOpenAIWSConn_LeaseAndTimeHelpers_NilAndClosedBranches(t *testing.T) {
	var nilConn *openAIWSConn
	nilConn.touch()
	require.Equal(t, time.Time{placeholder, nilConn.createdAt())
	require.Equal(t, time.Time{placeholder, nilConn.lastUsedAt())
	...
	conn := newOpenAIWSConn("lease_state", 1, &openAIWSFakeConn{placeholder, nil)
...
func TestOpenAIWSConnLease_ReadWriteNilConnBranches(t *testing.T) {
	lease := &openAIWSConnLease{placeholder
	require.ErrorIs(t, lease.WriteJSON(map[string]any{"k": "v"placeholder, time.Second), errOpenAIWSConnClosed)
	require.ErrorIs(t, lease.WriteJSONContext(context.Background(), map[string]any{"k": "v"placeholder), errOpenAIWSConnClosed)
...
func TestOpenAIWSConnLease_ReleasedLeaseGuards(t *testing.T) {
	conn := newOpenAIWSConn("released_guard", 1, &openAIWSFakeConn{placeholder, nil)
	lease := &openAIWSConnLease{conn: connplaceholder
	...
	require.ErrorIs(t, lease.WriteJSON(map[string]any{"k": "v"placeholder, time.Second), errOpenAIWSConnClosed)
	require.ErrorIs(t, lease.WriteJSONContext(context.Background(), map[string]any{"k": "v"placeholder), errOpenAIWSConnClosed)
	require.ErrorIs(t, lease.WriteJSONWithContextTimeout(context.Background(), map[string]any{"k": "v"placeholder, time.Second), errOpenAIWSConnClosed)
...
func TestOpenAIWSConnLease_MarkBrokenAfterRelease_NoEviction(t *testing.T) {
	conn := newOpenAIWSConn("released_markbroken", 7, &openAIWSFakeConn{placeholder, nil)
	ap := &openAIWSAccountPool{
...
func TestOpenAIWSConn_AdditionalGuardBranches(t *testing.T) {
	...
	connBusy := newOpenAIWSConn("busy_ctx", 1, &openAIWSFakeConn{placeholder, nil)
	...
	connClosed := newOpenAIWSConn("closed_guard", 1, &openAIWSFakeConn{placeholder, nil)
	connClosed.close()
	require.ErrorIs(
		t,
		connClosed.writeJSONWithTimeout(context.Background(), map[string]any{"k": "v"placeholder, time.Second),
	...
	require.ErrorIs(t, connNoWS.writeJSON(map[string]any{"k": "v"placeholder, context.Background()), errOpenAIWSConnClosed)
	...
	connOK := newOpenAIWSConn("ok", 1, &openAIWSFakeConn{placeholder, nil)
	require.NoError(t, connOK.writeJSON(map[string]any{"k": "v"placeholder, nil))
	_, err = connOK.readMessageWithContextTimeout(context.Background(), 0)
placeholder
	require.NoError(t, connOK.pingWithTimeout(0))

	connZero := newOpenAIWSConn("zero_ts", 1, &openAIWSFakeConn{placeholder, nil)
	...
	copied := cloneHeader(http.Header{
		"X-Empty": []string{placeholder,
		"X-Test":  []string{"v1"placeholder,
placeholder)
	require.Contains(t, copied, "X-Empty")
	require.Nil(t, copied["X-Empty"])
	require.Equal(t, "v1", copied.Get("X-Test"))

	closeOpenAIWSConns([]*openAIWSConn{nil, connOKplaceholder)
placeholder

func TestOpenAIWSConnPool_CanceledWaiterReturnsDeliveredLease(t *testing.T) {
	conn := newOpenAIWSConn("cancelled_delivery", 1, &openAIWSFakeConn{placeholder, nil)
...
func TestOpenAIWSConnLease_MarkBrokenEvictsConn(t *testing.T) {
	...
	conn := newOpenAIWSConn("broken_me", accountID, &openAIWSFakeConn{placeholder, nil)
	ap := pool.getOrCreateAccountPool(accountID)
	ap.mu.Lock()
	ap.conns[conn.id] = conn
	ap.mu.Unlock()
...
func TestOpenAIWSConnPool_TargetConnCountAndPrewarmBranches(t *testing.T) {
	...
	busy := newOpenAIWSConn("busy_target", 2, &openAIWSFakeConn{placeholder, nil)
	...
	req := openAIWSAcquireRequest{
		Account: &Account{ID: 999, Platform: PlatformOpenAI, Type: AccountTypeAPIKeyplaceholder,
		WSURL:   "wss://example.com/v1/responses",
placeholder
	...
	failPool.setClientDialerForTest(&openAIWSAlwaysFailDialer{placeholder)
...
func TestOpenAIWSConnPool_Acquire_ErrorBranches(t *testing.T) {
	var nilPool *openAIWSConnPool
	_, err := nilPool.Acquire(context.Background(), openAIWSAcquireRequest{placeholder)
placeholder
...
placeholder
	...
	account := &Account{ID: 2001, Platform: PlatformOpenAI, Type: AccountTypeAPIKeyplaceholder
...
placeholder)
	...
	account2 := &Account{ID: 2002, Platform: PlatformOpenAI, Type: AccountTypeAPIKeyplaceholder
	ap2 := fullPool.getOrCreateAccountPool(account2.ID)
	conn := newOpenAIWSConn("queue_full", account2.ID, &openAIWSFakeConn{placeholder, nil)
	require.True(t, conn.tryAcquire())
...
type openAIWSFakeDialer struct{placeholder
...
func (d *openAIWSFakeDialer) Dial(
	...
	_ = headers
	_ = proxyURL
	return &openAIWSFakeConn{placeholder, 0, nil, nil
placeholder
...
func (c *openAIWSPingBlockingConn) ReadMessage(context.Context) ([]byte, error) {
	return []byte(`{"type":"response.completed","response":{"id":"resp_blocking_ping"placeholderplaceholder`), nil
placeholder
...
placeholder
	return []byte(`{"type":"response.completed","response":{"id":"resp_fake"placeholderplaceholder`), nil
placeholder
...
placeholder
	...
	case <-ctx.Done():
		return nil, ctx.Err()
	case <-timer.C:
		return []byte(`{"type":"response.completed","response":{"id":"resp_blocking"placeholderplaceholder`), nil
placeholder
placeholder
...
type openAIWSWriteBlockingConn struct{placeholder
...
func (c *openAIWSWriteBlockingConn) ReadMessage(context.Context) ([]byte, error) {
	return []byte(`{"type":"response.completed","response":{"id":"resp_write_block"placeholderplaceholder`), nil
placeholder
...
type openAIWSPingFailConn struct{placeholder
...
func (c *openAIWSPingFailConn) ReadMessage(context.Context) ([]byte, error) {
	return []byte(`{"type":"response.completed","response":{"id":"resp_ping_fail"placeholderplaceholder`), nil
placeholder
...
func (c *openAIWSContextProbeConn) ReadMessage(context.Context) ([]byte, error) {
	return []byte(`{"type":"response.completed","response":{"id":"resp_ctx_probe"placeholderplaceholder`), nil
placeholder
...
type openAIWSNilConnDialer struct{placeholder
...
func TestOpenAIWSConnPool_DialConnNilConnection(t *testing.T) {
	...
	pool := newOpenAIWSConnPool(cfg)
	pool.setClientDialerForTest(&openAIWSNilConnDialer{placeholder)
	account := &Account{ID: 91, Platform: PlatformOpenAI, Type: AccountTypeAPIKeyplaceholder
