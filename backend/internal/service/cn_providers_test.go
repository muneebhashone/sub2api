...
func TestCNParseF64(t *testing.T) {
	...
	cases := []struct {
		...
		ok   bool
placeholder{
		{"float64", float64(12.5), 12.5, trueplaceholder,
		{"int", 100, 100, trueplaceholder,
		{"numeric string", "33.3", 33.3, trueplaceholder,
		{"trim string", "  7  ", 7, trueplaceholder,
		{"non-numeric string", "abc", 0, falseplaceholder,
		{"nil", nil, 0, falseplaceholder,
placeholder
...
func TestParseKimiUsageTiers(t *testing.T) {
	...
	body := []byte(`{
		"limits": [
			{"name": "5h", "detail": {"limit": 1000, "remaining": 600, "resetTime": "2026-08-14T15:00:00Z"placeholderplaceholder,
			{"name": "ignored-second-detail", "detail": {"limit": 999, "remaining": 0, "resetTime": "2026-08-14T20:00:00Z"placeholderplaceholder
		],
		"usage": {"limit": 10000, "remaining": 4000, "resetTime": "2026-08-18T00:00:00Z"placeholder
placeholder`)
...
func TestParseKimiUsageTiers_LimitZero(t *testing.T) {
	t.Parallel()
	body := []byte(`{"limits":[{"detail":{"limit":0,"remaining":0,"resetTime":"2026-08-14T15:00:00Z"placeholderplaceholder]placeholder`)
	tiers := parseKimiUsageTiers(body)
...
func TestParseZhipuTokenTiers_UnitClassification(t *testing.T) {
	...
	data := gjson.Parse(`{
		"limits": [
			{"type":"TOKENS_LIMIT","unit":6,"percentage":70,"nextResetTime":1700000000000placeholder,
			{"type":"TOKENS_LIMIT","unit":3,"percentage":20,"nextResetTime":1700000099999placeholder
		]
placeholder`)
...
func TestParseZhipuTokenTiers_SingleTierOldPlan(t *testing.T) {
	t.Parallel()
	data := gjson.Parse(`{"limits":[{"type":"TOKENS_LIMIT","unit":3,"percentage":15,"nextResetTime":1700000000000placeholder]placeholder`)
	tiers := parseZhipuTokenTiers(data)
	require.Len(t, tiers, 1)
	require.Equal(t, "5h", tiers[0].Window)
placeholder
...
func TestParseZhipuTokenTiers_FallbackHeuristic(t *testing.T) {
	...
	data := gjson.Parse(`{
		"limits": [
			{"type":"TOKENS_LIMIT","percentage":50,"nextResetTime":1700000000000placeholder,
			{"type":"TOKENS_LIMIT","percentage":10placeholder
		]
placeholder`)
	tiers := parseZhipuTokenTiers(data)
	require.Len(t, tiers, 2)
...
func TestParseZhipuTokenTiers_IgnoresNonTokenEntries(t *testing.T) {
	t.Parallel()
	data := gjson.Parse(`{"limits":[{"type":"OTHER_LIMIT","unit":3,"percentage":99placeholder]placeholder`)
...
func TestCNQuotaExtraUpdates(t *testing.T) {
	...
	tiers := []CNQuotaTier{
		{Window: "5h", UsedPercent: 40, ResetAt: "2026-08-14T15:00:00Z"placeholder,
		{Window: "weekly", UsedPercent: 60, ResetAt: "2026-08-18T00:00:00Z"placeholder,
placeholder
...
func TestCNProviderResponseIndicatesInsufficientBalance(t *testing.T) {
	t.Parallel()
	positive := []string{
		`{"error":{"message":"余额不足"placeholderplaceholder`,
		`{"error":{"message":"Insufficient balance"placeholderplaceholder`,
		`{"code":"insufficient_credit"placeholder`,
...
placeholder
	negative := []string{
		`{"error":{"message":"rate limit exceeded"placeholderplaceholder`,
		`{"error":{"message":"quota exhausted"placeholderplaceholder`,
		``,
placeholder
...
func TestCNBalanceURL(t *testing.T) {
	t.Parallel()
	kimi := &Account{Platform: PlatformKimiplaceholder
	require.Equal(t, "https://api.moonshot.cn/v1/users/me/balance", cnBalanceURL(kimi))

	deepseek := &Account{
		Platform:    PlatformDeepseek,
placeholder"base_url": "https://api.deepseek.com"placeholder,
placeholder
...
placeholder
	...
	partial := &Account{
		Platform: PlatformKimi,
		Extra:    map[string]any{"kimi_5h_reset_at": "2026-08-14T15:00:00Z"placeholder, // 无 used_percent
placeholder
	require.Empty(t, filterNil(cnProviderThresholdCandidates(partial, PlatformKimi)))

	// 空 Extra / nil account 安全返回。
	require.Empty(t, filterNil(cnProviderThresholdCandidates(&Account{Platform: PlatformKimiplaceholder, PlatformKimi)))
placeholder
...
func TestEvaluateAccountSchedulingThreshold_KimiCodingPlan(t *testing.T) {
	...
	placeholder,
placeholder
	decision := EvaluateAccountSchedulingThreshold(account, map[string]int{PlatformKimi: 80placeholder, now)
...
func TestEvaluateAccountSchedulingThreshold_CNWindowResetSkipped(t *testing.T) {
	...
	placeholder,
placeholder
	require.False(t, EvaluateAccountSchedulingThreshold(expired, map[string]int{PlatformZhipu: 80placeholder, now).ShouldPause)
	...
	placeholder,
placeholder
	require.False(t, EvaluateAccountSchedulingThreshold(low, map[string]int{PlatformZhipu: 80placeholder, now).ShouldPause)
placeholder
...
placeholder"account_mode": AccountModePayGplaceholder,
		Extra:       map[string]any{"kimi_5h_reset_at": future5h.Format(time.RFC3339)placeholder,
placeholder
...
placeholder"api_key": "sk-kimi"placeholder,
placeholder
...
placeholder"api_key": "sk-leak"placeholder,
placeholder
	...
	openai := &Account{
		...
		Type:        AccountTypeAPIKey,
placeholder"api_key": "sk-openai"placeholder,
placeholder
...
func TestBuildUpstreamModelsRequest_CNProviders(t *testing.T) {
	...
	svc := &AccountTestService{cfg: &config.Config{placeholderplaceholder
	cases := []struct {
		name     string
		platform string
		mode     string
		wantURL  string
placeholder{
		{"kimi default", PlatformKimi, "", "https://api.moonshot.cn/v1/models"placeholder,
		{"kimi coding", PlatformKimi, AccountModeCoding, "https://api.kimi.com/coding/v1/models"placeholder,
		{"zhipu default", PlatformZhipu, "", "https://open.bigmodel.cn/api/paas/v4/models"placeholder,
		{"zhipu coding", PlatformZhipu, AccountModeCoding, "https://open.bigmodel.cn/api/coding/paas/v4/models"placeholder,
		{"deepseek", PlatformDeepseek, "", "https://api.deepseek.com/v1/models"placeholder,
placeholder
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			creds := map[string]any{"api_key": "sk-test"placeholder
			if tc.mode != "" {
				creds["account_mode"] = tc.mode
		placeholder
			account := &Account{ID: 1, Platform: tc.platform, Type: AccountTypeAPIKey, Credentials: credsplaceholder
...
func TestGetAPIProtocol(t *testing.T) {
	...
	placeholder
	placeholderPlatform: platform, Type: AccountTypeAPIKey, Credentials: credsplaceholder
placeholder
	...
	require.Equal(t, APIProtocolChatCompletions, (&Account{Platform: PlatformOpenAI, Type: AccountTypeAPIKeyplaceholder).GetAPIProtocol(), "非 CN 供应商恒为默认")
placeholder
...
placeholder{
		{"kimi payg", PlatformKimi, AccountModePayG, DefaultKimiPayGBaseURL, DefaultKimiPayGAnthropicBaseURL, DefaultKimiPayGBaseURLplaceholder,
		{"kimi coding", PlatformKimi, AccountModeCoding, DefaultKimiCodingBaseURL, DefaultKimiCodingAnthropicBaseURL, DefaultKimiCodingBaseURLplaceholder,
		{"zhipu payg", PlatformZhipu, AccountModePayG, DefaultZhipuPayGBaseURL, DefaultZhipuAnthropicBaseURL, DefaultZhipuPayGBaseURLplaceholder,
		{"zhipu coding", PlatformZhipu, AccountModeCoding, DefaultZhipuCodingBaseURL, DefaultZhipuAnthropicBaseURL, DefaultZhipuCodingBaseURLplaceholder,
		{"deepseek", PlatformDeepseek, AccountModePayG, DefaultDeepseekBaseURL, DefaultDeepseekAnthropicBaseURL, DefaultDeepseekBaseURLplaceholder,
placeholder
...
placeholder"api_protocol": APIProtocolAnthropicplaceholder,
placeholder).GetAnthropicProtocolBaseURL())
	require.Equal(t, "https://api.kimi.com/coding", (&Account{
		Platform: PlatformKimi, Type: AccountTypeAPIKey,
placeholder"api_protocol": APIProtocolAnthropic, "account_mode": AccountModeCodingplaceholder,
placeholder).GetAnthropicProtocolBaseURL())
...
placeholder).GetAnthropicProtocolBaseURL())
	...
	require.Equal(t, "https://custom.example.com/anthropic", (&Account{
		Platform: PlatformZhipu, Type: AccountTypeAPIKey,
placeholder"api_protocol": APIProtocolAnthropic, "base_url": "https://custom.example.com/anthropic"placeholder,
placeholder).GetAnthropicProtocolBaseURL())

	// 非 Anthropic 协议返回空串
	require.Empty(t, (&Account{
		Platform: PlatformZhipu, Type: AccountTypeAPIKey,
placeholder"base_url": "https://open.bigmodel.cn/api/paas/v4"placeholder,
placeholder).GetAnthropicProtocolBaseURL())
placeholder
...
placeholder
	...
	ccAccount := &Account{
		Platform: PlatformDeepseek, Type: AccountTypeAPIKey,
placeholder"base_url": "https://ds-relay.example.com"placeholder,
placeholder
...
func TestBuildOpenAIResponsesURLForPlatform(t *testing.T) {
	...
	require.Equal(t, "https://token.cvte.com/responses", buildOpenAIResponsesURLForPlatform(PlatformDeepseek, "https://token.cvte.com"))
	require.Equal(t, "https://token.cvte.com/v1/responses", buildOpenAIResponsesURLForPlatform(PlatformDeepseek, "https://token.cvte.com/v1"))
...
func TestNormalizeDeepSeekResponsesRequestBody(t *testing.T) {
	...
	deepseekResponses := &Account{
		Platform: PlatformDeepseek, Type: AccountTypeAPIKey,
placeholder"api_protocol": APIProtocolResponsesplaceholder,
placeholder
	body := []byte(`{"model":"deepseek-v4-pro","store":true,"previous_response_id":"resp_123","input":"hi"placeholder`)
	...
	deepseekAdaptive := &Account{
		Platform: PlatformDeepseek, Type: AccountTypeAPIKey,
placeholder"api_protocol": APIProtocolAdaptiveplaceholder,
placeholder
	...
	deepseekCC := &Account{Platform: PlatformDeepseek, Type: AccountTypeAPIKeyplaceholder
	require.Equal(t, string(body), string(normalizeDeepSeekResponsesRequestBody(deepseekCC, body)))

	// openai 账号原样返回
	openai := &Account{Platform: PlatformOpenAI, Type: AccountTypeAPIKeyplaceholder
...
placeholder
	...
	zhipu.Extra = map[string]any{"anthropic_apikey_auth_scheme": "authorization_bearer"placeholder
