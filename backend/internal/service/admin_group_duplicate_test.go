...
type duplicateGroupRepoStub struct {
	...
	names              map[string]struct{placeholder
...
func newDuplicateGroupRepoStub(source *Group) *duplicateGroupRepoStub {
	repo := &duplicateGroupRepoStub{
		...
		names:           make(map[string]struct{placeholder),
...
placeholder
	if source != nil {
		...
		repo.names[source.Name] = struct{placeholder{placeholder
placeholder
	return repo
placeholder
...
placeholder
	...
	r.names[group.Name] = struct{placeholder{placeholder
...
func groupDuplicateTestPointer[T any](value T) *T { return &value placeholder
...
func TestDuplicateGroupCopiesConfigurationDeeplyAndResetsRuntimeState(t *testing.T) {
	...
	source := &Group{
		...
		VideoModelPrices: map[string]map[string]float64{
			VideoPriceFamilyGrokImagineVideo15: {VideoBillingResolution720P: 0.14placeholder,
	placeholder,
		...
		ModelRouting:                    map[string][]int64{"gpt-*": {13, 17placeholderplaceholder,
		ModelRoutingEnabled:             true,
		MCPXMLInject:                    true,
		SupportedModelScopes:            []string{"claude", "gemini_text"placeholder,
		SortOrder:                       9,
		...
		MessagesDispatchModelConfig: OpenAIMessagesDispatchModelConfig{
			...
			ExactModelMappings: map[string]string{"claude-special": "gpt-special"placeholder,
	placeholder,
		ModelsListConfig:        GroupModelsListConfig{Enabled: true, Models: []string{"gpt-5.4", "gpt-5-mini"placeholderplaceholder,
		RPMLimit:                99,
		...
		AccountGroups:           []AccountGroup{{AccountID: 13, GroupID: 41, Priority: 37placeholderplaceholder,
placeholder
	repo := newDuplicateGroupRepoStub(source)
	repo.sourceBindings[source.ID] = []AccountGroup{
		{AccountID: 13, GroupID: source.ID, Priority: 37placeholder,
		{AccountID: 17, GroupID: source.ID, Priority: 8placeholder,
placeholder
	svc := &adminServiceImpl{groupRepo: repo, groupDuplicateRepo: repoplaceholder
...
placeholder
	...
	require.Equal(t, []int64{source.IDplaceholder, repo.createdFromSources)
	require.Equal(t, []AccountGroup{
		{AccountID: 13, GroupID: duplicate.ID, Priority: 37placeholder,
		{AccountID: 17, GroupID: duplicate.ID, Priority: 8placeholder,
placeholder, repo.createdBindings[duplicate.ID])
...
func TestDuplicateGroupRecoversSameOperationAndScopesByAdmin(t *testing.T) {
	source := &Group{ID: 9, Name: "team", Platform: PlatformAnthropic, Status: StatusActiveplaceholder
	repo := newDuplicateGroupRepoStub(source)
	svc := &adminServiceImpl{groupRepo: repo, groupDuplicateRepo: repoplaceholder
...
func TestDuplicateGroupAdvancesNameAndTruncatesUnicodeByRunes(t *testing.T) {
	source := &Group{ID: 12, Name: "team", Platform: PlatformAnthropic, Status: StatusActiveplaceholder
	repo := newDuplicateGroupRepoStub(source)
	repo.names["team (Copy)"] = struct{placeholder{placeholder
	svc := &adminServiceImpl{groupRepo: repo, groupDuplicateRepo: repoplaceholder
...
func TestDuplicateGroupAtomicCreateFailureReturnsNoCopy(t *testing.T) {
	source := &Group{ID: 15, Name: "team", Platform: PlatformAnthropic, Status: StatusActiveplaceholder
	repo := newDuplicateGroupRepoStub(source)
