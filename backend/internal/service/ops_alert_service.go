...
placeholder
	...
	placeholder
		...
		placeholder
			...
			placeholder
		placeholder
	placeholder else if activeEvent != nil {
			resolvedAt := now
...
placeholder
	...
	placeholder else {
			transport = (&http.Transport{placeholder).Clone()
	placeholder
placeholder
...
placeholder
	...
	placeholder
		return &validatedWebhookTarget{
			...
			pinnedIPs: []net.IP{ipplaceholder,
	placeholder, nil
placeholder
...
placeholder
	if ip4 := ip.To4(); ip4 != nil {
		ip = ip4
placeholder else if ip16 := ip.To16(); ip16 != nil {
		ip = ip16
placeholder else {
		return false
placeholder
...
func uniqueResolvedIPs(ips []net.IP) []net.IP {
	seen := make(map[string]struct{placeholder, len(ips))
