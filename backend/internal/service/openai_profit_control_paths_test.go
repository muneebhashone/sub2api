...
func TestProfitControl_PreviousResponseStickyVetoKeepsBinding(t *testing.T) {
	...
	svc := &OpenAIGatewayService{
		accountRepo:        stubOpenAIAccountRepo{accounts: []Account{expensiveplaceholderplaceholder,
		cache:              cache,
...
placeholder
	...
	svc.accountRepo = stubOpenAIAccountRepo{accounts: []Account{recoveredplaceholderplaceholder
	selection, err = svc.SelectAccountByPreviousResponseID(ctx, &groupID, "resp_profit", "gpt-5.1", nil, false)
placeholder
...
placeholder
	cache := &upstreamCostTrackingConcurrencyCache{loadMap: map[int64]*AccountLoadInfo{
		cheap.ID:     {AccountID: cheap.IDplaceholder,
...
placeholder
	...
	excluded := map[int64]struct{placeholder{cheap.ID: {placeholderplaceholder
...
placeholder)
	selectionOrder := []openAIAccountCandidateScore{{
		account:  expensive,
		loadInfo: &AccountLoadInfo{AccountID: expensive.IDplaceholder,
placeholderplaceholder

	selection, _, err := scheduler.tryAcquireOpenAISelectionOrder(ctx, OpenAIAccountScheduleRequest{Platform: PlatformOpenAIplaceholder, selectionOrder)
placeholder
...
placeholderplaceholder
	svc := &OpenAIGatewayService{
		accountRepo:        schedulerTestOpenAIAccountRepo{accounts: []Account{*expensiveplaceholderplaceholder,
		cfg:                &config.Config{placeholder,
		rateLimitService:   newOpenAIAdvancedSchedulerRateLimitService("true"),
		concurrencyService: NewConcurrencyService(cache),
placeholder
...
placeholder
	...
	svc.accountRepo = schedulerTestOpenAIAccountRepo{accounts: []Account{*recoveredplaceholderplaceholder
...
func TestProfitControl_GateUsesUserOverrideRate(t *testing.T) {
	...
	svc := &OpenAIGatewayService{
		userGroupRateResolver: newUserGroupRateResolver(
			profitControlUserRateRepo{rate: &overrideplaceholder, nil, time.Minute, nil, "test.profit",
		),
placeholder
	groupID := int64(7)
...
placeholder
	svc := &OpenAIGatewayService{
		schedulerSnapshot: &SchedulerSnapshotService{groupRepo: profitControlGroupRepo{group: memberGroupplaceholderplaceholder,
placeholder
...
func TestProfitControl_EligibilityFunctionVetoes(t *testing.T) {
	now := time.Now()
	gateCtx := context.WithValue(context.Background(), openAIProfitControlGateCtxKey{placeholder, &openAIProfitControlGate{
		threshold: 0.5,
		pricingAt: now,
placeholder)
