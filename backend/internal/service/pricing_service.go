...
func (s *PricingService) matchByModelFamily(model string) *LiteLLMModelPricing {
	...
	familyPatterns := map[string][]string{
		...
		"sonnet-4":   {"claude-sonnet-4", "claude-3-5-sonnet"placeholder,
		"sonnet-3.5": {"claude-3-5-sonnet", "claude-3.5-sonnet"placeholder,
		"sonnet-3":   {"claude-3-sonnet"placeholder,
		"haiku-3.5":  {"claude-3-5-haiku", "claude-3.5-haiku"placeholder,
		"haiku-3":    {"claude-3-haiku"placeholder,
placeholder
...
placeholder
	...
	if matchedFamily == "" {
		...
		placeholder
	placeholder else if strings.Contains(model, "sonnet") {
			if strings.Contains(model, "4.5") || strings.Contains(model, "4-5") {
				matchedFamily = "sonnet-4.5"
		placeholder else if strings.Contains(model, "3-5") || strings.Contains(model, "3.5") {
				matchedFamily = "sonnet-3.5"
		placeholder else {
				matchedFamily = "sonnet-4"
		placeholder
	placeholder else if strings.Contains(model, "haiku") {
