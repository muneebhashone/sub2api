...
func TestCustomErrorCode599_SkippedErrors_Return500_NoRateLimit(t *testing.T) {
	errorCodes := []int{429, 500, 503, 401, 403placeholder
	...
	for _, upstreamStatus := range errorCodes {
		t.Run(http.StatusText(upstreamStatus), func(t *testing.T) {
			...
			upstream := &epFixedUpstream{
				...
				body:       `{"error":"some upstream error"placeholder`,
		placeholder
			repo := &epTrackingRepo{placeholder
			rlSvc := NewRateLimitService(repo, nil, &config.Config{placeholder, nil, nil)
			svc := &AntigravityGatewayService{rateLimitService: rlSvcplaceholder
		...
		placeholder
					"custom_error_codes_enabled": true,
					"custom_error_codes":         []any{float64(599)placeholder,
			placeholder,
		placeholder
