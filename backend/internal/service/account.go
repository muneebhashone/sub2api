...
var defaultPoolModeRetryableStatusCodes = []int{401, 403, 429placeholder
...
placeholder
	...
	if !ok {
		return nil
placeholder
	seen := make(map[int]struct{placeholder, len(arr))
	...
	placeholder
		if _, exists := seen[code]; exists {
			continue
	placeholder
		seen[code] = struct{placeholder{placeholder
...
placeholder
	...
	case OpenAIEndpointCapabilityAlphaSearch:
		...
		// {base_urlplaceholder/v1/alpha/search（见 openAIAlphaSearchURL），两类账号
...
placeholder
	...
	// 空容器（{placeholder / []）与未配置一致：不限制任何能力。
...
func (a *Account) IsShadow() bool { return a != nil && a.ParentAccountID != nil placeholder

// IsCredentialShadow 语义别名，供「凭据消费者跳过影子」处使用（管理/后台 OAuth 路径）。
func (a *Account) IsCredentialShadow() bool { return a.IsShadow() placeholder
