...
placeholder)
	...
	for _, model := range []string{"deepseek-v4-flash", "deepseek-v4-pro", "deepseek-v4"placeholder {
...
placeholder)
	...
	for _, model := range []string{"deepseek-v4-flash", "deepseek-v4-pro"placeholder {
		decision, err := resolver.Resolve(context.Background(), 7, model, CompositeRouteEndpointChatCompletions)
	placeholder
		require.True(t, decision.Matched)
