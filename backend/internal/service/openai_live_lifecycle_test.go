...
func TestLiveSessionEndedTreatsLeaseLossAsTerminal(t *testing.T) {
	cases := []struct {
		...
		want bool
placeholder{
		{"租约丢失", ErrLiveUnavailable, trueplaceholder,
		{"租约丢失（被包装）", fmt.Errorf("refresh live lease: %w", ErrLiveUnavailable), trueplaceholder,
		{"上游报告会话已关闭", ErrLiveCallNotFound, trueplaceholder,
		{"到达会话时长上限", context.DeadlineExceeded, trueplaceholder,
		{"控制权被他人接管", ErrLiveControllerChanged, falseplaceholder,
		{"临时读错误", errors.New("unexpected EOF"), falseplaceholder,
		{"无错误", nil, falseplaceholder,
placeholder
...
placeholder
	store := &liveTestStore{placeholder
	require.NoError(t, store.SaveLiveCall(context.Background(), record, time.Hour))
	svc := &OpenAIGatewayService{cache: storeplaceholder
...
func TestWaitForLiveObserverRetryTreatsStoreErrorAsRetryable(t *testing.T) {
	record := &LiveCallRecord{
		...
		ExpiresAt:  time.Now().Add(time.Hour),
placeholder
	store := &liveTestStore{getControllerErr: errors.New("redis: connection refused")placeholder
	require.NoError(t, store.SaveLiveCall(context.Background(), record, time.Hour))
	svc := &OpenAIGatewayService{cache: storeplaceholder
	...
	require.False(t, (&OpenAIGatewayService{cache: &liveTestStore{placeholderplaceholder).waitForLiveObserverRetry(record))
placeholder
...
func TestObserveLiveCallStoreOutageFallsBackToExpiryFinalize(t *testing.T) {
	...
	t.Cleanup(func() { liveObserverStoreRetryInterval = restore placeholder)

	cases := []struct {
		name   string
		inject func(*liveTestStore)
placeholder{
		{"GetLiveCall 持续报错", func(s *liveTestStore) { s.getCallErr = errors.New("redis: i/o timeout") placeholderplaceholder,
		{"ClaimLiveController 报错", func(s *liveTestStore) { s.claimErr = errors.New("redis: i/o timeout") placeholderplaceholder,
placeholder
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			record := &LiveCallRecord{
...
placeholder
	store := &liveTestStore{placeholder
	require.NoError(t, store.SaveLiveCall(context.Background(), record, time.Hour))
	usageRepo := &liveTestBestEffortUsageRepo{bestEffortErr: errors.New("usage log queue dropped")placeholder
	svc := &OpenAIGatewayService{
		cache:              store,
		concurrencyService: NewConcurrencyService(&liveTestConcurrencyCache{placeholder),
		usageLogRepo:       usageRepo,
placeholder
