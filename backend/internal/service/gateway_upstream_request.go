...
placeholder
	...
	placeholder
placeholder else if account.IsCustomBaseURLEnabled() {
...
// 返回最终 header（可能为空字符串）。
func filterVertexBetaTokens(header string, drop map[string]struct{placeholder) string {
...
func (s *GatewayService) computeFinalAnthropicBeta(
	...
	body []byte,
	effectiveDropSet map[string]struct{placeholder,
) (string, bool) {
	clientBeta := ""
...
func (e *BetaBlockedError) Error() string { return e.Message placeholder
...
type betaPolicyResult struct {
	...
	filterSet map[string]struct{placeholder // tokens to filter (may be nil)
placeholder
...
placeholder
	...
	placeholder
		...
		case BetaPolicyActionBlock:
			...
			placeholder
				result.blockErr = &BetaBlockedError{Message: msgplaceholder
		placeholder
		case BetaPolicyActionFilter:
			if result.filterSet == nil {
				result.filterSet = make(map[string]struct{placeholder)
		placeholder
			result.filterSet[rule.BetaToken] = struct{placeholder{placeholder
	placeholder
placeholder
	return result
placeholder
...
func mergeDropSets(policySet map[string]struct{placeholder, extra ...string) map[string]struct{placeholder {
	if len(policySet) == 0 && len(extra) == 0 {
		return defaultDroppedBetasSet
placeholder
	m := make(map[string]struct{placeholder, len(defaultDroppedBetasSet)+len(policySet)+len(extra))
...
func (s *GatewayService) getBetaPolicyFilterSet(ctx context.Context, c *gin.Context, account *Account, model string) map[string]struct{placeholder {
	if c != nil {
		if v, ok := c.Get(betaPolicyFilterSetKey); ok {
			if fs, ok := v.(map[string]struct{placeholder); ok {
				return fs
		placeholder
	placeholder
placeholder
...
func filterBetaTokens(tokens []string, filterSet map[string]struct{placeholder) []string {
	...
	placeholder
		if _, present := tokenSet[rule.BetaToken]; present {
			...
			if msg == "" {
				msg = "beta feature " + rule.BetaToken + " is not allowed"
		placeholder
			return &BetaBlockedError{Message: msgplaceholder
	placeholder
placeholder
	return nil
placeholder
