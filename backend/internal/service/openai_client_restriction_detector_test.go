...
func TestOpenAICodexClientRestrictionDetector_Detect(t *testing.T) {
	...
	t.Run("未开启开关时绕过", func(t *testing.T) {
		...
		account := &Account{Platform: PlatformOpenAI, Type: AccountTypeOAuth, Extra: map[string]any{placeholderplaceholder

		result := detector.Detect(newCodexDetectorTestContext("curl/8.0", ""), account, CodexRestrictionPolicy{placeholder, nil)
...
placeholder)
	...
	placeholder
		...
		result := detector.Detect(newCodexDetectorTestContext("codex_cli_rs/0.99.0", ""), account, CodexRestrictionPolicy{placeholder, nil)
...
placeholder)
	...
	placeholder
		...
		result := detector.Detect(newCodexDetectorTestContext("codex_vscode/1.0.0", ""), account, CodexRestrictionPolicy{placeholder, nil)
		require.True(t, result.Enabled)
		require.True(t, result.Matched)
		require.Equal(t, CodexClientRestrictionReasonMatchedUA, result.Reason)
placeholder)
	...
	placeholder
		...
		result := detector.Detect(newCodexDetectorTestContext("codex_app/2.1.0", ""), account, CodexRestrictionPolicy{placeholder, nil)
		require.True(t, result.Enabled)
		require.True(t, result.Matched)
		require.Equal(t, CodexClientRestrictionReasonMatchedUA, result.Reason)
placeholder)
	...
	placeholder
		...
		result := detector.Detect(newCodexDetectorTestContext("myterm/0.141.0", "codex_chatgpt_desktop"), account, CodexRestrictionPolicy{placeholder, nil)
		require.True(t, result.Enabled)
		require.True(t, result.Matched)
...
placeholder)
	...
	placeholder
		...
		result := detector.Detect(newCodexDetectorTestContext("curl/8.0", "my_client"), account, CodexRestrictionPolicy{placeholder, nil)
		require.True(t, result.Enabled)
		require.False(t, result.Matched)
...
placeholder)
	...
	t.Run("开启 ForceCodexCLI 时允许通过", func(t *testing.T) {
		detector := NewOpenAICodexClientRestrictionDetector(&config.Config{
			Gateway: config.GatewayConfig{ForceCodexCLI: trueplaceholder,
	placeholder)
		account := &Account{
			Platform: PlatformOpenAI,
			Type:     AccountTypeOAuth,
...
func TestOpenAICodexClientRestrictionDetector_Detect_AllowedClients(t *testing.T) {
	...
	placeholder
		...
		result := detector.Detect(newCodexDetectorTestContext(claudeCodeUA, claudeCodeOriginator), account, CodexRestrictionPolicy{placeholder, nil)
		require.True(t, result.Enabled)
		require.False(t, result.Matched)
		require.Equal(t, CodexClientRestrictionReasonNotMatchedUA, result.Reason)
placeholder)
...
placeholder)
	...
	placeholder
		result := detector.Detect(
			...
			account,
			CodexRestrictionPolicy{Whitelist: []openai.AllowedClientEntry{{Originator: "Claude Code", UAContains: []string{"Claude Code/"placeholderplaceholderplaceholderplaceholder,
			nil,
		)
		require.True(t, result.Enabled)
		require.True(t, result.Matched)
...
placeholder)
	...
	placeholder
		result := detector.Detect(
			...
			account,
			CodexRestrictionPolicy{placeholder,
			nil,
		)
		require.True(t, result.Enabled)
		require.False(t, result.Matched)
...
placeholder
	...
	t.Run("AppServer OFF：未列名客户端拒", func(t *testing.T) {
		...
		r := d.Detect(newCodexDetectorTestContext("opencode/1.0", "opencode"), acc(), CodexRestrictionPolicy{placeholder, nil)
...
placeholder)
	...
	t.Run("AppServer ON + 引擎头 → 放行(app_server)", func(t *testing.T) {
		...
		r := d.Detect(c, acc(), CodexRestrictionPolicy{AllowAppServerClients: true, EngineFingerprintSignals: openai.DefaultEngineFingerprintSignalsplaceholder, nil)
...
placeholder)
	...
	t.Run("AppServer ON + 无引擎头 + strict → 拒", func(t *testing.T) {
		d := NewOpenAICodexClientRestrictionDetector(nil)
		r := d.Detect(newCodexDetectorTestContext("opencode/1.0", "opencode"), acc(), CodexRestrictionPolicy{AllowAppServerClients: true, EngineFingerprintSignals: openai.DefaultEngineFingerprintSignalsplaceholder, nil)
		require.False(t, r.Matched)
...
placeholder)
	...
	t.Run("白名单 skip=true + 无引擎头 + strict → 放行", func(t *testing.T) {
		...
		pol := CodexRestrictionPolicy{
			Whitelist: []openai.AllowedClientEntry{{Originator: "opencode", UAContains: []string{"opencode/"placeholder, SkipEngineFingerprint: trueplaceholderplaceholder,
	placeholder
...
placeholder)
	...
	t.Run("版本门仍卡官方：官方 originator 无版本 → VersionUndetectable", func(t *testing.T) {
		d := NewOpenAICodexClientRestrictionDetector(nil)
		r := d.Detect(newCodexDetectorTestContext("noversion", "codex_cli_rs"), acc(), CodexRestrictionPolicy{placeholder, nil)
		require.False(t, r.Matched)
...
placeholder
	...
	t.Run("版本太低：携带 DetectedVersion + MinCodexVersion", func(t *testing.T) {
		...
		r := d.Detect(c, acc(), CodexRestrictionPolicy{MinCodexVersion: "0.42.0"placeholder, nil)
		require.False(t, r.Matched)
...
placeholder)
	...
	t.Run("版本太高：携带 DetectedVersion + MaxCodexVersion", func(t *testing.T) {
		...
		r := d.Detect(c, acc(), CodexRestrictionPolicy{MaxCodexVersion: "0.42.0"placeholder, nil)
		require.False(t, r.Matched)
...
func TestDetect_EngineFingerprintSignals(t *testing.T) {
	gin.SetMode(gin.TestMode)
	det := NewOpenAICodexClientRestrictionDetector(&config.Config{placeholder)
	acct := &Account{Platform: PlatformOpenAI, Type: AccountTypeOAuth, Extra: map[string]any{"codex_cli_only": trueplaceholderplaceholder
	...
	t.Run("body通道: 勾body_path后 仅body命中 → 放行", func(t *testing.T) {
		bodyPolicy := CodexRestrictionPolicy{
			EngineFingerprintSignals: []openai.EngineFingerprintSignal{
				{Type: openai.FingerprintSignalBodyPath, Match: []string{"client_metadata.x-codex-window-id"placeholder, Required: trueplaceholder,
		placeholder,
	placeholder
		c := newCodexDetectorTestContext(officialUA, "")
		got := det.Detect(c, acct, bodyPolicy, []byte(`{"client_metadata":{"x-codex-window-id":"c3"placeholderplaceholder`))
		require.True(t, got.Matched)
placeholder)
placeholder
...
func TestDetect_AccountAppServerToggle(t *testing.T) {
	...
	d := NewOpenAICodexClientRestrictionDetector(nil)
	acctOn := func() *Account {
	placeholderPlatform: PlatformOpenAI, Type: AccountTypeOAuth, Extra: map[string]any{"codex_cli_only": true, "codex_cli_only_allow_app_server": trueplaceholderplaceholder
placeholder
...
placeholder
	withFP := func(ua, originator string) *gin.Context {
		...
		return c
placeholder
	defaultSignals := CodexRestrictionPolicy{EngineFingerprintSignals: openai.DefaultEngineFingerprintSignalsplaceholder
...
placeholder)
	...
	t.Run("账号 app-server OFF + 全局 ON → 放行(OR)", func(t *testing.T) {
		pol := CodexRestrictionPolicy{AllowAppServerClients: true, EngineFingerprintSignals: openai.DefaultEngineFingerprintSignalsplaceholder
