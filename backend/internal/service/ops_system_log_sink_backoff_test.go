...
func TestOpsSystemLogSinkFlushBackoffFor(t *testing.T) {
	sink := &OpsSystemLogSink{flushBackoff: time.Second, flushBackoffMax: 8 * time.Secondplaceholder
	...
	cases := []struct {
		...
		want     time.Duration
placeholder{
		{"first_failure", 1, time.Secondplaceholder,
		{"second_failure", 2, 2 * time.Secondplaceholder,
		{"third_failure", 3, 4 * time.Secondplaceholder,
		{"fourth_failure", 4, 8 * time.Secondplaceholder,
		{"capped", 9, 8 * time.Secondplaceholder,
		{"large_streak_does_not_overflow", 1000, 8 * time.Secondplaceholder,
		{"zero_streak_uses_base", 0, time.Secondplaceholder,
		{"negative_streak_uses_base", -1, time.Secondplaceholder,
placeholder
...
func TestOpsSystemLogSinkFlushBackoffForFallbacks(t *testing.T) {
	zero := &OpsSystemLogSink{placeholder
...
placeholder
	...
	inverted := &OpsSystemLogSink{flushBackoff: 5 * time.Second, flushBackoffMax: time.Secondplaceholder
...
placeholder
	...
	if !waitForOpsSystemLogCondition(t, 2*time.Second, func() bool { return atomic.LoadInt64(&calls) >= 1 placeholder) {
...
placeholder
	...
	if !waitForOpsSystemLogCondition(t, 3*time.Second, func() bool { return atomic.LoadInt64(&calls) >= 3 placeholder) {
...
placeholder
	...
	if !waitForOpsSystemLogCondition(t, 3*time.Second, func() bool { return atomic.LoadInt64(&calls) >= 2 placeholder) {
...
placeholder
	...
	if !waitForOpsSystemLogCondition(t, 3*time.Second, func() bool { return atomic.LoadInt64(&calls) >= 10 placeholder) {
