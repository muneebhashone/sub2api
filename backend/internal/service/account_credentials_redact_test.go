...
placeholder
	incoming := map[string]any{
		...
		"model_mapping": map[string]any{"foo": "bar"placeholder,
placeholder
	...
	require.Equal(t, map[string]any{"foo": "bar"placeholder, out["model_mapping"])
placeholder
...
func TestMergePreservingSensitiveCreds_DoesNotMutateInputs(t *testing.T) {
	existing := map[string]any{"refresh_token": "rt"placeholder
	incoming := map[string]any{"base_url": "x"placeholder
...
func TestMergePreservingSensitiveCreds_NilInputs(t *testing.T) {
	out := MergePreservingSensitiveCreds(nil, map[string]any{"base_url": "x"placeholder)
	require.Equal(t, "x", out["base_url"])
	require.NotContains(t, out, "refresh_token")

	out2 := MergePreservingSensitiveCreds(map[string]any{"refresh_token": "rt"placeholder, nil)
