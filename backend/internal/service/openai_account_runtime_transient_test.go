...
placeholder
	...
	firstShouldDisable := svc.handleOpenAIAccountUpstreamError(context.Background(), account, http.StatusBadGateway, http.Header{placeholder, []byte(`{"error":{"message":"Upstream request failed","type":"upstream_error"placeholderplaceholder`), "gpt-5.5")
	secondShouldDisable := svc.handleOpenAIAccountUpstreamError(context.Background(), account, http.StatusBadGateway, http.Header{placeholder, []byte(`{"error":{"message":"Upstream request failed","type":"upstream_error"placeholderplaceholder`), "gpt-5.5")
...
func TestHandleOpenAITransientError_TransientStatusesUseModelScope(t *testing.T) {
	for _, statusCode := range []int{http.StatusInternalServerError, http.StatusBadGateway, http.StatusServiceUnavailable, http.StatusGatewayTimeout, 520, 521, 522, 523, 524placeholder {
		...
		placeholder
			...
			firstShouldDisable := svc.handleOpenAIAccountUpstreamError(context.Background(), account, statusCode, http.Header{placeholder, []byte(`{"error":{"message":"temporary upstream failure"placeholderplaceholder`), "gpt-5.5")
			secondShouldDisable := svc.handleOpenAIAccountUpstreamError(context.Background(), account, statusCode, http.Header{placeholder, []byte(`{"error":{"message":"temporary upstream failure"placeholderplaceholder`), "gpt-5.5")
...
func TestHandleOpenAITransientError_529RemainsOverloadOnly(t *testing.T) {
	require.False(t, shouldCooldownOpenAITransientUpstreamError(529, []byte(`{"error":{"message":"overloaded"placeholderplaceholder`)))
placeholder
...
placeholder
	...
	for range 2 {
		svc.handleOpenAIAccountUpstreamError(context.Background(), account, http.StatusBadGateway, http.Header{placeholder, []byte(`{"error":{"message":"temporary upstream failure"placeholderplaceholder`), canonicalModel)
placeholder
...
placeholder
	...
	shouldDisable := svc.handleOpenAIAccountUpstreamError(context.Background(), account, http.StatusBadRequest, http.Header{placeholder, []byte(`{"error":{"message":"Invalid type for input[0].arguments"placeholderplaceholder`), "gpt-5.5")
...
func TestHandleOpenAITransientError_HardDisableStillBlocksWholeAccount(t *testing.T) {
	svc := &OpenAIGatewayService{placeholder
	account := &Account{ID: 5106, Platform: PlatformOpenAI, Type: AccountTypeAPIKeyplaceholder
