...
func expandMappingToCache(cache *channelCache, ch *Channel, gid int64, platform string) {
	...
	placeholder
		...
		gpKey := channelGroupPlatformKey{groupID: gid, platform: mappingPlatformplaceholder
		for src, dst := range platformMapping {
			...
			placeholder)
		placeholder else {
				key := channelModelKey{groupID: gid, platform: mappingPlatform, model: strings.ToLower(src)placeholder
...
func matchingPlatforms(groupPlatform string) []string {
	if groupPlatform == PlatformComposite {
		return []string{PlatformAnthropic, PlatformGemini, PlatformOpenAI, PlatformAntigravity, PlatformGrok, PlatformKimi, PlatformZhipu, PlatformDeepseekplaceholder
placeholder
...
func checkPricesNotNegative(p ChannelModelPricing) error {
	checks := []struct {
		...
		val   *float64
placeholder{
		{"input_price", p.InputPriceplaceholder,
...
placeholder
	for _, c := range []struct {
		...
		val   *float64
placeholder{
		{"fast_multiplier", p.FastMultiplierplaceholder,
		{"flex_multiplier", p.FlexMultiplierplaceholder,
placeholder {
		if c.val != nil && *c.val <= 0 {
