			...
			placeholder
				for _, model := range pricing.Models {
					if strings.HasSuffix(model, "*") {
						...
						gpKey := channelGroupPlatformKey{groupID: gid, platform: platformplaceholder
						cache.wildcardByGroupPlatform[gpKey] = append(cache.wildcardByGroupPlatform[gpKey], &wildcardPricingEntry{
							prefix:  prefix,
							pricing: pricing,
					placeholder)
				placeholder else {
						key := channelModelKey{groupID: gid, platform: platform, model: strings.ToLower(model)placeholder
		...
		placeholder
			...
			placeholder
				for src, dst := range platformMapping {
					...
					placeholder)
				placeholder else {
						key := channelModelKey{groupID: gid, platform: platform, model: strings.ToLower(src)placeholder
...
func matchingPlatforms(groupPlatform string) []string {
	if groupPlatform == PlatformAntigravity {
		return []string{PlatformAntigravity, PlatformAnthropic, PlatformGeminiplaceholder
placeholder
	return []string{groupPlatformplaceholder
placeholder
...
func (c *channelCache) matchWildcard(groupID int64, platform, modelLower string) *ChannelModelPricing {
	gpKey := channelGroupPlatformKey{groupID: groupID, platform: platformplaceholder
...
placeholder
	...
	key := channelModelKey{groupID: groupID, platform: platform, model: strings.ToLower(model)placeholder
...
placeholder
	...
	if mapped, ok := cache.mappingByGroupModel[key]; ok {
		...
		result.Mapped = true
placeholder else if mapped := cache.matchWildcardMapping(groupID, platform, strings.ToLower(model)); mapped != "" {
		result.MappedModel = mapped
		result.Mapped = true
placeholder
...
placeholder
	...
	// 失效新旧分组的 auth 缓存
	if s.authCacheInvalidator != nil {
		seen := make(map[int64]struct{placeholder, len(oldGroupIDs)+len(channel.GroupIDs))
		for _, gid := range oldGroupIDs {
			if _, ok := seen[gid]; !ok {
				seen[gid] = struct{placeholder{placeholder
...
placeholder
	return modelEntry{pattern: pattern, prefix: prefix, wildcard: isWildplaceholder
placeholder
...
type CreateChannelInput struct {
	...
	ModelMapping       map[string]map[string]string // platform → {src→dstplaceholder
