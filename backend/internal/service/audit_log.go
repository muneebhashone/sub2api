...
var auditBodySensitiveExactKeys = func() map[string]struct{placeholder {
	builtin := []string{
		"code", "codes", "pin", "cvv",
...
placeholder
	set := make(map[string]struct{placeholder, len(builtin)+len(SensitiveCredentialKeys)+16)
	for _, k := range builtin {
		set[auditNormalizeBodyKey(k)] = struct{placeholder{placeholder
placeholder
