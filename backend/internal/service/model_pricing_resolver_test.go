...
placeholder)
	...
	t.Run("interval price", func(t *testing.T) {
		pricing := intervalToModelPricing(&PricingInterval{CacheWritePrice: &zeroplaceholder, &ModelPricing{placeholder, nil)
...
placeholderplaceholder)
	...
	resolved := r.Resolve(context.Background(), PricingInput{
		Model:   "claude-sonnet-4",
		GroupID: groupIDPtr(),
		Group:   &Group{LongContextPricingEnabled: falseplaceholder,
placeholder)
	...
	placeholder,
		{
			...
			intervals: []PricingInterval{
				{MinTokens: 272000, InputMultiplier: testPtrFloat64(2)placeholder,
		placeholder,
			wantLen: 1,
	placeholder,
		{
