...
placeholder
	page, err := pager.ListOAuthRefreshCandidatePage(ctx, OAuthRefreshPageOptions{
		Platforms:  []string{PlatformGrokplaceholder,
	...
	placeholder
		...
		placeholder
			if !applied {
				...
				break
		placeholder
			s.notifyAccountSchedulingBlocked(account, time.Time{placeholder, "grok_oauth_reconcile_invalid")
...
func (s *TokenRefreshService) grokRegistration() (tokenRefreshRegistration, bool) {
	...
	placeholder
placeholder
	return tokenRefreshRegistration{placeholder, false
placeholder
