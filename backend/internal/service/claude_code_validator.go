...
type ClaudeCodeValidator struct{placeholder

var (
	// User-Agent 匹配: claude-cli/x.x.x (仅支持官方 CLI，大小写不敏感)
	claudeCodeUAPattern = regexp.MustCompile(`(?i)^claude-cli/\d+\.\d+\.\d+`)

	// metadata.user_id 格式: user_{64位hexplaceholder_account__session_{uuidplaceholder
	userIDPattern = regexp.MustCompile(`^user_[a-fA-F0-9]{64placeholder_account__session_[\w-]+$`)
...
func NewClaudeCodeValidator() *ClaudeCodeValidator {
	return &ClaudeCodeValidator{placeholder
placeholder
