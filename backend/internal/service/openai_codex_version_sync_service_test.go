...
placeholder
	return &codexVersionSyncSettingRepoStub{values: valuesplaceholder
placeholder
...
func newCodexVersionSyncService(
	...
	return NewOpenAICodexVersionSyncService(repo, &SettingService{placeholder, github, openAICodexVersionSyncInterval)
placeholder
...
func TestLatestCodexStableReleaseVersion(t *testing.T) {
	releases := []*GitHubRelease{
		{TagName: "rusty-v8-v150.4.0"placeholder,
		{TagName: "rust-v0.147.0-alpha.4", Prerelease: trueplaceholder,
		{TagName: "rust-v0.146.0"placeholder,
		{TagName: "rust-v0.145.0"placeholder,
		{TagName: "rust-v0.999.0", Draft: trueplaceholder,
		{TagName: "not-a-tag"placeholder,
		nil,
placeholder

	require.Equal(t, "0.146.0", latestCodexStableReleaseVersion(releases))
	require.Empty(t, latestCodexStableReleaseVersion(nil))
	require.Empty(t, latestCodexStableReleaseVersion([]*GitHubRelease{{TagName: "rusty-v8-v150.4.0"placeholderplaceholder))
	// 预发布 tag 即使漏标 Prerelease 也要被版本号后缀挡住。
	require.Empty(t, latestCodexStableReleaseVersion([]*GitHubRelease{{TagName: "rust-v0.147.0-alpha.4"placeholderplaceholder))
placeholder
...
placeholder)
	github := &codexVersionSyncGitHubStub{releases: []*GitHubRelease{{TagName: "rust-v0.145.0"placeholderplaceholderplaceholder
...
func TestOpenAICodexVersionSyncEnabledByDefault(t *testing.T) {
	for _, value := range []string{"", "true"placeholder {
		repo := newCodexVersionSyncSettingRepoStub(map[string]string{
	...
	placeholder)
		...
		require.Equal(t, []string{"0.146.0"placeholder, repo.syncedWrites(), "开关值 %q", value)
placeholder
placeholder
...
func TestOpenAICodexVersionSyncUsesLatestReleaseEndpoint(t *testing.T) {
	repo := newCodexVersionSyncSettingRepoStub(nil)
	github := &codexVersionSyncGitHubStub{
		latest: &GitHubRelease{TagName: "rust-v0.146.0"placeholder,
		// 列表若被调用会给出不同答案，用于证明取值确实来自主路径。
		releases: []*GitHubRelease{{TagName: "rust-v0.145.0"placeholderplaceholder,
placeholder
...
placeholder{
		...
		{name: "latest 属于其他 tag 家族", latest: &GitHubRelease{TagName: "rusty-v8-v150.4.0"placeholderplaceholder,
		// 端点语义若变化（返回 draft/prerelease），过滤仍然拦得住，并照常回退。
		{name: "latest 是预发布", latest: &GitHubRelease{TagName: "rust-v0.147.0-alpha.4", Prerelease: trueplaceholderplaceholder,
		{name: "latest 抓取失败", latestErr: errors.New("network down")placeholder,
		// 上游返回空对象：不得因此 panic，按拿不到处理。
		{name: "latest 为空", latest: nilplaceholder,
placeholder
...
func TestOpenAICodexVersionSyncLatestSharesFiltering(t *testing.T) {
	...
	github := &codexVersionSyncGitHubStub{
		...
		latest:   &GitHubRelease{TagName: "rust-vv0.99.0"placeholder,
		releases: []*GitHubRelease{{TagName: "rust-v0.146.0"placeholderplaceholder,
placeholder
...
func TestGetOpenAICodexClientVersionPriority(t *testing.T) {
	tests := []struct {
		...
		want     string
placeholder{
		{name: "面板覆写优先", override: "0.150.0", synced: "0.146.0", want: "0.150.0"placeholder,
		{name: "覆写为空时用同步值", synced: "0.146.0", want: "0.146.0"placeholder,
		{name: "两者皆空时用内置常量", want: codexCLIVersionplaceholder,
		{name: "非法覆写回退同步值", override: "latest", synced: "0.146.0", want: "0.146.0"placeholder,
		{name: "非法同步值回退内置常量", synced: "not-a-version", want: codexCLIVersionplaceholder,
placeholder
...
func TestGetOpenAICodexClientVersionFallsBackOnError(t *testing.T) {
	svc := NewSettingService(&codexVersionSettingRepoStub{err: errors.New("db down")placeholder, nil)
...
placeholder)
	...
	// 非 `{clientplaceholder/{versionplaceholder` 形态无法重建，原样返回，由收口整体回退规范身份。
