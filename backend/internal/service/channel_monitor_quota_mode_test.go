...
func newQuotaModeService(repo *quotaModeRepoStub) *ChannelMonitorService {
	svc := NewChannelMonitorService(repo, &duplicateChannelMonitorEncryptor{placeholder)
...
placeholder
	return &ChannelMonitorQuotaFetcher{
		usage:    usage,
		accounts: &stubMonitorAccountSource{accounts: accountsplaceholder,
...
placeholderplaceholder
	...
	fetcher := newQuotaModeFetcher(map[int64]*Account{
		9: {ID: 9, Platform: domain.PlatformKimi, Credentials: map[string]any{"account_mode": AccountModeCodingplaceholderplaceholder,
placeholder, nil)
	fetcher.cnQuota = &stubMonitorCNQuotaSource{result: &CNProviderQuotaProbeResult{
		Success:         true,
		CredentialValid: true,
		Tiers:           []CNQuotaTier{{Window: "5h", UsedPercent: 30placeholderplaceholder,
placeholderplaceholder
...
placeholder
	...
	require.Equal(t, []int64{1placeholder, repo.markedIDs)
placeholder
...
func TestRunCheck_QuotaProbeAttachesSnapshotToPrimaryRowOnly(t *testing.T) {
	...
	repo := &quotaModeRepoStub{monitor: &ChannelMonitor{
		...
		ExtraModels:     []string{"gpt-extra"placeholder,
		Enabled:         true,
		IntervalSeconds: 60,
...
placeholderplaceholder
	...
	usage := &stubMonitorUsageSource{usage: &UsageInfo{
		FiveHour: &UsageProgress{Utilization: 20placeholder,
placeholderplaceholder
	svc.SetQuotaFetcher(newQuotaModeFetcher(map[int64]*Account{
		12: {ID: 12, Platform: domain.PlatformOpenAIplaceholder,
placeholder, usage))
...
func TestAttachQuotaSnapshot_NoteOnlyWhenProbeMessageEmpty(t *testing.T) {
	results := []*CheckResult{
		{Model: "primary", Status: MonitorStatusOperational, Message: "challenge passed"placeholder,
		{Model: "extra"placeholder,
placeholder
	failed := &domain.MonitorQuotaSnapshot{Success: false, Error: "boom"placeholder
	...
	quiet := []*CheckResult{{Model: "primary", Status: MonitorStatusOperationalplaceholderplaceholder
...
func TestValidateLinkedAccount_Matrix(t *testing.T) {
	...
	fetcher := newQuotaModeFetcher(map[int64]*Account{
		1: {ID: 1, Platform: domain.PlatformKimiplaceholder,
placeholder, nil)
	svc.SetQuotaFetcher(fetcher)
...
func TestRevalidateLinkedAccount_QuotaErrorsProbeUnbinds(t *testing.T) {
	...
	quota := &ChannelMonitor{Provider: MonitorProviderKimi, CheckMode: MonitorCheckModeQuota, AccountID: int64Ptr(9)placeholder
	...
	probe := &ChannelMonitor{Provider: MonitorProviderKimi, CheckMode: MonitorCheckModeProbe, AccountID: int64Ptr(9)placeholder
	...
	quotaNoAccount := &ChannelMonitor{Provider: MonitorProviderKimi, CheckMode: MonitorCheckModeQuotaplaceholder
...
func TestRevalidateLinkedAccount_PlatformMismatch(t *testing.T) {
	svc := NewChannelMonitorService(nil, nil)
	svc.SetQuotaFetcher(newQuotaModeFetcher(map[int64]*Account{
		2: {ID: 2, Platform: domain.PlatformDeepseekplaceholder,
placeholder, nil))

	quota := &ChannelMonitor{Provider: MonitorProviderKimi, CheckMode: MonitorCheckModeQuota, AccountID: int64Ptr(2)placeholder
	require.ErrorIs(t, svc.revalidateLinkedAccount(context.Background(), quota), ErrChannelMonitorProviderIncompatible)

	probe := &ChannelMonitor{Provider: MonitorProviderKimi, CheckMode: MonitorCheckModeProbe, AccountID: int64Ptr(2)placeholder
	require.NoError(t, svc.revalidateLinkedAccount(context.Background(), probe))
...
placeholder{
		{
			...
			account: &Account{ID: 1, Platform: domain.PlatformDeepseek, Credentials: map[string]any{"account_mode": AccountModeCodingplaceholderplaceholder,
	...
	placeholder,
		...
		placeholder"account_mode": AccountModeCoding, "base_url": "https://cw.example.com"placeholderplaceholder,
			wantErr: ErrChannelMonitorAccountNotSupportable,
	placeholder,
		{
			name:    "kimi coding default endpoint ok",
			account: &Account{ID: 3, Platform: domain.PlatformKimi, Credentials: map[string]any{"account_mode": AccountModeCodingplaceholderplaceholder,
	placeholder,
		{
			name:    "zhipu coding default endpoint ok",
			account: &Account{ID: 4, Platform: domain.PlatformZhipu, Credentials: map[string]any{"account_mode": AccountModeCodingplaceholderplaceholder,
	placeholder,
		{
			name:    "zhipu payg has no balance endpoint",
			account: &Account{ID: 5, Platform: domain.PlatformZhipuplaceholder,
			wantErr: ErrChannelMonitorAccountNotSupportable,
	placeholder,
		{
			name:    "kimi payg ok",
			account: &Account{ID: 6, Platform: domain.PlatformKimiplaceholder,
	placeholder,
		{
			name:    "deepseek payg ok",
			account: &Account{ID: 7, Platform: domain.PlatformDeepseekplaceholder,
	placeholder,
		{
			name:    "anthropic api key cannot query usage",
			account: &Account{ID: 8, Platform: domain.PlatformAnthropic, Type: AccountTypeAPIKeyplaceholder,
			wantErr: ErrChannelMonitorAccountNotSupportable,
	placeholder,
		{
			name:    "anthropic oauth ok",
			account: &Account{ID: 9, Platform: domain.PlatformAnthropic, Type: AccountTypeOAuthplaceholder,
	placeholder,
		{
			name:    "anthropic setup token ok (local estimation)",
			account: &Account{ID: 10, Platform: domain.PlatformAnthropic, Type: AccountTypeSetupTokenplaceholder,
	placeholder,
		{
			name:    "openai api key cannot query usage",
			account: &Account{ID: 11, Platform: domain.PlatformOpenAI, Type: AccountTypeAPIKeyplaceholder,
			wantErr: ErrChannelMonitorAccountNotSupportable,
	placeholder,
		{
			name:    "openai oauth ok",
			account: &Account{ID: 12, Platform: domain.PlatformOpenAI, Type: AccountTypeOAuthplaceholder,
	placeholder,
		{
			// 防过度拦截：gemini/grok/antigravity 走本地统计/值通道降级，不会永久 error。
			name:    "gemini api key ok",
			account: &Account{ID: 13, Platform: domain.PlatformGemini, Type: AccountTypeAPIKeyplaceholder,
	placeholder,
		{
			name:    "grok ok",
			account: &Account{ID: 14, Platform: domain.PlatformGrokplaceholder,
	placeholder,
		{
			name:    "antigravity ok",
			account: &Account{ID: 15, Platform: domain.PlatformAntigravityplaceholder,
	placeholder,
placeholder
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
...
func TestValidateLinkedAccount_CapabilityRejected(t *testing.T) {
	svc := NewChannelMonitorService(nil, nil)
	svc.SetQuotaFetcher(newQuotaModeFetcher(map[int64]*Account{
		1: {ID: 1, Platform: domain.PlatformDeepseek, Credentials: map[string]any{"account_mode": AccountModeCodingplaceholderplaceholder,
placeholder, nil))
...
func TestRevalidateLinkedAccount_Capability(t *testing.T) {
	svc := NewChannelMonitorService(nil, nil)
	svc.SetQuotaFetcher(newQuotaModeFetcher(map[int64]*Account{
		2: {ID: 2, Platform: domain.PlatformDeepseek, Credentials: map[string]any{"account_mode": AccountModeCodingplaceholderplaceholder,
placeholder, nil))

	quota := &ChannelMonitor{Provider: MonitorProviderDeepseek, CheckMode: MonitorCheckModeQuota, AccountID: int64Ptr(2)placeholder
	...
	probe := &ChannelMonitor{Provider: MonitorProviderDeepseek, CheckMode: MonitorCheckModeProbe, AccountID: int64Ptr(2)placeholder
	require.NoError(t, svc.revalidateLinkedAccount(context.Background(), probe))
...
placeholder
	...
	err := applyMonitorUpdate(probeKimi(), ChannelMonitorUpdateParams{Provider: &providerplaceholder)
...
placeholder
	newName := "renamed"
	require.NoError(t, applyMonitorUpdate(legacy, ChannelMonitorUpdateParams{Name: &newNameplaceholder))
placeholder

// --- quota → probe 切换的 key 管控（validateProbeAPIKey） ---

func TestValidateProbeAPIKey_QuotaToProbeRequiresFreshKey(t *testing.T) {
	svc := NewChannelMonitorService(nil, &duplicateChannelMonitorEncryptor{placeholder)

	quota := &ChannelMonitor{Provider: MonitorProviderKimi, CheckMode: MonitorCheckModeQuota, APIKey: "NEW:"placeholder
	...
	require.NoError(t, svc.validateProbeAPIKey(
		&ChannelMonitor{Provider: MonitorProviderKimi, CheckMode: MonitorCheckModeProbe, APIKey: "OLD:sk-live"placeholder, ""))
placeholder
