...
placeholder
	return &ContextPricingSchedule{Basis: basis, Tiers: tiers, TimePricing: resolvedTimePricingSchedule(resolved)placeholder, nil
placeholder
...
placeholder
	if len(probed) == 0 {
		return nil
placeholder
	sort.SliceStable(probed, func(i, j int) bool { return probed[i].start < probed[j].start placeholder)
	out := &TimePricingSchedule{Timezone: cfg.Timezone, Periods: make([]TimePricingPeriod, 0, len(probed))placeholder
