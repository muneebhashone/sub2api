...
placeholder()
	...
	hbStop := make(chan struct{placeholder)
...
func (w *BatchImageWorker) runJobHeartbeat(ctx context.Context, batchID string, lock BatchImageJobLock, stop <-chan struct{placeholder, done chan<- struct{placeholder) {
	defer close(done)
