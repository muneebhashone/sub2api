...
placeholder
	for _, item := range gjson.GetBytes(response, "output").Array() {
		if !item.IsObject() {
			continue
	placeholder
		event, err := sjson.SetBytes([]byte(`{"type":"response.output_item.done"placeholder`), "output_index", outputIndex)
		if err != nil {
			return nil, false
	placeholder
...
placeholder
	...
	completed, err := sjson.SetRawBytes([]byte(`{"type":"response.completed"placeholder`), "response", response)
	if err != nil {
		return nil, false
placeholder
...
func openAICompactUsageParsableByCodex(usage gjson.Result) bool {
	if !usage.IsObject() {
		return false
placeholder
	for _, field := range []string{"input_tokens", "output_tokens", "total_tokens"placeholder {
