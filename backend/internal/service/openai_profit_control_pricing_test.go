...
func TestProfitControl_GateReuseKeepsThresholdAcrossFailover(t *testing.T) {
	...
	reCtx := svc.withOpenAIProfitControlGate(ctx, &groupID)
	reGate, ok := reCtx.Value(openAIProfitControlGateCtxKey{placeholder).(*openAIProfitControlGate)
	require.True(t, ok)
	...
	otherGate, _ := otherCtx.Value(openAIProfitControlGateCtxKey{placeholder).(*openAIProfitControlGate)
...
func TestProfitControl_PricingAtFixesDownstreamPeakFactor(t *testing.T) {
	...
	ctx := context.WithValue(profitControlTestCtx(group), openAIPricingAtCtxKey{placeholder, pricingAt)
...
func TestProfitControl_UsesAccountRateInsteadOfProbeSnapshot(t *testing.T) {
	gate := &openAIProfitControlGate{threshold: 0.5, pricingAt: time.Now().Add(-12 * time.Hour)placeholder
	ctx := context.WithValue(context.Background(), openAIProfitControlGateCtxKey{placeholder, gate)
...
func TestProfitControl_AccountRateSemantics(t *testing.T) {
	...
	base := context.WithValue(profitControlTestCtx(group), openAIPricingAtCtxKey{placeholder, now)
	gate := (&OpenAIGatewayService{placeholder).resolveOpenAIProfitControlGate(base, &group.ID)
	require.NotNil(t, gate)
	gateCtx := context.WithValue(base, openAIProfitControlGateCtxKey{placeholder, gate)
...
func TestOpenAIUsagePricingAt(t *testing.T) {
	...
	require.Equal(t, fixed, openAIUsagePricingAt(&OpenAIRecordUsageInput{PricingAt: fixedplaceholder))
	fallback := openAIUsagePricingAt(&OpenAIRecordUsageInput{placeholder)
...
func TestOpenAIProfitControlStickyBindingOccursOnlyAfterTerminalAdmission(t *testing.T) {
	...
	cache := &schedulerTestGatewayCache{
		sessionBindings: map[string]int64{cacheKey: expensiveIDplaceholder,
placeholder
...
placeholder)
	...
	t.Run("keeps scheduled group of the existing gate", func(t *testing.T) {
		...
		gate, ok := turnCtx.Value(openAIProfitControlGateCtxKey{placeholder).(*openAIProfitControlGate)
		require.True(t, ok)
		require.NotNil(t, gate)
