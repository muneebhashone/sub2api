...
placeholder)
	...
	placeholder
		...
		require.Equal(t, []string{got.IDplaceholder, queue.enqueued)
		...
		require.Equal(t, []int64{11placeholder, authCache.userIDs)
...
placeholder)
	...
	placeholderplaceholder
		userRate := 0.5
		svc.UserGroupRateRepo = &publicBatchImageUserGroupRateRepo{rates: map[int64]*float64{groupID: &userRateplaceholderplaceholder

		got, err := svc.Submit(ctx, BatchImageOwner{UserID: 11, APIKeyID: 22, GroupID: &groupIDplaceholder, validBatchImageSubmitRequest(), "")
	placeholder
...
placeholder)
	...
	t.Run("pricing missing rejects before provider submit", func(t *testing.T) {
		svc, repo, queue, gemini, _ := newTestBatchImagePublicService(true)
		svc.Pricing = &fakeBatchImagePricingResolver{err: ErrBatchImageSettlementPricingMissingplaceholder
...
placeholder)
	...
	placeholderplaceholder
		...
		_, err := svc.Submit(ctx, BatchImageOwner{UserID: 11, APIKeyID: 22, GroupID: &groupIDplaceholder, validBatchImageSubmitRequest(), "")
...
placeholder)
	...
	t.Run("validates request fields", func(t *testing.T) {
		tests := []struct {
			...
			want   error
	placeholder{
			{name: "missing_model", mutate: func(r *BatchImageSubmitRequest) { r.Model = "" placeholder, want: ErrBatchImageInvalidModelplaceholder,
			{name: "empty_items", mutate: func(r *BatchImageSubmitRequest) { r.Items = nil placeholder, want: ErrBatchImageInvalidItemsplaceholder,
			{name: "duplicate_custom_ids", mutate: func(r *BatchImageSubmitRequest) { r.Items[1].CustomID = r.Items[0].CustomID placeholder, want: ErrBatchImageDuplicateCustomIDInRequestplaceholder,
			{name: "empty_prompt", mutate: func(r *BatchImageSubmitRequest) { r.Items[0].Prompt = " " placeholder, want: ErrBatchImageInvalidItemsplaceholder,
...
placeholder)
	...
	t.Run("rejects too many items", func(t *testing.T) {
		svc, _, _, _, _ := newTestBatchImagePublicService(true)
		req := validBatchImageSubmitRequest()
		req.Items = append(req.Items, BatchImageSubmitItem{CustomID: "too_many", Prompt: "x"placeholder)
...
placeholder)
	...
	t.Run("insufficient balance rejects before provider submit", func(t *testing.T) {
		svc, repo, queue, gemini, _ := newTestBatchImagePublicService(true)
		billing := &fakeBatchImageBillingRepo{err: ErrBatchImageInsufficientBalanceplaceholder
...
placeholder)
	...
	t.Run("provider failure with release failure enqueues billing retry", func(t *testing.T) {
		...
		for _, job := range repo.jobs {
			...
			require.Equal(t, []string{job.BatchIDplaceholder, queue.enqueued)
	placeholder
placeholder)
...
placeholder)
	...
	placeholder
		...
		require.Len(t, gemini.submits, 1)
		require.Equal(t, []string{first.IDplaceholder, queue.enqueued)
placeholder)
...
placeholder
	...
	got, err := svc.List(ctx, BatchImageOwner{UserID: 11, APIKeyID: visibleKeyIDplaceholder, BatchImageJobsQuery{Limit: 20placeholder)
placeholder
...
placeholder)
	...
	placeholder)placeholder
		...
		got, err := svc.ListModels(ctx, BatchImageOwner{UserID: 11, APIKeyID: 22, GroupID: &groupIDplaceholder)
	placeholder
...
placeholder)
	...
	t.Run("filters models without batch image pricing", func(t *testing.T) {
		...
		svc.Pricing = &fakeBatchImagePricingResolver{
			unitPrice:     0.25,
			missingModels: map[string]bool{"gemini-3.1-flash-lite-image": trueplaceholder,
	placeholder
		accountRepo := svc.AccountRepo.(*publicBatchImageAccountRepo)
		accountRepo.accounts = []Account{testBatchImageMappedAccount(303, AccountTypeAPIKey, map[string]any{
			"gemini-2.5-flash-image":      "gemini-2.5-flash-image",
...
placeholder)
	...
	t.Run("rejects when group disables batch image", func(t *testing.T) {
		...
		groupID := int64(7)
		svc.GroupRepo = &publicBatchImageGroupRepo{groups: map[int64]*Group{
			groupID: {ID: groupID, AllowBatchImageGeneration: falseplaceholder,
	placeholderplaceholder

		_, err := svc.ListModels(ctx, BatchImageOwner{UserID: 11, APIKeyID: 22, GroupID: &groupIDplaceholder)
		require.ErrorIs(t, err, ErrBatchImageGroupDisabled)
placeholder)
placeholder
...
func TestBatchImagePublicService_StatusItemsAndCancel(t *testing.T) {
	...
	placeholder
		...
		_, err = svc.Get(ctx, BatchImageOwner{UserID: 11, APIKeyID: 999placeholder, "imgbatch_status")
...
placeholder)
	...
	placeholder
		...
		repo.items["imgbatch_items"] = []CreateBatchImageItemParams{
			{JobID: "imgbatch_items", CustomID: "ok_1", Status: BatchImageItemStatusSuccess, ProviderSourceObject: &sourceObject, MimeType: &mime, FileExtension: &ext, ImageCount: 1placeholder,
			{JobID: "imgbatch_items", CustomID: "bad_1", Status: BatchImageItemStatusFailed, ProviderSourceObject: &sourceObject, ErrorCode: &code, ErrorMessage: &msgplaceholder,
			{JobID: "imgbatch_items", CustomID: "ok_2", Status: BatchImageItemStatusSuccess, MimeType: &mime, FileExtension: &ext, ImageCount: 1placeholder,
	placeholder

		page, err := svc.ListItems(ctx, testBatchImageOwner(), "imgbatch_items", BatchImageItemsQuery{Limit: 1placeholder)
	placeholder
		require.True(t, page.HasMore)
		require.Len(t, page.Data, 1)
		require.Equal(t, "ok_1", page.Data[0].CustomID)

		filtered, err := svc.ListItems(ctx, testBatchImageOwner(), "imgbatch_items", BatchImageItemsQuery{Status: "failed", Limit: 100placeholder)
	placeholder
	...
	placeholder
		...
		_, err = svc.ListItems(ctx, BatchImageOwner{UserID: 12, APIKeyID: 22placeholder, "imgbatch_items", BatchImageItemsQuery{placeholder)
		require.ErrorIs(t, err, ErrBatchImageJobNotFound)
placeholder)
	...
	placeholder
		...
		billing := svc.BillingRepo.(*fakeBatchImageBillingRepo)
		require.Empty(t, billing.releases)
		require.Equal(t, []string{"imgbatch_cancel"placeholder, queue.enqueued)
...
func newTestBatchImagePublicService(enabled bool) (*BatchImagePublicService, *fakeBatchImageRepository, *publicBatchImageQueue, *publicBatchImageProvider, *publicBatchImageProvider) {
	...
	gemini := &publicBatchImageProvider{name: BatchImageProviderGeminiAPIplaceholder
	vertex := &publicBatchImageProvider{name: BatchImageProviderVertexplaceholder
	svc := &BatchImagePublicService{
		Repo:        repo,
		AccountRepo: &publicBatchImageAccountRepo{accounts: []Account{testBatchImageAccount(101, AccountTypeAPIKey), testBatchImageAccount(202, AccountTypeServiceAccount)placeholderplaceholder,
		Queue:       queue,
		...
		BillingRepo: &fakeBatchImageBillingRepo{placeholder,
		AuthCache:   &fakeBatchImageAuthCacheInvalidator{placeholder,
...
func testBatchImageOwner() BatchImageOwner {
	return BatchImageOwner{UserID: 11, APIKeyID: 22placeholder
placeholder
...
func validBatchImageSubmitRequest() BatchImageSubmitRequest {
	return BatchImageSubmitRequest{
		...
		AspectRatio:      "1:1",
		ImageSize:        "1K",
		Metadata:         map[string]string{"project": "campaign-a", "secret": strings.Repeat("x", 300)placeholder,
		Items: []BatchImageSubmitItem{
			{CustomID: "cover_001", Prompt: "hero"placeholder,
			{CustomID: "cover_002", Prompt: "clean"placeholder,
	placeholder,
placeholder
placeholder
...
func testBatchImageAccount(id int64, accountType string) Account {
	return Account{
		...
		Schedulable:   true,
		Priority:      int(id),
		Credentials:   map[string]any{"api_key": "test-secret"placeholder,
		Concurrency:   1,
		RateLimitedAt: nil,
placeholder
placeholder
...
func (q *publicBatchImageQueue) Reserve(context.Context, time.Duration) (ReservedBatchImageJob, error) {
	return ReservedBatchImageJob{placeholder, ErrBatchImageQueueEmpty
placeholder
...
func (p *publicBatchImageProvider) Name() string { return p.name placeholder

func (p *publicBatchImageProvider) SupportsAccount(*Account) bool { return true placeholder
