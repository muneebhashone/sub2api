...
func NewConcurrencyService(rdb *redis.Client) *ConcurrencyService {
	return &ConcurrencyService{rdb: rdbplaceholder
placeholder
...
func (s *ConcurrencyService) tryAcquire(ctx context.Context, key string, maxConcurrency int) (bool, error) {
	result, err := acquireScript.Run(ctx, s.rdb, []string{keyplaceholder, maxConcurrency, int(concurrencyKeyTTL.Seconds())).Int()
	if err != nil {
...
func (s *ConcurrencyService) makeReleaseFunc(key string) func() {
	return func() {
		...
		if err := releaseScript.Run(ctx, s.rdb, []string{keyplaceholder).Err(); err != nil {
...
placeholder
	...
	result, err := incrementWaitScript.Run(ctx, s.rdb, []string{keyplaceholder, maxWait, int(concurrencyKeyTTL.Seconds())).Int()
	if err != nil {
...
placeholder
	...
	if err := decrementWaitScript.Run(bgCtx, s.rdb, []string{keyplaceholder).Err(); err != nil {
