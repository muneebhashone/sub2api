...
func openAIResetTestScheduler(reset float64) *defaultOpenAIAccountScheduler {
	...
	cfg.Gateway.OpenAIWS.SchedulerScoreWeights = config.GatewayOpenAIWSSchedulerScoreWeights{
		...
		QuotaHeadroom: 0,
placeholder
	return &defaultOpenAIAccountScheduler{service: &OpenAIGatewayService{cfg: cfgplaceholderplaceholder
placeholder
...
func TestBuildOpenAIAccountLoadPlan_ResetWeightPrefersSoonestReset(t *testing.T) {
	...
	filtered := []*Account{
		{ID: 1, Priority: 0, SessionWindowEnd: &laterplaceholder,
		{ID: 2, Priority: 0, SessionWindowEnd: &soonplaceholder,
placeholder
	sched := openAIResetTestScheduler(5.0)

	plan := sched.buildOpenAIAccountLoadPlan(context.Background(), OpenAIAccountScheduleRequest{placeholder, filtered, map[int64]*AccountLoadInfo{placeholder)
...
func TestBuildOpenAIAccountLoadPlan_ResetWeightIgnoresNilWindow(t *testing.T) {
	...
	filtered := []*Account{
		{ID: 1, Priority: 0, SessionWindowEnd: nilplaceholder,
		{ID: 2, Priority: 0, SessionWindowEnd: &soonplaceholder,
placeholder
	sched := openAIResetTestScheduler(5.0)
