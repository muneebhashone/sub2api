...
func resolveGrokFreeQuotaGateSettings(cfg *config.Config) (grokFreeQuotaGateSettings, bool) {
	if cfg == nil || !cfg.Gateway.Grok.FreeQuotaSoftGateEnabled {
		return grokFreeQuotaGateSettings{placeholder, false
placeholder
...
placeholder
	...
	seenMissing := make(map[int64]struct{placeholder)
...
placeholder
		...
		if err != nil {
			...
			if settings.cacheTTL > 0 {
				for _, accountID := range missingIDs {
					s.grokFreeQuotaGateCache.Store(accountID, grokFreeQuotaGateCacheEntry{checkedAt: nowplaceholder)
			placeholder
		placeholder
	...
	placeholder else {
			...
			placeholder
				...
				s.grokFreeQuotaGateCache.Store(accountID, grokFreeQuotaGateCacheEntry{tokens: tokens, checkedAt: now, known: trueplaceholder)
