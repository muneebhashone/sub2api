...
placeholder
	if statusCode < 200 || statusCode >= 300 {
		...
		// 会丢掉真正的上游错误信息，例如 `{"error":{"message":"No available accounts ..."placeholderplaceholder`）。
		res.Status = MonitorStatusError
...
placeholder,
	MonitorProviderGemini: {
		// Gemini 把 model 名写在 URL path 上：/v1beta/models/{modelplaceholder:generateContent
		buildPath: func(model string) string { return fmt.Sprintf(providerGeminiPathTemplate, model) placeholder,
		buildBody: func(_, prompt string) ([]byte, error) {
			return json.Marshal(map[string]any{
				"contents": []map[string]any{
					{"parts": []map[string]any{{"text": promptplaceholderplaceholderplaceholder,
			placeholder,
				"generationConfig": map[string]any{"maxOutputTokens": monitorChallengeMaxTokensplaceholder,
		placeholder)
	placeholder,
		// 使用 x-goog-api-key header 而不是 ?key= query，避免 *url.Error 把 key 回填到错误日志。
		buildHeaders: func(apiKey string) map[string]string {
			return map[string]string{"x-goog-api-key": apiKeyplaceholder
	placeholder,
...
var providerOpenAIResponsesAdapter = providerAdapter{
	buildPath: func(string) string { return providerOpenAIResponsesPath placeholder,
	buildBody: func(model, prompt string) ([]byte, error) {
		return json.Marshal(map[string]any{
			"model":             model,
...
placeholder{
	...
	{regexp.MustCompile(`eyJ[A-Za-z0-9_-]{8,placeholder\.eyJ[A-Za-z0-9_-]{8,placeholder\.[A-Za-z0-9_-]{8,placeholder`), "eyJ***placeholder.JWT***"placeholder,
placeholder
...
func sanitizeErrorMessage(msg string) string {
	if msg == "" {
		return msg
placeholder
	msg = monitorSensitiveQueryParamRegex.ReplaceAllString(msg, `${1placeholderplaceholder`)
