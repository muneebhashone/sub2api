...
type cachedPassthroughRule struct {
	*model.ErrorPassthroughRule
	...
	errorCodeSet   map[int]struct{placeholder // 预计算的 error code set
placeholder
...
func (s *ErrorPassthroughService) setLocalCache(rules []*model.ErrorPassthroughRule) {
	...
	for i, r := range rules {
		cr := &cachedPassthroughRule{ErrorPassthroughRule: rplaceholder
		if len(r.Keywords) > 0 {
	...
	placeholder
		if len(r.ErrorCodes) > 0 {
			cr.errorCodeSet = make(map[int]struct{placeholder, len(r.ErrorCodes))
...
func (s *ErrorPassthroughService) containsIntSet(set map[int]struct{placeholder, val int) bool {
	_, ok := set[val]
	return ok
placeholder
