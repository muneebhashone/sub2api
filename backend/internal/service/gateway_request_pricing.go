...
type gatewayTokenRequestPricingAtCtxKey struct{placeholder
type gatewayTokenRequestBillingGroupCtxKey struct{placeholder
...
placeholder
	...
	ctx = context.WithValue(ctx, gatewayTokenRequestPricingAtCtxKey{placeholder, pricingAt)
	...
	if group, ok := ctx.Value(ctxkey.Group).(*Group); ok && IsGroupContextValid(group) {
		ctx = context.WithValue(ctx, gatewayTokenRequestBillingGroupCtxKey{placeholder, group)
placeholder
...
placeholder
	pricingAt, ok := ctx.Value(gatewayTokenRequestPricingAtCtxKey{placeholder).(time.Time)
...
func gatewayTokenRequestBillingGroupFromContext(ctx context.Context) *Group {
	if ctx == nil {
		return nil
placeholder
	group, _ := ctx.Value(gatewayTokenRequestBillingGroupCtxKey{placeholder).(*Group)
