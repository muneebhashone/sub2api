...
func TestApplyGrokImagineImageGeometryMapsOpenAISize(t *testing.T) {
	...
	out, err := applyGrokImagineImageGeometry([]byte(`{"model":"grok-imagine-image-2.0","prompt":"hi","size":"1152x1536"placeholder`))
placeholder
...
func TestApplyGrokImagineImageGeometryKeepsClientGeometry(t *testing.T) {
	...
	out, err := applyGrokImagineImageGeometry([]byte(`{"size":"1024x1024","resolution":"2K","aspect_ratio":"16:9"placeholder`))
placeholder
	require.False(t, gjson.GetBytes(out, "size").Exists())
	require.Equal(t, "2k", gjson.GetBytes(out, "resolution").String())
...
func TestSanitizeGrokMediaForwardBodyConvertsImageSize(t *testing.T) {
	...
	out, contentType, err := sanitizeGrokMediaForwardBody(
		...
		[]byte(`{"model":"grok-imagine-image","prompt":"hi","size":"1024x1024"placeholder`),
		"application/json",
	)
placeholder
...
func TestParseGrokMediaRequestKeepsImageResolutionOutOfVideoNormalize(t *testing.T) {
	...
	info := ParseGrokMediaRequest("application/json", []byte(`{"model":"grok-imagine-image-2.0","resolution":"2K","aspect_ratio":"16:9"placeholder`))
