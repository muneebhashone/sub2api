...
placeholder
	svc := &OpenAIGatewayService{
		cfg:              cfg,
		rateLimitService: &RateLimitService{settingService: NewSettingService(repo, cfg)placeholder,
placeholder
...
placeholder
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			repo := &openAIAdvancedSchedulerSettingRepoStub{values: tt.valuesplaceholder
			svc := &OpenAIGatewayService{cfg: cfg, rateLimitService: &RateLimitService{settingService: NewSettingService(repo, cfg)placeholderplaceholder

			require.Equal(t, (&OpenAIGatewayService{cfg: cfgplaceholder).openAIWSSchedulerWeights(), svc.openAIWSSchedulerWeightsForRequest(context.Background()))
	placeholder)
placeholder
placeholder
...
func TestOpenAIGatewayService_SelectAccountWithScheduler_DefaultDisabled_LoadBatchReportsFilterReasons(t *testing.T) {
	...
	ctx := withOpenAIQuotaAutoPauseSettings(context.Background(), OpsOpenAIAccountQuotaAutoPauseSettings{DefaultThreshold7d: 0.9placeholder)
	groupID := int64(10107)
	quotaPaused := Account{
		ID:          36003,
		Platform:    PlatformOpenAI,
...
placeholder
	...
	svc := &OpenAIGatewayService{
		accountRepo:        schedulerTestOpenAIAccountRepo{accounts: []Account{quotaPaused, mappingMiss, excludedplaceholderplaceholder,
		cache:              &schedulerTestGatewayCache{placeholder,
		cfg:                cfg,
		concurrencyService: NewConcurrencyService(schedulerTestConcurrencyCache{placeholder),
placeholder
	...
	selection, decision, err := svc.SelectAccountWithScheduler(
		...
		"",
		"gpt-5.4-mini",
		map[int64]struct{placeholder{excluded.ID: {placeholderplaceholder,
		OpenAIUpstreamTransportAny,
		false,
	)
	...
	placeholder,
		{
			...
			Priority:    5,
	placeholder
				"openai_capabilities": []any{"chat_completions", "embeddings"placeholder,
		placeholder,
	placeholder,
placeholder
	cfg := &config.Config{placeholder
...
placeholder
	...
	unsupported := Account{
		...
		Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 5,
		Extra: map[string]any{"openai_responses_supported": falseplaceholder,
placeholder

	t.Run("生图意图仅选中支持 responses 的账号", func(t *testing.T) {
		svc := newSvc([]Account{supported, unsupportedplaceholder)
...
placeholder)
	...
	t.Run("仅有不支持 responses 的账号时生图意图无可用账号", func(t *testing.T) {
		svc := newSvc([]Account{unsupportedplaceholder)
		selection, _, err := svc.SelectAccountWithSchedulerForCapability(
			ctx, &groupID, "", "", "gpt-image-2", nil,
			OpenAIUpstreamTransportAny, OpenAIEndpointCapabilityResponses,
			false, false, false,
...
placeholder
	...
	selection, _, err := svc.SelectAccountWithScheduler(
		ctx, &groupID, "", "", "gpt-5.4-mini", map[int64]struct{placeholder{38123: {placeholderplaceholder, OpenAIUpstreamTransportAny, false,
	)
placeholder
	require.ErrorIs(t, err, ErrNoAvailableAccounts)
	require.Nil(t, selection)
...
func TestOpenAIGatewayService_SelectAccountWithScheduler_NoAvailableErrorReportsEmptyPool(t *testing.T) {
	...
	svc := &OpenAIGatewayService{
		accountRepo:        schedulerTestOpenAIAccountRepo{placeholder,
		cache:              &schedulerTestGatewayCache{placeholder,
		cfg:                &config.Config{placeholder,
		rateLimitService:   newOpenAIAdvancedSchedulerRateLimitService("true"),
		concurrencyService: NewConcurrencyService(schedulerTestConcurrencyCache{placeholder),
...
func TestOpenAIGatewayService_SelectAccountWithScheduler_SkipsQuarantinedSharedProxy(t *testing.T) {
	...
	proxyB := int64(4699)
	accounts := []Account{
		{ID: 469801, Platform: PlatformOpenAI, Type: AccountTypeOAuth, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 0, ProxyID: &proxyAplaceholder,
		{ID: 469802, Platform: PlatformOpenAI, Type: AccountTypeOAuth, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 0, ProxyID: &proxyAplaceholder,
		{ID: 469803, Platform: PlatformOpenAI, Type: AccountTypeOAuth, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 5, ProxyID: &proxyBplaceholder,
placeholder
	cfg := &config.Config{placeholder
	cfg.Gateway.Scheduling.LoadBatchEnabled = false
	svc := &OpenAIGatewayService{
...
func TestOpenAIGatewayService_SelectAccountWithScheduler_SessionStickyRateLimitedAccountFallsBackToFreshCandidate(t *testing.T) {
	...
	staleSticky := &Account{ID: 31001, Platform: PlatformOpenAI, Type: AccountTypeOAuth, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 0, GroupIDs: []int64{groupIDplaceholderplaceholder
	staleBackup := &Account{ID: 31002, Platform: PlatformOpenAI, Type: AccountTypeOAuth, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 5, GroupIDs: []int64{groupIDplaceholderplaceholder
	freshSticky := &Account{ID: 31001, Platform: PlatformOpenAI, Type: AccountTypeOAuth, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 0, GroupIDs: []int64{groupIDplaceholder, RateLimitResetAt: &rateLimitedUntilplaceholder
	freshBackup := &Account{ID: 31002, Platform: PlatformOpenAI, Type: AccountTypeOAuth, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 5, GroupIDs: []int64{groupIDplaceholderplaceholder
	cache := &schedulerTestGatewayCache{sessionBindings: map[string]int64{"openai:session_hash_rate_limited": 31001placeholderplaceholder
	snapshotCache := &openAISnapshotCacheStub{snapshotAccounts: []*Account{staleSticky, staleBackupplaceholder, accountsByID: map[int64]*Account{31001: freshSticky, 31002: freshBackupplaceholderplaceholder
	snapshotService := &SchedulerSnapshotService{cache: snapshotCacheplaceholder
	svc := &OpenAIGatewayService{
		accountRepo:        schedulerTestOpenAIAccountRepo{accounts: []Account{*freshSticky, *freshBackupplaceholderplaceholder,
		cache:              cache,
		cfg:                &config.Config{placeholder,
		rateLimitService:   newOpenAIAdvancedSchedulerRateLimitService("true"),
...
func TestOpenAIGatewayService_SelectAccountForModelWithExclusions_StaleUsageWindowResetSkipsPause(t *testing.T) {
	...
	placeholder,
placeholder
	secondary := Account{ID: 35502, Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 5placeholder
	svc := &OpenAIGatewayService{accountRepo: schedulerTestOpenAIAccountRepo{accounts: []Account{primary, secondaryplaceholderplaceholder, cfg: &config.Config{placeholderplaceholder
...
func TestOpenAIGatewayService_SelectAccountForModelWithExclusions_FreshUsageWindowStillPauses(t *testing.T) {
	...
	placeholder,
placeholder
	secondary := Account{ID: 35602, Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 5placeholder
	svc := &OpenAIGatewayService{accountRepo: schedulerTestOpenAIAccountRepo{accounts: []Account{primary, secondaryplaceholderplaceholder, cfg: &config.Config{placeholderplaceholder
...
func TestOpenAIGatewayService_SelectAccountForModelWithExclusions_SkipsFreshlyRateLimitedSnapshotCandidate(t *testing.T) {
	...
	groupID := int64(10102)
	rateLimitedUntil := time.Now().Add(30 * time.Minute)
	stalePrimary := &Account{ID: 32001, Platform: PlatformOpenAI, Type: AccountTypeOAuth, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 0, GroupIDs: []int64{groupIDplaceholderplaceholder
	staleSecondary := &Account{ID: 32002, Platform: PlatformOpenAI, Type: AccountTypeOAuth, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 5, GroupIDs: []int64{groupIDplaceholderplaceholder
	freshPrimary := &Account{ID: 32001, Platform: PlatformOpenAI, Type: AccountTypeOAuth, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 0, GroupIDs: []int64{groupIDplaceholder, RateLimitResetAt: &rateLimitedUntilplaceholder
	freshSecondary := &Account{ID: 32002, Platform: PlatformOpenAI, Type: AccountTypeOAuth, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 5, GroupIDs: []int64{groupIDplaceholderplaceholder
	snapshotCache := &openAISnapshotCacheStub{snapshotAccounts: []*Account{stalePrimary, staleSecondaryplaceholder, accountsByID: map[int64]*Account{32001: freshPrimary, 32002: freshSecondaryplaceholderplaceholder
	snapshotService := &SchedulerSnapshotService{cache: snapshotCacheplaceholder
	svc := &OpenAIGatewayService{
		accountRepo:       schedulerTestOpenAIAccountRepo{accounts: []Account{*freshPrimary, *freshSecondaryplaceholderplaceholder,
		cfg:               &config.Config{placeholder,
		rateLimitService:  newOpenAIAdvancedSchedulerRateLimitService("true"),
		schedulerSnapshot: snapshotService,
placeholder
...
func TestOpenAIGatewayService_SelectAccountWithScheduler_SessionStickyDBRuntimeRecheckSkipsStaleCachedAccount(t *testing.T) {
	...
	groupID := int64(10103)
	rateLimitedUntil := time.Now().Add(30 * time.Minute)
	staleSticky := &Account{ID: 33001, Platform: PlatformOpenAI, Type: AccountTypeOAuth, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 0, GroupIDs: []int64{groupIDplaceholderplaceholder
	staleBackup := &Account{ID: 33002, Platform: PlatformOpenAI, Type: AccountTypeOAuth, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 5, GroupIDs: []int64{groupIDplaceholderplaceholder
	dbSticky := Account{ID: 33001, Platform: PlatformOpenAI, Type: AccountTypeOAuth, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 0, GroupIDs: []int64{groupIDplaceholder, RateLimitResetAt: &rateLimitedUntilplaceholder
	dbBackup := Account{ID: 33002, Platform: PlatformOpenAI, Type: AccountTypeOAuth, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 5, GroupIDs: []int64{groupIDplaceholderplaceholder
	cache := &schedulerTestGatewayCache{sessionBindings: map[string]int64{"openai:session_hash_db_runtime_recheck": 33001placeholderplaceholder
	snapshotCache := &openAISnapshotCacheStub{
		snapshotAccounts: []*Account{staleSticky, staleBackupplaceholder,
		accountsByID:     map[int64]*Account{33001: staleSticky, 33002: staleBackupplaceholder,
placeholder
	snapshotService := &SchedulerSnapshotService{cache: snapshotCacheplaceholder
	svc := &OpenAIGatewayService{
		accountRepo:        schedulerTestOpenAIAccountRepo{accounts: []Account{dbSticky, dbBackupplaceholderplaceholder,
		cache:              cache,
		cfg:                &config.Config{placeholder,
		rateLimitService:   newOpenAIAdvancedSchedulerRateLimitService("true"),
		schedulerSnapshot:  snapshotService,
...
func TestOpenAIGatewayService_SelectAccountForModelWithExclusions_DBRuntimeRecheckSkipsStaleCachedCandidate(t *testing.T) {
	...
	groupID := int64(10104)
	rateLimitedUntil := time.Now().Add(30 * time.Minute)
	stalePrimary := &Account{ID: 34001, Platform: PlatformOpenAI, Type: AccountTypeOAuth, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 0, GroupIDs: []int64{groupIDplaceholderplaceholder
	staleSecondary := &Account{ID: 34002, Platform: PlatformOpenAI, Type: AccountTypeOAuth, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 5, GroupIDs: []int64{groupIDplaceholderplaceholder
	dbPrimary := Account{ID: 34001, Platform: PlatformOpenAI, Type: AccountTypeOAuth, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 0, GroupIDs: []int64{groupIDplaceholder, RateLimitResetAt: &rateLimitedUntilplaceholder
	dbSecondary := Account{ID: 34002, Platform: PlatformOpenAI, Type: AccountTypeOAuth, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 5, GroupIDs: []int64{groupIDplaceholderplaceholder
	snapshotCache := &openAISnapshotCacheStub{
		snapshotAccounts: []*Account{stalePrimary, staleSecondaryplaceholder,
		accountsByID:     map[int64]*Account{34001: stalePrimary, 34002: staleSecondaryplaceholder,
placeholder
	snapshotService := &SchedulerSnapshotService{cache: snapshotCacheplaceholder
	svc := &OpenAIGatewayService{
		accountRepo:       schedulerTestOpenAIAccountRepo{accounts: []Account{dbPrimary, dbSecondaryplaceholderplaceholder,
		cfg:               &config.Config{placeholder,
		rateLimitService:  newOpenAIAdvancedSchedulerRateLimitService("true"),
		schedulerSnapshot: snapshotService,
placeholder
...
func TestOpenAIGatewayService_SelectAccountWithScheduler_DBFreshGroupRecheckReleasesMovedAccount(t *testing.T) {
	...
	stalePrimary := &Account{ID: 34101, Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 0, GroupIDs: []int64{groupIDplaceholderplaceholder
	staleBackup := &Account{ID: 34102, Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 10, GroupIDs: []int64{groupIDplaceholderplaceholder
	dbPrimary := *stalePrimary
	dbPrimary.GroupIDs = []int64{otherGroupIDplaceholder
	dbBackup := *staleBackup
	snapshotCache := &openAISnapshotCacheStub{
		snapshotAccounts: []*Account{stalePrimary, staleBackupplaceholder,
		accountsByID:     map[int64]*Account{stalePrimary.ID: stalePrimary, staleBackup.ID: staleBackupplaceholder,
placeholder
	acquiredIDs, releasedIDs := []int64{placeholder, []int64{placeholder
	cfg := &config.Config{RunMode: config.RunModeStandardplaceholder
	svc := &OpenAIGatewayService{
		accountRepo:       schedulerTestOpenAIAccountRepo{accounts: []Account{dbPrimary, dbBackupplaceholderplaceholder,
		cfg:               cfg,
...
placeholder, []openAIAccountCandidateScore{
		{account: stalePrimary, loadInfo: &AccountLoadInfo{AccountID: stalePrimary.IDplaceholderplaceholder,
		{account: staleBackup, loadInfo: &AccountLoadInfo{AccountID: staleBackup.IDplaceholderplaceholder,
placeholder)
placeholder
	require.Equal(t, staleBackup.ID, selection.Account.ID)
	require.Equal(t, []int64{stalePrimary.ID, staleBackup.IDplaceholder, acquiredIDs)
	require.Equal(t, []int64{stalePrimary.IDplaceholder, releasedIDs)
	selection.ReleaseFunc()
placeholder
...
func TestOpenAIGatewayService_SelectAccountWithLoadAwareness_DBFreshGroupRecheckWaitsOnValidAccount(t *testing.T) {
	...
	stalePrimary := &Account{ID: 34201, Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 0, GroupIDs: []int64{groupIDplaceholderplaceholder
	staleBackup := &Account{ID: 34202, Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 10, GroupIDs: []int64{groupIDplaceholderplaceholder
	dbPrimary := *stalePrimary
	dbPrimary.GroupIDs = []int64{otherGroupIDplaceholder
	dbBackup := *staleBackup
	snapshotCache := &openAISnapshotCacheStub{
...
placeholder
	...
	svc := &OpenAIGatewayService{
		...
		schedulerSnapshot: &SchedulerSnapshotService{cache: snapshotCacheplaceholder,
		concurrencyService: NewConcurrencyService(schedulerTestConcurrencyCache{
			acquireResults: map[int64]bool{staleBackup.ID: falseplaceholder,
	placeholder),
placeholder
...
func TestOpenAIGatewayService_RecheckSelectedOpenAIAccountFromDB_SimpleModeUsesFullPool(t *testing.T) {
	grouped := Account{ID: 34301, Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Status: StatusActive, Schedulable: true, Concurrency: 1, GroupIDs: []int64{99placeholderplaceholder
	svc := &OpenAIGatewayService{
		accountRepo:       schedulerTestOpenAIAccountRepo{accounts: []Account{groupedplaceholderplaceholder,
		cfg:               &config.Config{RunMode: config.RunModeSimpleplaceholder,
		schedulerSnapshot: &SchedulerSnapshotService{cache: &openAISnapshotCacheStub{placeholderplaceholder,
placeholder
	requestedGroupID := int64(100)

	for _, groupID := range []*int64{nil, &requestedGroupIDplaceholder {
...
placeholder
	...
	standardSvc := &OpenAIGatewayService{
		accountRepo:       schedulerTestOpenAIAccountRepo{accounts: []Account{grouped, ungroupedplaceholderplaceholder,
		cfg:               &config.Config{RunMode: config.RunModeStandardplaceholder,
		schedulerSnapshot: &SchedulerSnapshotService{cache: &openAISnapshotCacheStub{placeholderplaceholder,
placeholder
...
func TestOpenAIGatewayService_SelectAccountWithScheduler_SessionStickyEscapeByTTFT(t *testing.T) {
	...
	placeholder,
placeholder
	cache := &schedulerTestGatewayCache{sessionBindings: map[string]int64{"openai:session_hash_sticky_ttft": 21101placeholderplaceholder
	cfg := &config.Config{placeholder
	cfg.Gateway.OpenAIScheduler.StickyEscapeEnabled = true
	cfg.Gateway.OpenAIScheduler.StickyEscapeTTFTMs = 15000
	cfg.Gateway.OpenAIScheduler.StickyEscapeErrorRate = 0.5
	concurrencyCache := schedulerTestConcurrencyCache{acquireResults: map[int64]bool{21102: trueplaceholderplaceholder
	svc := &OpenAIGatewayService{
		accountRepo:        schedulerTestOpenAIAccountRepo{accounts: accountsplaceholder,
		cache:              cache,
		cfg:                cfg,
...
func TestOpenAIGatewayService_SelectAccountWithScheduler_SessionStickyEscapeByErrorRate(t *testing.T) {
	...
	groupID := int64(10102)
	accounts := []Account{
		{ID: 21201, Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 0, GroupIDs: []int64{groupIDplaceholderplaceholder,
		{ID: 21202, Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 1, GroupIDs: []int64{groupIDplaceholderplaceholder,
placeholder
	cache := &schedulerTestGatewayCache{sessionBindings: map[string]int64{"openai:session_hash_sticky_error_rate": 21201placeholderplaceholder
	cfg := &config.Config{placeholder
	cfg.Gateway.OpenAIScheduler.StickyEscapeEnabled = true
	cfg.Gateway.OpenAIScheduler.StickyEscapeTTFTMs = 15000
	cfg.Gateway.OpenAIScheduler.StickyEscapeErrorRate = 0.5
	svc := &OpenAIGatewayService{
		...
		cfg:                cfg,
		rateLimitService:   newOpenAIAdvancedSchedulerRateLimitService("true"),
		concurrencyService: NewConcurrencyService(schedulerTestConcurrencyCache{acquireResults: map[int64]bool{21202: trueplaceholderplaceholder),
		openaiAccountStats: newOpenAIAccountRuntimeStats(),
placeholder
	for i := 0; i < 3; i++ {
...
func TestOpenAIGatewayService_SelectAccountWithScheduler_SessionStickyBusyEscapes(t *testing.T) {
	...
	groupID := int64(10103)
	accounts := []Account{
		{ID: 21301, Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 0, GroupIDs: []int64{groupIDplaceholderplaceholder,
		{ID: 21302, Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 1, GroupIDs: []int64{groupIDplaceholderplaceholder,
placeholder
	cache := &schedulerTestGatewayCache{sessionBindings: map[string]int64{"openai:session_hash_sticky_busy_escape": 21301placeholderplaceholder
	cfg := &config.Config{placeholder
	cfg.Gateway.OpenAIScheduler.StickyEscapeEnabled = true
	cfg.Gateway.OpenAIScheduler.StickyEscapeTTFTMs = 15000
	cfg.Gateway.OpenAIScheduler.StickyEscapeErrorRate = 0.5
	...
	cfg.Gateway.Scheduling.StickySessionWaitTimeout = 45 * time.Second
	concurrencyCache := schedulerTestConcurrencyCache{
		acquireResults: map[int64]bool{21301: false, 21302: trueplaceholder,
		waitCounts:     map[int64]int{21301: 999placeholder,
		loadMap: map[int64]*AccountLoadInfo{
			21301: {AccountID: 21301, LoadRate: 95, WaitingCount: 9placeholder,
			21302: {AccountID: 21302, LoadRate: 1, WaitingCount: 0placeholder,
	placeholder,
placeholder
	svc := &OpenAIGatewayService{
		accountRepo:        schedulerTestOpenAIAccountRepo{accounts: accountsplaceholder,
...
func TestOpenAIGatewayService_SelectAccountWithScheduler_SessionStickyEscapeDisabledKeepsLegacyBehavior(t *testing.T) {
	...
	groupID := int64(10104)
	accounts := []Account{
		{ID: 21401, Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 0, GroupIDs: []int64{groupIDplaceholderplaceholder,
		{ID: 21402, Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 1, GroupIDs: []int64{groupIDplaceholderplaceholder,
placeholder
	cache := &schedulerTestGatewayCache{sessionBindings: map[string]int64{"openai:session_hash_sticky_disabled": 21401placeholderplaceholder
	cfg := &config.Config{placeholder
	cfg.Gateway.OpenAIScheduler.StickyEscapeEnabled = false
	cfg.Gateway.OpenAIScheduler.StickyEscapeTTFTMs = 15000
	cfg.Gateway.OpenAIScheduler.StickyEscapeErrorRate = 0.5
	...
	cfg.Gateway.Scheduling.StickySessionWaitTimeout = 45 * time.Second
	concurrencyCache := schedulerTestConcurrencyCache{
		acquireResults: map[int64]bool{21401: false, 21402: trueplaceholder,
		waitCounts:     map[int64]int{21401: 999placeholder,
placeholder
	svc := &OpenAIGatewayService{
		accountRepo:        schedulerTestOpenAIAccountRepo{accounts: accountsplaceholder,
		cache:              cache,
...
func TestOpenAIGatewayService_SelectAccountWithScheduler_SubscriptionPriorityChoosesSubscriptionPoolFirst(t *testing.T) {
	...
	cheapAPIKey.GroupIDs = []int64{groupIDplaceholder
	accounts := []Account{
		{
			ID:          21601,
			Platform:    PlatformOpenAI,
...
placeholder
	concurrencyCache := schedulerTestConcurrencyCache{
		acquireResults: map[int64]bool{21601: true, 21602: trueplaceholder,
		loadMap: map[int64]*AccountLoadInfo{
			21601: {AccountID: 21601, LoadRate: 90, WaitingCount: 1placeholder,
			21602: {AccountID: 21602, LoadRate: 0, WaitingCount: 0placeholder,
	placeholder,
placeholder
	...
	accounts := []Account{
		{
			...
			Priority:    0,
			GroupIDs:    []int64{groupIDplaceholder,
	placeholder"plan_type": "team"placeholder,
	placeholder,
		{
			ID:          21612,
			Platform:    PlatformOpenAI,
...
placeholder
	concurrencyCache := schedulerTestConcurrencyCache{
		acquireResults: map[int64]bool{21611: false, 21612: trueplaceholder,
		loadMap: map[int64]*AccountLoadInfo{
			21611: {AccountID: 21611, LoadRate: 0, WaitingCount: 0placeholder,
			21612: {AccountID: 21612, LoadRate: 90, WaitingCount: 1placeholder,
	placeholder,
placeholder
	svc := &OpenAIGatewayService{
		accountRepo:        schedulerTestOpenAIAccountRepo{accounts: accountsplaceholder,
	...
	accounts := []Account{
		{
			...
			Priority:    10,
			GroupIDs:    []int64{groupIDplaceholder,
	placeholder"plan_type": "pro"placeholder,
	placeholder,
		{
			ID:          21622,
			Platform:    PlatformOpenAI,
...
placeholder
	concurrencyCache := schedulerTestConcurrencyCache{
		acquireResults: map[int64]bool{21621: true, 21622: trueplaceholder,
		loadMap: map[int64]*AccountLoadInfo{
			21621: {AccountID: 21621, LoadRate: 90, WaitingCount: 1placeholder,
			21622: {AccountID: 21622, LoadRate: 0, WaitingCount: 0placeholder,
	placeholder,
placeholder
	svc := &OpenAIGatewayService{
		accountRepo:        schedulerTestOpenAIAccountRepo{accounts: accountsplaceholder,
	...
	accounts := []Account{
		{
			...
			AccountGroups: []AccountGroup{
				{AccountID: 21631, GroupID: groupID, Priority: 100placeholder,
		placeholder,
			GroupIDs: []int64{groupIDplaceholder,
	placeholder,
		{
			...
			Priority:    100000,
			AccountGroups: []AccountGroup{
				{AccountID: 21632, GroupID: groupID, Priority: 1placeholder,
		placeholder,
			GroupIDs: []int64{groupIDplaceholder,
	placeholder,
placeholder
...
func TestOpenAIAccountScheduler_SkipsAccountBlockedForRequestedModel(t *testing.T) {
	now := time.Now()
	account := &Account{ID: 21633, Platform: PlatformOpenAI, Type: AccountTypeAPIKeyplaceholder
	svc := &OpenAIGatewayService{openaiModelTransient: newOpenAIAccountModelTransientState(128)placeholder
	...
	require.False(t, scheduler.isAccountRequestCompatible(context.Background(), account, OpenAIAccountScheduleRequest{RequestedModel: "gpt-5.5"placeholder))
	require.True(t, scheduler.isAccountRequestCompatible(context.Background(), account, OpenAIAccountScheduleRequest{RequestedModel: "gpt-5.6-sol"placeholder))
placeholder
...
func TestDefaultOpenAIAccountScheduler_ShouldEscapeStickyAccount_ThresholdBoundary(t *testing.T) {
	...
	scheduler := &defaultOpenAIAccountScheduler{stats: statsplaceholder
	...
	placeholder,
		{
			...
			Priority:    5,
			AccountGroups: []AccountGroup{
				{AccountID: 2402, GroupID: groupIDplaceholder,
		placeholder,
	placeholder,
placeholder
	cache := &schedulerTestGatewayCache{
...
placeholder
	svc := &OpenAIGatewayService{
		accountRepo:        schedulerTestOpenAIAccountRepo{accounts: accountsplaceholder,
		cache:              &schedulerTestGatewayCache{sessionBindings: map[string]int64{placeholderplaceholder,
		cfg:                cfg,
		rateLimitService:   newOpenAIAdvancedSchedulerRateLimitService("true"),
		concurrencyService: NewConcurrencyService(concurrencyCache),
placeholder
	...
	placeholder,
		{
			...
			Concurrency: 1,
			Priority:    0,
			GroupIDs:    []int64{otherGroupIDplaceholder,
	placeholder,
placeholder
	cfg := &config.Config{placeholder
	cfg.Gateway.OpenAIWS.LBTopK = 2
...
placeholderplaceholder
	concurrencyCache := schedulerTestConcurrencyCache{
		acquireResults: map[int64]bool{38001: false, 38002: trueplaceholder,
placeholder
	svc := &OpenAIGatewayService{
		accountRepo:        schedulerGroupAwareOpenAIAccountRepo{schedulerTestOpenAIAccountRepo{accounts: accountsplaceholderplaceholder,
		cache:              cache,
...
placeholder
	concurrencyCache := schedulerTestConcurrencyCache{
		acquireResults: map[int64]bool{38011: false, 38012: trueplaceholder,
placeholder
	svc := &OpenAIGatewayService{
		accountRepo:        schedulerTestOpenAIAccountRepo{accounts: accountsplaceholder,
		cache:              &schedulerTestGatewayCache{placeholder,
