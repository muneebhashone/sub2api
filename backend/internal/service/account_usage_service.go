...
type ClaudeUsageResponse struct {
	FiveHour struct {
		Utilization float64 `json:"utilization"`
		ResetsAt    string  `json:"resets_at"`
placeholder `json:"five_hour"`
	SevenDay struct {
		Utilization float64 `json:"utilization"`
		ResetsAt    string  `json:"resets_at"`
placeholder `json:"seven_day"`
	SevenDaySonnet struct {
		Utilization float64 `json:"utilization"`
		ResetsAt    string  `json:"resets_at"`
placeholder `json:"seven_day_sonnet"`
...
func (s *AccountUsageService) getOpenAIUsage(ctx context.Context, account *Account, force bool) (*UsageInfo, error) {
	now := time.Now()
	usage := &UsageInfo{UpdatedAt: &nowplaceholder
...
placeholder
	...
	if stats, err := s.usageLogRepo.GetAccountWindowStats(ctx, account.ID, codexWindowStatsStart(usage.FiveHour, 5*time.Hour, now)); err == nil {
		if usage.FiveHour == nil {
			usage.FiveHour = &UsageProgress{Utilization: 0placeholder
	placeholder
...
placeholder
	...
	if stats, err := s.usageLogRepo.GetAccountWindowStats(ctx, account.ID, codexWindowStatsStart(usage.SevenDay, 7*24*time.Hour, now)); err == nil {
		if usage.SevenDay == nil {
			usage.SevenDay = &UsageProgress{Utilization: 0placeholder
	placeholder
...
placeholder
	...
	if account != nil && account.IsGrokOAuth() && s.grokQuotaService != nil && (force || grokBillingSnapshotNeedsRefresh(account, time.Now())) && s.shouldProbeGrokBilling(account.ID, time.Now(), force) {
		...
		if err == nil && result != nil && result.Billing != nil {
			...
			mergeAccountExtra(account, map[string]any{grokBillingExtraKey: result.Billingplaceholder)
	placeholder else if err != nil && force {
			return nil, err
	placeholder
placeholder
...
placeholder
	...
	placeholder else if s.usageLogRepo != nil {
...
placeholder
	...
	progress := &UsageProgress{Utilization: parseExtraFloat64(usedRaw)placeholder
