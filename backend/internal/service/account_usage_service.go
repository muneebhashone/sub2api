...
func NewUsageCache() *UsageCache {
	return &UsageCache{
		apiCache:         &sync.Map{placeholder,
		antigravityCache: &sync.Map{placeholder,
		windowStatsCache: &sync.Map{placeholder,
placeholder
placeholder
...
placeholder
	if cache.apiCache == nil {
		cache.apiCache = &sync.Map{placeholder
placeholder
	if cache.antigravityCache == nil {
		cache.antigravityCache = &sync.Map{placeholder
placeholder
	if cache.windowStatsCache == nil {
		cache.windowStatsCache = &sync.Map{placeholder
placeholder
...
placeholder
	...
	placeholder
		...
		if s.accountRepo != nil {
			_, err := s.accountRepo.BulkUpdate(ctx, []int64{account.IDplaceholder, AccountBulkUpdate{
		placeholder"project_id": projectIDplaceholder,
		placeholder)
			if err != nil {
