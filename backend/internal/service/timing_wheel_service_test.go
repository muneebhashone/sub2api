...
placeholder
	...
	called := false
	captured("k", func() { called = true placeholder)
	if !called {
...
placeholder
	...
	ch := make(chan struct{placeholder, 1)
	svc.Schedule("once", 30*time.Millisecond, func() { ch <- struct{placeholder{placeholder placeholder)
...
placeholder
	...
	ch := make(chan struct{placeholder, 1)
	svc.Schedule("cancel", 80*time.Millisecond, func() { ch <- struct{placeholder{placeholder placeholder)
	svc.Cancel("cancel")
...
placeholder
	...
	var count int32
	svc.ScheduleRecurring("rec", 30*time.Millisecond, func() { atomic.AddInt32(&count, 1) placeholder)
