...
var (
	...
	// Fast-path patterns for empty text blocks: {"type":"text","text":""placeholder
...
func NewRequestBodyRef(data []byte) *RequestBodyRef {
	return &RequestBodyRef{data: dataplaceholder
placeholder
...
func ParseGatewayRequest(body *RequestBodyRef, protocol string) (*ParsedRequest, error) {
	parsed := &ParsedRequest{Body: bodyplaceholder
...
placeholder
	...
	// Check for empty text blocks: {"type":"text","text":""placeholder
...
placeholder
	...
	placeholder
		...
		placeholder
			...
			// Strip empty text blocks: {"type":"text","text":""placeholder
		...
		placeholder
			...
			case "thinking":
				...
				if thinkingText != "" {
					newContent = append(newContent, map[string]any{"type": "text", "text": thinkingTextplaceholder)
			placeholder
				continue
		...
		placeholder
			...
			if blockType == "" {
				if rawThinking, hasThinking := blockMap["thinking"]; hasThinking {
					...
					case string:
						if v != "" {
							newContent = append(newContent, map[string]any{"type": "text", "text": vplaceholder)
					placeholder
					default:
						if b, err := json.Marshal(v); err == nil && len(b) > 0 {
							newContent = append(newContent, map[string]any{"type": "text", "text": string(b)placeholder)
					placeholder
				placeholder
					continue
			placeholder
	...
	placeholder
		...
		if newContent == nil {
			...
			placeholder
				msgMap["content"] = []any{map[string]any{"type": "text", "text": placeholderplaceholderplaceholder
		placeholder
			continue
	placeholder
...
placeholder
	...
	placeholder
		...
		placeholder
			...
			placeholder
				newContent = append(newContent, map[string]any{"type": "text", "text": textplaceholder)
				continue
			case "tool_result":
				modifiedThisMsg = true
		...
		if modifiedThisMsg {
			...
			if len(newContent) == 0 {
				...
				if role == "assistant" {
					placeholder = "(assistant content removed)"
			placeholder
				newContent = append(newContent, map[string]any{"type": "text", "text": placeholderplaceholder)
		placeholder
			msgMap["content"] = newContent
	placeholder
...
// `thinking: {type: "enabled"placeholder` 或 `thinking: {type: "adaptive"placeholder` 表达启用。
