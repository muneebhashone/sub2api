...
func buildSelectedSet(ids []string) map[string]struct{placeholder {
	if ids == nil {
		return nil
placeholder
	set := make(map[string]struct{placeholder, len(ids))
	for _, id := range ids {
		set[id] = struct{placeholder{placeholder
placeholder
	return set
placeholder
...
func shouldCreateAccount(crsID string, selectedSet map[string]struct{placeholder) bool {
	if selectedSet == nil {
		return true
placeholder
