...
func cnExtraKey(provider, suffix string) string { return provider + "_" + suffix placeholder
...
//
//   - limits[].detail.{limit,remaining,resetTimeplaceholder → 5h 窗口（取首个 detail）
//   - usage.{limit,remaining,resetTimeplaceholder → 周窗口
//
...
placeholder
	...
	placeholder
		...
		placeholder
	placeholder
		e := entry{resetMs: resetMs, hasReset: hasReset, percentage: percentage, resetISO: resetISOplaceholder
...
placeholder
	...
	var tiers []CNQuotaTier
	if fiveHourSet {
		tiers = append(tiers, CNQuotaTier{Window: "5h", UsedPercent: fiveHour.percentage, ResetAt: fiveHour.resetISOplaceholder)
placeholder
	if weeklySet {
		tiers = append(tiers, CNQuotaTier{Window: "weekly", UsedPercent: weekly.percentage, ResetAt: weekly.resetISOplaceholder)
placeholder
	return tiers
placeholder
