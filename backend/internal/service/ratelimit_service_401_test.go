...
func TestRateLimitService_HandleOAuth401_WithTempUnschedCache(t *testing.T) {
	...
	service := NewRateLimitService(repo, nil, &config.Config{placeholder, nil, tempCache)
	service.SetTokenCacheInvalidator(invalidator)
	account := &Account{
		ID:       202,
placeholder
...
func TestRateLimitService_HandleOAuth401_TempUnschedCacheError(t *testing.T) {
	repo := &rateLimitAccountRepoStub{placeholder
	invalidator := &tokenCacheInvalidatorRecorder{placeholder
	tempCache := &tempUnschedCacheStub{setErr: errors.New("cache error")placeholder
	service := NewRateLimitService(repo, nil, &config.Config{placeholder, nil, tempCache)
	service.SetTokenCacheInvalidator(invalidator)
	account := &Account{
		ID:       203,
...
placeholder{
		{
			...
			cfg:      &config.Config{RateLimit: config.RateLimitConfig{OAuth401CooldownMinutes: 0placeholderplaceholder,
			expected: 5 * time.Minute,
	placeholder,
		{
			name:     "custom_cooldown_10_minutes",
			cfg:      &config.Config{RateLimit: config.RateLimitConfig{OAuth401CooldownMinutes: 10placeholderplaceholder,
			expected: 10 * time.Minute,
	placeholder,
		{
			name:     "custom_cooldown_1_minute",
			cfg:      &config.Config{RateLimit: config.RateLimitConfig{OAuth401CooldownMinutes: 1placeholderplaceholder,
			expected: 1 * time.Minute,
	placeholder,
		{
			name:     "negative_value_uses_default",
			cfg:      &config.Config{RateLimit: config.RateLimitConfig{OAuth401CooldownMinutes: -5placeholderplaceholder,
			expected: 5 * time.Minute,
	placeholder,
placeholder
...
func TestRateLimitService_OAuth401Cooldown_NilConfig(t *testing.T) {
	service := &RateLimitService{cfg: nilplaceholder
	result := service.oauth401Cooldown()
