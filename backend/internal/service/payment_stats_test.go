...
placeholder
	...
	require.Equal(t, CurrencyAmounts{"CNY": 15, "USD": 10placeholder, stats.TotalAmount)
	require.Equal(t, CurrencyAmounts{"CNY": 10, "USD": 10placeholder, stats.TodayAmount)
	require.Equal(t, CurrencyAmounts{"CNY": 7.5, "USD": 10placeholder, stats.AvgAmount)
...
placeholder
	...
	require.Equal(t, []DailyStats{
		{Date: "2026-07-24", Amount: CurrencyAmounts{"CNY": 15.56placeholder, Count: 2placeholder,
		{Date: "2026-07-25", Amount: CurrencyAmounts{"USD": 30placeholder, Count: 2placeholder,
placeholder, daily)

	methods := buildMethodDistribution(orders)
	require.Equal(t, []PaymentMethodStat{
		{Type: "alipay", Amount: CurrencyAmounts{"USD": 10placeholder, Count: 1placeholder,
		{Type: "stripe", Amount: CurrencyAmounts{"CNY": 15.56, "USD": 20placeholder, Count: 3placeholder,
placeholder, methods)

	users := buildTopUsers(orders)
	require.Equal(t, TopUsersByCurrency{
		"CNY": {
			{UserID: 2, Email: "bob@example.com", Amount: 10placeholder,
			{UserID: 1, Email: "alice@example.com", Amount: 5.56placeholder,
	placeholder,
		"USD": {
			{UserID: 1, Email: "alice@example.com", Amount: 20placeholder,
			{UserID: 2, Email: "bob@example.com", Amount: 10placeholder,
	placeholder,
placeholder, users)
placeholder
...
func paymentStatsTestOrder(userID int64, email, currency string, amount float64, paidAt *time.Time) *dbent.PaymentOrder {
	return &dbent.PaymentOrder{
		...
		PayAmount:        amount,
		PaidAt:           paidAt,
		ProviderSnapshot: map[string]any{"currency": currencyplaceholder,
placeholder
placeholder
