...
placeholder
	...
	placeholder
		...
		if m := videoSuffixRe.FindStringSubmatch(id); m != nil {
			// 视频模型: {familyplaceholder-{orientationplaceholder-{durationplaceholders
			famID = id[:len(id)-len(m[0])]
			orientation = m[1]
			duration, _ = strconv.Atoi(m[2])
			modelType = "video"
	placeholder else if m := imageSuffixRe.FindStringSubmatch(id); m != nil {
			// 图像模型（带方向）: {familyplaceholder-{orientationplaceholder
			famID = id[:len(id)-len(m[0])]
			orientation = m[1]
			modelType = "image"
	placeholder else if cfg, ok := soraModelConfigs[id]; ok && cfg.Type == "image" {
			// 已知的无后缀图像模型（如 gpt-image）
			famID = id
			orientation = "square"
			modelType = "image"
	placeholder else if strings.Contains(id, "image") {
