...
placeholder
	...
	snapshot := buildPaymentOrderProviderSnapshot(sel, CreateOrderRequest{placeholder)
...
placeholder
	...
	instance, err := client.PaymentProviderInstance.Create().
		...
		SetConfig(`{"secretKey":"do-not-copy"placeholder`).
...
func TestBuildPaymentOrderProviderSnapshot_UsesWxpayJSAPIAppIDForOpenIDOrders(t *testing.T) {
	...
	placeholder,
		PaymentMode: "jsapi",
placeholder, CreateOrderRequest{OpenID: "openid-123"placeholder)
...
func TestBuildPaymentOrderProviderSnapshot_IncludesEasyPayMerchantIdentity(t *testing.T) {
	...
	placeholder,
		PaymentMode: "popup",
placeholder, CreateOrderRequest{PaymentType: payment.TypeAlipayplaceholder)
