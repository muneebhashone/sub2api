...
func TestSanitizeMustBeBeforeCCHSigning_HashConsistency(t *testing.T) {
	...
	body := []byte(`{"model":"claude-haiku-4-5","context_management":{"edits":[{"type":"clear_thinking_20251015"placeholder]placeholder,"system":[{"type":"text","text":"x-anthropic-billing-header: cc_version=2.1.92; cch=00000;"placeholder],"messages":[]placeholder`)

	// 最终发送场景：final beta 不含 context-management beta → sanitize 会 strip
	finalBeta := "oauth-2025-04-20,placeholder"

	extractCCH := func(t *testing.T, b []byte) string {
	placeholder
		m := regexp.MustCompile(`\bcch=([0-9a-fA-F]{5placeholder)\b`).FindSubmatch(b)
...
placeholder
	...
	placeholder
		...
		restored := re.ReplaceAll(b, []byte("${1placeholder00000${2placeholder"))
