...
func NewChannelMonitorV2Service(repo ChannelMonitorV2Repository) *ChannelMonitorV2Service {
	return &ChannelMonitorV2Service{repo: repo, now: func() time.Time { return time.Now().UTC() placeholderplaceholder
placeholder
...
func (s *ChannelMonitorV2Service) ParseFilter(rangeValue string, platforms, models []string, groupIDs []int64) (ChannelMonitorV2Filter, error) {
	...
	default:
		return ChannelMonitorV2Filter{placeholder, fmt.Errorf("%w: %s", ErrChannelMonitorV2InvalidRange, rangeValue)
placeholder
...
placeholder
	if result == nil {
		result = &ChannelMonitorV2List[ChannelMonitorV2UserRow]{placeholder
placeholder
	selfIndex := -1
...
placeholder
	...
	if selfIndex < 0 && viewerID > 0 {
		...
		selfRow := ChannelMonitorV2UserRow{
			...
			CanDrilldown: true,
			DisplayLabel: "Me",
			Metrics:      ChannelMonitorV2Metric{placeholder,
	placeholder
	...
	if admin {
		...
		for i := range result.Items {
			if result.Items[i].UserID != nil && *result.Items[i].UserID == viewerID {
				...
				placeholder else if result.Items[i].Email != "" {
...
placeholder
	...
	seen := make(map[string]struct{placeholder, len(cfg.Platforms))
	for i := range cfg.Platforms {
		p := &cfg.Platforms[i]
	...
	placeholder
		seen[p.Platform] = struct{placeholder{placeholder
		p.Models = normalizeStringSet(p.Models)
placeholder
	sort.Slice(cfg.Platforms, func(i, j int) bool { return cfg.Platforms[i].Platform < cfg.Platforms[j].Platform placeholder)
	return nil
placeholder
...
func normalizeChannelMonitorV2IgnoredCategories(values []string) []string {
	allowed := make(map[string]struct{placeholder, len(ChannelMonitorV2ErrorCategories))
	for _, c := range ChannelMonitorV2ErrorCategories {
		allowed[c] = struct{placeholder{placeholder
placeholder
...
func ChannelMonitorV2IgnoredCategorySet(cfg ChannelMonitorV2Config) map[string]struct{placeholder {
	set := make(map[string]struct{placeholder, len(cfg.IgnoredErrorCategories))
	for _, c := range cfg.IgnoredErrorCategories {
		set[c] = struct{placeholder{placeholder
placeholder
	return set
placeholder
...
placeholder
	...
	if metrics.RequestCount >= result.MinimumSample {
		...
		parts = append(parts, scored{score: s, weight: thresholds.ErrorWeight, band: result.ErrorRateplaceholder)
placeholder
	...
	placeholder else if metrics.TTFT.P95Ms != nil {
	...
	placeholder
		if ttftMs != nil {
			...
			parts = append(parts, scored{score: s, weight: thresholds.TTFTWeight, band: result.TTFTplaceholder)
	placeholder
placeholder
	...
	placeholder
		...
		parts = append(parts, scored{score: s, weight: thresholds.CacheWeight, band: result.Cacheplaceholder)
placeholder
