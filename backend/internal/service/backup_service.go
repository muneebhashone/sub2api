...
placeholder
	...
	if err != nil {
		...
		return record, err
placeholder
	defer func() { _ = cleanupBackupFiles(archivePath) placeholder()
...
placeholder
	defer func() { _ = gzReader.Close() placeholder()
...
placeholder
	...
	sort.Slice(ordered, func(i, j int) bool { return ordered[i].Index < ordered[j].Index placeholder)
...
placeholder
	...
	if len(record.Parts) > 0 {
		...
		sort.Slice(parts, func(i, j int) bool { return parts[i].Index < parts[j].Index placeholder)
...
placeholder
	...
	seen := make(map[string]struct{placeholder, len(record.Parts)+1)
