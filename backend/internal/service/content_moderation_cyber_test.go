...
placeholder
	...
	svc.RecordCyberPolicyEvent(context.Background(), CyberPolicyRecordInput{
		...
		UpstreamBody:    `{"error":{"code":"cyber_policy"placeholderplaceholder`,
		UpstreamStatus:  400,
placeholder)
...
placeholder{
		{
			name:     "excluded group",
			config:   `{"all_groups":false,"group_ids":[8],"ban_threshold":1placeholder`,
			groupID:  &groupID,
			model:    "gpt-5",
			wantLogs: 0,
	placeholder,
		{
			...
			config:   `{"all_groups":false,"group_ids":[7],"ban_threshold":1placeholder`,
			groupID:  nil,
			model:    "gpt-5",
			wantLogs: 0,
	placeholder,
		{
			name:     "excluded model",
			config:   `{"all_groups":true,"model_filter":{"type":"include","models":["gpt-4o"]placeholder,"ban_threshold":1placeholder`,
			groupID:  &groupID,
			model:    "gpt-5",
			wantLogs: 0,
	placeholder,
		{
			...
			config:     `{"enabled":false,"mode":"off","sample_rate":0,"all_groups":false,"group_ids":[7],"model_filter":{"type":"include","models":["gpt-5"]placeholder,"ban_threshold":1placeholder`,
			groupID:    &groupID,
			model:      "gpt-5",
			wantCalls:  []bool{falseplaceholder,
			wantLogs:   1,
			wantBanned: true,
	placeholder,
placeholder
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			userRepo := &contentModerationTestUserRepo{user: &User{ID: 1, Role: RoleUser, Status: StatusActiveplaceholderplaceholder
			svc := NewContentModerationService(
				&contentModerationTestSettingRepo{values: map[string]string{
					SettingKeyRiskControlEnabled:      "true",
...
func TestRecordCyberPolicyEvent_RuntimeSnapshotRefreshFailureKeepsStaleScope(t *testing.T) {
	...
	settingRepo := &contentModerationRuntimeSettingRepo{values: map[string]string{
		SettingKeyRiskControlEnabled:      "true",
		SettingKeyContentModerationConfig: `{"all_groups":true,"model_filter":{"type":"include","models":["gpt-5"]placeholderplaceholder`,
placeholderplaceholder
	svc := NewContentModerationService(settingRepo, repo, nil, nil, nil, nil, nil, nil)
...
func TestRecordCyberPolicyEvent_CreateLogBeforeEmail(t *testing.T) {
	repo := &cyberOrderingTestRepo{placeholder
	svc := NewContentModerationService(
		&contentModerationTestSettingRepo{values: map[string]string{
			SettingKeyRiskControlEnabled: "true",
placeholder
...
func TestApplyFlaggedAccountSideEffects_PassesExcludeCyberFlag(t *testing.T) {
	...
	svc.applyFlaggedAccountSideEffects(context.Background(), cfgExclude, &ContentModerationLog{Flagged: true, UserID: &userIDplaceholder)

	cfgDefault := defaultContentModerationConfig() // 默认 false
	svc.applyFlaggedAccountSideEffects(context.Background(), cfgDefault, &ContentModerationLog{Flagged: true, UserID: &userIDplaceholder)

	require.Equal(t, []bool{true, falseplaceholder, repo.snapshotCountCalls(),
...
placeholder)
	...
	require.Equal(t, []bool{falseplaceholder, repo.snapshotCountCalls(),
