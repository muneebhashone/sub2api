...
placeholder
	if apiKeyID > 0 {
		c.Set("api_key", &APIKey{ID: apiKeyIDplaceholder)
placeholder
	return c, rec
placeholder
...
func TestRelayOpenAICodexTurnState_SetsHeaderAndRecordsProvenance(t *testing.T) {
	...
	account := &Account{ID: 42placeholder
	c, _ := newTurnStateTestContext(t, 7, "sess-relay")

	upstream := http.Header{placeholder
...
func TestRelayOpenAICodexTurnState_ClearsStaleValueWhenUpstreamAbsent(t *testing.T) {
	...
	svc.relayOpenAICodexTurnState(c, &Account{ID: 43placeholder, http.Header{placeholder)
...
func TestStageOpenAICodexTurnState_StagedHeaders(t *testing.T) {
	...
	// 真正提交时才记录
	svc.noteStagedOpenAICodexTurnStateCommitted(c, &Account{ID: 44placeholder, staged)
	...
	stageOpenAICodexTurnState(&staged, http.Header{placeholder)
	require.Empty(t, staged.Get("X-Codex-Turn-State"))
	var nilStaged http.Header
	stageOpenAICodexTurnState(&nilStaged, http.Header{placeholder)
...
func TestNoteStagedOpenAICodexTurnStateCommitted_NoopWithoutState(t *testing.T) {
	...
	svc.noteStagedOpenAICodexTurnStateCommitted(c, &Account{ID: 60placeholder, nil)
	svc.noteStagedOpenAICodexTurnStateCommitted(c, &Account{ID: 60placeholder, http.Header{"X-Request-Id": []string{"rid"placeholderplaceholder)
...
placeholder
	...
	t.Run("same_account_keeps_echo", func(t *testing.T) {
		...
		upstream := http.Header{placeholder
		upstream.Set("x-codex-turn-state", "blob-A")
		svc.relayOpenAICodexTurnState(c, &Account{ID: 42placeholder, upstream)

		h := newOutbound("blob-A")
		svc.guardOpenAICodexTurnStateEcho(c, &Account{ID: 42placeholder, h)
		require.Equal(t, "blob-A", h.Get("x-codex-turn-state"))
placeholder)
	...
	t.Run("foreign_account_strips_echo", func(t *testing.T) {
		...
		h := newOutbound("blob-A")
		svc.guardOpenAICodexTurnStateEcho(c, &Account{ID: 43placeholder, h)
...
func TestWriteOpenAIPassthroughResponseHeaders_RelaysAndClearsTurnState(t *testing.T) {
	...
	writeOpenAIPassthroughResponseHeaders(dst, http.Header{"Content-Type": []string{"application/json"placeholderplaceholder, nil)
...
func TestWriteOpenAIPassthroughResponseHeaders_RelaysReasoningIncluded(t *testing.T) {
	...
	writeOpenAIPassthroughResponseHeaders(
		dst,
		src,
		responseheaders.CompileHeaderFilter(config.ResponseHeaderConfig{placeholder),
	)
...
placeholder)
	...
	t.Run("multi_line_values_merged_single_line", func(t *testing.T) {
		...
		ensureOpenAIRemoteCompactionV2BetaFeature(h)
		require.Equal(t, []string{"feature_a,feature_b,remote_compaction_v2"placeholder, h.Values("x-codex-beta-features"))
placeholder)
placeholder
...
func TestApplyOpenAICodexBetaFeatures(t *testing.T) {
	oauthAccount := &Account{ID: 1, Platform: PlatformOpenAI, Type: AccountTypeOAuthplaceholder
	apiKeyAccount := &Account{ID: 2, Platform: PlatformOpenAI, Type: AccountTypeAPIKeyplaceholder
...
placeholder"chatgpt_account_id": "test-account"placeholder,
placeholder
	...
	require.Equal(t, []string{"some_other_feature"placeholder, declared.Values("x-codex-beta-features"),
		"客户端已声明时原样保留")

	apiKeyHeaders := build(t, &Account{Platform: PlatformOpenAI, Type: AccountTypeAPIKeyplaceholder, "")
