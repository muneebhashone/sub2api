...
placeholder
	...
	view := openAIRequestView{body: bodyplaceholder
	var seen uint8
...
placeholder
	if account != nil && account.IsOpenAI() && account.IsOAuth() {
		if reasoningBody, reasoningChanged, err := normalizeOpenAIResponsesReasoningMode(normalized); err != nil {
			return body, false, err
	placeholder else if reasoningChanged {
...
placeholder
	if schemaBody, schemaChanged, err := normalizeOpenAIResponseFormatSchemasBody(normalized); err != nil {
		return body, false, err
placeholder else if schemaChanged {
...
placeholder
	if tierResult.Type != gjson.String {
		return "", &ErrInvalidOpenAIServiceTier{Value: "<non-string>"placeholder
placeholder
	raw := strings.TrimSpace(tierResult.String())
	if raw == "" {
		return "", &ErrInvalidOpenAIServiceTier{Value: rawplaceholder
placeholder
	norm := normalizedOpenAIServiceTierValue(raw)
	if norm == "" {
		return "", &ErrInvalidOpenAIServiceTier{Value: boundInvalidOpenAIServiceTierValue(raw)placeholder
placeholder
	return norm, nil
placeholder
...
placeholder
	...
	for _, userScoped := range []bool{true, falseplaceholder {
