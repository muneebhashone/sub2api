...
placeholder{
		{
			...
			body: `{"status":"incomplete","incomplete_details":{"reason":"max_output_tokens"placeholder,` +
				`"output":[{"type":"reasoning","summary":[]placeholder]placeholder`,
	placeholder,
		{
			// HTTP 200 携带的失败响应是上游瞬时故障，不构成工具能力证据。
			name: "failed_status_on_http_200",
			body: `{"status":"failed","error":{"code":"server_error","message":"upstream hiccup"placeholder,"output":[]placeholder`,
	placeholder,
placeholder
...
placeholder{
		{
			...
			body:   `{"status":"completed","output":[{"type":"function_call","name":"probe_ping"placeholder]placeholder`,
			want:   true,
	placeholder,
		{
			...
			status: http.StatusOK,
			body:   `{"status":"completed","output":[{"type":"reasoning"placeholder]placeholder`,
			want:   false,
	placeholder,
		{
			...
			status: http.StatusOK,
			body:   `{"status":"incomplete","incomplete_details":{"reason":"content_filter"placeholder,"output":[]placeholder`,
			want:   false,
	placeholder,
		{
			...
			status: http.StatusOK,
			body:   `{"output":[{"type":"function_call","name":"probe_ping"placeholder]placeholder`,
			want:   true,
	placeholder,
		{
			name:   "endpoint_absent_404",
			status: http.StatusNotFound,
			body:   `{"error":{"message":"Not Found"placeholderplaceholder`,
			want:   false,
	placeholder,
		{
			...
			body:   `{"status":"failed"placeholder`,
			want:   true,
	placeholder,
placeholder
...
func TestResponsesProbeVerdictIsConclusive(t *testing.T) {
	cases := []struct {
		...
		want   bool
placeholder{
		{"200_completed", 200, `{"status":"completed","output":[]placeholder`, trueplaceholder,
		{"200_incomplete_max_output_tokens", 200,
			`{"status":"incomplete","incomplete_details":{"reason":"max_output_tokens"placeholderplaceholder`, falseplaceholder,
		{"200_incomplete_content_filter", 200,
			`{"status":"incomplete","incomplete_details":{"reason":"content_filter"placeholderplaceholder`, trueplaceholder,
		{"200_incomplete_without_reason", 200, `{"status":"incomplete"placeholder`, trueplaceholder,
		{"200_failed", 200, `{"status":"failed"placeholder`, falseplaceholder,
		{"200_no_status_field", 200, `{"output":[]placeholder`, trueplaceholder,
		{"200_non_json", 200, `not-json`, trueplaceholder,
		{"200_empty_body", 200, ``, trueplaceholder,
		// 非 2xx 只看状态码，不读 body。
		{"404_ignores_body_status", 404, `{"status":"failed"placeholder`, trueplaceholder,
		{"500_ignores_body_status", 500, `{"status":"incomplete","incomplete_details":{"reason":"max_output_tokens"placeholderplaceholder`, trueplaceholder,
placeholder
