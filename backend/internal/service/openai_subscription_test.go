...
placeholder))
	...
	t.Cleanup(func() { chatGPTSubscriptionsURL = oldURL placeholder)
...
placeholder, "access-token", "", "acc_123")
...
placeholder))
	...
	t.Cleanup(func() { chatGPTAccountsCheckURL = oldURL placeholder)

	got := fetchChatGPTAccountInfo(context.Background(), func(proxyURL string) (*req.Client, error) {
		return req.C().SetTimeout(5 * time.Second), nil
placeholder, "access-token", "", "org-expired-workspace")
...
placeholder))
	...
	got := fetchChatGPTAccountInfo(context.Background(), func(proxyURL string) (*req.Client, error) {
		return req.C().SetTimeout(5 * time.Second), nil
placeholder, "access-token", "", "org-deactivated-workspace")
...
func TestChatGPTAccountInfoBelongsToTokenAccount(t *testing.T) {
	require.False(t, chatGPTAccountInfoBelongsToTokenAccount(
		&OpenAITokenInfo{ChatGPTAccountID: "personal-a"placeholder, &ChatGPTAccountInfo{AccountID: "workspace-b"placeholder))
	require.True(t, chatGPTAccountInfoBelongsToTokenAccount(
		&OpenAITokenInfo{ChatGPTAccountID: "personal-a"placeholder, &ChatGPTAccountInfo{AccountID: "PERSONAL-A"placeholder))
	// 任一侧缺 ID 时无法区分，保持既有行为（采用 accounts/check 的值）。
	require.True(t, chatGPTAccountInfoBelongsToTokenAccount(
		&OpenAITokenInfo{placeholder, &ChatGPTAccountInfo{AccountID: "workspace-b"placeholder))
	require.True(t, chatGPTAccountInfoBelongsToTokenAccount(
		&OpenAITokenInfo{ChatGPTAccountID: "personal-a"placeholder, &ChatGPTAccountInfo{placeholder))
placeholder
			...
			"accounts": map[string]any{
				"default": map[string]any{
					"account": map[string]any{
						...
						"is_default": true,
				placeholder,
					"entitlement": map[string]any{"expires_at": futureAtplaceholder,
			placeholder,
		placeholder,
	placeholder)
placeholder))
			...
			"accounts": map[string]any{
				workspaceAccountID: map[string]any{
					"account": map[string]any{
						...
						"is_default": true,
				placeholder,
					"entitlement": map[string]any{"expires_at": workspaceExpiresAtplaceholder,
			placeholder,
		placeholder,
	placeholder,
		onSubscription: func(accountID string) map[string]any {
			...
			return map[string]any{"plan_type": "pro", "active_until": personalActiveUntil, "will_renew": trueplaceholder
	placeholder,
placeholder)
	defer server.Close()
...
placeholder
	svc := &OpenAIOAuthService{privacyClientFactory: newTestPrivacyClientFactory()placeholder
			...
			"accounts": map[string]any{
				personalAccountID: map[string]any{
					"account": map[string]any{
						...
						"is_default": true,
				placeholder,
					"entitlement": map[string]any{"expires_at": entitlementExpiresAtplaceholder,
			placeholder,
		placeholder,
	placeholder,
