...
func TestAdminService_GetGroupRateMultipliers(t *testing.T) {
	t.Run("returns entries for group", func(t *testing.T) {
		repo := &userGroupRateRepoStubForGroupRate{
			getByGroupIDData: map[int64][]UserGroupRateEntry{
				10: {
					{UserID: 1, UserName: "alice", UserEmail: "alice@test.com", RateMultiplier: ptrFloat(1.5)placeholder,
					{UserID: 2, UserName: "bob", UserEmail: "bob@test.com", RateMultiplier: ptrFloat(0.8)placeholder,
			placeholder,
		placeholder,
	placeholder
...
func TestAdminService_BatchSetGroupRPMOverrides(t *testing.T) {
	t.Run("syncs entries to repo", func(t *testing.T) {
		...
		svc := &adminServiceImpl{userGroupRateRepo: repoplaceholder
		override := 20
		entries := []GroupRPMOverrideInput{{UserID: 2, RPMOverride: &overrideplaceholderplaceholder
...
placeholder)
	...
	t.Run("rejects negative override as bad request", func(t *testing.T) {
		...
		err := svc.BatchSetGroupRPMOverrides(context.Background(), 10, []GroupRPMOverrideInput{
			{UserID: 2, RPMOverride: &negativeplaceholder,
	placeholder)
	placeholder
