...
func newVertexServiceAccountHTTPClient(proxyURL string) (*http.Client, error) {
	...
	if proxyURL == "" {
		return servertiming.InstrumentClient(&http.Client{Timeout: 15 * time.Secondplaceholder), nil
placeholder
...
placeholder
	...
	if err := proxyutil.ConfigureTransportProxy(transport, parsedProxy); err != nil {
		return nil, err
placeholder
	return servertiming.InstrumentClient(&http.Client{Timeout: 15 * time.Second, Transport: transportplaceholder), nil
placeholder
