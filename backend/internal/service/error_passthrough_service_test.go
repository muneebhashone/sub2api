...
func newTestService(rules []*model.ErrorPassthroughRule) *ErrorPassthroughService {
	repo := &mockErrorPassthroughRepo{rules: rulesplaceholder
...
func newCachedRuleForTest(rule *model.ErrorPassthroughRule) *cachedPassthroughRule {
	cr := &cachedPassthroughRule{ErrorPassthroughRule: ruleplaceholder
...
placeholder
	if len(rule.ErrorCodes) > 0 {
		cr.errorCodeSet = make(map[int]struct{placeholder, len(rule.ErrorCodes))
		for _, code := range rule.ErrorCodes {
			cr.errorCodeSet[code] = struct{placeholder{placeholder
	placeholder
placeholder
	return cr
placeholder
...
func TestRuleMatches_OnlyErrorCodes_AnyMode(t *testing.T) {
	...
	rule := newCachedRuleForTest(&model.ErrorPassthroughRule{
		Enabled:    true,
		ErrorCodes: []int{422, 400placeholder,
		Keywords:   []string{placeholder,
		MatchMode:  model.MatchModeAny,
placeholder)
	...
	tests := []struct {
		...
		expected   bool
placeholder{
		{"状态码匹配 422", 422, "any message", trueplaceholder,
		{"状态码匹配 400", 400, "any message", trueplaceholder,
		{"状态码不匹配 500", 500, "any message", falseplaceholder,
		{"状态码不匹配 429", 429, "any message", falseplaceholder,
placeholder
...
func TestRuleMatches_OnlyKeywords_AnyMode(t *testing.T) {
	...
	rule := newCachedRuleForTest(&model.ErrorPassthroughRule{
		Enabled:    true,
		ErrorCodes: []int{placeholder,
		Keywords:   []string{"context limit", "model not supported"placeholder,
		MatchMode:  model.MatchModeAny,
placeholder)
	...
	tests := []struct {
		...
		expected   bool
placeholder{
		{"关键词匹配 context limit", 500, "error: context limit reached", trueplaceholder,
		{"关键词匹配 model not supported", 400, "the model not supported here", trueplaceholder,
		{"关键词不匹配", 422, "some other error", falseplaceholder,
		{"关键词大小写 - 自动转换", 500, "Context Limit exceeded", trueplaceholder,
placeholder
...
func TestRuleMatches_BothConditions_AnyMode(t *testing.T) {
	...
	rule := newCachedRuleForTest(&model.ErrorPassthroughRule{
		Enabled:    true,
		ErrorCodes: []int{422, 400placeholder,
		Keywords:   []string{"context limit"placeholder,
		MatchMode:  model.MatchModeAny,
placeholder)
...
placeholder{
		{
			name:            "空平台列表匹配所有",
			rulePlatforms:   []string{placeholder,
	...
	placeholder,
		{
			name:            "精确匹配 anthropic",
			rulePlatforms:   []string{"anthropic", "openai"placeholder,
			requestPlatform: "anthropic",
			expected:        true,
	placeholder,
		{
	...
	placeholder,
		{
			name:            "匹配 antigravity",
			rulePlatforms:   []string{"antigravity"placeholder,
	...
	rules := []*model.ErrorPassthroughRule{
		{
			...
			Priority:   1,
			ErrorCodes: []int{422placeholder,
			Platforms:  []string{"anthropic"placeholder,
			MatchMode:  model.MatchModeAny,
	placeholder,
		{
			ID:         2,
			...
			Priority:   2,
			ErrorCodes: []int{422placeholder,
			Platforms:  []string{"openai"placeholder,
			MatchMode:  model.MatchModeAny,
	placeholder,
		{
			ID:         3,
...
func TestMatchRule_EmptyRules(t *testing.T) {
	svc := newTestService([]*model.ErrorPassthroughRule{placeholder)
	matched := svc.MatchRule("anthropic", 422, []byte("error"))
...
placeholder
	...
	tests := []struct {
		...
		expected bool
placeholder{
		{"完全匹配", "Context Limit reached", trueplaceholder,
		{"小写匹配", "context limit reached", trueplaceholder,
		{"大写匹配", "CONTEXT LIMIT REACHED", trueplaceholder,
		{"混合大小写", "ConTeXt LiMiT error", trueplaceholder,
		{"不匹配", "some other error", falseplaceholder,
placeholder
	...
	rules := []*model.ErrorPassthroughRule{
		{
			...
			Platforms:       []string{"anthropic", "antigravity"placeholder,
...
placeholder
	...
	t.Run("Anthropic 422 with context limit", func(t *testing.T) {
		body := []byte(`{"type":"error","error":{"type":"invalid_request","message":"The context limit has been reached"placeholderplaceholder`)
...
placeholder)
	...
	t.Run("Antigravity 422 with context limit", func(t *testing.T) {
		body := []byte(`{"error":"context limit exceeded"placeholder`)
...
placeholder)
	...
	t.Run("Wrong keyword", func(t *testing.T) {
		body := []byte(`{"error":"rate limit exceeded"placeholder`)
		matched := svc.MatchRule("anthropic", 422, body)
	...
	rules := []*model.ErrorPassthroughRule{
		{
			...
			Enabled:         true,
			Priority:        1,
			ErrorCodes:      []int{500, 502, 503placeholder,
			MatchMode:       model.MatchModeAny,
	...
	placeholder,
		{
			...
			rule: &model.ErrorPassthroughRule{
				Name:            "Valid Rule",
				MatchMode:       model.MatchModeAll,
				ErrorCodes:      []int{500placeholder,
				Keywords:        []string{"internal error"placeholder,
				PassthroughCode: false,
...
func TestCreate_ForceRefreshCacheAfterWrite(t *testing.T) {
	...
	repo := &mockErrorPassthroughRepo{rules: []*model.ErrorPassthroughRule{placeholderplaceholder
	cache := newMockErrorPassthroughCache([]*model.ErrorPassthroughRule{staleRuleplaceholder, true)

	svc := &ErrorPassthroughService{repo: repo, cache: cacheplaceholder
	svc.setLocalCache([]*model.ErrorPassthroughRule{staleRuleplaceholder)
...
placeholder
	...
	body := []byte(`{"message":"Service temporarily unavailable after multiple retries, please try again later"placeholder`)
...
func TestUpdate_ForceRefreshCacheAfterWrite(t *testing.T) {
	...
	repo := &mockErrorPassthroughRepo{rules: []*model.ErrorPassthroughRule{originalRuleplaceholderplaceholder
	cache := newMockErrorPassthroughCache([]*model.ErrorPassthroughRule{originalRuleplaceholder, true)

	svc := &ErrorPassthroughService{repo: repo, cache: cacheplaceholder
	svc.setLocalCache([]*model.ErrorPassthroughRule{originalRuleplaceholder)

	updatedRule := newPassthroughRuleForWritePathTest(1, "new keyword", "新消息")
	_, err := svc.Update(ctx, updatedRule)
placeholder

	oldBody := []byte(`{"message":"old keyword"placeholder`)
	oldMatched := svc.MatchRule("anthropic", 503, oldBody)
	assert.Nil(t, oldMatched, "更新后旧关键词不应继续命中")

	newBody := []byte(`{"message":"new keyword"placeholder`)
...
func TestDelete_ForceRefreshCacheAfterWrite(t *testing.T) {
	...
	repo := &mockErrorPassthroughRepo{rules: []*model.ErrorPassthroughRule{ruleplaceholderplaceholder
	cache := newMockErrorPassthroughCache([]*model.ErrorPassthroughRule{ruleplaceholder, true)
...
placeholder
	...
	body := []byte(`{"message":"to be deleted"placeholder`)
	matched := svc.MatchRule("anthropic", 503, body)
...
func TestNewService_StartupReloadFromDBToHealStaleCache(t *testing.T) {
	...
	repo := &mockErrorPassthroughRepo{rules: []*model.ErrorPassthroughRule{latestRuleplaceholderplaceholder
	cache := newMockErrorPassthroughCache([]*model.ErrorPassthroughRule{staleRuleplaceholder, true)

	svc := NewErrorPassthroughService(repo, cache)

	matchedFresh := svc.MatchRule("anthropic", 503, []byte(`{"message":"fresh keyword"placeholder`))
	require.NotNil(t, matchedFresh)
	assert.Equal(t, int64(1), matchedFresh.ID)

	matchedStale := svc.MatchRule("anthropic", 503, []byte(`{"message":"stale keyword"placeholder`))
...
func TestUpdate_RefreshFailureShouldNotKeepStaleEnabledRule(t *testing.T) {
	...
	repo := &mockErrorPassthroughRepo{
		rules:   []*model.ErrorPassthroughRule{staleRuleplaceholder,
...
// Helper functions
func testIntPtr(i int) *int       { return &i placeholder
func testStrPtr(s string) *string { return &s placeholder
