...
placeholder
	...
	// 我们只直接测 checkRPM。
	svc := NewBillingCacheService(nil, nil, nil, nil, cache, rateRepo, &config.Config{placeholder, nil)
	t.Cleanup(svc.Stop)
	return svc
placeholder
...
func TestBillingCacheService_CheckRPM_OverrideTakesPrecedenceOverGroup(t *testing.T) {
	...
	cache := &userRPMCacheStub{userGroupCounts: []int{1, 2, 3placeholderplaceholder
	repo := &rpmOverrideRepoStub{override: &overrideplaceholder
	svc := newBillingServiceForRPM(t, cache, repo)

	user := &User{ID: 1, RPMLimit: 100placeholder // 全局上限设高，不干扰 override 测试
	group := &Group{ID: 10, RPMLimit: 100placeholder
...
func TestBillingCacheService_CheckRPM_UserLimitIsGlobalHardCap(t *testing.T) {
	...
	cache := &userRPMCacheStub{userCounts: []int{1, 2, 3placeholderplaceholder
	repo := &rpmOverrideRepoStub{override: &overrideplaceholder
	svc := newBillingServiceForRPM(t, cache, repo)

	user := &User{ID: 1, RPMLimit: 2placeholder // 全局硬上限=2，应覆盖 override=100
	group := &Group{ID: 10, RPMLimit: 100placeholder
...
func TestBillingCacheService_CheckRPM_OverrideZeroSkipsGroupButUserStillApplies(t *testing.T) {
	zero := 0
	// user 计数: 依次返回 1..6
	cache := &userRPMCacheStub{userCounts: []int{1, 2, 3, 4, 5, 6placeholderplaceholder
	repo := &rpmOverrideRepoStub{override: &zeroplaceholder
	svc := newBillingServiceForRPM(t, cache, repo)

	user := &User{ID: 1, RPMLimit: 5placeholder
	group := &Group{ID: 10, RPMLimit: 100placeholder
...
func TestBillingCacheService_CheckRPM_OverrideZeroAndUserZeroIsFullyUnlimited(t *testing.T) {
	zero := 0
	cache := &userRPMCacheStub{placeholder
	repo := &rpmOverrideRepoStub{override: &zeroplaceholder
	svc := newBillingServiceForRPM(t, cache, repo)

	user := &User{ID: 1, RPMLimit: 0placeholder // user 也不限
	group := &Group{ID: 10, RPMLimit: 100placeholder
...
func TestBillingCacheService_CheckRPM_NilOverrideFallsThroughToGroup(t *testing.T) {
	...
	cache := &userRPMCacheStub{userGroupCounts: []int{5, 6placeholderplaceholder
	repo := &rpmOverrideRepoStub{override: nilplaceholder
	svc := newBillingServiceForRPM(t, cache, repo)

	user := &User{ID: 1, RPMLimit: 999placeholder // 全局上限很高，group 先超
	group := &Group{ID: 10, RPMLimit: 5placeholder
...
func TestBillingCacheService_CheckRPM_OverrideLookupErrorFallsThroughToGroup(t *testing.T) {
	cache := &userRPMCacheStub{userGroupCounts: []int{3placeholderplaceholder
	repo := &rpmOverrideRepoStub{err: errors.New("db down")placeholder
	svc := newBillingServiceForRPM(t, cache, repo)

	user := &User{ID: 1, RPMLimit: 0placeholder
	group := &Group{ID: 10, RPMLimit: 10placeholder
...
func TestBillingCacheService_CheckRPM_UserLevelFallbackWhenGroupUnlimited(t *testing.T) {
	...
	user := &User{ID: 1, RPMLimit: 2placeholder
	group := &Group{ID: 10, RPMLimit: 0placeholder // 分组未设限
...
func TestBillingCacheService_CheckRPM_NoLimitsConfiguredIsNoop(t *testing.T) {
	...
	user := &User{ID: 1, RPMLimit: 0placeholder
	group := &Group{ID: 10, RPMLimit: 0placeholder
...
func TestBillingCacheService_CheckRPM_RedisErrorFailOpen(t *testing.T) {
	cache := &userRPMCacheStub{userGroupErr: errors.New("redis unavailable")placeholder
	repo := &rpmOverrideRepoStub{override: nilplaceholder
	svc := newBillingServiceForRPM(t, cache, repo)
...
func TestBillingCacheService_CheckRPM_NoGroupUsesUserOnly(t *testing.T) {
	cache := &userRPMCacheStub{userCounts: []int{1, 2, 3placeholderplaceholder
	repo := &rpmOverrideRepoStub{placeholder
	svc := newBillingServiceForRPM(t, cache, repo)
...
func TestBillingCacheService_CheckRPM_NilUserIsNoop(t *testing.T) {
	...
	require.NoError(t, svc.checkRPM(context.Background(), nil, &Group{ID: 1, RPMLimit: 10placeholder))
	require.EqualValues(t, 0, atomic.LoadInt32(&cache.userGroupCalls))
	require.EqualValues(t, 0, atomic.LoadInt32(&cache.userCalls))
