...
func TestCalculateAudioCost(t *testing.T) {
	...
	cfg := &audioPriceConfig{RealtimePerMin: &rt, TTSPerMChars: &tts, STTPerHour: &sttplaceholder
	...
	// explicit 0 → free
	zero := 0.0
	require.Equal(t, 0.0, s.CalculateAudioCost("realtime", 1, &audioPriceConfig{RealtimePerMin: &zeroplaceholder, 1).ActualCost)
placeholder

func floatPtr(v float64) *float64 { return &v placeholder
