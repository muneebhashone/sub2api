...
func compatCyberUpstreamRecorder() *httpUpstreamRecorder {
	return &httpUpstreamRecorder{resp: &http.Response{
		...
		Header:     http.Header{"Content-Type": []string{"text/event-stream"placeholder, "x-request-id": []string{"rid_cyber"placeholderplaceholder,
...
func TestForwardAsChatCompletions_BufferedCyberPolicyNoFailover(t *testing.T) {
	...
	body := []byte(`{"model":"gpt-5.5","messages":[{"role":"user","content":"hi"placeholder],"stream":falseplaceholder`)
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/chat/completions", bytes.NewReader(body))
	c.Request.Header.Set("Content-Type", "application/json")

	svc := &OpenAIGatewayService{httpUpstream: compatCyberUpstreamRecorder()placeholder
...
func TestForwardAsChatCompletions_StreamCyberPolicyDropsResult(t *testing.T) {
	...
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	body := []byte(`{"model":"gpt-5.5","messages":[{"role":"user","content":"hi"placeholder],"stream":trueplaceholder`)
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/chat/completions", bytes.NewReader(body))
	c.Request.Header.Set("Content-Type", "application/json")
...
func TestForwardAsAnthropic_BufferedCyberPolicyNoFailover(t *testing.T) {
	...
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	body := []byte(`{"model":"gpt-5.5","max_tokens":1024,"messages":[{"role":"user","content":"hi"placeholder],"stream":falseplaceholder`)
...
func TestForwardAsAnthropic_StreamCyberPolicyNoFailover(t *testing.T) {
	...
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	body := []byte(`{"model":"gpt-5.5","max_tokens":1024,"messages":[{"role":"user","content":"hi"placeholder],"stream":trueplaceholder`)
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/messages", bytes.NewReader(body))
	c.Request.Header.Set("Content-Type", "application/json")
