...
placeholder{
		{
			...
			body:   `{"error":{"code":"new_sensitive","message":"image is sensitive"placeholderplaceholder`,
			want:   true,
	placeholder,
		{
			name:   "content policy violation code",
			status: http.StatusForbidden,
			body:   `{"response":{"error":{"code":"content_policy_violation"placeholderplaceholderplaceholder`,
			want:   true,
	placeholder,
		{
			name:   "cyber policy code",
			status: http.StatusForbidden,
			body:   `{"error":{"code":"cyber_policy","message":"request rejected"placeholderplaceholder`,
			want:   true,
	placeholder,
		{
			name:   "moderation feature unavailable",
			status: http.StatusForbidden,
			body:   `{"error":{"message":"The moderation feature is not available for this request"placeholderplaceholder`,
			want:   true,
	placeholder,
		{
			name:   "explicit prompt moderation rejection",
			status: http.StatusForbidden,
			body:   `{"error":{"message":"request rejected by content moderation"placeholderplaceholder`,
			want:   true,
	placeholder,
		{
			name:   "entitlement forbidden",
			status: http.StatusForbidden,
			body:   `{"error":{"message":"subscription required"placeholderplaceholder`,
			want:   false,
	placeholder,
		{
			name:   "account policy suspension is not request policy",
			status: http.StatusForbidden,
			body:   `{"error":{"message":"account suspended due to policy violation"placeholderplaceholder`,
			want:   false,
	placeholder,
		{
			name:   "structured account suspension overrides policy reason",
			status: http.StatusForbidden,
			body:   `{"error":{"code":"account_suspended","reason":"policy_violation","message":"account suspended due to policy violation"placeholderplaceholder`,
			want:   false,
	placeholder,
		{
			name:   "ambiguous policy violation code is not enough",
			status: http.StatusForbidden,
			body:   `{"error":{"code":"policy_violation","message":"policy violation"placeholderplaceholder`,
			want:   false,
	placeholder,
		{
			name:   "policy violation with request scoped message",
			status: http.StatusForbidden,
			body:   `{"error":{"code":"policy_violation","message":"request blocked by policy"placeholderplaceholder`,
			want:   true,
	placeholder,
		{
			name:   "permission-denied usage guidelines is request scoped",
			status: http.StatusForbidden,
			body:   `{"code":"permission-denied","error":"Content violates usage guidelines. "placeholder`,
			want:   true,
	placeholder,
		{
			name:   "permission-denied entitlement stays on the account path",
			status: http.StatusForbidden,
			body:   `{"code":"permission-denied","error":"Access to the chat endpoint is denied"placeholder`,
			want:   false,
	placeholder,
		{
			name:   "structured account code overrides usage guidelines phrase",
			status: http.StatusForbidden,
			body:   `{"error":{"code":"account_suspended","message":"Content violates usage guidelines."placeholderplaceholder`,
			want:   false,
	placeholder,
		{
			name:   "wrong status",
			status: http.StatusBadRequest,
			body:   `{"error":{"code":"new_sensitive"placeholderplaceholder`,
			want:   false,
	placeholder,
placeholder
...
func TestGrokContentPolicy403DoesNotMutateOrFailover(t *testing.T) {
	...
	account := &Account{ID: 4715, Platform: PlatformGrok, Type: AccountTypeOAuthplaceholder
	...
	resp := &http.Response{StatusCode: http.StatusForbidden, Header: http.Header{placeholderplaceholder
...
placeholder
			...
			"temp_unschedulable_rules": []any{map[string]any{
				...
				"keywords":         []any{"text is sensitive"placeholder,
...
func TestGrokContentPolicy403SharedErrorFallbackDoesNotMutate(t *testing.T) {
	gin.SetMode(gin.TestMode)
	body := []byte(`{"error":{"code":"content_filter","message":"prohibited content"placeholderplaceholder`)
	repo := &grokQuotaAccountRepo{placeholder
	svc := &OpenAIGatewayService{accountRepo: repoplaceholder
	account := &Account{
		ID:       4719,
...
func TestGrokContentPolicySSEErrorDoesNotMutateOrFailover(t *testing.T) {
	...
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		...
		Body: io.NopCloser(strings.NewReader(
			"data: {\"type\":\"error\",\"error\":{\"code\":\"new_sensitive\",\"message\":\"text is sensitive\"placeholderplaceholder\n\n",
		)),
placeholderplaceholder
	svc := &OpenAIGatewayService{accountRepo: repo, httpUpstream: upstreamplaceholder
	account := &Account{ID: 4721, Platform: PlatformGrok, Type: AccountTypeOAuth, Concurrency: 1placeholder
	recorder := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(recorder)
	c.Request = httptest.NewRequest(http.MethodGet, "/v1/responses", nil)
	payload := []byte(`{"type":"response.create","model":"grok-4.5","input":"hi"placeholder`)
	var writes [][]byte
...
func TestGrokPermissionDeniedContentRefusalDoesNotMutateOrFailover(t *testing.T) {
	repo := &grokQuotaAccountRepo{placeholder
	svc := &OpenAIGatewayService{accountRepo: repoplaceholder
	account := &Account{ID: 4785, Platform: PlatformGrok, Type: AccountTypeOAuthplaceholder
	body := []byte(`{"code":"permission-denied","error":"Content violates usage guidelines. "placeholder`)
...
func TestHandleGrokAccountUpstreamErrorEntitlement403KeepsDefaultCooldown(t *testing.T) {
	repo := &grokQuotaAccountRepo{placeholder
	svc := &OpenAIGatewayService{accountRepo: repoplaceholder
	account := &Account{ID: 4716, Platform: PlatformGrok, Type: AccountTypeOAuthplaceholder
	before := time.Now()
...
placeholder {
		...
		placeholder
			...
			placeholder,
		placeholder
			body := []byte(`{"error":{"message":"grok access or entitlement denied"placeholderplaceholder`)
...
placeholder
	...
	account := &Account{Type: AccountTypeAPIKey, Credentials: map[string]any{"pool_mode": trueplaceholderplaceholder
	...
	placeholder
				...
				"temp_unschedulable_rules": []any{
					map[string]any{
						"error_code":       float64(http.StatusForbidden),
						"keywords":         []any{"entitlement denied"placeholder,
	...
	placeholder
		...
		svc.handleGrokAccountUpstreamError(
			context.Background(), account, http.StatusForbidden, nil,
			[]byte(`{"error":{"message":"grok access or entitlement denied"placeholderplaceholder`),
		)
...
placeholder
			...
			"temp_unschedulable_rules": []any{
				map[string]any{
					"error_code":       float64(http.StatusForbidden),
					"keywords":         []any{"subscription"placeholder,
					"duration_minutes": float64(7),
			placeholder,
		placeholder,
	placeholder,
...
placeholder
			...
			"temp_unschedulable_rules": []any{
				map[string]any{
					"error_code":       float64(http.StatusForbidden),
					"keywords":         []any{"different failure"placeholder,
					"duration_minutes": float64(7),
			placeholder,
		placeholder,
	placeholder,
