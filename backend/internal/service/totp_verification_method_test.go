...
placeholder
	userRepo := &totpVMUserRepoStub{user: userplaceholder
	values := map[string]string{placeholder
	if emailVerifyEnabled {
		values[SettingKeyEmailVerifyEnabled] = "true"
placeholder
	settingSvc := NewSettingService(&totpVMSettingRepoStub{values: valuesplaceholder, nil)
...
func TestGetVerificationMethodAdminAlwaysPassword(t *testing.T) {
	admin := &User{ID: 1, Email: "admin@example.com", Role: RoleAdminplaceholder
...
func TestGetVerificationMethodRegularUserFollowsEmailVerifySetting(t *testing.T) {
	user := &User{ID: 2, Email: "user@example.com", Role: RoleUserplaceholder
...
func TestTotpDisableAdminUsesPasswordEvenWithEmailVerifyEnabled(t *testing.T) {
	admin := &User{ID: 1, Email: "admin@example.com", Role: RoleAdmin, TotpEnabled: trueplaceholder
...
func TestTotpDisableRegularUserStillRequiresEmailCode(t *testing.T) {
	user := &User{ID: 2, Email: "user@example.com", Role: RoleUser, TotpEnabled: trueplaceholder
