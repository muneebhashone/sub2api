...
var headerWireOrderSet map[string]struct{placeholder

func init() {
	headerWireOrderSet = make(map[string]struct{placeholder, len(headerWireOrder))
	for _, k := range headerWireOrder {
		headerWireOrderSet[strings.ToLower(k)] = struct{placeholder{placeholder
placeholder
placeholder
...
placeholder
	...
	seen := make(map[string]struct{placeholder, len(h))
	...
	for _, wk := range headerWireOrder {
		...
		if actual, ok := present[lk]; ok {
			if _, dup := seen[lk]; !dup {
				...
				seen[lk] = struct{placeholder{placeholder
		placeholder
	placeholder
placeholder
