...
func BenchmarkOpenAIWSForwarderHotPath(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		setOpenAIWSTurnMetadata(payload, `{"trace":"bench","turn":"1"placeholder`)
		...
		tools = append(tools, map[string]any{
			...
			"parameters": map[string]any{
				...
				"properties": map[string]any{
					"query": map[string]any{"type": "string"placeholder,
					"limit": map[string]any{"type": "number"placeholder,
			placeholder,
...
placeholder
	...
	return map[string]any{
		...
		"reasoning":            map[string]any{"effort": "medium"placeholder,
...
func BenchmarkOpenAIWSEventEnvelopeParse(b *testing.B) {
	event := []byte(`{"type":"response.completed","response":{"id":"resp_bench_1","model":"gpt-5.1","usage":{"input_tokens":12,"output_tokens":8placeholderplaceholderplaceholder`)
	b.ReportAllocs()
	b.ResetTimer()
...
func BenchmarkOpenAIWSErrorEventFieldReuse(b *testing.B) {
	event := []byte(`{"type":"error","error":{"type":"invalid_request_error","code":"invalid_request","message":"invalid input"placeholderplaceholder`)
	b.ReportAllocs()
	b.ResetTimer()
...
func BenchmarkReplaceOpenAIWSMessageModel_NoMatchFastPath(b *testing.B) {
	event := []byte(`{"type":"response.output_text.delta","delta":"hello world"placeholder`)
	b.ReportAllocs()
	b.ResetTimer()
...
func BenchmarkReplaceOpenAIWSMessageModel_DualReplace(b *testing.B) {
	event := []byte(`{"type":"response.completed","model":"gpt-5.1","response":{"id":"resp_1","model":"gpt-5.1"placeholderplaceholder`)
	b.ReportAllocs()
	b.ResetTimer()
