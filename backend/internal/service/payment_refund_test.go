...
func TestPrepDeductBalanceRequiresForceWhenBalanceIsInsufficient(t *testing.T) {
	for _, tc := range []struct {
		...
		wantWarning bool
placeholder{
		{name: "insufficient balance", balance: 40, wantWarning: trueplaceholder,
		{name: "forced insufficient balance", balance: 40, force: true, wantDeduct: 40placeholder,
		{name: "equal balance", balance: 100, wantDeduct: 100placeholder,
placeholder {
		t.Run(tc.name, func(t *testing.T) {
			plan := &RefundPlan{RefundAmount: 100placeholder
			svc := &PaymentService{userRepo: &mockUserRepo{getByIDUser: &User{Balance: tc.balanceplaceholderplaceholderplaceholder

			result := svc.prepDeduct(context.Background(), &dbent.PaymentOrder{
				UserID:    1,
				OrderType: payment.OrderTypeBalance,
		placeholder, plan, tc.force)
...
placeholder
	...
	result, err := (&PaymentService{entClient: client, userRepo: repoplaceholder).ExecuteRefund(ctx, plan)
placeholder
...
placeholder
	...
	userRepo := &mockUserRepo{placeholder
...
placeholder
	...
	result, err := svc.finishRefund(ctx, plan, &payment.RefundResponse{Status: payment.ProviderStatusPendingplaceholder)
placeholder
	require.NotNil(t, result)
	require.False(t, result.Success)
...
func TestFinishRefundSuccessStatusesFinalize(t *testing.T) {
	for _, status := range []string{payment.ProviderStatusSuccess, payment.ProviderStatusRefundedplaceholder {
		...
		placeholder
			...
			result, err := svc.finishRefund(ctx, plan, &payment.RefundResponse{Status: statusplaceholder)
		placeholder
			require.NotNil(t, result)
			require.True(t, result.Success)
...
func TestQueryAndFinalizeRefundFinalizesProviderStatuses(t *testing.T) {
	for _, tc := range []struct {
		...
		available  float64
placeholder{
		{name: "success", status: payment.ProviderStatusSuccess, wantStatus: OrderStatusRefunded, wantDeduct: 100, available: 100placeholder,
		{name: "success clamps current balance", status: payment.ProviderStatusSuccess, wantStatus: OrderStatusRefunded, wantDeduct: 35, available: 35placeholder,
		{name: "failed", status: payment.ProviderStatusFailed, wantStatus: OrderStatusRefundFailedplaceholder,
		{name: "pending", status: payment.ProviderStatusPending, wantStatus: OrderStatusRefundPendingplaceholder,
placeholder {
		t.Run(tc.name, func(t *testing.T) {
			ctx := context.Background()
			client := newPaymentConfigServiceTestClient(t)
			...
			svc := &PaymentService{
				entClient:    client,
				loadBalancer: &captureLoadBalancer{placeholder,
		...
		placeholder
			restore := replacePaymentProviderFactoryForTest(t, &refundQueryProviderTestDouble{
				refundResponse: &payment.RefundResponse{RefundID: "rf_test", Status: tc.statusplaceholder,
		placeholder)
			defer restore()
...
func TestQueryAndFinalizeRefundUnsupportedProviderReturnsClearError(t *testing.T) {
	...
	svc := &PaymentService{entClient: client, loadBalancer: &captureLoadBalancer{placeholderplaceholder
	restore := replacePaymentProviderFactoryForTest(t, refundProviderTestDouble{placeholder)
	defer restore()
...
placeholder
	...
	_, err = client.PaymentAuditLog.Create().
		...
		SetOperator("admin").
		SetDetail(`{"refundID":"rf_test","deductionRollbackOK":trueplaceholder`).
		Save(ctx)
placeholder
	return order
placeholder
...
placeholder
	...
	createPaymentProviderFromInstance = func(providerKey, instanceID string, config map[string]string) (payment.Provider, error) {
		return prov, nil
placeholder
	return func() { createPaymentProviderFromInstance = original placeholder
placeholder

type refundProviderTestDouble struct{placeholder

func (refundProviderTestDouble) Name() string { return "refund-test" placeholder
