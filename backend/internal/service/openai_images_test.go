...
func (b *openAIImagesReadErrorBody) Read([]byte) (int, error) { return 0, b.err placeholder
func (b *openAIImagesReadErrorBody) Close() error             { return nil placeholder
...
func TestOpenAIImagesRequestModerationBody_JSONEditIncludesInputImageURLs(t *testing.T) {
	parsed := &OpenAIImagesRequest{
		...
		InputImageURLs: []string{"https://example.com/source.png"placeholder,
...
placeholder
	...
	require.Equal(t, []string{"https://example.com/source.png", "https://example.com/mask.png"placeholder, input.Images)
placeholder
...
func TestOpenAIGatewayServiceParseOpenAIImagesRequest_NormalizesOfficialAndCustomSizes(t *testing.T) {
	...
	tests := []struct {
		...
		wantTier string
placeholder{
		{size: "1024x1024", wantTier: "1K"placeholder,
		{size: "1536x1024", wantTier: "2K"placeholder,
		{size: "1024x1536", wantTier: "2K"placeholder,
		{size: "2048x2048", wantTier: "2K"placeholder,
...
placeholder
	...
	for _, tt := range tests {
		t.Run(tt.size, func(t *testing.T) {
			body := []byte(`{"model":"gpt-image-2","prompt":"draw a cat","size":"` + tt.size + `"placeholder`)
...
func TestOpenAIGatewayServiceParseOpenAIImagesRequest_LegacyImageModelUnknownSizePassthrough(t *testing.T) {
	gin.SetMode(gin.TestMode)
	body := []byte(`{"model":"gpt-image-1.5","prompt":"draw a cat","size":"2048x1152"placeholder`)
...
func TestOpenAIGatewayServiceParseOpenAIImagesRequest_AllowsGrokImageModels(t *testing.T) {
	...
	for _, model := range []string{"grok-imagine", "grok-imagine-image", "grok-imagine-image-quality", "grok-imagine-edit"placeholder {
		t.Run(model, func(t *testing.T) {
			body := []byte(fmt.Sprintf(`{"model":%q,"prompt":"draw a cat","response_format":"b64_json"placeholder`, model))
			req := httptest.NewRequest(http.MethodPost, "/v1/images/generations", bytes.NewReader(body))
			req.Header.Set("Content-Type", "application/json")
			rec := httptest.NewRecorder()
			c, _ := gin.CreateTestContext(rec)
...
func TestOpenAIGatewayServiceParseOpenAIImagesRequest_JSONEditURLs(t *testing.T) {
	...
	body := []byte(`{
		...
		"images":[{"image_url":"https://example.com/source.png"placeholder],
...
placeholder
	require.NotNil(t, parsed)
	require.Equal(t, []string{"https://example.com/source.png"placeholder, parsed.InputImageURLs)
...
placeholder, openAIUpstreamErrorBodyReadLimit)
placeholder
...
func TestNewOpenAIImageStatusError_UsesProvidedReadLimit(t *testing.T) {
	...
	body := fmt.Sprintf(`{"error":{"padding":"%s","message":"diagnostic-marker"placeholderplaceholder`, padding)
...
placeholder)
	...
	t.Run("alpha search 允许 OpenAI OAuth/PAT 与 APIKey 账号，拒绝 Grok", func(t *testing.T) {
		// OAuth/PAT 走 chatgpt.com Codex 端点，APIKey 走 {base_urlplaceholder/v1/alpha/search，
...
placeholder)
	...
	t.Run("responses 能力：仍需通过 chat_completions 配置集校验", func(t *testing.T) {
		...
		account := &Account{
			...
			Type:     AccountTypeAPIKey,
	placeholder
				"openai_capabilities": []any{"embeddings"placeholder,
		placeholder,
	placeholder
...
func findOpenAIImageTestSSEEvent(events []openAIImageTestSSEEvent, name string) (openAIImageTestSSEEvent, bool) {
	...
	placeholder
placeholder
	return openAIImageTestSSEEvent{placeholder, false
placeholder

func TestOpenAIGatewayServiceForwardImages_OAuthPassesNAndReturnsAllImages(t *testing.T) {
	gin.SetMode(gin.TestMode)
	body := []byte(`{"model":"gpt-image-2","prompt":"draw a cat","size":"1024x1024","quality":"high","n":3placeholder`)
	...
	svc.httpUpstream = &httpUpstreamRecorder{
		resp: &http.Response{
			...
			Header: http.Header{
				...
				"X-Request-Id": []string{"req_img_badreq"placeholder,
		placeholder,
			Body: io.NopCloser(strings.NewReader(
				`{"error":{"message":"Invalid value for 'size': expected one of 1024x1024, 1536x1024.","type":"invalid_request_error","param":"size","code":"unknown_parameter"placeholderplaceholder`,
			)),
	placeholder,
placeholder
...
func TestOpenAIGatewayServiceForwardImages_OAuthNonStreamModerationBlockedReturnsClientError(t *testing.T) {
	gin.SetMode(gin.TestMode)
	body := []byte(`{"model":"gpt-image-2","prompt":"draw blocked image","response_format":"b64_json"placeholder`)
...
placeholder
	...
	svc.httpUpstream = &httpUpstreamRecorder{
		resp: &http.Response{
			...
			Header: http.Header{
				"Content-Type": []string{"text/event-stream"placeholder,
				"X-Request-Id": []string{"req_img_blocked"placeholder,
		placeholder,
			Body: io.NopCloser(strings.NewReader(
				"data: {\"type\":\"response.created\",\"response\":{\"created_at\":1710000020placeholderplaceholder\n\n" +
					"data: {\"type\":\"error\",\"error\":{\"type\":\"image_generation_user_error\",\"code\":\"moderation_blocked\",\"message\":\"Your request was rejected by the safety system. safety_violations=[sexual].\"placeholderplaceholder\n\n" +
					"data: {\"type\":\"response.failed\",\"response\":{\"id\":\"resp_blocked\",\"status\":\"failed\",\"error\":{\"type\":\"image_generation_user_error\",\"code\":\"moderation_blocked\",\"message\":\"Your request was rejected by the safety system. safety_violations=[sexual].\"placeholderplaceholderplaceholder\n\n",
			)),
	placeholder,
placeholder
	...
	svc := &OpenAIGatewayService{
		httpUpstream: &httpUpstreamRecorder{
			resp: &http.Response{
				...
				Header: http.Header{
					"Content-Type": []string{"text/event-stream"placeholder,
					"X-Request-Id": []string{"req_img_server_error"placeholder,
			placeholder,
				Body: io.NopCloser(strings.NewReader(
...
func TestOpenAIImagesOAuthBodyReadTransportErrorFailover(t *testing.T) {
	...
	resp := &http.Response{
		StatusCode: http.StatusOK,
		Header: http.Header{
			"X-Request-Id": []string{"req_h2_read_failure"placeholder,
			"X-Upstream":   []string{"preserved"placeholder,
	placeholder,
		Body: &openAIImagesReadErrorBody{err: errors.New("stream error: stream ID 11; INTERNAL_ERROR; received from peer")placeholder,
placeholder
	account := &Account{ID: 5400, Name: "openai-oauth", Platform: PlatformOpenAI, Type: AccountTypeOAuthplaceholder
	svc := &OpenAIGatewayService{placeholder
	...
	require.ErrorAs(t, err, &failoverErr)
	require.Equal(t, http.StatusBadGateway, failoverErr.StatusCode)
	require.JSONEq(t, `{"error":{"type":"upstream_error","code":"upstream_http2_stream_error","message":"Upstream HTTP/2 stream failed"placeholderplaceholder`, string(failoverErr.ResponseBody))
...
func TestOpenAIImagesOAuthBodyReadErrorsNotMisclassified(t *testing.T) {
	tests := []struct {
		...
		err  error
placeholder{
		{name: "context canceled", err: context.Canceledplaceholder,
		{name: "response too large", err: fmt.Errorf("%w: limit=1", ErrUpstreamResponseBodyTooLarge)placeholder,
		{name: "semantic error", err: &OpenAIImagesUpstreamError{StatusCode: http.StatusBadRequest, ErrorType: "invalid_request_error", Code: "invalid_value", Message: "bad image request"placeholderplaceholder,
placeholder
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			c, _ := gin.CreateTestContext(rec)
			c.Request = httptest.NewRequest(http.MethodPost, "/v1/images/generations", nil)
			resp := &http.Response{StatusCode: http.StatusOK, Header: make(http.Header)placeholder
			err := tt.err
		...
		placeholder
			...
			got := (&OpenAIGatewayService{placeholder).handleOpenAIImagesOAuthResponseError(context.Background(), c, &Account{Platform: PlatformOpenAIplaceholder, "gpt-image-2", "", resp, OpenAIImagesJSONKeepaliveAdjustedWrittenSize(c), err)
			var failoverErr *UpstreamFailoverError
...
func TestOpenAIImagesOAuthTransportErrorAfterDownstreamWriteDoesNotFailover(t *testing.T) {
	...
	account := &Account{ID: 5401, Name: "openai-oauth", Platform: PlatformOpenAI, Type: AccountTypeOAuthplaceholder
	resp := &http.Response{Header: http.Header{"X-Request-Id": []string{"req_after_write"placeholderplaceholderplaceholder

	err := (&OpenAIGatewayService{placeholder).handleOpenAIImagesOAuthResponseError(context.Background(), c, account, "gpt-image-2", "", resp, before, classifiedErr)
...
func TestShouldClassifyOpenAIUpstreamStreamReadErrorTransportStrings(t *testing.T) {
	for _, message := range []string{"unexpected EOF", "connection reset by peer", "broken pipe", "use of closed network connection"placeholder {
	...
	svc := &OpenAIGatewayService{
		httpUpstream: &httpUpstreamRecorder{
			resp: &http.Response{
				...
				Header: http.Header{
					"Content-Type": []string{"text/event-stream"placeholder,
					"X-Request-Id": []string{"req_img_server_error_after_partial"placeholder,
			placeholder,
				Body: io.NopCloser(strings.NewReader(
					"data: {\"type\":\"response.image_generation_call.partial_image\",\"partial_image_b64\":\"cGFydGlhbA==\",\"partial_image_index\":0,\"output_format\":\"png\"placeholder\n\n" +
						"data: {\"type\":\"error\",\"error\":{\"type\":\"server_error\",\"code\":\"server_error\",\"message\":\"The image service failed after partial output.\"placeholderplaceholder\n\n",
				)),
		placeholder,
	placeholder,
placeholder
...
placeholder{
		{
			...
			payload:    `{"type":"error","error":{"type":"invalid_request_error","code":"invalid_value","message":"bad size"placeholderplaceholder`,
			wantStatus: http.StatusBadRequest,
	placeholder,
		{
			name:       "content policy",
			payload:    `{"type":"error","error":{"type":"image_generation_user_error","code":"content_policy_violation","message":"blocked"placeholderplaceholder`,
			wantStatus: http.StatusBadRequest,
	placeholder,
		{
			name:       "rate limit remains distinct from server error",
			payload:    `{"type":"error","error":{"type":"rate_limit_exceeded","code":"rate_limit_exceeded","message":"try again"placeholderplaceholder`,
	...
	svc := &OpenAIGatewayService{
		...
		httpUpstream: &httpUpstreamRecorder{
			resp: &http.Response{
				...
				Header: http.Header{
					"Content-Type": []string{"application/json"placeholder,
					"X-Request-Id": []string{"req_img_stream_json"placeholder,
			placeholder,
				Body: io.NopCloser(strings.NewReader(`{"created":1710000008,"usage":{"input_tokens":12,"output_tokens":21,"output_tokens_details":{"image_tokens":9placeholderplaceholder,"data":[{"b64_json":"aGVsbG8=","revised_prompt":"draw a cat"placeholder]placeholder`)),
		placeholder,
	placeholder,
placeholder
	parsed, err := svc.ParseOpenAIImagesRequest(c, body)
	...
	svc := &OpenAIGatewayService{
		...
		httpUpstream: &httpUpstreamRecorder{
			resp: &http.Response{
				...
				Header: http.Header{
					"Content-Type": []string{"text/event-stream"placeholder,
					"X-Request-Id": []string{"req_img_stream_json_mislabeled"placeholder,
			placeholder,
				Body: io.NopCloser(strings.NewReader(`{"created":1710000009,"usage":{"input_tokens":10,"output_tokens":18,"output_tokens_details":{"image_tokens":8placeholderplaceholder,"data":[{"b64_json":"ZmluYWw="placeholder]placeholder`)),
		placeholder,
	placeholder,
placeholder
	parsed, err := svc.ParseOpenAIImagesRequest(c, body)
	...
	svc := &OpenAIGatewayService{
		...
		httpUpstream: &httpUpstreamRecorder{
			resp: &http.Response{
				...
				Header: http.Header{
					"Content-Type": []string{"text/event-stream"placeholder,
					"X-Request-Id": []string{"req_img_stream_multiline"placeholder,
			placeholder,
				Body: io.NopCloser(strings.NewReader(
					"data: {\"type\":\"image_generation.completed\",\n" +
						"data: \"usage\":{\"input_tokens\":10,\"output_tokens\":18,\"output_tokens_details\":{\"image_tokens\":8placeholderplaceholder,\n" +
						"data: \"b64_json\":\"ZmluYWw=\",\"output_format\":\"png\"placeholder\n\n" +
						"data: [DONE]\n\n",
				)),
		placeholder,
	placeholder,
...
func TestExtractOpenAIImagesBillableCountFromJSONBytes_CompletedEvent(t *testing.T) {
	body := []byte(`{"type":"image_generation.completed","b64_json":"ZmluYWw=","usage":{"input_tokens":10,"output_tokens":18placeholderplaceholder`)
...
func TestOpenAIGatewayServiceForwardImages_OAuthStreamingTransformsEvents(t *testing.T) {
	gin.SetMode(gin.TestMode)
	body := []byte(`{"model":"gpt-image-2","prompt":"draw a cat","stream":true,"response_format":"url"placeholder`)
...
placeholder
	...
	upstream := &httpUpstreamRecorder{
		resp: &http.Response{
			...
			Header: http.Header{
				"Content-Type": []string{"text/event-stream"placeholder,
				"X-Request-Id": []string{"req_img_stream"placeholder,
		placeholder,
			Body: io.NopCloser(strings.NewReader(
...
func TestOpenAIGatewayServiceForwardImages_APIKeyStreamingDrainsAfterClientDisconnect(t *testing.T) {
	gin.SetMode(gin.TestMode)
	body := []byte(`{"model":"gpt-image-2","prompt":"draw a cat","stream":trueplaceholder`)
	...
	c, _ := gin.CreateTestContext(rec)
	c.Request = req
	c.Writer = &failingOpenAIImageWriter{ResponseWriter: c.Writer, failAfter: 1placeholder
	...
	placeholder,
		httpUpstream: &httpUpstreamRecorder{
			resp: &http.Response{
				...
				Header: http.Header{
					"Content-Type": []string{"text/event-stream"placeholder,
					"X-Request-Id": []string{"req_img_stream_disconnect_apikey"placeholder,
			placeholder,
				Body: io.NopCloser(strings.NewReader(
					"data: {\"type\":\"image_generation.partial_image\",\"b64_json\":\"cGFydGlhbA==\"placeholder\n\n" +
						"data: {\"type\":\"image_generation.completed\",\"usage\":{\"input_tokens\":3,\"output_tokens\":4,\"output_tokens_details\":{\"image_tokens\":2placeholderplaceholder,\"b64_json\":\"ZmluYWw=\",\"output_format\":\"png\"placeholder\n\n" +
						"data: [DONE]\n\n",
				)),
		placeholder,
	placeholder,
...
placeholder
	...
	c, _ := gin.CreateTestContext(rec)
	c.Request = req
	c.Set("api_key", &APIKey{ID: 100placeholder)
...
placeholder
	...
	upstream := &httpUpstreamRecorder{
		resp: &http.Response{
			...
			Header: http.Header{
				"Content-Type": []string{"text/event-stream"placeholder,
				"X-Request-Id": []string{"req_img_edit_123"placeholder,
		placeholder,
			Body: io.NopCloser(strings.NewReader(
				"data: {\"type\":\"response.completed\",\"response\":{\"created_at\":1710000002,\"usage\":{\"input_tokens\":13,\"output_tokens\":21,\"output_tokens_details\":{\"image_tokens\":8placeholderplaceholder,\"tool_usage\":{\"image_gen\":{\"images\":1placeholderplaceholder,\"output\":[{\"type\":\"image_generation_call\",\"result\":\"ZWRpdGVk\",\"revised_prompt\":\"replace background with aurora\",\"output_format\":\"webp\",\"quality\":\"high\"placeholder]placeholderplaceholder\n\n" +
					"data: [DONE]\n\n",
			)),
	placeholder,
placeholder
...
placeholder
	...
	upstream := &httpUpstreamRecorder{
		...
		placeholder,
			Body: io.NopCloser(strings.NewReader(
				"data: {\"type\":\"response.created\",\"response\":{\"created_at\":1710000003,\"tools\":[{\"type\":\"image_generation\",\"model\":\"gpt-image-2\",\"background\":\"transparent\",\"output_format\":\"webp\",\"quality\":\"high\",\"size\":\"1024x1024\"placeholder]placeholderplaceholder\n\n" +
					"data: {\"type\":\"response.image_generation_call.partial_image\",\"partial_image_b64\":\"cGFydGlhbA==\",\"partial_image_index\":0,\"output_format\":\"webp\",\"background\":\"transparent\"placeholder\n\n" +
					"data: {\"type\":\"response.completed\",\"response\":{\"created_at\":1710000003,\"usage\":{\"input_tokens\":7,\"output_tokens\":10,\"output_tokens_details\":{\"image_tokens\":5placeholderplaceholder,\"tool_usage\":{\"image_gen\":{\"images\":1placeholderplaceholder,\"tools\":[{\"type\":\"image_generation\",\"model\":\"gpt-image-2\",\"background\":\"transparent\",\"output_format\":\"webp\",\"quality\":\"high\",\"size\":\"1024x1024\"placeholder],\"output\":[{\"type\":\"image_generation_call\",\"result\":\"ZWRpdGVk\",\"revised_prompt\":\"replace background with aurora\",\"output_format\":\"webp\"placeholder]placeholderplaceholder\n\n" +
					"data: [DONE]\n\n",
			)),
	placeholder,
placeholder
...
func TestCollectOpenAIImagesFromResponsesBody_FallsBackToOutputItemDone(t *testing.T) {
	body := []byte(
		"data: {\"type\":\"response.created\",\"response\":{\"created_at\":1710000004placeholderplaceholder\n\n" +
			"data: {\"type\":\"response.output_item.done\",\"item\":{\"id\":\"ig_123\",\"type\":\"image_generation_call\",\"result\":\"aGVsbG8=\",\"revised_prompt\":\"draw a cat\",\"output_format\":\"png\",\"quality\":\"high\"placeholderplaceholder\n\n" +
			"data: {\"type\":\"response.completed\",\"response\":{\"created_at\":1710000004,\"tool_usage\":{\"image_gen\":{\"images\":1placeholderplaceholder,\"output\":[]placeholderplaceholder\n\n" +
			"data: [DONE]\n\n",
	)
...
placeholder
	...
	require.JSONEq(t, `{"images":1placeholder`, string(usageRaw))
placeholder

func TestCollectOpenAIImagesFromResponsesBody_MultilineSSE(t *testing.T) {
	body := []byte(
		"data: {\"type\":\"response.completed\",\n" +
			"data: \"response\":{\"created_at\":1710000010,\"usage\":{\"input_tokens\":5,\"output_tokens\":9,\"output_tokens_details\":{\"image_tokens\":4placeholderplaceholder,\"tool_usage\":{\"image_gen\":{\"images\":1placeholderplaceholder,\"output\":[{\"type\":\"image_generation_call\",\"result\":\"ZmluYWw=\",\"output_format\":\"png\"placeholder]placeholderplaceholder\n\n" +
			"data: [DONE]\n\n",
	)
...
placeholder
	...
	upstream := &httpUpstreamRecorder{
		resp: &http.Response{
			...
			Header: http.Header{
				"Content-Type": []string{"text/event-stream"placeholder,
				"X-Request-Id": []string{"req_img_stream_output_item_done"placeholder,
		placeholder,
			Body: io.NopCloser(strings.NewReader(
				"data: {\"type\":\"response.output_item.done\",\"item\":{\"id\":\"ig_123\",\"type\":\"image_generation_call\",\"result\":\"ZmluYWw=\",\"revised_prompt\":\"draw a cat\",\"output_format\":\"png\"placeholderplaceholder\n\n" +
					"data: {\"type\":\"response.completed\",\"response\":{\"created_at\":1710000005,\"usage\":{\"input_tokens\":5,\"output_tokens\":9,\"output_tokens_details\":{\"image_tokens\":4placeholderplaceholder,\"tool_usage\":{\"image_gen\":{\"images\":1placeholderplaceholder,\"output\":[]placeholderplaceholder\n\n" +
					"data: [DONE]\n\n",
			)),
	placeholder,
placeholder
...
placeholder
	...
	svc.httpUpstream = &httpUpstreamRecorder{
		resp: &http.Response{
			...
			Header: http.Header{
				"Content-Type": []string{"text/event-stream"placeholder,
				"X-Request-Id": []string{"req_img_stream_multiline_oauth"placeholder,
		placeholder,
			Body: io.NopCloser(strings.NewReader(
				"data: {\"type\":\"response.completed\",\n" +
					"data: \"response\":{\"created_at\":1710000011,\"usage\":{\"input_tokens\":6,\"output_tokens\":10,\"output_tokens_details\":{\"image_tokens\":5placeholderplaceholder,\"tool_usage\":{\"image_gen\":{\"images\":1placeholderplaceholder,\"output\":[{\"type\":\"image_generation_call\",\"result\":\"TXVsdGlsaW5l\",\"output_format\":\"png\"placeholder]placeholderplaceholder\n\n" +
					"data: [DONE]\n\n",
			)),
	placeholder,
placeholder
...
placeholder
	...
	upstream := &httpUpstreamRecorder{
		resp: &http.Response{
			...
			Header: http.Header{
				"Content-Type": []string{"text/event-stream"placeholder,
				"X-Request-Id": []string{"req_img_stream_disconnect_oauth"placeholder,
		placeholder,
			Body: io.NopCloser(strings.NewReader(
				"data: {\"type\":\"response.image_generation_call.partial_image\",\"partial_image_b64\":\"cGFydGlhbA==\",\"partial_image_index\":0,\"output_format\":\"png\"placeholder\n\n" +
					"data: {\"type\":\"response.completed\",\"response\":{\"created_at\":1710000009,\"usage\":{\"input_tokens\":5,\"output_tokens\":9,\"output_tokens_details\":{\"image_tokens\":4placeholderplaceholder,\"tool_usage\":{\"image_gen\":{\"images\":1placeholderplaceholder,\"output\":[{\"type\":\"image_generation_call\",\"result\":\"ZmluYWw=\",\"output_format\":\"png\"placeholder]placeholderplaceholder\n\n" +
					"data: [DONE]\n\n",
			)),
	placeholder,
placeholder
