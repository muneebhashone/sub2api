...
placeholder, 5*time.Second, time.Millisecond, "all callers must miss the cache before the loader is released")
	close(unblock)
	wg.Wait()
	...
	accounts := []Account{
		{
			...
			Extra:              map[string]any{"window_cost_limit": 100.0placeholder,
...
placeholder
	repo := &usageLogWindowBatchRepoStub{
		batchResult: map[int64]*usagestats.AccountStats{
			2: {StandardCost: 22.0placeholder,
	placeholder,
placeholder
	svc := &GatewayService{
...
placeholder
	...
	placeholder,
placeholder
	repo := &usageLogWindowBatchRepoStub{placeholder
	svc := &GatewayService{
		sessionLimitCache: cache,
		usageLogRepo:      repo,
placeholder
...
placeholder
	...
	cache := &sessionLimitCacheHotpathStub{placeholder
	repo := &usageLogWindowBatchRepoStub{
		batchErr: errors.New("batch failed"),
		singleResult: map[int64]*usagestats.AccountStats{
			2: {StandardCost: 33.0placeholder,
	placeholder,
placeholder
	svc := &GatewayService{
		sessionLimitCache: cache,
...
placeholder
	...
	require.Equal(t, []string{"claude-3-5-haiku", "claude-3-5-sonnet"placeholder, models1)
...
placeholder
	...
	require.Equal(t, []string{"claude-3-5-haiku", "claude-3-5-sonnet"placeholder, models3)
	require.Equal(t, int64(1), repo.listByGroupCalls.Load())
	...
	require.Equal(t, []string{"claude-3-7-sonnet"placeholder, models4)
...
placeholder
	...
	require.Equal(t, []string{"claude-3-5-sonnet", "gemini-2.5-pro"placeholder, models)
...
placeholder{
		{
			...
			placeholder"model_mapping": map[string]any{"stale-model": "upstream-model"placeholderplaceholder,
			...
			accounts: []Account{
				{
					ID:          2,
					Platform:    PlatformOpenAI,
			placeholder"model_mapping": map[string]any{"configured-model": "configured-upstream"placeholderplaceholder,
			placeholder,
				{
					ID:          3,
					Platform:    PlatformOpenAI,
	...
	placeholder,
		{
			...
			placeholder,
		placeholder,
			want: []string{"configured-model"placeholder,
	placeholder,
placeholder

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			repo := &modelsListAccountRepoStub{byGroup: map[int64][]Account{groupID: tt.accountsplaceholderplaceholder
			svc := &GatewayService{
				accountRepo:        repo,
				modelsListCache:    gocache.New(time.Minute, time.Minute),
				modelsListCacheTTL: time.Minute,
			...
			placeholder,
				{
					ID:          2,
					Platform:    PlatformAnthropic,
			placeholder"model_mapping": map[string]any{"claude-mapped": "claude-upstream"placeholderplaceholder,
			placeholder,
		placeholder,
	placeholder,
placeholder
...
placeholder
	...
	require.Equal(t, []string{"claude-mapped"placeholder, svc.GetAvailableModels(context.Background(), &groupID, ""))
placeholder
...
placeholder
	group9 := int64(9)
	group10 := int64(10)
	svc.modelsListCache.Set(modelsListCacheKey(&group9, PlatformAnthropic), []string{"a"placeholder, time.Minute)
	svc.modelsListCache.Set(modelsListCacheKey(&group9, PlatformGemini), []string{"b"placeholder, time.Minute)
	svc.modelsListCache.Set(modelsListCacheKey(&group10, PlatformAnthropic), []string{"c"placeholder, time.Minute)
	svc.modelsListCache.Set("invalid-key", []string{"d"placeholder, time.Minute)
...
placeholder
	...
	repo := stubOpenAIAccountRepo{accounts: []Account{accountplaceholderplaceholder
	concurrency := NewConcurrencyService(stubConcurrencyCache{placeholder)
...
placeholder
	...
	t.Run("without_prefetch_reads_cache_once", func(t *testing.T) {
		cache := &stickyGatewayCacheHotpathStub{stickyID: account.IDplaceholder
		svc := &GatewayService{
			accountRepo:        repo,
			cache:              cache,
			cfg:                cfg,
