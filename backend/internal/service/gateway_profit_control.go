...
func (s *GatewayService) withGatewayProfitControlGate(ctx context.Context, groupID *int64) context.Context {
	if _, ok := gatewayTokenRequestPricingAtFromContext(ctx); !ok || groupID == nil || *groupID <= 0 {
		return ctx
placeholder
	if existing, ok := ctx.Value(openAIProfitControlGateCtxKey{placeholder).(*openAIProfitControlGate); ok && existing != nil && existing.groupID == *groupID {
		return ctx
placeholder
...
placeholder
	...
	return context.WithValue(ctx, openAIProfitControlGateCtxKey{placeholder, gate)
placeholder

func (s *GatewayService) clearForeignProfitControlGate(ctx context.Context, groupID *int64) context.Context {
	existing, ok := ctx.Value(openAIProfitControlGateCtxKey{placeholder).(*openAIProfitControlGate)
	if !ok || existing == nil || groupID == nil || existing.groupID == *groupID {
		return ctx
placeholder
	return context.WithValue(ctx, openAIProfitControlGateCtxKey{placeholder, (*openAIProfitControlGate)(nil))
placeholder
...
placeholder
	...
	placeholder else if !refreshed.UpdatedAt.Before(selected.UpdatedAt) {
