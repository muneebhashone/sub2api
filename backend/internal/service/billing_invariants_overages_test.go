...
placeholder{
		{
			...
			account: Account{Platform: PlatformAntigravity, Extra: map[string]any{"allow_overages": trueplaceholderplaceholder,
			want:    true,
	placeholder,
		{
			name:    "antigravity平台显式false拒绝",
			account: Account{Platform: PlatformAntigravity, Extra: map[string]any{"allow_overages": falseplaceholderplaceholder,
			want:    false,
	placeholder,
		{
			name:    "字段缺失默认拒绝",
	...
	placeholder,
		{
			name:    "非bool类型按拒绝处理",
			account: Account{Platform: PlatformAntigravity, Extra: map[string]any{"allow_overages": "true"placeholderplaceholder,
			want:    false,
	placeholder,
		{
			name:    "非antigravity平台即使为true也拒绝",
			account: Account{Platform: PlatformAnthropic, Extra: map[string]any{"allow_overages": trueplaceholderplaceholder,
			want:    false,
	placeholder,
placeholder
...
func TestBillingInvariant_OveragesDeniedNoCreditsInjection(t *testing.T) {
	quotaExhaustedBody := []byte(`{"error":{"status":"RESOURCE_EXHAUSTED","message":"QUOTA_EXHAUSTED"placeholderplaceholder`)
...
placeholder{
		{
			...
			extra: map[string]any{placeholder, // allow_overages 缺失 → 拒绝
	placeholder,
		{
...
placeholder
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			upstream := &mockSmartRetryUpstream{placeholder
			account := &Account{
				ID:       201,
				Name:     "acc-201",
		...
		placeholder
			params := antigravityRetryLoopParams{
				...
				accountRepo:    &stubAntigravityAccountRepo{placeholder,
		...
		placeholder
			...
			result := svc.handleSmartRetry(params, resp, quotaExhaustedBody, "https://ag-1.test", 0, []string{"https://ag-1.test"placeholder)
