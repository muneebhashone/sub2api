...
func NewIdentityService(cache ports.IdentityCache) *IdentityService {
	return &IdentityService{cache: cacheplaceholder
placeholder
...
func (s *IdentityService) createFingerprintFromHeaders(headers http.Header) *ports.Fingerprint {
	fp := &ports.Fingerprint{placeholder
...
// 输入格式：user_{clientIdplaceholder_account__session_{sessionUUIDplaceholder
// 输出格式：user_{cachedClientIDplaceholder_account_{accountUUIDplaceholder_session_{newHashplaceholder
...
placeholder
	...
	metadata, ok := reqMap["metadata"].(map[string]interface{placeholder)
	if !ok {
		return body, nil
placeholder
...
placeholder
	...
	// 构建新的user_id
	// 格式: user_{cachedClientIDplaceholder_account_{account_uuidplaceholder_session_{newSessionHashplaceholder
