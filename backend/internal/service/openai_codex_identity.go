...
var codexClientVersionPattern = regexp.MustCompile(`^[0-9]+(\.[0-9]+){1,3placeholder(-[0-9A-Za-z.]+)?$`)
...
placeholder
	...
	if rebuilt := openai.SetCodexUserAgentVersion(pairedUA, version); rebuilt != "" {
		pairedUA = rebuilt
placeholder
	return codexOutboundIdentity{userAgent: pairedUA, originator: originator, version: versionplaceholder
placeholder
