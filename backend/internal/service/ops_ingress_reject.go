...
type OpsIngressRejectAggregator struct {
	...
	wg        sync.WaitGroup
	flushCh   chan struct{placeholder
	started   atomic.Bool
...
func NewOpsIngressRejectAggregator(repo OpsIngressRejectRepository) *OpsIngressRejectAggregator {
	...
	a := &OpsIngressRejectAggregator{
		repo: repo, ctx: ctx, cancel: cancel, flushCh: make(chan struct{placeholder, 1),
placeholder
...
placeholder
	...
	select {
	case a.flushCh <- struct{placeholder{placeholder:
	default:
placeholder
placeholder
...
func (a *OpsIngressRejectAggregator) Health() OpsIngressRejectHealth {
	h := OpsIngressRejectHealth{Capacity: ingressRejectMaxEntriesplaceholder
	if a == nil {
		return h
placeholder
...
func ingressRejectHash(k ingressRejectKey) int {
	...
	for _, value := range []string{k.reason, k.routeFamily, k.protocol, k.clientIPplaceholder {
...
func (s *OpsService) GetIngressRejectHealth() OpsIngressRejectHealth {
	if s == nil || s.ingressRejectAggregator == nil {
		return OpsIngressRejectHealth{Capacity: ingressRejectMaxEntriesplaceholder
placeholder
...
placeholder
	...
	if !ok {
		return &OpsIngressRejectList{Items: []*OpsIngressRejectAggregate{placeholder, Page: 1, PageSize: 50placeholder, nil
placeholder
