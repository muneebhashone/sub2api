...
func TestUpstreamBillingProbeNonOpenAIWithoutBaseURLIsUnsupportedWithoutRequest(t *testing.T) {
	account := &Account{
		...
		Type:        AccountTypeAPIKey,
		Status:      StatusActive,
placeholder"api_key": "sk-grok-official"placeholder,
placeholder
	repo := &upstreamBillingProbeAccountRepo{accounts: map[int64]*Account{account.ID: accountplaceholderplaceholder
...
func TestUpstreamBillingProbeOfficialAPIBaseURLIsUnsupportedWithoutRequest(t *testing.T) {
	cases := []struct {
		...
		baseURL  string
placeholder{
		{PlatformAnthropic, "https://api.anthropic.com"placeholder,
		{PlatformAnthropic, "https://api.anthropic.com:443"placeholder,
		{PlatformAnthropic, "https://api.anthropic.com./"placeholder,
		{PlatformAnthropic, "HTTPS://API.ANTHROPIC.COM/"placeholder,
		{PlatformGemini, "https://generativelanguage.googleapis.com"placeholder,
		{PlatformAntigravity, "https://cloudcode-pa.googleapis.com"placeholder,
		{PlatformGrok, "https://api.x.ai/v1"placeholder,
		{PlatformGrok, "https://us-east-1.api.x.ai/v1"placeholder,
		{PlatformGrok, "https://eu-west-1.api.x.ai/v1"placeholder,
		{PlatformGrok, "https://cli-chat-proxy.grok.com/v1"placeholder,
		// 跨平台官方域同样必无该端点，一并拦截。
		{PlatformAnthropic, "https://api.x.ai/v1"placeholder,
		{PlatformGrok, "https://api.openai.com"placeholder,
placeholder
	for i, tc := range cases {
		account := &Account{
			...
			Type:        AccountTypeAPIKey,
			Status:      StatusActive,
	placeholder"api_key": "sk-official", "base_url": tc.baseURLplaceholder,
	placeholder
		repo := &upstreamBillingProbeAccountRepo{accounts: map[int64]*Account{account.ID: accountplaceholderplaceholder
		upstream := &httpUpstreamRecorder{placeholder
		svc := newUpstreamBillingProbeTestService(repo, upstream, &upstreamBillingProbeSettingRepo{placeholder)
...
func TestUpstreamBillingProbeSetAccountEnabledAcceptsGrokAPIKey(t *testing.T) {
	grokAPIKey := &Account{
		...
		Type:        AccountTypeAPIKey,
		Status:      StatusActive,
placeholder"api_key": "sk", "base_url": "https://relay.example"placeholder,
placeholder
	grokOAuth := &Account{ID: 152, Platform: PlatformGrok, Type: AccountTypeOAuth, Status: StatusActiveplaceholder
