...
placeholder
	...
	if details != nil {
		...
		if payload.RateLimitResetCredits == nil {
			payload.RateLimitResetCredits = &OpenAIRateLimitResetCredits{placeholder
	placeholder
...
placeholder
	...
	placeholder
		...
		resp, err := client.R().
			SetContext(callCtx).
			SetHeaders(headers).
			SetBody(map[string]string{"redeem_request_id": redeemRequestIDplaceholder).
			SetSuccessResult(&payload).
...
placeholder
	...
	snap := &OpenAICodexUsageSnapshot{placeholder
