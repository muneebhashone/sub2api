...
func (s *SettingService) IsUserErrorViewAllowed(ctx context.Context) bool {
	vals, err := s.settingRepo.GetMultiple(ctx, []string{SettingKeyAllowUserViewErrorRequestsplaceholder)
	if err != nil {
...
var legacyClaudeCodeCodexWhitelistEntry = openai.AllowedClientEntry{
	...
	UAContains: []string{"Claude Code/"placeholder,
placeholder
...
placeholder
	...
	s.codexRestrictionPolicyCache.Store(&cachedCodexRestrictionPolicy{expiresAt: 0placeholder)
	return nil
placeholder
...
placeholder
	...
	if v, err := s.settingRepo.GetValue(dbCtx, SettingKeyCodexCLIOnlyEngineFingerprintSignals); err == nil && strings.TrimSpace(v) != "" {
		return nil // 已配置/已迁移
placeholder else if err != nil && !errors.Is(err, ErrSettingNotFound) {
...
placeholder else if !errors.Is(err, ErrSettingNotFound) {
...
func normalizedCodexClientMarkers(markers []string) map[string]struct{placeholder {
	normalized := make(map[string]struct{placeholder, len(markers))
	for _, marker := range markers {
		marker = strings.TrimSpace(marker)
		if marker == "" {
			continue
	placeholder
		normalized[strings.ToLower(marker)] = struct{placeholder{placeholder
placeholder
	return normalized
placeholder
...
placeholder
	...
	placeholder
		...
		pol := CodexRestrictionPolicy{EngineFingerprintSignals: openai.DefaultEngineFingerprintSignalsplaceholder // 安全默认：默认种子指纹信号
...
placeholder)
	if pol, ok := result.(CodexRestrictionPolicy); ok {
		return pol
placeholder
	return CodexRestrictionPolicy{EngineFingerprintSignals: openai.DefaultEngineFingerprintSignalsplaceholder
placeholder
...
placeholder
	...
	if err := json.Unmarshal([]byte(trimmed), &entries); err != nil {
		return fmt.Errorf("must be empty or a valid JSON array of {originator, ua_containsplaceholder")
placeholder
	return nil
placeholder
...
func validateDefaultPlatformQuotaMap(m map[string]*DefaultPlatformQuotaSetting) error {
	...
	placeholder
		if pq == nil {
			continue
	placeholder
		for _, v := range []*float64{pq.DailyLimitUSD, pq.WeeklyLimitUSD, pq.MonthlyLimitUSDplaceholder {
...
placeholder{
		{"email", settings.Email.PlatformQuotasplaceholder,
		{"linuxdo", settings.LinuxDo.PlatformQuotasplaceholder,
		{"oidc", settings.OIDC.PlatformQuotasplaceholder,
		{"wechat", settings.WeChat.PlatformQuotasplaceholder,
		{"github", settings.GitHub.PlatformQuotasplaceholder,
		{"google", settings.Google.PlatformQuotasplaceholder,
		{"dingtalk", settings.DingTalk.PlatformQuotasplaceholder,
placeholder {
		if pgs.pq != nil {
...
placeholder
	...
	placeholder)
		...
		placeholder)
			return gatewayForwardingSettingsResult{fp: true, claudeOAuthSystemPromptInjection: true, rewriteMessageCacheControl: s.defaultRewriteMessageCacheControl(), clientDatelineNormalization: trueplaceholder, nil
	placeholder
		fp := true
...
placeholder)
	if r, ok := val.(gatewayForwardingSettingsResult); ok {
		return r
placeholder
	return gatewayForwardingSettingsResult{fp: true, claudeOAuthSystemPromptInjection: true, clientDatelineNormalization: trueplaceholder
placeholder
...
placeholder
	...
	settings := OpsOpenAIAccountQuotaAutoPauseSettings{placeholder
