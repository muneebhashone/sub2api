...
func TestBatchImageSettlementService_SettlesAndChargesSuccessfulImagesOnly(t *testing.T) {
	...
	usageLogs := &openAIRecordUsageLogRepoStub{placeholder
	svc := &BatchImageSettlementService{
		Repo: repo, BillingRepo: billing, Pricing: &fakeBatchImagePricingResolver{unitPrice: 0.25placeholder,
...
func TestBatchImageSettlementService_IdempotentAfterBillingCrash(t *testing.T) {
	...
	repo.jobs[job.BatchID] = job
	billing := &fakeBatchImageBillingRepo{alreadyApplied: map[string]bool{BatchImageCaptureRequestID(job.BatchID): trueplaceholderplaceholder
	svc := &BatchImageSettlementService{Repo: repo, BillingRepo: billing, Pricing: &fakeBatchImagePricingResolver{unitPrice: 0.25placeholderplaceholder
...
placeholder{
		...
		{name: "counts_exceed_item_count", mutate: func(j *BatchImageJob) { j.SuccessCount = 2; j.FailCount = 2; j.ItemCount = 3 placeholder, want: ErrBatchImageSettlementInvalidCountsplaceholder,
...
func TestBatchImageSettlementService_CostExceedingHoldDoesNotCharge(t *testing.T) {
	...
	repo.jobs[job.BatchID] = job
	billing := &fakeBatchImageBillingRepo{placeholder
	svc := &BatchImageSettlementService{Repo: repo, BillingRepo: billing, Pricing: &fakeBatchImagePricingResolver{unitPrice: 0.50placeholderplaceholder
...
func TestBatchImagePipelineProcessor_FailsAndReleasesAfterSettlementRetryLimit(t *testing.T) {
	...
	repo.jobs[job.BatchID] = job
	billing := &fakeBatchImageBillingRepo{captureErr: errors.New("temporary billing timeout")placeholder
	settlement := &BatchImageSettlementService{Repo: repo, BillingRepo: billing, Pricing: &fakeBatchImagePricingResolver{unitPrice: 0.25placeholderplaceholder
	processor := &BatchImagePipelineProcessor{
		ProviderProcessor: &BatchImageProviderProcessor{Repo: repo, ProviderRegistry: NewBatchImageProviderRegistry(&fakeProcessorProvider{placeholder), AccountResolver: &fakeBatchImageAccountResolver{account: &Account{placeholderplaceholderplaceholder,
		SettlementService: settlement,
...
func TestReleaseBatchImageBalanceHold_TreatsFingerprintConflictAsReleased(t *testing.T) {
	...
	billing := &fakeBatchImageBillingRepo{releaseErr: ErrUsageBillingRequestConflictplaceholder
...
placeholder
	if cmd != nil {
		...
		if _, ok := r.seen[cmd.RequestID]; ok || r.alreadyApplied[cmd.RequestID] {
			*calls = append(*calls, cmd)
			return &BatchImageBalanceHoldResult{Applied: falseplaceholder, nil
	placeholder
		r.seen[cmd.RequestID] = struct{placeholder{placeholder
placeholder
	*calls = append(*calls, cmd)
	return &BatchImageBalanceHoldResult{Applied: trueplaceholder, nil
placeholder
