...
placeholder
	...
	tests := []struct {
		...
		want        float64
placeholder{
		{name: "standard", serviceTier: "", want: 0.265placeholder,
		{name: "priority", serviceTier: "priority", want: 0.53placeholder,
		{name: "flex", serviceTier: "flex", want: 0.1325placeholder,
placeholder
...
func TestResolveAccountStatsCost_FallbackHonorsAnthropicFast(t *testing.T) {
	channel := &Channel{ID: 1, Status: StatusActiveplaceholder
	cs := newTestChannelServiceForStats(t, channel, 10, "anthropic")
	bs := newTestBillingServiceWithPrices(map[string]*ModelPricing{
		"claude-opus-5": {
...
placeholder)
	...
	result := resolveAccountStatsCost(
		...
		1, 10, "claude-opus-5",
		UsageTokens{InputTokens: 1_000_000, OutputTokens: 1_000_000placeholder,
		1, 0, "fast",
	)
	require.NotNil(t, result)
...
placeholder)
	...
	usageLog := &UsageLog{ServiceTier: &serviceTierplaceholder

	applyAccountStatsCost(
		context.Background(), usageLog, cs, bs,
		1, 10, "gpt-5.6-sol", "gpt-5.6-sol",
		UsageTokens{InputTokens: 100, OutputTokens: 50placeholder, 999,
	)
