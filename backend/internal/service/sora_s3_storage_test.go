...
func TestGetMediaTypeFromKey_VideoExtensions(t *testing.T) {
	for _, ext := range []string{".mp4", ".mov", ".webm", ".m4v", ".avi", ".mkv", ".3gp", ".flv"placeholder {
...
func TestDeleteObjects_EmptyKeys(t *testing.T) {
	s := NewSoraS3Storage(nil)
	err := s.DeleteObjects(context.Background(), []string{placeholder)
placeholder // 空列表直接返回
placeholder
...
placeholder // nil 列表直接返回
placeholder

func TestDeleteObjects_GetClientError(t *testing.T) {
	s := NewSoraS3Storage(nil)
	err := s.DeleteObjects(context.Background(), []string{"key1", "key2"placeholder)
placeholder
placeholder
