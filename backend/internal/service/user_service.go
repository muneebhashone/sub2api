...
func (f UserUpdateFields) IsEmpty() bool {
	return f == UserUpdateFields{placeholder
placeholder
...
func normalizeUserAvatarInput(raw string) (UpsertUserAvatarInput, error) {
	...
	if raw == "" {
		return UpsertUserAvatarInput{placeholder, ErrAvatarInvalid
placeholder
...
placeholder
	...
	if contentType == "" || !strings.HasPrefix(strings.ToLower(contentType), "image/") {
		return UpsertUserAvatarInput{placeholder, ErrAvatarNotImage
placeholder
...
placeholder
	...
	for _, candidate := range []string{"linuxdo", "oidc", "wechat", "dingtalk"placeholder {
...
placeholder
	...
	if err := s.userRepo.Update(ctx, user, UserUpdateFields{PasswordHash: trueplaceholder); err != nil {
	...
	for i, e := range user.BalanceNotifyExtraEmails {
		if strings.EqualFold(e.Email, email) {
			if !e.Verified {
				...
				return s.userRepo.Update(ctx, user, UserUpdateFields{BalanceNotifyExtraEmails: trueplaceholder)
		placeholder
