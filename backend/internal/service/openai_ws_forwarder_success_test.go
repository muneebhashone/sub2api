...
func TestOpenAIGatewayService_BuildOpenAIWSHeadersDeviceModePreservesNamespacedClientSessionIdentity(t *testing.T) {
	...
	account := newTestOAuthAccount(1300, map[string]any{codexFingerprintModeExtraKey: "device"placeholder)
...
func TestOpenAIGatewayService_Forward_WSv2_CodexFingerprintHandshakeBodyParityAndDefaultCacheKey(t *testing.T) {
	...
	captureConn := &openAIWSCaptureConn{
		events: [][]byte{
			[]byte(`{"type":"response.completed","response":{"id":"resp_ws_fingerprint","model":"gpt-5.2","usage":{"input_tokens":2,"output_tokens":1placeholderplaceholderplaceholder`),
	placeholder,
placeholder
	captureDialer := &openAIWSCaptureDialer{conn: captureConnplaceholder
	pool := newOpenAIWSConnPool(cfg)
...
placeholder)
	...
	body := []byte(`{"model":"gpt-5.2","stream":true,"prompt_cache_key":"body-session","client_metadata":{"session_id":"body-session","x-codex-turn-metadata":"{\"installation_id\":\"body-install\",\"session_id\":\"body-session\",\"thread_id\":\"body-thread\",\"turn_id\":\"body-turn\",\"window_id\":\"body-window\",\"sandbox\":\"seatbelt\"placeholder"placeholder,"input":[{"type":"input_text","text":"hi"placeholder]placeholder`)
	result, err := svc.Forward(context.Background(), c, account, body)
placeholder
	require.NotNil(t, result)
