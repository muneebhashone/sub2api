...
func TestIdempotencyCoordinator_RequireKey(t *testing.T) {
	...
	_, err := coordinator.Execute(context.Background(), IdempotencyExecuteOptions{
		...
		ActorScope: "admin:1",
		RequireKey: true,
		Payload:    map[string]any{"a": 1placeholder,
placeholder, func(ctx context.Context) (any, error) {
		return map[string]any{"ok": trueplaceholder, nil
placeholder)
placeholder
...
func TestIdempotencyCoordinator_ReplaySucceededResult(t *testing.T) {
	...
	exec := func(ctx context.Context) (any, error) {
		execCount++
		return map[string]any{"count": execCountplaceholder, nil
placeholder
...
func TestIdempotencyCoordinator_ReclaimExpiredSucceededRecord(t *testing.T) {
	...
	opts := IdempotencyExecuteOptions{
		...
		Payload:        map[string]any{"k": "v"placeholder,
placeholder
...
placeholder
	...
	_, err = coordinator.Execute(context.Background(), IdempotencyExecuteOptions{
		...
		RequireKey:     true,
		IdempotencyKey: "case-2",
		Payload:        map[string]any{"a": 2placeholder,
placeholder, func(ctx context.Context) (any, error) {
		return map[string]any{"ok": trueplaceholder, nil
placeholder)
placeholder
...
func TestIdempotencyCoordinator_ConcurrentSameKeySingleSideEffect(t *testing.T) {
	...
	opts := IdempotencyExecuteOptions{
		...
		Payload:        map[string]any{"v": 1placeholder,
placeholder
...
type failingIdempotencyRepo struct{placeholder
...
func TestIdempotencyCoordinator_StoreUnavailableMetrics(t *testing.T) {
	resetIdempotencyMetricsForTest()
	coordinator := NewIdempotencyCoordinator(failingIdempotencyRepo{placeholder, DefaultIdempotencyConfig())
...
placeholder
	...
	fp1, err := BuildIdempotencyFingerprint("", "", "", map[string]any{"a": 1placeholder)
placeholder
	require.NotEmpty(t, fp1)
	fp2, err := BuildIdempotencyFingerprint("POST", "/", "anonymous", map[string]any{"a": 1placeholder)
placeholder
	require.Equal(t, fp1, fp2)

	_, err = BuildIdempotencyFingerprint("POST", "/x", "u:1", map[string]any{"bad": make(chan int)placeholder)
placeholder
...
func TestRetryAfterSecondsFromErrorBranches(t *testing.T) {
	...
	err := ErrIdempotencyInProgress.WithMetadata(map[string]string{"retry_after": "12"placeholder)
	require.Equal(t, 12, RetryAfterSecondsFromError(err))

	err = ErrIdempotencyInProgress.WithMetadata(map[string]string{"retry_after": "bad"placeholder)
...
type noIDOwnerRepo struct{placeholder
...
func (noIDOwnerRepo) ExtendProcessingLock(context.Context, int64, string, time.Time, time.Time) (bool, error) {
	return false, nil
placeholder
func (noIDOwnerRepo) MarkSucceeded(context.Context, int64, int, string, time.Time) error { return nil placeholder
func (noIDOwnerRepo) MarkFailedRetryable(context.Context, int64, string, time.Time, time.Time) error {
	return nil
placeholder
func (noIDOwnerRepo) DeleteExpired(context.Context, time.Time, int) (int64, error) { return 0, nil placeholder
...
placeholder
	...
	coordinator = NewIdempotencyCoordinator(noIDOwnerRepo{placeholder, cfg)
	_, err = coordinator.Execute(context.Background(), IdempotencyExecuteOptions{
		Scope:          "scope-no-id",
		IdempotencyKey: "k3",
		Payload:        map[string]any{"a": 1placeholder,
...
func TestIdempotencyCoordinator_ConflictBranchesAndDecodeError(t *testing.T) {
	now := time.Now()
	fp, err := BuildIdempotencyFingerprint("POST", "/x", "u:1", map[string]any{"a": 1placeholder)
placeholder
	badBody := "{bad-json"
...
func TestIdempotencyCoordinator_MarkAndMarshalBranches(t *testing.T) {
	repo := &markBehaviorRepo{inMemoryIdempotencyRepo: *newInMemoryIdempotencyRepo()placeholder
	coordinator := NewIdempotencyCoordinator(repo, DefaultIdempotencyConfig())
...
placeholder
	...
	_, err = coordinator.Execute(context.Background(), IdempotencyExecuteOptions{
		...
		Payload:        map[string]any{"a": 1placeholder,
placeholder, func(ctx context.Context) (any, error) {
		return map[string]any{"bad": make(chan int)placeholder, nil
placeholder)
placeholder
	require.Equal(t, infraerrors.Code(ErrIdempotencyStoreUnavail), infraerrors.Code(err))
...
placeholder)
	...
	body, err := c.marshalStoredResponse(map[string]any{"long": "abcdefghijklmnopqrstuvwxyz"placeholder)
placeholder
