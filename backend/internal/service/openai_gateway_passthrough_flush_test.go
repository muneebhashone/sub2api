...
func TestOpenAIStreamingPassthroughBareErrorTerminatesBeforeDone(t *testing.T) {
	errorEvent := "event: error\n" +
		`data: {"type":"error","error":{"code":"content_policy","message":"request blocked by policy"placeholder,"usage":{"input_tokens":6,"output_tokens":0,"total_tokens":6placeholderplaceholder` + "\n\n"
...
placeholder
	...
	require.Equal(t, []int{len(body)placeholder, writer.flushBodyLengths)
	require.Equal(t, 6, result.usage.InputTokens)
	require.Zero(t, result.usage.OutputTokens)
placeholder
...
func TestOpenAIStreamingPassthroughBareErrorDrainsAuthoritativeFailedUsage(t *testing.T) {
	errorEvent := "event: error\n" +
		`data: {"type":"error","error":{"code":"content_policy","message":"request blocked by policy"placeholderplaceholder` + "\n\n"
	failedEvent := "event: response.failed\n" +
		`data: {"type":"response.failed","response":{"id":"resp_failed","usage":{"input_tokens":9,"output_tokens":2placeholder,"error":{"code":"content_policy","message":"request blocked by policy"placeholderplaceholderplaceholder` + "\n\n"
