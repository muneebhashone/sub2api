...
placeholder
	require.Equal(t, []string{"@example.com", "@foo.bar", "*.edu.cn"placeholder, settings.RegistrationEmailSuffixWhitelist)
placeholder
...
func TestSettingService_ChannelMonitorShowQuotaFailsClosed(t *testing.T) {
	...
	missingRuntime := NewSettingService(&settingPublicRepoStub{values: map[string]string{placeholderplaceholder, &config.Config{placeholder).GetChannelMonitorRuntime(context.Background())
	require.False(t, missingRuntime.ShowQuota)
	missingPublic, err := NewSettingService(&settingPublicRepoStub{values: map[string]string{placeholderplaceholder, &config.Config{placeholder).
		GetPublicSettings(context.Background())
placeholder
...
placeholderplaceholder, &config.Config{placeholder).GetChannelMonitorRuntime(context.Background())
	require.True(t, runtime.ShowQuota)

	for _, value := range []string{"false", "TRUE", "1", "yes", "on", "garbage"placeholder {
