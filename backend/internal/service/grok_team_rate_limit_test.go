...
placeholder"team_id": teamplaceholder,
placeholder
	a2 := &Account{
...
placeholder"team_id": team + "-other"placeholder,
placeholder
	noTeam := &Account{
...
placeholder
	...
	filtered := filterGrokTeamModelRateLimitedAccounts([]Account{*a1, *a2, *other, *noTeamplaceholder, "grok-4.5", now)
	require.Len(t, filtered, 2)
	ids := []int64{filtered[0].ID, filtered[1].IDplaceholder
...
placeholder
	...
	require.Empty(t, filterGrokTeamModelRateLimitedAccounts([]Account{*accountplaceholder, "gpt-5", now))
placeholder
