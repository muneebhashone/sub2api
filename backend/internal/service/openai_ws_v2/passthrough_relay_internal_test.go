...
func TestRelayUsageTerminalWithoutUsageKeepsFallback(t *testing.T) {
	...
	state := &relayState{placeholder
	parseUsageAndAccumulate(state, []byte(`{"type":"response.in_progress","usage":{"input_tokens":9,"output_tokens":4placeholderplaceholder`), "response.in_progress", nil)
	parseUsageAndAccumulate(state, []byte(`{"type":"response.completed","response":{"id":"resp_1"placeholderplaceholder`), "response.completed", nil)

	turnUsage := finalizeRelayTurnUsage(state)
	require.Equal(t, Usage{InputTokens: 9, OutputTokens: 4placeholder, turnUsage)
...
func TestRelayUsageTerminalZeroKeepsFallback(t *testing.T) {
	...
	state := &relayState{placeholder
	parseUsageAndAccumulate(state, []byte(`{"type":"response.in_progress","usage":{"input_tokens":9,"output_tokens":4placeholderplaceholder`), "response.in_progress", nil)
	parseUsageAndAccumulate(state, []byte(`{"type":"response.completed","usage":{"input_tokens":0,"output_tokens":0placeholderplaceholder`), "response.completed", nil)

	require.Equal(t, Usage{InputTokens: 9, OutputTokens: 4placeholder, finalizeRelayTurnUsage(state))
	require.Equal(t, Usage{InputTokens: 9, OutputTokens: 4placeholder, state.usage)
placeholder
...
func TestRelayUsageTerminalNonZeroReplacesFallbackAsWhole(t *testing.T) {
	...
	state := &relayState{placeholder
	parseUsageAndAccumulate(state, []byte(`{"type":"response.in_progress","usage":{"input_tokens":9,"output_tokens":4,"input_tokens_details":{"cached_tokens":2placeholderplaceholderplaceholder`), "response.in_progress", nil)
	parseUsageAndAccumulate(state, []byte(`{"type":"response.completed","usage":{"input_tokens":3,"output_tokens":0,"input_tokens_details":{"cached_tokens":0placeholderplaceholderplaceholder`), "response.completed", nil)

	require.Equal(t, Usage{InputTokens: 3placeholder, finalizeRelayTurnUsage(state))
	require.Equal(t, Usage{InputTokens: 3placeholder, state.usage)
placeholder
...
func TestObserveUpstreamMessageBareErrorClearsTurnStateAndFinalizesUsageOnce(t *testing.T) {
	...
	observed := observeUpstreamMessage(
		state,
		[]byte(`{"type":"error","usage":{"input_tokens":3,"output_tokens":1placeholder,"error":{"message":"failed"placeholderplaceholder`),
		now.Add(-3*time.Second),
		func() time.Time { return now placeholder,
		nil,
	)
	...
	require.Equal(t, Usage{InputTokens: 3, OutputTokens: 1placeholder, observed.usage)
	require.Equal(t, observed.usage, state.usage)
	require.Equal(t, Usage{placeholder, state.turnUsage)
	...
	observeUpstreamMessage(state, []byte(`{"type":"error","error":{"message":"again"placeholderplaceholder`), now, func() time.Time { return now placeholder, nil)
	finalizePendingBareError(state, now)
	require.Equal(t, Usage{InputTokens: 3, OutputTokens: 1placeholder, state.usage)
placeholder
...
func TestObserveUpstreamMessageErrorThenFailedSettlesUsageOnce(t *testing.T) {
	...
	errorObserved := observeUpstreamMessage(
		state,
		[]byte(`{"type":"error","error":{"code":"server_is_overloaded","message":"overloaded"placeholderplaceholder`),
		now,
		func() time.Time { return now placeholder,
		nil,
	)
	...
	require.Equal(t, Usage{placeholder, state.usage)

	failedObserved := observeUpstreamMessage(
		state,
		[]byte(`{"type":"response.failed","response":{"id":"resp_1","usage":{"input_tokens":7,"output_tokens":2placeholderplaceholderplaceholder`),
		now,
		func() time.Time { return now placeholder,
		nil,
	)
	require.True(t, failedObserved.terminal)
	require.Equal(t, Usage{InputTokens: 7, OutputTokens: 2placeholder, failedObserved.usage)
...
func TestObserveUpstreamMessageBareErrorBeforeNextCompletedKeepsBothTurns(t *testing.T) {
	...
	now := time.Unix(400, 0)
	state := &relayState{placeholder
	parseUsageAndAccumulate(state, []byte(`{"type":"response.in_progress","usage":{"input_tokens":5,"output_tokens":1placeholderplaceholder`), "response.in_progress", nil)
	bare := observeUpstreamMessage(
		state,
		[]byte(`{"type":"error","error":{"message":"first turn failed"placeholderplaceholder`),
		now,
		func() time.Time { return now placeholder,
		nil,
	)
	...
	require.Equal(t, Usage{InputTokens: 5, OutputTokens: 1placeholder, bare.usage)

	completed := observeUpstreamMessage(
		state,
		[]byte(`{"type":"response.completed","response":{"id":"resp_next","usage":{"input_tokens":3,"output_tokens":2placeholderplaceholderplaceholder`),
		now,
		func() time.Time { return now placeholder,
		nil,
	)
	require.True(t, completed.terminal)
	require.Equal(t, Usage{InputTokens: 3, OutputTokens: 2placeholder, completed.usage)
	require.Equal(t, Usage{InputTokens: 8, OutputTokens: 3placeholder, state.usage)
placeholder
...
placeholder, &relayState{requestModel: "gpt-5"placeholder, observedUpstreamEvent{
		terminal:  true,
		eventType: "error",
		usage:     Usage{InputTokens: 4, OutputTokens: 1placeholder,
placeholder)
	require.Equal(t, 1, called)
	require.Empty(t, bareError.RequestID)
	require.Equal(t, "error", bareError.TerminalEventType)
	require.Equal(t, Usage{InputTokens: 4, OutputTokens: 1placeholder, bareError.Usage)
