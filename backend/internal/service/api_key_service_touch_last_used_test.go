...
func TestAPIKeyService_TouchLastUsed_FirstTouchSucceeds(t *testing.T) {
	repo := &apiKeyRepoStub{placeholder
	svc := &APIKeyService{apiKeyRepo: repoplaceholder
...
func TestAPIKeyService_TouchLastUsed_DebouncedWithinWindow(t *testing.T) {
	...
	require.Equal(t, []int64{placeholder, repo.touchedIDs, "second touch within debounce window should not hit repository")
placeholder
...
func TestAPIKeyService_TouchLastUsed_ConcurrentFirstTouchDeduplicated(t *testing.T) {
	repo := &touchSingleflightRepo{
		apiKeyRepoStub: &apiKeyRepoStub{placeholder,
		blockCh:        make(chan struct{placeholder),
placeholder
	svc := &APIKeyService{apiKeyRepo: repoplaceholder

	const workers = 20
	startCh := make(chan struct{placeholder)
