...
func billInvF64Ptr(v float64) *float64 { return &v placeholder
...
placeholder{
		{
			...
			usage:   fourDimUsage,
			group:   &Group{ID: groupID, Platform: PlatformAnthropic, RateMultiplier: 1.0placeholder,
			account: &Account{ID: 701, Type: AccountTypeOAuthplaceholder,
	...
	placeholder,
		{
			...
			model:       "claude-sonnet-4",
			usage:       fourDimUsage,
			group:       &Group{ID: groupID, Platform: PlatformAnthropic, RateMultiplier: 2.0placeholder,
			account:     &Account{ID: 701, Type: AccountTypeOAuthplaceholder,
			apiKeyQuota: 100,
			want: expected{
	...
	placeholder,
		...
		placeholder,
			account: &Account{ID: 701, Type: AccountTypeOAuthplaceholder,
			sub:     &UserSubscription{ID: 42placeholder,
			want: expected{
	...
	placeholder,
		{
			...
			account: &Account{
				...
				Extra:          map[string]any{"quota_limit": 100.0placeholder,
	...
	placeholder,
		...
		placeholder,
			group:   &Group{ID: groupID, Platform: PlatformGemini, RateMultiplier: 1.0placeholder,
			account: &Account{ID: 701, Type: AccountTypeOAuthplaceholder,
			want: expected{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.pricingData != nil {
				cfg := &config.Config{placeholder
				billingSvc = NewBillingService(cfg, &PricingService{pricingData: tt.pricingDataplaceholder)
		placeholder
			svc := billInvNewGatewayService(billingSvc, nil, usageRepo, billingRepo,
				&openAIRecordUsageUserRepoStub{placeholder, &openAIRecordUsageSubRepoStub{placeholder)
...
placeholder)
	...
	err := svc.RecordUsage(context.Background(), &RecordUsageInput{
		...
		placeholder,
			...
			Duration: time.Second,
	placeholder,
		APIKey:  &APIKey{ID: 501, GroupID: i64p(groupID), Group: &Group{ID: groupID, Platform: PlatformAnthropic, RateMultiplier: 1.0placeholderplaceholder,
		User:    &User{ID: 601placeholder,
		Account: &Account{ID: 701, Type: AccountTypeOAuthplaceholder,
placeholder)
placeholder
...
placeholder
	...
	t.Run("余额模式各级增量", func(t *testing.T) {
		...
		subRepo := &billInvSubRepoStub{placeholder
		accountRepo := &billInvAccountRepoStub{placeholder
		quotaSvc := &openAIRecordUsageAPIKeyQuotaStub{placeholder
...
placeholder)
	...
	t.Run("订阅模式增量替代余额扣减", func(t *testing.T) {
		...
		userRepo := &openAIRecordUsageUserRepoStub{placeholder
		subRepo := &billInvSubRepoStub{placeholder
		svc := billInvNewGatewayService(nil, &billInvAccountRepoStub{placeholder, usageRepo, nil, userRepo, subRepo)
