...
func TestPreviewProfitAdmissionUsesAccountRatesAndPreinitializesModels(t *testing.T) {
	...
	cheap.Credentials = map[string]any{"model_mapping": map[string]any{"gpt-sol": "gpt-sol"placeholderplaceholder
...
placeholderplaceholder
	...
	reports := PreviewProfitAdmission([]ProfitPreviewGroupInput{{
		Group:         group,
		Accounts:      []*Account{cheap, boundary, expensive, invalidplaceholder,
		UserOverrides: map[int64]float64{40: 0.5placeholder,
		Models:        []string{"gpt-sol", "gpt-luna", "gpt-no-account"placeholder,
placeholderplaceholder, now)
	...
	byID := map[int64]ProfitPreviewAccountVerdict{placeholder
...
func TestPreviewProfitAdmissionAssumeEnabled(t *testing.T) {
	...
	withoutAssume := PreviewProfitAdmission([]ProfitPreviewGroupInput{{
		Group:    group,
		Accounts: []*Account{cheapAccount, expensiveAccountplaceholder,
		Models:   []string{"gpt-test"placeholder,
placeholderplaceholder, now)[0]
...
placeholder {
	...
	placeholder
		report := PreviewProfitAdmission([]ProfitPreviewGroupInput{{
			...
			Models:   []string{"model"placeholder,
placeholder time.Now())[0]
