...
func TestIsGrokVideoStatusBillable(t *testing.T) {
	...
	require.True(t, IsGrokVideoStatusBillable([]byte(`{
		...
		"video":{"url":"https://vidgen.x.ai/x.mp4","duration":8,"respect_moderation":trueplaceholder
placeholder`)))
	...
	require.False(t, IsGrokVideoStatusBillable([]byte(`{"status":"expired"placeholder`)))
	require.False(t, IsGrokVideoStatusBillable([]byte(`{"status":"failed"placeholder`)))
	// done without video.url is not billable
	require.False(t, IsGrokVideoStatusBillable([]byte(`{"status":"done"placeholder`)))
	// URL alone (legacy/non-official shapes) is not enough
	require.False(t, IsGrokVideoStatusBillable([]byte(`{"url":"https://example.com/v.mp4"placeholder`)))
	require.False(t, IsGrokVideoStatusBillable([]byte(`{"download_url":"/v1/videos/task/content"placeholder`)))
	// "completed" is not the official enum value
	require.False(t, IsGrokVideoStatusBillable([]byte(`{"status":"completed","video":{"url":"https://vidgen.x.ai/x.mp4"placeholderplaceholder`)))
placeholder
...
placeholder
	...
	body := []byte(`{
		"status":"done",
		"model":"grok-imagine-video-1.5",
		"video":{"url":"https://vidgen.x.ai/signed.mp4","duration":12,"respect_moderation":trueplaceholder
placeholder`)
...
placeholder
	...
	body := []byte(`{"status":"done","video":{"url":"https://vidgen.x.ai/signed.mp4"placeholderplaceholder`)
...
func TestExtractGrokVideoBillingRejectsNonDoneStatus(t *testing.T) {
	t.Parallel()
	pending := &GrokVideoPendingBilling{Model: "m", VideoDurationSeconds: 8, VideoResolution: "720p"placeholder
	require.Nil(t, ExtractGrokVideoBillingFromStatusBody(
		[]byte(`{"status":"pending","video":{"url":"https://vidgen.x.ai/x.mp4","duration":8placeholderplaceholder`),
		pending, "req",
	))
	require.Nil(t, ExtractGrokVideoBillingFromStatusBody(
		[]byte(`{"status":"completed","video":{"url":"https://vidgen.x.ai/x.mp4","duration":8placeholderplaceholder`),
		pending, "req",
	))
placeholder
...
func TestGrokMediaUsageFromResponseVideoStatusBillsOnOfficialDone(t *testing.T) {
	...
	meta := grokMediaUsageFromResponse(
		...
		[]byte(`{"status":"done","model":"grok-imagine-video-1.5","video":{"url":"https://vidgen.x.ai/a.mp4","duration":9placeholderplaceholder`),
	)
	...
	pendingOnly := grokMediaUsageFromResponse(
		GrokMediaEndpointVideoStatus,
		GrokMediaRequestInfo{placeholder,
		[]byte(`{"status":"pending"placeholder`),
	)
	...
	completed := grokMediaUsageFromResponse(
		GrokMediaEndpointVideoStatus,
		GrokMediaRequestInfo{placeholder,
		[]byte(`{"status":"completed","video":{"url":"https://vidgen.x.ai/a.mp4","duration":9placeholderplaceholder`),
	)
