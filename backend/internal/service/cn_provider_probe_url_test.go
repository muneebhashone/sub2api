...
placeholder
	...
	formatOnly, err := cnValidateProbeURL(&config.Config{
		Security: config.SecurityConfig{URLAllowlist: config.URLAllowlistConfig{Enabled: falseplaceholderplaceholder,
placeholder, "https://relay.attacker.example/v1/usages")
placeholder
	require.Equal(t, "https://relay.attacker.example/v1/usages", formatOnly)
placeholder

// recordingHTTPUpstream 断言探测被策略拒绝时没有任何上游请求发出。
type recordingHTTPUpstream struct{ calls int placeholder
