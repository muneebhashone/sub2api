...
placeholder
	...
	order := newOpenAILegacyUpstreamRateOrder([]*Account{grokCheap, anthropicExpensive, nilplaceholder, now, defaultOpenAIOAuthSchedulingRateMultiplier)
	require.False(t, order.enabled)
	require.Empty(t, order.rates)
	require.Zero(t, order.compare(grokCheap, anthropicExpensive))

	factors := openAIUpstreamCostFactors([]*Account{grokCheap, anthropicExpensiveplaceholder, now, defaultOpenAIOAuthSchedulingRateMultiplier)
	...
	mixed := newOpenAILegacyUpstreamRateOrder(
		[]*Account{grokCheap, openAICheap, anthropicExpensive, openAIExpensiveplaceholder,
