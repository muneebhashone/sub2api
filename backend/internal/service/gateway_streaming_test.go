...
func TestParseSSEUsage_MessageStart(t *testing.T) {
	...
	data := `{"type":"message_start","message":{"usage":{"input_tokens":100,"cache_creation_input_tokens":50,"cache_read_input_tokens":200placeholderplaceholderplaceholder`
...
func TestParseSSEUsage_MessageDelta(t *testing.T) {
	...
	data := `{"type":"message_delta","usage":{"output_tokens":42placeholderplaceholder`
	svc.parseSSEUsage(data, usage)
...
func TestParseSSEUsage_DeltaDoesNotOverwriteStartValues(t *testing.T) {
	...
	// 先处理 message_start
	svc.parseSSEUsage(`{"type":"message_start","message":{"usage":{"input_tokens":100placeholderplaceholderplaceholder`, usage)
	require.Equal(t, 100, usage.InputTokens)

	// 再处理 message_delta（output_tokens > 0, input_tokens = 0）
	svc.parseSSEUsage(`{"type":"message_delta","usage":{"output_tokens":50placeholderplaceholder`, usage)
...
func TestParseSSEUsage_DeltaOverwritesWithNonZero(t *testing.T) {
	...
	svc.parseSSEUsage(`{"type":"message_delta","usage":{"input_tokens":200,"output_tokens":100,"cache_creation_input_tokens":30,"cache_read_input_tokens":60placeholderplaceholder`, usage)
...
func TestParseSSEUsage_DeltaDoesNotResetCacheCreationBreakdown(t *testing.T) {
	...
	svc.parseSSEUsage(`{"type":"message_start","message":{"usage":{"input_tokens":100,"cache_creation":{"ephemeral_5m_input_tokens":30,"ephemeral_1h_input_tokens":70placeholderplaceholderplaceholderplaceholder`, usage)
	require.Equal(t, 30, usage.CacheCreation5mTokens)
	require.Equal(t, 70, usage.CacheCreation1hTokens)

	// 后续 delta 带默认 0，不应覆盖已有非零值
	svc.parseSSEUsage(`{"type":"message_delta","usage":{"output_tokens":12,"cache_creation":{"ephemeral_5m_input_tokens":0,"ephemeral_1h_input_tokens":0placeholderplaceholderplaceholder`, usage)
...
func TestParseSSEUsage_UnknownType(t *testing.T) {
	...
	svc.parseSSEUsage(`{"type":"content_block_delta","delta":{"text":"hello"placeholderplaceholder`, usage)
	require.Equal(t, 0, usage.InputTokens)
	require.Equal(t, 0, usage.OutputTokens)
placeholder
...
func TestHandleStreamingResponse_CacheTokens(t *testing.T) {
	...
	go func() {
		...
		_, _ = pw.Write([]byte("data: {\"type\":\"message_start\",\"message\":{\"usage\":{\"input_tokens\":10,\"cache_creation_input_tokens\":20,\"cache_read_input_tokens\":30placeholderplaceholderplaceholder\n\n"))
		_, _ = pw.Write([]byte("data: {\"type\":\"message_delta\",\"usage\":{\"output_tokens\":15placeholderplaceholder\n\n"))
...
func TestHandleStreamingResponse_SpecialCharactersInJSON(t *testing.T) {
	...
	go func() {
		...
		_, _ = pw.Write([]byte("data: {\"type\":\"content_block_delta\",\"index\":0,\"delta\":{\"type\":\"text_delta\",\"text\":\"Hello \\\"world\\\"\\n你好\"placeholderplaceholder\n\n"))
		_, _ = pw.Write([]byte("data: {\"type\":\"message_start\",\"message\":{\"usage\":{\"input_tokens\":5placeholderplaceholderplaceholder\n\n"))
		_, _ = pw.Write([]byte("data: {\"type\":\"message_delta\",\"usage\":{\"output_tokens\":3placeholderplaceholder\n\n"))
		_, _ = pw.Write([]byte("data: [DONE]\n\n"))
placeholder()
...
func TestHandleStreamingResponse_StreamReadErrorBeforeOutput_TriggersFailover(t *testing.T) {
	...
	resp := &http.Response{
		...
		Body:       &streamReadCloser{err: io.ErrUnexpectedEOFplaceholder,
placeholder
...
func TestHandleStreamingResponse_StreamReadErrorAfterOutput_PassesThrough(t *testing.T) {
	...
	resp := &http.Response{
		...
		Body: &streamReadCloser{
			payload: []byte("data: {\"type\":\"message_start\",\"message\":{\"usage\":{\"input_tokens\":5placeholderplaceholderplaceholder\n\n"),
...
func TestSanitizeStreamError_KnownErrors(t *testing.T) {
	cases := []struct {
		...
		want string
placeholder{
		{"unexpected EOF", io.ErrUnexpectedEOF, "unexpected EOF"placeholder,
		{"EOF", io.EOF, "EOF"placeholder,
		{"context canceled", context.Canceled, "canceled"placeholder,
		{"deadline exceeded", context.DeadlineExceeded, "deadline exceeded"placeholder,
		{"ECONNRESET 直接", syscall.ECONNRESET, "connection reset by peer"placeholder,
		{"EPIPE", syscall.EPIPE, "broken pipe"placeholder,
		{"ETIMEDOUT", syscall.ETIMEDOUT, "connection timed out"placeholder,
		{"未识别错误兜底", errors.New("weird internal error"), "upstream connection error"placeholder,
		{"nil 返回空串", nil, ""placeholder,
placeholder
...
placeholder
	...
	resp := &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"text/event-stream"placeholderplaceholder,
		Body:       &streamReadCloser{err: netErrplaceholder,
placeholder

	_, err := svc.handleStreamingResponse(context.Background(), resp, c, &Account{ID: 1placeholder, time.Now(), "model", "model", false)
placeholder
...
func TestHandleStreamingResponse_SSEErrorEvent_ReturnsTypedErrorWithRawData(t *testing.T) {
	...
	const errorJSON = `{"type":"error","error":{"type":"overloaded_error","message":"Anthropic upstream is overloaded"placeholderplaceholder`
...
func TestHandleStreamingResponse_SSEErrorEvent_AfterPartialStreamOutput(t *testing.T) {
	...
	const errorJSON = `{"type":"error","error":{"type":"rate_limit_error","message":"Rate limited"placeholderplaceholder`
