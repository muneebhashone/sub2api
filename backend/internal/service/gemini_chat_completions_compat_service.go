...
placeholder else if useUpstreamStream {
...
placeholder
	...
	closeOpenBlock := func() bool {
		if openBlockIndex < 0 {
			return false
	placeholder
		disconnected := emitAnthropicEvent(&apicompat.AnthropicStreamEvent{Type: "content_block_stop"placeholder)
		openBlockIndex = -1
		openBlockType = ""
		return disconnected
placeholder
...
placeholder) {
		return &geminiStreamResult{usage: &usage, firstTokenMs: firstTokenMsplaceholder, nil
placeholder
	if emitAnthropicEvent(&apicompat.AnthropicStreamEvent{Type: "message_stop"placeholder) {
		return &geminiStreamResult{usage: &usage, firstTokenMs: firstTokenMsplaceholder, nil
placeholder
