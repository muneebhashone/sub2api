...
func (s *EmailService) sendMailPlain(addr string, auth smtp.Auth, from, to string, msg []byte, host string) error {
	dialer := &net.Dialer{Timeout: smtpDialTimeoutplaceholder
...
placeholder
	...
	if ok, _ := client.Extension("STARTTLS"); ok {
		if err = client.StartTLS(&tls.Config{ServerName: host, MinVersion: tls.VersionTLS12placeholder); err != nil {
...
<head>
    ...
    <style>
        ...
        .header h1 { margin: 0; font-size: 24px; placeholder
        .content { padding: 40px 30px; text-align: center; placeholder
        .button { display: inline-block; background: linear-gradient(135deg, #667eea 0%%, #764ba2 100%%); color: white; padding: 14px 32px; text-decoration: none; border-radius: 8px; font-size: 16px; font-weight: 600; margin: 20px 0; placeholder
        .button:hover { opacity: 0.9; placeholder
        .info { color: #666; font-size: 14px; line-height: 1.6; margin-top: 20px; placeholder
        .link-fallback { color: #666; font-size: 12px; word-break: break-all; margin-top: 20px; padding: 15px; background-color: #f8f9fa; border-radius: 4px; placeholder
        .footer { background-color: #f8f9fa; padding: 20px; text-align: center; color: #999; font-size: 12px; placeholder
        .warning { color: #e74c3c; font-weight: 500; placeholder
    </style>
</head>
<body>
    <div class="container">
