...
func TestGetModelPricing_OpenAICompactAliasesFallback(t *testing.T) {
	...
	tests := []struct {
		...
		longContext int
placeholder{
		{model: "gpt5.5", inputPrice: 2.5e-6, outputPrice: 15e-6, cacheRead: 0.25e-6, longContext: 272000placeholder,
		{model: "openai/gpt5.4", inputPrice: 2.5e-6, outputPrice: 15e-6, cacheRead: 0.25e-6, longContext: 272000placeholder,
		{model: "gpt5.4-mini", inputPrice: 7.5e-7, outputPrice: 4.5e-6, cacheRead: 7.5e-8, longContext: 0placeholder,
		{model: "gpt5.3codexspark", inputPrice: 1.5e-6, outputPrice: 12e-6, cacheRead: 0.15e-6, longContext: 0placeholder,
placeholder
...
func TestGetFallbackPricing_FamilyMatching(t *testing.T) {
	...
	floatPtr := func(v float64) *float64 { return &v placeholder
...
placeholder{
		...
		{name: "openai gpt5.3 codex spark", model: "gpt-5.3-codex-spark", expectedInput: placeholder,
		{name: "openai legacy gpt5.1 falls back to gpt5.4", model: "gpt-5.1", expectedInput: 2.5e-6placeholder,
		{name: "openai legacy gpt5.1 codex falls back to gpt5.3 codex", model: "gpt-5.1-codex", expectedInput: placeholder,
		{name: "openai legacy codex mini latest falls back to gpt5.3 codex", model: "codex-mini-latest", expectedInput: placeholder,
	...
	placeholder,
		...
		// ---- 负向用例 ----
		{name: "qwen unknown no fallback", model: "qwen-max", expectNilPricing: trueplaceholder,
		// doubao-pro / doubao-embedding（纯文本）不在白名单，不回退；仅 doubao-embedding-vision 显式命中。
		{name: "doubao unknown no fallback", model: "doubao-pro", expectNilPricing: trueplaceholder,
		{name: "doubao text embedding no fallback", model: "doubao-embedding-text-240515", expectNilPricing: trueplaceholder,
		{name: "hunyuan unknown no fallback", model: "hunyuan-t1", expectNilPricing: trueplaceholder,
		{name: "moonshot v1 not covered", model: "moonshot-v1-8k", expectNilPricing: trueplaceholder,
		// bare k3 仅精确/后缀匹配：相似未知型号不得因含 "k3" 误命中。
		{name: "k3-like unknown no fallback", model: "foo-k3-bar", expectNilPricing: trueplaceholder,
		// 路径最后一段不是 /k3：foo-k3 不得因 HasSuffix("/k3") 或 Contains 误命中。
		{name: "path segment not bare k3 no fallback", model: "vendor/foo-k3", expectNilPricing: trueplaceholder,
...
func TestCalculateCost_DoubaoEmbeddingVisionDifferentialInput(t *testing.T) {
	...
	mixed := UsageTokens{InputTokens: 1340, ImageInputTokens: 28placeholder
...
placeholder
	...
	textOnly := UsageTokens{InputTokens: placeholder
...
placeholder
	...
	weird := UsageTokens{InputTokens: 10, ImageInputTokens: 50placeholder
...
func TestCalculateVideoCostUsesSeparateConfig(t *testing.T) {
	...
	imagePrice := 0.4
	videoPrice := 0.08
	imageCost := svc.CalculateImageCost("grok-imagine-video", "2K", 1, &ImagePriceConfig{Price2K: &imagePriceplaceholder, 1.0)
	videoCost := svc.CalculateVideoCost("grok-imagine-video", "480p", 1, 10, &VideoPriceConfig{Price480P: &videoPriceplaceholder, 0.5)
...
func TestGetModelPricing_Grok46OfficialFallback(t *testing.T) {
	...
	for _, model := range []string{"grok-4.6", "grok-4.6-latest"placeholder {
		model := model
		t.Run(model, func(t *testing.T) {
			pricing, err := svc.GetModelPricing(model)
		placeholder
...
func TestCalculateCostUnified_GroupLongContextToggleUsesPresetLadder(t *testing.T) {
	...
	tokens := UsageTokens{InputTokens: 250000, OutputTokens: 1000placeholder

	off := &Group{LongContextPricingEnabled: falseplaceholder
	disabled, err := svc.CalculateCostUnified(CostInput{
		Model: "grok-4.5", Group: off, Tokens: tokens, RateMultiplier: 1, Resolver: resolver,
placeholder)
placeholder

	on := &Group{LongContextPricingEnabled: trueplaceholder
...
placeholder
	...
	for _, model := range []string{"grok-2-image-1212", "grok-2-audio", "grok-5-video", "x-ai/grok-6-image"placeholder {
