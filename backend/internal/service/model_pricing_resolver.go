...
placeholder else if input.GroupID != nil && r.channelService != nil {
...
func (r *ModelPricingResolver) resolveConfiguredPricing(config *ChannelModelPricing, model, source string) *ResolvedPricing {
	...
	if mode == "" {
		mode = BillingModeToken
placeholder
	resolved := &ResolvedPricing{Mode: mode, Source: source, channelPricing: configplaceholder
	if mode == BillingModePerRequest || mode == BillingModeImage || mode == BillingModeVideo {
...
func (r *ModelPricingResolver) applyTokenOverrides(chPricing *ChannelModelPricing, resolved *ResolvedPricing) {
	if resolved.BasePricing == nil {
		resolved.BasePricing = &ModelPricing{placeholder
placeholder else {
...
func intervalToModelPricing(iv *PricingInterval, base *ModelPricing, chPricing *ChannelModelPricing) *ModelPricing {
	pricing := &ModelPricing{placeholder
	if base != nil {
		*pricing = *base
placeholder
...
placeholder else if iv.InputMultiplier != nil {
...
placeholder else if iv.OutputMultiplier != nil {
...
placeholder else if iv.CacheWriteMultiplier != nil {
...
placeholder else if iv.CacheReadMultiplier != nil {
