...
type openAIProfitControlGateCtxKey struct{placeholder
...
type openAIProfitControlSuppressCtxKey struct{placeholder

// openAIPricingAtCtxKey 携带请求级定价时刻 pricingAt：门的 D 与 RecordUsage
// 的高峰因子共用，保证一个请求从准入到扣费不中途变价。
type openAIPricingAtCtxKey struct{placeholder
...
func (s *OpenAIGatewayService) WithOpenAIRequestPricingContext(ctx context.Context, groupID *int64) (context.Context, time.Time) {
	...
	ctx = context.WithValue(ctx, openAIPricingAtCtxKey{placeholder, pricingAt)
...
func WithOpenAIProfitControlSuppressed(ctx context.Context) context.Context {
	return context.WithValue(ctx, openAIProfitControlSuppressCtxKey{placeholder, struct{placeholder{placeholder)
placeholder
...
func (s *OpenAIGatewayService) WithOpenAITurnPricingContext(ctx context.Context, groupID *int64) (context.Context, time.Time) {
	pricingAt := timezone.Now()
	ctx = context.WithValue(ctx, openAIPricingAtCtxKey{placeholder, pricingAt)
	if _, suppressed := ctx.Value(openAIProfitControlSuppressCtxKey{placeholder).(struct{placeholder); suppressed {
		return ctx, pricingAt
placeholder
	if existing, ok := ctx.Value(openAIProfitControlGateCtxKey{placeholder).(*openAIProfitControlGate); ok && existing != nil {
...
placeholder
	...
	if gate == nil {
		...
		if existing, ok := ctx.Value(openAIProfitControlGateCtxKey{placeholder).(*openAIProfitControlGate); ok && existing != nil {
			return context.WithValue(ctx, openAIProfitControlGateCtxKey{placeholder, (*openAIProfitControlGate)(nil)), pricingAt
	placeholder
		return ctx, pricingAt
placeholder
	openAIProfitControlObserverInstance.recordInstall(gate.groupID, gate.platform, gate.threshold)
	return context.WithValue(ctx, openAIProfitControlGateCtxKey{placeholder, gate), pricingAt
placeholder
...
func openAIPricingAtFromContext(ctx context.Context) (time.Time, bool) {
	pricingAt, ok := ctx.Value(openAIPricingAtCtxKey{placeholder).(time.Time)
...
placeholder
	...
	if gate == nil {
		...
		if existing, ok := ctx.Value(openAIProfitControlGateCtxKey{placeholder).(*openAIProfitControlGate); ok && existing != nil && groupID != nil && existing.groupID != *groupID {
...
placeholder
	...
	if ctxGroup, ok := ctx.Value(ctxkey.Group).(*Group); ok && IsGroupContextValid(ctxGroup) && ctxGroup.ID == *groupID {
		group = ctxGroup
placeholder else if s.schedulerSnapshot != nil {
...
func attachSelectionProfitGate(ctx context.Context, sel *AccountSelectionResult) *AccountSelectionResult {
	if sel == nil {
		return nil
placeholder
	if gate, ok := ctx.Value(openAIProfitControlGateCtxKey{placeholder).(*openAIProfitControlGate); ok && gate != nil {
		sel.profitGate = gate
placeholder
	return sel
placeholder
...
func ContextWithSelectionProfitGate(ctx context.Context, sel *AccountSelectionResult) context.Context {
	if sel == nil || sel.profitGate == nil {
		return ctx
placeholder
	if existing, ok := ctx.Value(openAIProfitControlGateCtxKey{placeholder).(*openAIProfitControlGate); ok && existing == sel.profitGate {
		return ctx
placeholder
	return context.WithValue(ctx, openAIProfitControlGateCtxKey{placeholder, sel.profitGate)
placeholder
...
var openAIProfitControlObserverInstance = &openAIProfitControlObserver{placeholder
...
func (o *openAIProfitControlObserver) stats(groupID int64, platform string) *openAIProfitControlGroupStats {
	...
	placeholder
placeholder
	v, _ := o.groups.LoadOrStore(key, &openAIProfitControlGroupStats{placeholder)
	if s, ok := v.(*openAIProfitControlGroupStats); ok {
		return s
placeholder
	// 不可达：map 中只存 *openAIProfitControlGroupStats；兜底返回独立实例避免 panic。
	return &openAIProfitControlGroupStats{placeholder
placeholder
