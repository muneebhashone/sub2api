...
func moderationProxyIDPtr(v int64) *int64 { return &v placeholder
...
func TestContentModerationCallRoutesThroughProxy(t *testing.T) {
	...
	placeholder
		...
		_ = json.NewEncoder(w).Encode(moderationAPIResponse{Results: []moderationAPIResult{{Flagged: falseplaceholderplaceholderplaceholder)
placeholder))
...
placeholder
	...
	proxyRepo := &contentModerationTestProxyRepo{proxies: map[int64]*Proxy{
		7: {ID: 7, Name: "audit-proxy", Protocol: "http", Host: host, Port: port, Status: StatusActiveplaceholder,
...
placeholder))
	...
	proxyRepo := &contentModerationTestProxyRepo{getByIDErr: errors.New("proxy deleted")placeholder
	svc := NewContentModerationService(nil, nil, nil, nil, nil, proxyRepo, nil, nil)
...
func TestContentModerationProxyURLResolutionCached(t *testing.T) {
	proxyRepo := &contentModerationTestProxyRepo{proxies: map[int64]*Proxy{
		3: {ID: 3, Name: "p", Protocol: "http", Host: "127.0.0.1", Port: 8080, Status: StatusActiveplaceholder,
placeholderplaceholder
	svc := NewContentModerationService(nil, nil, nil, nil, nil, proxyRepo, nil, nil)
...
func TestContentModerationUpdateConfigProxyIDSemantics(t *testing.T) {
	...
	proxyRepo := &contentModerationTestProxyRepo{proxies: map[int64]*Proxy{
		5: {ID: 5, Name: "p", Protocol: "http", Host: "127.0.0.1", Port: 8080, Status: StatusActiveplaceholder,
placeholderplaceholder
	svc := NewContentModerationService(settingRepo, nil, nil, nil, nil, proxyRepo, nil, nil)
	ctx := context.Background()

	view, err := svc.UpdateConfig(ctx, UpdateContentModerationConfigInput{ProxyID: moderationProxyIDPtr(5)placeholder)
	if err != nil {
...
placeholder
	...
	// nil 表示不修改，代理保持不变。
	enabled := true
	view, err = svc.UpdateConfig(ctx, UpdateContentModerationConfigInput{Enabled: &enabledplaceholder)
	if err != nil {
...
placeholder
	...
	// 0 表示清除，恢复直连。
	view, err = svc.UpdateConfig(ctx, UpdateContentModerationConfigInput{ProxyID: moderationProxyIDPtr(0)placeholder)
	if err != nil {
...
placeholder
	...
	// 不存在的代理必须被校验拒绝。
	if _, err := svc.UpdateConfig(ctx, UpdateContentModerationConfigInput{ProxyID: moderationProxyIDPtr(404)placeholder); err == nil {
...
placeholder))
	...
	savedCfg.APIKeys = []string{"sk-saved"placeholder
...
placeholderplaceholder
	...
	result, err := svc.TestAPIKeys(context.Background(), TestContentModerationAPIKeysInput{APIKeys: []string{"sk-input"placeholderplaceholder)
	if err != nil {
...
placeholder))
	...
	result, err = svc.TestAPIKeys(context.Background(), TestContentModerationAPIKeysInput{
		APIKeys: []string{"sk-input"placeholder,
