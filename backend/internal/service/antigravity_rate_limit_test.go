...
func TestHandleUpstreamError_429_NonModelRateLimit_UsesMappedModelKey(t *testing.T) {
	repo := &stubAntigravityAccountRepo{placeholder
	svc := &AntigravityGatewayService{accountRepo: repoplaceholder
	account := &Account{ID: 20, Name: "acc-20", Platform: PlatformAntigravityplaceholder

	body := buildGeminiRateLimitBody("5s")

	result := svc.handleUpstreamError(context.Background(), "[test]", account, http.StatusTooManyRequests, http.Header{placeholder, body, "claude-opus-4-6", 0, "", false)
...
func TestSetAntigravityModelRateLimits_GeminiWritesFamilyScope(t *testing.T) {
	repo := &stubAntigravityAccountRepo{placeholder
	svc := &AntigravityGatewayService{placeholder
	account := &Account{ID: 789, Platform: PlatformAntigravityplaceholder
	resetAt := time.Now().Add(30 * time.Second)
...
func TestSetAntigravityModelRateLimits_ClaudeDoesNotWriteGeminiScope(t *testing.T) {
	repo := &stubAntigravityAccountRepo{placeholder
	svc := &AntigravityGatewayService{placeholder
	account := &Account{ID: 790, Platform: PlatformAntigravityplaceholder
	resetAt := time.Now().Add(30 * time.Second)
