...
func flusherPtrTime(t time.Time) *time.Time { return &t placeholder
...
func newTestFlusher(cache quotaDirtyCache, writer quotaSnapshotWriter) *UserPlatformQuotaUsageFlusher {
	return &UserPlatformQuotaUsageFlusher{
		...
		metrics:      &FlusherMetrics{placeholder,
placeholder
placeholder
...
func TestFlusher_PopSnapshotUpsert(t *testing.T) {
	keys := []UserPlatformQuotaKey{
		{UserID: 1, Platform: "anthropic"placeholder,
		{UserID: 2, Platform: "openai"placeholder,
placeholder
	cache := &mockQuotaDirtyCache{
		popSequence: [][]UserPlatformQuotaKey{keysplaceholder, // 第 1 次返回 keys，之后空
		getEntries: []*UserPlatformQuotaCacheEntry{
			makeEntry(1.0, 2.0, 3.0),
			makeEntry(4.0, 5.0, 6.0),
	placeholder,
placeholder
	writer := &mockQuotaSnapshotWriter{placeholder
...
placeholder
	cache := &mockQuotaDirtyCache{
		popSequence: [][]UserPlatformQuotaKey{keysplaceholder,
		getEntries: []*UserPlatformQuotaCacheEntry{
			makeEntry(1.0, 2.0, 3.0),
			nil, // MISS
	placeholder,
...
placeholder
	...
	writer := &mockQuotaSnapshotWriter{returnErr: writeErrplaceholder
	f := newTestFlusher(cache, writer)
...
func TestFlusher_FKViolationDropsNoReadd(t *testing.T) {
	keys := []UserPlatformQuotaKey{
		{UserID: 999, Platform: "anthropic"placeholder,
placeholder
	cache := &mockQuotaDirtyCache{
		popSequence: [][]UserPlatformQuotaKey{keysplaceholder,
		getEntries: []*UserPlatformQuotaCacheEntry{
	...
	placeholder,
placeholder
	writer := &mockQuotaSnapshotWriter{returnErr: ErrUserPlatformQuotaFKViolationplaceholder
	f := newTestFlusher(cache, writer)
...
func TestScenario_ZeroPercentCompany(t *testing.T) {
	cache := &mockQuotaDirtyCache{
		...
		popSequence: [][]UserPlatformQuotaKey{placeholder,
placeholder
	writer := &mockQuotaSnapshotWriter{placeholder
	f := newTestFlusher(cache, writer)
...
placeholder{
		{"零值回退 2s", 0, 2 * time.Secondplaceholder,
		{"负数回退 2s", -100, 2 * time.Secondplaceholder,
		{"正常 2000ms 保留", 2000, 2 * time.Secondplaceholder,
		{"正常 500ms 保留", 500, 500 * time.Millisecondplaceholder,
placeholder
...
func TestFlusher_ReaddFailCounts(t *testing.T) {
	keys := []UserPlatformQuotaKey{
		{UserID: 10, Platform: "anthropic"placeholder,
		{UserID: 11, Platform: "openai"placeholder,
placeholder
	...
	placeholder
		f := newTestFlusher(cache, &mockQuotaSnapshotWriter{placeholder)
...
func TestNewUserPlatformQuotaUsageFlusher_ClampsBatchSize(t *testing.T) {
	cases := []struct {
		...
		want int
placeholder{
		{"超上限被 clamp", 7000, maxFlushBatchSizeplaceholder,
		{"恰好上限保留", maxFlushBatchSize, maxFlushBatchSizeplaceholder,
		{"零回退默认", 0, defaultFlushBatchSizeplaceholder,
		{"负数回退默认", -5, defaultFlushBatchSizeplaceholder,
		{"正常值保留", 500, 500placeholder,
placeholder
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			cfg := &config.Config{placeholder
...
func TestScenario_NinetyPercentCompany(t *testing.T) {
	keys := []UserPlatformQuotaKey{
		{UserID: 101, Platform: "anthropic"placeholder,
		{UserID: 102, Platform: "anthropic"placeholder,
		{UserID: 103, Platform: "openai"placeholder,
		{UserID: 104, Platform: "openai"placeholder,
		{UserID: 105, Platform: "anthropic"placeholder,
placeholder
