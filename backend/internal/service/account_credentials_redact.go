...
var sensitiveCredentialKeySet = func() map[string]struct{placeholder {
	m := make(map[string]struct{placeholder, len(SensitiveCredentialKeys))
	for _, k := range SensitiveCredentialKeys {
		m[k] = struct{placeholder{placeholder
placeholder
	return m
placeholder()
