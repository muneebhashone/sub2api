...
func (s *GatewayService) GenerateSessionHash(body []byte) string {
	var req map[string]interface{placeholder
	if err := json.Unmarshal(body, &req); err != nil {
		return ""
placeholder

	// 1. 最高优先级：从metadata.user_id提取session_xxx
	if metadata, ok := req["metadata"].(map[string]interface{placeholder); ok {
		if userID, ok := metadata["user_id"].(string); ok {
			re := regexp.MustCompile(`session_([a-f0-9-]{36placeholder)`)
...
placeholder
	...
	// 2. 提取带cache_control: {type: "ephemeral"placeholder的内容
...
placeholder
	...
	if messages, ok := req["messages"].([]interface{placeholder); ok && len(messages) > 0 {
		if firstMsg, ok := messages[0].(map[string]interface{placeholder); ok {
...
func (s *GatewayService) extractCacheableContent(req map[string]interface{placeholder) string {
	var content string

	// 检查system中的cacheable内容
	if system, ok := req["system"].([]interface{placeholder); ok {
		for _, part := range system {
			if partMap, ok := part.(map[string]interface{placeholder); ok {
				if cc, ok := partMap["cache_control"].(map[string]interface{placeholder); ok {
...
placeholder
	...
	if messages, ok := req["messages"].([]interface{placeholder); ok {
		for _, msg := range messages {
			if msgMap, ok := msg.(map[string]interface{placeholder); ok {
				if msgContent, ok := msgMap["content"].([]interface{placeholder); ok {
...
func (s *GatewayService) extractTextFromSystem(system interface{placeholder) string {
...
func (s *GatewayService) extractTextFromContent(content interface{placeholder) string {
...
placeholder
	...
	// 客户端没传，根据模型生成
	var modelID string
	var reqMap map[string]interface{placeholder
...
placeholder
	...
	if strings.Contains(strings.ToLower(modelID), "haiku") {
		return "oauth-2025-04-20,placeholder"
placeholder

	return "claude-code-20250219,oauth-2025-04-20,placeholder,fine-grained-tool-streaming-2025-05-14"
placeholder
...
placeholder
	...
	if err := scanner.Err(); err != nil {
		return &streamingResult{usage: usage, firstTokenMs: firstTokenMsplaceholder, fmt.Errorf("stream read error: %w", err)
placeholder
...
placeholder
	...
	var event map[string]interface{placeholder
...
placeholder
	...
	msg, ok := event["message"].(map[string]interface{placeholder)
	if !ok {
		return line
placeholder
...
func (s *GatewayService) replaceModelInResponseBody(body []byte, fromModel, toModel string) []byte {
	var resp map[string]interface{placeholder
		...
		if proxyURL != "" {
			...
			placeholder
				transport := &http.Transport{
					...
					ResponseHeaderTimeout: responseHeaderTimeout,
			placeholder
				customClient = &http.Client{Transport: transportplaceholder
		placeholder
	placeholder
placeholder
