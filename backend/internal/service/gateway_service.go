...
placeholder
	...
	if billingErr != nil || ccErr != nil {
		...
		return body
placeholder
	out, ok := setJSONRawBytes(body, "system", buildJSONArrayRaw([][]byte{billingBlock, ccPromptBlockplaceholder))
	if !ok {
...
placeholder else {
	...
	placeholder else if s.cfg != nil && s.cfg.Gateway.InjectBetaForAPIKey {
