...
func TestAPIKeyAuthSnapshotProfitControlRoundtrip(t *testing.T) {
	...
	payload, err := json.Marshal(&APIKeyAuthCacheEntry{Snapshot: snapshotplaceholder)
placeholder
...
placeholder
	...
	gwSvc := &OpenAIGatewayService{placeholder
...
func TestAPIKeyAuthSnapshotOldVersionEvicted(t *testing.T) {
	...
	materialized, used, err := svc.applyAuthCacheEntry("sk-old", &APIKeyAuthCacheEntry{Snapshot: snapshotplaceholder)
placeholder
