...
func (s *AntigravityOAuthService) loadProjectIDWithRetry(ctx context.Context, accessToken, proxyURL string, maxRetries int) (*loadCodeAssistResult, error) {
	...
	placeholder
		...
		if err != nil {
			lastErr = err
	placeholder else if loadResp == nil {
...
placeholder
	...
	if lastSubscription != nil {
		return &loadCodeAssistResult{Subscription: lastSubscriptionplaceholder, fmt.Errorf("获取 project_id 失败 (重试 %d 次后): %w", maxRetries, lastErr)
placeholder
