...
func computeInfraHealth(now time.Time, overview *OpsDashboardOverview) float64 {
	...
	placeholder else if overview.SystemMetrics.RedisOK != nil && !*overview.SystemMetrics.RedisOK {
...
placeholder
	...
	placeholder
		...
		if hb.LastErrorAt != nil && (hb.LastSuccessAt == nil || hb.LastErrorAt.After(*hb.LastSuccessAt)) {
			failedJobs++
	placeholder else if hb.LastSuccessAt != nil && now.Sub(*hb.LastSuccessAt) > 15*time.Minute {
			failedJobs++
	placeholder
placeholder
