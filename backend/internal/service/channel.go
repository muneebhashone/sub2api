...
type Channel struct {
	...
	// 渠道级模型映射（按平台分组：platform → {src→dstplaceholder）
...
placeholder
	if p.TimePricing != nil {
		cp.TimePricing = &ChannelTimePricing{Timezone: p.TimePricing.Timezoneplaceholder
...
func validateIntervalPrices(iv *PricingInterval, idx int) error {
	prices := []struct {
		...
		val  *float64
placeholder{
		{"input_price", iv.InputPriceplaceholder,
		{"output_price", iv.OutputPriceplaceholder,
		{"cache_write_price", iv.CacheWritePriceplaceholder,
		{"cache_read_price", iv.CacheReadPriceplaceholder,
		{"per_request_price", iv.PerRequestPriceplaceholder,
placeholder
...
placeholder
	multipliers := []struct {
		...
		val  *float64
placeholder{
		{"input_multiplier", iv.InputMultiplierplaceholder,
		{"output_multiplier", iv.OutputMultiplierplaceholder,
		{"cache_write_multiplier", iv.CacheWriteMultiplierplaceholder,
		{"cache_read_multiplier", iv.CacheReadMultiplierplaceholder,
placeholder
...
placeholder
	...
	type dedupKey struct {
		...
		name     string
placeholder
	seen := make(map[dedupKey]struct{placeholder)
...
placeholder
	...
	add := func(platform, displayName string, pricing *ChannelModelPricing) {
		key := dedupKey{platform: platform, name: strings.ToLower(displayName)placeholder
		if _, ok := seen[key]; ok {
			return
	placeholder
