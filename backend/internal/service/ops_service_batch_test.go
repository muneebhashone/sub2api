...
placeholder
	...
	detail := `{"authorization":"Bearer secret-token"placeholder`
	entries := []*OpsInsertErrorLogInput{
		{
			ErrorBody:            `{"error":"bad","access_token":"secret"placeholder`,
...
placeholder
	...
	require.Contains(t, *first.UpstreamErrorsJSON, "[placeholder]")
...
placeholder
	...
	err := svc.RecordErrorBatch(context.Background(), []*OpsInsertErrorLogInput{
		{ErrorMessage: "first"placeholder,
		{ErrorMessage: "second"placeholder,
placeholder)
placeholder
...
placeholder
	...
	err := svc.RecordError(context.Background(), &OpsInsertErrorLogInput{
		...
		UpstreamErrors: []*OpsUpstreamErrorEvent{
			{Stage: string(GatewayFailureStageInference), UpstreamStatusCode: 403, Message: staleMessage, Detail: staleDetailplaceholder,
			{
