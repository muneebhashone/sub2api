...
placeholder
	...
	respBody := []byte(`{
		"error": {
			...
			"details": [
				{"@type": "type.googleapis.com/google.rpc.ErrorInfo", "metadata": {"model": "gemini-3-pro-high"placeholder, "reason": "RATE_LIMIT_EXCEEDED"placeholder,
				{"@type": "type.googleapis.com/google.rpc.RetryInfo", "retryDelay": "39s"placeholder
			]
	placeholder
placeholder`)
...
placeholder
	...
	origBody := []byte(`{"error":{"code":503,"status":"UNAVAILABLE"placeholderplaceholder`)
	resp := &http.Response{
		StatusCode: http.StatusServiceUnavailable,
		Header:     http.Header{"X-Test": {"original"placeholderplaceholder,
placeholder
...
func TestHandleSingleAccountRetryInPlace_ContextCanceled(t *testing.T) {
	upstream := &mockSmartRetryUpstream{
		responses: []*http.Response{nilplaceholder,
		errors:    []error{nilplaceholder,
placeholder
...
placeholder
	upstream := &mockSmartRetryUpstream{
		...
		responses: []*http.Response{nil, successRespplaceholder,
...
placeholder
	...
	upstream := &mockSmartRetryUpstream{
		responses: []*http.Response{resp503, successRespplaceholder,
		errors:    []error{nil, nilplaceholder,
placeholder
...
func TestAntigravityRetryLoop_503_SingleAccount_InPlaceRetryUsed_E2E(t *testing.T) {
	...
	initial503Body := []byte(`{
		"error": {
			...
			"details": [
				{"@type": "type.googleapis.com/google.rpc.ErrorInfo", "metadata": {"model": "gemini-3-pro"placeholder, "reason": "MODEL_CAPACITY_EXHAUSTED"placeholder,
				{"@type": "type.googleapis.com/google.rpc.RetryInfo", "retryDelay": "10s"placeholder
			],
...
placeholder
	...
	upstream := &mockSmartRetryUpstream{
		...
		responses: []*http.Response{initial503Resp, successRespplaceholder,
		errors:    []error{nil, nilplaceholder,
placeholder
