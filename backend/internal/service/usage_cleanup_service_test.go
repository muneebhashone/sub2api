...
func TestUsageCleanupServiceExecuteTaskProgressError(t *testing.T) {
	repo := &cleanupRepoStub{
		deleteQueue: []cleanupDeleteResponse{
			{deleted: 2placeholder,
			{deleted: 0placeholder,
	placeholder,
		updateErr: errors.New("update failed"),
placeholder
	cfg := &config.Config{UsageCleanup: config.UsageCleanupConfig{Enabled: true, BatchSize: 2placeholderplaceholder
	svc := NewUsageCleanupService(repo, nil, nil, cfg)
	task := &UsageCleanupTask{
		ID: 8,
		Filters: UsageCleanupFilters{
...
func TestUsageCleanupServiceExecuteTaskDeleteCanceled(t *testing.T) {
	repo := &cleanupRepoStub{
		deleteQueue: []cleanupDeleteResponse{
			{err: context.Canceledplaceholder,
	placeholder,
placeholder
	cfg := &config.Config{UsageCleanup: config.UsageCleanupConfig{Enabled: true, BatchSize: 2placeholderplaceholder
	svc := NewUsageCleanupService(repo, nil, nil, cfg)
...
func TestUsageCleanupServiceExecuteTaskMarkFailedUpdateError(t *testing.T) {
	repo := &cleanupRepoStub{
		deleteQueue: []cleanupDeleteResponse{
			{err: errors.New("boom")placeholder,
	placeholder,
...
func TestUsageCleanupServiceExecuteTaskDashboardRecomputeError(t *testing.T) {
	dashboardRepo := &dashboardRepoStub{recomputeErr: errors.New("recompute failed")placeholder
	repo := &cleanupRepoStub{
		deleteQueue: []cleanupDeleteResponse{
			{deleted: 0placeholder,
	placeholder,
placeholder
	dashboard := NewDashboardAggregationService(dashboardRepo, nil, &config.Config{
		DashboardAgg: config.DashboardAggregationConfig{Enabled: trueplaceholder,
placeholder)
	cfg := &config.Config{UsageCleanup: config.UsageCleanupConfig{Enabled: true, BatchSize: 2placeholderplaceholder
...
placeholder
	...
	defer repo.mu.Unlock()
	require.Len(t, repo.markSucceeded, 1)
	require.Eventually(t, func() bool { return dashboardRepo.recomputeCalls == 1 placeholder, time.Second, 10*time.Millisecond)
placeholder

func TestUsageCleanupServiceExecuteTaskDashboardRecomputeSuccess(t *testing.T) {
	dashboardRepo := &dashboardRepoStub{placeholder
	repo := &cleanupRepoStub{
		deleteQueue: []cleanupDeleteResponse{
			{deleted: 0placeholder,
	placeholder,
...
func TestUsageCleanupServiceCancelTaskStatusError(t *testing.T) {
	repo := &cleanupRepoStub{statusErr: errors.New("status broken")placeholder
	cfg := &config.Config{UsageCleanup: config.UsageCleanupConfig{Enabled: trueplaceholderplaceholder
	svc := NewUsageCleanupService(repo, nil, nil, cfg)
...
func TestUsageCleanupServiceIsTaskCanceledError(t *testing.T) {
	repo := &cleanupRepoStub{statusErr: errors.New("status err")placeholder
	svc := NewUsageCleanupService(repo, nil, nil, &config.Config{UsageCleanup: config.UsageCleanupConfig{Enabled: trueplaceholderplaceholder)
