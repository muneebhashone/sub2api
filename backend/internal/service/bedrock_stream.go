		...
		for {
			...
			if err != nil {
				if err == io.EOF {
					return
			placeholder
				_ = sendEvent(decodeEvent{err: errplaceholder)
				return
		placeholder
			lastReadAt.Store(time.Now().UnixNano())
			if !sendEvent(decodeEvent{payload: payloadplaceholder) {
				return
		placeholder
	placeholder
placeholder()
		...
		placeholder
			...
			placeholder
				if errors.Is(ev.err, context.Canceled) || errors.Is(ev.err, context.DeadlineExceeded) {
					return &streamingResult{usage: usage, firstTokenMs: firstTokenMs, clientDisconnect: trueplaceholder, nil
			placeholder
				return &streamingResult{usage: usage, firstTokenMs: firstTokenMsplaceholder, fmt.Errorf("bedrock stream read error: %w", ev.err)
		placeholder
...
// Bedrock payload 格式：{"bytes":"<base64-encoded-json>"placeholder
...
//
//	{"type":"message_delta","delta":{...placeholder,"amazon-bedrock-invocationMetrics":{"inputTokenCount":150,"outputTokenCount":42placeholderplaceholder
//
// 转换为：
//
//	{"type":"message_delta","delta":{...placeholder,"usage":{"input_tokens":150,"output_tokens":42placeholderplaceholder
