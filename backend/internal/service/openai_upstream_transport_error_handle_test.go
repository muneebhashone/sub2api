...
func TestHandleOpenAIUpstreamTransportError_PersistentEvictsAndFailsOver(t *testing.T) {
	...
	account := &Account{ID: 4627, Name: "proxy-expired", Platform: PlatformOpenAIplaceholder
...
func TestHandleOpenAIUpstreamTransportError_TransientFailsOverWithoutEviction(t *testing.T) {
	repo := &openaiTransportAccountRepoStub{placeholder
	svc := &OpenAIGatewayService{accountRepo: repoplaceholder
	account := &Account{ID: 99, Name: "flaky", Platform: PlatformOpenAIplaceholder
	c, rec := newOpenAITransportErrTestContext()
...
func TestHandleOpenAIUpstreamTransportError_ContextCanceled_NoFailoverNoEviction(t *testing.T) {
	repo := &openaiTransportAccountRepoStub{placeholder
	svc := &OpenAIGatewayService{accountRepo: repoplaceholder
	account := &Account{ID: 77, Name: "healthy", Platform: PlatformOpenAIplaceholder
	c, rec := newOpenAITransportErrTestContext()
...
func TestHandleOpenAIUpstreamTransportError_WrappedContextCanceled_NoFailover(t *testing.T) {
	repo := &openaiTransportAccountRepoStub{placeholder
	svc := &OpenAIGatewayService{accountRepo: repoplaceholder
	account := &Account{ID: 78, Name: "healthy2", Platform: PlatformOpenAIplaceholder
...
func TestTempUnscheduleOpenAITransportError_NilAccountRepo_InMemoryBlockOnly(t *testing.T) {
	...
	svc := &OpenAIGatewayService{accountRepo: nilplaceholder
	account := &Account{ID: 55, Name: "no-db", Platform: PlatformOpenAIplaceholder
...
func TestHandleOpenAIUpstreamTransportError_DeadlineExceeded_StillFailsOver(t *testing.T) {
	repo := &openaiTransportAccountRepoStub{placeholder
	svc := &OpenAIGatewayService{accountRepo: repoplaceholder
	account := &Account{ID: 79, Name: "slow", Platform: PlatformOpenAIplaceholder
	c, _ := newOpenAITransportErrTestContext()
...
func TestHandleOpenAIUpstreamTransportError_RecordsOllamaActivityOnly(t *testing.T) {
	...
	svc := &OpenAIGatewayService{
		accountRepo:     &openaiTransportAccountRepoStub{placeholder,
...
placeholder"api_key": "k-openai", "base_url": "https://api.openai.com"placeholder,
placeholder
	c, _ := newOpenAITransportErrTestContext()
...
func TestHandleOpenAIAccountUpstreamError_RecordsOllamaActivityOnly(t *testing.T) {
	deferred := NewDeferredService(nil, nil, time.Second)
	svc := &OpenAIGatewayService{deferredService: deferredplaceholder
	ollama := &Account{
...
placeholder
	...
	_ = svc.handleOpenAIAccountUpstreamError(context.Background(), ollama, http.StatusTooManyRequests, http.Header{placeholder, []byte(`{"error":{"message":"rate"placeholderplaceholder`), "gpt-test")
	_ = svc.handleOpenAIAccountUpstreamError(context.Background(), other, http.StatusTooManyRequests, http.Header{placeholder, []byte(`{"error":{"message":"rate"placeholderplaceholder`), "gpt-test")
