...
func (s *RateLimitService) IsOpenAIAdvancedSchedulerStickyWeightedEnabled(ctx context.Context) bool {
	if s == nil || s.settingService == nil {
		return false
placeholder
	gateway := &OpenAIGatewayService{rateLimitService: splaceholder
...
placeholder
	...
	if err := s.accountRepo.SetTempUnschedulable(ctx, account.ID, *decision.Until, reason); err != nil {
		slog.Warn("account_scheduling_threshold_set_temp_unsched_failed",
			...
			"error", err)
placeholder else if s.tempUnschedCache != nil {
...
placeholder
	...
	case 400:
		...
		if strings.Contains(strings.ToLower(upstreamMsg), "organization has been disabled") {
			...
			shouldDisable = true
	placeholder else if account.Platform == PlatformAnthropic && strings.Contains(strings.ToLower(upstreamMsg), "credit balance") {
			// Anthropic API key 余额不足（语义等同 402），停止调度
			msg := "Credit balance exhausted (400): " + upstreamMsg
			s.handleAuthError(ctx, account, msg)
			shouldDisable = true
	placeholder else if strings.Contains(strings.ToLower(upstreamMsg), "identity verification is required") {
	...
	placeholder
		...
		placeholder
			...
			break
	placeholder
		// OpenAI: {"detail":"Unauthorized"placeholder 表示 token 完全无效（非标准 OpenAI 错误格式），直接标记 error
	...
	default:
		...
		placeholder
			...
			shouldDisable = true
	placeholder else if statusCode >= 500 {
			// 未启用自定义错误码时：仅记录5xx错误
...
func (s *RateLimitService) handleAuthError(ctx context.Context, account *Account, errorMsg string) {
	s.notifyAccountSchedulingBlocked(account, time.Time{placeholder, "auth_error")
...
func (s *RateLimitService) handleOpenAI403(ctx context.Context, account *Account, upstreamMsg string, responseBody []byte) (shouldDisable bool) {
	...
	// {"error":{...placeholderplaceholder 结构化错误。这类响应描述的是「这条链路 / 这个端点被挡了」，
...
func (s *RateLimitService) handleCustomErrorCode(ctx context.Context, account *Account, statusCode int, errorMsg string) {
	...
	s.notifyAccountSchedulingBlocked(account, time.Time{placeholder, "custom_error_code")
...
func (s *RateLimitService) handle429(ctx context.Context, account *Account, headers http.Header, responseBody []byte) {
	...
	placeholder); ok && checker.ShouldRetryOpenAIOAuth429(account, headers, responseBody) {
			return
	placeholder
placeholder
...
placeholder
	...
	if chosen == nil {
		return nil
placeholder
	return &anthropic429Result{resetAt: *chosen, fiveHourReset: reset5hplaceholder
placeholder
...
placeholder
	...
	if _, err := repo.BulkUpdate(ctx, []int64{account.IDplaceholder, AccountBulkUpdate{
placeholder"plan_type": planTypeplaceholder,
placeholder); err != nil {
...
placeholder
	...
	if settings == nil {
		...
		if cooldown <= 0 {
			cooldown = 10
	placeholder
		settings = &OverloadCooldownSettings{Enabled: true, CooldownMinutes: cooldownplaceholder
placeholder
...
placeholder
	...
	if resetStr := headers.Get("anthropic-ratelimit-unified-5h-reset"); resetStr != "" {
		...
		placeholder else if needInitWindow || account.SessionWindowEnd == nil || !end.Equal(*account.SessionWindowEnd) {
...
placeholder
	...
	result := &SuccessfulTestRecoveryResult{placeholder
...
func (s *RateLimitService) RecoverAccountAfterSuccessfulTest(ctx context.Context, accountID int64) (*SuccessfulTestRecoveryResult, error) {
	return s.RecoverAccountState(ctx, accountID, AccountRecoveryOptions{placeholder)
placeholder
...
type tempUnschedulableModelContextKey struct{placeholder
...
placeholder
	return context.WithValue(ctx, tempUnschedulableModelContextKey{placeholder, model)
placeholder
...
placeholder
	if ctx == nil {
		return ""
placeholder
	model, _ := ctx.Value(tempUnschedulableModelContextKey{placeholder).(string)
...
placeholder
	...
	placeholder
		...
		if matchedKeyword == "" {
			continue
	placeholder
		matches = append(matches, tempUnschedulableRuleMatch{rule: rule, ruleIndex: idx, matchedKeyword: matchedKeywordplaceholder)
placeholder
	return matches
placeholder
...
func (s *RateLimitService) triggerStreamTimeoutError(ctx context.Context, account *Account, model string) bool {
	...
	s.notifyAccountSchedulingBlocked(account, time.Time{placeholder, "stream_timeout_error")
	if err := s.accountRepo.SetError(ctx, account.ID, errorMsg); err != nil {
