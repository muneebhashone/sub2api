...
func (f APIKeyUpdateFields) IsEmpty() bool {
	return f == APIKeyUpdateFields{placeholder
placeholder
...
type authCacheSubscriptionReadyKey struct{placeholder

func withAuthCacheSubscriptionReady(ctx context.Context, ready func()) context.Context {
	return context.WithValue(ctx, authCacheSubscriptionReadyKey{placeholder, ready)
placeholder
...
func NotifyAuthCacheSubscriptionReady(ctx context.Context) {
	if ready, ok := ctx.Value(authCacheSubscriptionReadyKey{placeholder).(func()); ok && ready != nil {
		ready()
placeholder
placeholder
...
type APIKeyService struct {
	...
	authLookupSlots           chan struct{placeholder
...
func (s *APIKeyService) AuthLookupMetrics() APIKeyAuthLookupMetrics {
	if s == nil {
		return APIKeyAuthLookupMetrics{placeholder
placeholder
...
placeholder
	svc.authLookupSlots = make(chan struct{placeholder, lookupConcurrency)
...
func (s *APIKeyService) currentConcurrencyForAPIKey(ctx context.Context, apiKeyID int64) int {
	if s == nil || s.concurrencyService == nil || apiKeyID <= 0 {
		return 0
placeholder
	counts, err := s.concurrencyService.GetAPIKeyConcurrencyBatch(ctx, []int64{apiKeyIDplaceholder)
	if err != nil {
		return 0
placeholder
...
placeholder
	if req.ClearExpiration {
		...
		if apiKey.Status == StatusAPIKeyExpired {
			apiKey.Status = StatusActive
	placeholder
placeholder else if req.ExpiresAt != nil {
...
placeholder
	...
	if apiKey.Quota > 0 && newQuotaUsed >= apiKey.Quota {
		...
		if err := s.apiKeyRepo.Update(ctx, apiKey, APIKeyUpdateFields{Status: trueplaceholder); err != nil {
