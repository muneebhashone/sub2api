...
func (f *fakeIncrCache) IncrUserPlatformQuotaUsageCache(ctx context.Context, userID int64, platform string, cost float64, ttl time.Duration, markDirty bool) error {
	f.calls = append(f.calls, incrCall{userID, platform, cost, ttl, markDirtyplaceholder)
	return nil
placeholder
...
func TestIncrementUserPlatformQuotaUsage_SyncCallsCache(t *testing.T) {
	fake := &fakeIncrCache{placeholder
...
placeholder
	if fake.calls[0] != (incrCall{userID: 101, platform: "anthropic", cost: 0.25, ttl: 120 * time.Second, markDirty: falseplaceholder) {
		t.Errorf("call[0] = %+v", fake.calls[0])
placeholder
	if fake.calls[1] != (incrCall{userID: 101, platform: "openai", cost: 0.50, ttl: 120 * time.Second, markDirty: falseplaceholder) {
...
type fakeFullCache struct {
	...
	dirty map[UserPlatformQuotaKey]struct{placeholder
placeholder
...
func (f *fakeFullCache) ReaddDirtyUserPlatformQuotaKeys(_ context.Context, keys []UserPlatformQuotaKey) error {
	...
	defer f.mu.Unlock()
	if f.dirty == nil {
		f.dirty = make(map[UserPlatformQuotaKey]struct{placeholder)
placeholder
	for _, k := range keys {
		f.dirty[k] = struct{placeholder{placeholder
placeholder
	return nil
placeholder
...
func TestCheckUserPlatformQuotaEligibility_NoRecordMeansUnlimited(t *testing.T) {
	repo := &fakeQuotaRepo{rec: nilplaceholder
	cache := &fakeFullCache{placeholder
	s := newServiceForPreflight(t, repo, cache)
	if err := s.checkUserPlatformQuotaEligibility(context.Background(), 1, "anthropic"); err != nil {
...
placeholderplaceholder
	...
	cache := &fakeFullCache{entry: &UserPlatformQuotaCacheEntry{DailyUsageUSD: 1.0placeholderplaceholder
	s := newServiceForPreflight(t, repo, cache)
	err := s.checkUserPlatformQuotaEligibility(context.Background(), 1, "anthropic")
	if !errors.Is(err, ErrUserPlatformDailyQuotaExhausted) {
...
func (f *fakeZeroQuotaCache) GetUserPlatformQuotaCache(_ context.Context, _ int64, _ string) (*UserPlatformQuotaCacheEntry, bool, error) {
	...
	entry := &UserPlatformQuotaCacheEntry{
		DailyUsageUSD:    0,
		DailyLimitUSD:    &daily,
		DailyWindowStart: func() *time.Time { t := time.Now().UTC(); return &t placeholder(),
		SchemaVersion:    UserPlatformQuotaCacheSchemaV1,
placeholder
...
func TestCheckUserPlatformQuotaEligibility_StandardMode_BlocksWhenLimitZero(t *testing.T) {
	fake := &fakeZeroQuotaCache{placeholder
	cfg := &config.Config{placeholder
	cfg.Billing.UserPlatformQuotaCacheTTLSeconds = 60
	s := &BillingCacheService{
		cache:                 fake,
		cfg:                   cfg,
		userPlatformQuotaRepo: &fakeQuotaRepo{placeholder,
placeholder
	err := s.checkUserPlatformQuotaEligibility(context.Background(), 1, "anthropic")
	if !errors.Is(err, ErrUserPlatformDailyQuotaExhausted) {
...
func TestCheckBillingEligibility_SubscriptionMode_BypassesPlatformQuota(t *testing.T) {
	fake := &fakeZeroQuotaCache{placeholder // GetUserPlatformQuotaCache 返回 limit=0，若被调用则拦截
	cfg := &config.Config{placeholder
	cfg.Billing.UserPlatformQuotaCacheTTLSeconds = 60
	s := &BillingCacheService{
		cache:                 fake,
...
placeholder
	sub := &UserSubscription{Status: "active"placeholder
	user := &User{ID: 42placeholder
...
func TestCheckBillingEligibility_NonSubscriptionGroup_AppliesQuota(t *testing.T) {
	called := &fakeZeroQuotaCache{placeholder
	cfg := &config.Config{placeholder
	cfg.Billing.UserPlatformQuotaCacheTTLSeconds = 60
	s := &BillingCacheService{
		cache:                 called,
...
func TestCheckUserPlatformQuotaEligibility_NoRow_WritesSentinel(t *testing.T) {
	repo := &fakeQuotaRepo{rec: nilplaceholder // DB 无行
	cache := &fakeFullCache{placeholder        // entry=nil → Get 返回 MISS
...
func TestCheckUserPlatformQuotaEligibility_RedisGetError_NoSentinelBackfill(t *testing.T) {
	repo := &fakeQuotaRepo{rec: nilplaceholder
	cache := &fakeFullCache{getErr: errors.New("redis get down")placeholder
	svc := newServiceForPreflight(t, repo, cache)
	svc.cfg.Billing.UserPlatformQuotaSentinelTTLSeconds = 3600
...
func TestCheckUserPlatformQuotaEligibility_NoRow_SentinelSetFailsFailOpen(t *testing.T) {
	...
	repo := &fakeQuotaRepo{rec: nilplaceholder
	cache := &fakeFullCache{setErr: errors.New("redis set timeout")placeholder
	svc := newServiceForPreflight(t, repo, cache)
	svc.cfg.Billing.UserPlatformQuotaSentinelTTLSeconds = 3600
...
placeholder
	cache := &fakeFullCache{entry: sentinelplaceholder // entry 非 nil → Get HIT
	svc := newServiceForPreflight(t, &fakeQuotaRepo{placeholder, cache)
...
placeholder{
		{
			name: "has_limit",
			setup: func() *BillingCacheService {
				entry := &UserPlatformQuotaCacheEntry{DailyLimitUSD: &dailyplaceholder
				svc := newServiceForPreflight(t, &fakeQuotaRepo{placeholder, &fakeFullCache{entry: entryplaceholder)
				return svc
		placeholder,
			want: true,
	placeholder,
		{
			...
			setup: func() *BillingCacheService {
				entry := &UserPlatformQuotaCacheEntry{placeholder // 三个 limit 字段全 nil
				svc := newServiceForPreflight(t, &fakeQuotaRepo{placeholder, &fakeFullCache{entry: entryplaceholder)
				return svc
		placeholder,
			want: false,
	placeholder,
		{
			...
			setup: func() *BillingCacheService {
				...
				svc := newServiceForPreflight(t, &fakeQuotaRepo{placeholder, &fakeFullCache{placeholder)
				return svc
		placeholder,
			want: true, // fail-safe
	placeholder,
		{
			name: "redis_err",
			setup: func() *BillingCacheService {
				svc := newServiceForPreflight(t, &fakeQuotaRepo{placeholder, &fakeFullCache{getErr: errors.New("redis down")placeholder)
				return svc
		placeholder,
			want: true, // fail-safe
	placeholder,
