...
func schedInvNewGatewayCache() *schedInvGatewayCache {
	return &schedInvGatewayCache{bindings: make(map[string]int64)placeholder
placeholder
...
func (c *schedInvGatewayCache) SetSessionAccountID(_ context.Context, groupID int64, sessionHash string, accountID int64, ttl time.Duration) error {
	...
	c.setCalls = append(c.setCalls, schedInvSetCall{groupID: groupID, hash: sessionHash, accountID: accountID, ttl: ttlplaceholder)
	return nil
placeholder
...
func (c *schedInvGatewayCache) RefreshSessionTTL(_ context.Context, groupID int64, sessionHash string, ttl time.Duration) error {
	c.mu.Lock()
	defer c.mu.Unlock()
	c.refreshCalls = append(c.refreshCalls, schedInvSetCall{groupID: groupID, hash: sessionHash, ttl: ttlplaceholder)
	return nil
placeholder
...
func TestSchedulingInvariant_SessionHashSourcePriority(t *testing.T) {
	...
	sessionCtx := &SessionContext{ClientIP: "10.1.2.3", UserAgent: "claude-cli/2.0.0", APIKeyID: 77placeholder
...
placeholderplaceholder
	messages := []any{msg("user", "hello invariants")placeholder
	...
	fallbackB := mustParseSessionHashRequest(t, anthropicSessionBody("plain system", messages, ""),
		&SessionContext{ClientIP: "10.9.9.9", UserAgent: "other-agent/1.0", APIKeyID: 88placeholder)
...
func TestSchedulingInvariant_StickySession_TTLIsOneHour(t *testing.T) {
	...
	t.Run("BindStickySession传递TTL", func(t *testing.T) {
		cache := schedInvNewGatewayCache()
		svc := &GatewayService{cache: cacheplaceholder
...
placeholder)
	...
	t.Run("miniredis过期语义", func(t *testing.T) {
		...
		svc := &GatewayService{cache: &schedInvRedisGatewayCache{rdb: rdbplaceholderplaceholder
...
func TestSchedulingInvariant_AccountModelMappingFiltersSelection(t *testing.T) {
	...
	t.Run("不支持请求模型的账号被排除", func(t *testing.T) {
		...
		restricted.Credentials = map[string]any{
			"model_mapping": map[string]any{"claude-opus-4-6": "claude-opus-4-6"placeholder,
	placeholder
...
placeholder)
	...
	t.Run("所有账号都不支持模型_返回无可用账号", func(t *testing.T) {
		...
		a.Credentials = map[string]any{"model_mapping": map[string]any{"claude-opus-4-6": "claude-opus-4-6"placeholderplaceholder
		b := schedInvAnthropicAccount(2, 1)
		b.Credentials = map[string]any{"model_mapping": map[string]any{"claude-haiku-4-5": "claude-haiku-4-5"placeholderplaceholder
	...
	newSvc := func(pricingModels []string) (*GatewayService, *mockGroupRepoForGateway) {
		ch := Channel{
			...
			ModelPricing: []ChannelModelPricing{
				{Platform: PlatformAnthropic, Models: pricingModelsplaceholder,
		placeholder,
			ModelMapping: map[string]map[string]string{
				PlatformAnthropic: {"claude-sonnet-4-5": "claude-sonnet-4-6"placeholder,
		placeholder,
	placeholder
		channelSvc := newTestChannelService(makeStandardRepo(ch, map[int64]string{groupID: PlatformAnthropicplaceholder))
		repo := schedInvAccountRepo(func() Account {
			a := schedInvAnthropicAccount(1, 1)
			a.AccountGroups = []AccountGroup{{AccountID: 1, GroupID: groupIDplaceholderplaceholder
			return a
	placeholder())
		groupRepo := &mockGroupRepoForGateway{
			groups: map[int64]*Group{
				groupID: {ID: groupID, Platform: PlatformAnthropic, Status: StatusActive, Hydrated: trueplaceholder,
		placeholder,
	placeholder
		svc := &GatewayService{
			accountRepo:    repo,
...
placeholder
	...
	t.Run("映射目标不在渠道定价_选号被拒绝", func(t *testing.T) {
		...
		gid := groupID
		svc, groupRepo := newSvc([]string{"claude-opus-4-6"placeholder)
...
placeholder)
	...
	t.Run("映射目标在渠道定价_正常选号", func(t *testing.T) {
		...
		gid := groupID
		svc, groupRepo := newSvc([]string{"claude-sonnet-4-6"placeholder)
