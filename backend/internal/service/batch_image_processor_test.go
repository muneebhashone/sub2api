...
placeholder
	...
	require.Len(t, repo.items[job.BatchID], 2)
	gotIDs := []string{repo.items[job.BatchID][0].CustomID, repo.items[job.BatchID][1].CustomIDplaceholder
	require.ElementsMatch(t, []string{"ok", "bad"placeholder, gotIDs)
...
func TestBatchImageResultIndexer_ReconcilesMissingAndUnknownCustomIDs(t *testing.T) {
	repo := newFakeBatchImageRepository()
	outputRef := "files/output"
	job := &BatchImageJob{BatchID: "imgbatch_reconcile", ProviderOutputRef: &outputRef, ItemCount: 3placeholder
	// 预创建提交时的 pending 条目（提交流程的行为）。
	require.NoError(t, repo.BulkCreateBatchImageItems(context.Background(), []CreateBatchImageItemParams{
		{JobID: job.BatchID, CustomID: "a", Status: BatchImageItemStatusPendingplaceholder,
		{JobID: job.BatchID, CustomID: "b", Status: BatchImageItemStatusPendingplaceholder,
		{JobID: job.BatchID, CustomID: "c", Status: BatchImageItemStatusPendingplaceholder,
placeholder))
	// provider 输出：a 成功，b 失败，c 漏掉，多出未知的 x。
	output := strings.Join([]string{
		`{"key":"a","response":{"candidates":[{"content":{"parts":[{"inlineData":{"mimeType":"image/png","data":"` + batchImageTestData + `"placeholderplaceholder]placeholderplaceholder]placeholderplaceholder`,
		`{"key":"b","error":{"code":"SAFETY","message":"blocked"placeholderplaceholder`,
		`{"key":"x","error":{"code":"UNKNOWN","message":"not ours"placeholderplaceholder`,
placeholder, "\n") + "\n"
	provider := &fakeProcessorProvider{result: outputplaceholder
