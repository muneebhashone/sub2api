...
placeholder{
		{
			...
			body:       []byte(`{"error":{"message":"model not found"placeholderplaceholder`),
			want:       true,
	placeholder,
		{
			name:       "404 model_not_found code",
			statusCode: http.StatusNotFound,
			body:       []byte(`{"error":{"code":"model_not_found","message":"The requested model was not found"placeholderplaceholder`),
			want:       true,
	placeholder,
		{
			name:       "404 unknown model message",
			statusCode: http.StatusNotFound,
			body:       []byte(`{"error":{"message":"unknown model gpt-5.4"placeholderplaceholder`),
			want:       true,
	placeholder,
		{
			name:       "404 endpoint not found is not model specific",
			statusCode: http.StatusNotFound,
			body:       []byte(`{"error":{"message":"endpoint not found"placeholderplaceholder`),
			want:       false,
	placeholder,
		{
