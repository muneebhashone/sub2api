...
placeholder
	...
	if statusResp.StatusCode >= 300 {
		defer func() { _ = statusResp.Body.Close() placeholder()
		SetOpsLatencyMs(c, OpsUpstreamLatencyMsKey, time.Since(upstreamStart).Milliseconds())
...
placeholder
	...
	if err != nil {
		return nil, s.handleOpenAIUpstreamTransportError(ctx, c, account, err, false)
placeholder
	defer func() { _ = contentResp.Body.Close() placeholder()
...
placeholder
	...
	for _, imageURL := range info.InputImageURLs {
		if imageURL = strings.TrimSpace(imageURL); imageURL != "" {
			images = append(images, map[string]string{"url": imageURLplaceholder)
	placeholder
placeholder
	for _, upload := range info.Uploads {
		...
		if err != nil {
			return nil, "", err
	placeholder
		images = append(images, map[string]string{"url": dataURLplaceholder)
placeholder
	if len(images) > 0 {
...
placeholder
	if maskImageURL != "" {
		payload["mask"] = map[string]string{"url": maskImageURLplaceholder
placeholder
...
placeholder
	...
	case GrokMediaEndpointImagesEdits:
		imageFields = []string{"image", "images", "mask"placeholder
	case GrokMediaEndpointVideosGenerations:
		imageFields = []string{"image", "images", "reference_images"placeholder
placeholder
	var err error
...
func extractGrokMediaVideoRequestID(body []byte) string {
	if len(body) == 0 || !gjson.ValidBytes(body) {
		return ""
placeholder
	for _, path := range []string{"request_id", "id", "data.request_id", "data.id", "video.request_id", "video.id", "task_id", "data.task_id", "video.task_id"placeholder {
