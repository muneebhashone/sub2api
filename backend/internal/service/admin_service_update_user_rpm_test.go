...
func TestAdminService_UpdateUser_InvalidatesAuthCacheOnRPMLimitChange(t *testing.T) {
	base := &userRepoStub{user: &User{ID: 42, Email: "u@example.com", RPMLimit: 10placeholderplaceholder
...
placeholder
	...
	require.Equal(t, []int64{42placeholder, invalidator.userIDs, "仅修改 RPMLimit 也应失效 API Key 认证缓存")
placeholder

func TestAdminService_UpdateUser_NoInvalidateWhenRPMLimitUnchanged(t *testing.T) {
	base := &userRepoStub{user: &User{ID: 42, Email: "u@example.com", RPMLimit: 10, Username: "old"placeholderplaceholder
	repo := &rpmUserRepoStub{userRepoStub: baseplaceholder
	invalidator := &authCacheInvalidatorStub{placeholder
	svc := &adminServiceImpl{
		userRepo:             repo,
