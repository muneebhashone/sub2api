...
type openAIAutoResetContextKey struct{placeholder

func withOpenAIAutoResetContext(ctx context.Context) context.Context {
	if ctx == nil {
		ctx = context.Background()
placeholder
	return context.WithValue(ctx, openAIAutoResetContextKey{placeholder, true)
placeholder
...
func isOpenAIAutoResetContext(ctx context.Context) bool {
	if ctx == nil {
		return false
placeholder
	value, _ := ctx.Value(openAIAutoResetContextKey{placeholder).(bool)
	return value
placeholder
...
func (s *OpenAIQuotaAutoResetService) Notify(accountID int64) {
	if s == nil || accountID <= 0 {
		return
placeholder
	if _, loaded := s.pending.LoadOrStore(accountID, struct{placeholder{placeholder); loaded {
		return
placeholder
	select {
	case <-s.ctx.Done():
...
placeholder
	...
	placeholder, PlatformOpenAI, AccountTypeOAuth, StatusActive, "", 0, "")
		if err != nil {
...
placeholder
	...
	placeholder,
		...
		RequireKey: true,
placeholder, func(execCtx context.Context) (any, error) {
	...
	placeholder
		...
		return openAIAutoResetConsumeResult{Code: resetResult.Code, WindowsReset: resetResult.WindowsResetplaceholder, nil
placeholder)
	if err != nil {
...
placeholder
	...
	snapshot := &OpenAICodexUsageSnapshot{UpdatedAt: now.UTC().Format(time.RFC3339)placeholder
...
func selectOpenAIAutoResetCandidate(candidates []openAIAutoResetCreditCandidate, available int, previous *OpenAIAutoResetCreditState, cycleHash string) (openAIAutoResetCreditCandidate, error) {
	if available <= 0 {
		return openAIAutoResetCreditCandidate{placeholder, infraerrors.Conflict("OPENAI_AUTO_RESET_NO_CREDIT", "no reset credit is available")
placeholder
	if len(candidates) < available {
		return openAIAutoResetCreditCandidate{placeholder, infraerrors.Conflict("OPENAI_AUTO_RESET_CREDIT_DETAILS_INCOMPLETE", "reset credit details are incomplete")
placeholder
	for _, candidate := range candidates {
		if _, err := time.Parse(time.RFC3339, candidate.ExpiresAt); err != nil {
			return openAIAutoResetCreditCandidate{placeholder, infraerrors.Conflict("OPENAI_AUTO_RESET_CREDIT_EXPIRY_INVALID", "reset credit expiration is invalid")
	placeholder
placeholder
...
placeholder)
	if previous != nil && previous.AttemptCycleHash == cycleHash && previous.AttemptCreditHash != "" {
		...
		placeholder
	placeholder
		return openAIAutoResetCreditCandidate{placeholder, infraerrors.Conflict("OPENAI_AUTO_RESET_ORIGINAL_CREDIT_UNAVAILABLE", "the original reset credit cannot be confirmed; refusing to switch credits")
placeholder
	if len(sorted) == 0 || strings.TrimSpace(sorted[0].ID) == "" {
		return openAIAutoResetCreditCandidate{placeholder, infraerrors.Conflict("OPENAI_AUTO_RESET_CREDIT_ID_MISSING", "the earliest reset credit has no official id")
placeholder
	return sorted[0], nil
placeholder
...
placeholder
	...
	for _, window := range []*OpenAIRateLimitWindow{usage.RateLimit.PrimaryWindow, usage.RateLimit.SecondaryWindowplaceholder {
		if window == nil {
			continue
	placeholder
...
func (s *OpenAIQuotaAutoResetService) persistState(ctx context.Context, accountID int64, state *OpenAIAutoResetCreditState) error {
	if state == nil {
		return nil
placeholder
	return s.accountRepo.UpdateExtra(ctx, accountID, map[string]any{OpenAIAutoResetCreditStateExtraKey: stateplaceholder)
placeholder
...
func (s *OpenAIQuotaAutoResetService) failState(ctx context.Context, accountID int64, state *OpenAIAutoResetCreditState, code string, cause error) error {
	if state == nil {
		state = &OpenAIAutoResetCreditState{placeholder
placeholder
