...
placeholder
	...
	for key, value := range wantVisibleMethods {
		initial[key] = value
placeholder
	repo := &paymentConfigSettingRepoStub{values: initialplaceholder
	svc := &PaymentConfigService{settingRepo: repoplaceholder

	enabled := true
	err := svc.UpdatePaymentConfig(context.Background(), UpdatePaymentConfigRequest{Enabled: &enabledplaceholder)
	if err != nil {
		t.Fatalf("UpdatePaymentConfig returned error: %v", err)
placeholder
...
placeholderplaceholder
	...
	emptyString := ""
	err := svc.UpdatePaymentConfig(context.Background(), UpdatePaymentConfigRequest{
		EnabledTypes:      []string{placeholder,
