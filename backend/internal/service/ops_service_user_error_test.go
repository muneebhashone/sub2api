...
func TestListUserErrorRequests_EnablesMatchDeletedKeyOwner(t *testing.T) {
	...
	if _, err := svc.ListUserErrorRequests(context.Background(), uid, &OpsErrorLogFilter{placeholder); err != nil {
		t.Fatal(err)
placeholder
...
placeholder
	...
	svcOwner := &OpsService{opsRepo: &stubOpsRepoForUserErr{detailToReturn: mk()placeholderplaceholder
...
placeholder
	...
	// 他人 → NotFound,不泄露存在性
	svcOther := &OpsService{opsRepo: &stubOpsRepoForUserErr{detailToReturn: mk()placeholderplaceholder
