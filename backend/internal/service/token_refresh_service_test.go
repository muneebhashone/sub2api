...
placeholder
	repo.accountsByID = map[int64]*Account{account.ID: accountplaceholder
	scheduler := &tokenRefreshSchedulerCache{placeholder
	cfg := &config.Config{TokenRefresh: config.TokenRefreshConfig{MaxRetries: 1placeholderplaceholder
...
placeholder
	...
	svc := &TokenRefreshService{
		...
		cfg:                    &config.TokenRefreshConfig{MaxRetries: 1, ProviderFailureThreshold: 1placeholder,
...
placeholder
	ctx, cancel := context.WithCancel(context.Background())
	repo := &tokenRefreshAccountRepo{cancelOnUpdate: cancelplaceholder
	repo.accountsByID = map[int64]*Account{account.ID: accountplaceholder
	invalidator := &tokenCacheInvalidatorStub{placeholder
	scheduler := &tokenRefreshSchedulerCache{placeholder
	cache := &mockTokenCacheForRefreshAPI{lockResult: trueplaceholder
...
placeholder
	...
	for _, tt := range tests {
		...
		placeholder
			...
			cache := &mockTokenCacheForRefreshAPI{lockResult: trueplaceholder
			service, _ := buildPathAService(repo, cache, invalidator)
			blocker := &tokenRefreshRuntimeBlocker{placeholder
			service.SetAccountRuntimeBlocker(blocker)
			refresher := &tokenRefresherStub{err: errors.New("invalid_grant: revoked")placeholder
...
placeholder
	...
	for _, tt := range tests {
		...
		placeholder
			...
			blocker := &tokenRefreshRuntimeBlocker{placeholder
			service.SetAccountRuntimeBlocker(blocker)
			refresher := &tokenRefresherStub{err: errors.New("temporary provider timeout")placeholder
...
func TestTokenRefreshService_GrokMissingConditionalMutationContractContainsProviderCycle(t *testing.T) {
	tests := []struct {
		...
		refreshErr error
placeholder{
		{name: "permanent failure", refreshErr: errors.New("invalid_grant: revoked")placeholder,
		{name: "transient failure", refreshErr: errors.New("temporary provider timeout")placeholder,
placeholder
	...
	for _, tt := range tests {
		...
		placeholder
			account := &Account{
				...
				Status:      StatusActive,
				Schedulable: true,
		placeholder"refresh_token": "attempted"placeholder,
		placeholder
			refresher := &tokenRefresherStub{err: tt.refreshErrplaceholder
			...
			state := &tokenRefreshProviderState{service: svcplaceholder
			state.recordResult(err)
...
placeholder{
		...
		placeholder,
			expectedCASCalls: func(repo *tokenRefreshAccountRepo) int { return repo.conditionalErrorCalls placeholder,
	placeholder,
		{
		...
		placeholder,
			expectedCASCalls: func(repo *tokenRefreshAccountRepo) int { return repo.conditionalTempCalls placeholder,
	placeholder,
placeholder
	...
	for _, tt := range tests {
		...
		placeholder
			...
			refresher := &tokenRefresherStub{err: tt.upstreamErrplaceholder
