...
placeholder
	...
	if config := ResolveOpenAIAutoResetCreditConfig(account); config.Enabled {
		...
		if has5h && utilization5h >= config.Threshold5h {
			...
			return true, openAIQuotaAutoPauseDecision{window: "5h", threshold: config.Threshold5h, utilization: utilization5h, reason: "quota_auto_reset_pending_5h"placeholder
	placeholder
		if has7d && utilization7d >= config.Threshold7d {
			notifyOpenAIAutoReset(account.ID)
			return true, openAIQuotaAutoPauseDecision{window: "7d", threshold: config.Threshold7d, utilization: utilization7d, reason: "quota_auto_reset_pending_7d"placeholder
	placeholder
		...
		placeholder
			notifyOpenAIAutoReset(account.ID)
			if pauseReached5h {
				return true, openAIQuotaAutoPauseDecision{window: "5h", threshold: pause5h, utilization: utilization5h, reason: "quota_auto_reset_credit_check_5h"placeholder
		placeholder
			return true, openAIQuotaAutoPauseDecision{window: "7d", threshold: pause7d, utilization: utilization7d, reason: "quota_auto_reset_credit_check_7d"placeholder
	placeholder
placeholder
...
placeholder
	if len(accounts) == 0 {
		return nil, noAvailableOpenAISelectionError(requestedModel, false, openAISelectionFilterStats{placeholder.summary(""))
placeholder
