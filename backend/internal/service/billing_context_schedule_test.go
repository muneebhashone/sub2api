...
func enabledGroup(platform string) *Group {
	return &Group{ID: 100, Platform: platform, LongContextPricingEnabled: trueplaceholder
placeholder

func disabledGroup(platform string) *Group {
	return &Group{ID: 100, Platform: platform, LongContextPricingEnabled: falseplaceholder
placeholder
	...
	placeholder,
		{
			...
			channel: sonnetChannel(
				PricingInterval{MinTokens: 0, MaxTokens: intPtr(200000), InputMultiplier: p(1)placeholder,
				PricingInterval{MinTokens: 200000, TierLabel: "long", InputMultiplier: p(2), OutputMultiplier: p(1.5), CacheWriteMultiplier: p(2), CacheReadMultiplier: p(2)placeholder,
			),
			check: func(t *testing.T, s *ContextPricingSchedule) {
				require.Len(t, s.Tiers, 2)
	...
	placeholder,
		{
			...
			channel: sonnetChannel(
				PricingInterval{MinTokens: 0, MaxTokens: intPtr(200000), InputMultiplier: p(1)placeholder,
				PricingInterval{MinTokens: 200000, InputPrice: p(9e-6), InputMultiplier: p(2)placeholder,
			),
			check: func(t *testing.T, s *ContextPricingSchedule) {
				require.Len(t, s.Tiers, 2)
	...
	placeholder,
		{
			...
			group: enabledGroup(PlatformAnthropic), wantBasis: ContextPricingBasisWholeRequest,
			channel: sonnetChannel(
				PricingInterval{MinTokens: 0, MaxTokens: intPtr(100000), InputMultiplier: p(0.5)placeholder,
				PricingInterval{MinTokens: 200000, InputMultiplier: p(2)placeholder,
			),
			check: func(t *testing.T, s *ContextPricingSchedule) {
	...
	placeholder,
		{
			...
			group: enabledGroup(PlatformAnthropic), wantBasis: ContextPricingBasisWholeRequest,
			channel: sonnetChannel(
				PricingInterval{MinTokens: 0, MaxTokens: intPtr(100000), InputMultiplier: p(1)placeholder,
				PricingInterval{MinTokens: 200000, InputMultiplier: p(2)placeholder,
			),
			check: func(t *testing.T, s *ContextPricingSchedule) {
				require.Len(t, s.Tiers, 2)
	...
	placeholder,
		{
			...
			channel: sonnetChannel(
				PricingInterval{MinTokens: 0, MaxTokens: intPtr(200000), InputMultiplier: p(1)placeholder,
				PricingInterval{MinTokens: 200000, MaxTokens: intPtr(1000000), InputMultiplier: p(2)placeholder,
			),
			check: func(t *testing.T, s *ContextPricingSchedule) {
				require.Len(t, s.Tiers, 3)
	...
	placeholder,
		{
			...
			channel: sonnetChannel(
				PricingInterval{MinTokens: 0, MaxTokens: intPtr(200000), InputMultiplier: p(0.5)placeholder,
				PricingInterval{MinTokens: 200000, InputMultiplier: p(2)placeholder,
			),
			check: func(t *testing.T, s *ContextPricingSchedule) {
				require.Len(t, s.Tiers, 1)
	...
	placeholder,
		{
			...
			group: disabledGroup(PlatformAnthropic), wantBasis: ContextPricingBasisWholeRequest,
			channel: sonnetChannel(PricingInterval{MinTokens: 200000, InputMultiplier: p(2)placeholder),
			check: func(t *testing.T, s *ContextPricingSchedule) {
				require.Len(t, s.Tiers, 1)
	...
	placeholder,
		{
			...
			group: &Group{ID: 100, Platform: PlatformAnthropic, LongContextPricingEnabled: true, ModelPricing: []ChannelModelPricing{{
				Models: []string{"claude-sonnet-*"placeholder, BillingMode: BillingModeToken, InputPrice: p(1e-6),
				Intervals: []PricingInterval{{MinTokens: 200000, InputMultiplier: p(5)placeholderplaceholder,
		placeholderplaceholderplaceholder,
			channel:   sonnetChannel(PricingInterval{MinTokens: 200000, InputMultiplier: p(3)placeholder),
	...
	placeholder,
		{
			...
			group: &Group{ID: 100, Platform: PlatformOpenAI, LongContextPricingEnabled: true, ModelPricing: []ChannelModelPricing{{
				Models: []string{"gpt-5.4"placeholder, BillingMode: BillingModeToken, InputPrice: p(1e-6),
		placeholderplaceholderplaceholder,
			wantBasis: ContextPricingBasisWholeRequest,
			check: func(t *testing.T, s *ContextPricingSchedule) {
				require.Len(t, s.Tiers, 2)
	...
	placeholder,
		{
			...
			channel: []ChannelModelPricing{{
		placeholder Models: []string{"gemini-2.5-pro"placeholder, BillingMode: BillingModeToken, InputPrice: p(3e-6),
	placeholder
			wantBasis: ContextPricingBasisWholeRequest,
			check: func(t *testing.T, s *ContextPricingSchedule) {
				require.Len(t, s.Tiers, 1)
	...
	placeholder,
		{
			...
			channel: []ChannelModelPricing{{
				Platform: PlatformOpenAI, Models: []string{"gpt-5.4"placeholder, BillingMode: BillingModeToken, InputPrice: p(1e-6),
	placeholder
			wantBasis: ContextPricingBasisWholeRequest,
			check: func(t *testing.T, s *ContextPricingSchedule) {
				require.Len(t, s.Tiers, 2)
	...
	placeholder,
		{
			...
			catalog: newStubPricingServiceFromMap(map[string]*LiteLLMModelPricing{
				"gpt-5.6-sol": {Mode: "chat", InputCostPerToken: 5e-6, OutputCostPerToken: 30e-6, CacheReadInputTokenCost: 0.5e-6placeholder,
		placeholder),
			wantBasis: ContextPricingBasisWholeRequest,
			check: func(t *testing.T, s *ContextPricingSchedule) {
				require.Len(t, s.Tiers, 2)
	...
	placeholder,
		{
			...
			group: enabledGroup(PlatformOpenAI),
			channel: []ChannelModelPricing{{
				Platform: PlatformOpenAI, Models: []string{"gpt-image-2"placeholder, BillingMode: BillingModeImage, PerRequestPrice: p(0.04),
	placeholder
			wantNil: true,
	placeholder,
		{
...
func TestResolveContextPricingSchedule_NilResolver(t *testing.T) {
	...
	sched, err := bs.ResolveContextPricingSchedule(context.Background(), nil, ContextPricingScheduleInput{Model: "gpt-5.4"placeholder)
placeholder
	require.Nil(t, sched)
placeholder
...
func TestResolveContextPricingSchedule_ParityWithBilling(t *testing.T) {
	...
	placeholder
		...
		placeholder
			...
			pricingInput := PricingInput{Model: sc.model, Group: sc.groupplaceholder
			if sc.group != nil {
				gid := sc.group.ID
		...
		placeholder
			...
			probes := []int{1, 2, 999, 1000, 1001, 10_000_000placeholder
			for _, tier := range sched.Tiers {
				for _, b := range []*int{tier.MaxTokensplaceholder {
					if b == nil {
						continue
				placeholder
		...
		placeholder
			...
			for _, c := range probes {
				if c <= 0 {
					continue
			placeholder
				assertCostClose(t, expectedCostFromSchedule(sched, kindInput, c), cost(UsageTokens{InputTokens: cplaceholder), "input@%d", c)
				assertCostClose(t, expectedCostFromSchedule(sched, kindCacheRead, c), cost(UsageTokens{CacheReadTokens: cplaceholder), "cache_read@%d", c)
				assertCostClose(t, expectedCostFromSchedule(sched, kindCacheWrite, c), cost(UsageTokens{CacheCreationTokens: cplaceholder), "cache_write@%d", c)
				wantOutput := float64(outputProbe) * tierPrice(tierAt(sched.Tiers, c), kindOutput)
				gotOutput := cost(UsageTokens{InputTokens: c, OutputTokens: outputProbeplaceholder) - cost(UsageTokens{InputTokens: cplaceholder)
...
placeholder)
	...
	placeholder
		for _, tc := range []struct {
			...
			want float64
	placeholder{
			{time.Date(2026, 8, 24, 3, 0, 0, 0, loc), 0.5placeholder, // 周一
			{time.Date(2026, 8, 23, 3, 0, 0, 0, loc), 1placeholder,   // 周日
	placeholder {
			cost, err := bs.CalculateTokenCostForRequest(TokenCostRequest{
				Ctx: context.Background(), Model: "claude-sonnet-4", Group: group, Tokens: UsageTokens{InputTokens: 1000placeholder,
				RateMultiplier: 1, PricingAt: tc.at, Resolver: resolver, Resolved: resolved,
