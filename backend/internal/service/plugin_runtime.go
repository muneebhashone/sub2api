...
placeholder
	...
	client := hcplugin.NewClient(&hcplugin.ClientConfig{
		...
		Cmd:              cmd,
		AllowedProtocols: []hcplugin.Protocol{hcplugin.ProtocolGRPCplaceholder,
	...
	placeholder,
		...
		UnixSocketConfig: &hcplugin.UnixSocketConfig{TempDir: socketDirplaceholder,
		SkipHostEnv:      true,
placeholder)
...
placeholder
	...
	defer cancel()
	info, err := api.GetInfo(infoCtx, &pluginv1.GetInfoRequest{placeholder)
	if err != nil {
		runtime.kill()
...
placeholder
	health, err := api.Health(infoCtx, &pluginv1.HealthRequest{placeholder)
...
func (r *pluginRuntime) validateAndApplyNormalizedConfig(ctx context.Context, configJSON []byte) ([]byte, error) {
	validation, err := r.api.ValidateConfig(ctx, &pluginv1.ValidateConfigRequest{ConfigJson: configJSONplaceholder)
	if err != nil {
...
placeholder
	applied, err := r.api.ApplyConfig(ctx, &pluginv1.ApplyConfigRequest{ConfigJson: configJSONplaceholder)
	if err != nil {
...
func (r *pluginRuntime) finishRequest() {
	if r.inFlight.Add(-1) == 0 && r.draining.Load() {
		r.doneOnce.Do(func() { close(r.done) placeholder)
placeholder
placeholder
...
placeholderplaceholderplaceholder); err != nil {
		cancel()
...
placeholder
	if frameError := first.GetError(); frameError != nil {
		cancel()
		return nil, &PluginTransportError{Code: frameError.Code, Message: frameError.Message, RequestSent: frameError.RequestSentplaceholder
placeholder
...
func sendPluginRequestBody(stream pluginv1.TransportPlugin_ForwardClient, body io.ReadCloser) error {
	if body != nil {
		...
		for {
			...
			if read > 0 {
				...
				if sendErr := stream.Send(&pluginv1.ForwardRequest{Frame: &pluginv1.ForwardRequest_BodyChunk{BodyChunk: chunkplaceholderplaceholder); sendErr != nil {
					return sendErr
			placeholder
		placeholder
			if err == io.EOF {
	...
	placeholder
placeholder
	if err := stream.Send(&pluginv1.ForwardRequest{Frame: &pluginv1.ForwardRequest_BodyEnd{BodyEnd: trueplaceholderplaceholder); err != nil {
		return err
placeholder
...
func receivePluginResponseBody(stream pluginv1.TransportPlugin_ForwardClient, writer *io.PipeWriter, sendErr <-chan error) {
	...
	placeholder
		if frameError := frame.GetError(); frameError != nil {
			_ = writer.CloseWithError(&PluginTransportError{Code: frameError.Code, Message: frameError.Message, RequestSent: frameError.RequestSentplaceholder)
			return
	placeholder
placeholder
placeholder
...
func headersToPlugin(headers http.Header) map[string]*pluginv1.HeaderValues {
	...
	for key, values := range headers {
		out[key] = &pluginv1.HeaderValues{Values: append([]string(nil), values...)placeholder
placeholder
	return out
placeholder
