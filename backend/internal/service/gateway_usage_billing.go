...
placeholder else {
		...
		placeholder else if deps.billingCacheService != nil {
...
placeholder else if p.Cost.ActualCost > 0 {
...
placeholder
	...
	placeholder
placeholder else if p.Cost.ActualCost > 0 && p.User != nil {
...
placeholder, &recordUsageOpts{placeholder)
placeholder
...
placeholder
	...
	if resolved := s.resolveChannelPricing(ctx, billingModel, apiKey); resolved != nil {
		...
		cost, err = s.billingService.CalculateCostUnified(CostInput{
			...
			Resolved:       resolved,
	placeholder)
placeholder else if opts.LongContextThreshold > 0 {
