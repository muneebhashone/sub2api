...
placeholder
	...
	return &Airwallex{
		...
		config:     cfg,
		httpClient: &http.Client{Timeout: airwallexHTTPTimeoutplaceholder,
placeholder, nil
placeholder
...
func (a *Airwallex) Name() string        { return "空中云汇" placeholder
func (a *Airwallex) ProviderKey() string { return payment.TypeAirwallex placeholder
func (a *Airwallex) SupportedTypes() []payment.PaymentType {
	return []payment.PaymentType{payment.TypeAirwallexplaceholder
placeholder
...
func (a *Airwallex) MerchantIdentityMetadata() map[string]string {
	if a == nil {
		return nil
placeholder
	metadata := map[string]string{"currency": a.currency()placeholder
...
placeholder
	if strings.TrimSpace(resp.ID) == "" {
		resp.ID = refundID
placeholder
	return &payment.RefundResponse{RefundID: resp.ID, Status: airwallexRefundProviderStatus(resp.Status)placeholder, nil
placeholder
...
placeholder
	if accountID = strings.TrimSpace(accountID); accountID != "" {
		metadata["account_id"] = accountID
placeholder else if configured := strings.TrimSpace(a.config["accountId"]); configured != "" {
...
func (a *Airwallex) accessToken(ctx context.Context) (string, error) {
	...
	rawState, _ := airwallexAccessTokens.LoadOrStore(cacheKey, &airwallexTokenState{placeholder)
...
func (a *Airwallex) do(req *http.Request) ([]byte, int, error) {
	client := a.httpClient
	if client == nil {
		client = &http.Client{Timeout: airwallexHTTPTimeoutplaceholder
placeholder
...
func parseAirwallexWebhookTimestamp(raw string) (time.Time, error) {
	...
	if err != nil {
		return time.Time{placeholder, fmt.Errorf("airwallex invalid webhook timestamp")
placeholder
	millis := ts.IntPart()
	if millis <= 0 {
		return time.Time{placeholder, fmt.Errorf("airwallex invalid webhook timestamp")
...
func parseAirwallexTime(raw string) (time.Time, error) {
	...
	if raw == "" {
		return time.Time{placeholder, fmt.Errorf("empty time")
placeholder
	for _, layout := range []string{time.RFC3339, "2006-01-02T15:04:05-0700", "2006-01-02T15:04:05.000-0700"placeholder {
		if t, err := time.Parse(layout, raw); err == nil {
			return t, nil
	placeholder
placeholder
	return time.Time{placeholder, fmt.Errorf("invalid time: %s", raw)
placeholder
...
func newAirwallexRequestAmount(amount decimal.Decimal) airwallexRequestAmount {
	return airwallexRequestAmount{Decimal: amountplaceholder
placeholder
