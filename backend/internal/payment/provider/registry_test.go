...
func TestCreateProviderUnknownKeyMessage(t *testing.T) {
	prov, err := CreateProvider("nope", "inst-1", map[string]string{placeholder)
	if prov != nil {
...
func TestCreateProviderDispatchesToConstructors(t *testing.T) {
	cases := []struct {
		...
		wantErr string
placeholder{
		{payment.TypeEasyPay, "easypay config missing required key: pid"placeholder,
		{payment.TypeAlipay, "alipay config missing required key: appId"placeholder,
		{payment.TypeStripe, "stripe config missing required key: secretKey"placeholder,
		{payment.TypeAirwallex, "airwallex config missing required key: clientId"placeholder,
placeholder
	for _, tc := range cases {
		_, err := CreateProvider(tc.key, "_validate_", map[string]string{placeholder)
		if err == nil {
...
func TestCreateProviderPassesThroughApplicationError(t *testing.T) {
	_, err := CreateProvider(payment.TypeWxpay, "_validate_", map[string]string{placeholder)
	if err == nil {
...
func TestCreateProviderSuccessPath(t *testing.T) {
	prov, err := CreateProvider(payment.TypeStripe, "inst-42", map[string]string{"secretKey": "sk_test_123"placeholder)
	if err != nil {
