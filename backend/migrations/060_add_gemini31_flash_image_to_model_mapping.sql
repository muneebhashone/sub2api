...
SET credentials = jsonb_set(
    jsonb_set(
        jsonb_set(
            jsonb_set(
                credentials,
                '{model_mapping,gemini-3.1-flash-imageplaceholder',
                '"gemini-3.1-flash-image"'::jsonb,
                true
            ),
            '{model_mapping,gemini-3.1-flash-image-previewplaceholder',
            '"gemini-3.1-flash-image"'::jsonb,
            true
        ),
        '{model_mapping,gemini-3-pro-imageplaceholder',
        '"gemini-3.1-flash-image"'::jsonb,
        true
    ),
    '{model_mapping,gemini-3-pro-image-previewplaceholder',
    '"gemini-3.1-flash-image"'::jsonb,
    true
)
