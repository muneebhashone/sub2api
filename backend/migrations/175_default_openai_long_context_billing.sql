...
BEGIN
    ...
    NEW.extra := COALESCE(NEW.extra, '{placeholder'::jsonb);
    IF NEW.parent_account_id IS NOT NULL AND NEW.quota_dimension = 'spark' THEN
        SELECT CASE
            ...
            WHEN NOT (COALESCE(parent.extra, '{placeholder'::jsonb) ? 'openai_long_context_billing_enabled') THEN 'false'::jsonb
        ...
        NEW.extra := jsonb_set(
            NEW.extra,
            '{openai_long_context_billing_enabledplaceholder',
...
BEGIN
    WITH updated_shadows AS (
        ...
        SET extra = jsonb_set(
            COALESCE(shadow.extra, '{placeholder'::jsonb),
            '{openai_long_context_billing_enabledplaceholder',
...
WHERE platform = 'openai'
  AND COALESCE(extra, '{placeholder'::jsonb) ? 'openai_long_context_billing_enabled'
...
WHERE platform = 'openai'
  ...
  AND NOT (COALESCE(extra, '{placeholder'::jsonb) ? 'openai_long_context_billing_enabled');
