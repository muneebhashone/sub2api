...
placeholder
	...
	if *jsonOut {
		...
		enc.SetIndent("", "  ")
		if err := enc.Encode(map[string]any{"evaluated_at": evalAt, "reports": reportsplaceholder); err != nil {
...
placeholder
	...
	for _, report := range reports {
		...
		counts := map[string]int{placeholder
		...
		placeholder
			if len(a.ModelMapping) > 0 {
				...
				for k, v := range a.ModelMapping {
					mapping[k] = v
			placeholder
				account.Credentials = map[string]any{"model_mapping": mappingplaceholder
		placeholder
