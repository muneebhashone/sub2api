	...
	raw := []byte(`{
		...
		placeholder,
			"accounts": [{
				...
				"rate_multiplier": 0.2
		placeholder],
			"user_overrides": {"40": null, "41": 0.4placeholder,
			"models": ["gpt-test"]
	placeholder]
placeholder`)
...
placeholder
	...
	require.Equal(t, map[int64]float64{41: 0.4placeholder, inputs[0].UserOverrides)
...
func TestParsePreviewInputsRejectsEmptyGroups(t *testing.T) {
	for _, raw := range [][]byte{
		[]byte(`{"groups":nullplaceholder`),
		[]byte(`{"groups":[]placeholder`),
placeholder {
...
placeholder
	...
	want := []string{"min-d-zero", "min-d-zero-alt"placeholder
	if len(got) != len(want) {
