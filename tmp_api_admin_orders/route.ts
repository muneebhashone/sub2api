...
import { Prisma placeholder from '@prisma/client';
...
export async function GET(request: NextRequest) {
  ...
  const where: Prisma.OrderWhereInput = {placeholder;
  if (status) where.status = status as any;
  if (userId) where.userId = Number(userId);
  if (dateFrom || dateTo) {
    where.createdAt = {placeholder;
  ...
  const [orders, total] = await Promise.all([
    prisma.order.findMany({
      ...
      placeholder,
    placeholder),
    prisma.order.count({ where placeholder),
  ]);
