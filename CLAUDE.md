...
- 各环境的 `docker-compose.override.yml` 已通过 `depends_on: !reset {placeholder` 和 `redis: profiles: [disabled]` 去掉了对容器 Redis 的依赖
...
# curl -fsSL "https://github.com/docker/buildx/releases/download/${LATESTplaceholder/buildx-${LATESTplaceholder.linux-amd64" -o ~/.docker/cli-plugins/docker-buildx
...
# 2) 确认现网容器还在（只读检查）
ssh clicodeplus "docker ps --format 'table {{.Namesplaceholderplaceholder\t{{.Imageplaceholderplaceholder\t{{.Portsplaceholderplaceholder' | sed -n '1,200p'"
```
...
services:
  sub2api:
    ...
    environment:
      - DATABASE_HOST=${DATABASE_HOST:-postgresplaceholder
      - DATABASE_SSLMODE=${DATABASE_SSLMODE:-disableplaceholder
      - REDIS_HOST=db.clicodeplus.com
    depends_on: !reset {placeholder
  redis:
    profiles:
      - disabled
YAML
...
> 以下接口文档中，`${BASEplaceholder` 代表环境基础地址，`${KEYplaceholder` 代表用户提供的 admin token。
...
```bash
curl -s "${BASEplaceholder/api/v1/admin/accounts?platform=antigravity&page=1&page_size=100" \
...
```json
{
  ...
  "message": "success",
  "data": {
    "items": [{"id": 1, "name": "xxx@gmail.com", "platform": "antigravity", "status": "active", ...placeholder],
    "total": 66
  placeholder
placeholder
```
...
```bash
curl -s "${BASEplaceholder/api/v1/admin/accounts/1" -H "x-api-key: ${KEYplaceholder"
```
...
```bash
curl -N -X POST "${BASEplaceholder/api/v1/admin/accounts/1/test" \
  -H "x-api-key: ${KEYplaceholder" \
  -H "Content-Type: application/json" \
  -d '{"model_id": "claude-opus-4-6"placeholder'
```
...
```bash
curl -X POST "${BASEplaceholder/api/v1/admin/accounts/1/clear-rate-limit" \
  -H "x-api-key: ${KEYplaceholder"
```
...
```bash
curl -X POST "${BASEplaceholder/api/v1/admin/accounts/1/clear-error" \
  -H "x-api-key: ${KEYplaceholder"
```
...
```bash
curl -s "${BASEplaceholder/api/v1/admin/accounts/1/models" -H "x-api-key: ${KEYplaceholder"
```
...
```bash
curl -X POST "${BASEplaceholder/api/v1/admin/accounts/1/refresh" -H "x-api-key: ${KEYplaceholder"
```
...
```bash
curl -X POST "${BASEplaceholder/api/v1/admin/accounts/1/refresh-tier" -H "x-api-key: ${KEYplaceholder"
```
...
```bash
curl -s "${BASEplaceholder/api/v1/admin/accounts/1/stats" -H "x-api-key: ${KEYplaceholder"
```
...
```bash
curl -s "${BASEplaceholder/api/v1/admin/accounts/1/usage" -H "x-api-key: ${KEYplaceholder"
```
...
```bash
# 示例：更新账号优先级为 100
curl -X PUT "${BASEplaceholder/api/v1/admin/accounts/1" \
  -H "x-api-key: ${KEYplaceholder" \
  -H "Content-Type: application/json" \
  -d '{"priority": 100placeholder'
```
...
```bash
# 示例：批量设置多个账号优先级为 100
curl -X POST "${BASEplaceholder/api/v1/admin/accounts/bulk-update" \
  -H "x-api-key: ${KEYplaceholder" \
  -H "Content-Type: application/json" \
  -d '{"account_ids": [1, 2, 3], "priority": 100placeholder'
```
...
ACCOUNT_IDS=$(curl -s "${BASEplaceholder/api/v1/admin/accounts?platform=antigravity&page=1&page_size=100" \
  -H "x-api-key: ${KEYplaceholder" | python3 -c "
import json, sys
data = json.load(sys.stdin)
for item in data['data']['items']:
    print(f\"{item['id']placeholder|{item['name']placeholder\")
")

while IFS='|' read -r ID NAME; do
    echo "测试账号 ID=${IDplaceholder (${NAMEplaceholder)..."
    RESPONSE=$(curl -s --max-time 60 -N \
      -X POST "${BASEplaceholder/api/v1/admin/accounts/${IDplaceholder/test" \
      -H "x-api-key: ${KEYplaceholder" \
      -H "Content-Type: application/json" \
      -d "{\"model_id\": \"${MODELplaceholder\"placeholder" 2>&1)
...
```bash
curl -s "${BASEplaceholder/api/v1/admin/ops/concurrency" -H "x-api-key: ${KEYplaceholder"
```
...
```bash
curl -s "${BASEplaceholder/api/v1/admin/ops/account-availability" -H "x-api-key: ${KEYplaceholder"
```
...
```bash
curl -s "${BASEplaceholder/api/v1/admin/ops/realtime-traffic" -H "x-api-key: ${KEYplaceholder"
```
...
```bash
curl -s "${BASEplaceholder/api/v1/admin/ops/request-errors?page=1&page_size=50" \
  -H "x-api-key: ${KEYplaceholder"
```
...
```bash
curl -s "${BASEplaceholder/api/v1/admin/ops/upstream-errors?page=1&page_size=50" \
  -H "x-api-key: ${KEYplaceholder"
```
...
```bash
curl -s "${BASEplaceholder/api/v1/admin/ops/dashboard/overview" -H "x-api-key: ${KEYplaceholder"
```
...
```bash
curl -s "${BASEplaceholder/api/v1/admin/settings" -H "x-api-key: ${KEYplaceholder"
```
...
```bash
curl -X PUT "${BASEplaceholder/api/v1/admin/settings" \
  -H "x-api-key: ${KEYplaceholder" \
  -H "Content-Type: application/json" \
  -d '{ ... placeholder'
```
...
```bash
curl -s "${BASEplaceholder/api/v1/admin/settings/admin-api-key" -H "x-api-key: ${KEYplaceholder"
```
...
```bash
curl -s "${BASEplaceholder/api/v1/admin/users?page=1&page_size=20" -H "x-api-key: ${KEYplaceholder"
```
...
```bash
curl -s "${BASEplaceholder/api/v1/admin/users/1" -H "x-api-key: ${KEYplaceholder"
```
...
```bash
curl -X POST "${BASEplaceholder/api/v1/admin/users/1/balance" \
  -H "x-api-key: ${KEYplaceholder" \
  -H "Content-Type: application/json" \
  -d '{"amount": 100, "reason": "充值"placeholder'
```
...
```bash
curl -s "${BASEplaceholder/api/v1/admin/groups" -H "x-api-key: ${KEYplaceholder"
```
...
```bash
curl -s "${BASEplaceholder/api/v1/admin/groups/all" -H "x-api-key: ${KEYplaceholder"
```
