...
function usage() {
  ...
  sub2api-admin.js accounts create --json '{...placeholder' | --file account.json
  sub2api-admin.js accounts update <id> --json '{...placeholder' | --file patch.json
  ...
  sub2api-admin.js accounts apply-oauth <id> --json '{...placeholder' | --file credentials.json
  sub2api-admin.js accounts batch-create --file accounts.json
  sub2api-admin.js accounts batch-update-credentials --json '{...placeholder' | --file payload.json
  sub2api-admin.js accounts bulk-update --ids 1,2 --json '{...placeholder' | --file patch.json
  ...
  sub2api-admin.js accounts crs-preview --json '{...placeholder' | --file payload.json
  sub2api-admin.js accounts crs-sync --json '{...placeholder' | --file payload.json
  sub2api-admin.js accounts import-codex-session --json '{...placeholder' | --file payload.json
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    if (token.startsWith("--")) {
      ...
      placeholder else {
        if (flags[key] === undefined) {
          flags[key] = next;
        placeholder else if (Array.isArray(flags[key])) {
          flags[key].push(next);
        placeholder else {
    ...
    placeholder
  placeholder
  return { positional, flags placeholder;
placeholder

function authHeaders() {
  if (!BASE_URL) throw new Error("Missing SUB2API_BASE_URL");
  if (ADMIN_API_KEY) return { "x-api-key": ADMIN_API_KEY placeholder;
  if (ADMIN_JWT) return { Authorization: `Bearer ${ADMIN_JWTplaceholder` placeholder;
...
function encodeQuery(params) {
  ...
  for (const [key, value] of Object.entries(params)) {
    ...
    pairs.push(`${encodeURIComponent(key)placeholder=${encodeURIComponent(String(value))placeholder`);
  placeholder
  return pairs.length ? `?${pairs.join("&")placeholder` : "";
placeholder

function normalizeAdminPath(pathname) {
  if (!pathname.startsWith("/")) pathname = `/${pathnameplaceholder`;
  if (pathname.startsWith("/api/v1/admin/")) return pathname;
  if (pathname.startsWith("/admin/")) return `/api/v1${pathnameplaceholder`;
  return pathname;
placeholder
...
async function listAccounts(options = {placeholder) {
  ...
  placeholder;
  return apiRequest("GET", `/api/v1/admin/accounts${encodeQuery(params)placeholder`);
placeholder

async function getAccount(id) {
  return apiRequest("GET", `/api/v1/admin/accounts/${idplaceholder`);
placeholder

async function deleteAccount(id) {
  return apiRequest("DELETE", `/api/v1/admin/accounts/${idplaceholder`);
placeholder
...
function parseBool(value, name = "value") {
  ...
  throw new Error(`${nameplaceholder must be true or false`);
placeholder
...
function parseIds(value) {
  ...
  return String(value)
    ...
    .map((id) => {
      const n = Number(id);
      if (!Number.isFinite(n)) throw new Error(`invalid id: ${idplaceholder`);
      return n;
    placeholder);
placeholder

function readJsonPayload(flags, { required = true placeholder = {placeholder) {
...
async function accountData(flags) {
  ...
  placeholder
  return adminRequest("GET", `/admin/accounts/data${encodeQuery(params)placeholder`);
placeholder
...
async function commandAccounts(args) {
  ...
  if (sub === "export") {
    ...
    if (args.flags.file) {
      ...
      printJson({ file: path.resolve(args.flags.file), count: Array.isArray(data) ? data.length : data.count placeholder);
    placeholder else {
      printJson(data);
    placeholder
    return;
  ...
  if (sub === "update") {
    ...
    printJson(await adminRequest("PUT", `/admin/accounts/${idplaceholder`, readJsonPayload(args.flags)));
    return;
  placeholder
  ...
  if (sub === "keep-only") {
    ...
    const data = await listAccounts({ pageSize: Number(args.flags["page-size"] || 500) placeholder);
    const items = data.items || [];
    const keep = items.find((item) => item.name === name);
    if (!keep) throw new Error(`account not found: ${nameplaceholder`);
    const targets = items.filter((item) => item.name !== name);
    const results = [];
    for (const item of targets) {
      const out = await deleteAccount(item.id);
      results.push({ id: item.id, name: item.name, result: out placeholder);
    placeholder
    printJson({ kept: { id: keep.id, name: keep.name placeholder, deleted: results placeholder);
    return;
  placeholder
  ...
  if (sub === "usage") {
    ...
    const params = encodeQuery({ source: args.flags.source, force: args.flags.force ? "true" : undefined placeholder);
    printJson(await adminRequest("GET", `/admin/accounts/${idplaceholder/usage${paramsplaceholder`));
    return;
  placeholder
  ...
  if (sub === "stats") {
    ...
    printJson(await adminRequest("GET", `/admin/accounts/${idplaceholder/stats${encodeQuery({ days: args.flags.days || 30 placeholder)placeholder`));
    return;
  placeholder
  ...
  if (sub === "today-stats") {
    ...
    printJson(await adminRequest("GET", `/admin/accounts/${idplaceholder/today-stats`));
    return;
  placeholder

  if (sub === "batch-today-stats") {
    printJson(await adminRequest("POST", "/admin/accounts/today-stats/batch", { account_ids: parseIds(args.flags.ids) placeholder));
    return;
  placeholder
  ...
  if (sub === "set-status") {
    ...
    printJson(await adminRequest("PUT", `/admin/accounts/${idplaceholder`, { status placeholder));
    return;
  placeholder
  ...
  if (sub === "set-schedulable") {
    ...
    printJson(await adminRequest("POST", `/admin/accounts/${idplaceholder/schedulable`, { schedulable: parseBool(schedulable, "schedulable") placeholder));
    return;
  placeholder
  ...
  if (noBodyPostById[sub]) {
    const id = args.positional[2];
    if (!id) throw new Error(`accounts ${subplaceholder requires <id>`);
    printJson(await adminRequest("POST", `/admin/accounts/${idplaceholder/${noBodyPostById[sub]placeholder`));
    return;
  placeholder
  ...
  if (sub === "models") {
    ...
    printJson(await adminRequest("GET", `/admin/accounts/${idplaceholder/models`));
    return;
  placeholder
  ...
  if (sub === "apply-oauth") {
    ...
    printJson(await adminRequest("POST", `/admin/accounts/${idplaceholder/apply-oauth-credentials`, readJsonPayload(args.flags)));
    return;
  placeholder
  ...
  if (sub === "batch-create") {
    ...
    if (!Array.isArray(accounts)) throw new Error("batch-create payload must be an array or {accounts:[...]placeholder");
    printJson(await adminRequest("POST", "/admin/accounts/batch", { accounts placeholder));
    return;
  placeholder
  ...
  if (sub === "bulk-update") {
    const patch = readJsonPayload(args.flags, { required: false placeholder) || {placeholder;
    const payload = args.flags.ids ? { account_ids: parseIds(args.flags.ids), ...patch placeholder : patch;
  ...
  if (sub === "batch-refresh") {
    printJson(await adminRequest("POST", "/admin/accounts/batch-refresh", { account_ids: parseIds(args.flags.ids) placeholder));
    return;
  placeholder

  if (sub === "batch-clear-error") {
    printJson(await adminRequest("POST", "/admin/accounts/batch-clear-error", { account_ids: parseIds(args.flags.ids) placeholder));
    return;
  placeholder
  ...
  if (sub === "temp-unschedulable") {
    ...
    printJson(await adminRequest("GET", `/admin/accounts/${idplaceholder/temp-unschedulable`));
    return;
  placeholder
  ...
  if (sub === "reset-temp-unschedulable") {
    ...
    printJson(await adminRequest("DELETE", `/admin/accounts/${idplaceholder/temp-unschedulable`));
    return;
  placeholder
  ...
  if (sub === "import-json") {
    ...
    const live = await listAccounts({ pageSize: 500 placeholder);
    const liveItems = live.items || [];
    const template = liveItems.find((item) => item.name === templateName);
    if (!template) throw new Error(`template account not found in backend: ${templateNameplaceholder`);
    ...
    const templateModelMapping = (template.credentials && template.credentials.model_mapping) || {placeholder;
    ...
    for (const acc of planned) {
      if (existingNames.has(acc.name)) {
        results.push({ name: acc.name, skipped: true, reason: "already exists" placeholder);
        continue;
      placeholder
      const payload = {
        ...acc,
      ...
      results.push({ name: acc.name, id: created.id, skipped: false placeholder);
    placeholder
    printJson({ imported: results placeholder);
    return;
  placeholder

  throw new Error(`unknown accounts subcommand: ${sub || "(missing)"placeholder`);
placeholder
...
async function commandGroups(args) {
  ...
  if (sub === "all") {
    ...
    return;
  placeholder
  throw new Error(`unknown groups subcommand: ${sub || "(missing)"placeholder`);
placeholder
...
async function commandCrudResource(args, name, basePath, options = {placeholder) {
  ...
  placeholder
  if (sub === "get") {
    if (!id) throw new Error(`${nameplaceholder get requires <id>`);
    printJson(await adminRequest("GET", `${basePathplaceholder/${idplaceholder`));
    return;
  placeholder
  if (sub === "create") {
  ...
  placeholder
  if (sub === "update") {
    if (!id) throw new Error(`${nameplaceholder update requires <id>`);
    printJson(await adminRequest("PUT", `${basePathplaceholder/${idplaceholder`, readJsonPayload(args.flags)));
    return;
  placeholder
  if (sub === "delete") {
    if (!id) throw new Error(`${nameplaceholder delete requires <id>`);
    printJson(await adminRequest("DELETE", `${basePathplaceholder/${idplaceholder`));
    return;
  placeholder
  if (options.toggle && sub === "toggle") {
    const enabled = args.positional[3];
    if (!id || enabled === undefined) throw new Error(`${nameplaceholder toggle requires <id> <true|false>`);
    printJson(await adminRequest("PUT", `${basePathplaceholder/${idplaceholder`, { enabled: parseBool(enabled, "enabled") placeholder));
    return;
  placeholder
  throw new Error(`unknown ${nameplaceholder subcommand: ${sub || "(missing)"placeholder`);
placeholder
...
async function commandApi(args) {
  ...
  const body = readJsonPayload(args.flags, { required: false placeholder);
...
async function main() {
  ...
  placeholder
  if (root === "error-rules") {
    await commandCrudResource(args, "error-rules", "/admin/error-passthrough-rules", { toggle: true placeholder);
    return;
  placeholder
  ...
  if (root === "api") {
    ...
    return;
  placeholder
  throw new Error(`unknown command: ${rootplaceholder`);
placeholder
