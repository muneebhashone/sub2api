...
it('submits bulk edit using current filters when target mode is filtered-results', async () => {
  ...
  // expect adminAPI.accounts.bulkUpdate to receive { filters: ... placeholder rather than account_ids
placeholder)
```
...
export type AccountBulkEditTarget =
  | { mode: 'selected'; accountIds: number[]; selectedPlatforms: AccountPlatform[]; selectedTypes: AccountType[] placeholder
  | { mode: 'filtered'; filters: AccountListFilters; previewCount: number; selectedPlatforms: AccountPlatform[]; selectedTypes: AccountType[] placeholder
```
...
if (props.target.mode === 'selected') {
  await adminAPI.accounts.bulkUpdate({ account_ids: props.target.accountIds, ...updates placeholder)
placeholder else {
  await adminAPI.accounts.bulkUpdate({ filters: props.target.filters, ...updates placeholder)
placeholder
```
