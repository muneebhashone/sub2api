  ...
  <AppLayout>
    <TablePageLayout>
      <template #filters>
            ...
            <template #after>
              ...
              <div class="relative" ref="autoRefreshDropdownRef">
                ...
                >
                  ...
                  <span class="hidden md:inline">
                    {{
                      autoRefreshEnabled
                        ? t('admin.accounts.autoRefreshCountdown', { seconds: autoRefreshCountdown placeholder)
                ...
                >
                  <div class="p-2">
                    ...
                    >
                      <span>{{ t('admin.accounts.enableAutoRefresh') placeholderplaceholder</span>
                    ...
                    <button
                      ...
                      class="flex w-full items-center justify-between rounded-md px-3 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:text-gray-200 dark:hover:bg-dark-700"
                    >
                      <span>{{ autoRefreshIntervalLabel(sec) placeholderplaceholder</span>
              ...
              <div class="relative" ref="accountToolsDropdownRef">
                ...
                >
                  ...
                  <span class="hidden md:inline">{{ t('admin.accounts.moreActions') placeholderplaceholder</span>
                ...
                <Teleport to="body">
                  <div
                    ...
                    @click.stop
                  >
                    <div class="overflow-y-auto p-2" :style="{ maxHeight: `${accountToolsDropdownPosition.maxHeightplaceholderpx` placeholder">
                      <div class="px-2 py-2">
                        <div class="text-xs font-semibold uppercase tracking-wide text-gray-400 dark:text-gray-500">
                          {{ t('admin.accounts.dataActions') placeholderplaceholder
                        </div>
                      </div>
                      <button class="account-tools-menu-item" @click="openSyncFromCrs">
                        ...
                        <span class="flex-1 text-left">{{ t('admin.accounts.syncFromCrs') placeholderplaceholder</span>
                      </button>
                      <button class="account-tools-menu-item" @click="openImportData">
                        <span class="account-tools-menu-icon bg-emerald-50 text-emerald-600 dark:bg-emerald-900/30 dark:text-emerald-300">
                          <Icon name="upload" size="sm" />
                        </span>
                        <span class="flex-1 text-left">{{ t('admin.accounts.dataImport') placeholderplaceholder</span>
                      </button>
                      <button class="account-tools-menu-item" @click="openExportDataDialogFromMenu">
                        ...
                        <span class="flex-1 text-left">
                          {{ selIds.length ? t('admin.accounts.dataExportSelected') : t('admin.accounts.dataExport') placeholderplaceholder
                        </span>
                        <span
                          v-if="selIds.length"
                        ...
                        >
                          {{ t('admin.accounts.selectedCount', { count: selIds.length placeholder) placeholderplaceholder
                        </span>
                      </button>
                      ...
                      <div class="px-2 py-2">
                        <div class="text-xs font-semibold uppercase tracking-wide text-gray-400 dark:text-gray-500">
                          {{ t('admin.accounts.toolActions') placeholderplaceholder
                        </div>
                      </div>
                      <button class="account-tools-menu-item" @click="openErrorPassthrough">
                        ...
                        <span class="flex-1 text-left">{{ t('admin.errorPassthrough.title') placeholderplaceholder</span>
                      </button>
                      <button class="account-tools-menu-item" @click="openTLSFingerprintProfiles">
                        <span class="account-tools-menu-icon bg-slate-100 text-slate-600 dark:bg-slate-700 dark:text-slate-200">
                          <Icon name="lock" size="sm" />
                        </span>
                        <span class="flex-1 text-left">{{ t('admin.tlsFingerprintProfiles.title') placeholderplaceholder</span>
                      </button>
                      ...
                      <div class="px-2 py-2">
                        <div class="flex items-center justify-between gap-3">
                          <span class="text-xs font-semibold uppercase tracking-wide text-gray-400 dark:text-gray-500">
                            {{ t('admin.accounts.viewColumns') placeholderplaceholder
                          </span>
                      ...
                      <div class="grid grid-cols-1 gap-1">
                        ...
                        >
                          <span class="truncate">{{ col.label placeholderplaceholder</span>
        ...
        >
          <span>{{ t('admin.accounts.listPendingSyncHint') placeholderplaceholder</span>
          <button
            class="btn btn-secondary px-2 py-1 text-xs"
            @click="syncPendingListChanges"
          >
            {{ t('admin.accounts.listPendingSyncAction') placeholderplaceholder
          </button>
        </div>
      </template>
      <template #table>
        ...
        >
          ...
          <template #cell-name="{ row, value placeholder">
            <div class="flex flex-col">
              ...
              <span v-else class="font-medium text-gray-900 dark:text-white">{{ value placeholderplaceholder</span>
              <span
                v-if="accountDisplayEmail(row)"
                class="text-xs text-gray-500 dark:text-gray-400 truncate max-w-[200px]"
                :title="accountDisplayEmail(row) + (row.parent_chatgpt_account_id ? ' · ' + row.parent_chatgpt_account_id : '')"
              >
                {{ accountDisplayEmail(row) placeholderplaceholder
              </span>
            </div>
          </template>
          ...
          <template #cell-platform_type="{ row placeholder">
            <div class="flex min-w-0 flex-col gap-1">
              <div class="flex flex-wrap items-center gap-1">
                ...
                >
                  {{ getAntigravityTierLabel(row) placeholderplaceholder
                </span>
              </div>
              <div
              ...
              >
                ...
                <span>{{ getOpenAICompactMeta(row)?.label placeholderplaceholder</span>
              </div>
            </div>
          </template>
          ...
          <template #header-usage="{ column placeholder">
          ...
          <template #cell-proxy="{ row placeholder">
            <div class="flex flex-col gap-1">
              ...
              <div v-if="row.proxy && row.proxy.expires_at" class="flex items-center gap-2 text-xs">
                <span class="text-gray-600 dark:text-gray-300">{{ formatDateTime(row.proxy.expires_at) placeholderplaceholder</span>
                <span :class="proxyExpiryBadge(row.proxy)">{{ proxyExpiryText(row.proxy) placeholderplaceholder</span>
              </div>
              <div v-if="row.proxy_fallback_origin_id" class="flex items-center gap-1">
                <span class="inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-200" :title="t('admin.accounts.fallbackActiveTip', { origin: row.proxy_fallback_origin_name placeholder)">
                  {{ t('admin.accounts.fallbackActive') placeholderplaceholder
                </span>
                <button class="text-xs px-1.5 py-0.5 rounded border border-gray-300 dark:border-dark-600 text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-dark-700" @click="onRevertFallback(row)">{{ t('admin.accounts.revertProxy') placeholderplaceholder</button>
              </div>
            </div>
          </template>
          <template #cell-rate_multiplier="{ row placeholder">
            <span class="inline-flex items-center gap-1 text-sm font-mono text-gray-700 dark:text-gray-300">
              <span>{{ formatMultiplier(row.rate_multiplier ?? 1) placeholderplaceholderx</span>
              <span
    ...
    <ConfirmDialog :show="showCreateShadowDialog" :title="t('admin.accounts.createSparkShadow')" :message="t('admin.accounts.createSparkShadowConfirm', { name: creatingShadowAcc?.name placeholder)" @confirm="confirmCreateSparkShadow" @cancel="showCreateShadowDialog = false" />
    <ConfirmDialog :show="showExportDataDialog" :title="t('admin.accounts.dataExport')" :message="t('admin.accounts.dataExportConfirmMessage')" :confirm-text="t('admin.accounts.dataExportConfirm')" :cancel-text="t('common.cancel')" @confirm="handleExportData" @cancel="showExportDataDialog = false">
      <label class="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
        <input type="checkbox" class="h-4 w-4 rounded border-gray-300 text-primary-600 focus:ring-primary-500" v-model="includeProxyOnExport" />
        <span>{{ t('admin.accounts.dataExportIncludeProxies') placeholderplaceholder</span>
      </label>
    </ConfirmDialog>
...
import { fetchAllAccountIds placeholder from '@/utils/accountSelection'
import { buildGrokUsageRefreshKey, buildOpenAIUsageRefreshKey placeholder from '@/utils/accountUsageRefresh'
...
import { getFloatingPanelPosition placeholder from '@/utils/floatingPanel'
import { formatMultiplier placeholder from '@/utils/formatters'
import type { Account, AccountPlatform, AccountSchedulerGroupScore, AccountType, AccountUsageInfo, Proxy as AccountProxy, AdminGroup, WindowStats, ClaudeModel, UpstreamBillingProbeSnapshot placeholder from '@/types'
...
const accountToolsDropdownStyle = computed(() => ({
  top: accountToolsDropdownPosition.top == null ? 'auto' : `${accountToolsDropdownPosition.topplaceholderpx`,
  bottom: accountToolsDropdownPosition.bottom == null ? 'auto' : `${accountToolsDropdownPosition.bottomplaceholderpx`,
  left: `${accountToolsDropdownPosition.leftplaceholderpx`,
  width: `${accountToolsDropdownPosition.widthplaceholderpx`
placeholder))
...
const loadInitialAccountSortState = (): AccountSortState => {
  const fallback: AccountSortState = { sort_by: 'name', sort_order: 'asc' placeholder
  try {
...
const usageBatchByAccountId = ref<Record<string, AccountUsageInfo | null>>({placeholder)
const usageBatchErrorByAccountId = ref<Record<string, string | null>>({placeholder)
const usageBatchLoadingByAccountId = ref<Record<string, boolean>>({placeholder)
const usageBatchRequestTokenByAccountId = ref<Record<string, number>>({placeholder)
const usageBatchCache = new Map<number, { data: AccountUsageInfo; ts: number placeholder>()
...
const flushQueuedUsageBatch = async () => {
  ...
  try {
    ...
    const usageMap = result.usage ?? {placeholder
    const errorMap = result.errors ?? {placeholder
    const now = Date.now()
    const nextUsage = { ...usageBatchByAccountId.value placeholder
    const nextErrors = { ...usageBatchErrorByAccountId.value placeholder
    const nextLoading = { ...usageBatchLoadingByAccountId.value placeholder
    ...
    for (const accountID of accountIDs) {
      ...
      if (usage) {
        usageBatchCache.set(accountID, { data: usage, ts: now placeholder)
      placeholder else {
...
const queueBatchedUsage = (account: Account, options?: { force?: boolean placeholder) => {
...
useSwipeSelect(accountTableRef, {
  ...
  deselect,
  batchUpdate
placeholder, swipeVirtualContext)
...
function getAccountPlanType(row: any): string | undefined {
  ...
  if (row.platform === 'grok') {
    const extra = (row.extra || {placeholder) as Record<string, any>
...
function getOpenAICompactMeta(row: any): { label: string; className: string; dotClass: string placeholder | null {
...
function getOpenAICompactTitle(row: any): string {
  ...
  return `${labelplaceholder | ${t('admin.accounts.openai.compactLastChecked')placeholder: ${formatDateTime(new Date(checkedAt))placeholder`
placeholder
...
const allColumns = computed(() => {
  ...
  placeholder
  c.push({ key: 'usage', label: t('admin.accounts.columns.usageWindows'), sortable: false placeholder)
  c.push(
...
const handleBulkDelete = async () => {
  ...
  if (!confirm(t('admin.accounts.bulkActions.confirmDelete', { count: accountIds.length placeholder))) return
  try {
    ...
    placeholder else {
      appStore.showSuccess(t('admin.accounts.bulkActions.deleteSuccess', { count: result.success placeholder))
      clearSelection()
    placeholder
    await reload()
  placeholder catch (error) {
...
const handleBulkProbeUpstreamBilling = async () => {
  ...
  try {
    ...
    placeholder else {
      appStore.showSuccess(t('admin.accounts.upstreamBilling.batchCompleted', { count: results.length placeholder))
    placeholder
  placeholder catch (error) {
...
const updateSchedulableInList = (accountIds: number[], schedulable: boolean) => {
  ...
  accounts.value = accounts.value.map((account) => (idSet.has(account.id) ? { ...account, schedulable placeholder : account))
placeholder
const normalizeBulkSchedulableResult = (
  result: {
    ...
    success_ids?: number[]
    failed_ids?: number[]
    results?: Array<{ account_id: number; success: boolean placeholder>
  placeholder,
  accountIds: number[]
) => {
...
const handleBulkToggleSchedulable = async (schedulable: boolean) => {
  const accountIds = [...selIds.value]
  try {
    const result = await adminAPI.accounts.bulkUpdate(accountIds, { schedulable placeholder)
    ...
    if (successCount > 0 && failedCount === 0) {
      const message = schedulable
        ? t('admin.accounts.bulkSchedulableEnabled', { count: successCount placeholder)
        : t('admin.accounts.bulkSchedulableDisabled', { count: successCount placeholder)
...
const collectSelectionMetadata = (rows: Account[]) => {
  ...
  return { selectedPlatforms, selectedTypes placeholder
placeholder
...
const openBulkEditFiltered = async () => {
  ...
  const { selectedPlatforms, selectedTypes placeholder = collectSelectionMetadata(preview.items)
  bulkEditTarget.value = {
    mode: 'filtered',
    filters,
...
const accountMatchesCurrentFilters = (account: Account) => {
  ...
  if (filters.group) {
    ...
    placeholder else if (!groupIds.includes(Number(filters.group))) {
      return false
    placeholder
  placeholder
  ...
  if (filters.privacy_mode) {
    ...
    placeholder else if (privacyMode !== filters.privacy_mode) {
      return false
    placeholder
  placeholder
...
const handleExportData = async () => {
  ...
  try {
    ...
    if (dataPayload.skipped_shadows && dataPayload.skipped_shadows > 0) {
      appStore.showWarning(t('admin.accounts.dataExportedSkippedShadows', { count: dataPayload.skipped_shadows placeholder))
    placeholder else {
  ...
  placeholder catch (error: any) {
    ...
    placeholder else if (isStepUpBlocked(error)) {
      appStore.showError(
...
const handleDuplicateAccount = async (a: Account) => {
  ...
  try {
    ...
    appStore.showSuccess(t('admin.accounts.duplicateSuccess', { name: duplicate.name placeholder))
    reload()
  placeholder catch (error: any) {
...
const confirmCreateSparkShadow = async () => {
  ...
  if (!a) return
  try {
    await adminAPI.accounts.createSparkShadow(a.id, { name: `${a.nameplaceholder (Spark)` placeholder)
...
const proxyExpiryText = (p: AccountProxy): string => {
  const { key, params placeholder = proxyExpiryLabelKey(p.expires_at, p.status)
