      ...
      <template #table>
        ...
        >
          ...
          <template #cell-notify_mode="{ row placeholder">
            ...
            >
              {{ row.notify_mode === 'popup' ? t('admin.announcements.notifyModeLabels.popup') : t('admin.announcements.notifyModeLabels.silent') placeholderplaceholder
            </span>
          </template>
    ...
    >
      <form id="announcement-form" @submit.prevent="handleSave" class="space-y-4">
        ...
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          <div>
            <label class="input-label">{{ t('admin.announcements.form.notifyMode') placeholderplaceholder</label>
            <Select v-model="form.notify_mode" :options="notifyModeOptions" />
            <p class="input-hint">{{ t('admin.announcements.form.notifyModeHint') placeholderplaceholder</p>
          </div>
        </div>
...
const notifyModeOptions = computed(() => [
  { value: 'silent', label: t('admin.announcements.notifyModeLabels.silent') placeholder,
  { value: 'popup', label: t('admin.announcements.notifyModeLabels.popup') placeholder
])
