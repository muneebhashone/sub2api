      ...
      <template #table>
        ...
        >
          ...
          <template #cell-status="{ row placeholder">
            ...
            >
              {{ statusStyleOf(row.status).label placeholderplaceholder
            </span>
          </template>

          <template #cell-billing_model_source="{ row placeholder">
            <span class="text-xs text-gray-700 dark:text-gray-300">
              {{ billingSourceLabelOf(row.billing_model_source) placeholderplaceholder
            </span>
          </template>
        </AvailableChannelsTable>
      </template>
 ...
 */
const statusStyles = computed<Record<ChannelStatus, { label: string; cls: string placeholder>>(() => ({
  [CHANNEL_STATUS_ACTIVE]: {
...
const DEFAULT_STATUS_STYLE = { label: '-', cls: '' placeholder
const DEFAULT_BILLING_LABEL = '-'

function statusStyleOf(status: ChannelStatus | undefined): { label: string; cls: string placeholder {
