        ...
        <DataTable :columns="columns" :data="groups" :loading="loading">
          ...
          <template #cell-account_count="{ row placeholder">
            <div class="space-y-0.5 text-xs">
              <div>
                <span class="text-gray-500 dark:text-gray-400">{{ t('admin.groups.accountsAvailable') placeholderplaceholder</span>
                <span class="ml-1 font-medium text-emerald-600 dark:text-emerald-400">{{ (row.active_account_count || 0) - (row.rate_limited_account_count || 0) placeholderplaceholder</span>
                <span class="ml-1 inline-flex items-center rounded bg-gray-100 px-1.5 py-0.5 font-medium text-gray-800 dark:bg-dark-600 dark:text-gray-300">{{ t('admin.groups.accountsUnit') placeholderplaceholder</span>
              </div>
              <div v-if="row.rate_limited_account_count">
                <span class="text-gray-500 dark:text-gray-400">{{ t('admin.groups.accountsRateLimited') placeholderplaceholder</span>
                <span class="ml-1 font-medium text-amber-600 dark:text-amber-400">{{ row.rate_limited_account_count placeholderplaceholder</span>
                <span class="ml-1 inline-flex items-center rounded bg-gray-100 px-1.5 py-0.5 font-medium text-gray-800 dark:bg-dark-600 dark:text-gray-300">{{ t('admin.groups.accountsUnit') placeholderplaceholder</span>
              </div>
              <div>
                <span class="text-gray-500 dark:text-gray-400">{{ t('admin.groups.accountsTotal') placeholderplaceholder</span>
                <span class="ml-1 font-medium text-gray-700 dark:text-gray-300">{{ row.account_count || 0 placeholderplaceholder</span>
                <span class="ml-1 inline-flex items-center rounded bg-gray-100 px-1.5 py-0.5 font-medium text-gray-800 dark:bg-dark-600 dark:text-gray-300">{{ t('admin.groups.accountsUnit') placeholderplaceholder</span>
              </div>
            </div>
          </template>
          ...
          <template #cell-usage="{ row placeholder">
            ...
            <div v-else class="space-y-0.5 text-xs">
              <div class="text-gray-500 dark:text-gray-400">
                <span class="text-gray-400 dark:text-gray-500">{{ t('admin.groups.usageToday') placeholderplaceholder</span>
                <span class="ml-1 font-medium text-gray-700 dark:text-gray-300">${{ formatCost(usageMap.get(row.id)?.today_cost ?? 0) placeholderplaceholder</span>
              </div>
              <div class="text-gray-500 dark:text-gray-400">
                <span class="text-gray-400 dark:text-gray-500">{{ t('admin.groups.usageTotal') placeholderplaceholder</span>
                <span class="ml-1 font-medium text-gray-700 dark:text-gray-300">${{ formatCost(usageMap.get(row.id)?.total_cost ?? 0) placeholderplaceholder</span>
              </div>
            </div>
          </template>
          ...
          <template #cell-actions="{ row placeholder">
            <div class="flex items-center gap-1">
              ...
              >
                ...
                <span class="text-xs">{{ t('admin.groups.rateMultipliers') placeholderplaceholder</span>
              </button>
              <button
      ...
      <form id="create-group-form" @submit.prevent="handleCreateGroup" class="space-y-5">
        ...
        <div v-if="copyAccountsGroupOptions.length > 0">
          ...
          <div v-if="createForm.copy_accounts_from_group_ids.length > 0" class="flex flex-wrap gap-1.5 mb-2">
            ...
            >
              {{ copyAccountsGroupOptions.find(o => o.value === groupId)?.label || `#${groupIdplaceholder` placeholderplaceholder
              <button
                type="button"
          ...
          <select
            ...
            placeholder"
          >
            <option value="">{{ t('admin.groups.copyAccounts.selectPlaceholder') placeholderplaceholder</option>
            <option
        ...
        <div v-if="createForm.platform === 'sora'" class="border-t pt-4">
          <label class="block mb-2 font-medium text-gray-700 dark:text-gray-300">
            {{ t('admin.groups.soraPricing.title') placeholderplaceholder
          </label>
          <p class="text-xs text-gray-500 dark:text-gray-400 mb-3">
            {{ t('admin.groups.soraPricing.description') placeholderplaceholder
          </p>
          <div class="grid grid-cols-2 gap-3 mb-4">
            <div>
              <label class="input-label">{{ t('admin.groups.soraPricing.image360') placeholderplaceholder</label>
              <input
            ...
            <div>
              <label class="input-label">{{ t('admin.groups.soraPricing.image540') placeholderplaceholder</label>
              <input
          ...
          <div class="grid grid-cols-2 gap-3">
            <div>
              <label class="input-label">{{ t('admin.groups.soraPricing.video') placeholderplaceholder</label>
              <input
            ...
            <div>
              <label class="input-label">{{ t('admin.groups.soraPricing.videoHd') placeholderplaceholder</label>
              <input
          ...
          <div class="mt-3">
            <label class="input-label">{{ t('admin.groups.soraPricing.storageQuota') placeholderplaceholder</label>
            ...
            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
              {{ t('admin.groups.soraPricing.storageQuotaHint') placeholderplaceholder
            </p>
          </div>
        </div>
        ...
        <div v-if="createForm.platform === 'antigravity'" class="border-t pt-4">
          ...
          <div class="space-y-2">
            <label class="flex items-center gap-2 cursor-pointer">
              ...
              />
              <span class="text-sm text-gray-700 dark:text-gray-300">{{ t('admin.groups.supportedScopes.claude') placeholderplaceholder</span>
            </label>
            <label class="flex items-center gap-2 cursor-pointer">
              <input
                type="checkbox"
                ...
                class="h-4 w-4 rounded border-gray-300 text-primary-600 focus:ring-primary-500 dark:border-dark-600 dark:bg-dark-700"
              />
              <span class="text-sm text-gray-700 dark:text-gray-300">{{ t('admin.groups.supportedScopes.geminiText') placeholderplaceholder</span>
            </label>
            <label class="flex items-center gap-2 cursor-pointer">
              <input
                type="checkbox"
                ...
                class="h-4 w-4 rounded border-gray-300 text-primary-600 focus:ring-primary-500 dark:border-dark-600 dark:bg-dark-700"
              />
              <span class="text-sm text-gray-700 dark:text-gray-300">{{ t('admin.groups.supportedScopes.geminiImage') placeholderplaceholder</span>
            </label>
          </div>
          <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">{{ t('admin.groups.supportedScopes.hint') placeholderplaceholder</p>
        </div>
        ...
        <div v-if="createForm.platform === 'antigravity'" class="border-t pt-4">
          ...
          <div class="flex items-center gap-3">
            ...
            <span class="text-sm text-gray-500 dark:text-gray-400">
              {{ createForm.mcp_xml_inject ? t('admin.groups.mcpXml.enabled') : t('admin.groups.mcpXml.disabled') placeholderplaceholder
            </span>
          </div>
        </div>
        ...
        <div v-if="createForm.platform === 'openai'" class="border-t border-gray-200 dark:border-dark-400 pt-4 mt-4">
          <h4 class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">{{ t('admin.groups.openaiMessages.title') placeholderplaceholder</h4>

          <!-- 允许 Messages 调度开关 -->
          <div class="flex items-center justify-between">
            <label class="text-sm text-gray-600 dark:text-gray-400">{{ t('admin.groups.openaiMessages.allowDispatch') placeholderplaceholder</label>
            <button
              type="button"
          ...
          <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">{{ t('admin.groups.openaiMessages.allowDispatchHint') placeholderplaceholder</p>

          <!-- 默认映射模型（仅当开关打开时显示） -->
          <div v-if="createForm.allow_messages_dispatch" class="mt-3">
            <label class="input-label">{{ t('admin.groups.openaiMessages.defaultModel') placeholderplaceholder</label>
            <input
              ...
              class="input"
            />
            <p class="input-hint">{{ t('admin.groups.openaiMessages.defaultModelHint') placeholderplaceholder</p>
          </div>
        </div>
        ...
        <div v-if="['openai', 'antigravity', 'anthropic', 'gemini'].includes(createForm.platform)" class="border-t border-gray-200 dark:border-dark-400 pt-4 mt-4 space-y-4">
          ...
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                {{ createForm.require_oauth_only ? '已启用 — 排除 API Key 类型账号' : '未启用' placeholderplaceholder
              </p>
            </div>
            <button
              type="button"
          ...
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                {{ createForm.require_privacy_set ? '已启用 — Privacy 未设置的账号将被排除' : '未启用' placeholderplaceholder
              </p>
            </div>
            <button
              type="button"
        ...
        <div
          ...
          class="border-t pt-4"
        >
          <label class="input-label">{{ t('admin.groups.invalidRequestFallback.title') placeholderplaceholder</label>
          <Select
          ...
          />
          <p class="input-hint">{{ t('admin.groups.invalidRequestFallback.hint') placeholderplaceholder</p>
        </div>
        ...
        <div v-if="createForm.platform === 'anthropic'" class="border-t pt-4">
          ...
          <div class="flex items-center gap-3 mb-3">
            ...
            <span class="text-sm text-gray-500 dark:text-gray-400">
              {{ createForm.model_routing_enabled ? t('admin.groups.modelRouting.enabled') : t('admin.groups.modelRouting.disabled') placeholderplaceholder
            </span>
          </div>
            ...
            >
              <div class="flex items-start gap-3">
                <div class="flex-1 space-y-2">
                  <div>
                    <label class="input-label text-xs">{{ t('admin.groups.modelRouting.modelPattern') placeholderplaceholder</label>
                    <input
                  ...
                  <div>
                    <label class="input-label text-xs">{{ t('admin.groups.modelRouting.accounts') placeholderplaceholder</label>
                    <!-- 已选账号标签 -->
                    ...
                    <div class="relative account-search-container">
                      ...
                      >
                        <button
                          ...
                          :class="{ 'opacity-50': rule.accounts.some(a => a.id === account.id) placeholder"
                    ...
                    <p class="text-xs text-gray-400 mt-1">{{ t('admin.groups.modelRouting.accountsHint') placeholderplaceholder</p>
                  </div>
                </div>
                <button
                  type="button"
      ...
      >
        ...
        <div v-if="copyAccountsGroupOptionsForEdit.length > 0">
          ...
          <div v-if="editForm.copy_accounts_from_group_ids.length > 0" class="flex flex-wrap gap-1.5 mb-2">
            <span
              ...
              class="inline-flex items-center gap-1 rounded-full bg-primary-100 px-2.5 py-1 text-xs font-medium text-primary-700 dark:bg-primary-900/30 dark:text-primary-300"
            >
              {{ copyAccountsGroupOptionsForEdit.find(o => o.value === groupId)?.label || `#${groupIdplaceholder` placeholderplaceholder
              <button
                type="button"
          ...
          <p class="input-hint">{{ t('admin.groups.copyAccounts.hintEdit') placeholderplaceholder</p>
        </div>
        <div>
          <label class="input-label">{{ t('admin.groups.form.rateMultiplier') placeholderplaceholder</label>
          <input
        ...
        <div v-if="editForm.platform === 'antigravity'" class="border-t pt-4">
          ...
          <div class="flex items-center gap-3">
            ...
            <span class="text-sm text-gray-500 dark:text-gray-400">
              {{ editForm.mcp_xml_inject ? t('admin.groups.mcpXml.enabled') : t('admin.groups.mcpXml.disabled') placeholderplaceholder
            </span>
          </div>
        </div>
        ...
        <div v-if="['openai', 'antigravity', 'anthropic', 'gemini'].includes(editForm.platform)" class="border-t border-gray-200 dark:border-dark-400 pt-4 mt-4 space-y-4">
          ...
          <div class="flex items-center justify-between">
            <div>
              <label class="text-sm text-gray-600 dark:text-gray-400">仅允许 OAuth 账号</label>
              <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                {{ editForm.require_oauth_only ? '已启用 — 排除 API Key 类型账号' : '未启用' placeholderplaceholder
              </p>
            </div>
            <button
              type="button"
          ...
          <div class="flex items-center justify-between">
            <div>
              <label class="text-sm text-gray-600 dark:text-gray-400">仅允许隐私保护已设置的账号</label>
              <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                {{ editForm.require_privacy_set ? '已启用 — Privacy 未设置的账号将被排除' : '未启用' placeholderplaceholder
              </p>
            </div>
            <button
              type="button"
        ...
        <div v-if="editForm.platform === 'anthropic'" class="border-t pt-4">
          ...
          <div class="flex items-center gap-3 mb-3">
            ...
            <span class="text-sm text-gray-500 dark:text-gray-400">
              {{ editForm.model_routing_enabled ? t('admin.groups.modelRouting.enabled') : t('admin.groups.modelRouting.disabled') placeholderplaceholder
            </span>
          </div>
      ...
      <div class="space-y-4">
        ...
        >
          ...
          >
            ...
            <div class="flex-1">
              <div class="font-medium text-gray-900 dark:text-white">{{ group.name placeholderplaceholder</div>
            ...
            <div class="text-sm text-gray-400">
              #{{ group.id placeholderplaceholder
            </div>
          </div>
        </VueDraggable>
      </div>
...
const copyAccountsGroupOptions = computed(() => {
  ...
  return eligibleGroups.map((g) => ({
    value: g.id,
    label: `${g.nameplaceholder (${g.account_count || 0placeholder 个账号)`
  placeholder))
placeholder)
...
const getCreateRuleSearchKey = (rule: ModelRoutingRule) => `create-${resolveCreateRuleKey(rule)placeholder`
const getEditRuleSearchKey = (rule: ModelRoutingRule) => `edit-${resolveEditRuleKey(rule)placeholder`
...
const loadUsageSummary = async () => {
  ...
  try {
    ...
    const map = new Map<number, { today_cost: number; total_cost: number placeholder>()
    for (const item of data) {
      map.set(item.group_id, { today_cost: item.today_cost, total_cost: item.total_cost placeholder)
    placeholder
    usageMap.value = map
...
const handleCreateGroup = async () => {
  ...
  try {
    // 构建请求数据，包含模型路由配置
    const { sora_storage_quota_gb: createQuotaGb, ...createRest placeholder = createForm
    const requestData = {
      ...createRest,
...
const handleUpdateGroup = async () => {
  ...
  try {
    ...
    const { sora_storage_quota_gb: editQuotaGb, ...editRest placeholder = editForm
    const payload = {
      ...editRest,
