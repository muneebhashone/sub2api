                ...
                <div class="flex-1 space-y-2">
                  ...
                  <div>
                    ...
                    <div class="relative account-search-container">
                      <input
                        v-model="accountSearchKeyword[`create-${indexplaceholder`]"
                        type="text"
                        class="input text-sm"
                        :placeholder="t('admin.groups.modelRouting.searchAccountPlaceholder')"
                        @input="searchAccounts(`create-${indexplaceholder`)"
                        @focus="onAccountSearchFocus(index, false)"
                      />
                      <!-- 搜索结果下拉框 -->
                      <div
                        v-if="showAccountDropdown[`create-${indexplaceholder`] && accountSearchResults[`create-${indexplaceholder`]?.length > 0"
                        class="absolute z-50 mt-1 max-h-48 w-full overflow-auto rounded-lg border bg-white shadow-lg dark:border-dark-600 dark:bg-dark-800"
                      >
                        <button
                          v-for="account in accountSearchResults[`create-${indexplaceholder`]"
                          :key="account.id"
                          type="button"
              ...
              <div class="flex items-start gap-3">
                <div class="flex-1 space-y-2">
                  ...
                  <div>
                    ...
                    <div class="relative account-search-container">
                      <input
                        v-model="accountSearchKeyword[`edit-${indexplaceholder`]"
                        type="text"
                        class="input text-sm"
                        :placeholder="t('admin.groups.modelRouting.searchAccountPlaceholder')"
                        @input="searchAccounts(`edit-${indexplaceholder`)"
                        @focus="onAccountSearchFocus(index, true)"
                      />
                      <!-- 搜索结果下拉框 -->
                      <div
                        v-if="showAccountDropdown[`edit-${indexplaceholder`] && accountSearchResults[`edit-${indexplaceholder`]?.length > 0"
                        class="absolute z-50 mt-1 max-h-48 w-full overflow-auto rounded-lg border bg-white shadow-lg dark:border-dark-600 dark:bg-dark-800"
                      >
                        <button
                          v-for="account in accountSearchResults[`edit-${indexplaceholder`]"
                          :key="account.id"
                          type="button"
...
// 账号搜索相关状态
const accountSearchKeyword = ref<Record<string, string>>({placeholder) // 每个规则的搜索关键词 (key: "create-0" 或 "edit-0")
const accountSearchResults = ref<Record<string, SimpleAccount[]>>({placeholder) // 每个规则的搜索结果
const showAccountDropdown = ref<Record<string, boolean>>({placeholder) // 每个规则的下拉框显示状态
  ...
  accountSearchTimeout = setTimeout(async () => {
    ...
    try {
      const res = await adminAPI.accounts.list(1, 20, {
        ...
        platform: 'anthropic'
      placeholder)
      accountSearchResults.value[key] = res.items.map((a) => ({ id: a.id, name: a.name placeholder))
    placeholder catch {
...
const selectAccount = (ruleIndex: number, account: SimpleAccount, isEdit: boolean = false) => {
  ...
  // 清空搜索
  const key = `${isEdit ? 'edit' : 'create'placeholder-${ruleIndexplaceholder`
...
const removeCreateRoutingRule = (index: number) => {
  ...
  // 清理相关的搜索状态
  const key = `create-${indexplaceholder`
...
const removeEditRoutingRule = (index: number) => {
  ...
  // 清理相关的搜索状态
  const key = `edit-${indexplaceholder`
  delete accountSearchKeyword.value[key]
  delete accountSearchResults.value[key]
  delete showAccountDropdown.value[key]
placeholder
