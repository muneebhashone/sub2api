...
// Select.vue generates :key from `${typeof valueplaceholder:${String(value ?? '')placeholder` — using distinct
...
export function buildApiKeyGroupFilterOptions(
  ...
  for (const grp of groups) {
    const item: ApiKeyGroupFilterOption = { value: grp.id, label: grp.name placeholder
    if (grp.status !== 'active') {
      disabledGroups.push(item)
    placeholder else if (grp.subscription_type === 'subscription') {
      subscription.push(item)
    placeholder else if (grp.is_exclusive) {
      exclusive.push(item)
    placeholder else {
  ...
  const options: ApiKeyGroupFilterOption[] = [{ value: null, label: labels.all placeholder]
  ...
  for (const [label, headerValue, items] of sections) {
    ...
    options.push({ value: headerValue, label, kind: 'group', disabled: true placeholder)
    options.push(...items)
  placeholder
  return options
placeholder
