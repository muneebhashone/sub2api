        ...
        <div class="flex flex-wrap items-center gap-3">
          ...
          <div class="flex flex-1 flex-wrap items-center justify-end gap-2">
            ...
            <button @click="handleExportCodes" class="btn btn-secondary">
              {{ t('admin.redeem.exportCsv') placeholderplaceholder
            </button>
            <button
            ...
            >
              ...
              {{ t('admin.redeem.batchUpdate') placeholderplaceholder
            </button>
            <button @click="showGenerateDialog = true" class="btn btn-primary">
              {{ t('admin.redeem.generateCodes') placeholderplaceholder
            </button>
          </div>
        </div>
      </template>
      ...
      <template #table>
        ...
        >
          ...
          <template #cell-type="{ value placeholder">
            ...
            >
              {{ t('admin.redeem.types.' + value) placeholderplaceholder
            </span>
          </template>

          <template #cell-value="{ value, row placeholder">
            <span class="text-sm font-medium text-gray-900 dark:text-white">
              <template v-if="row.type === 'balance'">${{ value.toFixed(2) placeholderplaceholder</template>
              <template v-else-if="row.type === 'subscription'">
                {{ row.validity_days || 30 placeholderplaceholder {{ t('admin.redeem.days') placeholderplaceholder
                <span v-if="row.group" class="ml-1 text-xs text-gray-500 dark:text-gray-400"
                  >({{ row.group.name placeholderplaceholder)</span
                >
              </template>
              <template v-else>{{ value placeholderplaceholder</template>
            </span>
          </template>
          ...
          <template #cell-used_by="{ value, row placeholder">
            <span class="text-sm text-gray-500 dark:text-dark-400">
              {{ row.user?.email || (value ? t('admin.redeem.userPrefix', { id: value placeholder) : '-') placeholderplaceholder
            </span>
          </template>

          <template #cell-used_at="{ value placeholder">
            <span class="text-sm text-gray-500 dark:text-dark-400">{{
              value ? formatDateTime(value) : '-'
            placeholderplaceholder</span>
          </template>

          <template #cell-expires_at="{ value, row placeholder">
            <span
              :class="[
                'text-sm',
                row.status === 'expired'
            ...
            >
              {{ value ? formatDateTime(value) : t('admin.redeem.neverExpires') placeholderplaceholder
            </span>
          </template>
      ...
      <template #pagination>
        ...
        >
          <span class="text-sm font-medium text-primary-900 dark:text-primary-100">
            {{ t('admin.redeem.selectedCount', { count: selectedCount placeholder) placeholderplaceholder
          </span>
          <div class="flex flex-wrap items-center gap-2">
            ...
            >
              {{ t('admin.redeem.clearSelection') placeholderplaceholder
            </button>
            <button
              type="button"
        ...
        <div v-if="filters.status === 'unused'" class="flex justify-end">
          <button @click="showDeleteUnusedDialog = true" class="btn btn-danger">
            {{ t('admin.redeem.deleteAllUnused') placeholderplaceholder
          </button>
        </div>
      </template>
    </TablePageLayout>
      ...
      <div v-if="showGenerateDialog" class="fixed inset-0 z-50 flex items-center justify-center">
        ...
        >
          <h2 class="mb-4 text-lg font-semibold text-gray-900 dark:text-white">
            {{ t('admin.redeem.generateCodesTitle') placeholderplaceholder
          </h2>
          <form @submit.prevent="handleGenerateCodes" class="space-y-4">
            <div>
              <label class="input-label">{{ t('admin.redeem.codeType') placeholderplaceholder</label>
            ...
            <template v-if="generateForm.type === 'subscription'">
              <div>
                <label class="input-label">{{ t('admin.redeem.selectGroup') placeholderplaceholder</label>
                <Select
              ...
              <div>
                <label class="input-label">{{ t('admin.redeem.validityDays') placeholderplaceholder</label>
                <input
            ...
            <div>
              <label class="input-label">{{ t('admin.redeem.codeExpiry') placeholderplaceholder</label>
            ...
            <div>
              <label class="input-label">{{ t('admin.redeem.count') placeholderplaceholder</label>
              <input
            ...
            <div class="flex justify-end gap-3 pt-2">
              ...
              <button type="submit" :disabled="generating" class="btn btn-primary">
                {{ generating ? t('admin.redeem.generating') : t('admin.redeem.generate') placeholderplaceholder
              </button>
            </div>
          </form>
        </div>
      ...
      >
        ...
        >
          <h2 class="mb-1 text-lg font-semibold text-gray-900 dark:text-white">
            {{ t('admin.redeem.batchUpdateTitle') placeholderplaceholder
          </h2>
          ...
          <form data-test="batch-update-form" class="space-y-4" @submit.prevent="handleBatchUpdate">
            <div class="space-y-2">
              <label class="flex items-center gap-2 text-sm font-medium text-gray-700 dark:text-gray-300">
                ...
                />
                {{ t('admin.redeem.batchFields.status') placeholderplaceholder
              </label>
              <Select
            ...
            <div class="space-y-2">
              <label class="flex items-center gap-2 text-sm font-medium text-gray-700 dark:text-gray-300">
                <input
                  ...
                  class="h-4 w-4 rounded border-gray-300 text-primary-600 focus:ring-primary-500"
                />
                {{ t('admin.redeem.batchFields.expiresAt') placeholderplaceholder
              </label>
            ...
            <div class="space-y-2">
              <label class="flex items-center gap-2 text-sm font-medium text-gray-700 dark:text-gray-300">
                <input
                  ...
                  class="h-4 w-4 rounded border-gray-300 text-primary-600 focus:ring-primary-500"
                />
                {{ t('admin.redeem.batchFields.notes') placeholderplaceholder
              </label>
              <textarea
            ...
            <div class="space-y-2">
              <label class="flex items-center gap-2 text-sm font-medium text-gray-700 dark:text-gray-300">
                <input
                  ...
                  class="h-4 w-4 rounded border-gray-300 text-primary-600 focus:ring-primary-500"
                />
                {{ t('admin.redeem.batchFields.group') placeholderplaceholder
              </label>
              <Select
            ...
            <div class="flex justify-end gap-3 pt-2">
              ...
              >
                {{ batchUpdating ? t('common.submitting') : t('admin.redeem.batchUpdate') placeholderplaceholder
              </button>
            </div>
          </form>
        </div>
        ...
        <div class="relative z-10 w-full max-w-lg rounded-xl bg-white shadow-xl dark:bg-dark-800">
            ...
            <div class="flex items-center gap-3">
              ...
              <div>
                <h2 class="text-base font-semibold text-gray-900 dark:text-white">
                  {{ t('admin.redeem.generatedSuccessfully') placeholderplaceholder
                </h2>
                <p class="text-sm text-gray-500 dark:text-gray-400">
                  {{ t('admin.redeem.codesCreated', { count: generatedCodes.length placeholder) placeholderplaceholder
                </p>
              </div>
            </div>
            <button
          ...
          <div class="p-5">
            <div class="relative">
              <textarea
                ...
                :style="{ height: textareaHeight placeholder"
          ...
          >
            ...
            >
              ...
              {{ copiedAll ? t('admin.redeem.copied') : t('admin.redeem.copyAll') placeholderplaceholder
            </button>
            <button @click="downloadGeneratedCodes" class="btn btn-primary flex items-center gap-2">
              <Icon name="download" size="sm" :stroke-width="2" />
              {{ t('admin.redeem.download') placeholderplaceholder
            </button>
          </div>
        </div>
      </div>
...
const batchGroupOptions = computed(() => [
  { value: null, label: t('admin.redeem.clearGroup') placeholder,
...
const textareaHeight = computed(() => {
  ...
  return `${calculatedHeightplaceholderpx`
placeholder)
...
const downloadGeneratedCodes = () => {
  const blob = new Blob([generatedCodesText.value], { type: 'text/plain' placeholder)
  const url = window.URL.createObjectURL(blob)
  const link = document.createElement('a')
  link.href = url
  link.download = `redeem-codes-${new Date().toISOString().split('T')[0]placeholder.txt`
...
const columns = computed<Column[]>(() => [
  { key: 'select', label: '' placeholder,
  { key: 'code', label: t('admin.redeem.columns.code') placeholder,
  { key: 'type', label: t('admin.redeem.columns.type'), sortable: true placeholder,
  { key: 'value', label: t('admin.redeem.columns.value'), sortable: true placeholder,
  { key: 'status', label: t('admin.redeem.columns.status'), sortable: true placeholder,
  { key: 'used_by', label: t('admin.redeem.columns.usedBy') placeholder,
  { key: 'used_at', label: t('admin.redeem.columns.usedAt'), sortable: true placeholder,
  { key: 'expires_at', label: t('admin.redeem.columns.expiresAt'), sortable: true placeholder,
  { key: 'actions', label: t('admin.redeem.columns.actions') placeholder
])
...
const filterStatusOptions = computed(() => [
  { value: '', label: t('admin.redeem.allStatus') placeholder,
  { value: 'unused', label: t('admin.redeem.unused') placeholder,
  { value: 'used', label: t('admin.redeem.used') placeholder,
  { value: 'expired', label: t('admin.redeem.status.expired') placeholder,
  { value: 'disabled', label: t('admin.redeem.status.disabled') placeholder
])

const batchStatusOptions = computed(() => [
  { value: 'unused', label: t('admin.redeem.status.unused') placeholder,
  { value: 'disabled', label: t('admin.redeem.status.disabled') placeholder
])

const batchExpiryModeOptions = computed(() => [
  { value: 'clear', label: t('admin.redeem.neverExpires') placeholder,
  { value: 'custom', label: t('admin.redeem.customExpiry') placeholder
])
...
const {
  ...
  toggleVisible
placeholder = useTableSelection<RedeemCode>({
  rows: codes,
  getId: (code) => code.id
placeholder)
...
const redeemCodeExpiryOptions = computed<{ value: RedeemCodeExpiryOption; label: string placeholder[]>(() => [
  { value: 'never', label: t('admin.redeem.neverExpires') placeholder,
  { value: '1', label: t('admin.redeem.expiryPresetDays', { days: 1 placeholder) placeholder,
  { value: '3', label: t('admin.redeem.expiryPresetDays', { days: 3 placeholder) placeholder,
  { value: '7', label: t('admin.redeem.expiryPresetDays', { days: 7 placeholder) placeholder,
  { value: 'custom', label: t('admin.redeem.customExpiry') placeholder
])
...
const toDatetimeLocalInputValue = (date: Date) => {
  ...
  return `${date.getFullYear()placeholder-${pad(date.getMonth() + 1)placeholder-${pad(date.getDate())placeholderT${pad(
    date.getHours()
  )placeholder:${pad(date.getMinutes())placeholder`
placeholder
...
const buildBatchUpdateFields = (): BatchUpdateRedeemCodeFields | null => {
  const fields: BatchUpdateRedeemCodeFields = {placeholder
...
const handleExportCodes = async () => {
  try {
    ...
    const link = document.createElement('a')
    link.href = url
    link.download = `redeem-codes-${new Date().toISOString().split('T')[0]placeholder.csv`
    document.body.appendChild(link)
    link.click()
    document.body.removeChild(link)
    window.URL.revokeObjectURL(url)
...
const confirmDeleteUnused = async () => {
  try {
    ...
    const unusedCodesResponse = await adminAPI.redeem.list(1, 1000, { status: 'unused' placeholder)
    ...
    appStore.showSuccess(t('admin.redeem.codesDeleted', { count: result.deleted placeholder))
    showDeleteUnusedDialog.value = false
    loadCodes()
  placeholder catch (error: any) {
...
const handleBatchUpdate = async () => {
  ...
  try {
    ...
    appStore.showSuccess(t('admin.redeem.batchUpdateSuccess', { count: result.updated placeholder))
    showBatchUpdateDialog.value = false
    clearSelectedCodes()
    loadCodes()
  placeholder catch (error: any) {
