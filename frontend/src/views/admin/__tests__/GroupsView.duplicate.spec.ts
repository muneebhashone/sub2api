...
describe('GroupsView duplicate action', () => {
  beforeEach(() => {
    localStorage.clear()
    vi.spyOn(console, 'error').mockImplementation(() => {placeholder)
    for (const fn of [
      listGroups,
      duplicateGroup,
      getModelsListCandidates,
  ...
  it('ignores repeated clicks while the duplicate request is in flight', async () => {
    ...
    duplicateGroup.mockImplementationOnce(
      () => new Promise<AdminGroup>((resolve) => { resolveDuplicate = resolve placeholder)
    )
    const wrapper = mountView()
    await flushPromises()
    ...
    resolveDuplicate({ ...sourceGroup, id: 43, name: 'Primary (Copy)', status: 'inactive' placeholder)
    await flushPromises()
