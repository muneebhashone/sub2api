...
const createAdminUser = (overrides: Partial<AdminUser> = {placeholder): AdminUser => ({
  id: 42,
...
const DataTableStub = {
  ...
  template: `
    <div>
      ...
      <div data-test="row-order">{{ data.map(row => row.email).join(',') placeholderplaceholder</div>
      <div data-test="selected-keys">{{ (selectedKeys || []).join(',') placeholderplaceholder</div>
...
const BulkEditUserModalStub = {
  ...
  template: `
    <div v-if="show" data-test="bulk-modal">
      <span data-test="bulk-modal-ids">{{ selectedIds.join(',') placeholderplaceholder</span>
...
describe('admin UsersView', () => {
  beforeEach(() => {
    ...
    getBatchUsersUsage.mockResolvedValue({ stats: {placeholder placeholder)
    listEnabledDefinitions.mockResolvedValue([])
    getBatchUserAttributes.mockResolvedValue({ values: {placeholder placeholder)
  placeholder)
  ...
  it('clears usage current-page sort when switching to last_used_at server sort', async () => {
    ...
    listUsers.mockResolvedValue({
      items: [
        createAdminUser({ id: 1, email: 'last-used-first@example.com' placeholder),
        createAdminUser({ id: 2, email: 'usage-first@example.com' placeholder)
      ],
      total: 2,
      page: 1,
      page_size: 20,
    ...
    getBatchUsersUsage.mockResolvedValue({
      stats: {
        1: { user_id: 1, today_actual_cost: 1, total_actual_cost: 1, by_platform: [] placeholder,
        2: { user_id: 2, today_actual_cost: 9, total_actual_cost: 9, by_platform: [] placeholder
      placeholder
    placeholder)
    ...
    listUsers.mockImplementation(async (page: number) => {
      const user = page === 2
        ? createAdminUser({
          ...
          placeholder)
        : createAdminUser({ id: 42, email: 'page-one@example.com' placeholder)
      return {
        items: [user],
        total: 2,
        page,
