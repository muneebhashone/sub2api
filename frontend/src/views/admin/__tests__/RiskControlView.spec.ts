...
import type { ContentModerationConfig, UpdateContentModerationConfig placeholder from '@/api/admin/riskControl'
    ...
    useI18n: () => ({
      t: (key: string, params?: Record<string, string | number>) => {
        if (key === 'admin.riskControl.preBlockAPIKeyLoadSummary') {
          return `同步并发 ${params?.activeplaceholder / 可用 Key ${params?.availableplaceholder，累计 ${params?.totalplaceholder 次，worker：${params?.workerActiveplaceholder / ${params?.workerTotalplaceholder`
        placeholder
        return key.replace(/\{(\w+)\placeholder/g, (_, token) => String(params?.[token] ?? `{${tokenplaceholderplaceholder`))
      placeholder,
    placeholder),
  placeholder
placeholder)
