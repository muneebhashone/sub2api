...
import { defineComponent, ref placeholder from 'vue'

import UsageView from '../UsageView.vue'

const { list, exportList, getStats, getSnapshotV2, getById, getModelStats, listErrorLogs, routeQuery, aoaToSheet, sheetAddAoa, saveAs, xlsxWrite placeholder = vi.hoisted(() => {
  ...
  return {
    ...
    getModelStats: vi.fn(),
    listErrorLogs: vi.fn(),
    routeQuery: {placeholder as Record<string, string>,
		aoaToSheet: vi.fn(() => ({placeholder)),
		sheetAddAoa: vi.fn(),
		saveAs: vi.fn(),
...
vi.mock('file-saver', () => ({ saveAs placeholder))

vi.mock('xlsx', () => ({
	utils: {
		aoa_to_sheet: aoaToSheet,
		sheet_add_aoa: sheetAddAoa,
		book_new: vi.fn(() => ({placeholder)),
...
const UsageFiltersStub = defineComponent({
  setup(_, { expose placeholder) {
    ...
    placeholder)
    return { userKeyword placeholder
  placeholder,
  template: '<div><span data-test="user-filter-label">{{ userKeyword placeholderplaceholder</span><slot name="after-reset" /></div>',
placeholder)
...
const ModelDistributionChartStub = {
  ...
  template: `
    <div data-test="model-chart">
      <span class="metric">{{ metric placeholderplaceholder</span>
...
describe('admin UsageView route filters', () => {
  beforeEach(() => {
    ...
    list.mockReset().mockResolvedValue({ items: [], total: 0, pages: 0 placeholder)
    ...
    placeholder)
    getSnapshotV2.mockReset().mockResolvedValue({ trend: [], models: [], groups: [] placeholder)
    getModelStats.mockReset().mockResolvedValue({ models: [] placeholder)
    getById.mockReset()
  placeholder)
  ...
  it('shows the routed user while applying user_id to usage requests', async () => {
    ...
    getById.mockResolvedValue({ id: 42, email: 'route-user@test.com' placeholder)

    const wrapper = mountRouteFilteredUsageView()
    await flushPromises()

    expect(getById).toHaveBeenCalledWith(42, true)
    expect(list).toHaveBeenCalledWith(expect.objectContaining({ user_id: 42 placeholder), expect.anything())
  ...
  it('does not apply a stale routed user label after user_id changes', async () => {
    routeQuery.user_id = '42'
    let resolveLookup!: (user: { id: number; email: string placeholder) => void
    getById.mockReturnValue(new Promise((resolve) => { resolveLookup = resolve placeholder))
    ...
    resolveLookup({ id: 42, email: 'stale-user@test.com' placeholder)
    await flushPromises()
  ...
  it('does not overwrite newer user input when the routed user lookup succeeds', async () => {
    ...
    resolveLookup({ id: 42, email: 'route-user@test.com' placeholder)
    await flushPromises()
  ...
  it('does not overwrite newer user input when the routed user lookup fails', async () => {
    ...
    getById.mockReturnValue(new Promise((_, reject) => { rejectLookup = reject placeholder))
...
describe('admin UsageView request ID column visibility', () => {
  ...
  it('keeps request ID hidden by default and allows enabling it from column settings', async () => {
    ...
    expect(usageTable.props('columns')).not.toEqual(
      expect.arrayContaining([expect.objectContaining({ key: 'request_id' placeholder)]),
    )
    ...
    expect(usageTable.props('columns')).toEqual(
      expect.arrayContaining([expect.objectContaining({ key: 'request_id', label: 'Request ID' placeholder)]),
    )
...
describe('admin UsageView errors tab filter forwarding', () => {
  beforeEach(() => {
    ...
    getSnapshotV2.mockResolvedValue({ trend: [], models: [], groups: [] placeholder)
    getModelStats.mockResolvedValue({ models: [] placeholder)
    listErrorLogs.mockResolvedValue({ items: [], total: 0, pages: 0 placeholder)
  placeholder)
...
describe('admin UsageView ranking tab', () => {
  ...
  it('mounts ranking lazily and drill-down sets user filter then jumps back to usage tab', async () => {
    ...
    expect(list).toHaveBeenCalledWith(expect.objectContaining({ user_id: 5 placeholder), expect.anything())
  placeholder)
placeholder)
