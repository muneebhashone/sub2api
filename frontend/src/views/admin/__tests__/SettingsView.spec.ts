...
const localeRef = vi.hoisted(() => ({ value: "zh-CN" placeholder));
...
vi.mock("vue-i18n", async () => {
  ...
  const translations: Record<string, string> = {
    ...
    "admin.settings.paymentVisibleMethods.methodLabel": "{titleplaceholder 可见方式",
    ...
    "admin.settings.paymentVisibleMethods.sourceRequiredError": "{titleplaceholder 已启用，请先选择支付来源。",
    ...
    "admin.settings.openaiExperimentalScheduler.defaultPlaceholder": "配置/默认：{valueplaceholder",
  ...
  return {
    ...
    useI18n: () => ({
      t: (key: string, params?: Record<string, string>) =>
        (translations[key] ?? key).replace(/\{(\w+)\placeholder/g, (_, token) => params?.[token] ?? `{${tokenplaceholderplaceholder`),
      locale: localeRef,
    placeholder),
  placeholder;
placeholder);
...
const ToggleStub = defineComponent({
  ...
  inheritAttrs: false,
  setup(props, { attrs, emit placeholder) {
    return () =>
      h("input", {
        ...attrs,
        class: "toggle-stub",
      ...
      h(
        ...
        (props.options as Array<Record<string, unknown>>).map((option) =>
          h(
            "option",
            {
              key: `${String(option.value ?? "")placeholder:${String(option.label ?? "")placeholder`,
...
const baseSettingsResponse = {
  ...
  default_platform_quotas: {
    anthropic:   { daily: null, weekly: null, monthly: null placeholder,
    openai:      { daily: null, weekly: 12.5, monthly: null placeholder,
    gemini:      { daily: null, weekly: null, monthly: 200 placeholder,
    antigravity: { daily: null, weekly: null, monthly: null placeholder,
  placeholder,
placeholder;
...
describe("admin SettingsView payment visible method controls", () => {
  beforeEach(() => {
    ...
    getSettings.mockResolvedValue({ ...baseSettingsResponse placeholder);
  ...
  it("submits the compact home page toggle", async () => {
    ...
    expect(updateSettings).toHaveBeenCalledWith(
      expect.objectContaining({ compact_home_enabled: true placeholder),
    );
  placeholder);
  ...
  it("shows valid passkey RP configuration and persists the sign-in toggle", async () => {
    ...
    expect(updateSettings).toHaveBeenCalledWith(
      expect.objectContaining({ passkey_enabled: false placeholder),
    );
  placeholder);
  ...
  it("loads, edits, validates, and saves forwarded client-IP headers", async () => {
    ...
    await input.trigger("keydown", { key: "Enter" placeholder);
    await input.setValue("X-CLIENT-IP");
    await input.trigger("keydown", { key: "Enter" placeholder);
  ...
  it("submits Claude OAuth system prompt injection gateway settings", async () => {
    const blocks = `[{"type":"text","text":"custom block","cache_control":trueplaceholder]`;
    getSettings.mockResolvedValueOnce({
      ...baseSettingsResponse,
  ...
  it("updates provider enablement immediately and reloads providers", async () => {
    ...
    getProviders.mockReset();
    getProviders
      .mockResolvedValueOnce({ data: [provider] placeholder)
      .mockResolvedValueOnce({ data: [{ ...provider, enabled: true placeholder] placeholder);
    updateProvider.mockResolvedValue({ data: { ...provider, enabled: true placeholder placeholder);
    ...
    expect(updateProvider).toHaveBeenCalledWith(7, { enabled: true placeholder);
  ...
  it("normalizes null supported_types from API so provider card stays visible", async () => {
    ...
    placeholder;
    getProviders.mockReset();
    getProviders.mockResolvedValue({ data: [providerWithNullTypes] placeholder);
    ...
    const PaymentProviderListCapture = defineComponent({
      ...
      setup(props) {
        ...
        return () => h("div", { class: "provider-list-capture" placeholder);
      placeholder,
    placeholder);
...
describe("admin SettingsView platform quota matrix", () => {
  beforeEach(() => {
    ...
    updateSettings.mockImplementation(async (payload) => ({
      ...
      ...payload,
    placeholder));
    getWebSearchEmulationConfig.mockResolvedValue({ enabled: false, providers: [] placeholder);
    updateWebSearchEmulationConfig.mockResolvedValue({ enabled: false, providers: [] placeholder);
    getAdminApiKey.mockResolvedValue({ exists: false, masked_key: "" placeholder);
    getOverloadCooldownSettings.mockResolvedValue({placeholder);
    getRateLimit429CooldownSettings.mockResolvedValue({placeholder);
    updateRateLimit429CooldownSettings.mockResolvedValue({placeholder);
    getStreamTimeoutSettings.mockResolvedValue({placeholder);
    getRectifierSettings.mockResolvedValue({placeholder);
    getBetaPolicySettings.mockResolvedValue({placeholder);
    getGroups.mockResolvedValue([]);
    listProxies.mockResolvedValue({ items: [] placeholder);
    getProviders.mockResolvedValue({ data: [] placeholder);
  placeholder);
  ...
  it("加载后 form.default_platform_quotas 含全 5 平台，从嵌套 JSON 正确读取数值", async () => {
    getSettings.mockResolvedValueOnce({
      ...baseSettingsResponse,
      default_platform_quotas: {
        anthropic: { daily: 5, weekly: null, monthly: null placeholder,
        openai:    { daily: null, weekly: 12.5, monthly: null placeholder,
    ...
    // 缺失平台应补全为 null
    expect(quotas["gemini"]).toEqual({ daily: null, weekly: null, monthly: null placeholder);
    expect(quotas["antigravity"]).toEqual({ daily: null, weekly: null, monthly: null placeholder);
  placeholder);
  ...
  it("空输入（v-model.number 产出 \"\"）在提交时清洗为 null 而非空字符串", async () => {
    ...
    getSettings.mockResolvedValueOnce({
      ...baseSettingsResponse,
      default_platform_quotas: {
        anthropic: { daily: 10, weekly: null, monthly: null placeholder,
        openai:    { daily: null, weekly: null, monthly: null placeholder,
        gemini:    { daily: null, weekly: null, monthly: null placeholder,
        antigravity: { daily: null, weekly: null, monthly: null placeholder,
      placeholder,
    placeholder);
