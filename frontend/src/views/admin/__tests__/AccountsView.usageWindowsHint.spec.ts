...
const HelpTooltipStub = {
  ...
  template: '<span data-test="usage-windows-hint">{{ content placeholderplaceholder</span>'
placeholder
...
describe('admin AccountsView usage windows hint', () => {
  ...
  it('keeps groups available when loading proxies fails', async () => {
    ...
    getAllGroups.mockResolvedValue([{ id: 7, name: 'production' placeholder])
  ...
  it('keeps Ollama Cloud in the single usage column and ignores legacy column preferences', async () => {
    ...
    const columns = wrapper.getComponent(DataTableStub).props('columns') as Array<{ key: string placeholder>
  ...
  it('renders the upstream billing trust warning next to the declared-rate column', async () => {
    ...
    const columns = wrapper.getComponent(DataTableStub).props('columns') as Array<{ key: string; sortable: boolean placeholder>
