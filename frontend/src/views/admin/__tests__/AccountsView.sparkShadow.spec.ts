...
vi.mock('@/api/admin', () => ({
  adminAPI: {
    ...
    placeholder,
    proxies: { getAll: getAllProxies placeholder,
    groups: { getAll: getAllGroups placeholder
  placeholder
placeholder))

vi.mock('@/stores/app', () => ({
  useAppStore: () => ({ showError, showSuccess, showInfo: vi.fn() placeholder)
placeholder))

vi.mock('@/stores/auth', () => ({
  useAuthStore: () => ({ token: 'test-token' placeholder)
placeholder))
...
describe('admin AccountsView — 外审 F2:spark 影子创建接线', () => {
  beforeEach(() => {
    ...
    listWithEtag.mockResolvedValue({ notModified: true, etag: null, data: null placeholder)
    getBatchTodayStats.mockResolvedValue({ stats: {placeholder placeholder)
    getAllProxies.mockResolvedValue([])
    getAllGroups.mockResolvedValue([])
    duplicateAccount.mockResolvedValue({ id: 998, name: 'parent-acc (Copy)' placeholder)
    createSparkShadow.mockResolvedValue({ id: 999, name: 'parent-acc (Spark)' placeholder)
  placeholder)
  ...
  it('AccountActionMenu 的 duplicate 事件一键复制账号并刷新列表', async () => {
    ...
    wrapper.findComponent(AccountActionMenu).vm.$emit('duplicate', { id: 42, name: 'parent-acc' placeholder)
    await flushPromises()
  ...
  it('同一账号复制请求未完成时忽略重复点击', async () => {
    let resolveDuplicate!: (account: { id: number; name: string placeholder) => void
    duplicateAccount.mockImplementationOnce(() => new Promise(resolve => { resolveDuplicate = resolve placeholder))
    const wrapper = mountView()
    await flushPromises()

    const menu = wrapper.findComponent(AccountActionMenu)
    menu.vm.$emit('duplicate', { id: 42, name: 'parent-acc' placeholder)
    menu.vm.$emit('duplicate', { id: 42, name: 'parent-acc' placeholder)
    await flushPromises()

    expect(duplicateAccount).toHaveBeenCalledTimes(1)
    resolveDuplicate({ id: 998, name: 'parent-acc (Copy)' placeholder)
    await flushPromises()
    wrapper.unmount()
  placeholder)
  ...
  it('AccountActionMenu 的 create-spark-shadow 事件触发 createSparkShadow API + 成功提示', async () => {
    ...
    menu.vm.$emit('create-spark-shadow', { id: 42, name: 'parent-acc' placeholder)
    await flushPromises()
    ...
    expect(createSparkShadow).toHaveBeenCalledWith(42, { name: 'parent-acc (Spark)' placeholder)
  ...
  it('用户取消确认时不调用 API', async () => {
    ...
    wrapper.findComponent(AccountActionMenu).vm.$emit('create-spark-shadow', { id: 42, name: 'parent-acc' placeholder)
    await flushPromises()
...
describe('admin AccountsView — 账号行展示', () => {
  ...
  it('影子行 email 单元格显示 parent_email，PlatformTypeBadge 接收 parent_plan_type/parent_privacy_mode', async () => {
    ...
    listAccounts.mockResolvedValue({ items: [shadowAccount], total: 1, page: 1, page_size: 20, pages: 1 placeholder)
  ...
  it('仅将具有安全 base_url 的 API Key 账号名称链接到站点主页', async () => {
    listAccounts.mockResolvedValue({
      items: [
        { id: 101, name: 'relay-account', platform: 'openai', type: 'apikey', credentials: { base_url: 'https://relay.example.com/api/v1/' placeholder placeholder,
        { id: 102, name: 'oauth-account', platform: 'openai', type: 'oauth', credentials: { base_url: 'https://oauth.example.com/v1' placeholder placeholder,
        { id: 103, name: 'invalid-url', platform: 'openai', type: 'apikey', credentials: { base_url: 'javascript:alert(1)' placeholder placeholder,
      ],
      total: 3,
      page: 1,
      page_size: 20,
