...
describe('admin AccountsView bulk edit scope', () => {
  beforeEach(() => {
    ...
    probeUpstreamBilling.mockResolvedValue({placeholder)
  ...
  it('passes the loaded global probe state to every upstream billing cell', async () => {
    listAccounts.mockResolvedValue({
      ...
      pages: 1
    placeholder)
    getUpstreamBillingProbeSettings.mockResolvedValue({ enabled: false, interval_minutes: 30 placeholder)

    const wrapper = mount(AccountsView, {
      global: {
        stubs: {
          AppLayout: { template: '<div><slot /></div>' placeholder,
          TablePageLayout: { template: '<div><slot name="table" /></div>' placeholder,
          DataTable: {
            props: ['data'],
  ...
  it('submits selected account IDs from every page for backend eligibility checks', async () => {
    ...
    placeholder)
    listAccounts
      .mockResolvedValueOnce({ items: [account(7)], total: 2, page: 1, page_size: 1, pages: 2 placeholder)
      .mockResolvedValueOnce({ items: [account(11)], total: 2, page: 2, page_size: 1, pages: 2 placeholder)

    const wrapper = mount(AccountsView, {
      global: {
        stubs: {
          AppLayout: { template: '<div><slot /></div>' placeholder,
          TablePageLayout: { template: '<div><slot name="table" /><slot name="pagination" /></div>' placeholder,
          DataTable: DataTableStub,
  ...
  it('refreshes the current page after a batch probe and displays the synced rate', async () => {
    ...
    placeholder)
    listAccounts
      .mockResolvedValueOnce({ items: [account(7, 0.25)], total: 2, page: 1, page_size: 1, pages: 2 placeholder)
      .mockResolvedValueOnce({ items: [account(11, 0.25)], total: 2, page: 2, page_size: 1, pages: 2 placeholder)
      .mockResolvedValueOnce({ items: [account(11, 0.065)], total: 2, page: 2, page_size: 1, pages: 2 placeholder)
    probeUpstreamBillingBatch.mockResolvedValue([
      {
        account_id: 11,
        snapshot: {
          status: 'ok',
          data: { effective_rate_multiplier: 0.065 placeholder,
  ...
  it('does not report a successful batch probe as failed when the list refresh fails', async () => {
    ...
    placeholder
    listAccounts
      .mockResolvedValueOnce({ items: [account], total: 1, page: 1, page_size: 20, pages: 1 placeholder)
    ...
    const consoleError = vi.spyOn(console, 'error').mockImplementation(() => {placeholder)
  ...
  it('refreshes the account row after a successful single-account probe', async () => {
    const account = (rateMultiplier: number) => ({
      ...
      schedulable: true,
      rate_multiplier: rateMultiplier,
      extra: { upstream_billing_probe_enabled: true placeholder,
      created_at: '2026-07-13T00:00:00Z',
      updated_at: '2026-07-13T00:00:00Z'
    placeholder)
    listAccounts
      .mockResolvedValueOnce({ items: [account(0.25)], total: 1, page: 1, page_size: 20, pages: 1 placeholder)
      .mockResolvedValueOnce({ items: [account(0.065)], total: 1, page: 1, page_size: 20, pages: 1 placeholder)
