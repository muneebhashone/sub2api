...
const switchTrendTimeRange = `${switchTrendWindowHoursplaceholderh`
...
function handleBackToList() {
  ...
  if (target === 'requestList') {
    ...
    showErrorDetails.value = false
    showRequestDetails.value = true
  placeholder else if (target === 'errorList') {
    showErrorModal.value = false
    showRequestDetails.value = false
    showErrorDetails.value = true
  placeholder
...
async function refreshOverviewWithCancel(fetchSeq: number, signal: AbortSignal) {
  if (!opsEnabled.value) return
  try {
    const data = await opsAPI.getDashboardOverview(buildApiParams(), { signal placeholder)
...
async function refreshSwitchTrendWithCancel(fetchSeq: number, signal: AbortSignal) {
  ...
  try {
    const data = await opsAPI.getThroughputTrend(buildSwitchTrendParams(), { signal placeholder)
    if (fetchSeq !== dashboardFetchSeq) return
...
async function refreshThroughputTrendWithCancel(fetchSeq: number, signal: AbortSignal) {
  ...
  try {
    const data = await opsAPI.getThroughputTrend(buildApiParams(), { signal placeholder)
    if (fetchSeq !== dashboardFetchSeq) return
...
async function refreshLatencyHistogramWithCancel(fetchSeq: number, signal: AbortSignal) {
  ...
  try {
    const data = await opsAPI.getLatencyHistogram(buildApiParams(), { signal placeholder)
    if (fetchSeq !== dashboardFetchSeq) return
...
async function refreshErrorTrendWithCancel(fetchSeq: number, signal: AbortSignal) {
  ...
  loadingErrorTrend.value = true
  try {
    const data = await opsAPI.getErrorTrend(buildApiParams(), { signal placeholder)
    if (fetchSeq !== dashboardFetchSeq) return
...
async function refreshErrorDistributionWithCancel(fetchSeq: number, signal: AbortSignal) {
  ...
  try {
    const data = await opsAPI.getErrorDistribution(buildApiParams(), { signal placeholder)
    if (fetchSeq !== dashboardFetchSeq) return
