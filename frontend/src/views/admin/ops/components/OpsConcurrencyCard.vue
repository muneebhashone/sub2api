...
import { opsAPI, type OpsAccountAvailabilityStatsResponse, type OpsConcurrencyStatsResponse, type OpsUserConcurrencyStatsResponse placeholder from '@/api/admin/ops'
...
const platformRows = computed((): SummaryRow[] => {
  const concStats = concurrency.value?.platform || {placeholder
  const availStats = availability.value?.platform || {placeholder

  const platforms = new Set([...Object.keys(concStats), ...Object.keys(availStats)])

  return Array.from(platforms).map(platform => {
    const conc = concStats[platform] || {placeholder
    const avail = availStats[platform] || {placeholder
    ...
    placeholder
  placeholder).sort((a, b) => b.concurrency_percentage - a.concurrency_percentage)
placeholder)

// 分组维度汇总
const groupRows = computed((): SummaryRow[] => {
  const concStats = concurrency.value?.group || {placeholder
  const availStats = availability.value?.group || {placeholder

  const groupIds = new Set([...Object.keys(concStats), ...Object.keys(availStats)])

  const rows = Array.from(groupIds)
    .map(gid => {
      const conc = concStats[gid] || {placeholder
      const avail = availStats[gid] || {placeholder
      ...
      return {
        key: gid,
        name: String(conc.group_name || avail.group_name || `Group ${gidplaceholder`),
...
const accountRows = computed((): AccountRow[] => {
  const concStats = concurrency.value?.account || {placeholder
  const availStats = availability.value?.account || {placeholder

  const accountIds = new Set([...Object.keys(concStats), ...Object.keys(availStats)])

  const rows = Array.from(accountIds)
    .map(aid => {
      const conc = concStats[aid] || {placeholder
      const avail = availStats[aid] || {placeholder
      ...
      return {
        key: aid,
        name: String(conc.account_name || avail.account_name || `Account ${aidplaceholder`),
        platform: String(conc.platform || avail.platform || ''),
...
const userRows = computed((): UserRow[] => {
  const userStats = userConcurrency.value?.user || {placeholder

  return Object.keys(userStats)
    .map(uid => {
      const u = userStats[uid] || {placeholder
      return {
        key: uid,
        user_id: safeNumber(u.user_id),
        user_email: u.user_email || `User ${uidplaceholder`,
        username: u.username || '',
...
function getLoadBarStyle(loadPct: number): string {
  return `width: ${Math.min(100, Math.max(0, loadPct))placeholder%`
placeholder
...
function formatDuration(seconds: number): string {
  ...
  if (seconds < 60) return `${Math.round(seconds)placeholders`
  const minutes = Math.floor(seconds / 60)
  if (minutes < 60) return `${minutesplaceholderm`
  const hours = Math.floor(minutes / 60)
  return `${hoursplaceholderh`
placeholder
  ...
  <div class="flex h-full flex-col rounded-3xl bg-white p-6 shadow-sm ring-1 ring-gray-900/5 dark:bg-dark-800 dark:ring-dark-700">
    ...
    <div class="mb-4 flex shrink-0 items-center justify-between gap-3">
      <h3 class="flex items-center gap-2 text-sm font-bold text-gray-900 dark:text-white">
        ...
        {{ t('admin.ops.concurrency.title') placeholderplaceholder
      </h3>
      <div class="flex items-center gap-2">
        ...
        <button
          ...
          @click="loadData"
        >
          <svg class="h-3 w-3" :class="{ 'animate-spin': loading placeholder" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    >
      {{ t('admin.ops.concurrency.disabledHint') placeholderplaceholder
    </div>
    ...
    <div v-else class="flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl border border-gray-200 dark:border-dark-700">
      ...
      <div class="flex shrink-0 items-center justify-between border-b border-gray-200 bg-gray-50 px-3 py-2 dark:border-dark-700 dark:bg-dark-900">
        ...
        <span class="text-[10px] text-gray-500 dark:text-gray-400">
          {{ t('admin.ops.concurrency.totalRows', { count: displayRows.length placeholder) placeholderplaceholder
        </span>
      </div>

      <!-- 空状态 -->
      <div v-if="displayRows.length === 0" class="flex flex-1 items-center justify-center text-sm text-gray-500 dark:text-gray-400">
        {{ t('admin.ops.concurrency.empty') placeholderplaceholder
      </div>
        ...
        <div v-for="row in (displayRows as UserRow[])" :key="row.key" class="rounded-lg bg-gray-50 p-2.5 dark:bg-dark-900">
          ...
          <div class="mb-1.5 flex items-center justify-between gap-2">
            <div class="flex min-w-0 flex-1 items-center gap-1.5">
              <span class="truncate text-[11px] font-bold text-gray-900 dark:text-white" :title="row.username || row.user_email">
                {{ row.username || row.user_email placeholderplaceholder
              </span>
            ...
            <div class="flex shrink-0 items-center gap-2 text-[10px]">
              <span class="font-mono font-bold text-gray-900 dark:text-white"> {{ row.current_in_use placeholderplaceholder/{{ row.max_capacity placeholderplaceholder </span>
              <span :class="['font-bold', getLoadTextClass(row.load_percentage)]"> {{ Math.round(row.load_percentage) placeholderplaceholder% </span>
            </div>
          </div>
          ...
          <div v-if="row.waiting_in_queue > 0" class="mt-1.5 flex justify-end">
            <span class="rounded-full bg-purple-100 px-1.5 py-0.5 text-[10px] font-semibold text-purple-700 dark:bg-purple-900/30 dark:text-purple-400">
              {{ t('admin.ops.concurrency.queued', { count: row.waiting_in_queue placeholder) placeholderplaceholder
            </span>
          </div>
        </div>
      </div>
          ...
          <div class="mb-2 flex items-center justify-between gap-2">
            <div class="flex items-center gap-2">
              ...
              <span v-if="displayDimension === 'group' && row.platform" class="text-[10px] text-gray-400 dark:text-gray-500">
                {{ row.platform.toUpperCase() placeholderplaceholder
              </span>
            </div>
            <div class="flex shrink-0 items-center gap-2 text-[10px]">
              <span class="font-mono font-bold text-gray-900 dark:text-white"> {{ row.used_concurrency placeholderplaceholder/{{ row.total_concurrency placeholderplaceholder </span>
              <span :class="['font-bold', getLoadTextClass(row.concurrency_percentage)]"> {{ row.concurrency_percentage placeholderplaceholder% </span>
            </div>
          </div>
          ...
          <div class="flex flex-wrap items-center gap-x-3 gap-y-1 text-[10px]">
            ...
            <div class="flex items-center gap-1">
              ...
              <span class="text-gray-600 dark:text-gray-300">
                <span class="font-bold text-green-600 dark:text-green-400">{{ row.available_accounts placeholderplaceholder</span
                >/{{ row.total_accounts placeholderplaceholder
              </span>
              <span class="text-gray-400 dark:text-gray-500">{{ row.availability_percentage placeholderplaceholder%</span>
            </div>
            ...
            >
              {{ t('admin.ops.concurrency.rateLimited', { count: row.rate_limited_accounts placeholder) placeholderplaceholder
            </span>
            ...
            <template v-if="row.scope_rate_limit_count && Object.keys(row.scope_rate_limit_count).length > 0">
              <span
                ...
                :title="t('admin.ops.concurrency.scopeRateLimitedTooltip', { scope, count placeholder)"
              >
                {{ formatScopeName(scope as string) placeholderplaceholder {{ count placeholderplaceholder
              </span>
            </template>
            ...
            >
              {{ t('admin.ops.concurrency.errorAccounts', { count: row.error_accounts placeholder) placeholderplaceholder
            </span>
          ...
          <div class="mb-1.5 flex items-center justify-between gap-2">
            <div class="min-w-0 flex-1">
              ...
              <div class="mt-0.5 text-[9px] text-gray-400 dark:text-gray-500">
                {{ row.group_name placeholderplaceholder
              </div>
            </div>
            <div class="flex shrink-0 items-center gap-2">
              <!-- 并发使用 -->
              <span class="font-mono text-[11px] font-bold text-gray-900 dark:text-white"> {{ row.current_in_use placeholderplaceholder/{{ row.max_capacity placeholderplaceholder </span>
              <!-- 状态徽章 -->
              <span
              ...
              >
                ...
                {{ t('admin.ops.accountAvailability.available') placeholderplaceholder
              </span>
              <span
              ...
              >
                ...
                {{ formatDuration(row.rate_limit_remaining_sec || 0) placeholderplaceholder
              </span>
              <span
              ...
              >
                ...
                {{ formatDuration(row.overload_remaining_sec || 0) placeholderplaceholder
              </span>
              <span
              ...
              >
                ...
                {{ t('admin.ops.accountAvailability.accountError') placeholderplaceholder
              </span>
              <span
                v-else
              ...
              >
                {{ t('admin.ops.accountAvailability.unavailable') placeholderplaceholder
              </span>
            </div>
          </div>
