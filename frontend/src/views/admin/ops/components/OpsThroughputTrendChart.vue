...
import { Chart as ChartJS, CategoryScale, Filler, Legend, LineElement, LinearScale, PointElement, Title, Tooltip placeholder from 'chart.js'
import { Line placeholder from 'vue-chartjs'
import type { ChartComponentRef placeholder from 'vue-chartjs'
import type { OpsThroughputGroupBreakdownItem, OpsThroughputPlatformBreakdownItem, OpsThroughputTrendPoint placeholder from '@/api/admin/ops'
...
const options = computed(() => {
  ...
  return {
    ...
    plugins: {
      ...
      zoom: {
        pan: { enabled: true, mode: 'x' as const, modifierKey: 'ctrl' as const placeholder,
        zoom: { wheel: { enabled: true placeholder, pinch: { enabled: true placeholder, mode: 'x' as const placeholder
      placeholder
    placeholder,
    scales: {
      x: {
      ...
      y: {
        ...
        ticks: { color: c.text, font: { size: 10 placeholder placeholder
      placeholder,
      y1: {
        type: 'linear' as const,
        display: true,
        ...
        grid: { display: false placeholder,
        ticks: { color: c.green, font: { size: 10 placeholder placeholder
      placeholder
    placeholder
  placeholder
placeholder)
...
function downloadChart() {
  ...
  a.href = url
  a.download = `ops-throughput-${new Date().toISOString().slice(0, 19).replace(/[:T]/g, '-')placeholder.png`
  a.click()
placeholder
</script>
  ...
  <div class="flex h-full flex-col rounded-3xl bg-white p-6 shadow-sm ring-1 ring-gray-900/5 dark:bg-dark-800 dark:ring-dark-700">
    <div class="mb-4 flex shrink-0 items-center justify-between">
      <h3 class="flex items-center gap-2 text-sm font-bold text-gray-900 dark:text-white">
        ...
        {{ t('admin.ops.throughputTrend') placeholderplaceholder
        <HelpTooltip v-if="!props.fullscreen" :content="t('admin.ops.tooltips.throughputTrend')" />
      </h3>
      <div class="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
        <span class="flex items-center gap-1"><span class="h-2 w-2 rounded-full bg-blue-500"></span>{{ t('admin.ops.qps') placeholderplaceholder</span>
        <span class="flex items-center gap-1"><span class="h-2 w-2 rounded-full bg-green-500"></span>{{ t('admin.ops.tpsK') placeholderplaceholder</span>
        <template v-if="!props.fullscreen">
          ...
          <button
            ...
            @click="resetZoom"
          >
            {{ t('admin.ops.charts.resetZoom') placeholderplaceholder
          </button>
          <button
            type="button"
            ...
            @click="downloadChart"
          >
            {{ t('admin.ops.charts.downloadChart') placeholderplaceholder
          </button>
        </template>
      </div>
    </div>
    ...
    <div v-if="(props.topGroups?.length ?? 0) > 0" class="mb-3 flex flex-wrap gap-2">
      ...
      >
        <span class="max-w-[180px] truncate">{{ g.group_name || `#${g.group_idplaceholder` placeholderplaceholder</span>
        <span class="text-gray-400 dark:text-gray-500">{{ formatNumber(g.request_count) placeholderplaceholder</span>
      </button>
    </div>
    ...
    <div v-else-if="(props.byPlatform?.length ?? 0) > 0" class="mb-3 flex flex-wrap gap-2">
      ...
      >
        <span class="uppercase">{{ p.platform placeholderplaceholder</span>
        <span class="text-gray-400 dark:text-gray-500">{{ formatNumber(p.request_count) placeholderplaceholder</span>
      </button>
    </div>
