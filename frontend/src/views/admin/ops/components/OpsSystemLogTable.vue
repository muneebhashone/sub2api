...
const cleanupCurrentFilter = async () => {
  ...
  try {
    ...
    appStore.showSuccess(`清理完成，删除 ${res.deleted || 0placeholder 条日志`)
    page.value = 1
  ...
  <section class="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm dark:border-dark-700 dark:bg-dark-900/60">
    <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
      ...
      <div class="flex flex-wrap items-center gap-2 text-xs">
        <span class="rounded-md bg-gray-100 px-2 py-1 text-gray-700 dark:bg-dark-700 dark:text-gray-200">队列 {{ health.queue_depth placeholderplaceholder/{{ health.queue_capacity placeholderplaceholder</span>
        <span class="rounded-md bg-gray-100 px-2 py-1 text-gray-700 dark:bg-dark-700 dark:text-gray-200">写入 {{ health.written_count placeholderplaceholder</span>
        <span class="rounded-md bg-amber-100 px-2 py-1 text-amber-700 dark:bg-amber-900/30 dark:text-amber-300">丢弃 {{ health.dropped_count placeholderplaceholder</span>
        <span class="rounded-md bg-red-100 px-2 py-1 text-red-700 dark:bg-red-900/30 dark:text-red-300">失败 {{ health.write_failed_count placeholderplaceholder</span>
      </div>
    </div>
    ...
    <div class="mb-4 rounded-xl border border-gray-200 bg-gray-50 p-3 dark:border-dark-700 dark:bg-dark-800/70">
      ...
      <div class="grid grid-cols-1 gap-3 md:grid-cols-6">
        ...
        <div class="flex items-end gap-2">
          ...
          <button type="button" class="btn btn-primary btn-sm" :disabled="runtimeSaving" @click="saveRuntimeConfig">
            {{ runtimeSaving ? '保存中...' : '保存并生效' placeholderplaceholder
          </button>
      ...
      <p v-if="health.last_error" class="mt-2 text-xs text-red-600 dark:text-red-400">最近写入错误：{{ health.last_error placeholderplaceholder</p>
    </div>
        ...
        <table class="min-w-full divide-y divide-gray-200 dark:divide-dark-700">
          ...
          <tbody class="divide-y divide-gray-100 dark:divide-dark-800">
            <tr v-for="row in logs" :key="row.id" class="align-top">
              ...
              <td class="px-3 py-2 text-xs text-gray-700 dark:text-gray-300">{{ row.component || '-' placeholderplaceholder</td>
              <td class="max-w-[680px] px-3 py-2 text-xs text-gray-700 dark:text-gray-300">{{ row.message placeholderplaceholder</td>
              <td class="px-3 py-2 text-xs text-gray-600 dark:text-gray-400">
                <div>req: {{ row.request_id || '-' placeholderplaceholder</div>
                <div>client: {{ row.client_request_id || '-' placeholderplaceholder</div>
                <div>user: {{ row.user_id || '-' placeholderplaceholder / acc: {{ row.account_id || '-' placeholderplaceholder</div>
                <div>{{ row.platform || '-' placeholderplaceholder / {{ row.model || '-' placeholderplaceholder</div>
              </td>
            </tr>
          </tbody>
        </table>
