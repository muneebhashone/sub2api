              ...
              <div v-if="!isDesktopViewport" class="divide-y divide-gray-100 dark:divide-dark-800">
                <div v-for="(row, idx) in items" :key="idx" class="space-y-2 p-4">
                  <div class="flex flex-wrap items-center gap-2">
                    ...
                    <span class="text-xs font-medium text-gray-700 dark:text-gray-200">{{ (row.platform || 'unknown').toUpperCase() placeholderplaceholder</span>
                    <span class="ml-auto text-[11px] text-gray-500 dark:text-gray-400">{{ formatDateTime(row.created_at) placeholderplaceholder</span>
                  </div>
                  <div class="break-all text-xs text-gray-600 dark:text-gray-300">{{ row.model || '-' placeholderplaceholder</div>
                  <div class="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-gray-600 dark:text-gray-300">
                    <span>{{ typeof row.duration_ms === 'number' ? `${row.duration_msplaceholder ms` : '-' placeholderplaceholder</span>
                    <span>{{ row.status_code ?? '-' placeholderplaceholder</span>
                  </div>
