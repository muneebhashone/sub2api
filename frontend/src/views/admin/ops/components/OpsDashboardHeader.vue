...
import { opsAPI, type OpsDashboardOverview, type OpsMetricThresholds, type OpsRealtimeTrafficSummary placeholder from '@/api/admin/ops'
...
function formatCustomTimeRangeLabel(startTime: string, endTime: string): string {
  ...
  const formatDate = (d: Date) => {
    ...
    return `${monthplaceholder-${dayplaceholder ${hourplaceholder:${minuteplaceholder`
  placeholder
  return `${formatDate(start)placeholder ~ ${formatDate(end)placeholder`
placeholder
...
const timeRangeOptions = computed(() => [
  ...
  { value: '24h', label: t('admin.ops.timeRange.24h') placeholder,
  {
    ...
    label: props.timeRange === 'custom' && props.customStartTime && props.customEndTime
      ? `${t('admin.ops.timeRange.custom')placeholder (${formatCustomTimeRangeLabel(props.customStartTime, props.customEndTime)placeholder)`
      : t('admin.ops.timeRange.custom')
  placeholder
])

const queryModeOptions = computed(() => [
  { value: 'auto', label: t('admin.ops.queryMode.auto') placeholder,
  { value: 'raw', label: t('admin.ops.queryMode.raw') placeholder,
  { value: 'preagg', label: t('admin.ops.queryMode.preagg') placeholder
])

const groupOptions = computed(() => {
  const filtered = props.platform ? groups.value.filter((g) => g.platform === props.platform) : groups.value
  return [{ value: null, label: t('common.all') placeholder, ...filtered.map((g) => ({ value: g.id, label: g.name placeholder))]
placeholder)
...
onMounted(async () => {
  try {
    ...
    groups.value = list.map((g) => ({ id: g.id, name: g.name, platform: g.platform placeholder))
  placeholder catch (e) {
...
function makeZeroRealtimeTrafficSummary(): OpsRealtimeTrafficSummary {
  ...
  return {
    ...
    qps: { current: 0, peak: 0, avg: 0 placeholder,
    tps: { current: 0, peak: 0, avg: 0 placeholder
  placeholder
placeholder
...
const diagnosisReport = computed<DiagnosisItem[]>(() => {
  ...
  if (sm) {
    ...
    if (cpuPct > 90) {
      report.push({
        type: 'critical',
        message: t('admin.ops.diagnosis.cpuCritical', { usage: cpuPct.toFixed(1) placeholder),
        impact: t('admin.ops.diagnosis.cpuCriticalImpact'),
        action: t('admin.ops.diagnosis.cpuCriticalAction')
      placeholder)
    placeholder else if (cpuPct > 80) {
      report.push({
        type: 'warning',
        message: t('admin.ops.diagnosis.cpuHigh', { usage: cpuPct.toFixed(1) placeholder),
    ...
    if (memPct > 90) {
      report.push({
        type: 'critical',
        message: t('admin.ops.diagnosis.memoryCritical', { usage: memPct.toFixed(1) placeholder),
        impact: t('admin.ops.diagnosis.memoryCriticalImpact'),
        action: t('admin.ops.diagnosis.memoryCriticalAction')
      placeholder)
    placeholder else if (memPct > 85) {
      report.push({
        type: 'warning',
        message: t('admin.ops.diagnosis.memoryHigh', { usage: memPct.toFixed(1) placeholder),
  ...
  if (ttftP99 > 500) {
    report.push({
      type: 'warning',
      message: t('admin.ops.diagnosis.ttftHigh', { ttft: ttftP99.toFixed(0) placeholder),
  ...
  if (upstreamRatePct > 5) {
    ...
    placeholder)
  placeholder else if (upstreamRatePct > 2) {
    report.push({
      type: 'warning',
  ...
  if (errorPct > 3) {
    ...
    placeholder)
  placeholder else if (errorPct > 0.5) {
    report.push({
      type: 'warning',
    ...
    <div class="flex flex-wrap items-center justify-between gap-4 border-b border-gray-100 pb-4 dark:border-dark-700">
      <div>
        ...
        <div v-if="!props.fullscreen" class="mt-1 flex items-center gap-3 text-xs text-gray-500 dark:text-gray-400">
          ...
          <span>·</span>
          <span>{{ t('common.refresh') placeholderplaceholder: {{ props.lastUpdated ? props.lastUpdated.toLocaleString('zh-CN', { year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', second: '2-digit' placeholder).replace(/\//g, '-') : t('common.unknown') placeholderplaceholder</span>
      ...
      <div class="flex flex-wrap items-center gap-3">
        ...
        >
          <svg class="h-4 w-4" :class="{ 'animate-spin': loading placeholder" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path
              stroke-linecap="round"
              stroke-linejoin="round"
              stroke-width="2"
        ...
        >
          ...
          <span class="hidden sm:inline">{{ t('admin.ops.alertRules.manage') placeholderplaceholder</span>
        </button>
        ...
        >
          ...
          <span class="hidden sm:inline">{{ t('common.settings') placeholderplaceholder</span>
        </button>
        ...
        <div class="grid h-full grid-cols-1 gap-6 md:grid-cols-[200px_1fr] md:items-center">
          ...
          >
                  ...
                  <div v-for="(item, idx) in diagnosisReport" :key="idx" class="flex gap-3">
                    ...
                    <div class="flex-1">
                      ...
                      <div v-if="item.action" class="mt-1 text-[11px] text-blue-600 dark:text-blue-400 flex items-center gap-1">
                        ...
                        {{ item.action placeholderplaceholder
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <div class="relative flex items-center justify-center">
              ...
              <div class="absolute flex flex-col items-center">
                ...
                <span :class="[props.fullscreen ? 'text-xs' : 'text-[10px]', 'font-bold uppercase tracking-wider text-gray-400']">{{ t('admin.ops.health') placeholderplaceholder</span>
              </div>
            </div>
          ...
          <div class="flex h-full flex-col justify-center py-2">
            ...
            <div :class="props.fullscreen ? 'space-y-4' : 'space-y-3'">
              <!-- Row 1: Current -->
              <div>
                <div :class="[props.fullscreen ? 'text-xs' : 'text-[10px]', 'font-bold uppercase text-gray-400']">{{ t('admin.ops.current') placeholderplaceholder</div>
              ...
              <div class="grid grid-cols-2 gap-3">
                <!-- Peak -->
                <div>
                  <div :class="[props.fullscreen ? 'text-xs' : 'text-[10px]', 'font-bold uppercase text-gray-400']">{{ t('admin.ops.peak') placeholderplaceholder</div>
                ...
                <!-- Average -->
                <div>
                  <div :class="[props.fullscreen ? 'text-xs' : 'text-[10px]', 'font-bold uppercase text-gray-400']">{{ t('admin.ops.average') placeholderplaceholder</div>
                  <div :class="[props.fullscreen ? 'text-base' : 'text-sm', 'mt-1 space-y-0.5 font-medium text-gray-600 dark:text-gray-400']">
                    <div class="flex items-baseline gap-1.5">
      ...
      <div class="grid h-full grid-cols-1 content-center gap-4 sm:grid-cols-2 lg:col-span-7 lg:grid-cols-3">
        ...
        <div class="rounded-2xl bg-gray-50 p-4 dark:bg-dark-900" style="order: 1;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              <span class="text-[10px] font-bold uppercase text-gray-400">{{ t('admin.ops.requestsTitle') placeholderplaceholder</span>
        ...
        <div class="rounded-2xl bg-gray-50 p-4 dark:bg-dark-900" style="order: 2;">
          ...
          <div class="mt-3 h-2 w-full overflow-hidden rounded-full bg-gray-200 dark:bg-dark-700">
            <div class="h-full transition-all" :class="getSLAThresholdLevel(slaPercent) === 'critical' ? 'bg-red-500' : getSLAThresholdLevel(slaPercent) === 'warning' ? 'bg-yellow-500' : 'bg-green-500'" :style="{ width: `${Math.max((slaPercent ?? 0) - 90, 0) * 10placeholder%` placeholder"></div>
          </div>
          <div class="mt-3 text-xs">
            <div class="flex justify-between">
              <span class="text-gray-500">{{ t('admin.ops.exceptions') placeholderplaceholder:</span>
              <span class="font-bold text-gray-900 dark:text-white">{{ formatNumber((overview.request_count_sla ?? 0) - (overview.success_count ?? 0)) placeholderplaceholder</span>
            </div>
          </div>
        </div>
        ...
        <div class="rounded-2xl bg-gray-50 p-4 dark:bg-dark-900" style="order: 4;">
          <div class="flex items-center justify-between">
            ...
            <button
              ...
              class="text-[10px] font-bold text-blue-500 hover:underline"
              type="button"
              @click="openDetails({ title: t('admin.ops.latencyDuration'), sort: 'duration_desc' placeholder)"
            >
              {{ t('admin.ops.requestDetails.details') placeholderplaceholder
            </button>
          </div>
        ...
        <div class="rounded-2xl bg-gray-50 p-4 dark:bg-dark-900" style="order: 5;">
          ...
          <div class="mt-3 grid grid-cols-1 gap-x-3 gap-y-1 text-xs 2xl:grid-cols-2">
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-gray-500">P95:</span>
              <span class="font-bold" :class="getThresholdColorClass(getTTFTThresholdLevel(ttftP95Ms))">{{ ttftP95Ms ?? '-' placeholderplaceholder</span>
              <span class="text-gray-400">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-gray-500">P90:</span>
              <span class="font-bold" :class="getThresholdColorClass(getTTFTThresholdLevel(ttftP90Ms))">{{ ttftP90Ms ?? '-' placeholderplaceholder</span>
              <span class="text-gray-400">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-gray-500">P50:</span>
              <span class="font-bold" :class="getThresholdColorClass(getTTFTThresholdLevel(ttftP50Ms))">{{ ttftP50Ms ?? '-' placeholderplaceholder</span>
              <span class="text-gray-400">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-gray-500">Avg:</span>
              <span class="font-bold" :class="getThresholdColorClass(getTTFTThresholdLevel(ttftAvgMs))">{{ ttftAvgMs ?? '-' placeholderplaceholder</span>
              <span class="text-gray-400">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-gray-500">Max:</span>
              <span class="font-bold" :class="getThresholdColorClass(getTTFTThresholdLevel(ttftMaxMs))">{{ ttftMaxMs ?? '-' placeholderplaceholder</span>
              <span class="text-gray-400">ms</span>
            </div>
          </div>
        </div>
      ...
      <div class="space-y-4">
        <div>
          <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
            {{ t('admin.ops.customTimeRange.startTime') placeholderplaceholder
          </label>
          <input
        ...
        <div>
          <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
            {{ t('admin.ops.customTimeRange.endTime') placeholderplaceholder
          </label>
          <input
