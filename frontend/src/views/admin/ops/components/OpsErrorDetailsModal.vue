...
import { opsAPI, type OpsErrorLog placeholder from '@/api/admin/ops'
import { buildOpsErrorTimeParams placeholder from '../utils/opsErrorParams'
...
const statusCodeSelectOptions = computed(() => {
  ...
  return [
    { value: null, label: t('common.all') placeholder,
    ...codes.map((c) => ({ value: c, label: String(c) placeholder)),
    { value: 'other', label: t('admin.ops.errorDetails.statusCodeOther') || 'Other' placeholder
  ]
placeholder)
...
const ownerSelectOptions = computed(() => {
  return [
    ...
    { value: 'provider', label: t('admin.ops.errorDetails.owner.provider') || 'provider' placeholder,
    { value: 'client', label: t('admin.ops.errorDetails.owner.client') || 'client' placeholder,
    { value: 'platform', label: t('admin.ops.errorDetails.owner.platform') || 'platform' placeholder
  ]
placeholder)
...
const viewModeSelectOptions = computed(() => {
  return [
    { value: 'errors', label: t('admin.ops.errorDetails.viewErrors') || 'errors' placeholder,
    { value: 'excluded', label: t('admin.ops.errorDetails.viewExcluded') || 'excluded' placeholder,
    { value: 'all', label: t('common.all') placeholder
  ]
placeholder)
...
const phaseSelectOptions = computed(() => {
  const options = [
    { value: '', label: t('common.all') placeholder,
    { value: 'request', label: t('admin.ops.errorDetails.phase.request') || 'request' placeholder,
    { value: 'auth', label: t('admin.ops.errorDetails.phase.auth') || 'auth' placeholder,
    { value: 'account_auth', label: t('admin.ops.errorDetails.phase.account_auth') || 'account_auth' placeholder,
    { value: 'routing', label: t('admin.ops.errorDetails.phase.routing') || 'routing' placeholder,
    { value: 'upstream', label: t('admin.ops.errorDetails.phase.upstream') || 'upstream' placeholder,
    { value: 'network', label: t('admin.ops.errorDetails.phase.network') || 'network' placeholder,
    { value: 'internal', label: t('admin.ops.errorDetails.phase.internal') || 'internal' placeholder
  ]
  return options
placeholder)
