    ...
    <div v-else class="space-y-6 p-6">
      ...
      <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div v-if="detail.attempted_key_prefix" class="rounded-xl bg-gray-50 p-4 dark:bg-dark-900">
          <div class="text-xs font-bold uppercase tracking-wider text-gray-400">{{ t('admin.ops.errorDetail.attemptedKeyPrefix') placeholderplaceholder</div>
          <div class="mt-1 font-mono text-sm font-medium text-gray-900 dark:text-white">
            {{ detail.attempted_key_prefix placeholderplaceholder
          </div>
        </div>

        <div v-if="detail.deleted_key_owner_email" class="rounded-xl bg-gray-50 p-4 dark:bg-dark-900">
          <div class="text-xs font-bold uppercase tracking-wider text-gray-400">{{ t('admin.ops.errorDetail.deletedKeyOwner') placeholderplaceholder</div>
          <div class="mt-1 text-sm font-medium text-gray-900 dark:text-white">
            {{ detail.deleted_key_owner_email placeholderplaceholder
            <span v-if="detail.deleted_key_name" class="ml-1 text-xs text-gray-500 dark:text-gray-400">({{ detail.deleted_key_name placeholderplaceholder)</span>
            <span class="ml-2 inline-flex items-center rounded px-1.5 py-0.5 text-[10px] font-bold ring-1 ring-inset bg-red-50 text-red-700 ring-red-600/20 dark:bg-red-900/30 dark:text-red-400 dark:ring-red-500/30">
              {{ t('admin.ops.errorDetail.keyDeletedBadge') placeholderplaceholder
            </span>
          </div>
        </div>
      </div>
      ...
      <div class="rounded-xl bg-gray-50 p-6 dark:bg-dark-900">
        ...
        <pre class="mt-4 max-h-[520px] overflow-auto rounded-xl border border-gray-200 bg-white p-4 text-xs text-gray-800 dark:border-dark-700 dark:bg-dark-800 dark:text-gray-100"><code>{{ prettyJSON(primaryResponseBody || '') placeholderplaceholder</code></pre>
      </div>
...
import { resolvePrimaryResponseBody, resolveUpstreamPayload placeholder from '../utils/errorDetailResponse'
