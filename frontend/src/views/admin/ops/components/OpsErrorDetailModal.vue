  ...
  <BaseDialog :show="show" :title="title" width="full" :close-on-click-outside="true" @close="close">
    ...
    <div v-else class="space-y-6 p-6">
      ...
      <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div class="rounded-xl bg-gray-50 p-4 dark:bg-dark-900">
          ...
          <div class="mt-1 break-all font-mono text-sm font-medium text-gray-900 dark:text-white">
            {{ requestId || '—' placeholderplaceholder
          </div>
        </div>
        ...
        <div class="rounded-xl bg-gray-50 p-4 dark:bg-dark-900">
          <div class="text-xs font-bold uppercase tracking-wider text-gray-400">{{ t('admin.ops.errorDetail.platform') placeholderplaceholder</div>
          <div class="mt-1 text-sm font-medium text-gray-900 dark:text-white">
            {{ detail.platform || '—' placeholderplaceholder
          </div>
        </div>

        <div class="rounded-xl bg-gray-50 p-4 dark:bg-dark-900">
          <div class="text-xs font-bold uppercase tracking-wider text-gray-400">{{ t('admin.ops.errorDetail.group') placeholderplaceholder</div>
          <div class="mt-1 text-sm font-medium text-gray-900 dark:text-white">
            {{ detail.group_name || (detail.group_id != null ? String(detail.group_id) : '—') placeholderplaceholder
          </div>
        </div>
        ...
        <div class="rounded-xl bg-gray-50 p-4 dark:bg-dark-900">
          ...
          <div class="mt-1 text-sm font-medium text-gray-900 dark:text-white">
            ...
            <template v-else>
              {{ displayModel(detail) || '—' placeholderplaceholder
            </template>
          </div>
        </div>
        ...
        <div class="rounded-xl bg-gray-50 p-4 dark:bg-dark-900">
          <div class="text-xs font-bold uppercase tracking-wider text-gray-400">{{ t('admin.ops.errorDetail.upstreamStatus') placeholderplaceholder</div>
          <div class="mt-1">
            <span :class="['inline-flex items-center rounded-lg px-2 py-1 text-xs font-black ring-1 ring-inset shadow-sm', upstreamStatusClass]">
              {{ detail.upstream_status_code ?? '—' placeholderplaceholder
            </span>
          </div>
        </div>
        ...
        <div class="rounded-xl bg-gray-50 p-4 dark:bg-dark-900">
          <div class="text-xs font-bold uppercase tracking-wider text-gray-400">{{ t('admin.ops.errorDetail.message') placeholderplaceholder</div>
          <div class="mt-1 break-words text-sm font-medium text-gray-900 dark:text-white" :title="rootCauseMessage">
            {{ rootCauseMessage || '—' placeholderplaceholder
          </div>
        </div>

        <div v-if="detail.api_key_prefix" class="rounded-xl bg-gray-50 p-4 dark:bg-dark-900">
          <div class="text-xs font-bold uppercase tracking-wider text-gray-400">{{ t('admin.ops.errorDetail.apiKeyPrefix') placeholderplaceholder</div>
          <div class="mt-1 font-mono text-sm font-medium text-gray-900 dark:text-white">
            {{ detail.api_key_prefix placeholderplaceholder
          </div>
        </div>
      ...
      <div v-if="rootCauseMessage" class="rounded-xl bg-amber-50 p-6 dark:bg-amber-900/10">
        <h3 class="text-sm font-black uppercase tracking-wider text-amber-900 dark:text-amber-200">{{ t('admin.ops.errorDetail.rootCause') placeholderplaceholder</h3>
        <div class="mt-3 break-words text-sm font-medium text-amber-900 dark:text-amber-100">{{ rootCauseMessage placeholderplaceholder</div>
      </div>

      <div class="rounded-xl bg-gray-50 p-6 dark:bg-dark-900">
        <h3 class="text-sm font-black uppercase tracking-wider text-gray-900 dark:text-white">{{ t('admin.ops.errorDetail.diagnosticPayloads') placeholderplaceholder</h3>
        <div v-if="!diagnosticPayloadSections.length" class="mt-4 text-sm text-gray-500 dark:text-gray-400">{{ t('common.noData') placeholderplaceholder</div>
        <div v-else class="mt-4 space-y-4">
          <div v-for="section in diagnosticPayloadSections" :key="section.key">
            <div class="mb-2 text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400">{{ diagnosticPayloadLabel(section.key) placeholderplaceholder</div>
            <pre class="max-h-[520px] overflow-auto rounded-xl border border-gray-200 bg-white p-4 text-xs text-gray-800 dark:border-dark-700 dark:bg-dark-800 dark:text-gray-100"><code>{{ prettyJSON(section.value) placeholderplaceholder</code></pre>
          </div>
        </div>
      </div>
      ...
      <div v-if="showUpstreamList" class="rounded-xl bg-gray-50 p-6 dark:bg-dark-900">
        <div class="flex flex-wrap items-center justify-between gap-2">
          <h3 class="text-sm font-black uppercase tracking-wider text-gray-900 dark:text-white">{{ t('admin.ops.errorDetails.upstreamErrors') placeholderplaceholder</h3>
          <div class="text-xs text-gray-500 dark:text-gray-400" v-if="correlatedUpstreamLoading">{{ t('common.loading') placeholderplaceholder</div>
        </div>

        <div v-if="!correlatedUpstreamLoading && !correlatedUpstreamErrors.length" class="mt-3 text-sm text-gray-500 dark:text-gray-400">
          {{ t('common.noData') placeholderplaceholder
        </div>
          ...
          >
            <div class="flex flex-wrap items-center justify-between gap-2">
              <div class="text-xs font-black text-gray-900 dark:text-white">
                #{{ idx + 1 placeholderplaceholder
                <span v-if="ev.type" class="ml-2 rounded-md bg-gray-100 px-2 py-0.5 font-mono text-[10px] font-bold text-gray-700 dark:bg-dark-700 dark:text-gray-200">{{ ev.type placeholderplaceholder</span>
              </div>
              <div class="flex items-center gap-2">
                <div class="font-mono text-xs text-gray-500 dark:text-gray-400">
                  {{ ev.status_code ?? '—' placeholderplaceholder
                </div>
                <button
                  type="button"
            ...
            <div class="mt-3 grid grid-cols-1 gap-2 text-xs text-gray-600 dark:text-gray-300 sm:grid-cols-2">
              <div>
                ...
                <span class="ml-1 font-mono">{{ ev.status_code ?? '—' placeholderplaceholder</span>
              </div>
              <div>
                <span class="text-gray-400">{{ t('admin.ops.errorDetail.upstreamEvent.requestId') placeholderplaceholder:</span>
                <span class="ml-1 font-mono">{{ ev.request_id || ev.client_request_id || '—' placeholderplaceholder</span>
              </div>
            </div>

            <div v-if="ev.message" class="mt-3 break-words text-sm font-medium text-gray-900 dark:text-white">{{ ev.message placeholderplaceholder</div>
            ...
            ><code>{{ prettyJSON(getUpstreamResponsePreview(ev)) placeholderplaceholder</code></pre>
          </div>
        </div>
      </div>
    </div>
    <template v-if="backToList" #footer>
      <button
        ...
        @click="goBack"
      >
        {{ t('admin.ops.errorDetail.backToList') placeholderplaceholder
      </button>
    </template>
  </BaseDialog>
</template>
...
import { opsAPI, type OpsErrorDetail placeholder from '@/api/admin/ops'
import { formatDateTime placeholder from '@/utils/format'
import { resolveUpstreamPayload placeholder from '../utils/errorDetailResponse'
...
const diagnosticPayloadSections = computed(() => {
  const current = detail.value
  if (!current) return []
  const candidates: Array<{ key: DiagnosticPayloadKey; value: string placeholder> = [
    { key: 'client', value: meaningfulPayload(current.error_body) placeholder,
    { key: 'upstream_message', value: meaningfulPayload(current.upstream_error_message) placeholder,
    { key: 'upstream_detail', value: meaningfulPayload(current.upstream_error_detail) placeholder,
    { key: 'upstream_events', value: meaningfulPayload(current.upstream_errors) placeholder
  ]
...
function meaningfulPayload(candidate: unknown): string {
  ...
  if (!value || value === '[]' || value === '{placeholder' || value.toLowerCase() === 'null') return ''
  return value
placeholder

function diagnosticPayloadLabel(key: DiagnosticPayloadKey): string {
  return t(`admin.ops.errorDetail.payloads.${keyplaceholder`)
placeholder
...
async function fetchCorrelatedUpstreamErrors(requestErrorId: number) {
  ...
  try {
    const res = await opsAPI.listRequestErrorUpstreamErrors(
      requestErrorId,
      { page: 1, page_size: 100, view: 'all' placeholder,
      { include_detail: true placeholder
    )
