      ...
      <template v-else-if="stats">
        ...
        <div class="grid grid-cols-2 gap-4 lg:grid-cols-4">
          ...
          <div class="card p-4">
            <div class="flex items-center gap-3">
              ...
              <div>
                <p class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('admin.dashboard.apiKeys') placeholderplaceholder</p>
                <p class="text-xl font-bold text-gray-900 dark:text-white">{{ stats.total_api_keys placeholderplaceholder</p>
                <p class="text-xs text-green-600 dark:text-green-400">{{ stats.active_api_keys placeholderplaceholder {{ t('common.active') placeholderplaceholder</p>
              </div>
            </div>
          </div>
          ...
          <div class="card p-4">
            <div class="flex items-center gap-3">
              ...
              <div>
                <p class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('admin.dashboard.accounts') placeholderplaceholder</p>
                <p class="text-xl font-bold text-gray-900 dark:text-white">{{ stats.total_accounts placeholderplaceholder</p>
                <p class="text-xs">
                  <span class="text-green-600 dark:text-green-400">{{ stats.normal_accounts placeholderplaceholder {{ t('common.active') placeholderplaceholder</span>
                  <span v-if="stats.error_accounts > 0" class="text-red-500 ml-1">{{ stats.error_accounts placeholderplaceholder {{ t('common.error') placeholderplaceholder</span>
                </p>
              </div>
            </div>
          </div>
          ...
          <div class="card p-4">
            <div class="flex items-center gap-3">
              ...
              <div>
                <p class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('admin.dashboard.todayRequests') placeholderplaceholder</p>
                <p class="text-xl font-bold text-gray-900 dark:text-white">{{ stats.today_requests placeholderplaceholder</p>
                <p class="text-xs text-gray-500 dark:text-gray-400">{{ t('common.total') placeholderplaceholder: {{ formatNumber(stats.total_requests) placeholderplaceholder</p>
              </div>
            </div>
          </div>
          ...
          <div class="card p-4">
            <div class="flex items-center gap-3">
              ...
              <div>
                <p class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('admin.dashboard.users') placeholderplaceholder</p>
                <p class="text-xl font-bold text-emerald-600 dark:text-emerald-400">+{{ stats.today_new_users placeholderplaceholder</p>
                <p class="text-xs text-gray-500 dark:text-gray-400">{{ t('common.total') placeholderplaceholder: {{ formatNumber(stats.total_users) placeholderplaceholder</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4 lg:grid-cols-4">
          ...
          <div class="card p-4">
            <div class="flex items-center gap-3">
              ...
              <div>
                <p class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('admin.dashboard.todayTokens') placeholderplaceholder</p>
                <p class="text-xl font-bold text-gray-900 dark:text-white">{{ formatTokens(stats.today_tokens) placeholderplaceholder</p>
                <p class="text-xs">
                  <span class="text-amber-600 dark:text-amber-400" :title="t('admin.dashboard.actual')">${{ formatCost(stats.today_actual_cost) placeholderplaceholder</span>
                  <span class="text-gray-400 dark:text-gray-500" :title="t('admin.dashboard.standard')"> / ${{ formatCost(stats.today_cost) placeholderplaceholder</span>
                </p>
              </div>
            </div>
          </div>
          ...
          <div class="card p-4">
            <div class="flex items-center gap-3">
              ...
              <div>
                <p class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('admin.dashboard.totalTokens') placeholderplaceholder</p>
                <p class="text-xl font-bold text-gray-900 dark:text-white">{{ formatTokens(stats.total_tokens) placeholderplaceholder</p>
                <p class="text-xs">
                  <span class="text-indigo-600 dark:text-indigo-400" :title="t('admin.dashboard.actual')">${{ formatCost(stats.total_actual_cost) placeholderplaceholder</span>
                  <span class="text-gray-400 dark:text-gray-500" :title="t('admin.dashboard.standard')"> / ${{ formatCost(stats.total_cost) placeholderplaceholder</span>
                </p>
              </div>
            </div>
          </div>
          ...
          <div class="card p-4">
            <div class="flex items-center gap-3">
              ...
              <div>
                <p class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('admin.dashboard.cacheToday') placeholderplaceholder</p>
                <p class="text-xl font-bold text-gray-900 dark:text-white">{{ formatTokens(stats.today_cache_read_tokens) placeholderplaceholder</p>
                <p class="text-xs text-gray-500 dark:text-gray-400">
                  {{ t('common.create') placeholderplaceholder: {{ formatTokens(stats.today_cache_creation_tokens) placeholderplaceholder
                </p>
              </div>
            </div>
          </div>
          ...
          <div class="card p-4">
            <div class="flex items-center gap-3">
              ...
              <div>
                <p class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('admin.dashboard.avgResponse') placeholderplaceholder</p>
                <p class="text-xl font-bold text-gray-900 dark:text-white">{{ formatDuration(stats.average_duration_ms) placeholderplaceholder</p>
                <p class="text-xs text-gray-500 dark:text-gray-400">{{ stats.active_users placeholderplaceholder {{ t('admin.dashboard.activeUsers') placeholderplaceholder</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="space-y-6">
          ...
          <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
            ...
            <div class="card p-4">
              <h3 class="text-sm font-semibold text-gray-900 dark:text-white mb-4">{{ t('admin.dashboard.modelDistribution') placeholderplaceholder</h3>
                  ...
                  <table class="w-full text-xs">
                    <thead>
                      <tr class="text-gray-500 dark:text-gray-400">
                        <th class="text-left pb-2">{{ t('admin.dashboard.model') placeholderplaceholder</th>
                        <th class="text-right pb-2">{{ t('admin.dashboard.requests') placeholderplaceholder</th>
                        <th class="text-right pb-2">{{ t('admin.dashboard.tokens') placeholderplaceholder</th>
                        <th class="text-right pb-2">{{ t('admin.dashboard.actual') placeholderplaceholder</th>
                        <th class="text-right pb-2">{{ t('admin.dashboard.standard') placeholderplaceholder</th>
                      </tr>
                    </thead>
                    <tbody>
                      <tr v-for="model in modelStats" :key="model.model" class="border-t border-gray-100 dark:border-gray-700">
                        <td class="py-1.5 text-gray-900 dark:text-white font-medium truncate max-w-[100px]" :title="model.model">{{ model.model placeholderplaceholder</td>
            ...
            <div class="card p-4">
              <h3 class="text-sm font-semibold text-gray-900 dark:text-white mb-4">{{ t('admin.dashboard.tokenUsageTrend') placeholderplaceholder</h3>
              <div class="h-48">
          ...
          <div class="card p-4">
            <h3 class="text-sm font-semibold text-gray-900 dark:text-white mb-4">{{ t('admin.dashboard.recentUsage') placeholderplaceholder (Top 12)</h3>
            <div class="h-64">
...
import { Line, Doughnut placeholder from 'vue-chartjs'
...
const userTrendChartData = computed(() => {
  ...
  userTrend.value.forEach(point => {
    ...
    const key = point.username || `User #${point.user_idplaceholder`
  ...
  const datasets = Array.from(userGroups.values()).map((group, idx) => ({
    ...
    backgroundColor: `${colors[idx % colors.length]placeholder20`,
    fill: false,
    tension: 0.3,
  placeholder))
