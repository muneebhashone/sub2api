<template>
  <AppLayout>
    <div class="space-y-6">
      ...
      >
        <div class="min-w-0">
          <h2 class="text-lg font-semibold text-gray-900 dark:text-white">
            {{ t("admin.plugins.title") placeholderplaceholder
          </h2>
          <p class="mt-1 max-w-3xl text-sm text-gray-500 dark:text-gray-400">
            {{ t("admin.plugins.description") placeholderplaceholder
          </p>
          <div
        ...
        <div class="flex flex-shrink-0 items-center gap-2">
          ...
          >
            ...
            {{ uploading ? t("common.processing") : t("admin.plugins.upload") placeholderplaceholder
          </button>
          <button
            type="button"
          ...
          >
            ...
            <span class="sr-only">{{ t("common.refresh") placeholderplaceholder</span>
          </button>
        </div>
      </section>

      <p class="text-xs text-gray-500 dark:text-gray-400">
        {{ t("admin.plugins.uploadHint") placeholderplaceholder
      </p>
      ...
      >
        <p>{{ t("admin.plugins.runtimeNotice") placeholderplaceholder</p>
        <p class="mt-1">{{ t("admin.plugins.menuNotice") placeholderplaceholder</p>
      </div>
      ...
      >
        ...
        <p class="mt-3 font-medium text-gray-800 dark:text-gray-200">
          {{ t("admin.plugins.empty") placeholderplaceholder
        </p>
        <p class="mt-1 max-w-lg text-sm text-gray-500 dark:text-gray-400">
          {{ t("admin.plugins.emptyHint") placeholderplaceholder
        </p>
      </div>
        ...
        >
          ...
          >
            <div class="min-w-0">
              <div class="flex flex-wrap items-center gap-2">
                ...
                >
                  {{ plugin.name placeholderplaceholder
                </h3>
                <span class="font-mono text-xs text-gray-500"
                  >v{{ plugin.version placeholderplaceholder</span
                >
                <span
                  class="rounded px-2 py-0.5 text-xs font-medium"
                  :class="stateClass(plugin.state)"
                >
                  {{ t(`admin.plugins.${plugin.stateplaceholder`) placeholderplaceholder
                </span>
              </div>
              <p class="mt-1 text-xs text-gray-500">
                {{ plugin.plugin_key
                placeholderplaceholder<span v-if="plugin.author"> · {{ plugin.author placeholderplaceholder</span>
              </p>
              <p
                v-if="plugin.description"
                class="mt-2 text-sm text-gray-600 dark:text-gray-300"
              >
                {{ plugin.description placeholderplaceholder
              </p>
            </div>
            <button
              type="button"
            ...
            >
              ...
              {{ t("admin.plugins.configure") placeholderplaceholder
            </button>
          </div>
          ...
          <div class="grid grid-cols-1 gap-x-6 gap-y-4 p-5 md:grid-cols-2">
            <div>
              <p class="text-xs font-medium uppercase text-gray-500">
                {{ t("admin.plugins.compatibility") placeholderplaceholder
              </p>
              <div class="mt-2 flex items-center gap-2">
                ...
                >
                  {{ t(`admin.plugins.${plugin.compatibility.statusplaceholder`) placeholderplaceholder
                </span>
              ...
              >
                <dt class="text-gray-500">
                  {{ t("admin.plugins.currentVersion") placeholderplaceholder
                </dt>
                <dd class="font-mono text-gray-800 dark:text-gray-200">
                  {{ plugin.compatibility.current_sub2api_version placeholderplaceholder
                </dd>
                <dt class="text-gray-500">
                  {{ t("admin.plugins.requiredVersion") placeholderplaceholder
                </dt>
                <dd class="font-mono text-gray-800 dark:text-gray-200">
                  {{ plugin.compatibility.required_sub2api_version placeholderplaceholder
                </dd>
                <dt class="text-gray-500">
                  {{ t("admin.plugins.recommendedVersion") placeholderplaceholder
                </dt>
                <dd class="font-mono text-gray-800 dark:text-gray-200">
                  {{ plugin.compatibility.recommended_sub2api_version || "-" placeholderplaceholder
                </dd>
              </dl>
            </div>
            ...
            <div>
              <p class="text-xs font-medium uppercase text-gray-500">
                {{ t("admin.plugins.runtime") placeholderplaceholder
              </p>
              <div class="mt-2 flex flex-wrap gap-2 text-xs">
                ...
                >
                  {{ t("admin.plugins.signature") placeholderplaceholder:
                  {{ t(`admin.plugins.${plugin.signature_statusplaceholder`) placeholderplaceholder
                </span>
              </div>
              <p
              ...
              >
                {{ plugin.last_error placeholderplaceholder
              </p>
              <p
                v-else-if="plugin.runtime_message"
                class="mt-3 break-words text-xs text-gray-500"
              >
                {{ plugin.runtime_message placeholderplaceholder
              </p>
            </div>
            ...
            <div class="md:col-span-2">
              ...
              >
                <span>{{ t("admin.plugins.rollout") placeholderplaceholder</span>
          ...
          >
            ...
            >
              ...
              {{ t("admin.plugins.test") placeholderplaceholder
            </button>
            <button
            ...
            >
              ...
              {{ t("admin.plugins.disable") placeholderplaceholder
            </button>
            <button
              v-else
              type="button"
            ...
            >
              ...
              {{ t("admin.plugins.enable") placeholderplaceholder
            </button>
            <button
              type="button"
            ...
            >
              ...
              {{ t("admin.plugins.uninstall") placeholderplaceholder
            </button>
          </div>
        </article>
      </div>
      ...
      <BaseDialog
        ...
        :title="
          t('admin.plugins.configTitle', { name: configPlugin?.name || '' placeholder)
        "
        width="full"
      ...
      >
        <div
          ...
          :style="{ height: `${iframeHeightplaceholderpx` placeholder"
        >
          <div
            v-if="uiLoading"
            class="absolute inset-0 z-10 flex items-center justify-center text-sm text-gray-500"
          >
            {{ t("admin.plugins.loadingUI") placeholderplaceholder
          </div>
          <div
            v-if="uiError"
          ...
          >
            ...
            <p class="mt-3 font-medium text-gray-800 dark:text-gray-200">
              {{ t("admin.plugins.uiUnavailable") placeholderplaceholder
            </p>
            <p class="mt-1 max-w-xl text-sm text-gray-500">{{ uiError placeholderplaceholder</p>
          </div>
          <iframe
            v-if="uiSession"
            ref="pluginFrame"
...
import { onBeforeUnmount, onMounted, ref placeholder from "vue";
import { useI18n placeholder from "vue-i18n";
import {
  adminAPI,
  type PluginInstallation,
  type PluginUISession,
placeholder from "@/api/admin";
...
import {
  ...
  stepUpBlockReason,
  useStepUp,
placeholder from "@/composables/useStepUp";
...
const rolloutValues = ref<Record<number, number>>({placeholder);
...
function errorMessage(error: unknown): string {
  if (typeof error === "object" && error !== null && "message" in error) {
    return String(
      (error as { message?: unknown placeholder).message || t("common.unknownError"),
    );
  placeholder
  ...
  pluginFrame.value.contentWindow.postMessage(
    {
      ...
      type: `${request.typeplaceholder.result`,
  ...
  try {
    switch (message.type) {
      ...
      case "config.load": {
        ...
        postBridgeResult(message, { ok: true, config placeholder);
        break;
      placeholder
      case "config.save": {
        if (
      ...
      case "config.test": {
        ...
        postBridgeResult(message, { ok: result.success, result placeholder);
        if (result.success)
