  ...
  <AppLayout>
    <TablePageLayout>
        ...
        <div class="flex flex-wrap items-center gap-3">
          ...
          <div class="flex flex-1 flex-wrap items-center justify-end gap-2">
            ...
            >
              ...
              {{ t('admin.proxies.batchQualityCheck') placeholderplaceholder
            </button>
            <button
            ...
            <button @click="showExportDataDialog = true" class="btn btn-secondary">
              {{ selectedCount > 0 ? t('admin.proxies.dataExportSelected') : t('admin.proxies.dataExport') placeholderplaceholder
            </button>
      ...
      <template #table>
        ...
        >
            ...
            <div class="flex items-center gap-1.5">
              ...
              <div class="relative">
                ...
                >
                  ...
                  >
                    <span class="truncate font-mono text-gray-600 dark:text-gray-300">{{ fmt.label placeholderplaceholder</span>
                  </button>
                </div>
              </div>
            </div>
          ...
          <template #cell-auth="{ row placeholder">
            <div v-if="row.username || row.password" class="flex items-center gap-1.5">
              <div class="flex flex-col text-xs">
                <span v-if="row.username" class="text-gray-700 dark:text-gray-200">{{ row.username placeholderplaceholder</span>
                <span v-if="row.password" class="font-mono text-gray-500 dark:text-gray-400">
                  {{ visiblePasswordIds.has(row.id) ? row.password : '••••••' placeholderplaceholder
                </span>
              </div>
              <button
                v-if="row.password"
          ...
          <template #cell-latency="{ row placeholder">
            <div class="flex flex-col gap-1">
              ...
              >
                <span>{{ t('admin.proxies.qualityInline', { grade: row.quality_grade || '-', score: row.quality_score ?? '-' placeholder) placeholderplaceholder</span>
                <span class="badge" :class="qualityOverallClass(row.quality_status)">
                  {{ qualityOverallLabel(row.quality_status) placeholderplaceholder
                </span>
              </div>
            </div>
          </template>

          <template #cell-expiry="{ row placeholder">
            <span v-if="!row.expires_at" class="text-sm text-gray-400">{{ t('admin.proxies.neverExpires') placeholderplaceholder</span>
            <div v-else class="flex flex-col text-xs">
              <span class="text-gray-700 dark:text-gray-200">{{ formatDateTime(row.expires_at) placeholderplaceholder</span>
              <span :class="expiryBadgeClass(row)">{{ expiryLabel(row) placeholderplaceholder</span>
            </div>
          </template>

          <template #cell-created_at="{ row placeholder">
            <span class="text-xs text-gray-600 dark:text-gray-300">{{ formatDateTime(row.created_at) placeholderplaceholder</span>
          </template>
          ...
          <template #cell-actions="{ row placeholder">
            <div class="flex items-center gap-1">
              ...
              >
                ...
                <span class="text-xs">{{ t('admin.proxies.qualityCheck') placeholderplaceholder</span>
              </button>
              <button
    ...
    >
      ...
      >
        ...
        <div>
          <label class="input-label">{{ t('admin.proxies.expiresAt') placeholderplaceholder</label>
          <div class="mb-2 flex flex-wrap gap-2">
            ...
            >
              {{ t('admin.proxies.nDays', { days: d placeholder) placeholderplaceholder
            </button>
          </div>
          <input
        ...
        <div>
          <label class="input-label">{{ t('admin.proxies.fallbackMode') placeholderplaceholder</label>
          <Select v-model="createForm.fallback_mode" :options="[
            { label: t('admin.proxies.fallbackNone'), value: 'none' placeholder,
            { label: t('admin.proxies.fallbackProxy'), value: 'proxy' placeholder,
            { label: t('admin.proxies.fallbackDirect'), value: 'direct' placeholder,
          ]" />
        </div>
        <div v-if="createForm.fallback_mode === 'proxy'">
          <label class="input-label">{{ t('admin.proxies.backupProxy') placeholderplaceholder</label>
          ...
          <div class="flex items-center justify-between gap-4">
            <div>
              <div class="text-sm text-gray-500 dark:text-gray-400">
                {{ qualityReportProxy?.name || '-' placeholderplaceholder
              </div>
              <div class="mt-1 text-sm text-gray-700 dark:text-gray-200">
                {{ qualityReport.summary placeholderplaceholder
              </div>
            </div>
            <div class="text-right">
              <div class="text-2xl font-semibold text-gray-900 dark:text-white">
                {{ qualityReport.score placeholderplaceholder
              </div>
              <div class="text-xs text-gray-500 dark:text-gray-400">
                {{ t('admin.proxies.qualityGrade', { grade: qualityReport.grade placeholder) placeholderplaceholder
              </div>
            </div>
          </div>
          <div class="mt-3 grid grid-cols-2 gap-2 text-xs text-gray-600 dark:text-gray-300">
            <div>{{ t('admin.proxies.qualityExitIP') placeholderplaceholder: {{ qualityReport.exit_ip || '-' placeholderplaceholder</div>
            <div>{{ t('admin.proxies.qualityCountry') placeholderplaceholder: {{ qualityReport.country || '-' placeholderplaceholder</div>
            <div>
              {{ t('admin.proxies.qualityBaseLatency') placeholderplaceholder:
              {{ typeof qualityReport.base_latency_ms === 'number' ? `${qualityReport.base_latency_msplaceholderms` : '-' placeholderplaceholder
            </div>
            <div>{{ t('admin.proxies.qualityCheckedAt') placeholderplaceholder: {{ new Date(qualityReport.checked_at * 1000).toLocaleString() placeholderplaceholder</div>
          </div>
        </div>
        ...
        <div class="max-h-80 overflow-auto rounded-lg border border-gray-200 dark:border-dark-600">
          <table class="min-w-full divide-y divide-gray-200 text-sm dark:divide-dark-700">
            <thead class="bg-gray-50 text-xs uppercase text-gray-500 dark:bg-dark-800 dark:text-dark-400">
              <tr>
                <th class="whitespace-nowrap px-3 py-2 text-left">{{ t('admin.proxies.qualityTableTarget') placeholderplaceholder</th>
                <th class="whitespace-nowrap px-3 py-2 text-left">{{ t('admin.proxies.qualityTableStatus') placeholderplaceholder</th>
                <th class="whitespace-nowrap px-3 py-2 text-left">HTTP</th>
                <th class="whitespace-nowrap px-3 py-2 text-left">{{ t('admin.proxies.qualityTableLatency') placeholderplaceholder</th>
                <th class="px-3 py-2 text-left">{{ t('admin.proxies.qualityTableMessage') placeholderplaceholder</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-gray-200 bg-white dark:divide-dark-700 dark:bg-dark-900">
              <tr v-for="item in qualityReport.items" :key="item.target">
                <td class="whitespace-nowrap px-3 py-2 text-gray-900 dark:text-white">{{ qualityTargetLabel(item.target) placeholderplaceholder</td>
                <td class="whitespace-nowrap px-3 py-2">
                  <span class="badge whitespace-nowrap" :class="qualityStatusClass(item.status)">{{ qualityStatusLabel(item.status) placeholderplaceholder</span>
                </td>
                <td class="whitespace-nowrap px-3 py-2 text-gray-600 dark:text-gray-300">{{ item.http_status ?? '-' placeholderplaceholder</td>
                <td class="whitespace-nowrap px-3 py-2 text-gray-600 dark:text-gray-300">
                  {{ typeof item.latency_ms === 'number' ? `${item.latency_msplaceholderms` : '-' placeholderplaceholder
                </td>
                <td class="px-3 py-2 text-gray-600 dark:text-gray-300">
                  <span>{{ item.message || '-' placeholderplaceholder</span>
                  <span v-if="item.cf_ray" class="ml-1 text-xs text-gray-400">(cf-ray: {{ item.cf_ray placeholderplaceholder)</span>
                </td>
              </tr>
            </tbody>
          </table>
...
import type { Proxy, ProxyAccountSummary, ProxyProtocol, ProxyQualityCheckResult placeholder from '@/types'
...
const columns = computed<Column[]>(() => [
  ...
  { key: 'auth', label: t('admin.proxies.columns.auth'), sortable: false placeholder,
  ...
  { key: 'expiry', label: t('admin.proxies.columns.expiry'), sortable: true placeholder,
  { key: 'created_at', label: t('admin.proxies.columns.createdAt'), sortable: true placeholder,
...
const statusOptions = computed(() => [
  ...
  { value: 'expired', label: t('admin.proxies.expired') placeholder
])
...
const {
  ...
  toggleVisible,
  batchUpdate
placeholder = useTableSelection<Proxy>({
  rows: proxies,
...
const backupProxyOptions = (excludeId?: number) =>
  allProxiesForBackup.value
    ...
    .map(p => ({ label: `${p.nameplaceholder (${p.hostplaceholder:${p.portplaceholder)`, value: p.id placeholder))
...
const loadProxies = async () => {
  ...
  try {
    const response = await adminAPI.proxies.list(
      ...
      { signal: currentAbortController.signal placeholder
    )
...
const handleQualityCheck = async (proxy: Proxy) => {
  ...
  try {
    ...
    appStore.showSuccess(
      t('admin.proxies.qualityCheckDone', { score: result.score, grade: result.grade placeholder)
    )
  placeholder catch (error: any) {
...
const runBatchProxyQualityChecks = async (ids: number[]) => {
  if (ids.length === 0) return { total: 0, healthy: 0, warn: 0, challenge: 0, failed: 0 placeholder
    ...
    while (index < ids.length) {
      ...
      try {
        ...
        if (result.challenge_count > 0) {
          challenge++
        placeholder else if (result.failed_count > 0) {
          failed++
        placeholder else if (result.warn_count > 0) {
          warn++
        placeholder else {
          healthy++
        placeholder
...
const toLocalDateStr = (dt: Date): string => {
  ...
  return `${yplaceholder-${mplaceholder-${dplaceholder`
placeholder
// base 为空 → 今天本地 00:00;否则该日期本地 00:00
const baseDateOrToday = (baseDateStr: string): Date => {
  const base = baseDateStr ? new Date(`${baseDateStrplaceholderT00:00:00`) : new Date()
...
const daysFromBase = (baseDateStr: string, targetDateStr: string): number | null => {
  ...
  const target = new Date(`${targetDateStrplaceholderT00:00:00`)
...
const expiryLabel = (row: Proxy): string => {
  const { key, params placeholder = proxyExpiryLabelKey(row.expires_at, row.status)
  ...
  try {
    const dataPayload = await adminAPI.proxies.exportData(
      selectedCount.value > 0
        ? { ids: Array.from(selectedProxyIds.value) placeholder
        : {
...
function buildAuthPart(row: any): string {
  ...
  if (user && pass) return `${userplaceholder:${passplaceholder@`
  if (user) return `${userplaceholder@`
  if (pass) return `:${passplaceholder@`
  return ''
placeholder

function buildProxyUrl(row: any): string {
  return `${row.protocolplaceholder://${buildAuthPart(row)placeholder${row.hostplaceholder:${row.portplaceholder`
placeholder
...
function getCopyFormats(row: any) {
  ...
  const formats = [
    { label: fullUrl, value: fullUrl placeholder,
  ]
  if (hasAuth) {
    const withoutProtocol = fullUrl.replace(/^[^:]+:\/\//, '')
    formats.push({ label: withoutProtocol, value: withoutProtocol placeholder)
  placeholder
  formats.push({ label: `${row.hostplaceholder:${row.portplaceholder`, value: `${row.hostplaceholder:${row.portplaceholder` placeholder)
  return formats
placeholder
