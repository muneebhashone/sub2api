    ...
    <TablePageLayout>
        ...
        <div class="flex flex-wrap items-center gap-3">
          ...
          <div class="flex flex-wrap items-center justify-end gap-2">
            ...
            <div class="flex items-center gap-2 md:contents">
              ...
              <div class="relative" ref="filterDropdownRef">
                ...
                >
                  ...
                  <span class="hidden md:inline">{{ t('admin.users.filterSettings') placeholderplaceholder</span>
                </button>
                <!-- Dropdown menu -->
                <div
              ...
              >
                ...
                <span class="hidden md:inline">{{ t('admin.users.attributes.configButton') placeholderplaceholder</span>
              </button>
            </div>
            ...
            >
              ...
              {{ t('admin.users.bulkLimits.action', { count: selectedCount placeholder) placeholderplaceholder
            </button>
      ...
      <template #table>
        ...
        >
          ...
          <template #cell-groups="{ row placeholder">
            <div v-if="allGroups.length > 0" class="flex flex-col gap-1">
              ...
              >
                ...
                <span class="font-medium text-purple-600 dark:text-purple-400">{{ getUserGroups(row).exclusive.length placeholderplaceholder</span>
                <span class="text-gray-500 dark:text-dark-400">{{ t('admin.users.exclusiveLabel') placeholderplaceholder</span>
                ...
                >
                  ...
                  <div class="flex flex-col gap-0.5 whitespace-nowrap">
                    <span v-for="g in getUserGroups(row).exclusive" :key="g.id">{{ g.name placeholderplaceholder</span>
                  </div>
                </div>
                <!-- 点击展开分组操作菜单 -->
                <div
                ...
                >
                  <div class="border-b border-gray-100 px-3 py-1.5 text-[10px] font-medium uppercase tracking-wider text-gray-400 dark:border-dark-600 dark:text-dark-400">
                    {{ t('admin.users.clickToReplace') placeholderplaceholder
                  </div>
                  <div
                  ...
                  >
                    ...
                    <span class="flex-1">{{ g.name placeholderplaceholder</span>
                  </div>
                </div>
              </span>
              <!-- 公开分组行 -->
              ...
              >
                ...
                <span class="font-medium text-gray-600 dark:text-dark-300">{{ getUserGroups(row).publicGroups.length placeholderplaceholder</span>
                <span class="text-gray-400 dark:text-dark-500">{{ t('admin.users.publicLabel') placeholderplaceholder</span>
                <!-- Tooltip: 向下弹出 -->
                <div class="pointer-events-none absolute left-0 top-full z-50 mt-1.5 rounded bg-gray-900 px-2.5 py-1.5 text-xs text-white opacity-0 shadow-lg transition-opacity duration-75 group-hover/pub:opacity-100 dark:bg-dark-600">
                  <div class="absolute left-4 bottom-full border-4 border-transparent border-b-gray-900 dark:border-b-dark-600"></div>
                  <div class="flex flex-col gap-0.5 whitespace-nowrap">
                    <span v-for="g in getUserGroups(row).publicGroups" :key="g.id">{{ g.name placeholderplaceholder</span>
                  </div>
                </div>
              </span>
              <!-- 都没有 -->
          ...
          <template #cell-balance="{ value, row placeholder">
            <div class="flex items-center gap-2">
              <div class="group relative">
                ...
                <div class="pointer-events-none absolute bottom-full left-1/2 z-50 mb-1.5 -translate-x-1/2 whitespace-nowrap rounded bg-gray-900 px-2 py-1 text-xs text-white opacity-0 shadow-lg transition-opacity duration-75 group-hover:opacity-100 dark:bg-dark-600">
                  {{ t('admin.users.balanceHistoryTip') placeholderplaceholder
          ...
          <template #cell-balance_platform_quota="{ row placeholder">
            <button
              type="button"
          ...
          <template
            ...
            :key="usageKey"
            #[`header-${usageKeyplaceholder`]="{ column placeholder"
          >
            <div class="flex items-center gap-1.5">
              ...
              <div class="usage-sort-trigger relative">
                <button
                  ...
                  :data-test="`usage-sort-trigger-${usageKeyplaceholder`"
                ...
                >
                  ...
                  >{{ usageSort.metric === 'today' ? t('admin.users.today') : t('admin.users.total') placeholderplaceholder</span>
                  <svg
                    v-if="usageSort && usageSort.key === usageKey"
                    class="h-3.5 w-3.5"
                    :class="{ 'rotate-180': usageSort.order === 'desc' placeholder"
                    fill="currentColor"
                    viewBox="0 0 20 20"
                  >
                    <path
                ...
                >
                  <button
                    ...
                    :data-test="`usage-sort-${usageKeyplaceholder-${metricplaceholder`"
                    @click.stop="toggleUsageSort(usageKey, metric)"
                  >
                    <span>{{ metric === 'today' ? t('admin.users.today') : t('admin.users.total') placeholderplaceholder</span>
                    <svg
                      v-if="getUsageSortOrder(usageKey, metric)"
                      class="h-3 w-3"
                      :class="{ 'rotate-180': getUsageSortOrder(usageKey, metric) === 'desc' placeholder"
                      fill="currentColor"
                      viewBox="0 0 20 20"
                    >
                      <path
                  ...
                  <div class="mt-1 border-t border-gray-100 px-3 py-1 text-[10px] normal-case tracking-normal text-gray-400 dark:border-dark-700 dark:text-dark-500">
                    {{ t('admin.users.sortCurrentPageOnly') placeholderplaceholder
                  </div>
                </div>
              </div>
            </div>
          ...
          <template #cell-usage_anthropic="{ row placeholder">
            <PlatformCostCell :usage="getPlatformUsage(row.id, 'anthropic')" />
          </template>

          <template #cell-usage_openai="{ row placeholder">
            <PlatformCostCell :usage="getPlatformUsage(row.id, 'openai')" />
          </template>

          <template #cell-usage_gemini="{ row placeholder">
            <PlatformCostCell :usage="getPlatformUsage(row.id, 'gemini')" />
          </template>

          <template #cell-usage_antigravity="{ row placeholder">
          ...
          <template v-for="user in users" :key="user.id">
            <template v-if="user.id === activeMenuId">
              ...
              >
                ...
                {{ t('admin.users.platformQuota.menuItem') placeholderplaceholder
              </button>
              ...
              >
                ...
                {{ t('admin.users.balanceHistory') placeholderplaceholder
              </button>
...
import type { AdminUser, AdminGroup, UserAttributeDefinition placeholder from '@/types'
...
const allColumns = computed<Column[]>(() => [
  ...
  { key: 'groups', label: t('admin.users.columns.groups'), sortable: false placeholder,
  ...
  { key: 'balance_platform_quota', label: t('admin.users.columns.balancePlatformQuota'), sortable: false placeholder,
  { key: 'usage', label: t('admin.users.columns.usage'), sortable: false placeholder,
  { key: 'usage_anthropic', label: t('admin.users.columns.usageAnthropic'), sortable: false placeholder,
  { key: 'usage_openai', label: t('admin.users.columns.usageOpenAI'), sortable: false placeholder,
  { key: 'usage_gemini', label: t('admin.users.columns.usageGemini'), sortable: false placeholder,
  { key: 'usage_antigravity', label: t('admin.users.columns.usageAntigravity'), sortable: false placeholder,
...
const hasVisibleAttributeColumns = computed(() =>
  attributeDefinitions.value.some((def) => def.enabled && !hiddenColumns.has(`attr_${def.idplaceholder`))
)
...
const getUserGroups = (user: AdminUser) => {
  ...
  for (const g of allGroups.value) {
    ...
    placeholder
  placeholder
  return { exclusive, publicGroups placeholder
placeholder

// Group filter options: "All Groups" + active exclusive groups (value = group name for fuzzy match)
const groupFilterOptions = computed(() => {
  const options: { value: string; label: string placeholder[] = [
    { value: '', label: t('admin.users.allAuthorizedGroups') placeholder
  ]
  for (const g of allGroups.value) {
    if (g.status !== 'active' || !g.is_exclusive || g.subscription_type !== 'standard') continue
    options.push({ value: g.name, label: g.name placeholder)
  placeholder
  return options
placeholder)
...
const builtInFilters = computed(() => [
  ...
  { key: 'group', name: t('admin.users.authorizedGroupFilter'), type: 'select' as const placeholder,
  { key: 'apiKeyGroup', name: t('admin.users.apiKeyGroupFilter'), type: 'select' as const placeholder
])
...
const platformQuotaStats = ref<Record<number, PlatformQuotaItem[]>>({placeholder)
...
type UsageSortState = { key: string; metric: UsageMetric; order: 'asc' | 'desc' placeholder | null
...
const loadInitialUsageSort = (): UsageSortState => {
  try {
    ...
    if (!raw) return null
    const parsed = JSON.parse(raw) as Partial<{ key: string; metric: string; order: string placeholder>
    ...
    return { key: parsed.key, metric, order placeholder
  placeholder catch {
    return null
  placeholder
placeholder
...
const toggleUsageSort = (key: string, metric: UsageMetric) => {
  ...
  if (cur && cur.key === key && cur.metric === metric) {
    usageSort.value = cur.order === 'desc' ? { key, metric, order: 'asc' placeholder : null
  placeholder else {
    usageSort.value = { key, metric, order: 'desc' placeholder
  placeholder
  persistUsageSort()
  openUsageSortMenu.value = null
placeholder
...
const sortedUsers = computed(() => {
  ...
  return [...users.value]
    .map((row, index) => ({ row, index placeholder))
    .sort((a, b) => {
...
placeholder = useTableSelection<AdminUser>({
  rows: sortedUsers,
  getId: (user) => user.id
placeholder)
...
const getUserSelectionLabel = (user: AdminUser) =>
  t('admin.users.bulkLimits.selectUser', { email: user.email placeholder)
      ...
      (async () => {
        try {
          ...
          for (let i = 0; i < userIds.length; i += CHUNK) {
            ...
            if (signal?.aborted) return
            if (typeof expectedSeq === 'number' && expectedSeq !== secondaryDataSeq) return
            const merged = { ...platformQuotaStats.value placeholder
...
const openActionMenu = (user: AdminUser, e: MouseEvent) => {
  ...
  placeholder else {
    ...
    menuPosition.value = { top, left placeholder
...
const groupReplaceOldGroup = ref<{ id: number; name: string placeholder | null>(null)
...
const loadUsers = async () => {
  ...
  try {
    ...
    usageStats.value = {placeholder
    userAttributeValues.value = {placeholder
    platformQuotaStats.value = {placeholder
...
const openGroupReplace = (user: AdminUser, group: { id: number; name: string placeholder) => {
  expandedGroupUserId.value = null
