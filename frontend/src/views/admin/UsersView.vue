  ...
  <AppLayout>
      ...
      <div class="card overflow-hidden">
        <DataTable :columns="columns" :data="users" :loading="loading">
          ...
          <template #cell-created_at="{ value placeholder">
            <span class="text-sm text-gray-500 dark:text-dark-400">{{ formatDate(value) placeholderplaceholder</span>
          </template>
      ...
      <form @submit.prevent="handleCreateUser" class="space-y-5">
        ...
        <div class="flex justify-end gap-3 pt-4">
          ...
          >
            ...
            {{ submitting ? t('admin.users.creating') : t('common.create') placeholderplaceholder
          </button>
        </div>
      </form>
    </Modal>
    ...
    >
      <form v-if="editingUser" @submit.prevent="handleUpdateUser" class="space-y-5">
        ...
        <div>
          <label class="input-label">{{ t('admin.users.password') placeholderplaceholder</label>
          <p class="text-xs text-gray-500 dark:text-dark-400 mb-1">{{ t('admin.users.leaveEmptyToKeep') placeholderplaceholder</p>
          <div class="flex gap-2">
            <div class="relative flex-1">
              <input
        ...
        <div class="flex justify-end gap-3 pt-4">
          ...
          >
            ...
            {{ submitting ? t('admin.users.updating') : t('common.update') placeholderplaceholder
          </button>
        </div>
      </form>
    </Modal>
    ...
    >
      <div v-if="viewingUser" class="space-y-4">
        ...
        <div class="flex items-center gap-3 p-4 rounded-xl bg-gray-50 dark:bg-dark-700">
          <div class="flex h-10 w-10 items-center justify-center rounded-full bg-primary-100 dark:bg-primary-900/30">
            <span class="text-lg font-medium text-primary-700 dark:text-primary-300">
              {{ viewingUser.email.charAt(0).toUpperCase() placeholderplaceholder
            </span>
          </div>
          <div>
            <p class="font-medium text-gray-900 dark:text-white">{{ viewingUser.email placeholderplaceholder</p>
            <p class="text-sm text-gray-500 dark:text-dark-400">{{ viewingUser.username placeholderplaceholder</p>
          </div>
        </div>
        ...
        <div v-else-if="userApiKeys.length === 0" class="text-center py-8">
          ...
          <p class="mt-2 text-sm text-gray-500 dark:text-dark-400">{{ t('admin.users.noApiKeys') placeholderplaceholder</p>
        </div>
          ...
          >
            <div class="flex items-start justify-between">
              <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2 mb-1">
                  <span class="font-medium text-gray-900 dark:text-white">{{ key.name placeholderplaceholder</span>
                  <span
                    :class="[
                      'badge text-xs',
                  ...
                  >
                    {{ key.status placeholderplaceholder
                  </span>
                </div>
                <p class="text-sm font-mono text-gray-500 dark:text-dark-400 truncate">
                  {{ key.key.substring(0, 20) placeholderplaceholder...{{ key.key.substring(key.key.length - 8) placeholderplaceholder
                </p>
              </div>
            </div>
            <div class="mt-3 flex flex-wrap gap-4 text-xs text-gray-500 dark:text-dark-400">
              <div class="flex items-center gap-1">
                ...
                <span>{{ t('admin.users.group') placeholderplaceholder: {{ key.group?.name || t('admin.users.none') placeholderplaceholder</span>
              </div>
              <div class="flex items-center gap-1">
                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25m-18 0A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75m-18 0v-7.5A2.25 2.25 0 015.25 9h13.5A2.25 2.25 0 0121 11.25v7.5" />
                </svg>
                <span>{{ t('admin.users.columns.created') placeholderplaceholder: {{ formatDate(key.created_at) placeholderplaceholder</span>
              </div>
            </div>
          </div>
        </div>
    ...
    >
      <div v-if="allowedGroupsUser" class="space-y-4">
        ...
        <div class="flex items-center gap-3 p-4 rounded-xl bg-gray-50 dark:bg-dark-700">
          <div class="flex h-10 w-10 items-center justify-center rounded-full bg-primary-100 dark:bg-primary-900/30">
            <span class="text-lg font-medium text-primary-700 dark:text-primary-300">
              {{ allowedGroupsUser.email.charAt(0).toUpperCase() placeholderplaceholder
            </span>
          </div>
          <div>
            <p class="font-medium text-gray-900 dark:text-white">{{ allowedGroupsUser.email placeholderplaceholder</p>
          </div>
        </div>
        ...
        <div v-else>
          <p class="text-sm text-gray-600 dark:text-dark-400 mb-3">
            {{ t('admin.users.allowedGroupsHint') placeholderplaceholder
          </p>
          ...
          <div v-if="standardGroups.length === 0" class="text-center py-6">
            ...
            <p class="mt-2 text-sm text-gray-500 dark:text-dark-400">{{ t('admin.users.noStandardGroups') placeholderplaceholder</p>
          </div>
          ...
          <div v-else class="space-y-2 max-h-64 overflow-y-auto">
            <label
              ...
              :class="{ 'bg-primary-50 dark:bg-primary-900/20 border-primary-300 dark:border-primary-700': selectedGroupIds.includes(group.id) placeholder"
            >
              <input
                type="checkbox"
                :value="group.id"
              ...
              />
              <div class="flex-1 min-w-0">
                <p class="font-medium text-gray-900 dark:text-white">{{ group.name placeholderplaceholder</p>
                <p v-if="group.description" class="text-sm text-gray-500 dark:text-dark-400 truncate">{{ group.description placeholderplaceholder</p>
              </div>
              <div class="flex items-center gap-2">
                <span class="badge badge-gray text-xs">{{ group.platform placeholderplaceholder</span>
                <span v-if="group.is_exclusive" class="badge badge-purple text-xs">{{ t('admin.groups.exclusive') placeholderplaceholder</span>
              </div>
            </label>
          </div>
          ...
          <div class="mt-4 pt-4 border-t border-gray-200 dark:border-dark-600">
            <label class="flex items-center gap-3 p-3 rounded-lg border border-gray-200 dark:border-dark-600 hover:bg-gray-50 dark:hover:bg-dark-700 cursor-pointer transition-colors"
              :class="{ 'bg-green-50 dark:bg-green-900/20 border-green-300 dark:border-green-700': selectedGroupIds.length === 0 placeholder"
            >
              <input
                type="radio"
              ...
              />
              <div class="flex-1">
                <p class="font-medium text-gray-900 dark:text-white">{{ t('admin.users.allowAllGroups') placeholderplaceholder</p>
                <p class="text-sm text-gray-500 dark:text-dark-400">{{ t('admin.users.allowAllGroupsHint') placeholderplaceholder</p>
              </div>
            </label>
          </div>
        </div>
      ...
      <template #footer>
        <div class="flex justify-end gap-3">
          ...
          >
            ...
            {{ savingAllowedGroups ? t('common.saving') : t('common.save') placeholderplaceholder
          </button>
        </div>
      </template>
    </Modal>
    ...
    >
      <form v-if="balanceUser" @submit.prevent="handleBalanceSubmit" class="space-y-5">
        <div class="flex items-center gap-3 p-4 rounded-xl bg-gray-50 dark:bg-dark-700">
          <div class="flex h-10 w-10 items-center justify-center rounded-full bg-primary-100 dark:bg-primary-900/30">
            <span class="text-lg font-medium text-primary-700 dark:text-primary-300">
              {{ balanceUser.email.charAt(0).toUpperCase() placeholderplaceholder
            </span>
          </div>
          <div class="flex-1">
            <p class="font-medium text-gray-900 dark:text-white">{{ balanceUser.email placeholderplaceholder</p>
            <p class="text-sm text-gray-500 dark:text-dark-400">
              {{ t('admin.users.currentBalance') placeholderplaceholder: ${{ balanceUser.balance.toFixed(2) placeholderplaceholder
            </p>
          </div>
        </div>
        ...
        <div>
          <label class="input-label">
            {{ balanceOperation === 'add' ? t('admin.users.depositAmount') : t('admin.users.withdrawAmount') placeholderplaceholder
          </label>
          <div class="relative">
          ...
          <p class="input-hint">
            {{ t('admin.users.amountHint') placeholderplaceholder
          </p>
        </div>
        ...
        <div>
          ...
          ></textarea>
          <p class="input-hint">{{ t('admin.users.notesOptional') placeholderplaceholder</p>
        </div>
        ...
        <div v-if="balanceForm.amount > 0" class="p-4 rounded-xl bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800/50">
          <div class="flex items-center justify-between text-sm">
            <span class="text-blue-700 dark:text-blue-300">{{ t('admin.users.newBalance') placeholderplaceholder:</span>
            <span class="font-bold text-blue-900 dark:text-blue-100">
              ${{ calculateNewBalance().toFixed(2) placeholderplaceholder
            </span>
          </div>
        </div>
        ...
        <div v-if="balanceOperation === 'subtract' && calculateNewBalance() < 0" class="p-4 rounded-xl bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800/50">
          <div class="flex items-center gap-2 text-sm text-red-700 dark:text-red-300">
            ...
            <span>{{ t('admin.users.insufficientBalance') placeholderplaceholder</span>
          </div>
        </div>
    ...
    <ConfirmDialog
      ...
      :message="t('admin.users.deleteConfirm', { email: deletingUser?.email placeholder)"
...
const handleSaveAllowedGroups = async () => {
  ...
  try {
    ...
    await adminAPI.users.update(allowedGroupsUser.value.id, { allowed_groups: allowedGroups placeholder)
