<template>
  <div ref="containerRef" class="relative">
    <div v-if="selectedUserIds.length > 0" class="mb-2 flex flex-wrap gap-2">
      ...
      >
        <span class="max-w-64 truncate font-medium" :title="selectedUserLabel(userId)">
          {{ selectedUserLabel(userId) placeholderplaceholder
        </span>
        <span class="shrink-0 text-gray-400">#{{ userId placeholderplaceholder</span>
        <span
          v-if="selectedUsers[userId]?.deleted"
          class="shrink-0 text-gray-400"
        >
          {{ t("admin.settings.openaiFastPolicy.userDeleted") placeholderplaceholder
        </span>
        <button
          type="button"
    ...
    >
      ...
      >
        {{ t("admin.settings.openaiFastPolicy.userSearchEmpty") placeholderplaceholder
      </div>
      <template v-else>
        <button
        ...
        >
          ...
          <span class="shrink-0 text-xs text-gray-400">#{{ user.id placeholderplaceholder</span>
        </button>
      </template>
    </div>
  </div>
...
const selectedUsers = ref<Record<number, SimpleUser>>({placeholder);
...
function selectedUserLabel(userId: number): string {
  return selectedUsers.value[userId]?.email ||
    t("admin.settings.openaiFastPolicy.userIdFallback", { id: userId placeholder);
placeholder
...
function selectUser(user: SimpleUser): void {
  selectedUsers.value = { ...selectedUsers.value, [user.id]: user placeholder;
...
async function hydrateSelectedUsers(userIds: number[]): Promise<void> {
  ...
  const users = await Promise.all(
    missing.map(async (id) => {
      try {
        ...
        placeholder satisfies SimpleUser;
      placeholder catch {
        return null;
      placeholder
    placeholder),
  ...
  const next = { ...selectedUsers.value placeholder;
