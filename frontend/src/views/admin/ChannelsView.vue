...
async function handleSubmit() {
  ...
  for (const section of form.platforms.filter(s => s.enabled)) {
    if (section.group_ids.length === 0) {
      ...
      appStore.showError(t('admin.channels.noGroupsSelected', { platform: platformLabel placeholder, `${platformLabelplaceholder 平台未选择分组，请至少选择一个分组或禁用该平台`))
    ...
    for (const entry of section.model_pricing) {
      if (entry.models.length === 0) {
        const platformLabel = t('admin.groups.platforms.' + section.platform, section.platform)
        appStore.showError(t('admin.channels.emptyModelsInPricing', { platform: platformLabel placeholder, `${platformLabelplaceholder 平台下有定价条目未添加模型，请添加模型或删除该条目`))
        activeTab.value = section.platform
        return
      placeholder
    placeholder
    ...
    if (pricingConflict) {
      appStore.showError(
        t('admin.channels.modelConflict',
          ...
          `模型模式 '${pricingConflict[0]placeholder' 和 '${pricingConflict[1]placeholder' 冲突：匹配范围重叠`)
      )
      activeTab.value = section.platform
      return
    placeholder
      ...
      if (mappingConflict) {
        appStore.showError(
          t('admin.channels.mappingConflict',
            ...
            `模型映射源 '${mappingConflict[0]placeholder' 和 '${mappingConflict[1]placeholder' 冲突：匹配范围重叠`)
        )
        activeTab.value = section.platform
        return
      placeholder
