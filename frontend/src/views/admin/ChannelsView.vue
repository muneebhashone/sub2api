    ...
    >
      <div class="channel-dialog-body">
        ...
        <div class="flex items-center border-b border-gray-200 dark:border-dark-700 flex-shrink-0 -mx-4 sm:-mx-6 px-4 sm:px-6 -mt-3 sm:-mt-4">
          ...
          >
            {{ t('admin.channels.form.basicSettings') placeholderplaceholder
          </button>
          ...
          >
            ...
            <span :class="platformTextClass(section.platform)">{{ t('admin.groups.platforms.' + section.platform, section.platform) placeholderplaceholder</span>
          </button>
        </div>
        ...
        <form id="channel-form" @submit.prevent="handleSubmit" class="flex-1 overflow-y-auto pt-4">
          ...
          <div v-show="activeTab === 'basic'" class="space-y-5">
            ...
            <!-- Platform Management -->
            <div class="space-y-3">
              <label class="input-label mb-0">{{ t('admin.channels.form.platformConfig') placeholderplaceholder</label>
              <div class="flex flex-wrap gap-2">
                ...
                >
                  ...
                  <span :class="platformTextClass(p)">{{ t('admin.groups.platforms.' + p, p) placeholderplaceholder</span>
                </label>
              </div>
            </div>
              ...
              <div class="flex items-center justify-between">
                <div>
                  <label class="text-sm font-medium text-gray-700 dark:text-gray-300">
                    {{ t('admin.channels.form.applyPricingToAccountStats') placeholderplaceholder
                  </label>
                  <p class="mt-0.5 text-xs text-gray-500 dark:text-gray-400">
                    {{ t('admin.channels.form.applyPricingToAccountStatsDesc') placeholderplaceholder
                  </p>
                </div>
                <Toggle
          ...
          >
            ...
            <div>
              <label class="input-label text-xs">
                {{ t('admin.channels.form.groups', 'Associated Groups') placeholderplaceholder <span class="text-red-500">*</span>
                <span v-if="section.group_ids.length > 0" class="ml-1 font-normal text-gray-400">
                  ({{ t('admin.channels.form.selectedCount', { count: section.group_ids.length placeholder) placeholderplaceholder)
                </span>
              </label>
              <div class="max-h-40 overflow-auto rounded-lg border border-gray-200 bg-gray-50 p-2 dark:border-dark-600 dark:bg-dark-900">
                ...
                <div v-else class="flex flex-wrap gap-1">
                  ...
                  >
                    ...
                    />
                    <span :class="['font-medium', platformTextClass(group.platform)]">{{ group.name placeholderplaceholder</span>
                    <span
            ...
            <div v-if="section.platform === 'openai'" class="border-t border-gray-200 pt-3 dark:border-dark-600">
              <div class="flex items-center justify-between gap-4">
                <div>
                  <label class="text-xs font-medium text-gray-700 dark:text-gray-300">
                    {{ t('admin.channels.form.codexImageGenerationBridge') placeholderplaceholder
                  </label>
                  <p class="mt-0.5 text-[11px] text-amber-600 dark:text-amber-400">
                    {{ t('admin.channels.form.codexImageGenerationBridgeHint') placeholderplaceholder
                  </p>
                </div>
            ...
            <div v-if="section.platform === 'anthropic'" class="border-t border-gray-200 pt-3 dark:border-dark-600">
              <div class="flex items-center justify-between gap-4">
                <div>
                  <label class="text-xs font-medium text-gray-700 dark:text-gray-300">
                    {{ t('admin.channels.form.bedrockCCCompat') placeholderplaceholder
                  </label>
                  <p class="mt-0.5 text-[11px] text-amber-600 dark:text-amber-400">
                    {{ t('admin.channels.form.bedrockCCCompatHint') placeholderplaceholder
                  </p>
                </div>
            ...
            <div>
              <div class="mb-1 flex items-center justify-between">
                ...
                <div class="flex items-center gap-2">
                  ...
                  >
                    {{ syncingPlatform === section.platform ? t('admin.channels.form.syncingModels') : t('admin.channels.form.syncLatestModels') placeholderplaceholder
                  </button>
            ...
            <div class="mt-4 border-t border-gray-200 pt-4 dark:border-dark-700 space-y-3">
              <div class="flex items-center justify-between">
                <h4 class="text-sm font-medium text-gray-700 dark:text-gray-300">
                  {{ t('admin.channels.form.accountStatsPricingRules') placeholderplaceholder
                </h4>
                <button
                  type="button"
                ...
                >
                  + {{ t('admin.channels.form.addRule') placeholderplaceholder
                </button>
              </div>
              ...
              >
                {{ t('admin.channels.form.noRulesConfigured') placeholderplaceholder
              </p>
              ...
              >
                ...
                <div>
                  <label class="text-xs text-gray-500 dark:text-gray-400">{{ t('admin.channels.form.ruleGroups') placeholderplaceholder</label>
                  <div class="mt-1 flex flex-wrap gap-1">
                    ...
                    >
                      ...
                      <span :class="['font-medium', platformTextClass(section.platform)]">{{ getGroupNameById(gid) placeholderplaceholder</span>
                    </label>
                  </div>
                  <p v-if="section.group_ids.length === 0" class="mt-1 text-xs text-gray-400">
                    {{ t('admin.channels.form.noGroupsInChannel') placeholderplaceholder
                  </p>
                </div>

                <div>
                  <label class="text-xs text-gray-500 dark:text-gray-400">{{ t('admin.channels.form.ruleAccounts') placeholderplaceholder</label>
                  ...
                  <div class="mt-1 flex flex-wrap gap-1">
                    ...
                    >
                      <span :class="['font-medium', platformTextClass(section.platform)]">{{ getRuleAccountLabel(accountId) placeholderplaceholder</span>
                  ...
                  <div class="relative mt-1 rule-account-search-container">
                    <input
                      v-model="ruleAccountSearchKeyword[`${section.platformplaceholder-${ruleIndexplaceholder`]"
                      type="text"
                      class="input text-sm"
                    ...
                    <div
                      v-if="showRuleAccountDropdown[`${section.platformplaceholder-${ruleIndexplaceholder`] && (ruleAccountSearchResults[`${section.platformplaceholder-${ruleIndexplaceholder`]?.length ?? 0) > 0"
                      class="absolute z-50 mt-1 max-h-48 w-full overflow-auto rounded-lg border bg-white shadow-lg dark:border-dark-600 dark:bg-dark-800"
                    >
                      <button
                        v-for="account in ruleAccountSearchResults[`${section.platformplaceholder-${ruleIndexplaceholder`]"
                        :key="account.id"
                        type="button"
                  ...
                  <p class="mt-1 text-xs text-gray-400">
                    {{ t('admin.channels.form.ruleAccountsHint') placeholderplaceholder
                  </p>
                </div>

                <div>
                  <div class="mb-1 flex items-center justify-between">
                    <label class="text-xs text-gray-500 dark:text-gray-400">{{ t('admin.channels.form.ruleModelPricing') placeholderplaceholder</label>
                  ...
                  <div v-if="rule.pricing.length === 0" class="rounded border border-dashed border-gray-300 p-2 text-center text-xs text-gray-400 dark:border-dark-500">
                    {{ t('admin.channels.form.noPricingRules') placeholderplaceholder
                  </div>
                  <div v-else class="space-y-2">
                    <PricingEntryCard
...
import { apiIntervalsToForm, apiTimePricingToForm, createDefaultTimePricingForm, findModelConflict, formIntervalsToAPI, formTimePricingToAPI, isValidPositiveMultiplier, mTokToPerToken, perTokenToMTok, validateIntervals, validateTimePricing placeholder from '@/components/admin/channel/types'
import type { AdminGroup, GroupPlatform placeholder from '@/types'
import type { Column placeholder from '@/components/common/types'
import { platformTextClass, platformBadgeLightClass placeholder from '@/utils/platformColors'
...
const billingModelSourceOptions = computed(() => [
  { value: 'channel_mapped', label: t('admin.channels.form.billingModelSourceChannelMapped', 'Bill by channel-mapped model') placeholder,
  ...
  { value: 'response_model', label: t('admin.channels.form.billingModelSourceResponse', 'Bill by upstream response model') placeholder
])
...
async function syncLatestModels(sectionIdx: number) {
  ...
  try {
    ...
    form.platforms[sectionIdx].model_pricing.push({
      ...
      time_pricing: createDefaultTimePricingForm()
    placeholder)
    appStore.showSuccess(t('admin.channels.form.syncModelsSuccess', { count: newModels.length placeholder))
...
function getGroupNameById(groupId: number): string {
  ...
  return group ? group.name : `#${groupIdplaceholder`
placeholder
...
const showRuleAccountDropdown = ref<Record<string, boolean>>({placeholder)
// Cache: account ID → name, populated when search results are selected
const ruleAccountNameCache = ref<Record<number, string>>({placeholder)
...
function onRuleAccountSearchInput(platform: string, ruleIndex: number) {
  const key = `${platformplaceholder-${ruleIndexplaceholder`
...
function selectRuleAccount(
  rule: { account_ids: number[] placeholder,
...
function removeRuleAccount(rule: { account_ids: number[] placeholder, accountId: number) {
...
function getRuleAccountLabel(accountId: number): string {
  ...
  return name ? `${nameplaceholder #${accountIdplaceholder` : `#${accountIdplaceholder`
placeholder
...
function clearAllRuleAccountSearchState() {
  ruleAccountSearchKeyword.value = {placeholder
  ruleAccountSearchResults.value = {placeholder
  showRuleAccountDropdown.value = {placeholder
placeholder
...
function formToAPI(): { group_ids: number[], model_pricing: ChannelModelPricing[], model_mapping: Record<string, Record<string, string>>, features_config: Record<string, unknown> placeholder {
  ...
  const codexImageGenerationBridge: Record<string, boolean> = {placeholder
  for (const section of form.platforms) {
    if (!section.enabled) continue
  ...
  const bedrockCCCompat: Record<string, boolean> = {placeholder
  for (const section of form.platforms) {
    if (!section.enabled) continue
    if (section.platform === 'anthropic') {
  ...
  return { group_ids: uniqueGroupIds, model_pricing, model_mapping, features_config: featuresConfig placeholder
placeholder
...
function resetForm() {
  ...
  ruleAccountSearchRunner.clearAll()
  clearAllRuleAccountSearchState()
  ruleAccountNameCache.value = {placeholder
placeholder
...
async function handleSubmit() {
  ...
  for (const section of form.platforms.filter(s => s.enabled)) {
    if (section.group_ids.length === 0) {
      ...
      appStore.showError(t('admin.channels.noGroupsSelected', { platform: platformLabel placeholder))
    ...
    for (const entry of section.model_pricing) {
      if (entry.models.length === 0) {
        const platformLabel = t('admin.groups.platforms.' + section.platform, section.platform)
        appStore.showError(t('admin.channels.emptyModelsInPricing', { platform: platformLabel placeholder))
        activeTab.value = section.platform
        return
      placeholder
    placeholder
    ...
    if (pricingConflict) {
      appStore.showError(
        t('admin.channels.modelConflict',
          { model1: pricingConflict[0], model2: pricingConflict[1] placeholder)
      )
      activeTab.value = section.platform
      return
    placeholder
      ...
      if (mappingConflict) {
        appStore.showError(
          t('admin.channels.mappingConflict',
            { model1: mappingConflict[0], model2: mappingConflict[1] placeholder)
        )
        activeTab.value = section.platform
        return
      placeholder
  ...
  for (const section of form.platforms.filter(s => s.enabled)) {
    for (const entry of section.model_pricing) {
      if (!isValidPositiveMultiplier(entry.fast_multiplier) ||
        ...
        appStore.showError(`${platformLabelplaceholder - ${modelLabelplaceholder: ${t('admin.channels.form.multiplierPositive')placeholder`)
        activeTab.value = section.platform
        return
      placeholder
  ...
  for (const section of form.platforms.filter(s => s.enabled)) {
    for (const entry of section.model_pricing) {
      ...
      if (timePricingError) {
        const platformLabel = t('admin.groups.platforms.' + section.platform, section.platform)
        const modelLabel = entry.models.join(', ') || t('admin.channels.form.unnamed')
        appStore.showError(`${platformLabelplaceholder - ${modelLabelplaceholder: ${timePricingErrorplaceholder`)
        activeTab.value = section.platform
        return
      placeholder
    placeholder
...
async function toggleChannelStatus(channel: Channel) {
  ...
  try {
    await adminAPI.channels.update(channel.id, { status: newStatus placeholder)
