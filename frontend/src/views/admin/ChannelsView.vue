    ...
    >
      <div class="channel-dialog-body">
        ...
        <div class="flex items-center border-b border-gray-200 dark:border-dark-700 flex-shrink-0 -mx-4 sm:-mx-6 px-4 sm:px-6 -mt-3 sm:-mt-4">
          ...
          >
            {{ t('admin.channels.form.basicSettings', 'Basic Settings') placeholderplaceholder
          </button>
        ...
        <form id="channel-form" @submit.prevent="handleSubmit" class="flex-1 overflow-y-auto pt-4">
          ...
          <div v-show="activeTab === 'basic'" class="space-y-5">
            ...
            <!-- Platform Management -->
            <div class="space-y-3">
              <label class="input-label mb-0">{{ t('admin.channels.form.platformConfig', 'Platform Configuration') placeholderplaceholder</label>
            ...
            <div>
              <label class="input-label text-xs">
                ...
                <span v-if="section.group_ids.length > 0" class="ml-1 font-normal text-gray-400">
                  ({{ t('admin.channels.form.selectedCount', { count: section.group_ids.length placeholder, `${section.group_ids.lengthplaceholder selected`) placeholderplaceholder)
                </span>
              </label>
...
async function handleSubmit() {
  ...
  for (const section of form.platforms.filter(s => s.enabled)) {
    if (section.group_ids.length === 0) {
      ...
      appStore.showError(t('admin.channels.noGroupsSelected', { platform: platformLabel placeholder, `${platformLabelplaceholder has no selected group. Select at least one group or disable this platform.`))
    ...
    for (const entry of section.model_pricing) {
      if (entry.models.length === 0) {
        const platformLabel = t('admin.groups.platforms.' + section.platform, section.platform)
        appStore.showError(t('admin.channels.emptyModelsInPricing', { platform: platformLabel placeholder, `${platformLabelplaceholder has a pricing entry without models. Add models or remove the entry.`))
        activeTab.value = section.platform
        return
      placeholder
    placeholder
    ...
    if (pricingConflict) {
      appStore.showError(
        t('admin.channels.modelConflict',
          ...
          `Model patterns '${pricingConflict[0]placeholder' and '${pricingConflict[1]placeholder' conflict: overlapping match range.`)
      )
      activeTab.value = section.platform
      return
    placeholder
      ...
      if (mappingConflict) {
        appStore.showError(
          t('admin.channels.mappingConflict',
            ...
            `Mapping source patterns '${mappingConflict[0]placeholder' and '${mappingConflict[1]placeholder' conflict: overlapping match range.`)
        )
        activeTab.value = section.platform
        return
      placeholder
