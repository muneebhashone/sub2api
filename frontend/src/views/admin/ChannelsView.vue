    ...
    >
      <form id="channel-form" @submit.prevent="handleSubmit" class="space-y-5">
        ...
        <div class="space-y-3">
          <div class="flex items-center justify-between">
            ...
            <div class="relative" v-if="availablePlatformsToAdd.length > 0">
              <button type="button" @click="showPlatformMenu = !showPlatformMenu" class="btn btn-secondary btn-sm">
                ...
                {{ t('admin.channels.form.addPlatform', '添加平台') placeholderplaceholder
              </button>
              <div
          ...
          >
            {{ t('admin.channels.form.noPlatforms', '点击"添加平台"开始配置渠道') placeholderplaceholder
          </div>
            ...
            >
              <div class="flex items-center gap-2">
                ...
                <span class="text-xs text-gray-400">
                  {{ section.group_ids.length placeholderplaceholder {{ t('admin.channels.groupsUnit', 'groups') placeholderplaceholder
                </span>
                <span v-if="Object.keys(section.model_mapping).length > 0" class="text-xs text-gray-400">
                  · {{ Object.keys(section.model_mapping).length placeholderplaceholder {{ t('admin.channels.form.mappingCount', 'mappings') placeholderplaceholder
                </span>
                <span v-if="section.model_pricing.length > 0" class="text-xs text-gray-400">
                  · {{ section.model_pricing.length placeholderplaceholder {{ t('admin.channels.pricingUnit', 'pricing rules') placeholderplaceholder
                </span>
              </div>
              <button
                type="button"
