<template>
  <AppLayout>
            ...
            <div class="min-w-[280px] space-y-2">
              ...
              <div v-if="row.group?.daily_limit_usd" class="usage-row">
                ...
                <div class="reset-info" v-if="row.daily_window_start">
                  ...
                  <span>{{ formatDailyUsageWindow(row) placeholderplaceholder</span>
                </div>
              </div>
            ...
            <div v-if="value">
              ...
              >
                <div v-if="remainingExpiry" class="text-xs text-gray-500">
                  {{ remainingExpiry placeholderplaceholder
                </div>
              </template>
            </div>
          ...
          <template #cell-actions="{ row placeholder">
            <div class="flex items-center gap-1">
              ...
              >
                ...
                <span class="text-xs">{{ t('admin.subscriptions.resetQuota') placeholderplaceholder</span>
              </button>
              <button
                v-if="row.status === 'active'"
              ...
              >
                <Icon name="refresh" size="sm" />
                <span class="text-xs">{{ t('admin.subscriptions.restore') placeholderplaceholder</span>
              </button>
            </div>
          </template>
    ...
    <ConfirmDialog
      ...
      :message="t('admin.subscriptions.restoreConfirm', { user: restoringSubscription?.user?.email placeholder)"
    ...
    <ConfirmDialog
      ...
      :message="t('admin.subscriptions.resetQuotaConfirm', { user: resettingSubscription?.user?.email placeholder)"
      ...
      <transition name="modal">
        <div v-if="showGuideModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" @mousedown.self="showGuideModal = false">
          ...
          <div class="relative max-h-[85vh] w-full max-w-2xl overflow-y-auto rounded-xl bg-white p-6 shadow-2xl dark:bg-dark-800">
            ...
            <h2 class="mb-4 text-lg font-bold text-gray-900 dark:text-white">{{ t('admin.subscriptions.guide.title') placeholderplaceholder</h2>
            <p class="mb-5 text-sm text-gray-500 dark:text-gray-400">{{ t('admin.subscriptions.guide.subtitle') placeholderplaceholder</p>
            ...
            <div class="mb-5">
              <h3 class="mb-2 flex items-center gap-2 text-sm font-semibold text-gray-900 dark:text-white">
                ...
                {{ t('admin.subscriptions.guide.step1.title') placeholderplaceholder
              </h3>
              <ol class="ml-8 list-decimal space-y-1 text-sm text-gray-600 dark:text-gray-300">
                <li>{{ t('admin.subscriptions.guide.step1.line1') placeholderplaceholder</li>
                <li>{{ t('admin.subscriptions.guide.step1.line2') placeholderplaceholder</li>
                <li>{{ t('admin.subscriptions.guide.step1.line3') placeholderplaceholder</li>
              </ol>
              <div class="ml-8 mt-2">
                <router-link
                  to="/admin/groups"
                ...
                >
                  {{ t('admin.subscriptions.guide.step1.link') placeholderplaceholder
            ...
            <div class="mb-5">
              <h3 class="mb-2 flex items-center gap-2 text-sm font-semibold text-gray-900 dark:text-white">
                ...
                {{ t('admin.subscriptions.guide.step2.title') placeholderplaceholder
              </h3>
              <ol class="ml-8 list-decimal space-y-1 text-sm text-gray-600 dark:text-gray-300">
                <li>{{ t('admin.subscriptions.guide.step2.line1') placeholderplaceholder</li>
                <li>{{ t('admin.subscriptions.guide.step2.line2') placeholderplaceholder</li>
                <li>{{ t('admin.subscriptions.guide.step2.line3') placeholderplaceholder</li>
              </ol>
            </div>
            ...
            <div class="mb-5">
              <h3 class="mb-2 flex items-center gap-2 text-sm font-semibold text-gray-900 dark:text-white">
                ...
                {{ t('admin.subscriptions.guide.step3.title') placeholderplaceholder
              </h3>
                ...
                <table class="w-full text-sm">
                  <tbody>
                    <tr v-for="(row, i) in guideActionRows" :key="i" class="border-b border-gray-100 dark:border-dark-700 last:border-0">
                      <td class="whitespace-nowrap bg-gray-50 px-3 py-2 font-medium text-gray-700 dark:bg-dark-700 dark:text-gray-300">{{ row.action placeholderplaceholder</td>
                      <td class="px-3 py-2 text-gray-600 dark:text-gray-400">{{ row.desc placeholderplaceholder</td>
                    </tr>
                  </tbody>
                </table>
              </div>
            ...
            <div class="rounded-lg bg-blue-50 p-3 text-xs text-blue-700 dark:bg-blue-900/20 dark:text-blue-300">
              {{ t('admin.subscriptions.guide.tip') placeholderplaceholder
            </div>

            <div class="mt-4 text-right">
              <button type="button" class="btn btn-primary btn-sm" @click="showGuideModal = false">{{ t('common.close') placeholderplaceholder</button>
            </div>
          </div>
        </div>
      </transition>
...
import { GROUP_PLATFORM_OPTIONS placeholder from '@/constants/platforms'
...
const guideActionRows = computed(() => [
  { action: t('admin.subscriptions.guide.actions.adjust'), desc: t('admin.subscriptions.guide.actions.adjustDesc') placeholder,
  { action: t('admin.subscriptions.guide.actions.resetQuota'), desc: t('admin.subscriptions.guide.actions.resetQuotaDesc') placeholder,
  { action: t('admin.subscriptions.guide.actions.revoke'), desc: t('admin.subscriptions.guide.actions.revokeDesc') placeholder
])
...
const platformFilterOptions = computed(() => [
  { value: '', label: t('admin.subscriptions.allPlatforms') placeholder,
...
const confirmResetQuota = async () => {
  ...
  try {
    await adminAPI.subscriptions.resetQuota(resettingSubscription.value.id, { daily: true, weekly: true, monthly: true placeholder)
...
const formatRemainingExpiry = (expiresAt: string): string | null => {
  ...
  if (duration.unit === 'days') {
    return t('admin.subscriptions.daysRemaining', { days: duration.days placeholder)
  placeholder
  if (duration.hours) {
    ...
    placeholder)
  placeholder
  return t('admin.subscriptions.minutesRemaining', { minutes: duration.minutes placeholder)
placeholder
...
const formatResetDuration = (parts: RemainingDurationParts): string => {
  if (parts.days > 0) {
    return t('admin.subscriptions.resetInDaysHours', { days: parts.days, hours: parts.hours placeholder)
  placeholder

  if (parts.hours > 0) {
    return t('admin.subscriptions.resetInHoursMinutes', { hours: parts.hours, minutes: parts.minutes placeholder)
  placeholder

  return t('admin.subscriptions.resetInMinutes', { minutes: parts.minutes placeholder)
placeholder

const formatQuotaEndDuration = (parts: RemainingDurationParts): string => {
  if (parts.days > 0) {
    return t('admin.subscriptions.quotaEndsInDaysHours', { days: parts.days, hours: parts.hours placeholder)
  placeholder

  if (parts.hours > 0) {
    return t('admin.subscriptions.quotaEndsInHoursMinutes', { hours: parts.hours, minutes: parts.minutes placeholder)
  placeholder

  return t('admin.subscriptions.quotaEndsInMinutes', { minutes: parts.minutes placeholder)
placeholder
