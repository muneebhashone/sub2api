import { describe, expect, it, beforeEach, afterEach, vi placeholder from 'vitest'
import { flushPromises, mount placeholder from '@vue/test-utils'
import { nextTick placeholder from 'vue'

import KeyUsageView from '../KeyUsageView.vue'

const { showInfo, showSuccess, showError, fetchPublicSettings placeholder = vi.hoisted(() => ({
  showInfo: vi.fn(),
  showSuccess: vi.fn(),
  showError: vi.fn(),
...
describe('KeyUsageView daily detail', () => {
  beforeEach(() => {
    ...
    Object.defineProperty(window, 'matchMedia', {
      configurable: true,
      value: vi.fn().mockReturnValue({ matches: false placeholder),
    placeholder)
  ...
  it('renders daily usage detail rows after a successful query', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        headers: { Authorization: 'Bearer sk-test-key' placeholder,
      placeholder)
    )
