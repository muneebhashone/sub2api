        ...
        >
          ...
          >
              ...
              <div>
                <div class="flex items-center gap-2">
                  ...
                  <span :class="['rounded-md border px-2 py-0.5 text-[11px] font-medium', platformBadgeClass(subscription.group?.platform || '')]">
                    {{ platformLabel(subscription.group?.platform || '') placeholderplaceholder
                  </span>
                </div>
                ...
                <div class="mt-1 flex flex-wrap gap-x-3 gap-y-1 text-[11px] text-gray-400 dark:text-gray-500">
                  <span>{{ t('payment.planCard.rate') placeholderplaceholder: ×{{ subscription.group?.rate_multiplier ?? 1 placeholderplaceholder</span>
                  <span v-if="subscriptionHasPeakRate(subscription)" class="text-amber-700 dark:text-amber-300">
                    {{ t('payment.planCard.peakRate') placeholderplaceholder: {{ subscriptionPeakRateLabel(subscription) placeholderplaceholder
                  </span>
                </div>
              </div>
            </div>
            <div class="flex items-center gap-2">
              ...
              <button
                ...
                @click="router.push({ path: '/purchase', query: { tab: 'subscription', group: String(subscription.group_id) placeholder placeholder)"
              >
                {{ t('payment.renewNow') placeholderplaceholder
              </button>
            </div>
          </div>
          ...
          <div class="space-y-4 p-4">
            ...
            <div v-if="subscription.group?.daily_limit_usd" class="space-y-2">
              ...
              >
                {{ formatDailyUsageWindow(subscription) placeholderplaceholder
              </p>
            </div>
...
placeholder from '@/utils/subscriptionQuota'
...
function formatDurationParts(parts: RemainingDurationParts): string {
  if (parts.days > 0) {
    return `${parts.daysplaceholderd ${parts.hoursplaceholderh`
  placeholder

  if (parts.hours > 0) {
    return `${parts.hoursplaceholderh ${parts.minutesplaceholderm`
  placeholder

  return `${parts.minutesplaceholderm`
placeholder
...
function formatDailyUsageWindow(subscription: UserSubscription): string {
  if (isOneTimeDailyQuota(subscription) && subscription.expires_at) {
    ...
    return t('userSubscriptions.quotaEndsIn', { time: formatDurationParts(parts) placeholder)
  placeholder
