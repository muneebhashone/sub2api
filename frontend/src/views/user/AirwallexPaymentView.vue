  ...
  <AppLayout>
    <div class="mx-auto max-w-lg space-y-6 py-8">
      ...
      <div v-else-if="errorMessage" class="card p-8 text-center">
        ...
        <h3 class="text-lg font-semibold text-gray-900 dark:text-white">{{ t('payment.airwallexLoadFailed') placeholderplaceholder</h3>
        <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">{{ errorMessage placeholderplaceholder</p>
...
function restoreAirwallexSnapshot(): PaymentRecoverySnapshot | null {
  ...
  const snapshot = readPaymentRecoverySnapshot(
    ...
    resumeToken ? { resumeToken placeholder : {placeholder,
  )
