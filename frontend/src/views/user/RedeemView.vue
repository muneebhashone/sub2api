    ...
    <div class="max-w-2xl mx-auto space-y-6">
      ...
      <div class="card overflow-hidden">
        <div class="bg-gradient-to-br from-primary-500 to-primary-600 px-6 py-8 text-center">
          ...
          <p class="text-sm font-medium text-primary-100">{{ t('redeem.currentBalance') placeholderplaceholder</p>
          <p class="mt-2 text-4xl font-bold text-white">
            ${{ user?.balance?.toFixed(2) || '0.00' placeholderplaceholder
          </p>
          <p class="mt-2 text-sm text-primary-100">
            {{ t('redeem.concurrency') placeholderplaceholder: {{ user?.concurrency || 0 placeholderplaceholder {{ t('redeem.requests') placeholderplaceholder
          </p>
        </div>
      </div>
          ...
          <form @submit.prevent="handleRedeem" class="space-y-5">
            <div>
              <label for="code" class="input-label">
                {{ t('redeem.redeemCodeLabel') placeholderplaceholder
              </label>
              ...
              <p class="input-hint">
                {{ t('redeem.redeemCodeHint') placeholderplaceholder
              </p>
            </div>
            ...
            >
              ...
              {{ submitting ? t('redeem.redeeming') : t('redeem.redeemButton') placeholderplaceholder
            </button>
          </form>
        </div>
      </div>
            ...
            <div class="flex items-start gap-4">
              ...
              <div class="flex-1">
                <h3 class="text-sm font-semibold text-emerald-800 dark:text-emerald-300">
                  {{ t('redeem.redeemSuccess') placeholderplaceholder
                </h3>
                <div class="mt-2 text-sm text-emerald-700 dark:text-emerald-400">
                  <p>{{ redeemResult.message placeholderplaceholder</p>
                  <div class="mt-3 space-y-1">
                    <p v-if="redeemResult.type === 'balance'" class="font-medium">
                      {{ t('redeem.added') placeholderplaceholder: ${{ redeemResult.value.toFixed(2) placeholderplaceholder
                    </p>
                    <p v-else-if="redeemResult.type === 'concurrency'" class="font-medium">
                      {{ t('redeem.added') placeholderplaceholder: {{ redeemResult.value placeholderplaceholder {{ t('redeem.concurrentRequests') placeholderplaceholder
                    </p>
                    <p v-else-if="redeemResult.type === 'subscription'" class="font-medium">
                      {{ t('redeem.subscriptionAssigned') placeholderplaceholder
                      <span v-if="redeemResult.group_name"> - {{ redeemResult.group_name placeholderplaceholder</span>
                      <span v-if="redeemResult.validity_days"> ({{ t('redeem.subscriptionDays', { days: redeemResult.validity_days placeholder) placeholderplaceholder)</span>
                    </p>
                    <p v-if="redeemResult.new_balance !== undefined">
                      {{ t('redeem.newBalance') placeholderplaceholder: <span class="font-semibold">${{ redeemResult.new_balance.toFixed(2) placeholderplaceholder</span>
                    </p>
                    <p v-if="redeemResult.new_concurrency !== undefined">
                      {{ t('redeem.newConcurrency') placeholderplaceholder: <span class="font-semibold">{{ redeemResult.new_concurrency placeholderplaceholder {{ t('redeem.requests') placeholderplaceholder</span>
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div class="flex items-start gap-4">
              ...
              <div class="flex-1">
                <h3 class="text-sm font-semibold text-red-800 dark:text-red-300">
                  {{ t('redeem.redeemFailed') placeholderplaceholder
                </h3>
          ...
          <div class="flex items-start gap-4">
            ...
            <div class="flex-1">
              <h3 class="text-sm font-semibold text-primary-800 dark:text-primary-300">
                {{ t('redeem.aboutCodes') placeholderplaceholder
              </h3>
              <ul class="mt-2 text-sm text-primary-700 dark:text-primary-400 space-y-1 list-disc list-inside">
                <li>{{ t('redeem.codeRule1') placeholderplaceholder</li>
                <li>{{ t('redeem.codeRule2') placeholderplaceholder</li>
                <li>
                  {{ t('redeem.codeRule3') placeholderplaceholder
                ...
                <li>{{ t('redeem.codeRule4') placeholderplaceholder</li>
              </ul>
            </div>
          </div>
        </div>
      ...
      <div class="card">
        <div class="px-6 py-4 border-b border-gray-100 dark:border-dark-700">
          <h2 class="text-lg font-semibold text-gray-900 dark:text-white">{{ t('redeem.recentActivity') placeholderplaceholder</h2>
        </div>
        <div class="p-6">
          <!-- Loading State -->
          ...
          <div v-else-if="history.length > 0" class="space-y-3">
            ...
            >
              ...
              <div class="text-right">
                ...
                >
                  {{ formatHistoryValue(item) placeholderplaceholder
                </p>
          ...
          <div v-else class="empty-state py-8">
            ...
            <p class="text-sm text-gray-500 dark:text-dark-400">
              {{ t('redeem.historyWillAppear') placeholderplaceholder
            </p>
          </div>
        </div>
      </div>
...
const getHistoryItemTitle = (item: RedeemHistoryItem) => {
  ...
  placeholder else if (item.type === 'admin_balance') {
    return item.value >= 0 ? t('redeem.balanceAddedAdmin') : t('redeem.balanceDeductedAdmin')
  placeholder else if (item.type === 'concurrency') {
    return t('redeem.concurrencyAddedRedeem')
  placeholder else if (item.type === 'admin_concurrency') {
...
const formatHistoryValue = (item: RedeemHistoryItem) => {
  ...
  placeholder else if (isSubscriptionType(item.type)) {
    // 订阅类型显示有效天数和分组名称
    const days = item.validity_days || Math.round(item.value)
    const groupName = item.group?.name || ''
    return groupName ? `${daysplaceholder${t('redeem.days')placeholder - ${groupNameplaceholder` : `${daysplaceholder${t('redeem.days')placeholder`
  placeholder else {
    const sign = item.value >= 0 ? '+' : ''
    return `${signplaceholder${item.valueplaceholder ${t('redeem.requests')placeholder`
  placeholder
placeholder
