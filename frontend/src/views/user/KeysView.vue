  ...
  <AppLayout>
    <TablePageLayout>
        ...
        <div class="flex justify-end gap-3">
          ...
          <div class="relative" ref="columnDropdownRef">
            ...
            >
              ...
              <span class="hidden md:inline">{{ t('keys.columnSettings') placeholderplaceholder</span>
            </button>
            <div
      ...
      <template #table>
        ...
        >
          ...
          <template #cell-key="{ value, row placeholder">
            <div class="flex items-center gap-2">
              <code class="code text-xs">
                {{ maskApiKey(value) placeholderplaceholder
              </code>
              <button
          ...
          <template #cell-group="{ row placeholder">
            <div class="group/dropdown relative">
              ...
              >
                ...
                <span class="text-xs text-gray-500 dark:text-gray-400">{{ t('keys.selectGroup') placeholderplaceholder</span>
                <svg
          ...
          <template #cell-current_concurrency="{ value placeholder">
            <span
              :class="[
            ...
            >
              {{ value ?? 0 placeholderplaceholder
            </span>
          </template>
          ...
          <template #cell-usage="{ row placeholder">
            <div class="text-sm">
              ...
              <div v-if="row.quota > 0" class="mt-1.5">
                <div class="flex items-center gap-1.5">
                  <span class="text-gray-500 dark:text-gray-400">{{ t('keys.quota') placeholderplaceholder:</span>
                  <span :class="[
                    'font-medium',
                    ...
                    ${{ row.quota_used?.toFixed(2) || '0.00' placeholderplaceholder / ${{ row.quota?.toFixed(2) placeholderplaceholder
                  </span>
                </div>
                <div class="mt-1 h-1.5 w-full overflow-hidden rounded-full bg-gray-200 dark:bg-dark-600">
                  <div
                    ...
                    :style="{ width: Math.min((row.quota_used / row.quota) * 100, 100) + '%' placeholder"
                  />
                </div>
              </div>
            </div>
          ...
          <template #cell-rate_limit="{ row placeholder">
            <div v-if="row.rate_limit_5h > 0 || row.rate_limit_1d > 0 || row.rate_limit_7d > 0" class="space-y-1.5 min-w-[140px]">
              ...
              <div v-if="row.rate_limit_5h > 0">
                <div class="flex items-center justify-between text-xs">
                  ...
                  <span :class="[
                    ...
                    ${{ row.usage_5h?.toFixed(2) || '0.00' placeholderplaceholder/${{ row.rate_limit_5h?.toFixed(2) placeholderplaceholder
                  </span>
                </div>
                <div class="h-1 w-full overflow-hidden rounded-full bg-gray-200 dark:bg-dark-600">
                  <div
                    ...
                    :style="{ width: Math.min((row.usage_5h / row.rate_limit_5h) * 100, 100) + '%' placeholder"
                  />
                </div>
                <div v-if="row.reset_5h_at && formatResetTime(row.reset_5h_at)" class="text-[10px] text-gray-400 dark:text-gray-500 tabular-nums">
                  ⟳ {{ formatResetTime(row.reset_5h_at) placeholderplaceholder
                </div>
              </div>
              <!-- 1d window -->
              <div v-if="row.rate_limit_1d > 0">
                <div class="flex items-center justify-between text-xs">
                  ...
                  <span :class="[
                    ...
                    ${{ row.usage_1d?.toFixed(2) || '0.00' placeholderplaceholder/${{ row.rate_limit_1d?.toFixed(2) placeholderplaceholder
                  </span>
                </div>
                <div class="h-1 w-full overflow-hidden rounded-full bg-gray-200 dark:bg-dark-600">
                  <div
                    ...
                    :style="{ width: Math.min((row.usage_1d / row.rate_limit_1d) * 100, 100) + '%' placeholder"
                  />
                </div>
                <div v-if="row.reset_1d_at && formatResetTime(row.reset_1d_at)" class="text-[10px] text-gray-400 dark:text-gray-500 tabular-nums">
                  ⟳ {{ formatResetTime(row.reset_1d_at) placeholderplaceholder
                </div>
              </div>
              <!-- 7d window -->
              <div v-if="row.rate_limit_7d > 0">
                <div class="flex items-center justify-between text-xs">
                  ...
                  <span :class="[
                    ...
                    ${{ row.usage_7d?.toFixed(2) || '0.00' placeholderplaceholder/${{ row.rate_limit_7d?.toFixed(2) placeholderplaceholder
                  </span>
                </div>
                <div class="h-1 w-full overflow-hidden rounded-full bg-gray-200 dark:bg-dark-600">
                  <div
                    ...
                    :style="{ width: Math.min((row.usage_7d / row.rate_limit_7d) * 100, 100) + '%' placeholder"
                  />
                </div>
                <div v-if="row.reset_7d_at && formatResetTime(row.reset_7d_at)" class="text-[10px] text-gray-400 dark:text-gray-500 tabular-nums">
                  ⟳ {{ formatResetTime(row.reset_7d_at) placeholderplaceholder
                </div>
              </div>
              <!-- Reset button -->
              <button
              ...
              >
                ...
                {{ t('keys.resetUsage') placeholderplaceholder
              </button>
            </div>
          ...
          <template #cell-expires_at="{ value placeholder">
            ...
            <span v-else class="text-sm text-gray-400 dark:text-dark-500">{{ t('keys.noExpiration') placeholderplaceholder</span>
          </template>
          ...
          <template #cell-status="{ value placeholder">
            <span :class="[
              ...
              {{ t('keys.status.' + value) placeholderplaceholder
            </span>
          </template>
          ...
          <template #cell-last_used_ip="{ value placeholder">
            <span v-if="value" class="text-sm text-gray-500 dark:text-dark-400">
    ...
    >
      <form id="key-form" @submit.prevent="handleSubmit" class="space-y-5">
        ...
        <div class="space-y-3">
          <div class="flex items-center justify-between">
            <label class="input-label mb-0">{{ t('keys.ipRestriction') placeholderplaceholder</label>
            <button
              type="button"
          ...
          <div v-if="formData.enable_ip_restriction" class="space-y-4 pt-2">
            <div>
              <label class="input-label">{{ t('keys.ipWhitelist') placeholderplaceholder</label>
              <textarea
              ...
              />
              <p class="input-hint">{{ t('keys.ipWhitelistHint') placeholderplaceholder</p>
            </div>

            <div>
              <label class="input-label">{{ t('keys.ipBlacklist') placeholderplaceholder</label>
              <textarea
              ...
              />
              <p class="input-hint">{{ t('keys.ipBlacklistHint') placeholderplaceholder</p>
            </div>
          </div>
        </div>
        ...
        <!-- Quota Limit Section -->
        <div class="space-y-3">
          <label class="input-label">{{ t('keys.quotaLimit') placeholderplaceholder</label>
          <!-- Switch commented out - always show input, 0 = unlimited
          <div class="flex items-center justify-between">
            <label class="input-label mb-0">{{ t('keys.quotaLimit') placeholderplaceholder</label>
            <button
              type="button"
          ...
          <div class="space-y-4">
            <div>
              ...
              <p class="input-hint">{{ t('keys.quotaAmountHint') placeholderplaceholder</p>
            </div>

            <!-- Quota used display (only in edit mode) -->
            <div v-if="showEditModal && selectedKey && selectedKey.quota > 0">
              <label class="input-label">{{ t('keys.quotaUsed') placeholderplaceholder</label>
              <div class="flex items-center gap-2">
                <div class="flex-1 rounded-lg bg-gray-100 px-3 py-2 dark:bg-dark-700">
                  <span class="font-medium text-gray-900 dark:text-white">
                    ${{ selectedKey.quota_used?.toFixed(4) || '0.0000' placeholderplaceholder
                  </span>
                  <span class="mx-2 text-gray-400">/</span>
                  <span class="text-gray-500 dark:text-gray-400">
                    ${{ selectedKey.quota?.toFixed(2) || '0.00' placeholderplaceholder
                  </span>
                </div>
                <button
                  type="button"
                ...
                >
                  {{ t('keys.reset') placeholderplaceholder
                </button>
              </div>
            </div>
          </div>
        ...
        <div class="space-y-3">
          <div class="flex items-center justify-between">
            <label class="input-label mb-0">{{ t('keys.rateLimitSection') placeholderplaceholder</label>
            <button
              type="button"
          ...
          <div v-if="formData.enable_rate_limit" class="space-y-4 pt-2">
            <p class="input-hint -mt-2">{{ t('keys.rateLimitHint') placeholderplaceholder</p>
            <!-- 5-Hour Limit -->
            <div>
              <label class="input-label">{{ t('keys.rateLimit5h') placeholderplaceholder</label>
              <div class="relative">
                <span class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500">$</span>
                <input
                ...
                <div class="flex items-center gap-2">
                  <div class="flex-1 rounded-lg bg-gray-100 px-3 py-2 dark:bg-dark-700 text-sm">
                    <span :class="[
                      ...
                      ${{ selectedKey.usage_5h?.toFixed(4) || '0.0000' placeholderplaceholder
                    </span>
                    <span class="mx-2 text-gray-400">/</span>
                    <span class="text-gray-500 dark:text-gray-400">
                      ${{ selectedKey.rate_limit_5h?.toFixed(2) || '0.00' placeholderplaceholder
                    </span>
                  </div>
                </div>
                <div class="mt-1 h-1.5 w-full overflow-hidden rounded-full bg-gray-200 dark:bg-dark-600">
                  <div
                    ...
                    :style="{ width: Math.min((selectedKey.usage_5h / selectedKey.rate_limit_5h) * 100, 100) + '%' placeholder"
                  />
                </div>
              </div>
            </div>
            ...
            <!-- Daily Limit -->
            <div>
              <label class="input-label">{{ t('keys.rateLimit1d') placeholderplaceholder</label>
              <div class="relative">
                <span class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500">$</span>
                <input
                ...
                <div class="flex items-center gap-2">
                  <div class="flex-1 rounded-lg bg-gray-100 px-3 py-2 dark:bg-dark-700 text-sm">
                    <span :class="[
                      ...
                      ${{ selectedKey.usage_1d?.toFixed(4) || '0.0000' placeholderplaceholder
                    </span>
                    <span class="mx-2 text-gray-400">/</span>
                    <span class="text-gray-500 dark:text-gray-400">
                      ${{ selectedKey.rate_limit_1d?.toFixed(2) || '0.00' placeholderplaceholder
                    </span>
                  </div>
                </div>
                <div class="mt-1 h-1.5 w-full overflow-hidden rounded-full bg-gray-200 dark:bg-dark-600">
                  <div
                    ...
                    :style="{ width: Math.min((selectedKey.usage_1d / selectedKey.rate_limit_1d) * 100, 100) + '%' placeholder"
                  />
                </div>
              </div>
            </div>
            ...
            <!-- 7-Day Limit -->
            <div>
              <label class="input-label">{{ t('keys.rateLimit7d') placeholderplaceholder</label>
              <div class="relative">
                <span class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500">$</span>
                <input
                ...
                <div class="flex items-center gap-2">
                  <div class="flex-1 rounded-lg bg-gray-100 px-3 py-2 dark:bg-dark-700 text-sm">
                    <span :class="[
                      ...
                      ${{ selectedKey.usage_7d?.toFixed(4) || '0.0000' placeholderplaceholder
                    </span>
                    <span class="mx-2 text-gray-400">/</span>
                    <span class="text-gray-500 dark:text-gray-400">
                      ${{ selectedKey.rate_limit_7d?.toFixed(2) || '0.00' placeholderplaceholder
                    </span>
                  </div>
                </div>
                <div class="mt-1 h-1.5 w-full overflow-hidden rounded-full bg-gray-200 dark:bg-dark-600">
                  <div
                    ...
                    :style="{ width: Math.min((selectedKey.usage_7d / selectedKey.rate_limit_7d) * 100, 100) + '%' placeholder"
                  />
                </div>
              </div>
            </div>
            ...
            <div v-if="showEditModal && selectedKey && (selectedKey.rate_limit_5h > 0 || selectedKey.rate_limit_1d > 0 || selectedKey.rate_limit_7d > 0)">
              <button
                ...
                class="btn btn-secondary text-sm"
              >
                {{ t('keys.resetRateLimitUsage') placeholderplaceholder
              </button>
            </div>
          </div>
        </div>
        ...
        <div class="space-y-3">
          <div class="flex items-center justify-between">
            <label class="input-label mb-0">{{ t('keys.expiration') placeholderplaceholder</label>
            <button
              type="button"
          ...
          <div v-if="formData.enable_expiration" class="space-y-4 pt-2">
            ...
            <div class="flex flex-wrap gap-2">
              ...
              >
                {{ showEditModal ? t('keys.extendDays', { days placeholder) : t('keys.expiresInDays', { days placeholder) placeholderplaceholder
              </button>
              <button
                type="button"
              ...
              >
                {{ t('keys.customDate') placeholderplaceholder
              </button>
            </div>

            <!-- Date picker (always show for precise adjustment) -->
            <div>
              <label class="input-label">{{ t('keys.expirationDate') placeholderplaceholder</label>
              <input
                v-model="formData.expiration_date"
                type="datetime-local"
                class="input"
              />
              <p class="input-hint">{{ t('keys.expirationDateHint') placeholderplaceholder</p>
            </div>

            <!-- Current expiration display (only in edit mode) -->
            <div v-if="showEditModal && selectedKey?.expires_at" class="text-sm">
              <span class="text-gray-500 dark:text-gray-400">{{ t('keys.currentExpiration') placeholderplaceholder: </span>
              <span class="font-medium text-gray-900 dark:text-white">
                {{ formatDateTime(selectedKey.expires_at) placeholderplaceholder
              </span>
            </div>
          </div>
        </div>
    ...
    <ConfirmDialog
      ...
      :message="t('keys.resetQuotaConfirmMessage', { name: selectedKey?.name, used: selectedKey?.quota_used?.toFixed(4) placeholder)"
    ...
    <ConfirmDialog
      ...
      :message="t('keys.resetRateLimitConfirmMessage', { name: selectedKey?.name placeholder)"
      :confirm-text="t('keys.reset')"
      :cancel-text="t('common.cancel')"
      :danger="true"
      ...
      >
        ...
        <div class="max-h-80 overflow-y-auto p-1.5">
          ...
          <div v-if="filteredGroupOptions.length === 0" class="py-4 text-center text-sm text-gray-400 dark:text-gray-500">
            {{ t('keys.noGroupFound') placeholderplaceholder
          </div>
        </div>
      </div>
    </Teleport>
...
import TablePageLayout from '@/components/layout/TablePageLayout.vue'
	...
	import type { ApiKey, Group, PublicSettings, SubscriptionType, GroupPlatform, UpdateApiKeyRequest placeholder from '@/types'
...
const formatDateTimeLocal = (isoDate: string): string => {
  ...
  return `${date.getFullYear()placeholder-${pad(date.getMonth() + 1)placeholder-${pad(date.getDate())placeholderT${pad(date.getHours())placeholder:${pad(date.getMinutes())placeholder`
placeholder
...
const allColumns = computed<Column[]>(() => [
  ...
  { key: 'id', label: t('keys.id'), sortable: true placeholder,
  { key: 'key', label: t('keys.apiKey'), sortable: false placeholder,
  { key: 'group', label: t('keys.group'), sortable: false placeholder,
  { key: 'current_concurrency', label: t('keys.currentConcurrency'), sortable: true placeholder,
  { key: 'usage', label: t('keys.usage'), sortable: false placeholder,
  { key: 'rate_limit', label: t('keys.rateLimitColumn'), sortable: false placeholder,
  ...
  { key: 'last_used_ip', label: t('keys.lastUsedIP'), sortable: false placeholder,
...
const dropdownPosition = ref<{ top?: number; bottom?: number; left: number placeholder | null>(null)
...
const groupFilterOptions = computed(() => [
  { value: '', label: t('keys.allGroups') placeholder,
  { value: 0, label: t('keys.noGroup') placeholder,
  ...groups.value.map((g) => ({ value: g.id, label: g.name placeholder))
])

const statusFilterOptions = computed(() => [
  { value: '', label: t('keys.allStatus') placeholder,
  { value: 'active', label: t('keys.status.active') placeholder,
  { value: 'inactive', label: t('keys.status.inactive') placeholder,
  { value: 'quota_exhausted', label: t('keys.status.quota_exhausted') placeholder,
  { value: 'expired', label: t('keys.status.expired') placeholder
])
...
const handleSubmit = async () => {
  ...
  if (formData.value.enable_expiration && formData.value.expiration_date) {
    ...
    placeholder
  placeholder else if (showEditModal.value) {
  ...
  placeholder : { rate_limit_5h: 0, rate_limit_1d: 0, rate_limit_7d: 0 placeholder
...
const resetQuotaUsed = async () => {
  ...
  try {
    await keysAPI.update(selectedKey.value.id, { reset_quota: true placeholder)
...
const resetRateLimitUsage = async () => {
  ...
  try {
    await keysAPI.update(selectedKey.value.id, { reset_rate_limit_usage: true placeholder)
