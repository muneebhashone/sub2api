  ...
  <div class="sora-root">
    ...
    <div class="sora-page">
      ...
      <div v-if="!soraEnabled" class="sora-not-enabled">
        ...
        <h2 class="sora-not-enabled-title">{{ t('sora.notEnabled') placeholderplaceholder</h2>
        <p class="sora-not-enabled-desc">{{ t('sora.notEnabledDesc') placeholderplaceholder</p>
      </div>
        ...
        <header class="sora-header">
          ...
          <div class="sora-header-right">
            ...
            <div v-if="activeTaskCount > 0" class="sora-queue-indicator">
              <span class="sora-queue-dot" :class="{ busy: hasGeneratingTask placeholder"></span>
              <span>{{ activeTaskCount placeholderplaceholder {{ t('sora.queueTasks') placeholderplaceholder</span>
            </div>
          </div>
        </header>
...
import soraAPI, { type QuotaInfo placeholder from '@/api/sora'
...
const tabs = computed(() => [
  { key: 'generate' as const, label: t('sora.tabGenerate') placeholder,
  { key: 'library' as const, label: t('sora.tabLibrary') placeholder
])

function onTaskCountChange(counts: { active: number; generating: boolean placeholder) {
