  ...
  <component :is="isPopup ? 'div' : AppLayout" :class="isPopup ? 'min-h-screen bg-gray-50 dark:bg-dark-900' : ''">
    <div class="mx-auto max-w-lg space-y-6 py-8" :class="isPopup ? 'px-4' : ''">
      ...
      <div v-else-if="initError" class="card p-8 text-center">
        ...
        <h3 class="text-lg font-semibold text-gray-900 dark:text-white">{{ t('payment.stripeLoadFailed') placeholderplaceholder</h3>
        <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">{{ initError placeholderplaceholder</p>
        <button class="btn btn-primary mt-6" @click="router.push('/purchase')">{{ t('payment.result.backToRecharge') placeholderplaceholder</button>
      </div>
      <template v-else>
        <!-- 金额头部 -->
        <div v-if="order" class="card overflow-hidden">
          <div class="bg-gradient-to-br from-[#635bff] to-[#4f46e5] px-6 py-6 text-center">
            ...
            <p class="mt-1 text-3xl font-bold text-white">{{ formatGatewayAmount(order.pay_amount) placeholderplaceholder</p>
          </div>
        </div>
        ...
        <template v-if="wechatQrUrl">
          <div class="card p-6">
            <div class="flex flex-col items-center space-y-4">
              <p class="text-lg font-semibold text-gray-900 dark:text-white">{{ t('payment.qr.scanWxpay') placeholderplaceholder</p>
              ...
              <p class="text-center text-sm text-gray-500 dark:text-gray-400">{{ t('payment.qr.scanWxpayHint') placeholderplaceholder</p>
            </div>
          </div>
          <div class="card p-4 text-center">
            <p class="text-sm text-gray-500 dark:text-gray-400">{{ t('payment.qr.waitingPayment') placeholderplaceholder</p>
          </div>
        </template>
        ...
        <template v-else-if="stripeSuccess">
          <div class="card p-6 text-center">
            <div class="flex flex-col items-center gap-3 py-4">
              ...
              <p class="text-sm text-gray-500 dark:text-gray-400">{{ t('payment.stripeSuccessProcessing') placeholderplaceholder</p>
            </div>
          </div>
        </template>
        ...
        <template v-else-if="showPaymentElement">
          <div class="card p-6">
            ...
            <p v-if="stripeError" class="mt-4 text-sm text-red-600 dark:text-red-400">{{ stripeError placeholderplaceholder</p>
          ...
          <div class="text-center">
            <button class="btn btn-secondary" @click="router.push('/purchase')">{{ t('payment.result.backToRecharge') placeholderplaceholder</button>
          </div>
        </template>

        <!-- 错误状态 -->
        <div v-if="stripeError && !showPaymentElement" class="card p-4">
          <p class="text-sm text-red-600 dark:text-red-400">{{ stripeError placeholderplaceholder</p>
          <button class="btn btn-secondary mt-3 w-full" @click="router.push('/purchase')">{{ t('payment.result.backToRecharge') placeholderplaceholder</button>
        </div>
      </template>
    </div>
  </component>
...
import { ref, computed, nextTick, onMounted, onUnmounted placeholder from 'vue'
...
async function confirmAlipay(stripe: Stripe, clientSecret: string, orderId: number) {
  ...
  const { error placeholder = await stripe.confirmAlipayPayment(clientSecret, { return_url: returnUrl placeholder)
  if (error) {
...
async function confirmWechatPay(stripe: Stripe, clientSecret: string) {
  const { paymentIntent, error placeholder = await (stripe as Stripe & {
    confirmWechatPayPayment: (cs: string, opts: Record<string, unknown>) => Promise<{ paymentIntent?: { status: string; next_action?: { wechat_pay_display_qr_code?: { image_data_url?: string placeholder placeholder placeholder; error?: { message?: string placeholder placeholder>
  placeholder).confirmWechatPayPayment(clientSecret, {
  ...
  if (qrData) {
    ...
    // 轮询支付完成状态
    startPolling()
  placeholder else if (paymentIntent?.status === 'succeeded') {
...
function mountPaymentElement(stripe: Stripe, clientSecret: string) {
  ...
  paymentElement.on('ready', () => { stripeReady.value = true placeholder)
placeholder
...
async function handleGenericPay() {
  ...
  stripeError.value = ''
  try {
    const { error placeholder = await stripeInstance.confirmPayment({
...
function scheduleClose() {
  if (window.opener) {
    redirectTimer = setTimeout(() => { window.close() placeholder, 2000)
  placeholder else {
    redirectTimer = setTimeout(() => {
      router.push({ path: '/payment/result', query: { order_id: String(route.query.order_id || ''), status: 'success' placeholder placeholder)
    placeholder, 2000)
  placeholder
placeholder
