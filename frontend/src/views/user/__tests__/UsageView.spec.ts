...
vi.mock('@/stores/app', () => ({
  useAppStore: () => ({ showError, showWarning, showSuccess, showInfo placeholder),
placeholder))
...
const simpleStub = { template: '<div><slot /></div>' placeholder
const chartStub = { template: '<div />' placeholder
...
const usageLog = {
  ...
  api_key: { name: 'demo-key' placeholder,
  billing_mode: 'token',
  request_type: 'sync',
  stream: false,
placeholder
...
describe('user UsageView', () => {
  beforeEach(() => {
    ...
    query.mockResolvedValue({ items: [usageLog], total: 1, pages: 1 placeholder)
    ...
    getDashboardModels.mockResolvedValue({
      models: [{ model: 'gpt-5.4', requests: 1, input_tokens: 10, output_tokens: 20, cache_creation_tokens: 0, cache_read_tokens: 0, total_tokens: 30, cost: 0.1, actual_cost: 0.08 placeholder],
    ...
    getDashboardSnapshotV2.mockResolvedValue({
      ...
      groups: [],
    placeholder)
    list.mockResolvedValue({ items: [{ id: 1, name: 'demo-key' placeholder] placeholder)
    getAvailable.mockResolvedValue([{ id: 1, name: 'default' placeholder])
  placeholder)
  ...
  it('exports csv with current filters and without admin-only fields', async () => {
    ...
    placeholder) as typeof window.URL.createObjectURL
    window.URL.revokeObjectURL = vi.fn(() => {placeholder) as typeof window.URL.revokeObjectURL
    const clickSpy = vi.spyOn(HTMLAnchorElement.prototype, 'click').mockImplementation(() => {placeholder)
