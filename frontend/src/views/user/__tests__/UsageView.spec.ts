...
const { query, getStatsByDateRange, list, showError, showWarning, showSuccess, showInfo placeholder = vi.hoisted(() => ({
  query: vi.fn(),
...
describe('user UsageView tooltip', () => {
  ...
  it('shows fast service tier and unit prices in user tooltip', async () => {
    ...
    getStatsByDateRange.mockResolvedValue({
      ...
      avg_duration_ms: 1,
    placeholder)
    list.mockResolvedValue({ items: [] placeholder)
  ...
  it('shows image billing metadata in the user cost tooltip', async () => {
    ...
    setupState.tooltipData = {
      ...
      image_size_breakdown: { '4K': 2 placeholder,
    placeholder
    setupState.tooltipVisible = true
    await nextTick()
