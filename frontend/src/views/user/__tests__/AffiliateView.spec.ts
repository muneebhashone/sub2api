...
const { copyToClipboard, getAffiliateDetail placeholder = vi.hoisted(() => ({
...
vi.mock('@/composables/useClipboard', () => ({
  useClipboard: () => ({ copyToClipboard placeholder),
placeholder))
...
describe('AffiliateView', () => {
  ...
  it('stacks long values and copy controls on mobile while retaining desktop rows', async () => {
    const wrapper = mount(AffiliateView, {
      global: {
        stubs: {
          AppLayout: { template: '<main><slot /></main>' placeholder,
          Icon: true,
        placeholder,
      placeholder,
    placeholder)
    ...
    expect(copyToClipboard).toHaveBeenNthCalledWith(
      2,
      `${window.location.originplaceholder/register?aff=${encodeURIComponent(affiliateCode)placeholder`,
