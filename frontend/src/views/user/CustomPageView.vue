      ...
      <div class="card flex-1 min-h-0 overflow-hidden">
        ...
        >
          <div class="max-w-md">
            ...
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
              {{ t('customPage.notFoundTitle') placeholderplaceholder
            </h3>
            <p class="mt-2 text-sm text-gray-500 dark:text-dark-400">
              {{ t('customPage.notFoundDesc') placeholderplaceholder
            </p>
          </div>
        </div>
        ...
        <div v-else-if="isMarkdownMode" class="flex h-full overflow-hidden">
          ...
          >
            <div class="toc-header">
              <span class="toc-title">{{ t('customPage.tableOfContents') placeholderplaceholder</span>
            ...
            <nav class="toc-nav">
              <a
                ...
                class="toc-item"
                :class="[
                  `toc-level-${item.levelplaceholder`,
                  { 'toc-active': activeHeadingId === item.id placeholder
                ]"
                @click.prevent="scrollToHeading(item.id)"
              >
                {{ item.text placeholderplaceholder
              </a>
            </nav>
          </aside>
          ...
          >
            ...
            <span class="ml-1 text-xs">{{ t('customPage.tableOfContents') placeholderplaceholder</span>
          </button>
        ...
        <div v-else-if="!isValidUrl" class="flex h-full items-center justify-center p-10 text-center">
          <div class="max-w-md">
            ...
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
              {{ t('customPage.notConfiguredTitle') placeholderplaceholder
            </h3>
            <p class="mt-2 text-sm text-gray-500 dark:text-dark-400">
              {{ t('customPage.notConfiguredDesc') placeholderplaceholder
            </p>
          </div>
        </div>
        ...
        <div v-else class="custom-embed-shell">
          ...
          >
            ...
            {{ t('customPage.openInNewTab') placeholderplaceholder
          </a>
          <iframe
            :src="embeddedUrl"
...
import { computed, nextTick, onMounted, onUnmounted, ref, watch placeholder from 'vue'
...
function generateHeadingId(text: string, index: number): string {
  const base = text
    ...
    .replace(/[^\w一-鿿]+/g, '-')
    .replace(/^-+|-+$/g, '')
  return base ? `${baseplaceholder-${indexplaceholder` : `heading-${indexplaceholder`
placeholder
...
async function fetchAndRenderMarkdown(slug: string) {
  ...
  try {
    ...
    placeholder)
    if (!resp.ok) {
      renderedHtml.value = `<p class="text-red-500">${t('common.pageNotFound')placeholder</p>`
      return
    placeholder
    ...
    raw = raw.replace(
      /!\[([^\]]*)\]\(([^)]+)\)/g,
      (match, alt, src) => isRelativeMarkdownAsset(src) ? `![${altplaceholder](${buildPageImageUrl(slug, src)placeholder)` : match
    )
    ...
    const withIds = sanitized.replace(
      ...
      (_, tag: string, content: string) => {
        ...
        toc.push({ id, text, level placeholder)
        return `<${tagplaceholder id="${idplaceholder">${contentplaceholder</${tagplaceholder>`
      placeholder
    )
...
function scrollToHeading(id: string) {
  ...
  if (!container) return
  const el = container.querySelector(`#${CSS.escape(id)placeholder`)
  if (el) {
    el.scrollIntoView({ behavior: 'smooth', block: 'start' placeholder)
...
function onContentScroll() {
  ...
  scrollRafId = requestAnimationFrame(() => {
    ...
    for (const item of tocItems.value) {
      const el = container.querySelector(`#${CSS.escape(item.id)placeholder`) as HTMLElement | null
      if (el) {
  ...
  container.querySelectorAll('pre').forEach((pre) => {
    ...
    btn.addEventListener('click', async () => {
      ...
      try {
        ...
        setTimeout(() => { btn.textContent = t('customPage.copyCode') placeholder, 2000)
      placeholder catch {
...
.toc-level-1 { padding-left: 8px; placeholder
.toc-level-2 { padding-left: 20px; placeholder
.toc-level-3 { padding-left: 32px; placeholder
.toc-level-4 { padding-left: 44px; placeholder
...
.markdown-page-content {
  ...
  color: inherit;
placeholder
.markdown-page-content h1 { @apply text-3xl font-bold mt-8 mb-4 pb-2 border-b border-gray-200 dark:border-dark-600; placeholder
.markdown-page-content h2 { @apply text-2xl font-bold mt-6 mb-3; placeholder
.markdown-page-content h3 { @apply text-xl font-semibold mt-5 mb-2; placeholder
.markdown-page-content h4 { @apply text-lg font-semibold mt-4 mb-2; placeholder
.markdown-page-content p { @apply mb-4; placeholder
.markdown-page-content ul { @apply list-disc pl-6 mb-4; placeholder
.markdown-page-content ol { @apply list-decimal pl-6 mb-4; placeholder
.markdown-page-content li { @apply mb-1; placeholder
.markdown-page-content a { @apply text-primary-500 hover:text-primary-600 underline; placeholder
.markdown-page-content blockquote { @apply border-l-4 border-gray-300 dark:border-dark-500 pl-4 italic text-gray-600 dark:text-dark-300 my-4; placeholder
.markdown-page-content img { @apply max-w-full h-auto rounded-lg my-4; placeholder
.markdown-page-content table { @apply w-full border-collapse my-4; placeholder
.markdown-page-content th { @apply border border-gray-300 dark:border-dark-500 px-3 py-2 bg-gray-50 dark:bg-dark-700 font-semibold text-left; placeholder
.markdown-page-content td { @apply border border-gray-300 dark:border-dark-500 px-3 py-2; placeholder
.markdown-page-content code { @apply bg-gray-100 dark:bg-dark-700 px-1.5 py-0.5 rounded text-sm font-mono; placeholder
.markdown-page-content pre { @apply bg-gray-900 dark:bg-dark-900 text-gray-100 p-4 rounded-lg overflow-x-auto my-4 relative; placeholder
.markdown-page-content pre code { @apply bg-transparent p-0 text-inherit; placeholder
.markdown-page-content hr { @apply my-6 border-gray-200 dark:border-dark-600; placeholder
...
.copy-btn {
  ...
  font-family: inherit;
placeholder
.copy-btn:hover { background: rgba(255, 255, 255, 0.25); placeholder
pre:hover .copy-btn { opacity: 1; placeholder
</style>
