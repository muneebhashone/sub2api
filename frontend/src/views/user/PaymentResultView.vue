...
import type { PublicOrderVerifyResult placeholder from '@/api/payment'
import type { OrderStatus, PaymentOrder placeholder from '@/types/payment'
...
async function resolveOrderFromOutTradeNo(outTradeNo: string): Promise<ResolvedOrder | null> {
  ...
  placeholder catch (_err: unknown) {
    try {
      ...
      return result.data
    placeholder catch (_innerErr: unknown) {
      return null
    placeholder
  placeholder
placeholder
