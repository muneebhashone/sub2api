  ...
  <AppLayout>
    <div class="mx-auto max-w-4xl space-y-6">
      <div class="grid grid-cols-1 gap-6 sm:grid-cols-3">
        ...
        <StatCard :title="t('profile.memberSince')" :value="formatDate(user?.created_at || '', { year: 'numeric', month: 'long' placeholder)" :icon="CalendarIcon" icon-variant="primary" />
      </div>
      ...
      <div v-if="contactInfo" class="card border-primary-200 bg-primary-50 dark:bg-primary-900/20 p-6">
        <div class="flex items-center gap-4">
          ...
          <div><h3 class="font-semibold text-primary-800 dark:text-primary-200">{{ t('common.contactSupport') placeholderplaceholder</h3><p class="text-sm font-medium">{{ contactInfo placeholderplaceholder</p></div>
        </div>
      </div>
...
import { ref, computed, h, onMounted placeholder from 'vue'; import { useI18n placeholder from 'vue-i18n'
import { useAuthStore placeholder from '@/stores/auth'; import { formatDate placeholder from '@/utils/format'
import { authAPI placeholder from '@/api'; import AppLayout from '@/components/layout/AppLayout.vue'
...
onMounted(async () => { try { const s = await authAPI.getPublicSettings(); contactInfo.value = s.contact_info || ''; balanceLowNotifyEnabled.value = s.balance_low_notify_enabled ?? false placeholder catch (error) { console.error('Failed to load contact info:', error) placeholder placeholder)
