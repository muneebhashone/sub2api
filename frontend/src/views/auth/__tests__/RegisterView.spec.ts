...
const { getPublicSettingsMock, registerMock, showErrorMock placeholder = vi.hoisted(() => ({
...
vi.mock('@/stores', () => ({
  useAuthStore: () => ({ register: (...args: unknown[]) => registerMock(...args) placeholder),
  useAppStore: () => ({
...
describe('RegisterView invitation layout', () => {
  ...
  it('submits a non-whitelist email domain so the backend can enforce its registration quota', async () => {
    ...
    expect(registerMock).toHaveBeenCalledWith(
      expect.objectContaining({ email: 'first@custom.example' placeholder)
    )
  ...
  it('still submits whitelisted email domains when the domain quota switch is disabled', async () => {
    ...
    expect(registerMock).toHaveBeenCalledWith(
      expect.objectContaining({ email: 'user@allowed.com' placeholder)
    )
    expect(showErrorMock).not.toHaveBeenCalled()
  placeholder)
placeholder)
