      ...
      <form @submit.prevent="handleLogin" class="space-y-5">
        ...
        <div v-if="showPasskeyLogin || showOAuthLogin" class="space-y-3 pt-1">
          ...
          >
            ...
            {{ passkeyLoading ? t('auth.passkeySigningIn') : t('auth.passkeySignIn') placeholderplaceholder
          </button>
...
placeholder): void {
  ...
  loginAgreementRevision.value =
    ...
    `${loginAgreementUpdatedAt.valueplaceholder:${documents.map((doc) => `${doc.idplaceholder:${doc.titleplaceholder`).join('|')placeholder`
...
function hasAcceptedLoginAgreement(revision: string): boolean {
  ...
  try {
    ...
    if (!raw) {
      return false
    placeholder
    const parsed = JSON.parse(raw) as { revision?: string placeholder
    ...
    if (actionCaptchaEnabled.value) {
      ...
      proof = tencentCaptchaEnabled.value
        ? {
          ...
          placeholder
        : { turnstile_token: result.token placeholder
    placeholder
    ...
    const result = await startOAuthLogin(
      ...
      tencentCaptchaEnabled.value
        ? {
          ...
          placeholder
        : { turnstile_token: proof.token placeholder
    )
