    ...
    <div class="space-y-6">
      ...
      <div class="text-center">
        <h2 class="text-2xl font-bold text-gray-900 dark:text-white">
          {{ t('auth.forgotPasswordTitle') placeholderplaceholder
        </h2>
        <p class="mt-2 text-sm text-gray-500 dark:text-dark-400">
          {{ t('auth.forgotPasswordHint') placeholderplaceholder
        </p>
      </div>
          ...
          <div class="flex flex-col items-center gap-4 text-center">
            ...
            <div>
              <h3 class="text-lg font-semibold text-green-800 dark:text-green-200">
                {{ t('auth.resetEmailSent') placeholderplaceholder
              </h3>
              <p class="mt-2 text-sm text-green-700 dark:text-green-300">
                {{ t('auth.resetEmailSentHint') placeholderplaceholder
              </p>
            </div>
          </div>
        </div>
      ...
      <form v-else @submit.prevent="handleSubmit" class="space-y-5">
        ...
        >
          ...
          {{ isLoading ? t('auth.sendingResetLink') : t('auth.sendResetLink') placeholderplaceholder
        </button>
      </form>
    </div>
...
import { getPublicSettings, forgotPassword placeholder from '@/api/auth'
