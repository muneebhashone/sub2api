      ...
      <transition name="fade">
        ...
        >
              ...
              <div class="space-y-3">
                <div class="space-y-1">
                  ...
                  <p class="text-xs text-gray-500 dark:text-dark-400">
                    {{
                      hasCurrentAuthToken
                        ? t('auth.oauthFlow.bindCurrentAccountDescription', { providerName placeholder)
                        : t('auth.oauthFlow.signInThenBindDescription', { providerName placeholder)
                    placeholderplaceholder
                  </p>
                </div>
                ...
                >
                  {{ hasCurrentAuthToken ? t('auth.oauthFlow.bindCurrentAccount') : t('auth.signIn') placeholderplaceholder
                </button>
              </div>
            </div>
          </template>
            ...
            >
              <div class="space-y-4">
                <div class="space-y-1">
                  ...
                  <p class="text-xs text-gray-500 dark:text-dark-400">
                    {{ t('auth.oauthFlow.chooseAccountActionHint') placeholderplaceholder
                  </p>
                </div>
          ...
          <template v-else-if="needsBindLogin">
            <p class="text-sm text-gray-700 dark:text-gray-300">
              {{ t('auth.oauthFlow.bindSignInToExistingAccount', { providerName placeholder) placeholderplaceholder
            </p>
            <div
              v-if="hasCurrentAuthToken"
              class="rounded-xl border border-gray-200 bg-gray-50 p-4 dark:border-dark-600 dark:bg-dark-800/60"
            >
              <div class="space-y-3">
                <div class="space-y-1">
                  <p class="text-sm font-medium text-gray-900 dark:text-white">
                    {{ t('auth.oauthFlow.bindCurrentAccountTitle') placeholderplaceholder
                  </p>
                  <p class="text-xs text-gray-500 dark:text-dark-400">
                    {{ t('auth.oauthFlow.bindCurrentAccountDescription', { providerName placeholder) placeholderplaceholder
                  </p>
                </div>
                ...
                >
                  {{ isSubmitting ? t('common.processing') : t('auth.oauthFlow.bindCurrentAccount') placeholderplaceholder
                </button>
              </div>
            </div>
...
function resolveWeChatStartURL(intent: 'bind_current_user' | 'adopt_existing_user_by_email'): string | null {
  ...
  return `${normalizedplaceholder/auth/oauth/wechat/bind/start?${params.toString()placeholder`
placeholder
