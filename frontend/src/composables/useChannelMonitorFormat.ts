...
import type { CheckMode, MonitorStatus, Provider placeholder from '@/api/admin/channelMonitor'
import {
  PROVIDER_OPENAI,
  PROVIDER_ANTHROPIC,
  PROVIDER_GEMINI,
...
export function useChannelMonitorFormat() {
  ...
  function statusLabel(s: MonitorStatus | ''): string {
    ...
    return t(`monitorCommon.status.${splaceholder`)
  placeholder
  ...
  function providerLabel(p: Provider | string): string {
    if (PROVIDERS.includes(p as Provider)) {
      return t(`monitorCommon.providers.${pplaceholder`)
    placeholder
    return p || '-'
  placeholder
  ...
  function checkModeLabel(m: CheckMode | string): string {
    if (m === 'probe' || m === 'quota' || m === 'quota_probe') {
      return t(`monitorCommon.checkMode.${mplaceholder`)
    placeholder
    return m || '-'
  placeholder
  ...
  function formatPercent(v: number | null | undefined): string {
    ...
    return `${v.toFixed(2)placeholder%`
  placeholder
  ...
  function formatRelativeTime(iso: string | null | undefined): string {
    ...
    if (diffSec < 60) return t('monitorCommon.relativeSecondsAgo', { n: diffSec placeholder)
    const diffMin = Math.floor(diffSec / 60)
    if (diffMin < 60) return t('monitorCommon.relativeMinutesAgo', { n: diffMin placeholder)
    const diffHour = Math.floor(diffMin / 60)
    if (diffHour < 24) return t('monitorCommon.relativeHoursAgo', { n: diffHour placeholder)
    const diffDay = Math.floor(diffHour / 24)
    return t('monitorCommon.relativeDaysAgo', { n: diffDay placeholder)
  placeholder
...
export function hslForPct(pct: number | null | undefined): string | undefined {
  ...
  return `hsl(${hueplaceholder ${HSL_SATURATIONplaceholder% ${HSL_LIGHTNESSplaceholder%)`
placeholder
