...
import { buildModelMappingObject, getModelsByPlatform, splitModelMappingObject placeholder from '../useModelWhitelist'
...
describe('useModelWhitelist', () => {
  ...
  it('combined 模式支持 Grok 4.5 官方别名映射', () => {
    const mapping = buildModelMappingObject(
      ...
      ['grok-4.5'],
      [
        { from: 'grok-latest', to: 'grok-4.5' placeholder,
        { from: 'grok-4.5-latest', to: 'grok-4.5' placeholder,
        { from: 'grok-build-latest', to: 'grok-4.5' placeholder
      ]
    )
  ...
  it('combined 模式会同时保留白名单身份映射和模型映射', () => {
    const mapping = buildModelMappingObject(
      ...
      ['gpt-5.4', 'claude-*'],
      [
        { from: 'gpt-latest', to: 'gpt-5.4' placeholder,
        { from: 'gpt-5.4', to: 'gpt-5.4-mini' placeholder
      ]
    )
  ...
  it('splitModelMappingObject 会把身份映射还原成白名单，其余保留为映射', () => {
    ...
    expect(parsed).toEqual({
      ...
      modelMappings: [{ from: 'gpt-latest', to: 'gpt-5.4' placeholder]
    placeholder)
  placeholder)
placeholder)
