 ...
 */
import { ref, readonly placeholder from 'vue'
import type { RouteLocationNormalized, RouteRecordRaw placeholder from 'vue-router'
...
export function generatePrefetchMap(routes: RouteRecordRaw[]): PrefetchConfig {
  const prefetchMap: PrefetchConfig = {placeholder
  ...
  const triggerPrefetch = (route: RouteLocationNormalized): void => {
    ...
    pendingPrefetchHandle.value = scheduleIdleCallback(
      () => {
        ...
        placeholder)
      placeholder,
      { timeout: 2000 placeholder // 2 秒超时
    )
  placeholder
