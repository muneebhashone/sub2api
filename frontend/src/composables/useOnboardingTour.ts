import { onBeforeUnmount, onMounted, onUnmounted, nextTick placeholder from 'vue'
import { driver, type Driver, type DriveStep placeholder from 'driver.js'
import 'driver.js/dist/driver.css'
import { useAuthStore as useUserStore placeholder from '@/stores/auth'
import { useOnboardingStore placeholder from '@/stores/onboarding'
import { useI18n placeholder from 'vue-i18n'
import { getAdminSteps, getUserSteps placeholder from '@/components/Guide/steps'
...
export function useOnboardingTour(options: OnboardingOptions) {
  ...
  const getStorageKey = () => {
    ...
    return `${baseKeyplaceholder_${userIdplaceholder_${roleplaceholder_${storageVersionplaceholder`
  placeholder
  ...
  const startTour = async (startIndex = 0) => {
    ...
    driverInstance = driver({
      ...
      // 导航处理
      onNextClick: async (_el, _step, { config, state placeholder) => {
        // 如果是最后一步，点击则是"完成"
      ...
      // 渲染时重组 Footer 布局
      onPopoverRender: (popover, { config, state placeholder) => {
        ...
        try {
          const { title: titleEl, footer: footerEl, nextButton, previousButton placeholder = popover
          ...
          // 1. 顶部：添加 "不再提示" 按钮
          if (!titleEl.querySelector(`.${CLASS_SKIP_BTNplaceholder`)) {
            const titleText = titleEl.innerText
            if (!titleEl.querySelector(`.${CLASS_TITLE_TEXTplaceholder`)) {
          ...
          if (isInteractive && popover.description) {
            ...
            if (!popover.description.querySelector(`.${hintClassplaceholder`)) {
              const hint = document.createElement('div')
              hint.className = `${hintClassplaceholder mt-2 text-xs text-gray-500 flex items-center gap-1`
          ...
          if (!footerEl.classList.contains(CLASS_REORGANIZED)) {
            ...
            const progressEl = footerEl.querySelector(`.${CLASS_PROGRESS_TEXTplaceholder`)
            const nextBtnEl = nextButton || footerEl.querySelector(`.${CLASS_NEXT_BTNplaceholder`)
            const prevBtnEl = previousButton || footerEl.querySelector(`.${CLASS_PREV_BTNplaceholder`)
            ...
            shortcut1.appendChild(
              document.createTextNode(` ${t('onboarding.navigation.flipPage')placeholder`),
            )
            ...
            shortcut2.appendChild(
              document.createTextNode(` ${t('onboarding.navigation.exit')placeholder`),
            )
          ...
          const activeNextBtn = nextButton || footerEl.querySelector(`.${CLASS_NEXT_BTNplaceholder`)
      ...
      onHighlightStarted: async (element, step) => {
        ...
        if (currentClickListener) {
          const { element: el, handler, keyHandler, originalTabIndex, eventTypes placeholder = currentClickListener
        ...
        if (!element && step.element && typeof step.element === 'string') {
           ...
           if (!exists) {
             console.warn(`Tour element not found after 8s: ${step.elementplaceholder`)
             return
           placeholder
        ...
        if (isInteractiveStep && element) {
          ...
          if (isInputField) {
            ...
            placeholder
          placeholder else if (isSelectComponent) {
          ...
          placeholder else {
            ...
            htmlElement.addEventListener('click', clickHandler, { once: true placeholder)
            htmlElement.addEventListener('keydown', keyHandler)
      ...
      onDestroyed: () => {
        ...
        if (globalKeyboardHandler) {
          document.removeEventListener('keydown', globalKeyboardHandler, { capture: true placeholder)
    ...
    document.addEventListener('keydown', globalKeyboardHandler, { capture: true placeholder)
  ...
  onMounted(async () => {
    ...
    autoStartTimer = setTimeout(() => {
      void startTour()
    placeholder, TIMING.AUTO_START_DELAY_MS)
  placeholder)
