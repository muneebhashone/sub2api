export default {
  ...
  subscriptionProgress: {
    title: '我的订阅',
    viewDetails: '查看订阅详情',
    activeCount: '{countplaceholder 个有效订阅',
    daily: '每日',
    weekly: '每周',
    monthly: '每月',
  ...
  version: {
    ...
    rollbackConfirm: '回退到 {versionplaceholder',
  ...
  announcements: {
    ...
    newCount: '有 {countplaceholder 条新公告',
    readAt: '已读时间',
    read: '已读',
    unread: '未读',
    startsAt: '开始时间',
  ...
  payment: {
    ...
    placeholder,
    currentBalance: '当前余额',
    groupFallback: '分组 #{idplaceholder',
    ...
    amountTooLow: '最低金额为 {minplaceholder',
    amountTooHigh: '最高金额为 {maxplaceholder',
    amountNoMethod: '该金额没有可用的支付方式',
    rechargeRatePreview: '当前倍率：1 CNY = {usdplaceholder USD',
    refundReason: '退款原因',
    ...
    errors: {
      tooManyPending: '待支付订单过多（最多 {maxplaceholder 个），请先完成或取消现有订单',
      ...
      TOO_MANY_PENDING: '待支付订单过多（最多 {maxplaceholder 个），请先完成或取消现有订单',
      DAILY_LIMIT_EXCEEDED: '今日充值已达上限，剩余额度 {remainingplaceholder',
      PAYMENT_GATEWAY_ERROR: '支付方式不可用',
      NO_AVAILABLE_INSTANCE: '暂无可用的支付通道',
      PAYMENT_PROVIDER_MISCONFIGURED: '支付通道配置错误，请联系管理员',
      WXPAY_CONFIG_MISSING_KEY: '微信支付配置缺少必填项：{keyplaceholder',
      WXPAY_CONFIG_INVALID_KEY_LENGTH: '微信支付 {keyplaceholder 长度错误，应为 {expectedplaceholder 字节（实际 {actualplaceholder）',
      WXPAY_CONFIG_INVALID_KEY: '微信支付 {keyplaceholder 格式错误，请确认复制了完整的 PEM 内容',
    ...
    admin: {
      ...
      subscriptionCnyPayPreview: 'CNY 通道实扣预览：{amountplaceholder',
      subscriptionCnyPayPreviewWithFee: '（含 {feeRateplaceholder% 手续费：{totalplaceholder）',
      validity: '有效期',
      validityUnit: '有效期单位',
      sortOrder: '排序',
      forSale: '上架状态',
