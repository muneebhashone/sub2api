export default {
    settings: {
      ...
      features: {
        ...
        affiliate: {
          ...
          customUsers: {
            ...
            batchButton: '批量设置比例（已选 {countplaceholder）',
            empty: '暂无专属配置用户',
            customBadge: '自定义',
            useGlobal: '沿用全局',
            resetTitle: '重置该用户的专属配置',
            resetMessage: '确认将 {emailplaceholder 的专属配置全部重置为默认？\n• 专属返利比例将清除（沿用全局）\n• 邀请码将重新生成为系统随机码（已分发的旧邀请链接将失效）',
            totalLabel: '共 {totalplaceholder 条',
            col: {
              email: '邮箱',
              username: '用户名',
              code: '邀请码',
          ...
          placeholder,
          batchModal: {
            title: '批量设置专属比例（已选 {countplaceholder 个用户）',
      ...
      registration: {
        ...
        emailSuffixWhitelistHint:
          "白名单域名的邮箱可无限注册；白名单非空时，其他可注册主域名各限注册一个账户。清空白名单后，所有域名均不限制注册数量（例如 {'@'placeholderqq.com, {'@'placeholdergmail.com, *.edu.cn）",
      ...
      apiKeyAcl: {
        ...
        forwardedClientIpHeadersLimit: '自定义客户端 IP 请求头最多允许 {maxplaceholder 个。',
        removeForwardedClientIpHeader: '移除 {headerplaceholder'
      placeholder,
      linuxdo: {
      ...
      defaults: {
        ...
        defaultSubscriptionsDuplicate: '默认订阅存在重复分组：{groupIdplaceholder。每个分组只能出现一次。',
      ...
      gatewayForwarding: {
        ...
        claudeOAuthSystemPromptBlocksHint: "每个 block 会保存为带 enabled、type、text、可选 cache_control 的 JSON。{'{'placeholderbilling_header{'placeholder'placeholder 会按请求动态生成；Claude Code 身份提示词和扩展提示词可直接编辑，也可用预设恢复默认值。",
        ...
        openaiCodexVersionSyncedValue: '当前同步到：{versionplaceholder',
      ...
      site: {
        ...
        tableDefaultPageSizeRangeError: '默认每页条数必须在 {minplaceholder-{maxplaceholder 之间',
        tablePageSizeOptionsFormatError: '可选每页条数格式无效，请输入 {minplaceholder-{maxplaceholder 之间的整数并用英文逗号分隔',
        customEndpoints: {
          title: '自定义端点',
          description: '添加额外的 API 端点地址，用户可在「API Keys」页面快速复制',
          itemLabel: '端点 #{nplaceholder',
          name: '名称',
        ...
        logoSizeError: '图片大小超过 300KB 限制（{sizeplaceholderKB）',
      ...
      payment: {
        ...
        balanceRechargePreview: '预览：1 CNY = {usdplaceholder USD',
        ...
        rechargeFeePreview: '预览：充值 100 元，手续费 {feeplaceholder 元',
        orderTimeout: '订单超时时间',
        ...
        validationFieldRequired: '{fieldplaceholder 不能为空',
        ...
        stripeWebhookApiVersionHint: 'Webhook 端点的 API 版本请与当前集成的 Stripe SDK 对齐，建议选择 {versionplaceholder；版本不一致可能导致回调事件解析失败。',
        ...
        enableConflict: '{methodplaceholder 已有启用中的服务商实例：{providerplaceholder。请先停用现有实例后再启用或切换。',
      placeholder,
      balanceNotify: {
        title: '余额不足提醒',
      ...
      soraS3: {
        ...
        deleteConfirm: '确定删除存储配置 {profileIDplaceholder 吗？',
        columns: {
          profile: '配置',
      ...
      openaiFastPolicy: {
        ...
        ruleHeader: '规则 #{indexplaceholder',
        removeRule: '删除规则',
        addRule: '新增规则',
      ...
      paymentVisibleMethods: {
        methodLabel: '{titleplaceholder 可见方式',
        methodHint: '控制前台结算页是否展示该方式，以及展示时使用的来源键。',
        sourceLabel: '支付来源',
        sourceHint: '启用后必须明确选择一个来源；未配置状态不会对外展示该支付方式。',
        sourceRequiredError: '{titleplaceholder 已启用，请先选择支付来源。'
      placeholder,
      openaiExperimentalScheduler: {
        ...
        defaultPlaceholder: '配置/默认：{valueplaceholder',
        topKLabel: 'TopK',
        priorityWeight: '优先级',
        loadWeight: '负载',
        queueWeight: '排队',
    ...
    errorPassthrough: {
      ...
      ruleUpdated: '规则更新成功',
      ruleDeleted: '规则删除成功',
      deleteConfirm: '确定要删除规则 "{nameplaceholder" 吗？',
      failedToLoad: '加载规则失败',
      failedToSave: '保存规则失败',
    ...
    tlsFingerprintProfiles: {
      ...
      deleteConfirm: '删除模板',
      deleteConfirmMessage: '确定要删除模板 "{nameplaceholder" 吗？使用此模板的账号将回退到内置默认值。',
