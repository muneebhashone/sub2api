export default {
  promptAudit: {
    ...
    configVersion: 'Config version v{versionplaceholder',
    tabs: { config: 'Configuration', events: 'Events' placeholder,
    actions: { refresh: 'Refresh runtime', retry: 'Retry', Allow: 'Allow', Warn: 'Warn', Block: 'Block' placeholder,
    common: { actions: 'Actions', never: 'Never' placeholder,
    mode: { off: 'Off', async_audit: 'Async audit only', blocking: 'Synchronous audit and block' placeholder,
    status: { disabled: 'Disabled', running: 'Running', degraded: 'Degraded', error: 'Error', healthy: 'Healthy', failed: 'Failed', stale: 'Stale heartbeat' placeholder,
    decisions: { pass: 'Pass', flag: 'Flag', critical: 'Critical' placeholder,
    riskLevels: { low: 'Low', medium: 'Medium', high: 'High', critical: 'Critical' placeholder,
    scanners: {
      violent: 'Violent',
    ...
    runtime: {
      ...
      queueBreakdown: 'queued {queuedplaceholder · processing {processingplaceholder · retry {retryplaceholder · done {doneplaceholder · failed {failedplaceholder',
      deliveryTotals: 'Total enqueued {enqueuedplaceholder · dropped {droppedplaceholder · processed {processedplaceholder · failed {failedplaceholder',
    placeholder,
    metrics: { total: 'Total', allowed: 'Allowed', flagged: 'Flagged', blocked: 'Blocked', unavailable: 'Unavailable', timeouts: 'Timeouts', failovers: 'Failovers' placeholder,
    pool: {
      ...
      probeProgress: 'Config validated ✓ · request sent · awaiting service response…', probeResult: 'Config ✓ · request ✓ · HTTP {httpplaceholder · {statusplaceholder · {latencyplaceholder ms',
      ...
      toggleNode: 'Toggle node {nameplaceholder', deleteConfirm: 'Remove “{nameplaceholder” from the draft? It takes effect after saving.',
    placeholder,
    policy: {
      ...
      searchGroups: 'Search groups', noGroups: 'No matching groups', missingGroups: 'Configured IDs for groups that no longer exist', selectedCount: '{countplaceholder groups selected',
    ...
    placeholder,
    saveBar: { enabled: 'Enable prompt audit', blocking: 'Synchronous blocking', blockingLatestTurnOnly: 'Only latest input and prior output', storePass: 'Store safe events', dirty: 'Unsaved changes', synced: 'Configuration synced' placeholder,
    blockingConfirm: {
    ...
    events: {
      ...
      startAt: 'Start time', endAt: 'End time', deleteSelected: 'Delete selected ({countplaceholder)', deleteByFilter: 'Delete by filter',
      ...
      timePresets: { '1d': 'Older than 1 day', '7d': 'Older than 7 days', '30d': 'Older than 30 days', '90d': 'Older than 90 days', all: 'All time', custom: 'Custom range' placeholder,
      ...
      selectAll: 'Select all events on this page', selectEvent: 'Select event {idplaceholder', time: 'Time', identity: 'User / email / API Key', user: 'Username', email: 'User email', apiKey: 'API Key name', group: 'Group', route: 'Endpoint / model', result: 'Decision / risk', preview: 'Redacted preview', empty: 'No matching events.',
      ...
      detailTitle: 'Prompt audit event details', tabs: { summary: 'Audit summary', risks: 'Specific risks', technical: 'Technical details' placeholder,
      ...
      placeholder,
      deleteConfirmTitle: 'Delete audit events?', deleteConfirmMessage: 'This permanently deletes {countplaceholder events and eligible orphan jobs.', filterDeleteCount: 'The server snapshot matches {countplaceholder events.', snapshotMax: 'Snapshot maximum event ID', expiresAt: 'Confirmation token expires', filterDeleteWarning: 'Only events at or below the preview high-water mark are deleted. Newer events survive. Any filter change requires a new preview.', confirmFilterDelete: 'Permanently delete',
    placeholder,
    messages: { saved: 'Prompt Audit configuration saved; plaintext API Key state was cleared.', probeSucceeded: 'The audit node is reachable.', deleted: 'Deleted {countplaceholder audit events.' placeholder,
    errors: {
