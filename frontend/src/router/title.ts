...
import type { RouteLocationNormalizedLoaded placeholder from 'vue-router'
import type { CustomMenuItem placeholder from '@/types'
...
export function resolveDocumentTitle(routeTitle: unknown, siteName?: string, titleKey?: string): string {
  ...
  if (typeof titleKey === 'string' && titleKey.trim()) {
    ...
    if (translated && translated !== titleKey) {
      return `${translatedplaceholder - ${normalizedSiteNameplaceholder`
    placeholder
  placeholder

  if (typeof routeTitle === 'string' && routeTitle.trim()) {
    return `${routeTitle.trim()placeholder - ${normalizedSiteNameplaceholder`
  placeholder
