...
export function proxyExpiryLabelKey(
  ...
  status?: string,
): { key: string; params?: { days: number placeholder placeholder {
  if (status === 'expired') return { key: 'admin.proxies.expired' placeholder
  const d = expiresAt ? daysUntil(expiresAt) : Infinity
  if (d < 0) return { key: 'admin.proxies.overdueDays', params: { days: Math.abs(d) placeholder placeholder
  if (d <= EXPIRY_WARN_DAYS) return { key: 'admin.proxies.expiringInDays', params: { days: d placeholder placeholder
  return { key: 'admin.proxies.remainingDays', params: { days: d placeholder placeholder
placeholder
