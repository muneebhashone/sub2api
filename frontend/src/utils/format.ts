...
import { i18n, getLocale placeholder from '@/i18n'
...
export function formatRelativeTime(date: string | Date | null | undefined): string {
  ...
  if (diffDays > 0) return i18n.global.t('common.time.daysAgo', { n: diffDays placeholder)
  if (diffHours > 0) return i18n.global.t('common.time.hoursAgo', { n: diffHours placeholder)
  if (diffMins > 0) return i18n.global.t('common.time.minutesAgo', { n: diffMins placeholder)
...
export function formatDateTimeLocalInput(timestampSeconds: number | null): string {
  ...
  return `${yearplaceholder-${monthplaceholder-${dayplaceholderT${hoursplaceholder:${minutesplaceholder`
placeholder
...
export function formatTokensK(tokens: number): string {
  if (tokens >= 1_000_000) return `${(tokens / 1_000_000).toFixed(1)placeholderM`
...
export function formatCompactNumber(
  ...
  options?: { allowBillions?: boolean placeholder
): string {
  if (num === null || num === undefined) return '0'
  ...
  if (allowBillions && abs >= 1_000_000_000) return `${(num / 1_000_000_000).toFixed(1)placeholderB`
  if (abs >= 1_000_000) return `${(num / 1_000_000).toFixed(1)placeholderM`
  if (abs >= 1_000) return `${(num / 1_000).toFixed(1)placeholderK`
  return num.toString()
placeholder
...
export function formatCountdown(targetDate: string | Date | null | undefined): string | null {
  ...
  if (diffDays > 0) {
    // 超过1天：显示 "Xd Yh"
    return i18n.global.t('common.time.countdown.daysHours', { d: diffDays, h: remainingHours placeholder)
  placeholder
  if (diffHours > 0) {
    // 小于1天：显示 "Xh Ym"
    return i18n.global.t('common.time.countdown.hoursMinutes', { h: diffHours, m: remainingMins placeholder)
  placeholder
  // 小于1小时：显示 "Ym"
  return i18n.global.t('common.time.countdown.minutes', { m: diffMins placeholder)
placeholder
...
export function formatCountdownWithSuffix(targetDate: string | Date | null | undefined): string | null {
  ...
  return i18n.global.t('common.time.countdown.withSuffix', { time: countdown placeholder)
placeholder
...
export function formatRelativeWithDateTime(date: string | Date | null | undefined): string {
  ...
  return `${relativeTimeplaceholder · ${dateTimeplaceholder`
placeholder
