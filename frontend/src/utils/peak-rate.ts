...
export function serverTimezoneLabel(utcOffset?: string | null): string {
  return utcOffset ? `UTC${utcOffsetplaceholder` : ''
placeholder
...
export function formatPeakRateWindow(
  ...
  const base = `${fields.peak_startplaceholder-${fields.peak_endplaceholder ×${fields.peak_rate_multiplier ?? 1placeholder`
  return tzLabel ? `${baseplaceholder (${tzLabelplaceholder)` : base
placeholder
