...
import { enqueueUsageRequest placeholder from '../usageLoadQueue'
...
function makeAccount(
  platform: string,
  type: string = 'oauth',
  proxy?: { host: string; port: number; username?: string | null placeholder | null
): Account {
  return {
    ...
    proxy: proxy
      ? { id: 1, name: 'p', protocol: 'http', host: proxy.host, port: proxy.port, username: proxy.username ?? null, status: 'active', created_at: '', updated_at: '' placeholder
      : undefined,
    credentials: {placeholder,
    created_at: '',
    updated_at: ''
  placeholder as unknown as Account
placeholder
...
describe('usageLoadQueue', () => {
  ...
  it('Anthropic 同代理出口串行执行，间隔 >= 1s', async () => {
    ...
    const acc = makeAccount('anthropic', 'oauth', { host: '1.2.3.4', port: 8080, username: 'u1' placeholder)
  ...
  it('Anthropic 不同代理出口并行执行', async () => {
    const timestamps: Record<string, number> = {placeholder
    const makeTracked = (key: string) => async () => {
      timestamps[key] = Date.now()
      return key
    placeholder

    const acc1 = makeAccount('anthropic', 'oauth', { host: '1.2.3.4', port: 8080, username: 'u1' placeholder)
    const acc2 = makeAccount('anthropic', 'oauth', { host: '5.6.7.8', port: 3128, username: 'u2' placeholder)
  ...
  it('Anthropic 相同代理连接信息的不同账号归为同一队列', async () => {
    ...
    const acc1 = makeAccount('anthropic', 'oauth', { host: '10.0.0.1', port: 3128, username: 'admin' placeholder)
    const acc2 = makeAccount('anthropic', 'setup-token', { host: '10.0.0.1', port: 3128, username: 'admin' placeholder)
  ...
  it('Anthropic 请求失败时 reject，后续任务继续执行', async () => {
    ...
    const acc = makeAccount('anthropic', 'oauth', { host: '99.99.99.99', port: 1234 placeholder)
  ...
  it('非 Anthropic 平台直接执行，不排队', async () => {
    ...
    const acc1 = makeAccount('gemini', 'oauth', { host: '1.2.3.4', port: 8080 placeholder)
    const acc2 = makeAccount('gemini', 'oauth', { host: '1.2.3.4', port: 8080 placeholder)
  ...
  it('Antigravity 平台直接执行，不排队', async () => {
    ...
    const acc1 = makeAccount('antigravity', 'oauth', { host: '1.2.3.4', port: 8080 placeholder)
    const acc2 = makeAccount('antigravity', 'oauth', { host: '1.2.3.4', port: 8080 placeholder)
  ...
  it('OpenAI 平台直接执行，不排队', async () => {
    ...
    const acc1 = makeAccount('openai', 'oauth', { host: '1.2.3.4', port: 8080 placeholder)
    const acc2 = makeAccount('openai', 'oauth', { host: '1.2.3.4', port: 8080 placeholder)
  ...
  it('Anthropic apikey 类型直接执行，不排队', async () => {
    ...
    const acc1 = makeAccount('anthropic', 'apikey', { host: '1.2.3.4', port: 8080 placeholder)
    const acc2 = makeAccount('anthropic', 'apikey', { host: '1.2.3.4', port: 8080 placeholder)
  ...
  it('返回值正确透传', async () => {
    ...
    const result = await enqueueUsageRequest(acc, async () => {
      return { usage: 42 placeholder
    placeholder)
    expect(result).toEqual({ usage: 42 placeholder)
  placeholder)

  it('非 Anthropic 返回值正确透传', async () => {
    const acc = makeAccount('gemini', 'oauth')
    const result = await enqueueUsageRequest(acc, async () => {
      return { quota: 100 placeholder
    placeholder)
    expect(result).toEqual({ quota: 100 placeholder)
  placeholder)
placeholder)
