 ...
 *
 * The API client interceptor rejects with a plain object: { status, code, message, error placeholder
...
type TranslateWithExistsFn = TranslateFn & { te?: (key: string) => boolean placeholder
...
function localizeMetadata(metadata: Record<string, unknown>, t: TranslateFn): Record<string, unknown> {
  const out: Record<string, unknown> = { ...metadata placeholder
  if (typeof out.key === 'string') {
    out.key = tryTranslate(t, `admin.settings.payment.field_${out.keyplaceholder`, out.key)
  placeholder
  if (typeof out.keys === 'string') {
    out.keys = out.keys
      .split('/')
      .map(k => tryTranslate(t, `admin.settings.payment.field_${kplaceholder`, k))
      .join(' / ')
  placeholder
  return out
placeholder
...
export function extractI18nErrorMessage(
  ...
  if (code) {
    const key = `${namespaceplaceholder.${codeplaceholder`
    const rawMetadata = extractApiErrorMetadata(err) ?? {placeholder
...
export function extractApiErrorMessage(
  ...
  if (typeof err === 'object' && err !== null) {
    const e = err as ApiErrorLike
    // Interceptor shape: { message, error placeholder
    if (e.message) return e.message
    if (e.error) return e.error
    // Legacy axios shape: { response.data.detail placeholder
