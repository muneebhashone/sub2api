import type { CodexUsageSnapshot placeholder from '@/types'
...
function resolveLegacy5h(snapshot: Record<string, unknown>): { used: number | null; resetAfterSeconds: number | null placeholder {
  ...
  if (primaryWindow != null && primaryWindow <= 360) {
    return { used: primaryUsed, resetAfterSeconds: primaryReset placeholder
  placeholder
  if (secondaryWindow != null && secondaryWindow <= 360) {
    return { used: secondaryUsed, resetAfterSeconds: secondaryReset placeholder
  placeholder
  return { used: secondaryUsed, resetAfterSeconds: secondaryReset placeholder
placeholder

function resolveLegacy7d(snapshot: Record<string, unknown>): { used: number | null; resetAfterSeconds: number | null placeholder {
  const primaryWindow = asNumber(snapshot.codex_primary_window_minutes)
  const secondaryWindow = asNumber(snapshot.codex_secondary_window_minutes)
  const primaryUsed = asNumber(snapshot.codex_primary_used_percent)
  const secondaryUsed = asNumber(snapshot.codex_secondary_used_percent)
...
function applyExpiredRule(window: ResolvedCodexUsageWindow, now: Date): ResolvedCodexUsageWindow {
  ...
  if (resetDate.getTime() <= now.getTime()) {
    return { usedPercent: 0, resetAt: resetDate.toISOString() placeholder
  placeholder
  return window
placeholder
...
export function resolveCodexUsageWindow(
  ...
  if (!snapshot) {
    return { usedPercent: null, resetAt: null placeholder
  placeholder
  ...
  return applyExpiredRule({ usedPercent, resetAt placeholder, now)
placeholder
