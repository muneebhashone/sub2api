<template>
  <BaseDialog :show="show" :title="t('admin.promptAudit.events.filterDeleteDialogTitle')" width="wide" @close="$emit('close')">
    <div class="space-y-5 text-sm">
      <p class="text-gray-500 dark:text-dark-300">{{ t('admin.promptAudit.events.filterDeleteDialogDesc') placeholderplaceholder</p>

      <fieldset>
        <legend class="text-xs font-medium text-gray-600 dark:text-dark-200">{{ t('admin.promptAudit.events.filterTimeRange') placeholderplaceholder</legend>
        <div class="mt-2 flex flex-wrap gap-2" role="radiogroup" :aria-label="t('admin.promptAudit.events.filterTimeRange')">
          ...
          >
            <input v-model="preset" type="radio" name="prompt-delete-range" :value="option.id" class="peer sr-only" :data-test="`range-preset-${option.idplaceholder`" @change="criteriaChanged" />
            <span class="inline-flex items-center rounded-full border border-gray-200 px-3 py-1.5 text-xs font-medium text-gray-600 transition-colors peer-checked:border-red-500 peer-checked:bg-red-50 peer-checked:text-red-700 peer-focus-visible:ring-2 peer-focus-visible:ring-red-500/30 dark:border-dark-600 dark:text-dark-300 dark:peer-checked:border-red-500 dark:peer-checked:bg-red-950/40 dark:peer-checked:text-red-300">
              {{ t(`admin.promptAudit.events.timePresets.${option.idplaceholder`) placeholderplaceholder
            </span>
          </label>
        </div>
        <p class="mt-2 text-xs text-gray-500 dark:text-dark-400">{{ t('admin.promptAudit.events.filterTimeRangeHint') placeholderplaceholder</p>
        <div v-if="preset === 'custom'" class="mt-3 grid gap-3 sm:grid-cols-2" data-test="custom-range">
          <label class="text-xs text-gray-600 dark:text-dark-200">
            <span>{{ t('admin.promptAudit.events.startAt') placeholderplaceholder</span>
          ...
          <label class="text-xs text-gray-600 dark:text-dark-200">
            <span>{{ t('admin.promptAudit.events.endAt') placeholderplaceholder</span>
          ...
          <p v-if="!canPreview" class="text-xs text-red-600 dark:text-red-400 sm:col-span-2">{{ t('admin.promptAudit.events.customRangeInvalid') placeholderplaceholder</p>
        </div>
      </fieldset>

      <div class="grid gap-3 sm:grid-cols-2">
        <label class="text-xs text-gray-600 dark:text-dark-200">
          <span>{{ t('admin.promptAudit.events.decision') placeholderplaceholder</span>
          <select v-model="local.decision" class="input mt-1 w-full" :aria-label="t('admin.promptAudit.events.decision')" data-test="delete-decision" @change="criteriaChanged">
            <option value="">{{ t('common.all') placeholderplaceholder</option>
            <option value="pass">{{ t('admin.promptAudit.decisions.pass') placeholderplaceholder</option>
            <option value="flag">{{ t('admin.promptAudit.decisions.flag') placeholderplaceholder</option>
            <option value="critical">{{ t('admin.promptAudit.decisions.critical') placeholderplaceholder</option>
          </select>
        </label>
        <label class="text-xs text-gray-600 dark:text-dark-200">
          <span>{{ t('admin.promptAudit.events.risk') placeholderplaceholder</span>
          <select v-model="local.risk_level" class="input mt-1 w-full" :aria-label="t('admin.promptAudit.events.risk')" data-test="delete-risk" @change="criteriaChanged">
            <option value="">{{ t('common.all') placeholderplaceholder</option>
            <option value="low">{{ t('admin.promptAudit.riskLevels.low') placeholderplaceholder</option>
            <option value="medium">{{ t('admin.promptAudit.riskLevels.medium') placeholderplaceholder</option>
            <option value="high">{{ t('admin.promptAudit.riskLevels.high') placeholderplaceholder</option>
            <option value="critical">{{ t('admin.promptAudit.riskLevels.critical') placeholderplaceholder</option>
          </select>
        </label>
      </div>

      <details class="rounded-xl border border-gray-200 px-4 py-3 dark:border-dark-700/60" data-test="more-conditions">
        <summary class="cursor-pointer select-none text-xs font-medium text-gray-600 dark:text-dark-200">{{ t('admin.promptAudit.events.moreConditions') placeholderplaceholder</summary>
        <div class="mt-3 grid gap-3 sm:grid-cols-2">
          <label class="text-xs text-gray-600 dark:text-dark-200">
            <span>{{ t('admin.promptAudit.events.endpoint') placeholderplaceholder</span>
          ...
          <label class="text-xs text-gray-600 dark:text-dark-200">
            <span>{{ t('admin.promptAudit.events.keyword') placeholderplaceholder</span>
          ...
          <label class="text-xs text-gray-600 dark:text-dark-200">
            <span>{{ t('admin.promptAudit.events.groupId') placeholderplaceholder</span>
          ...
          <label class="text-xs text-gray-600 dark:text-dark-200">
            <span>{{ t('admin.promptAudit.events.userId') placeholderplaceholder</span>
      ...
      <div v-if="preview" class="rounded-xl border border-red-200 bg-red-50/60 px-4 py-3 dark:border-red-900/60 dark:bg-red-950/20" data-test="delete-preview-result">
        <p class="text-sm font-semibold text-red-700 dark:text-red-300">{{ t('admin.promptAudit.events.filterDeleteCount', { count: preview.matched_count placeholder) placeholderplaceholder</p>
        <dl class="mt-2 grid grid-cols-[auto_1fr] gap-x-3 gap-y-1 text-xs text-gray-600 dark:text-dark-300">
          <dt>{{ t('admin.promptAudit.events.snapshotMax') placeholderplaceholder</dt>
          <dd>{{ preview.snapshot_max_id placeholderplaceholder</dd>
          <dt>Filter SHA-256</dt>
          <dd class="break-all font-mono">{{ preview.filter_hash placeholderplaceholder</dd>
          <dt>{{ t('admin.promptAudit.events.expiresAt') placeholderplaceholder</dt>
          <dd>{{ formatDate(preview.expires_at) placeholderplaceholder</dd>
        </dl>
        <p class="mt-2 rounded-lg bg-amber-50 px-3 py-2 text-xs text-amber-800 dark:bg-amber-950/30 dark:text-amber-200">{{ t('admin.promptAudit.events.filterDeleteWarning') placeholderplaceholder</p>
      </div>
      <p v-else class="rounded-xl border border-dashed border-gray-300 px-4 py-3 text-xs text-gray-500 dark:border-dark-600 dark:text-dark-400" data-test="delete-preview-empty">
        {{ t('admin.promptAudit.events.filterDeleteNeedPreview') placeholderplaceholder
      </p>
    </div>
    ...
    <template #footer>
      <div class="flex flex-wrap items-center justify-end gap-3">
        <p v-if="confirmDisabledReason" class="mr-auto text-xs text-gray-500 dark:text-dark-400" data-test="confirm-disabled-reason">
          {{ t(confirmDisabledReason) placeholderplaceholder
        </p>
        <button type="button" class="btn btn-secondary" @click="$emit('close')">{{ t('common.cancel') placeholderplaceholder</button>
        <button type="button" class="btn btn-secondary" :disabled="!canPreview || previewing || deleting" data-test="run-delete-preview" @click="requestPreview">
          {{ previewing ? t('admin.promptAudit.events.filterDeletePreviewing') : t('admin.promptAudit.events.filterDeletePreviewAction') placeholderplaceholder
        </button>
        <button
          type="button"
        ...
        >
          {{ deleting ? t('common.submitting') : t('admin.promptAudit.events.confirmFilterDelete') placeholderplaceholder
        </button>
      </div>
    </template>
  </BaseDialog>
...
import type { PromptDeletePreview, PromptEventFilters placeholder from '../types'
import {
