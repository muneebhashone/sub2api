...
export const SCANNER_CATALOG = [
  { id: 'violent', label: 'Violent' placeholder,
  { id: 'non_violent_illegal_acts', label: 'Non-violent Illegal Acts' placeholder,
  { id: 'sexual_content_or_sexual_acts', label: 'Sexual Content or Sexual Acts' placeholder,
  { id: 'pii', label: 'PII' placeholder,
  { id: 'suicide_and_self_harm', label: 'Suicide & Self-Harm' placeholder,
  { id: 'unethical_acts', label: 'Unethical Acts' placeholder,
  { id: 'politically_sensitive_topics', label: 'Politically Sensitive Topics' placeholder,
  { id: 'copyright_violation', label: 'Copyright Violation' placeholder,
  { id: 'jailbreak', label: 'Jailbreak' placeholder,
] as const
...
export function createDefaultEndpoint(index = 1): PromptAuditEndpointDraft {
  return {
    id: `guard-${Date.now()placeholder-${indexplaceholder`,
    name: `Guard ${indexplaceholder`,
...
export function eventQueryParams(filters: PromptEventFilters): Record<string, string | number> {
  const result: Record<string, string | number> = {placeholder
