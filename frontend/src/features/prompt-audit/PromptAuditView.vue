      ...
      <template v-else>
        <div class="mb-4" role="tablist" :aria-label="t('admin.promptAudit.title')">
          <div class="tabs inline-flex">
            <button
              ...
              role="tab"
              class="tab"
              :class="{ 'tab-active': activeTab === tab.id placeholder"
              :aria-selected="activeTab === tab.id"
              :data-test="`tab-${tab.idplaceholder`"
        ...
        <main class="card px-4 sm:px-6 lg:px-8">
          ...
          <div v-show="activeTab === 'events'" data-test="tab-panel-events">
            ...
            >
              <span>{{ t('admin.promptAudit.events.passEventsDisabled') placeholderplaceholder</span>
              <button type="button" class="btn btn-secondary btn-sm" @click="activeTab = 'config'">
                {{ t('admin.promptAudit.events.openConfiguration') placeholderplaceholder
              </button>
            </div>
            <EventWorkspace
              :events="events.items"
    ...
    <div v-if="draft && activeTab === 'config'" class="fixed inset-x-0 bottom-0 z-30 border-t border-gray-200 bg-white/95 px-4 py-3 shadow-[0_-12px_35px_rgba(15,23,42,0.08)] backdrop-blur dark:border-dark-700/80 dark:bg-dark-900/95 dark:shadow-[0_-12px_35px_rgba(0,0,0,0.35)] lg:left-64">
      <div class="mx-auto flex max-w-[1600px] flex-wrap items-center justify-between gap-3">
        <div class="flex flex-wrap items-center gap-x-5 gap-y-2">
          ...
          <SaveToggle :label="t('admin.promptAudit.saveBar.blockingLatestTurnOnly')" :model-value="draft.blocking_latest_turn_only" :disabled="!draft.enabled || !draft.blocking_enabled" data-test="blocking-latest-turn-only-toggle" @update:model-value="replaceDraft({ ...draft!, blocking_latest_turn_only: $event placeholder)" />
...
const pageTabs = computed(() => [
  { id: 'events' as const, label: t('admin.promptAudit.tabs.events') placeholder,
  { id: 'config' as const, label: t('admin.promptAudit.tabs.config') placeholder,
])
...
const loading = reactive({ config: false, runtime: false, groups: false, events: false, saving: false, detail: false, deleting: false, previewing: false placeholder)
...
const SaveToggle = defineComponent({
  ...
  setup(props, { emit, attrs placeholder) {
    return () => h('label', { class: ['flex items-center gap-2.5 text-sm', props.disabled ? 'cursor-not-allowed opacity-50' : 'cursor-pointer'] placeholder, [
      h('button', {
        ...attrs,
        type: 'button',
        role: 'switch',
      ...
      h('span', { class: 'select-none text-gray-700 dark:text-dark-200' placeholder, props.label),
    ])
  placeholder,
placeholder)
...
async function runFilterDeletePreview(value: PromptEventFilters) {
  ...
  placeholder finally { loading.previewing = false placeholder
placeholder
