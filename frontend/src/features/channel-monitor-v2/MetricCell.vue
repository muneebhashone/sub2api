<template>
  ...
  >
    ...
    <div class="min-w-0 flex-1">
      <span class="stat-label text-[10px] font-bold uppercase tracking-wider text-gray-400">{{ label placeholderplaceholder</span>
      <strong
        class="stat-value mt-1 block overflow-visible text-xl tabular-nums leading-tight !text-clip !whitespace-normal"
        :class="stateClass"
      >{{ value placeholderplaceholder</strong>
      <div
      ...
      >
        <span
          ...
          :key="`${indexplaceholder:${partplaceholder`"
          class="whitespace-nowrap tabular-nums"
        >{{ part placeholderplaceholder</span>
      </div>
      <small
        v-else-if="detail"
        class="mt-1.5 block text-[11px] leading-snug text-gray-400 dark:text-dark-400"
      >{{ detail placeholderplaceholder</small>
    </div>
  </div>
</template>
...
import type { HealthState placeholder from '@/api/channelMonitorV2'
