...
import {
  ...
  healthStateClass,
  scoreToBand,
placeholder from '../monitorFormat'
import type { MonitorHealth placeholder from '@/api/channelMonitorV2'
...
describe('monitorFormat accuracy', () => {
  ...
  it('builds score-based health classes for matrix cells', () => {
    ...
    expect(healthScoreClass({ ...health, score: null placeholder, 'overall', 0)).toBe('health-unknown')
  placeholder)
