const i18nT = (key: string, params?: Record<string, unknown>) => {
  const map: Record<string, string> = {
    ...
    'channelMonitorV2.bucket.minutes': '{countplaceholder分钟',
    ...
    'channelMonitorV2.matrix.scoreLine': '评分 {scoreplaceholder',
    'channelMonitorV2.metrics.successRateValue': '成功率 {valueplaceholder',
    'channelMonitorV2.metrics.errorRateValue': '错误率 {valueplaceholder',
    'channelMonitorV2.metrics.rpmValue': 'RPM {valueplaceholder',
    'channelMonitorV2.metrics.tpmValue': 'TPM {valueplaceholder',
    'channelMonitorV2.metrics.tpsValue': '每秒 Token {valueplaceholder',
    'channelMonitorV2.metrics.ttftValue': '首 Token {valueplaceholder',
    'channelMonitorV2.metrics.durationValue': '时长 {valueplaceholder',
    'channelMonitorV2.metrics.cacheRateValue': '缓存率 {valueplaceholder',
    'channelMonitorV2.matrix.noTrafficAt': '{timeplaceholder 无流量',
  ...
  return template.replace(/\{(\w+)\placeholder/g, (_, name) => String(params?.[name] ?? ''))
placeholder
...
import type { MonitorHealth, MonitorMetric placeholder from '@/api/channelMonitorV2'
...
function metrics(requestCount: number): MonitorMetric {
  return {
    ...
    ttft: { sample_count: requestCount, p50_ms: 100, p95_ms: 300, avg_ms: 150 placeholder,
    duration: { sample_count: requestCount, p50_ms: 500, p95_ms: 900, avg_ms: 600 placeholder,
    ...
    const wrapper = mount(RelayPulseMatrix, {
      props: {
        rows: [{
          ...
          health,
          buckets: [
            { bucket_start: '2026-08-01T00:00:00Z', metrics: metrics(10), health placeholder,
            { bucket_start: '2026-08-01T00:01:00Z', metrics: metrics(0), health placeholder,
          ],
        placeholder],
        coverage: {
    ...
    const wrapper = mount(RelayPulseMatrix, {
      props: {
        rows: [{
          ...
          health,
          buckets: [
            { bucket_start: '2026-08-01T00:02:00Z', metrics: metrics(10), health placeholder,
          ],
        placeholder],
        coverage: {
          requested_start: '2026-08-01T00:00:00Z',
