<template>
  ...
  >
    ...
    >
      ...
      >
        {{ t('channelMonitorV2.filters.labelValue', { label, value: selectionLabel placeholder) placeholderplaceholder
      </span>
      ...
      <Transition name="select-dropdown">
        ...
        >
          <button
            ...
            @click="clear"
          >
            <span>{{ allLabel placeholderplaceholder</span>
          ...
          >
            <span class="flex min-w-0 flex-1 items-center gap-2">
              ...
              <span class="min-w-0 flex-1 truncate">{{ option.label placeholderplaceholder</span>
            </span>
            <small v-if="option.count != null" class="text-xs text-gray-400">{{ formatCount(option.count) placeholderplaceholder</small>
          </button>
          <p v-if="options.length === 0" class="px-4 py-3 text-center text-xs text-gray-400">{{ t('channelMonitorV2.filters.empty') placeholderplaceholder</p>
        </div>
      </Transition>
    </Teleport>
  </div>
...
import type { CSSProperties placeholder from 'vue'
import { useI18n placeholder from 'vue-i18n'
import { computed, nextTick, onBeforeUnmount, ref, watch placeholder from 'vue'
import Icon from '@/components/icons/Icon.vue'
import { monitorIntlLocale placeholder from '@/features/channel-monitor-v2/monitorFormat'
...
const emit = defineEmits<{ 'update:modelValue': [value: string[]] placeholder>()
...
const instanceId = `filter-select-${Math.random().toString(36).slice(2, 9)placeholder`
...
const selectionLabel = computed(() => {
  ...
  placeholder
  return t('channelMonitorV2.filters.selectedCount', { count: props.modelValue.length placeholder)
placeholder)

const dropdownStyle = computed<CSSProperties>(() => {
  const trigger = triggerRef.value
  if (!trigger) return {placeholder
  ...
  return {
    ...
    top: `${rect.bottom + 4placeholderpx`,
    minWidth: `${minWidthplaceholderpx`,
    maxWidth: `${availableWidthplaceholderpx`,
    zIndex: '100000020',
  placeholder
placeholder)
...
function positionDropdown() {
  ...
  const preferredMinWidth = Math.max(200, rect.width)
  const minWidth = Math.min(preferredMinWidth, availableWidth)
  dropdown.style.left = `${leftplaceholderpx`
  dropdown.style.top = `${rect.bottom + 4placeholderpx`
  dropdown.style.minWidth = `${minWidthplaceholderpx`
  dropdown.style.maxWidth = `${availableWidthplaceholderpx`
placeholder
