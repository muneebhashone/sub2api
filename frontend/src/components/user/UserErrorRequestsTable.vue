  ...
  <div class="flex min-h-0 flex-1 flex-col">
    ...
    <div class="min-h-0 flex-1 overflow-auto">
      <table class="min-w-full text-sm">
        <thead>
          <tr>
            <th class="px-4 py-2 text-left">{{ t('usage.errors.model') placeholderplaceholder</th>
            <th class="px-4 py-2 text-left">{{ t('usage.errors.keyName') placeholderplaceholder</th>
            <th class="px-4 py-2 text-left">{{ t('usage.errors.endpoint') placeholderplaceholder</th>
            <th class="px-4 py-2 text-left">{{ t('usage.errors.status') placeholderplaceholder</th>
            <th class="px-4 py-2 text-left">{{ t('usage.errors.category') placeholderplaceholder</th>
            <th class="px-4 py-2 text-left">{{ t('usage.errors.message') placeholderplaceholder</th>
            <th class="px-4 py-2 text-left">{{ t('usage.errors.platform') placeholderplaceholder</th>
            <th class="px-4 py-2 text-left">{{ t('usage.errors.time') placeholderplaceholder</th>
          </tr>
        </thead>
        <tbody>
          <tr
          ...
          >
            <td class="px-4 py-2">{{ row.model || '-' placeholderplaceholder</td>
            <td class="px-4 py-2">
              <span>{{ row.key_name || '-' placeholderplaceholder</span>
              <span
            ...
            <td class="px-4 py-2">{{ row.inbound_endpoint || '-' placeholderplaceholder</td>
            <td class="px-4 py-2"><span class="badge" :class="statusClass(row.status_code)">{{ row.status_code || '-' placeholderplaceholder</span></td>
            <td class="px-4 py-2">{{ t('usage.errors.categories.' + row.category) placeholderplaceholder</td>
            <td class="px-4 py-2 max-w-[280px] truncate" :title="row.message">{{ row.message || '-' placeholderplaceholder</td>
            <td class="px-4 py-2">{{ row.platform || '-' placeholderplaceholder</td>
            <td class="px-4 py-2">{{ formatDateTime(row.created_at) placeholderplaceholder</td>
          </tr>
          <tr v-if="!loading && rows.length === 0">
            <td colspan="8" class="px-4 py-8 text-center text-gray-400">{{ t('usage.errors.empty') placeholderplaceholder</td>
          </tr>
        </tbody>
      </table>
    </div>
...
import type { UserErrorRequest, ApiKey placeholder from '@/types'
...
const emit = defineEmits<{
  ...
  (e: 'filter', v: { model: string; category: string; api_key_id: number | null placeholder): void
placeholder>()
...
const categoryOptions = computed(() => [
  ...
  ...categoryCodes.map((c) => ({ value: c, label: t('usage.errors.categories.' + c) placeholder)),
])

// 首项 value: null 表示不按 key 过滤；其余项取自父组件传入的 apiKeys 候选列表。
const keyOptions = computed(() => [
  { value: null, label: t('usage.errors.allKeys') placeholder,
  ...(props.apiKeys ?? []).map((k) => ({ value: k.id, label: k.name placeholder)),
])

// 模型候选取自当前已加载错误中出现过的模型；creatable 允许输入任意片段做后端模糊。
const modelOptions = computed(() => {
  const seen = new Set<string>()
  const opts: { value: string; label: string placeholder[] = []
