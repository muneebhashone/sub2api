    ...
    <div class="flex items-start gap-3">
      ...
      <div class="flex-1 min-w-0">
        <div class="text-base font-semibold truncate text-gray-900 dark:text-gray-100">
          {{ item.name placeholderplaceholder
        </div>
        <div class="mt-0.5 flex items-center gap-1.5 min-w-0">
          ...
          >
            {{ providerLabel(item.provider) placeholderplaceholder
          </span>
          <!-- 纯配额模式主模型是占位符 "quota"，展示层替换为本地化「配额」标签 -->
          <span class="font-mono text-xs truncate text-gray-500 dark:text-gray-400">
            {{ formatMonitorModel(item.primary_model) placeholderplaceholder
          </span>
          <span
          ...
          >
            {{ item.group_name placeholderplaceholder
          </span>
        </div>
      </div>
      <span
      ...
      >
        {{ statusLabel(item.primary_status) placeholderplaceholder
      </span>
    </div>
...
import {
  ...
  providerGradient,
placeholder from '@/composables/useChannelMonitorFormat'
import { isChannelMonitorQuotaVisible placeholder from '@/utils/featureFlags'
...
const availabilityLabel = computed(() => {
  const win = t(`channelStatus.windowTab.${props.windowplaceholder`)
  return `${t('monitorCommon.availabilityPrefix')placeholder · ${winplaceholder`
placeholder)

const extraModelsCountLabel = computed(() => {
  const count = props.item.extra_models?.length ?? 0
  if (count === 0) return undefined
  return t('monitorCommon.extraModelsCount', { n: count placeholder)
placeholder)
</script>
