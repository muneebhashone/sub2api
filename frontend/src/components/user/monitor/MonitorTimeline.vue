<template>
  <div class="mt-4 pt-3 border-t border-gray-100 dark:border-dark-700/60">
    ...
    >
      <span>{{ t('monitorCommon.history60pts', { n: length placeholder) placeholderplaceholder</span>
      <span class="tabular-nums">{{ t('monitorCommon.nextUpdateIn', { n: countdownSeconds placeholder) placeholderplaceholder</span>
    </div>
    ...
    >
      {{ t('monitorCommon.maintenancePaused') placeholderplaceholder
    </div>
    <div v-else class="flex items-end gap-[2px] h-5 w-full">
      <div
        ...
        :style="{ height: bar.heightPct + '%' placeholder"
        :title="bar.title"
      ></div>
    </div>
    ...
    >
      <span>{{ t('monitorCommon.past') placeholderplaceholder</span>
      <span>{{ t('monitorCommon.now') placeholderplaceholder</span>
    </div>
  </div>
</template>
...
import type { MonitorTimelinePoint placeholder from '@/api/channelMonitor'
...
const { statusLabel, formatLatency, formatRelativeTime placeholder = useChannelMonitorFormat()
...
const displayBars = computed<Bar[]>(() => {
  ...
  for (const point of real) {
    ...
    bars.push({
      colorClass,
      heightPct,
      title: `${relativeplaceholder · ${labelplaceholder · ${latencyplaceholderms`,
    placeholder)
  placeholder
