<template>
  <section class="pt-8 pb-10 md:pb-14">
    <div class="text-xs font-medium tracking-widest uppercase text-gray-400 dark:text-gray-500 mb-4">
      {{ t('channelStatus.hero.breadcrumb') placeholderplaceholder
    </div>
      ...
      <div class="flex flex-col items-start md:items-end gap-2.5">
        ...
        <div class="flex items-center gap-2">
          ...
          >
            ...
            ></span>
            {{ overallLabel placeholderplaceholder
          </span>
          <button
            type="button"
        ...
        <div class="text-xs text-gray-500 dark:text-gray-400 tabular-nums text-right">
          {{ updatedLabel placeholderplaceholder<span v-if="intervalSeconds > 0"> · {{ t('monitorCommon.pollEvery', { n: intervalSeconds placeholder) placeholderplaceholder</span>
        </div>
      </div>
    </div>
  </section>
...
const windowOptions = computed<{ value: MonitorWindow; label: string placeholder[]>(() => [
  ...
  { value: '15d', label: t('channelStatus.windowTab.15d') placeholder,
  { value: '30d', label: t('channelStatus.windowTab.30d') placeholder,
])

const overallLabel = computed(() => t(`channelStatus.overall.${props.overallStatusplaceholder`))
