...
const { isQuotaVisible placeholder = vi.hoisted(() => ({
...
function makeItem(overrides: Partial<UserMonitorView> = {placeholder): UserMonitorView {
  return {
    id: 1,
    name: 'claude-main',
    provider: 'kimi',
...
describe('MonitorCard quota snapshot visibility', () => {
  it('hides the quota block when the system switch is off even if data exists', () => {
    ...
    const wrapper = mountCard(
      makeItem({
        latest_quota: { source: 'cn_quota', success: true, fetched_at: '2026-08-18T00:00:00Z' placeholder,
      placeholder),
    )
    ...
    const wrapper = mountCard(
      makeItem({
        latest_quota: {
          ...
          tiers: [{ window: 'daily', label: 'requests', used_percent: 60 placeholder],
  ...
  it('keeps the real model name for probe monitors', () => {
    const wrapper = mountCard(makeItem({ primary_model: 'claude-sonnet-4-5' placeholder))
