  ...
  return {
    ...
    useI18n: () => ({
      t: (key: string, params?: Record<string, string>) => {
        ...
        if (key === 'profile.authBindings.codeSentTo') return `Code sent to ${params?.email || ''placeholder`.trim()
  ...
  it('shows third-party binding details and unbinds a connected provider', async () => {
    userApiMocks.unbindAuthIdentity.mockResolvedValue(
      createUser({
        ...
        auth_bindings: {
          email: { bound: true placeholder,
          linuxdo: { bound: false, can_unbind: false placeholder,
        placeholder,
      placeholder)
    )
  ...
  it('hides bind actions when provider details say bindable but the provider is disabled', () => {
    const wrapper = mount(ProfileIdentityBindingsSection, {
      ...
      props: {
        user: createUser({
          auth_bindings: {
            linuxdo: { bound: false, can_bind: true placeholder,
            oidc: { bound: false, can_bind: true placeholder,
          placeholder,
        placeholder),
        linuxdoEnabled: false,
        oidcEnabled: false,
