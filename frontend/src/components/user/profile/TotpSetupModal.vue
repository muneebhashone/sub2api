      ...
      <div class="relative w-full max-w-md transform rounded-xl bg-white p-6 shadow-xl transition-all dark:bg-dark-800">
        ...
        <div class="mb-6 text-center">
          <h3 class="text-xl font-semibold text-gray-900 dark:text-white">
            {{ t('profile.totp.setupTitle') placeholderplaceholder
          </h3>
          <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">
            {{ stepDescription placeholderplaceholder
          </p>
        </div>
        ...
        <div v-if="step === 0" class="space-y-6">
          ...
          <template v-else>
            ...
            <div v-if="verificationMethod === 'email'" class="space-y-4">
              <div>
                <label class="input-label">{{ t('profile.totp.emailCode') placeholderplaceholder</label>
                <div class="flex gap-2">
                  ...
                  >
                    {{ codeCooldown > 0 ? `${codeCooldownplaceholders` : (sendingCode ? t('common.sending') : t('profile.totp.sendCode')) placeholderplaceholder
                  </button>
                </div>
              </div>
            </div>
            ...
            <div v-else class="space-y-4">
              <div>
                <label class="input-label">{{ t('profile.currentPassword') placeholderplaceholder</label>
                <input
            ...
            <div v-if="verifyError" class="rounded-lg bg-red-50 p-3 text-sm text-red-700 dark:bg-red-900/30 dark:text-red-400">
              {{ verifyError placeholderplaceholder
            </div>
            ...
            <div class="flex justify-end gap-3 pt-4">
              ...
              >
                {{ setupLoading ? t('common.loading') : t('common.next') placeholderplaceholder
              </button>
            </div>
          </template>
        </div>
        ...
        <div v-if="step === 1" class="space-y-6">
          ...
          <template v-if="setupData">
            ...
            <div class="text-center">
              <p class="text-sm text-gray-500 dark:text-gray-400 mb-2">
                {{ t('profile.totp.manualEntry') placeholderplaceholder
              </p>
              <div class="flex items-center justify-center gap-2">
                <code class="rounded bg-gray-100 px-3 py-2 font-mono text-sm dark:bg-dark-700">
                  {{ setupData.secret placeholderplaceholder
                </code>
                <button
                  type="button"
          ...
          <div class="flex justify-end gap-3 pt-4">
            ...
            <button
              ...
              @click="step = 2"
            >
              {{ t('common.next') placeholderplaceholder
            </button>
          </div>
        </div>
          ...
          <form @submit.prevent="handleVerify">
            <div class="mb-6">
              <label class="input-label text-center block mb-3">
                {{ t('profile.totp.enterCode') placeholderplaceholder
              </label>
            ...
            <div class="flex justify-end gap-3">
              ...
              >
                {{ verifying ? t('common.verifying') : t('profile.totp.verify') placeholderplaceholder
              </button>
            </div>
          </form>
        </div>
...
import { ref, onMounted, onUnmounted, nextTick, watch, computed placeholder from 'vue'
import { useI18n placeholder from 'vue-i18n'
import { useAppStore placeholder from '@/stores/app'
import { totpAPI placeholder from '@/api'
import type { TotpSetupResponse placeholder from '@/types'
...
const verifyForm = ref({ emailCode: '', password: '' placeholder)
...
const handleVerifyAndSetup = async () => {
  ...
  try {
    const request = verificationMethod.value === 'email'
      ? { email_code: verifyForm.value.emailCode placeholder
      : { password: verifyForm.value.password placeholder
