      ...
      <div class="relative w-full max-w-md transform rounded-xl bg-white p-6 shadow-xl transition-all dark:bg-dark-800">
        ...
        <div class="mb-6">
          ...
          <h3 class="mt-4 text-center text-xl font-semibold text-gray-900 dark:text-white">
            {{ t('profile.totp.disableTitle') placeholderplaceholder
          </h3>
          <p class="mt-2 text-center text-sm text-gray-500 dark:text-gray-400">
            {{ t('profile.totp.disableWarning') placeholderplaceholder
          </p>
        </div>
        ...
        <form v-else @submit.prevent="handleDisable" class="space-y-4">
          ...
          <div class="flex justify-end gap-3 pt-4">
            ...
            >
              {{ loading ? t('common.processing') : t('profile.totp.confirmDisable') placeholderplaceholder
            </button>
          </div>
        </form>
      </div>
...
const handleDisable = async () => {
  ...
  try {
    const request = verificationMethod.value === 'email'
      ? { email_code: form.value.emailCode placeholder
      : { password: form.value.password placeholder
