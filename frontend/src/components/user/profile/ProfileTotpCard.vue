  ...
  <div class="card">
    <div class="border-b border-gray-100 px-6 py-4 dark:border-dark-700">
      <h2 class="text-lg font-medium text-gray-900 dark:text-white">
        {{ t('profile.totp.title') placeholderplaceholder
      </h2>
      <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
        {{ t('profile.totp.description') placeholderplaceholder
      </p>
    </div>
    <div class="px-6 py-6">
      <!-- Loading state -->
      ...
      <div v-else-if="status && !status.feature_enabled" class="flex items-center gap-4 py-4">
        ...
        <div>
          <p class="font-medium text-gray-700 dark:text-gray-300">
            {{ t('profile.totp.featureDisabled') placeholderplaceholder
          </p>
          <p class="text-sm text-gray-500 dark:text-gray-400">
            {{ t('profile.totp.featureDisabledHint') placeholderplaceholder
          </p>
        </div>
      </div>
      ...
      <div v-else-if="status?.enabled" class="flex items-center justify-between">
        <div class="flex items-center gap-4">
          ...
          <div>
            <p class="font-medium text-gray-900 dark:text-white">
              {{ t('profile.totp.enabled') placeholderplaceholder
            </p>
            <p v-if="status.enabled_at" class="text-sm text-gray-500 dark:text-gray-400">
              {{ t('profile.totp.enabledAt') placeholderplaceholder: {{ formatDate(status.enabled_at) placeholderplaceholder
            </p>
          </div>
        </div>
        <button
        ...
        >
          {{ t('profile.totp.disable') placeholderplaceholder
        </button>
      </div>
      ...
      <div v-else class="flex items-center justify-between">
        <div class="flex items-center gap-4">
          ...
          <div>
            <p class="font-medium text-gray-700 dark:text-gray-300">
              {{ t('profile.totp.notEnabled') placeholderplaceholder
            </p>
            <p class="text-sm text-gray-500 dark:text-gray-400">
              {{ t('profile.totp.notEnabledHint') placeholderplaceholder
            </p>
          </div>
        </div>
        <button
        ...
        >
          {{ t('profile.totp.enable') placeholderplaceholder
        </button>
      </div>
    </div>
...
import type { TotpStatus placeholder from '@/types'
