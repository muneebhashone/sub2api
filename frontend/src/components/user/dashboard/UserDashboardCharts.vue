  ...
  <div class="space-y-6">
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
      ...
      <div class="card relative overflow-hidden p-4">
        ...
        <h3 class="mb-4 text-sm font-semibold text-gray-900 dark:text-white">{{ t('dashboard.tokenUsageTrend') placeholderplaceholder</h3>
        <div class="h-48">
...
const lineOptions = {
  ...
  maintainAspectRatio: false,
  plugins: {
    legend: { display: true, position: 'top' as const placeholder,
    tooltip: {
      callbacks: {
        label: (context: any) => `${context.dataset.labelplaceholder: ${formatTokens(context.parsed.y)placeholder tokens`
      placeholder
    placeholder
  placeholder,
  scales: {
