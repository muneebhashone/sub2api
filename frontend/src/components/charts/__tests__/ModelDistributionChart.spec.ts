...
const messages: Record<string, string> = {
  ...
  'admin.redeem.userPrefix': 'User #{idplaceholder',
placeholder
  ...
  return {
    ...
    useI18n: () => ({
      t: (key: string, params?: Record<string, unknown>) => {
        ...
        return params
          ? message.replace(/\{(\w+)\placeholder/g, (_, name: string) => String(params[name]))
          : message
      placeholder,
    placeholder),
  placeholder
...
describe('ModelDistributionChart', () => {
  ...
  it('uses total_tokens and token ordering by default', () => {
    ...
    const label = options.plugins.tooltip.callbacks.label({
      label: 'model-a',
      raw: 1000,
      dataset: { data: [1000, 500] placeholder,
    placeholder)
  ...
  it('uses actual_cost and reorders rows in actual cost mode', () => {
    ...
    const label = options.plugins.tooltip.callbacks.label({
      label: 'model-b',
      raw: 1.4,
      dataset: { data: [1.4, 0.2] placeholder,
    placeholder)
  ...
  it('uses the dashboard user label policy and renders Others with a dedicated chart color', async () => {
    const wrapper = mount(ModelDistributionChart, {
      props: {
        ...
        rankingItems: [
          { user_id: 1, email: 'alpha@example.com', username: 'alpha', actual_cost: 12, requests: 10, tokens: 1000 placeholder,
          { user_id: 2, email: 'beta@example.com', username: '   ', actual_cost: 8, requests: 6, tokens: 600 placeholder,
          { user_id: 3, email: '   ', username: '', actual_cost: 0, requests: 0, tokens: 0 placeholder,
        ],
