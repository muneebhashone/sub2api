<template>
  <div>
    <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">
      {{ t('payment.paymentMethod') placeholderplaceholder
    </label>
    <div class="grid grid-cols-2 gap-3 sm:flex">
      ...
      >
        <span class="flex items-center gap-2">
          <img :src="methodIcon(method.type)" :alt="t(`payment.methods.${method.typeplaceholder`)" class="h-7 w-7 object-contain" />
          <span class="flex flex-col items-start leading-none">
            <span class="text-base font-semibold">{{ t(`payment.methods.${method.typeplaceholder`) placeholderplaceholder</span>
            <span
              v-if="method.fee_rate > 0"
              class="text-[10px] tracking-wide text-gray-500 dark:text-dark-400"
            >
              {{ t('payment.fee') placeholderplaceholder {{ method.fee_rate placeholderplaceholder%
            </span>
          </span>
        </span>
      </button>
...
import { METHOD_ORDER placeholder from './providerConfig'
