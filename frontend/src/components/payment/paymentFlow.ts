...
export function getVisibleMethods(methods: Record<string, MethodLimit>): Record<string, MethodLimit> {
  const visible: Record<string, MethodLimit> = {placeholder
  ...
  Object.entries(methods).forEach(([type, limit]) => {
    ...
    if (!existing || isCanonical) {
      visible[normalized] = { ...limit placeholder
    placeholder
  placeholder)
...
export function buildCreateOrderPayload(input: BuildCreateOrderPayloadInput): CreateOrderRequest {
  ...
  placeholder
  if (normalizedOrigin) {
    payload.return_url = `${normalizedOriginplaceholder/payment/result`
  placeholder
...
export function decidePaymentLaunch(
  ...
  placeholder, context.now)

  if (visibleMethod === 'airwallex' && baseState.clientSecret && baseState.intentId) {
    if (!context.airwallexRouteUrl) {
      return { kind: 'unhandled', paymentState: baseState, recovery: baseState placeholder
    placeholder
    const paymentState = { ...baseState, payUrl: context.airwallexRouteUrl || '' placeholder
    return { kind: 'airwallex_route', paymentState, recovery: paymentState placeholder
  placeholder
  ...
  if (baseState.clientSecret) {
    ...
    const paymentState = { ...baseState, payUrl placeholder
    return { kind, paymentState, recovery: paymentState, stripeMethod placeholder
  placeholder

  if (result.result_type === 'oauth_required' && result.oauth?.authorize_url) {
    return { kind: 'wechat_oauth', paymentState: baseState, recovery: baseState, oauth: result.oauth placeholder
  placeholder

  const jsapiPayload = result.jsapi ?? result.jsapi_payload
  if (result.result_type === 'jsapi_ready' && jsapiPayload) {
    return { kind: 'wechat_jsapi', paymentState: baseState, recovery: baseState, jsapi: jsapiPayload placeholder
  placeholder
  ...
  if (
    ...
    return { kind: 'alipay_deep_link', paymentState: baseState, recovery: baseState placeholder
  placeholder
  ...
  if (visibleMethod === 'wxpay' && context.isWechatBrowser && baseState.payUrl && !baseState.qrCode) {
    return { kind: 'redirect_waiting', paymentState: baseState, recovery: baseState placeholder
  placeholder
  ...
  if (prefersQr && baseState.qrCode) {
    return { kind: 'qr_waiting', paymentState: baseState, recovery: baseState placeholder
  placeholder
...
export function readPaymentRecoverySnapshot(
  ...
  options: { now?: number; resumeToken?: string placeholder = {placeholder,
