    ...
    <form id="provider-form" @submit.prevent="handleSave" class="space-y-4">
      ...
      <div v-if="form.provider_key === 'easypay'" class="space-y-3 rounded-lg border border-gray-100 p-3 dark:border-dark-700">
        <div class="flex items-center justify-between gap-3">
          <div>
            <h5 class="text-sm font-medium text-gray-900 dark:text-white">
              {{ t('admin.settings.payment.easypayCustomMethods') placeholderplaceholder
            </h5>
            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
              {{ t('admin.settings.payment.easypayCustomMethodsHint') placeholderplaceholder
            </p>
          </div>
          <button type="button" class="btn btn-secondary btn-sm" @click="addEasyPayCustomMethod">
            {{ t('admin.settings.payment.addCustomMethod') placeholderplaceholder
          </button>
        </div>
        <div v-if="easyPayCustomMethods.length" class="space-y-2">
          ...
          >
            <div>
              <label class="text-xs text-gray-500 dark:text-gray-400">{{ t('admin.settings.payment.customMethodType') placeholderplaceholder</label>
              <input v-model="method.type" type="text" class="input mt-0.5" placeholder="credit_card" />
            </div>
            <div>
              <label class="text-xs text-gray-500 dark:text-gray-400">{{ t('admin.settings.payment.customMethodUpstreamType') placeholderplaceholder</label>
              <input v-model="method.upstreamType" type="text" class="input mt-0.5" placeholder="credit_card" />
            </div>
            <div>
              <label class="text-xs text-gray-500 dark:text-gray-400">{{ t('admin.settings.payment.customMethodDisplayName') placeholderplaceholder</label>
      ...
      <div class="border-t border-gray-200 pt-4 dark:border-dark-700">
        <div class="mb-3 flex items-center gap-2">
          ...
          <HelpTooltip v-if="paymentGuide" trigger="click" width-class="w-80">
            ...
            <div class="space-y-3">
              <p class="font-medium text-white">{{ paymentGuide.summary placeholderplaceholder</p>
              <div
                v-for="item in paymentGuide.items"
                :key="item.title"
                class="space-y-1.5 border-t border-white/10 pt-2 first:border-t-0 first:pt-0"
              >
                <p class="font-medium text-white">{{ item.title placeholderplaceholder</p>
                <p><span class="text-gray-300">{{ t('admin.settings.payment.guideOpenLabel') placeholderplaceholder</span>{{ item.open placeholderplaceholder</p>
                <p><span class="text-gray-300">{{ t('admin.settings.payment.guideCallLabel') placeholderplaceholder</span>{{ item.call placeholderplaceholder</p>
                <p><span class="text-gray-300">{{ t('admin.settings.payment.guideFallbackLabel') placeholderplaceholder</span>{{ item.fallback placeholderplaceholder</p>
              </div>
              <p v-if="paymentGuide.note" class="border-t border-white/10 pt-2 text-[11px] text-gray-300">
                {{ paymentGuide.note placeholderplaceholder
              </p>
            </div>
          </HelpTooltip>
        </div>
        <p v-if="paymentGuide" class="mb-3 text-xs text-gray-500 dark:text-gray-400">
          {{ paymentGuide.summary placeholderplaceholder
        </p>
        <div class="space-y-3">
          <div v-for="field in resolvedFields" :key="field.key">
            ...
            <p v-if="field.hintKey" class="mt-1 text-xs leading-relaxed text-gray-500 dark:text-gray-400">
              {{ t(field.hintKey) placeholderplaceholder
            </p>
          </div>
        </div>
        ...
        <div v-if="providerWebhookUrl" class="mt-3 rounded-lg border border-blue-200 bg-blue-50 p-3 dark:border-blue-800/50 dark:bg-blue-900/20">
          <p class="text-xs text-blue-700 dark:text-blue-300">
            {{ t(providerWebhookHint) placeholderplaceholder
          </p>
          <code class="mt-1 block break-all rounded bg-blue-100 px-2 py-1 text-xs text-blue-800 dark:bg-blue-900/40 dark:text-blue-200">
            {{ providerWebhookUrl placeholderplaceholder
          </code>
          <p v-if="form.provider_key === 'stripe'" class="mt-2 text-xs leading-relaxed text-blue-700 dark:text-blue-300">
            {{ t('admin.settings.payment.stripeWebhookApiVersionHint', { version: STRIPE_SDK_API_VERSION placeholder) placeholderplaceholder
          </p>
        </div>
      </div>
...
import type { EasyPayCustomMethod, TypeOption placeholder from './providerConfig'
import {
...
const paymentModeOptions = computed(() => {
  if (form.provider_key === 'alipay') {
    ...
    return [
      { value: '', label: t('admin.settings.payment.modeQRCode') placeholder,
      { value: PAYMENT_MODE_REDIRECT, label: t('admin.settings.payment.modeRedirect') placeholder,
    ]
  placeholder
  return [
...
function addEasyPayCustomMethod() {
  easyPayCustomMethods.push({ type: '', upstreamType: '', displayName: '' placeholder)
placeholder
