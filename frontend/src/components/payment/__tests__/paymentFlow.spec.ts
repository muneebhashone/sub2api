...
import type { CreateOrderResult, MethodLimit placeholder from '@/types/payment'
import {
...
function methodLimit(overrides: Partial<MethodLimit> = {placeholder): MethodLimit {
  return {
    daily_limit: 0,
    daily_used: 0,
    daily_remaining: 0,
...
function createOrderResult(overrides: Partial<CreateOrderResult> = {placeholder): CreateOrderResult {
  return {
    order_id: 101,
    amount: 88,
    pay_amount: 88,
...
describe('getVisibleMethods', () => {
  it('normalizes provider aliases and keeps stripe as a top-level method', () => {
    const visible = getVisibleMethods({
      alipay_direct: methodLimit({ single_min: 5 placeholder),
      wxpay: methodLimit({ single_max: 100 placeholder),
      stripe: methodLimit({ fee_rate: 3 placeholder),
      airwallex: methodLimit({ single_min: 10 placeholder),
    placeholder)

    expect(visible).toEqual({
      alipay: methodLimit({ single_min: 5 placeholder),
      wxpay: methodLimit({ single_max: 100 placeholder),
      stripe: methodLimit({ fee_rate: 3 placeholder),
      airwallex: methodLimit({ single_min: 10 placeholder),
    placeholder)
  ...
  it('prefers canonical visible methods over aliases when both exist', () => {
    const visible = getVisibleMethods({
      alipay: methodLimit({ single_min: 2 placeholder),
      alipay_direct: methodLimit({ single_min: 9 placeholder),
      wxpay_direct: methodLimit({ fee_rate: 1.2 placeholder),
    placeholder)
