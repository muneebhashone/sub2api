...
export function planValiditySuffix(
  ...
  if (base === 'month') {
    return days === 1 ? t('payment.perMonth') : `${daysplaceholder${t('payment.months')placeholder`
  placeholder
  if (base === 'week') {
    return `${daysplaceholder${t('payment.weeks')placeholder`
  placeholder
  // 其余单位（含数据库默认的 day 与未知值）后端一律按天计费，展示保持一致。
  return `${daysplaceholder${t('payment.days')placeholder`
placeholder
