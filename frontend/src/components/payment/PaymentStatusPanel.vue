          ...
          <div v-if="paidOrder" class="w-full rounded-xl bg-gray-50 p-4 dark:bg-dark-800">
            <div class="space-y-2 text-sm">
              ...
              <div class="flex justify-between">
                ...
                <span class="font-medium text-gray-900 dark:text-white">{{ creditedAmountSymbol placeholderplaceholder{{ paidOrder.amount.toFixed(2) placeholderplaceholder</span>
              </div>
              <div class="flex justify-between">
                <span class="text-gray-500 dark:text-gray-400">{{ t('payment.orders.payAmount') placeholderplaceholder</span>
                <span class="font-medium text-gray-900 dark:text-white">{{ formatGatewayAmount(paidOrder.pay_amount, paidOrder.currency) placeholderplaceholder</span>
              </div>
            </div>
          </div>
        ...
        <div class="card p-6">
          <div class="flex flex-col items-center space-y-4 py-4 text-center">
            ...
            <p class="text-lg font-semibold text-gray-900 dark:text-white">
              {{ deepLinkState === 'backgrounded' ? t('payment.qr.alipayContinueInApp') : t('payment.qr.alipayOpening') placeholderplaceholder
            </p>
            <p class="text-sm text-gray-500 dark:text-gray-400">{{ t('payment.qr.alipayWaitingHint') placeholderplaceholder</p>
            <button
            ...
            >
              ...
              {{ t('payment.qr.reopenAlipay') placeholderplaceholder
            </button>
          </div>
        </div>
        ...
        <div data-test="alipay-qr-fallback" class="card p-6">
          <div class="flex flex-col items-center space-y-4">
            <div class="text-center">
              <p class="text-lg font-semibold text-gray-900 dark:text-white">{{ t('payment.qr.alipayFallbackTitle') placeholderplaceholder</p>
              <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ t('payment.qr.alipayFallbackHint') placeholderplaceholder</p>
            </div>
            <div class="w-full space-y-2 border-y border-gray-100 py-3 text-sm dark:border-dark-600">
              <div class="flex items-start justify-between gap-4">
                <span class="text-gray-500 dark:text-gray-400">{{ t('payment.orders.payAmount') placeholderplaceholder</span>
                <span class="font-semibold text-gray-900 dark:text-white">{{ displayPaymentAmount placeholderplaceholder</span>
              </div>
              <div class="flex items-start justify-between gap-4">
                <span class="text-gray-500 dark:text-gray-400">{{ t('payment.orders.orderNo') placeholderplaceholder</span>
                <span class="max-w-[70%] break-all text-right font-mono text-xs text-gray-900 dark:text-white">
                  {{ displayOrderNumber placeholderplaceholder
                </span>
              </div>
              <div class="flex items-start justify-between gap-4">
                <span class="text-gray-500 dark:text-gray-400">{{ t('payment.qr.expiresIn') placeholderplaceholder</span>
                <span class="font-semibold tabular-nums text-gray-900 dark:text-white">{{ countdownDisplay placeholderplaceholder</span>
              </div>
            </div>
            ...
            <p class="text-center text-sm leading-6 text-gray-600 dark:text-gray-300">
              {{ t('payment.qr.alipaySaveAndScanHint') placeholderplaceholder
            </p>
            <div class="grid w-full gap-2 sm:grid-cols-2">
              ...
              >
                ...
                {{ t('payment.qr.saveQRCode') placeholderplaceholder
              </button>
            </div>
...
import { getPaymentPopupFeatures, isBuiltInAlipayMethod, isBuiltInWxpayMethod placeholder from '@/components/payment/providerConfig'
import { currencySymbol, formatPaymentAmount, normalizePaymentCurrency placeholder from '@/components/payment/currency'
...
placeholder from './alipayDeepLink'
...
const displayOrderNumber = computed(() => props.outTradeNo || `#${props.orderIdplaceholder`)
...
function updateDeepLinkState(state: AlipayDeepLinkState) {
  ...
  if (state === 'fallback') {
    ...
    renderQR()
  placeholder else if (state === 'backgrounded') {
...
function saveQRCode() {
  ...
  link.download = `alipay-${props.outTradeNo || props.orderIdplaceholder.png`
