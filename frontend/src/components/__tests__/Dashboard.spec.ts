...
import { defineComponent, ref, onMounted, nextTick placeholder from 'vue'
...
vi.mock('@/api', () => ({
  authAPI: {
    getCurrentUser: vi.fn().mockResolvedValue({
      data: { id: 1, username: 'test', email: 'test@example.com', role: 'user', balance: 100, concurrency: 5, status: 'active', allowed_groups: null, created_at: '', updated_at: '' placeholder,
    placeholder),
    logout: vi.fn(),
    refreshToken: vi.fn(),
  placeholder,
...
const DashboardTestComponent = defineComponent({
  setup() {
    ...
    return { stats, loading, error, loadStats placeholder
  placeholder,
  template: `
    <div>
      <div v-if="loading" class="loading">加载中...</div>
      <div v-if="error" class="error">{{ error placeholderplaceholder</div>
      <div v-if="stats" class="stats">
        <span class="balance">{{ stats.balance placeholderplaceholder</span>
        <span class="api-keys">{{ stats.api_key_count placeholderplaceholder</span>
        <span class="today-requests">{{ stats.today_requests placeholderplaceholder</span>
        <span class="today-cost">{{ stats.today_cost placeholderplaceholder</span>
      </div>
...
describe('Dashboard 数据加载', () => {
  ...
  it('加载中显示 loading 状态', async () => {
    ...
    mockGetDashboardStats.mockImplementation(
      () => new Promise((resolve) => { resolveStats = resolve placeholder)
    )
  ...
  it('点击刷新按钮重新加载数据', async () => {
    ...
    // 更新数据
    const updatedStats = { ...fakeStats, today_requests: 200 placeholder
