...
import { computed, nextTick, onBeforeUnmount, onMounted, ref placeholder from 'vue'
...
const props = withDefaults(defineProps<{ appId: string; region?: string placeholder>(), { region: 'cn' placeholder)
    ...
    void nextTick()
      ...
      .then((TencentCaptcha) => {
        ...
        const handleResult = (result: TencentCaptchaResult): void => {
          ...
          finish(() => resolve({ ticket, randstr placeholder), region === 'intl')
        placeholder
        ...
        if (region === 'intl') {
          ...
          placeholder)
        placeholder else {
          instance = new TencentCaptcha(props.appId, handleResult, { userLanguage placeholder)
        placeholder
        instance.show()
      placeholder)
