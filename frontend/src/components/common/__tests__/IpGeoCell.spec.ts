...
describe('IpGeoCell', () => {
  ...
  it('renders a clickable fetch link in idle state and triggers fetchOne on click', async () => {
    mocks.getEntry.mockReturnValue({ status: 'idle' placeholder)
    const wrapper = mount(IpGeoCell, { props: { ip: '8.8.8.8' placeholder placeholder)
  ...
  it('renders loading state', () => {
    mocks.getEntry.mockReturnValue({ status: 'loading' placeholder)
    const wrapper = mount(IpGeoCell, { props: { ip: '8.8.8.8' placeholder placeholder)
  ...
  it('renders success state with label, tooltip detail, and a refresh button', async () => {
    mocks.getEntry.mockReturnValue({
      ...
      placeholder,
    placeholder)
    const wrapper = mount(IpGeoCell, { props: { ip: '121.35.47.43' placeholder placeholder)
  ...
  it('opens the external lookup page when the label is clicked', async () => {
    mocks.getEntry.mockReturnValue({ status: 'success', label: 'US · California', detail: {placeholder placeholder)
    const openSpy = vi.spyOn(window, 'open').mockImplementation(() => null)
    const wrapper = mount(IpGeoCell, { props: { ip: '8.8.4.4' placeholder placeholder)
  ...
  it('renders failed state as a clickable retry', async () => {
    mocks.getEntry.mockReturnValue({ status: 'error' placeholder)
    const wrapper = mount(IpGeoCell, { props: { ip: '8.8.8.8' placeholder placeholder)
  ...
  it('renders private state as non-clickable text', () => {
    mocks.getEntry.mockReturnValue({ status: 'private' placeholder)
    const wrapper = mount(IpGeoCell, { props: { ip: '192.168.1.1' placeholder placeholder)
