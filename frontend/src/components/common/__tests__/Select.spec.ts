...
describe('Select remote search', () => {
  const mountRemoteSelect = (props: Record<string, unknown> = {placeholder) => {
    const wrapper = mount(Select, {
      props: {
        modelValue: null,
        remote: true,
        options: [
          { value: 'alpha', label: 'Alpha account' placeholder,
          { value: 'beta', label: 'Beta account' placeholder,
        ],
        ...props,
      placeholder,
    placeholder)
  ...
  it('shows the loading text instead of empty text while loading with no options', async () => {
    const wrapper = mountRemoteSelect({ options: [], loading: true placeholder)
    await wrapper.get('button').trigger('click')
    await nextTick()
