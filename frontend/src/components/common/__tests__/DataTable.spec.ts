...
describe('DataTable', () => {
  ...
  it('emits controlled current-page selection while preserving off-page keys', async () => {
    const wrapper = mount(DataTable, {
      props: {
        columns: [{ key: 'name', label: 'Name' placeholder],
        data: [
          { id: 1, name: 'One' placeholder,
          { id: 2, name: 'Two' placeholder
        ],
        rowKey: 'id',
        selectable: true,
        selectedKeys: [99]
    ...
    await wrapper.setProps({ selectedKeys: selectedAll as number[] placeholder)
  ...
  it('keeps the single usage field shrinkable in a 320px mobile card', () => {
    ...
    const wrapper = mount(DataTable, {
      attachTo: viewport,
      props: {
        columns: [{ key: 'usage', label: 'Usage' placeholder],
        data: [{ id: 1, usage: 'snapshot' placeholder],
        rowKey: 'id'
      placeholder,
      slots: {
