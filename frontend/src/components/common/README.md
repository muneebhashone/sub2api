...
- `cell-{keyplaceholder` - Custom cell renderer for specific column (receives `row` and `value`)
...
<DataTable
  :columns="[
    ...
    { key: 'status', label: 'Status', formatter: (val) => val.toUpperCase() placeholder
  ]"
  :data="users"
  :loading="isLoading"
>
...
<EmptyState
  ...
  :action-to="{ name: 'users-create' placeholder"
/>
```
...
```typescript
import { DataTable, Pagination, Modal placeholder from '@/components/common'
```
