<template>
  <div class="relative">
    ...
    <template v-if="isAdmin">
      ...
      >
        <span v-if="currentVersion" class="font-medium">v{{ currentVersion placeholderplaceholder</span>
        <span
          v-else
          ...
          <div class="p-4">
            ...
            <template v-else>
              ...
              <div class="mb-4 text-center">
                <div class="inline-flex items-center gap-2">
                  <span
                    ...
                    >v{{ currentVersion placeholderplaceholder</span
                  >
              ...
              <div v-if="updateError" class="space-y-2">
                ...
                >
                  ...
                  <div class="min-w-0 flex-1">
                    <p class="text-sm font-medium text-red-700 dark:text-red-300">
                      {{ t('version.updateFailed') placeholderplaceholder
                    </p>
                    <p class="truncate text-xs text-red-600/70 dark:text-red-400/70">
                      {{ updateError placeholderplaceholder
                    </p>
                  </div>
                </div>
                ...
                >
                  {{ t('version.retry') placeholderplaceholder
                </button>
              </div>
              ...
              <div v-else-if="updateSuccess && needRestart" class="space-y-2">
                ...
                >
                  ...
                  <div class="min-w-0 flex-1">
                    ...
                    <p class="text-xs text-green-600/70 dark:text-green-400/70">
                      {{ t('version.restartRequired') placeholderplaceholder
                    </p>
                  </div>
                </div>
                ...
                >
                  ...
                  <template v-if="restarting">
                    <span>{{ t('version.restarting') placeholderplaceholder</span>
                    <span v-if="restartCountdown > 0" class="tabular-nums"
                      >({{ restartCountdown placeholderplaceholders)</span
                    >
                  </template>
                  <span v-else>{{ t('version.restartNow') placeholderplaceholder</span>
                </button>
              </div>
              ...
              <div v-else-if="hasUpdate && !isReleaseBuild" class="space-y-2">
                ...
                >
                  ...
                  <div class="min-w-0 flex-1">
                    <p class="text-sm font-medium text-amber-700 dark:text-amber-300">
                      {{ t('version.updateAvailable') placeholderplaceholder
                    </p>
                    <p class="text-xs text-amber-600/70 dark:text-amber-400/70">
                      v{{ latestVersion placeholderplaceholder
                    </p>
                  </div>
                  <svg
                ...
                >
                  ...
                  <p class="text-xs text-blue-600 dark:text-blue-400">
                    {{ t('version.sourceModeHint') placeholderplaceholder
                  </p>
                </div>
              </div>
              ...
              <div v-else-if="hasUpdate && isReleaseBuild" class="space-y-2">
                ...
                >
                  ...
                  {{ updating ? t('version.updating') : t('version.updateNow') placeholderplaceholder
                </button>
                ...
                >
                  {{ t('version.viewChangelog') placeholderplaceholder
              ...
              <div v-else class="space-y-2">
                ...
                >
                  ...
                  {{ t('version.viewRelease') placeholderplaceholder
                </a>
                ...
                <div class="border-t border-gray-100 pt-2 dark:border-dark-700">
                  ...
                  >
                    <span class="flex items-center gap-1.5">
                      ...
                      {{ t('version.rollback') placeholderplaceholder
                    </span>
                    <Icon
                      name="chevronDown"
                      size="xs"
                      ...
                      :class="{ 'rotate-180': rollbackPanelOpen placeholder"
                    />
                  </button>
                    ...
                    <div v-if="rollbackPanelOpen" class="mt-2 space-y-2">
                      ...
                      >
                        ...
                        <p class="min-w-0 flex-1 text-xs leading-4 text-blue-600 dark:text-blue-400">
                          {{ t('version.rollbackSourceHint') placeholderplaceholder
                        </p>
                      </div>
                      ...
                      <div v-else-if="rollbackVersionsError" class="space-y-2">
                        ...
                        >
                          {{ rollbackVersionsError placeholderplaceholder
                        </p>
                        <button
                      ...
                      >
                        {{ t('version.noRollbackVersions') placeholderplaceholder
                      </p>

                      <!-- Version list -->
                      <template v-else>
                        <p class="px-0.5 text-[11px] text-gray-400 dark:text-dark-500">
                          {{ t('version.rollbackSelectVersion') placeholderplaceholder
                        </p>
                        ...
                        >
                          <span class="flex items-center gap-2">
                            ...
                            <span
                              ...
                              "
                              >v{{ item.version placeholderplaceholder</span
                            >
                          </span>
                          <span class="text-[11px] tabular-nums text-gray-400 dark:text-dark-500">
                            {{ formatPublishedAt(item.published_at) placeholderplaceholder
                          </span>
                        </button>
                        ...
                        <transition name="rollback">
                          <div v-if="selectedRollbackVersion" class="space-y-2">
                            <p class="px-0.5 text-[11px] text-gray-400 dark:text-dark-500">
                              {{ t('version.manualRollbackCommand') placeholderplaceholder
                            </p>
                            ...
                            >
                              ...
                              >
                                ...
                                >
                                  ...
                                  />
                                  {{ copied ? t('version.copied') : t('version.copyCommand') placeholderplaceholder
                                </button>
                              </div>
                              <code
                                ...
                                >{{ activeManualCommand placeholderplaceholder</code
                              >
                            </div>
                            ...
                            >
                              ...
                              />
                              {{ t('version.rollbackWarning') placeholderplaceholder
                            </p>
                            ...
                            >
                              {{ rollbackError placeholderplaceholder
                            </p>
    ...
    <span v-else-if="version" class="text-xs text-gray-500 dark:text-dark-400">
      v{{ version placeholderplaceholder
    </span>
  </div>
</template>
...
const manualTabs = computed(() => [
  { key: 'script' as const, label: t('version.deployScript') placeholder,
  { key: 'docker' as const, label: t('version.deployDocker') placeholder
])

const scriptRollbackCommand = computed(() => {
  if (!selectedRollbackVersion.value) return ''
  const tag = `v${selectedRollbackVersion.valueplaceholder`
  return `curl -sSL https://raw.githubusercontent.com/${GITHUB_REPOplaceholder/${tagplaceholder/deploy/install.sh | sudo bash -s -- rollback ${tagplaceholder`
placeholder)

const dockerRollbackCommand = computed(() => {
  if (!selectedRollbackVersion.value) return ''
  return [
    `# ${t('version.dockerEditCompose')placeholder`,
    `image: ${DOCKER_IMAGEplaceholder:${selectedRollbackVersion.valueplaceholder`,
    '',
    `# ${t('version.dockerRecreate')placeholder`,
    'docker compose up -d'
  ].join('\n')
placeholder)
...
async function handleUpdate() {
  ...
  placeholder catch (error: unknown) {
    const err = error as { response?: { data?: { message?: string placeholder placeholder; message?: string placeholder
