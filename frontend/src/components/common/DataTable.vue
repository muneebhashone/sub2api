        ...
        <slot name="empty">
          <div class="flex flex-col items-center">
            ...
            <p class="text-lg font-medium text-gray-900 dark:text-gray-100">
              {{ t('empty.noData') placeholderplaceholder
            </p>
          </div>
        </slot>
      </div>
    ...
    <template v-else>
      <div v-if="selectable" class="flex items-center justify-end gap-2 px-1">
        <label class="flex items-center gap-2 text-sm font-medium text-gray-600 dark:text-gray-300">
          ...
          />
          <span>{{ t('common.selectAll') placeholderplaceholder</span>
        </label>
      </div>
      <div
        ...
        <div class="space-y-3">
          ...
          >
            <span class="text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-dark-400">
              {{ column.label placeholderplaceholder
            </span>
        ...
        <tr>
          ...
          <th
            ...
            :class="[
              ...
              { 'cursor-pointer hover:bg-gray-100 dark:hover:bg-dark-700': column.sortable placeholder,
          ...
          >
            <div :class="['flex items-center space-x-1', getHeaderContentAlignmentClass(column)]">
              <slot
                :name="`header-${column.keyplaceholder`"
                :column="column"
                :sort-key="sortKey"
            ...
            >
              <slot :name="`cell-${column.keyplaceholder`"
                ...
                {{ column.formatter
                   ...
                   : item.row[column.key] placeholderplaceholder
              </slot>
            </td>
          </tr>
...
import { useVirtualizer, observeElementRect as observeElementRectDefault placeholder from '@tanstack/vue-virtual'
...
const observeElementRectNonZero = (
  instance: any,
  cb: (rect: { width: number; height: number placeholder) => void
...
const readPersistedSortState = (): PersistedSortState | null => {
  ...
  try {
    ...
    if (!key) return null
    return { key, order: normalizeSortOrder(parsed.order) placeholder
  placeholder catch (e) {
...
const resolveInitialSortState = (): PersistedSortState | null => {
  ...
  if (!key) return null
  return { key, order: normalizeSortOrder(props.defaultSortOrder) placeholder
placeholder
...
const columnsSignature = computed(() =>
  props.columns.map((column) => `${column.keyplaceholder:${column.sortable ? '1' : '0'placeholder`).join('|')
)
...
const getRowSelectionLabel = (row: any, index: number) => {
  ...
  return `${t('common.selectOption')placeholder ${resolveRowKey(row, index)placeholder`
placeholder
...
const rowVirtualizer = useVirtualizer(computed(() => ({
  ...
  initialRect: { width: 0, height: estimatedViewportHeight() placeholder,
...
const renderRows = computed<Array<{ index: number; row: any; measure: boolean placeholder>>(() => {
  const data = sortedData.value ?? []
  if (shouldVirtualize.value) {
    return virtualItems.value.map(vr => ({ index: vr.index, row: data[vr.index], measure: true placeholder))
  placeholder
  return data.map((row, index) => ({ index, row, measure: false placeholder))
placeholder)
...
const getAdaptivePaddingClass = () => {
  ...
  if (columnCount >= 10) {
    return 'px-2' // 8px
  placeholder else if (columnCount >= 7) {
    return 'px-3' // 12px
  placeholder else if (columnCount >= 5) {
    return 'px-4' // 16px
  placeholder else {
...
watch(
  ...
  ([nextKey, nextOrder]) => {
    ...
    if (!key) return
    writePersistedSortState({ key, order: normalizeSortOrder(nextOrder) placeholder)
  placeholder,
  { flush: 'post' placeholder
)
...
/* ==========================================================================
   ...
   绕过 style.css 中 `* { scrollbar-color: transparent placeholder` 的全局悬停隐身诅咒！
   ========================================================================== */
