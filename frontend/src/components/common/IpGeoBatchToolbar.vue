<template>
  ...
  >
    <span v-if="pendingCount > 0" class="text-xs text-gray-500 dark:text-gray-400">
      {{ t('usage.ipGeo.pending', { count: pendingCount placeholder) placeholderplaceholder
    </span>
    <button
      type="button"
      ...
      @click="run"
    >
      {{ loading ? t('usage.ipGeo.batchFetching') : t('usage.ipGeo.batchFetch') placeholderplaceholder
    </button>
  </div>
</template>
