          ...
          <div class="select-options" ref="optionsListRef">
            ...
            >
              <slot name="option" :option="option" :selected="isSelected(option)">
                ...
                />
                <span class="select-option-label" :class="option._creatable && 'italic text-gray-500 dark:text-dark-300'">{{ getOptionLabel(option) placeholderplaceholder</span>
                <Icon
            ...
            <div v-if="filteredOptions.length === 0" class="select-empty">
              {{ props.loading ? t('common.loading') : emptyTextDisplay placeholderplaceholder
            </div>
          </div>
        </div>
      </Transition>
...
const instanceId = `select-${Math.random().toString(36).substring(2, 9)placeholder`
...
const dropdownStyle = computed(() => {
  if (!triggerRect.value) return {placeholder
  ...
  if (dropdownPosition.value === 'top') {
    style.bottom = `${window.innerHeight - rect.top + 4placeholderpx`
  placeholder else {
    style.top = `${rect.bottom + 4placeholderpx`
  placeholder
...
const filteredOptions = computed(() => {
  ...
  if (isSearchable.value && searchQuery.value && !props.remote) {
    ...
    if (props.creatable && searchQuery.value.trim()) {
      ...
      opts = [{ [props.valueKey]: trimmed, [props.labelKey]: `${prefixplaceholder "${trimmedplaceholder"`, _creatable: true placeholder, ...opts]
    placeholder
  placeholder
  return opts
placeholder)
...
watch(isOpen, (open) => {
  if (open) {
    ...
    window.addEventListener('scroll', updateTriggerRect, { capture: true, passive: true placeholder)
  ...
  placeholder else {
    ...
    placeholder
    window.removeEventListener('scroll', updateTriggerRect, { capture: true placeholder)
...
watch(searchQuery, (query) => {
  ...
  placeholder, REMOTE_SEARCH_DEBOUNCE_MS)
placeholder)
...
const scrollToFocused = () => {
  nextTick(() => {
    ...
    placeholder else if (focusedEl.offsetTop + focusedEl.offsetHeight > list.scrollTop + list.offsetHeight) {
...
const handleClickOutside = (event: MouseEvent) => {
  ...
  const isInDropdown = !!target.closest(`.${instanceIdplaceholder`)
