  ...
  <BaseDialog :show="show" :title="t('usage.exporting')" width="narrow" @close="handleCancel">
    <div class="space-y-4">
      <div class="text-sm text-gray-600 dark:text-gray-400">
        {{ t('usage.exportingProgress') placeholderplaceholder
      </div>
      <div class="flex items-center justify-between text-sm text-gray-700 dark:text-gray-300">
        <span>{{ t('usage.exportedCount', { current, total placeholder) placeholderplaceholder</span>
        <span class="font-medium text-gray-900 dark:text-white">{{ normalizedProgress placeholderplaceholder%</span>
      </div>
      <div class="h-2 w-full rounded-full bg-gray-200 dark:bg-dark-700">
        <div
          ...
          aria-valuemin="0"
          aria-valuemax="100"
          :aria-label="`${t('usage.exportingProgress')placeholder: ${normalizedProgressplaceholder%`"
          class="h-2 rounded-full bg-primary-600 transition-all"
          :style="{ width: `${normalizedProgressplaceholder%` placeholder"
        ></div>
      </div>
      <div v-if="estimatedTime" class="text-xs text-gray-500 dark:text-gray-400" aria-live="polite" aria-atomic="true">
        {{ t('usage.estimatedTime', { time: estimatedTime placeholder) placeholderplaceholder
      </div>
    </div>
    ...
    <template #footer>
      ...
      >
        {{ t('usage.cancelExport') placeholderplaceholder
      </button>
    </template>
  </BaseDialog>
</template>
