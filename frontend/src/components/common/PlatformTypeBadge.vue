  ...
  <div class="inline-flex flex-col gap-0.5 text-xs font-medium">
    ...
    <div class="inline-flex items-center overflow-hidden rounded-md">
      <span :class="['inline-flex items-center gap-1 px-2 py-1', platformClass]">
        ...
        <span>{{ platformLabel placeholderplaceholder</span>
      </span>
      <span :class="['inline-flex items-center gap-1 px-1.5 py-1', typeClass]">
        ...
        <span>{{ typeLabel placeholderplaceholder</span>
      </span>
    </div>
    ...
    <div v-if="planLabel || privacyBadge" class="inline-flex items-center overflow-hidden rounded-md">
      <span v-if="planLabel" :class="['inline-flex items-center gap-1 px-1.5 py-1', planBadgeClass]">
        ...
        <Icon
          ...
          aria-hidden="true"
        />
        <span>{{ planLabel placeholderplaceholder</span>
      </span>
      <span
        v-if="privacyBadge"
      ...
      >
        ...
        <span>{{ privacyBadge.label placeholderplaceholder</span>
      </span>
    </div>
    ...
    <div v-if="expiresLabel" class="text-[10px] leading-tight text-gray-400 dark:text-gray-500 pl-0.5" :title="subscriptionExpiresAt">
      {{ expiresLabel placeholderplaceholder
    </div>
  </div>
</template>
...
import type { AccountPlatform, AccountType placeholder from '@/types'
...
const expiresLabel = computed(() => {
  ...
  try {
    ...
    return `${t('admin.accounts.subscriptionExpires')placeholder ${yyyyplaceholder-${mmplaceholder-${ddplaceholder`
  placeholder catch {
    return ''
  placeholder
placeholder)
...
const privacyBadge = computed(() => {
  ...
  switch (props.privacyMode) {
    // OpenAI states
    case 'training_off':
      return { label: 'Private', icon: shieldCheck, title: t('admin.accounts.privacyTrainingOff'), class: 'bg-green-100 text-green-600 dark:bg-green-900/30 dark:text-green-400' placeholder
    case 'training_set_cf_blocked':
      return { label: 'CF', icon: shieldX, title: t('admin.accounts.privacyCfBlocked'), class: 'bg-yellow-100 text-yellow-600 dark:bg-yellow-900/30 dark:text-yellow-400' placeholder
    case 'training_set_failed':
      return { label: 'Fail', icon: shieldX, title: t('admin.accounts.privacyFailed'), class: 'bg-red-100 text-red-600 dark:bg-red-900/30 dark:text-red-400' placeholder
    // Antigravity states
    case 'privacy_set':
      return { label: 'Private', icon: shieldCheck, title: t('admin.accounts.privacyAntigravitySet'), class: 'bg-green-100 text-green-600 dark:bg-green-900/30 dark:text-green-400' placeholder
    case 'privacy_set_failed':
      return { label: 'Fail', icon: shieldX, title: t('admin.accounts.privacyAntigravityFailed'), class: 'bg-red-100 text-red-600 dark:bg-red-900/30 dark:text-red-400' placeholder
    default:
      return null
  placeholder
placeholder)
