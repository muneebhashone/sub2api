<template>
  <div>
    <label class="input-label">
      ...
      <span class="font-normal text-gray-400">{{ t('common.selectedCount', { count: modelValue.length placeholder) placeholderplaceholder</span>
    </label>
    <div
    ...
    >
      ...
      >
        ...
        />
        <span class="shrink-0 text-xs text-gray-400">{{ group.account_count || 0 placeholderplaceholder</span>
      </label>
      <div
        v-if="filteredGroups.length === 0"
        class="col-span-2 py-2 text-center text-sm text-gray-500 dark:text-gray-400"
      >
        {{ t('common.noGroupsAvailable') placeholderplaceholder
      </div>
    </div>
  </div>
</template>
