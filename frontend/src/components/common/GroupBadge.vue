  ...
  >
    ...
    <span v-if="showLabel" :class="labelClass">
      <template v-if="hasCustomRate">
        <!-- 原倍率删除线 + 专属倍率高亮 -->
        <span class="line-through opacity-50 mr-0.5">{{ rateMultiplier placeholderplaceholderx</span>
    ...
    <span v-if="hasPeakRate" :class="peakRateClass" :title="peakRateTitle">
      {{ peakRateText placeholderplaceholder
    </span>
  </span>
</template>
...
import { formatPeakRateWindow, serverTimezoneLabel placeholder from '@/utils/peak-rate'
...
const peakRateTitle = computed(() => {
  return t('common.peakRateTooltip', { window: peakRateText.value placeholder)
placeholder)
...
const labelText = computed(() => {
  const rateLabel = props.rateMultiplier !== undefined ? `${props.rateMultiplierplaceholderx` : ''
...
const labelClass = computed(() => {
  ...
  if (props.platform === 'antigravity') {
    return `${baseplaceholder bg-purple-200/60 text-purple-800 dark:bg-purple-800/40 dark:text-purple-300`
  placeholder
  if (props.platform === 'grok') {
    return `${baseplaceholder bg-zinc-300/70 text-zinc-800 dark:bg-zinc-700/60 dark:text-zinc-200`
  placeholder
  if (props.platform === 'kimi') {
    return `${baseplaceholder bg-pink-200/60 text-pink-800 dark:bg-pink-800/40 dark:text-pink-300`
  placeholder
  if (props.platform === 'zhipu') {
    return `${baseplaceholder bg-indigo-200/60 text-indigo-800 dark:bg-indigo-800/40 dark:text-indigo-300`
  placeholder
  if (props.platform === 'deepseek') {
    return `${baseplaceholder bg-teal-200/60 text-teal-800 dark:bg-teal-800/40 dark:text-teal-300`
  placeholder
  if (props.platform === 'composite') {
    return `${baseplaceholder bg-cyan-200/70 text-cyan-900 dark:bg-cyan-900/50 dark:text-cyan-300`
  placeholder
