<template>
  <div class="space-y-5">
    ...
    <div v-if="!embedded">
      <h1 class="text-2xl font-bold tracking-tight text-gray-900 dark:text-white sm:text-3xl">{{ t('modelPlaza.title') placeholderplaceholder</h1>
      <p class="mt-1.5 text-sm text-gray-500 dark:text-dark-400">{{ t('modelPlaza.description') placeholderplaceholder</p>
    </div>
    ...
    >
      ...
      {{ t('modelPlaza.anonymousHint') placeholderplaceholder
    </p>
    ...
    >
      {{ t('modelPlaza.loadFailed') placeholderplaceholder
    </div>
    <template v-else>
      <!-- 筛选区:平台 → 分组 → 倍率 -->
      <PlazaFilterBar
      ...
      >
        {{ searchActive ? t('modelPlaza.noSearchResult') : t('modelPlaza.empty') placeholderplaceholder
      </div>
    </template>
  </div>
</template>
...
import type { ModelPlazaGroup, ModelPlazaResponse placeholder from '@/api/modelPlaza'
...
const filteredGroups = computed(() => {
  ...
  if (q) {
    groups = groups
      .map((g) => ({ ...g, models: g.models.filter((m) => m.name.toLowerCase().includes(q)) placeholder))
