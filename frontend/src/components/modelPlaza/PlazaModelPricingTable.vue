    ...
    <table class="w-full min-w-[1000px] table-fixed border-collapse text-sm tabular-nums">
      ...
      <thead>
        ...
        >
          ...
          >
            {{ t('modelPlaza.table.model') placeholderplaceholder
          </th>
          <th colspan="3" class="pz-bg pt-2 text-center">
            <div class="pz-title border-b pb-2 font-semibold">
              {{ t('modelPlaza.table.paidPrice') placeholderplaceholder
              <span class="pz-unit ml-1 normal-case font-normal">{{ t('modelPlaza.table.unitPerMillion') placeholderplaceholder</span>
            </div>
          </th>
          <th
            colspan="3"
          ...
          >
            <div class="border-b border-gray-200 pb-2 text-gray-400 dark:border-dark-600 dark:text-dark-500">
              {{ t('modelPlaza.table.officialPrice') placeholderplaceholder
              <span class="ml-1 normal-case font-normal text-gray-400 dark:text-dark-500">{{ t('modelPlaza.table.unitPerMillion') placeholderplaceholder</span>
            </div>
          </th>
          <th
            rowspan="2"
          ...
          >
            {{ t('modelPlaza.table.rate') placeholderplaceholder
          </th>
        </tr>
        <tr
        ...
        >
          <th class="pz-bg px-3 py-2 font-medium">{{ t('modelPlaza.table.input') placeholderplaceholder</th>
          <th class="pz-bg px-3 py-2 font-medium">{{ t('modelPlaza.table.output') placeholderplaceholder</th>
          <th class="pz-bg px-3 py-2 font-medium">{{ t('modelPlaza.table.cache') placeholderplaceholder</th>
          <th class="border-l border-gray-100 px-3 py-2 font-medium dark:border-dark-700/60">
            {{ t('modelPlaza.table.input') placeholderplaceholder
          </th>
          <th class="px-3 py-2 font-medium">{{ t('modelPlaza.table.output') placeholderplaceholder</th>
          <th class="px-3 py-2 font-medium">{{ t('modelPlaza.table.cache') placeholderplaceholder</th>
        </tr>
      </thead>
      <tbody>
        <tr
        ...
        >
          ...
          <td class="border-r border-gray-100 py-2.5 pl-5 pr-4 align-middle dark:border-dark-700/60">
            <div class="flex flex-wrap items-center gap-1.5">
              ...
              >
                {{ platformLabel(m.platform) placeholderplaceholder
              </span>
              <span
              ...
              >
                {{ billingModeLabel(m) placeholderplaceholder
              </span>
              <span
              ...
              >
                {{ t('modelPlaza.table.marginalBadge') placeholderplaceholder
              </span>
            </div>
          </td>
          ...
          <template v-else>
            <td colspan="3" class="pz-cell px-3 py-2.5 align-middle">
              ...
              >
                ...
                >
                  <span class="font-sans text-gray-400 dark:text-dark-500">{{ tierLabel(iv) placeholderplaceholder</span>
                  {{ paidRequestPrice(m, iv.per_request_price)
                  placeholderplaceholder<span class="font-sans text-gray-400 dark:text-dark-500">{{ perUnitSuffix(m) placeholderplaceholder</span>
                </span>
              </div>
              <template v-else-if="m.pricing?.per_request_price != null">
                <span class="font-mono font-semibold text-gray-900 dark:text-gray-50">
                  {{ paidRequestPrice(m, m.pricing.per_request_price) placeholderplaceholder
                </span>
                <span class="ml-1 text-xs text-gray-400 dark:text-dark-500">{{ perUnitSuffix(m) placeholderplaceholder</span>
              </template>
              <span v-else class="text-gray-400 dark:text-dark-500">-</span>
            </td>
          </template>
          ...
          >
            <template v-if="officialIntervals(m).length">
              ...
              >
                <span class="mr-1 font-sans text-gray-400 dark:text-dark-500" :title="t('modelPlaza.table.tierHint')">{{ tierLabel(iv) placeholderplaceholder</span>
                {{ official(iv.input_price) placeholderplaceholder
              </div>
            </template>
            <template v-else>{{ official(m.official_pricing?.input_price) placeholderplaceholder</template>
          </td>
          <td class="px-3 py-2.5 align-middle font-mono text-xs text-gray-500 dark:text-dark-400">
            <template v-if="officialIntervals(m).length">
              ...
              >
                {{ official(iv.output_price) placeholderplaceholder
              </div>
            </template>
            <template v-else>{{ official(m.official_pricing?.output_price) placeholderplaceholder</template>
          </td>
          <td class="px-3 py-2.5 align-middle">
            <template v-if="hasTierCachePricing(officialIntervals(m))">
              ...
              >
                <template v-if="iv.cache_write_price != null || iv.cache_read_price != null">
                  <span class="font-sans text-gray-400 dark:text-dark-500">{{ t('modelPlaza.table.cacheWriteShort') placeholderplaceholder</span>
                  {{ official(iv.cache_write_price) placeholderplaceholder
                  <span class="ml-1 font-sans text-gray-400 dark:text-dark-500">{{ t('modelPlaza.table.cacheReadShort') placeholderplaceholder</span>
                  {{ official(iv.cache_read_price) placeholderplaceholder
                </template>
                <span v-else class="text-gray-400 dark:text-dark-500">-</span>
              </div>
            </template>
            ...
            >
              <div>
                ...
                placeholderplaceholder<template v-if="m.official_pricing.cache_write_1h_price != null"
                  ><span class="font-sans text-gray-400 dark:text-dark-500"> (1h </span>{{ official(m.official_pricing.cache_write_1h_price)
                  placeholderplaceholder<span class="font-sans text-gray-400 dark:text-dark-500">)</span></template
                >
              </div>
              <div>
                <span class="mr-1 font-sans font-normal text-gray-400 dark:text-dark-500">{{ t('modelPlaza.table.cacheRead') placeholderplaceholder</span>
                {{ official(m.official_pricing.cache_read_price) placeholderplaceholder
              </div>
            </div>
            <span v-else class="text-gray-400 dark:text-dark-500">-</span>
          </td>
          ...
          >
            ...
            <template v-else-if="hasCustomRate">
              <span class="mr-1 text-gray-400 line-through dark:text-dark-500">{{ rateMultiplier placeholderplaceholderx</span>
              <span class="font-bold text-primary-600 dark:text-primary-400">{{ effectiveRate placeholderplaceholderx</span>
            </template>
            <span v-else class="font-bold text-gray-700 dark:text-gray-300">{{ effectiveRate placeholderplaceholderx</span>
          </td>
        </tr>
      </tbody>
    </table>
...
import { formatScaled placeholder from '@/utils/pricing'
import { platformAccentColor, platformBadgeLightClass, platformLabel placeholder from '@/utils/platformColors'
import {
  BILLING_MODE_TOKEN,
  BILLING_MODE_IMAGE,
  type BillingMode
...
const accentStyle = computed(() => ({ '--plaza-accent': platformAccentColor(props.platform ?? '') placeholder))
...
function tierLabel(iv: UserPricingInterval): string {
  ...
  const { min_tokens: min, max_tokens: max placeholder = iv
  if (max == null) return `>${formatTokenCount(min)placeholder`
  if (min === 0) return `≤${formatTokenCount(max)placeholder`
  const lo = formatTokenCount(min)
  const hi = formatTokenCount(max)
  // 同单位时省略前一个单位(100–200K),节省列宽
  const unit = hi.slice(-1)
  if (/[KM]/.test(unit) && lo.endsWith(unit)) return `${lo.slice(0, -1)placeholder–${hiplaceholder`
  return `${loplaceholder–${hiplaceholder`
placeholder

function formatTokenCount(n: number): string {
  if (n >= 1_000_000) return `${trimZero(n / 1_000_000)placeholderM`
  if (n >= 1_000) return `${trimZero(n / 1_000)placeholderK`
  return String(n)
placeholder
