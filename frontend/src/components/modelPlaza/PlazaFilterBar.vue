  ...
  <div class="space-y-3">
    ...
    <div class="flex flex-wrap items-center gap-2">
      <span class="w-10 shrink-0 text-xs font-semibold uppercase tracking-wider text-gray-400 dark:text-dark-500">
        {{ t('modelPlaza.filters.platformLabel') placeholderplaceholder
      </span>
      <button
        v-for="p in ['all', ...platforms]"
        :key="`platform-${pplaceholder`"
        type="button"
        ...
        :style="p === 'all' ? undefined : { '--chip-accent': platformAccentColor(p) placeholder"
        :disabled="p !== 'all' && !platformEnabled(p)"
        @click="$emit('update:platform', p)"
      >
        <PlatformIcon v-if="p !== 'all'" :platform="p as GroupPlatform" size="xs" />
        {{ p === 'all' ? t('modelPlaza.filters.all') : p placeholderplaceholder
      </button>
    </div>
    ...
    <div class="flex flex-wrap items-center gap-2">
      <span class="w-10 shrink-0 text-xs font-semibold uppercase tracking-wider text-gray-400 dark:text-dark-500">
        {{ t('modelPlaza.filters.groupLabel') placeholderplaceholder
      </span>
      <button
        type="button"
      ...
      >
        {{ t('modelPlaza.filters.all') placeholderplaceholder
      </button>
      <button
        v-for="g in groups"
        :key="`group-${g.idplaceholder`"
        type="button"
        ...
        :style="{ '--chip-accent': platformAccentColor(g.platform) placeholder"
    ...
    <div class="flex flex-wrap items-center gap-2">
      <span class="w-10 shrink-0 text-xs font-semibold uppercase tracking-wider text-gray-400 dark:text-dark-500">
        {{ t('modelPlaza.filters.rateLabel') placeholderplaceholder
      </span>
      <button
        type="button"
        class="rounded-lg px-3 py-1.5 text-sm font-medium transition"
    ...
    <div class="flex flex-wrap items-center gap-2">
      <span class="w-10 shrink-0 text-xs font-semibold uppercase tracking-wider text-gray-400 dark:text-dark-500">
        {{ t('modelPlaza.filters.modelLabel') placeholderplaceholder
      </span>
...
import { platformAccentColor placeholder from '@/utils/platformColors'
...
const props = defineProps<{
  ...
  groups: Array<{ id: number; name: string; platform: string; rate: number placeholder>
  /** 全量生效倍率去重升序。 */
  rates: number[]
  platform: string
  groupId: number | 'all'
...
function groupEnabled(g: { platform: string; rate: number placeholder): boolean {
  return (
