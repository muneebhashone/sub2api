...
import type { UserAvailableChannel placeholder from '@/api/channels'
    ...
    platforms: [
      {
        ...
        supported_models: [{ name: 'claude-test', platform: 'anthropic', pricing: null placeholder],
      placeholder,
    ],
  placeholder,
]
...
const baseProps = {
  ...
  userGroupRates: { 1: 0.8 placeholder,
placeholder

function mountTable(props = {placeholder) {
  return mount(AvailableChannelsTable, {
    props: { ...baseProps, ...props placeholder,
    global: {
      plugins: [createPinia()],
      stubs: {
        Icon: { props: ['name'], template: '<i :data-icon="name" />' placeholder,
        PlatformIcon: { template: '<i data-platform-icon />' placeholder,
        GroupBadge: {
          props: ['name', 'rateMultiplier', 'userRateMultiplier'],
          template:
            '<span data-group-badge>{{ name placeholderplaceholder:{{ rateMultiplier placeholderplaceholder:{{ userRateMultiplier placeholderplaceholder</span>',
        placeholder,
        SupportedModelChip: {
          props: ['model', 'noPricingLabel'],
          template: '<span data-model-chip>{{ model.name placeholderplaceholder:{{ noPricingLabel placeholderplaceholder</span>',
        placeholder,
      placeholder,
    placeholder,
  placeholder)
...
describe('AvailableChannelsTable responsive surfaces', () => {
      ...
      rows: [
        {
          ...
          description: '',
          platforms: [{ platform: 'openai', groups: [], supported_models: [] placeholder],
        placeholder,
      ],
    placeholder)
    const mobile = wrapper.get('[data-testid="mobile-channels"]')
  ...
  it('provides loading and empty states on both responsive surfaces', async () => {
    const wrapper = mountTable({ loading: true, rows: [] placeholder)
    ...
    await wrapper.setProps({ loading: false placeholder)
