  ...
  <div class="table-wrapper">
    ...
    >
      <thead>
        <tr class="border-b border-gray-100 bg-gray-50/50 text-xs font-medium uppercase tracking-wide text-gray-500 dark:border-dark-700 dark:bg-dark-800/50 dark:text-gray-400">
          <th class="w-[180px] px-4 py-3 text-center">{{ columns.name placeholderplaceholder</th>
          <th class="w-[200px] px-4 py-3 text-left">{{ columns.description placeholderplaceholder</th>
          <th class="w-[140px] px-4 py-3 text-left">{{ columns.platform placeholderplaceholder</th>
          <th class="px-4 py-3 text-left">{{ columns.groups placeholderplaceholder</th>
          <th class="px-4 py-3 text-left">{{ columns.supportedModels placeholderplaceholder</th>
        </tr>
      </thead>
      <tbody v-if="loading">
        <tr>
      ...
      >
        ...
        >
          ...
          >
            {{ channel.name placeholderplaceholder
          </td>
          ...
          >
            <template v-if="channel.description">{{ channel.description placeholderplaceholder</template>
          ...
          <td class="align-top px-4 py-3">
            ...
            >
              ...
              {{ section.platform placeholderplaceholder
            </span>
          </td>
            ...
            <div class="flex flex-col gap-1.5">
              ...
              >
                ...
                >
                  ...
                  {{ t('availableChannels.exclusive') placeholderplaceholder
                </span>
                <div
                  v-for="g in exclusiveGroups(section)"
                  :key="`ex-${g.idplaceholder`"
                ...
                >
                  ...
                  >
                    ...
                    {{ peakRateLabel(g) placeholderplaceholder
                  </span>
                </div>
              </div>
              <div
              ...
              >
                ...
                >
                  ...
                  {{ t('availableChannels.public') placeholderplaceholder
                </span>
                <div
                  v-for="g in publicGroups(section)"
                  :key="`pub-${g.idplaceholder`"
                  class="inline-flex flex-wrap items-center gap-1"
                >
                  <GroupBadge
                    :name="g.name"
          ...
          <td class="align-top px-4 py-3">
            <div class="flex flex-wrap gap-1">
              <SupportedModelChip
                ...
                :key="`${section.platformplaceholder-${m.nameplaceholder`"
                :model="m"
    ...
    <div data-testid="mobile-channels" class="w-full min-w-0 overflow-x-hidden lg:hidden">
      ...
      <section
        v-else
        v-for="(channel, chIdx) in rows"
        :key="`mobile-${channel.nameplaceholder-${chIdxplaceholder`"
      ...
      >
        <header class="mb-3 min-w-0">
          ...
          <p class="mt-1 break-words text-xs leading-5 text-gray-500 dark:text-gray-400">
            {{ channel.description || '-' placeholderplaceholder
          </p>
        </header>
        ...
        <div class="divide-y divide-gray-100 dark:divide-dark-700/60">
          <div
            ...
            :key="`mobile-${channel.nameplaceholder-${section.platformplaceholder`"
            ...
            <dl class="mt-3 space-y-3">
              <div class="min-w-0">
                <dt class="mb-1.5 text-[11px] font-medium text-gray-500 dark:text-gray-400">
                  {{ columns.groups placeholderplaceholder
                </dt>
                <dd class="flex min-w-0 flex-col gap-2">
                  ...
                  >
                    ...
                    <div
                      v-for="g in publicGroups(section)"
                      :key="`mobile-pub-${g.idplaceholder`"
                      class="inline-flex max-w-full min-w-0 flex-wrap items-center gap-1"
                    >
                      <GroupBadge
                        class="max-w-full"
              ...
              <div class="min-w-0">
                <dt class="mb-1.5 text-[11px] font-medium text-gray-500 dark:text-gray-400">
                  {{ columns.supportedModels placeholderplaceholder
                </dt>
                <dd class="flex min-w-0 flex-wrap gap-1">
                  <SupportedModelChip
                    v-for="m in section.supported_models"
                    :key="`mobile-${section.platformplaceholder-${m.nameplaceholder`"
...
import type { UserAvailableChannel, UserAvailableGroup, UserChannelPlatformSection placeholder from '@/api/channels'
import type { GroupPlatform, SubscriptionType placeholder from '@/types'
import { platformBadgeClass placeholder from '@/utils/platformColors'
...
function peakRateTitle(group: UserAvailableGroup): string {
  return t('common.peakRateTooltip', { window: peakRateLabel(group) placeholder) + t('common.peakRateImageNote')
placeholder
</script>
