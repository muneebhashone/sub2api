  ...
  <DataTable :columns="columns" :data="rows" :loading="loading">
    <template #cell-name="{ row placeholder">
      <div class="flex items-center gap-2">
        <span class="font-medium text-gray-900 dark:text-white">{{ row.name placeholderplaceholder</span>
        <span
          v-if="row.platform"
          :class="[
        ...
        >
          ...
          {{ row.platform placeholderplaceholder
        </span>
      </div>
    </template>
...
import type { ChannelStatus, BillingModelSource placeholder from '@/constants/channel'
