<template>
  <div class="relative inline-block">
    ...
    >
      ...
      >
        {{ model.platform placeholderplaceholder
      </span>
      {{ model.name placeholderplaceholder
    </span>
    ...
    <Teleport to="body">
      ...
      >
        ...
        >
          <span class="truncate font-semibold">{{ model.name placeholderplaceholder</span>
          <span
            v-if="model.platform"
        ...
        <div class="p-3">
          <div v-if="!model.pricing" class="text-gray-500 dark:text-gray-400">
            {{ noPricingLabel placeholderplaceholder
          </div>

          <div v-else class="space-y-2 text-gray-700 dark:text-gray-300">
            <div class="flex justify-between">
              <span class="text-gray-500 dark:text-gray-400">{{ t(prefixKey('billingMode')) placeholderplaceholder</span>
              <span>{{ billingModeLabel placeholderplaceholder</span>
            </div>
            ...
            >
              <div class="mb-1 font-medium text-gray-600 dark:text-gray-400">
                {{ t(prefixKey('intervals')) placeholderplaceholder
              </div>
              <div class="space-y-1">
                <div
                ...
                >
                  <span class="text-gray-500 dark:text-gray-400">
                    <template v-if="iv.tier_label">{{ iv.tier_label placeholderplaceholder</template>
                    <template v-else>{{ formatRange(iv.min_tokens, iv.max_tokens) placeholderplaceholder</template>
                  </span>
                  <span>{{ formatInterval(iv, model.pricing.billing_mode) placeholderplaceholder</span>
                </div>
              </div>
            </div>
          </div>
...
import { computed, nextTick, onBeforeUnmount, ref placeholder from 'vue'
...
import type { UserPricingInterval, UserSupportedModel placeholder from '@/api/channels'
import PlatformIcon from '@/components/common/PlatformIcon.vue'
import type { GroupPlatform placeholder from '@/types'
import { platformBadgeClass, platformBorderClass, platformBadgeLightClass placeholder from '@/utils/platformColors'
...
function prefixKey(k: string): string {
  return `${props.pricingKeyPrefixplaceholder.${kplaceholder`
placeholder
...
function formatRange(min: number, max: number | null): string {
  ...
  return `(${minplaceholder, ${maxLabelplaceholder]`
placeholder
...
function formatInterval(iv: UserPricingInterval, mode: BillingMode): string {
  ...
  return `${inputplaceholder / ${outputplaceholder`
placeholder
...
const popoverStyle = ref<Record<string, string>>({ top: '0px', left: '0px' placeholder)
...
function updatePosition() {
  ...
  popoverStyle.value = {
    top: `${Math.round(top)placeholderpx`,
    left: `${Math.round(left)placeholderpx`,
  placeholder
placeholder
