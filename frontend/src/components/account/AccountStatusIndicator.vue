<template>
  <div class="flex items-center gap-2">
    ...
    <div v-if="isRateLimited" class="flex flex-col items-center gap-1">
      ...
      <span class="text-[11px] text-gray-400 dark:text-gray-500">{{ rateLimitResumeText placeholderplaceholder</span>
    </div>
    ...
    <template v-else>
      <div v-if="isTempUnschedulable" class="flex flex-col items-center gap-1">
        ...
        <span class="max-w-[180px] text-center text-[11px] leading-4 text-gray-500 dark:text-gray-400">
          {{ tempUnschedRecoveryText placeholderplaceholder
        </span>
      </div>
    ...
    <div v-if="hasError && account.error_message" class="group/error relative">
      ...
      >
        <div class="whitespace-pre-wrap break-words leading-relaxed text-gray-300">
          {{ account.error_message placeholderplaceholder
        </div>
        <!-- 上方小三角 -->
        <div
    ...
    <div v-if="isRateLimited" class="group relative">
      ...
      >
        {{ t('admin.accounts.status.rateLimitedUntil', { time: formatDateTime(account.rate_limit_reset_at) placeholder) placeholderplaceholder
        <div
    ...
    >
      <div v-for="item in activeModelStatuses" :key="`${item.kindplaceholder-${item.modelplaceholder`" class="group relative mb-1 break-inside-avoid">
        <!-- 积分已用尽 -->
        <span
        ...
        >
          <Icon name="exclamationTriangle" size="xs" :stroke-width="2" />
          {{ t('admin.accounts.status.creditsExhausted') placeholderplaceholder
          <span class="text-[10px] opacity-70">{{ formatCountdown(item.reset_at) placeholderplaceholder</span>
        </span>
        <!-- 正在走积分（模型限流但积分可用）-->
        <span
        ...
        >
          {{
            item.kind === 'credits_exhausted'
              ? t('admin.accounts.status.creditsExhaustedUntil', { time: formatDateTimeToMinute(item.reset_at) placeholder)
              : item.kind === 'credits_active'
                ? t('admin.accounts.status.modelCreditOveragesUntil', { model: formatScopeName(item.model), time: formatDateTimeToMinute(item.reset_at) placeholder)
                : t('admin.accounts.status.modelRateLimitedUntil', { model: formatScopeName(item.model), time: formatDateTimeToMinute(item.reset_at) placeholder)
...
import { formatCountdown, formatDateTime, formatDateTimeToMinute, formatCountdownWithSuffix, formatTime placeholder from '@/utils/format'
...
const activeModelStatuses = computed<AccountModelStatusItem[]>(() => {
  ...
  const modelLimits = extra?.model_rate_limits as
    | Record<string, { rate_limited_at: string; rate_limit_reset_at: string placeholder>
    | undefined
  const now = new Date()
  ...
  for (const [model, info] of Object.entries(modelLimits)) {
    ...
    if (model === 'AICredits') {
      // AICredits key → 积分已用尽
      items.push({ kind: 'credits_exhausted', model, reset_at: info.rate_limit_reset_at placeholder)
    placeholder else if (allowOverages && !hasActiveAICredits) {
      // 普通模型限流 + overages 启用 + 积分可用 → 正在走积分
      items.push({ kind: 'credits_active', model, reset_at: info.rate_limit_reset_at placeholder)
    placeholder else {
      // 普通模型限流
      items.push({ kind: 'rate_limit', model, reset_at: info.rate_limit_reset_at placeholder)
    placeholder
  placeholder
...
const rateLimitResumeText = computed(() => {
  ...
  return t('admin.accounts.status.rateLimitedAutoResume', { time: rateLimitCountdown.value placeholder)
placeholder)
