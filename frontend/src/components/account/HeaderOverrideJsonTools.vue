<template>
  ...
  >
    {{ t('admin.accounts.headerOverride.importJson') placeholderplaceholder
  </button>
  <button
    type="button"
    ...
    @click="copyAsJson"
  >
    {{ t('admin.accounts.headerOverride.copyJson') placeholderplaceholder
  </button>
  ...
  <div v-if="showImportPanel" ref="importPanelRef" class="w-full space-y-2">
    ...
    <div class="flex gap-2">
      <button
        ...
        @click="applyImport"
      >
        {{ t('admin.accounts.headerOverride.importJsonApply') placeholderplaceholder
      </button>
      <button
        type="button"
      ...
      >
        {{ t('admin.accounts.headerOverride.importJsonCancel') placeholderplaceholder
      </button>
    </div>
    <p class="text-xs text-gray-500 dark:text-gray-400">
      {{ t('admin.accounts.headerOverride.importJsonHint') placeholderplaceholder
    </p>
  </div>
</template>

<script setup lang="ts">
import { computed, nextTick, ref placeholder from 'vue'
...
const IMPORT_JSON_PLACEHOLDER = '{"user-agent": "my-client/1.0", "x-relay-token": "..."placeholder'
...
const toggleImportPanel = async () => {
  ...
  await nextTick()
  importPanelRef.value?.scrollIntoView({ block: 'nearest', behavior: 'smooth' placeholder)
  importTextareaRef.value?.focus({ preventScroll: true placeholder)
placeholder
