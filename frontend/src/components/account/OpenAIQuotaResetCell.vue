  ...
  <div v-if="visible" class="space-y-1">
    ...
    <div class="flex flex-wrap items-center gap-1.5">
      ...
      >
        ...
        {{ t('admin.accounts.openaiQuotaReset.count') placeholderplaceholder<span v-if="data"> {{ availableResetCount placeholderplaceholder</span>
      </button>
      ...
      >
        <svg
          class="h-2.5 w-2.5"
          :class="{ 'animate-spin': resetting placeholder"
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
        ...
        {{ t('admin.accounts.openaiQuotaReset.reset') placeholderplaceholder
      </button>
    </div>
    ...
    >
      ...
      >
        {{ autoResetStateLabel placeholderplaceholder
        <span v-if="autoResetState.trigger_window" class="ml-1 tabular-nums">
          {{ autoResetState.trigger_window placeholderplaceholder
        </span>
      </span>
      <span v-if="autoResetState.checked_at" class="text-gray-500 dark:text-gray-400">
        {{ formatResetCreditExpiry(autoResetState.checked_at, 'short') placeholderplaceholder
      </span>
      <span
      ...
      >
        {{ autoResetState.error_code placeholderplaceholder
      </span>
    </div>
    ...
    <div v-if="primaryResetCreditExpiry" class="space-y-1">
      <div class="flex flex-wrap items-center gap-1">
        <span
          ...
          :title="t('admin.accounts.openaiQuotaReset.expiresAtFull', { time: formatResetCreditExpiry(primaryResetCreditExpiry, 'full') placeholder)"
        >
          {{ t('admin.accounts.openaiQuotaReset.expiresAt', { time: formatResetCreditExpiry(primaryResetCreditExpiry, 'short') placeholder) placeholderplaceholder
        </span>
        <button
        ...
        >
          +{{ hiddenResetCreditCount placeholderplaceholder
        </button>
      </div>
      ...
      >
        <span class="sr-only">{{ t('admin.accounts.openaiQuotaReset.expirationDetails') placeholderplaceholder</span>
        <span
          v-for="(expiresAt, index) in resetCreditExpirations"
          :key="`${expiresAtplaceholder-${indexplaceholder`"
          class="flex min-w-0 items-center gap-1 tabular-nums"
          :title="t('admin.accounts.openaiQuotaReset.expiresAtFull', { time: formatResetCreditExpiry(expiresAt, 'full') placeholder)"
        >
          <span class="h-1 w-1 shrink-0 rounded-full bg-gray-400 dark:bg-dark-500" />
          <span class="truncate">{{ formatResetCreditExpiry(expiresAt, 'short') placeholderplaceholder</span>
        </span>
      </div>
    </div>
    ...
    >
      {{ resetWarning placeholderplaceholder
    </div>
    <div
      v-else-if="resetMessage"
      class="text-[10px] text-emerald-600 dark:text-emerald-400"
    >
      {{ resetMessage placeholderplaceholder
    </div>

    <ConfirmDialog
      :show="showResetConfirm"
      :title="t('admin.accounts.openaiQuotaReset.confirmTitle')"
      :message="t('admin.accounts.openaiQuotaReset.confirmMessage', { count: availableResetCount placeholder)"
...
const autoResetStateLabel = computed(() => {
  ...
  const keyByStatus: Record<string, string> = {
    ...
    failed: 'failed'
  placeholder
  return t(`admin.accounts.openaiQuotaReset.autoStatus.${keyByStatus[autoResetState.value.status]placeholder`)
placeholder)
...
const readCachedResetCredits = (account: Account): OpenAIQuotaUsage | null => {
  ...
  const { available_count: count, credits: rawCredits placeholder = cached as {
  ...
  const now = Date.now()
  const credits: { expires_at?: string placeholder[] = []
  if (Array.isArray(rawCredits)) {
    for (const credit of rawCredits) {
      if (!credit || typeof credit !== 'object') continue
      const expiresAt = (credit as { expires_at?: unknown placeholder).expires_at
      ...
      credits.push({ expires_at: expiresAt placeholder)
    placeholder
  placeholder
...
const resetCreditDetailsToggleLabel = computed(() => {
  ...
  placeholder
  return t('admin.accounts.openaiQuotaReset.expandExpirations', { count: hiddenResetCreditCount.value placeholder)
placeholder)
...
const extractErrorMessage = (e: unknown): string => {
  ...
  // errors into { status, code, message, reason, ... placeholder and re-rejects them, so
...
const confirmReset = async () => {
  ...
  try {
    ...
    placeholder else if (result.warning_code === 'account_state_recovery_failed') {
      resetWarning.value = t('admin.accounts.openaiQuotaReset.resetAccountRecoveryFailed')
    placeholder else if (result.warning_code === 'account_state_refresh_failed') {
