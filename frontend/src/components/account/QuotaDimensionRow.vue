...
const props = defineProps<{
  ...
  hourOptions: number[]
  dayOptions: { value: number; key: string placeholder[]
placeholder>()
...
<template>
  <div>
    ...
    <div v-if="quotaNotifyGlobalEnabled" class="flex items-center gap-2 mb-1">
      <span class="text-xs font-medium text-gray-700 dark:text-gray-300 flex-1 min-w-0">{{ label placeholderplaceholder</span>
      <span v-if="limit && limit > 0" class="text-xs font-medium text-gray-700 dark:text-gray-300 flex-1 min-w-0">{{ t('admin.accounts.quotaNotify.alert') placeholderplaceholder</span>
    </div>
    <label v-else class="text-xs font-medium text-gray-700 dark:text-gray-300 mb-1 block">{{ label placeholderplaceholder</label>
    ...
    <div v-if="hasResetMode" class="mt-1 flex items-center gap-2 flex-wrap">
      ...
      <span class="text-[11px] text-gray-500 dark:text-gray-400">
        <template v-if="resetMode === 'fixed'">{{ hintFixed placeholderplaceholder</template>
        <template v-else>{{ hintRolling placeholderplaceholder</template>
      </span>
    </div>

    <!-- Total dimension hint (no reset mode) -->
    <p v-if="!hasResetMode" class="input-hint mb-0 text-[11px]">{{ hintRolling placeholderplaceholder</p>
  </div>
</template>
