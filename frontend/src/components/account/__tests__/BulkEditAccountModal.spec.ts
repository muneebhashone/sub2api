...
const { showError, showSuccess, translate placeholder = vi.hoisted(() => ({
  showError: vi.fn(),
  showSuccess: vi.fn(),
...
describe('BulkEditAccountModal', () => {
  ...
  it('长上下文设置独立启用并提交布尔值', async () => {
    ...
    expect(adminAPI.accounts.bulkUpdate).toHaveBeenLastCalledWith([1, 2], {
      extra: { openai_long_context_billing_enabled: true placeholder
    placeholder)
    ...
    expect(adminAPI.accounts.bulkUpdate).toHaveBeenCalledWith([1, 2], {
      extra: { openai_long_context_billing_enabled: false placeholder
    placeholder)
  placeholder)
  ...
  it('端点能力默认值提交 null，表示恢复两个默认端点', async () => {
    ...
    expect(adminAPI.accounts.bulkUpdate).toHaveBeenCalledWith([1, 2], {
      credentials: { openai_capabilities: null placeholder
    placeholder)
  placeholder)
  ...
  it('Responses 路由独立启用，auto 提交 null，强制模式提交明确值', async () => {
    ...
    await flushPromises()
    expect(adminAPI.accounts.bulkUpdate).toHaveBeenLastCalledWith([1, 2], {
      extra: { openai_responses_mode: null placeholder
    placeholder)
    autoWrapper.unmount()
    ...
    await flushPromises()
    expect(adminAPI.accounts.bulkUpdate).toHaveBeenCalledWith([1, 2], {
      extra: { openai_responses_mode: 'force_responses' placeholder
    placeholder)
  placeholder)
  ...
  it('仅启用 Embeddings 时恢复 Responses 自动模式并精确提交联动字段', async () => {
    ...
    await flushPromises()
    expect(adminAPI.accounts.bulkUpdate).toHaveBeenCalledWith([1, 2], {
      credentials: { openai_capabilities: ['embeddings'] placeholder,
      extra: { openai_responses_mode: null placeholder
    placeholder)
  placeholder)
  ...
  it('目标变化后不提交已经隐藏的 OpenAI 设置', async () => {
    ...
    await wrapper.get('#bulk-edit-openai-endpoint-capabilities-enabled').setValue(true)
    await wrapper.get('#bulk-edit-openai-responses-mode-enabled').setValue(true)
    await wrapper.setProps({ selectedPlatforms: ['anthropic'], selectedTypes: ['apikey'] placeholder)
  ...
  it('筛选全量模式固定展示影子继承说明并按 filters 提交', async () => {
    const wrapper = mountModal({
      ...
      target: {
        mode: 'filtered',
        filters: { platform: 'openai', type: 'oauth', status: 'active' placeholder,
        previewCount: 20,
        selectedPlatforms: ['openai'],
        selectedTypes: ['oauth']
      placeholder
  ...
  it('筛选结果批量编辑可统一开启上游倍率自动探测', async () => {
    const wrapper = mountModal({
      ...
      target: {
        mode: 'filtered',
        filters: { platform: 'openai', type: 'apikey', status: 'active' placeholder,
        previewCount: 20,
        selectedPlatforms: ['openai'],
        selectedTypes: ['apikey']
      placeholder
  ...
  it('筛选 OpenAI 账号批量编辑应提交 Compact 模式和专属模型映射', async () => {
    const wrapper = mountModal({
      ...
      target: {
        mode: 'filtered',
        filters: { platform: 'openai' placeholder,
        previewCount: 12,
        selectedPlatforms: ['openai'],
