...
<template>
  <div class="rounded-lg border border-gray-200 dark:border-dark-600">
      ...
      <div class="flex items-center justify-between p-4" :class="{ 'pb-0': localEnabled && !collapsed placeholder">
        <div class="flex items-center gap-2 flex-1 cursor-pointer" @click="localEnabled && (collapsed = !collapsed)">
          <svg v-if="localEnabled" class="h-4 w-4 text-gray-400 transition-transform" :class="{ '-rotate-90': collapsed placeholder" viewBox="0 0 20 20" fill="currentColor">
            <path fill-rule="evenodd" d="M5.23 7.21a.75.75 0 011.06.02L10 11.168l3.71-3.938a.75.75 0 111.08 1.04l-4.25 4.5a.75.75 0 01-1.08 0l-4.25-4.5a.75.75 0 01.02-1.06z" clip-rule="evenodd" />
          </svg>
          <div>
            <label class="input-label mb-0 cursor-pointer">{{ t('admin.accounts.quotaLimitToggle') placeholderplaceholder</label>
      ...
      <div v-if="localEnabled && !collapsed" class="space-y-2 p-4 pt-3">
        ...
        <div>
          ...
          <div class="flex items-center gap-2 mb-1">
            ...
            <span v-if="quotaNotifyGlobalEnabled && dailyLimit && dailyLimit > 0" class="text-xs font-medium text-gray-700 dark:text-gray-300">{{ t('admin.accounts.quotaNotify.alert') placeholderplaceholder</span>
          </div>
          <!-- 输入行 -->
        ...
        <div>
          <div class="flex items-center gap-2 mb-1">
            ...
            <span v-if="quotaNotifyGlobalEnabled && weeklyLimit && weeklyLimit > 0" class="text-xs font-medium text-gray-700 dark:text-gray-300">{{ t('admin.accounts.quotaNotify.alert') placeholderplaceholder</span>
          </div>
          <div class="flex items-center gap-2">
            <div class="relative w-28 flex-shrink-0">
              <span class="absolute left-2.5 top-1/2 -translate-y-1/2 text-gray-500 dark:text-gray-400 text-sm">$</span>
        ...
        <div>
          <div class="flex items-center gap-2 mb-1">
            ...
            <span v-if="quotaNotifyGlobalEnabled && totalLimit && totalLimit > 0" class="text-xs font-medium text-gray-700 dark:text-gray-300">{{ t('admin.accounts.quotaNotify.alert') placeholderplaceholder</span>
          </div>
          <div class="flex items-center gap-2">
            <div class="relative w-28 flex-shrink-0">
              <span class="absolute left-2.5 top-1/2 -translate-y-1/2 text-gray-500 dark:text-gray-400 text-sm">$</span>
