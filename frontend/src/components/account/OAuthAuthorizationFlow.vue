<template>
  ...
  >
      ...
      <div class="flex-1">
        <h4 class="mb-3 font-semibold text-blue-900 dark:text-blue-200">{{ oauthTitle placeholderplaceholder</h4>

        <!-- Auth Method Selection -->
        <div v-if="showMethodSelection" class="mb-4">
          <label class="mb-2 block text-sm font-medium text-blue-800 dark:text-blue-300">
            {{ methodLabel placeholderplaceholder
          </label>
        ...
        <div v-if="inputMethod === 'refresh_token' || inputMethod === 'mobile_refresh_token'" class="space-y-4">
          ...
          >
            <p class="mb-3 text-sm text-blue-700 dark:text-blue-300">
              {{ t(getOAuthKey('refreshTokenDesc')) placeholderplaceholder
            </p>
            ...
            <div class="mb-4">
              ...
              >
                ...
                >
                  {{ t('admin.accounts.oauth.keysCount', { count: parsedRefreshTokenCount placeholder) placeholderplaceholder
                </span>
              </label>
              <textarea
              ...
              >
                {{ t('admin.accounts.oauth.batchCreateAccounts', { count: parsedRefreshTokenCount placeholder) placeholderplaceholder
              </p>
            </div>
        ...
        <div v-if="inputMethod === 'sso_cookie'" class="space-y-4">
          ...
          >
            <p class="mb-3 text-sm text-blue-700 dark:text-blue-300">
              {{ t(getOAuthKey('ssoCookieDesc')) placeholderplaceholder
            </p>
            ...
            <div class="mb-4">
              ...
              >
                <Icon name="key" size="sm" class="text-blue-500" />
                {{ t(getOAuthKey('ssoCookieLabel')) placeholderplaceholder
                <span
                  v-if="parsedSSOCount > 1"
                  class="rounded-full bg-blue-500 px-2 py-0.5 text-xs text-white"
                >
                  {{ t('admin.accounts.oauth.keysCount', { count: parsedSSOCount placeholder) placeholderplaceholder
                </span>
              </label>
              <textarea
              ...
              <p class="mt-1 text-xs text-blue-600 dark:text-blue-400">
                {{ t(getOAuthKey('ssoCookieHint')) placeholderplaceholder
              </p>
            </div>
            ...
            >
              ...
              <Icon v-else name="sparkles" size="sm" class="mr-2" />
              {{ loading ? t(getOAuthKey('convertingSSO')) : t(getOAuthKey('convertSSOAndCreate')) placeholderplaceholder
            </button>
          </div>
        </div>
        ...
        <div v-if="inputMethod === 'email_password'" class="space-y-4">
          ...
          >
            <p class="mb-3 text-sm text-blue-700 dark:text-blue-300">
              {{ t(getOAuthKey('emailPasswordDesc')) placeholderplaceholder
            </p>
            <div class="mb-4">
              <label
                class="mb-2 flex items-center gap-2 text-sm font-semibold text-gray-700 dark:text-gray-300"
              >
                ...
                {{ t(getOAuthKey('emailPasswordInputLabel')) placeholderplaceholder
                <span
                  v-if="parsedEmailPasswordCount > 1"
                  class="rounded-full bg-blue-500 px-2 py-0.5 text-xs text-white"
                >
                  {{ t('admin.accounts.oauth.keysCount', { count: parsedEmailPasswordCount placeholder) placeholderplaceholder
                </span>
              </label>
              <textarea
              ...
              ></textarea>
              <p class="mt-1 text-xs text-blue-600 dark:text-blue-400">
                {{ t(getOAuthKey('emailPasswordHint')) placeholderplaceholder
              </p>
            </div>
            <div
              v-if="error"
            ...
            >
              ...
              <Icon v-else name="sparkles" size="sm" class="mr-2" />
              {{ loading ? t(getOAuthKey('validating')) : t(getOAuthKey('validateAndCreate')) placeholderplaceholder
            </button>
          </div>
        </div>
        ...
        <div v-if="inputMethod === 'codex_session' || inputMethod === 'agent_identity'" class="space-y-4">
          ...
          >
            <p class="mb-3 text-sm text-blue-700 dark:text-blue-300">
              {{ t(isAgentIdentityInput ? 'admin.accounts.oauth.openai.agentIdentityDesc' : 'admin.accounts.oauth.openai.codexSessionDesc') placeholderplaceholder
            </p>
            ...
            <div class="mb-4">
              ...
              >
                <Icon name="key" size="sm" class="text-blue-500" />
                {{ t(isAgentIdentityInput ? 'admin.accounts.oauth.openai.agentIdentityInputLabel' : 'admin.accounts.oauth.openai.codexSessionInputLabel') placeholderplaceholder
                <span
                  v-if="parsedCodexSessionCount > 1"
                  class="rounded-full bg-blue-500 px-2 py-0.5 text-xs text-white"
                >
                  {{ t('admin.accounts.oauth.keysCount', { count: parsedCodexSessionCount placeholder) placeholderplaceholder
                </span>
              </label>
              <textarea
              ...
              ></textarea>
              <p class="mt-1 text-xs text-blue-600 dark:text-blue-400">
                {{ t(isAgentIdentityInput ? 'admin.accounts.oauth.openai.agentIdentityHint' : 'admin.accounts.oauth.openai.codexSessionHint') placeholderplaceholder
              </p>
            </div>
        ...
        <div v-if="inputMethod === 'codex_pat'" class="space-y-4">
          ...
          >
            <p class="mb-3 text-sm text-blue-700 dark:text-blue-300">
              {{ t('admin.accounts.oauth.openai.codexPatDesc') placeholderplaceholder
            </p>
            ...
            <div class="mb-4">
              ...
              >
                <Icon name="key" size="sm" class="text-blue-500" />
                {{ t('admin.accounts.oauth.openai.codexPatInputLabel') placeholderplaceholder
              </label>
              <textarea
              ...
              ></textarea>
              <p class="mt-1 text-xs text-blue-600 dark:text-blue-400">
                {{ t('admin.accounts.oauth.openai.codexPatHint') placeholderplaceholder
              </p>
            </div>
        ...
        <div v-if="inputMethod === 'cookie'" class="space-y-4">
          ...
          >
            <p class="mb-3 text-sm text-blue-700 dark:text-blue-300">
              {{ t('admin.accounts.oauth.cookieAutoAuthDesc') placeholderplaceholder
            </p>
            ...
            <div class="mb-4">
              ...
              >
                <Icon name="key" size="sm" class="text-blue-500" />
                {{ t('admin.accounts.oauth.sessionKey') placeholderplaceholder
                <span
                  v-if="parsedKeyCount > 1 && allowMultiple"
                  class="rounded-full bg-blue-500 px-2 py-0.5 text-xs text-white"
                >
                  {{ t('admin.accounts.oauth.keysCount', { count: parsedKeyCount placeholder) placeholderplaceholder
                </span>
                <button
                  v-if="showHelp"
                  type="button"
              ...
              <p
                ...
                class="mt-1 text-xs text-blue-600 dark:text-blue-400"
              >
                {{ t('admin.accounts.oauth.batchCreateAccounts', { count: parsedKeyCount placeholder) placeholderplaceholder
              </p>
            </div>
            ...
            >
              <h5 class="mb-2 font-semibold text-amber-800 dark:text-amber-200">
                {{ t('admin.accounts.oauth.howToGetSessionKey') placeholderplaceholder
              </h5>
              <ol
                class="list-inside list-decimal space-y-1 text-xs text-amber-700 dark:text-amber-300"
              >
                <li>{{ t('admin.accounts.oauth.step1') placeholderplaceholder</li>
                <li>{{ t('admin.accounts.oauth.step2') placeholderplaceholder</li>
                <li>{{ t('admin.accounts.oauth.step3') placeholderplaceholder</li>
                <li>{{ t('admin.accounts.oauth.step4') placeholderplaceholder</li>
                <li>{{ t('admin.accounts.oauth.step5') placeholderplaceholder</li>
                <li>{{ t('admin.accounts.oauth.step6') placeholderplaceholder</li>
              </ol>
              <p
        ...
        <div v-if="inputMethod === 'manual'" class="space-y-4">
          <p class="mb-4 text-sm text-blue-800 dark:text-blue-300">
            {{ oauthFollowSteps placeholderplaceholder
          </p>
            ...
            <div class="flex items-start gap-3">
              ...
              <div class="flex-1">
                <p class="mb-2 font-medium text-blue-900 dark:text-blue-200">
                  {{ oauthStep1GenerateUrl placeholderplaceholder
                </p>
                <div v-if="showProjectId && platform === 'gemini'" class="mb-3">
                  <label class="input-label flex items-center gap-2">
                    {{ t('admin.accounts.oauth.gemini.projectIdLabel') placeholderplaceholder
                    <a
                    ...
                    >
                      ...
                      {{ t('admin.accounts.oauth.gemini.howToGetProjectId') placeholderplaceholder
                    </a>
                  </label>
                  <input
                    v-model="projectId"
                  ...
                  <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                    {{ t('admin.accounts.oauth.gemini.projectIdHint') placeholderplaceholder
                  </p>
                </div>
                <button
                  v-if="!authUrl"
                ...
                >
                  ...
                  {{ loading ? t('admin.accounts.oauth.generating') : oauthGenerateAuthUrl placeholderplaceholder
                </button>
                <div v-else class="space-y-3">
                  ...
                  >
                    ...
                    {{ t('admin.accounts.oauth.regenerate') placeholderplaceholder
                  </button>
                </div>
              </div>
            </div>
          ...
          >
            <div class="flex items-start gap-3">
              ...
              <div class="flex-1">
                <p class="mb-2 font-medium text-blue-900 dark:text-blue-200">
                  {{ oauthStep2OpenUrl placeholderplaceholder
                </p>
                <p class="text-sm text-blue-700 dark:text-blue-300">
                  {{ oauthOpenUrlDesc placeholderplaceholder
                </p>
          ...
          >
            <div class="flex items-start gap-3">
              ...
              <div class="flex-1">
                <p class="mb-2 font-medium text-blue-900 dark:text-blue-200">
                  {{ oauthStep3EnterCode placeholderplaceholder
                </p>
                <p
                ...
                <div>
                  <label class="input-label">
                    ...
                    {{ oauthAuthCode placeholderplaceholder
                  </label>
                  <textarea
                  ...
                  <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">
                    ...
                    {{ oauthAuthCodeHint placeholderplaceholder
                  </p>
                  ...
                  >
                    <div class="flex items-start gap-2">
                      ...
                      <div class="text-sm text-amber-800 dark:text-amber-300">
                        <p class="font-semibold">{{ $t('admin.accounts.oauth.gemini.stateWarningTitle') placeholderplaceholder</p>
                        <p class="mt-1">{{ $t('admin.accounts.oauth.gemini.stateWarningDesc') placeholderplaceholder</p>
                      </div>
                    </div>
                  </div>
                </div>
...
const getOAuthKey = (key: string) => {
  ...
  if (props.platform === 'grok') return `admin.accounts.oauth.grok.${keyplaceholder`
...
watch(authCodeInput, (newVal) => {
  ...
  if (trimmed.includes('code=')) {
    try {
      // Try to parse as URL
      const url = trimmed.includes('?') ? new URL(trimmed) : new URL(`http://localhost/callback?${trimmed.replace(/^\?/, '')placeholder`)
