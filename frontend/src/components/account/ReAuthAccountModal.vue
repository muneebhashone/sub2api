  ...
  >
    <div v-if="account" class="space-y-5">
      ...
      <div
        ...
        class="rounded-lg border border-gray-200 bg-gray-50 p-4 dark:border-dark-600 dark:bg-dark-700"
      >
        <label class="input-label">{{ t('admin.accounts.oauth.gemini.redirectUri') placeholderplaceholder</label>
        <input
        ...
        />
        <p class="input-hint">{{ t('admin.accounts.oauth.gemini.redirectUriHint') placeholderplaceholder</p>
        <div class="mt-3 flex items-start gap-2">
          ...
          <label for="gemini-redirect-uri-confirm" class="text-sm text-gray-700 dark:text-gray-300">
            {{ t('admin.accounts.oauth.gemini.confirmRedirectUri') placeholderplaceholder
          </label>
        </div>
        <p v-if="geminiRedirectUri && !isValidGeminiRedirectUri" class="mt-2 text-xs text-red-600">
          {{ t('admin.accounts.oauth.gemini.invalidRedirectUri') placeholderplaceholder
        </p>
      </div>
...
const handleGenerateUrl = async () => {
  ...
  placeholder else if (isGemini.value) {
...
const defaultCallbackUrl =
  ...
  (typeof window !== 'undefined' ? `${window.location.originplaceholder/auth/callback` : '')
