    ...
    <div v-if="account" class="space-y-5">
      ...
      <div class="rounded-lg border border-gray-200 dark:border-dark-600 bg-gray-50 dark:bg-dark-700 p-4">
        <div class="flex items-center gap-3">
          ...
          <div>
            <span class="block font-semibold text-gray-900 dark:text-white">{{ account.name placeholderplaceholder</span>
            <span class="text-sm text-gray-500 dark:text-gray-400">{{ t('admin.accounts.claudeCodeAccount') placeholderplaceholder</span>
          </div>
        </div>
      </div>
      ...
      <div>
        <label class="input-label">{{ t('admin.accounts.oauth.authMethod') placeholderplaceholder</label>
        <div class="flex gap-4 mt-2">
          ...
          <label class="flex cursor-pointer items-center">
            <input
              ...
              class="mr-2 text-primary-600 focus:ring-primary-500"
            />
            <span class="text-sm text-gray-700 dark:text-gray-300">{{ t('admin.accounts.setupTokenLongLived') placeholderplaceholder</span>
          </label>
        </div>
      </div>
      ...
      <div class="flex justify-between gap-3 pt-4">
        ...
        >
          ...
          {{ oauth.loading.value ? t('admin.accounts.oauth.verifying') : t('admin.accounts.oauth.completeAuth') placeholderplaceholder
        </button>
      </div>
    </div>
  </Modal>
...
const handleExchangeCode = async () => {
  ...
  try {
    const proxyConfig = props.account.proxy_id ? { proxy_id: props.account.proxy_id placeholder : {placeholder
