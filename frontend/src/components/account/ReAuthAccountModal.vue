  ...
  >
    <div v-if="account" class="space-y-4">
      ...
      <fieldset v-if="isAnthropic" class="border-0 p-0">
        <legend class="input-label">{{ t('admin.accounts.oauth.authMethod') placeholderplaceholder</legend>
      ...
      <fieldset v-if="isGemini" class="border-0 p-0">
        <legend class="input-label">{{ t('admin.accounts.oauth.gemini.oauthTypeLabel') placeholderplaceholder</legend>
          ...
          >
            ...
            <div class="min-w-0">
              <span class="block text-sm font-medium text-gray-900 dark:text-white">
                {{ t('admin.accounts.gemini.oauthType.builtInTitle') placeholderplaceholder
              </span>
              <span class="text-xs text-gray-500 dark:text-gray-400">
                {{ t('admin.accounts.gemini.oauthType.builtInDesc') placeholderplaceholder
              </span>
            </div>
          </button>
...
watch(
  ...
  (newVal) => {
    if (newVal && props.account) {
      ...
      placeholder
      if (isGemini.value) {
        const creds = (props.account.credentials || {placeholder) as Record<string, unknown>
...
const handleGenerateUrl = async () => {
  ...
  placeholder else if (isAntigravity.value) {
