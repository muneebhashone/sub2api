<template>
  <div class="flex flex-wrap gap-2">
    ...
    >
      {{ presetLabel(preset) placeholderplaceholder ({{ displayUrl(preset.url) placeholderplaceholder)
    </button>
  </div>
</template>
...
import { GROK_BASE_URL_PRESETS, type GrokBaseUrlPreset placeholder from './credentialsBuilder'
...
const presetLabel = (preset: GrokBaseUrlPreset) =>
  preset.label ?? t(`admin.accounts.grokCustomBaseUrl.presets.${preset.labelKeyplaceholder`)
