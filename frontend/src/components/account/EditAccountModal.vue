  ...
  >
    ...
    >
      ...
      <div v-if="account.type === 'apikey'" class="space-y-4">
        ...
        <div v-else>
          <label class="input-label">{{ t('admin.accounts.cnProviders.apiProtocol.endpoints') placeholderplaceholder</label>
          <div class="mt-2 space-y-3">
            <div v-for="item in editAdaptiveProtocolOptions" :key="item.value">
              <label class="mb-1 block text-xs font-medium text-gray-600 dark:text-gray-400">
                {{ t(`admin.accounts.cnProviders.apiProtocol.${item.labelKeyplaceholder`) placeholderplaceholder
              </label>
              <input v-model="editAdaptiveBaseUrls[item.value]" type="text" class="input" />
            </div>
          </div>
          <p v-if="account.platform !== 'deepseek'" class="input-hint">
            {{ t('admin.accounts.cnProviders.apiProtocol.responsesFallbackDesc') placeholderplaceholder
          </p>
        </div>
        <!-- Account Mode Selection (CN providers) -->
        <div v-if="isCNApiKeyAccount">
          <label class="input-label">{{ t('admin.accounts.cnProviders.accountMode.title') placeholderplaceholder</label>
          <div class="mt-2 flex flex-wrap gap-2">
            ...
            >
              {{ t(`admin.accounts.cnProviders.accountMode.${opt.labelKeyplaceholder`) placeholderplaceholder
            </button>
          </div>
          <p class="input-hint">{{ t(`admin.accounts.cnProviders.accountMode.${editAccountModeplaceholderDesc`) placeholderplaceholder</p>
        </div>
        <!-- API Protocol Selection (CN providers) -->
        <div v-if="isCNApiKeyAccount">
          <label class="input-label">{{ t('admin.accounts.cnProviders.apiProtocol.title') placeholderplaceholder</label>
          <div class="mt-2 flex flex-wrap gap-2">
            <button
            ...
            >
              {{ t(`admin.accounts.cnProviders.apiProtocol.${opt.labelKeyplaceholder`) placeholderplaceholder
            </button>
          </div>
          <p class="input-hint">{{ t(`admin.accounts.cnProviders.apiProtocol.${cnProtocolDescKeyplaceholderDesc`) placeholderplaceholder</p>
        </div>
        <div>
      ...
      >
        <div class="flex items-center justify-between gap-4">
          <div class="min-w-0">
            <label class="input-label mb-0">{{ t('admin.accounts.grokClientToolCache.title') placeholderplaceholder</label>
            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
              {{ t('admin.accounts.grokClientToolCache.hint') placeholderplaceholder
            </p>
          </div>
          <Toggle
      ...
      <div v-if="account.type === 'bedrock'" class="space-y-4">
        ...
        <template v-if="!isBedrockAPIKeyMode">
          ...
          <div>
            ...
            />
            <p class="input-hint">{{ t('admin.accounts.bedrockSecretKeyLeaveEmpty') placeholderplaceholder</p>
          </div>
          <div>
        ...
        <div v-if="isBedrockAPIKeyMode">
          ...
          />
          <p class="input-hint">{{ t('admin.accounts.bedrockApiKeyLeaveEmpty') placeholderplaceholder</p>
        </div>
        ...
        <div class="border-t border-gray-200 pt-4 dark:border-dark-600">
          ...
          <div v-if="modelRestrictionMode === 'whitelist'">
            ...
            <p class="text-xs text-gray-500 dark:text-gray-400">
              {{ t('admin.accounts.selectedModels', { count: allowedModels.length placeholder) placeholderplaceholder
              <span v-if="allowedModels.length === 0 && modelMappings.length === 0">{{ t('admin.accounts.supportsAllModels') placeholderplaceholder</span>
            </p>
          </div>
          ...
          <div v-else class="space-y-3">
            ...
            <div class="flex flex-wrap gap-2">
              <button
                ...
                :key="preset.from"
                type="button"
                @click="modelMappings.push({ from: preset.from, to: preset.to placeholder)"
                :class="['rounded-lg px-3 py-1 text-xs transition-colors', preset.color]"
              >
                + {{ preset.label placeholderplaceholder
              </button>
      ...
      <div v-if="account.platform === 'antigravity'" class="border-t border-gray-200 pt-4 dark:border-dark-600">
        ...
        <div>
          ...
          <div class="mb-3 flex flex-wrap gap-2">
            ...
            >
              {{ isSyncingAntigravityUpstream ? t('admin.accounts.syncUpstreamModelsLoading') : t('admin.accounts.syncUpstreamModels') placeholderplaceholder
            </button>
          </div>
      ...
      >
        <div class="mb-3 flex items-center justify-between">
          <div>
            <label class="input-label mb-0">{{ t('admin.accounts.accountSchedulingThresholdOverride') placeholderplaceholder</label>
            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
              {{ t('admin.accounts.accountSchedulingThresholdOverrideHint') placeholderplaceholder
            </p>
          </div>
          <input
        ...
        <div v-if="accountSchedulingThresholdOverrideEnabled">
          <label class="input-label">{{ t('admin.accounts.accountSchedulingThresholdOverrideValue') placeholderplaceholder</label>
          <input
            ...
            class="input"
          />
          <p class="input-hint">{{ t('admin.accounts.accountSchedulingThresholdOverrideDisabledHint') placeholderplaceholder</p>
        </div>
      </div>
          ...
          >
            <div class="min-w-0">
              <p class="text-xs font-medium text-gray-700 dark:text-gray-200">
                {{ t('admin.accounts.upstreamBilling.syncRate') placeholderplaceholder
              </p>
              <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                {{ t('admin.accounts.upstreamBilling.syncRateHint') placeholderplaceholder
              </p>
            </div>
            <Toggle
          ...
          <div class="flex items-start gap-3 px-4 py-3">
            ...
            <div class="min-w-0 flex-1">
              <div class="flex flex-wrap items-center gap-2">
                <label class="input-label mb-0">{{ t('admin.accounts.openai.codexImageTool') placeholderplaceholder</label>
                <span
                  class="rounded-full px-2 py-0.5 text-[11px] font-medium"
                  :class="codexImageToolBadgeClass"
                >
                  {{ codexImageToolBadgeLabel placeholderplaceholder
                </span>
              </div>
              <p class="mt-1 text-xs leading-5 text-slate-600 dark:text-slate-300">
                {{ t('admin.accounts.openai.codexImageToolDesc') placeholderplaceholder
              </p>
            </div>
          </div>
          <div class="border-t border-sky-100 bg-white/70 p-2 dark:border-sky-900/50 dark:bg-dark-800/70">
            <div class="grid grid-cols-1 gap-2 sm:grid-cols-2">
              <button
                ...
                :key="option.value"
                type="button"
                :data-testid="`codex-image-tool-${option.valueplaceholder`"
              ...
              >
                ...
                <span class="min-w-0">
                  <span class="block text-sm font-medium">{{ option.label placeholderplaceholder</span>
                  <span class="mt-0.5 block text-xs leading-4 text-slate-500 dark:text-slate-400">{{ option.description placeholderplaceholder</span>
                </span>
              </button>
            </div>
          </div>
      ...
      >
        <div class="flex items-center justify-between gap-4">
          <div class="min-w-0">
            <label class="input-label mb-0">{{ t('admin.accounts.openai.planType') placeholderplaceholder</label>
            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
              {{ t('admin.accounts.openai.planTypeDesc') placeholderplaceholder
            </p>
          </div>
      ...
      >
        ...
        <div class="rounded-lg bg-gray-50 px-3 py-2 text-xs text-gray-600 dark:bg-dark-700 dark:text-gray-300">
          <span class="font-medium">{{ t(openAICompactStatusKey) placeholderplaceholder</span>
          <span
            v-if="account?.extra?.openai_compact_checked_at"
            class="ml-2 text-gray-500 dark:text-gray-400"
          >
            {{ t('admin.accounts.openai.compactLastChecked') placeholderplaceholder:
            {{ formatDateTime(new Date(String(account.extra.openai_compact_checked_at))) placeholderplaceholder
          </span>
        </div>
        <div>
      ...
      >
        <div class="space-y-2">
          <div class="flex items-center justify-between">
            <label class="input-label mb-0">{{ t('admin.accounts.autoPause5hDisabled') placeholderplaceholder</label>
            <button
              type="button"
          ...
          <p class="input-hint">{{ t('admin.accounts.autoPauseDisabledHint') placeholderplaceholder</p>
        </div>
        <div>
          <label class="input-label">{{ t('admin.accounts.autoPause5hThreshold') placeholderplaceholder</label>
          <input
          ...
          />
          <p class="input-hint">{{ t('admin.accounts.autoPauseThresholdHint') placeholderplaceholder</p>
        </div>
        <div class="space-y-2">
          <div class="flex items-center justify-between">
            <label class="input-label mb-0">{{ t('admin.accounts.autoPause7dDisabled') placeholderplaceholder</label>
            <button
              type="button"
        ...
        <div>
          <label class="input-label">{{ t('admin.accounts.autoPause7dThreshold') placeholderplaceholder</label>
          <input
      ...
      >
        <div class="flex items-center justify-between gap-4">
          <div class="min-w-0">
            <label class="input-label mb-0">{{ t('admin.accounts.autoResetCredit.title') placeholderplaceholder</label>
            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
              {{ t('admin.accounts.autoResetCredit.hint') placeholderplaceholder
            </p>
          </div>
          <button
            type="button"
        ...
        <div class="grid gap-4 sm:grid-cols-2">
          <div>
            <label class="input-label">{{ t('admin.accounts.autoResetCredit.threshold5h') placeholderplaceholder</label>
            <input
          ...
          <div>
            <label class="input-label">{{ t('admin.accounts.autoResetCredit.threshold7d') placeholderplaceholder</label>
            <input
        ...
        <p class="input-hint">{{ t('admin.accounts.autoResetCredit.thresholdHint') placeholderplaceholder</p>
      </div>
...
import { ref, reactive, computed, watch, nextTick placeholder from 'vue'
...
import { allSelectedGroupsEnableLongContextPricing placeholder from '@/components/account/longContextBilling'
...
const handleOllamaCloudUsageUpdated = (state: OllamaCloudUsageState) => {
  if (props.account) emit('updated', { ...props.account, ollama_cloud_usage: state placeholder)
placeholder
...
const cnAccountModeOptions = computed<Array<{ value: CnAccountMode; labelKey: 'payg' | 'coding' placeholder>>(
  () => {
    // DeepSeek 无 coding 套餐（与创建弹窗一致），仅保留按量付费。
    if (props.account?.platform === 'deepseek') {
      return [{ value: 'payg', labelKey: 'payg' placeholder]
    placeholder
    return [
      { value: 'payg', labelKey: 'payg' placeholder,
      { value: 'coding', labelKey: 'coding' placeholder
    ]
  placeholder
)
const cnProtocolOptions = computed<Array<{ value: CnApiProtocol; labelKey: string placeholder>>(() => {
  const opts: Array<{ value: CnApiProtocol; labelKey: string placeholder> = [
    { value: 'adaptive', labelKey: 'adaptive' placeholder,
    { value: 'chat_completions', labelKey: 'chatCompletions' placeholder,
    { value: 'anthropic', labelKey: 'anthropic' placeholder
  ]
  if (props.account?.platform === 'deepseek') {
    opts.push({ value: 'responses', labelKey: 'responses' placeholder)
  placeholder
  return opts
placeholder)
const editAdaptiveProtocolOptions = computed<Array<{ value: CnNativeApiProtocol; labelKey: string placeholder>>(() => {
  const opts: Array<{ value: CnNativeApiProtocol; labelKey: string placeholder> = [
    { value: 'chat_completions', labelKey: 'chatCompletions' placeholder,
    { value: 'anthropic', labelKey: 'anthropic' placeholder
  ]
  if (props.account?.platform === 'deepseek') opts.push({ value: 'responses', labelKey: 'responses' placeholder)
  return opts
placeholder)
...
function onCnPresetSelect(preset: { mode: CnAccountMode; protocol: CnApiProtocol; url: string placeholder) {
...
const statusOptions = computed(() => {
  ...
  if (form.status === 'error') {
    options.push({ value: 'error', label: t('admin.accounts.status.error') placeholder)
  placeholder
  return options
placeholder)
...
const applyOpenAIModelMappingCredentials = (credentials: Record<string, unknown>) => {
  ...
  if (shouldApplyModelMapping) {
    ...
    placeholder
  placeholder else if (!credentials.model_mapping) {
    delete credentials.model_mapping
  placeholder
...
const syncFormFromAccount = (newAccount: Account | null) => {
  ...
  if (newAccount.platform === 'openai' && (newAccount.type === 'oauth' || newAccount.type === 'setup-token' || newAccount.type === 'apikey')) {
    ...
    placeholder else if (codexImageGenerationBridgeValue === true) {
      codexImageToolMode.value = 'enabled'
    placeholder else if (codexImageGenerationBridgeValue === false) {
    ...
    if (compactMappings && typeof compactMappings === 'object') {
      openAICompactModelMappings.value = Object.entries(compactMappings).map(([from, to]) => ({ from, to placeholder))
    placeholder
  placeholder
  ...
  if (newAccount.type === 'apikey' && newAccount.credentials) {
    ...
    if (newAccount.platform === 'kimi' || newAccount.platform === 'zhipu' || newAccount.platform === 'deepseek') {
      ...
      const storedBaseUrls = (credentials.api_base_urls as Record<string, unknown> | undefined) || {placeholder
  ...
  placeholder else if (newAccount.type === 'bedrock' && newAccount.credentials) {
    ...
    const bedrockExtra = (newAccount.extra as Record<string, unknown>) || {placeholder
  ...
  placeholder else if ((newAccount.platform === 'gemini' || newAccount.platform === 'anthropic') && newAccount.type === 'service_account' && newAccount.credentials) {
    const credentials = newAccount.credentials as Record<string, unknown>
...
async function loadTLSProfiles() {
  try {
    ...
    tlsFingerprintProfiles.value = profiles.map(p => ({ id: p.id, name: p.name placeholder))
  placeholder catch {
...
const syncAntigravityUpstreamModels = async () => {
  ...
  try {
    ...
    for (const model of upstreamModels) {
      ...
      if (!exists) {
        antigravityModelMappings.value.push({ from: model, to: model placeholder)
        addedCount += 1
      placeholder
    placeholder

    if (addedCount > 0) {
      appStore.showSuccess(t('admin.accounts.syncUpstreamModelsSuccess', { count: addedCount, total: upstreamModels.length placeholder))
    placeholder else {
      appStore.showInfo(t('admin.accounts.syncUpstreamModelsNoChanges', { count: upstreamModels.length placeholder))
    placeholder
  placeholder catch (error) {
    const message = error instanceof Error ? error.message : t('admin.accounts.syncUpstreamModelsFailed')
    appStore.showError(t('admin.accounts.syncUpstreamModelsError', { message placeholder))
  placeholder finally {
  ...
  try {
    ...
    if (props.account.type === 'apikey') {
      ...
      if (isCNApiKeyAccount.value) {
        ...
        if (editApiProtocol.value === 'adaptive') {
          const defaults = defaultCNAdaptiveBaseUrls(cnPresetPlatform.value, editAccountMode.value)
          const protocolBaseUrls: Record<string, string> = {placeholder
          for (const item of editAdaptiveProtocolOptions.value) {
      ...
      placeholder else if (!hasExistingApiKey) {
    ...
    placeholder else if (props.account.type === 'upstream') {
      ...
      updatePayload.credentials = newCredentials
    placeholder else if ((props.account.platform === 'gemini' || props.account.platform === 'anthropic') && props.account.type === 'service_account') {
      const currentCredentials = (props.account.credentials as Record<string, unknown>) || {placeholder
      const newCredentials: Record<string, unknown> = { ...currentCredentials placeholder
      ...
      updatePayload.credentials = newCredentials
    placeholder else if (props.account.type === 'bedrock') {
      const currentCredentials = (props.account.credentials as Record<string, unknown>) || {placeholder
      const newCredentials: Record<string, unknown> = { ...currentCredentials placeholder
    ...
    if (props.account.platform === 'openai' && props.account.type === 'oauth' && !isSparkShadow.value) {
      const currentCredentials = (updatePayload.credentials as Record<string, unknown>) ||
        ((props.account.credentials as Record<string, unknown>) || {placeholder)
      updatePayload.credentials = applyPlanType({ ...currentCredentials placeholder, editPlanType.value)
    placeholder
    ...
    if (props.account.platform === 'anthropic' && (props.account.type === 'oauth' || props.account.type === 'setup-token')) {
      const currentExtra = (updatePayload.extra as Record<string, unknown>) || (props.account.extra as Record<string, unknown>) || {placeholder
      const newExtra: Record<string, unknown> = { ...currentExtra placeholder
    ...
    if (props.account.platform === 'openai' && (props.account.type === 'oauth' || props.account.type === 'setup-token' || props.account.type === 'apikey')) {
      ...
      placeholder else if (props.account.type === 'apikey') {
