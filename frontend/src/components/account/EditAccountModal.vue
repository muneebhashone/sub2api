      ...
      >
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
              {{ t('admin.accounts.openai.wsModeConcurrencyHint') placeholderplaceholder
            </p>
          </div>
          <div class="w-52">
          ...
          <div v-if="rpmLimitEnabled" class="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select v-model="rpmStrategy" class="input">
                <option value="tiered">{{ t('admin.accounts.quotaControl.rpmLimit.strategyTiered') placeholderplaceholder</option>
                <option value="sticky_exempt">{{ t('admin.accounts.quotaControl.rpmLimit.strategyStickyExempt') placeholderplaceholder</option>
              </select>
              <p class="input-hint">{{ t('admin.accounts.quotaControl.rpmLimit.strategyHint') placeholderplaceholder</p>
            </div>
          </div>
        </div>
...
const openAIWSModeOptions = computed(() => [
  ...
  { value: OPENAI_WS_MODE_DEDICATED, label: t('admin.accounts.openai.wsModeDedicated') placeholder
])
