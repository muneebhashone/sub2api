<template>
  <div v-if="shouldShowQuota" class="flex items-center gap-2">
    ...
    <span :class="['badge text-xs px-2 py-0.5 rounded font-medium', tierBadgeClass]">
      {{ tierLabel placeholderplaceholder
    </span>

    <!-- 限额文本 -->
    <span class="text-xs text-gray-500 dark:text-gray-400">{{ quotaText placeholderplaceholder</span>
    ...
    <div class="group/progress relative flex items-center gap-1">
      <div class="h-2 w-20 overflow-hidden rounded-full bg-gray-200 dark:bg-gray-700">
        <div
          ...
          :style="{ width: isRateLimited ? '100%' : '0%' placeholder"
        ></div>
      </div>
      ...
      >
        {{ isRateLimited ? '100%' : '0%' placeholderplaceholder
      </span>
      ...
      >
        ⚠️ {{ resetCountdown placeholderplaceholder
      </span>
...
import { computed, ref, watch, onUnmounted placeholder from 'vue'
import type { Account, GeminiCredentials placeholder from '@/types'
...
const resetCountdown = computed(() => {
  ...
  if (diffMinutes < 1) return `${diffSecondsplaceholders`
  if (diffHours < 1) {
    const secs = diffSeconds % 60
    return `${diffMinutesplaceholderm ${secsplaceholders`
  placeholder
  const mins = diffMinutes % 60
  return `${diffHoursplaceholderh ${minsplaceholderm`
placeholder)
  ...
  (limited) => {
    if (limited && !timer) {
      ...
      timer = setInterval(() => {
        now.value = new Date()
      placeholder, 1000)
    placeholder else if (!limited && timer) {
      // 解除限流，停止定时器
      clearInterval(timer)
      timer = null
    placeholder
  placeholder,
  { immediate: true placeholder // 立即执行，确保挂载时已限流的情况也能启动定时器
)
