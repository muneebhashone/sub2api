<template>
  <div v-if="shouldShowQuota">
    ...
    <div class="text-xs text-gray-400 dark:text-gray-500">
      ...
      >
        {{ t('admin.accounts.gemini.rateLimit.limited', { time: resetCountdown placeholder) placeholderplaceholder
      </span>
    </div>
  </div>
</template>
...
const tierLabel = computed(() => {
  ...
  if (isCodeAssist.value) {
    ...
    if (upper) return `GCP ${upperplaceholder`
    return 'GCP'
  placeholder
  ...
  if (isGoogleOne.value) {
    ...
    if (upper) return `Google One ${upperplaceholder`
    return 'Google One'
  placeholder
