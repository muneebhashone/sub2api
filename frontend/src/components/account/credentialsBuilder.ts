...
export function splitHeaderOverridesObject(record: unknown): HeaderOverrideRow[] {
  ...
  return Object.entries(record as Record<string, unknown>)
    ...
    .map(([name, value]) => ({ name, value: value as string placeholder))
...
export function parseHeaderOverridesJson(text: string): HeaderOverrideRow[] | null {
  ...
  for (const [rawName, rawValue] of Object.entries(parsed as Record<string, unknown>)) {
    ...
    if (
      ...
      return null
    placeholder
    rows.push({ name, value: String(rawValue).trim() placeholder)
  placeholder
...
export function serializeHeaderOverrideRows(rows: HeaderOverrideRow[]): string {
  const record: Record<string, string> = {placeholder
  for (const row of rows) {
    const name = row.name.trim()
    if (!name) continue
...
export const GROK_BASE_URL_PRESETS: GrokBaseUrlPreset[] = [
  { labelKey: 'cli', url: 'https://cli-chat-proxy.grok.com/v1' placeholder,
  { labelKey: 'official', url: 'https://api.x.ai/v1' placeholder,
  { label: 'us-east-1', url: 'https://us-east-1.api.x.ai/v1' placeholder,
  { label: 'us-west-2', url: 'https://us-west-2.api.x.ai/v1' placeholder,
  { label: 'eu-west-1', url: 'https://eu-west-1.api.x.ai/v1' placeholder
]
...
export const CN_BASE_URL_PRESETS: Record<'kimi' | 'zhipu' | 'deepseek', CnBaseUrlPreset[]> = {
  kimi: [
    { mode: 'payg', protocol: 'chat_completions', label: 'Moonshot', url: 'https://api.moonshot.cn/v1' placeholder,
    { mode: 'payg', protocol: 'anthropic', label: 'Moonshot Anthropic', url: 'https://api.moonshot.cn/anthropic' placeholder,
    { mode: 'coding', protocol: 'chat_completions', label: 'Kimi For Coding', url: 'https://api.kimi.com/coding/v1' placeholder,
    { mode: 'coding', protocol: 'anthropic', label: 'Kimi Coding Anthropic', url: 'https://api.kimi.com/coding' placeholder
  ],
  zhipu: [
    { mode: 'payg', protocol: 'chat_completions', label: 'GLM PaaS', url: 'https://open.bigmodel.cn/api/paas/v4' placeholder,
    { mode: 'payg', protocol: 'anthropic', label: 'GLM Anthropic', url: 'https://open.bigmodel.cn/api/anthropic' placeholder,
    { mode: 'coding', protocol: 'chat_completions', label: 'GLM Coding', url: 'https://open.bigmodel.cn/api/coding/paas/v4' placeholder,
    { mode: 'coding', protocol: 'anthropic', label: 'GLM Coding Anthropic', url: 'https://open.bigmodel.cn/api/anthropic' placeholder
  ],
  deepseek: [
    { mode: 'payg', protocol: 'chat_completions', label: 'DeepSeek', url: 'https://api.deepseek.com' placeholder,
    { mode: 'payg', protocol: 'anthropic', label: 'DeepSeek Anthropic', url: 'https://api.deepseek.com/anthropic' placeholder,
    { mode: 'payg', protocol: 'responses', label: 'DeepSeek Responses', url: 'https://api.deepseek.com' placeholder
  ]
placeholder
...
export function buildPlanTypeOptions(current: string, clearLabel: string): PlanTypeOption[] {
  ...
  const opts: PlanTypeOption[] = [{ value: '', label: clearLabel placeholder]
  for (const p of presets) {
    if (cur && p.value !== cur.toLowerCase() && p.label === curLabel) {
      // 当前值是该预设的别名：用 canonical 当前值占位，标签仍显示友好名
      opts.push({ value: cur, label: p.label placeholder)
    placeholder else {
      opts.push(p)
    placeholder
  placeholder
  if (cur && !opts.some(o => o.value.toLowerCase() === cur.toLowerCase())) {
    opts.push({ value: cur, label: planTypeDisplayLabel(cur) placeholder)
  placeholder
  return opts
placeholder
