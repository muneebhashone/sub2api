  ...
  <div ref="rootRef" v-if="showUsageWindows">
    ...
    >
      ...
      <div v-else-if="usageInfo" class="space-y-1">
        ...
        <div v-if="usageInfo.error" class="text-xs text-amber-600 dark:text-amber-400 truncate max-w-[200px]" :title="usageInfo.error">
          {{ usageInfo.error placeholderplaceholder
        </div>
        <!-- 5h Window -->
        <UsageProgressBar
    ...
    <template v-else-if="account.platform === 'antigravity' && account.type === 'oauth'">
      ...
      <div v-if="antigravityTierLabel" class="mb-1 flex items-center gap-1">
        ...
        >
          {{ antigravityTierLabel placeholderplaceholder
        </span>
        <!-- 不合格账户警告图标 -->
        <span
        ...
        >
          ...
          >
            {{ t('admin.accounts.ineligibleWarning') placeholderplaceholder
          </span>
        </span>
      </div>
      ...
      <div v-if="isForbidden" class="space-y-1">
        ...
        >
          {{ forbiddenLabel placeholderplaceholder
        </span>
        <div v-if="validationURL" class="flex items-center gap-1">
          ...
          >
            {{ t('admin.accounts.openVerification') placeholderplaceholder
          </a>
          <button
            type="button"
          ...
          >
            {{ linkCopied ? t('admin.accounts.linkCopied') : t('admin.accounts.copyLink') placeholderplaceholder
          </button>
        </div>
      </div>
      ...
      <div v-else-if="needsReauth" class="space-y-1">
        <span class="inline-block rounded px-1.5 py-0.5 text-[10px] font-medium bg-orange-100 text-orange-700 dark:bg-orange-900/40 dark:text-orange-300">
          {{ t('admin.accounts.needsReauth') placeholderplaceholder
        </span>
      </div>
      ...
      <div v-else-if="hasAntigravityQuotaFromAPI" class="space-y-1">
        ...
        <div v-if="aiCreditsDisplay" class="mt-1 text-[10px] text-gray-500 dark:text-gray-400">
          💳 {{ t('admin.accounts.aiCreditsBalance') placeholderplaceholder: {{ aiCreditsDisplay placeholderplaceholder
        </div>
      </div>
    ...
    <template v-else-if="account.platform === 'grok' && account.type === 'oauth'">
      ...
      <div v-else-if="isForbidden" class="space-y-1">
        <span class="inline-block rounded px-1.5 py-0.5 text-[10px] font-medium bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-300">
          {{ grokEntitlementLabel || t('admin.accounts.forbidden') placeholderplaceholder
        </span>
      </div>
      <div v-else-if="usageInfo" class="space-y-1">
        <div v-if="grokEntitlementLabel" class="mb-0.5">
          <span class="inline-block rounded bg-zinc-100 px-1.5 py-0.5 text-[10px] font-medium text-zinc-800 dark:bg-zinc-800 dark:text-zinc-200">
            {{ grokEntitlementLabel placeholderplaceholder
          </span>
        </div>
        <div v-if="showGrokStandaloneLocalStats && grokLocalUsage" class="mb-0.5 flex items-center">
          <div class="flex items-center gap-1.5 text-[9px] text-gray-500 dark:text-gray-400">
            <span class="rounded bg-gray-100 px-1.5 py-0.5 dark:bg-gray-800">
              {{ formatWindowRequests(grokLocalUsage) placeholderplaceholder req
            </span>
            <span class="rounded bg-gray-100 px-1.5 py-0.5 dark:bg-gray-800">
              {{ formatWindowTokens(grokLocalUsage) placeholderplaceholder
            </span>
            <span class="rounded bg-gray-100 px-1.5 py-0.5 dark:bg-gray-800" :title="t('usage.accountBilled')">
              A ${{ formatWindowCost(grokLocalUsage) placeholderplaceholder
            </span>
            <span
            ...
            >
              U ${{ formatWindowUserCost(grokLocalUsage) placeholderplaceholder
            </span>
          </div>
        </div>
        <UsageProgressBar
        ...
        >
          ...
          >
            {{ t('admin.accounts.usageWindow.grokPrepaid') placeholderplaceholder ${{ grokBillingMoneySummary.prepaid placeholderplaceholder
          </span>
          <span :title="t('admin.accounts.usageWindow.grokMonthlyLimit')">
            {{ t('admin.accounts.usageWindow.grokUsed') placeholderplaceholder
            {{ grokBillingMoneySummary.used placeholderplaceholder/{{ grokBillingMoneySummary.limit placeholderplaceholder
          </span>
          <span
            v-if="grokBillingMoneySummary.usedPercent != null"
            class="rounded bg-gray-100 px-1 py-0.5 dark:bg-gray-800"
          >
            {{ grokBillingMoneySummary.usedPercent placeholderplaceholder%
          </span>
          <span
          ...
          >
            {{ t('admin.accounts.usageWindow.grokOverageShort') placeholderplaceholder
            {{ grokBillingMoneySummary.onDemandUsed placeholderplaceholder/{{ grokBillingMoneySummary.onDemandCap placeholderplaceholder
          </span>
        </div>
        <UsageProgressBar
        ...
        <UsageProgressBar
          ...
          label="24h"
          :title="t('admin.accounts.usageWindow.grokFreeQuota24hHint', { limit: formatCompactNumber(grokFreeTokenBar.limit) placeholder)"
    ...
    <template v-else-if="account.platform === 'gemini'">
      ...
      <div v-if="geminiAuthTypeLabel" class="mb-1 flex items-center gap-1">
        ...
        >
          {{ geminiAuthTypeLabel placeholderplaceholder
        </span>
        <!-- Help icon -->
        <span
          class="group relative cursor-help"
        >
          ...
          <span
            class="pointer-events-none absolute left-0 top-full z-50 mt-1 w-80 whitespace-normal break-words rounded bg-gray-900 px-3 py-2 text-xs leading-relaxed text-white opacity-0 shadow-lg transition-opacity group-hover:opacity-100 dark:bg-gray-700"
          >
            <div class="font-semibold mb-1">{{ t('admin.accounts.gemini.quotaPolicy.title') placeholderplaceholder</div>
            <div class="mb-2 text-gray-300">{{ t('admin.accounts.gemini.quotaPolicy.note') placeholderplaceholder</div>
            <div class="space-y-1">
              <div><strong>{{ geminiQuotaPolicyChannel placeholderplaceholder:</strong></div>
              <div class="pl-2">• {{ geminiQuotaPolicyLimits placeholderplaceholder</div>
              <div class="mt-2">
      ...
      <div class="space-y-1">
        ...
        >
          <div class="flex items-center gap-1.5 text-[9px] text-gray-500 dark:text-gray-400">
            <span class="rounded bg-gray-100 px-1.5 py-0.5 dark:bg-gray-800">
              {{ formatKeyRequests placeholderplaceholder req
            </span>
            <span class="rounded bg-gray-100 px-1.5 py-0.5 dark:bg-gray-800">
              {{ formatKeyTokens placeholderplaceholder
            </span>
            <span class="rounded bg-gray-100 px-1.5 py-0.5 dark:bg-gray-800" :title="t('usage.accountBilled')">
              A ${{ formatKeyCost placeholderplaceholder
            </span>
            <span
              ...
              :title="t('usage.userBilled')"
            >
              U ${{ formatKeyUserCost placeholderplaceholder
            </span>
          </div>
        </div>
        <div
        ...
        <div v-else-if="geminiUsageAvailable" class="space-y-1">
          ...
          <p class="mt-1 text-[9px] leading-tight text-gray-400 dark:text-gray-500 italic">
            * {{ t('admin.accounts.gemini.quotaPolicy.simulatedNote') || 'Simulated quota' placeholderplaceholder
          </p>
        </div>
        <!-- AI Studio Client OAuth: show unlimited flow (no usage tracking) -->
        <div v-else class="text-xs text-gray-400">
          {{ t('admin.accounts.gemini.rateLimit.unlimited') placeholderplaceholder
        </div>
      </div>
    </template>
...
import { ref, computed, onMounted, onBeforeUnmount, onUnmounted, watch placeholder from 'vue'
...
const _usageCache = new Map<number, { data: AccountUsageInfo; ts: number placeholder>()
...
const props = withDefaults(
  defineProps<{
    ...
    requestBatchedUsage?: ((account: Account, options?: { force?: boolean placeholder) => void) | null
  placeholder>(),
  {
    todayStats: null,
...
const geminiAuthTypeLabel = computed(() => {
  ...
  return geminiUserLevel.value ? `${geminiChannelShort.valueplaceholder ${geminiUserLevel.valueplaceholder` : geminiChannelShort.value
placeholder)
...
const makeGrokQuotaBar = (quota?: { limit?: number | null; remaining?: number | null; reset_at?: string | null placeholder | null): GrokQuotaBarInfo | null => {
...
const grokBillingMoneySummary = computed(() => {
  ...
  placeholder else if (limit != null && limit > 0 && used != null) {
...
const grokFreeTokenBar = computed(() => {
  ...
  return { utilization: Math.min(100, (used / limit) * 100), limit placeholder
placeholder)
...
const formatWindowRequests = (stats: WindowStats) => formatCompactNumber(stats.requests, { allowBillions: false placeholder)
...
const copyValidationURL = async () => {
  ...
  try {
    ...
    setTimeout(() => { linkCopied.value = false placeholder, 2000)
  placeholder catch {
    // fallback: ignore
  placeholder
placeholder
...
const requestParentBatchUsage = (options?: { force?: boolean placeholder) => {
...
const loadUsage = async (options?: { source?: 'passive' | 'active'; bypassCache?: boolean placeholder) => {
  if (!shouldFetchUsage.value) return
  if (isBatchManaged.value) {
    requestParentBatchUsage({ force: options?.bypassCache === true placeholder)
    return
  placeholder
  ...
  try {
    ...
    if (!unmounted.value) {
      ...
      _usageCache.set(props.account.id, { data: result, ts: Date.now() placeholder)
    placeholder
...
const flushPendingAutoLoad = () => {
  ...
  loadUsage({ source placeholder).catch((e) => {
...
const handleGrokProbed = (result: GrokQuotaProbeResult) => {
  ...
  _usageCache.set(props.account.id, { data: merged, ts: Date.now() placeholder)
placeholder
...
const formatKeyRequests = computed(() => {
  ...
  return formatCompactNumber(props.todayStats.requests, { allowBillions: false placeholder)
placeholder)
...
watch(openAIUsageRefreshKey, (nextKey, prevKey) => {
  ...
  if (isBatchManaged.value) {
    requestParentBatchUsage({ force: true placeholder)
    return
  placeholder
...
watch(
  ...
  (nextToken, prevToken) => {
    ...
    const source = isAnthropicOAuthOrSetupToken.value ? 'passive' : undefined
    _usageCache.delete(props.account.id)
    loadUsage({ source, bypassCache: true placeholder).catch((e) => {
...
watch(
  ...
  () => {
    ...
    detachVisibilityObserver()
  placeholder,
  { immediate: true, flush: 'post' placeholder
)
