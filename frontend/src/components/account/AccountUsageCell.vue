    ...
    <template v-else-if="account.platform === 'antigravity' && account.type === 'oauth'">
      ...
      <div v-else-if="hasAntigravityQuotaFromAPI" class="space-y-1">
        ...
        <div v-if="antigravityAICreditsDisplay.length > 0" class="mt-1 flex flex-wrap gap-1">
          ...
          >
            <span>⚡</span>
            <span>{{ t('admin.accounts.aiCreditsBalance') placeholderplaceholder</span>
            <span>{{ credit.label placeholderplaceholder</span>
            <span class="font-mono">{{ credit.amount placeholderplaceholder</span>
            <span v-if="credit.minimumBalance !== null" class="text-[9px] opacity-75">
              min {{ credit.minimumBalance placeholderplaceholder
            </span>
          </div>
        </div>
      </div>
