...
import type { Account, AccountUsageInfo, GeminiCredentials placeholder from '@/types'
...
const geminiAuthTypeLabel = computed(() => {
  ...
  if (oauthType === 'google_one') {
    ...
    return `Google One ${tierLabelplaceholder`
  placeholder else if (oauthType === 'code_assist' || (!oauthType && isGeminiCodeAssist.value)) {
    ...
    return `GCP ${tierLabelplaceholder`
  placeholder else if (oauthType === 'ai_studio') {
