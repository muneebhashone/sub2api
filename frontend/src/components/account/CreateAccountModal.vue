      ...
      <div v-if="isCNPlatform">
        ...
        <div class="mt-2 grid grid-cols-1 gap-3 sm:grid-cols-2" data-tour="account-form-mode">
          ...
          >
            ...
            <div>
              <span class="block text-sm font-medium text-gray-900 dark:text-white">{{ t('admin.accounts.cnProviders.accountMode.payg') placeholderplaceholder</span>
              <span class="text-xs text-gray-500 dark:text-gray-400">{{ t('admin.accounts.cnProviders.accountMode.paygDesc') placeholderplaceholder</span>
            </div>
          </button>
          ...
          >
            ...
            <div>
              <span class="block text-sm font-medium text-gray-900 dark:text-white">{{ t('admin.accounts.cnProviders.accountMode.coding') placeholderplaceholder</span>
              <span class="text-xs text-gray-500 dark:text-gray-400">{{ t('admin.accounts.cnProviders.accountMode.codingDesc') placeholderplaceholder</span>
            </div>
          </button>
        </div>
      </div>
      ...
      <div v-if="isCNPlatform" class="mt-4">
        ...
        <div class="mt-2 grid grid-cols-1 gap-3 sm:grid-cols-3">
          ...
          >
            ...
            <div>
              <span class="block text-sm font-medium text-gray-900 dark:text-white">{{ t(`admin.accounts.cnProviders.apiProtocol.${opt.labelKeyplaceholder`) placeholderplaceholder</span>
              <span class="text-xs text-gray-500 dark:text-gray-400">{{ t(`admin.accounts.cnProviders.apiProtocol.${opt.labelKeyplaceholderDesc`) placeholderplaceholder</span>
            </div>
          </button>
        </div>
      </div>
          ...
          <div class="mt-2 space-y-3">
            <div v-for="item in cnAdaptiveProtocolOptions" :key="item.value">
              ...
              <input
                ...
                type="text"
                class="input"
                :data-testid="`cn-adaptive-base-url-${item.valueplaceholder`"
              />
            </div>
          </div>
...
const cnProtocolOptions = computed<Array<{ value: CnApiProtocol; labelKey: string placeholder>>(() => {
  ...
  return opts
placeholder)
const cnAdaptiveProtocolOptions = computed<Array<{ value: CnNativeApiProtocol; labelKey: string placeholder>>(() => {
  const opts: Array<{ value: CnNativeApiProtocol; labelKey: string placeholder> = [
    { value: 'chat_completions', labelKey: 'chatCompletions' placeholder,
    { value: 'anthropic', labelKey: 'anthropic' placeholder
  ]
  if (form.platform === 'deepseek') opts.push({ value: 'responses', labelKey: 'responses' placeholder)
  return opts
placeholder)
...
const resetForm = () => {
  ...
  accountMode.value = 'payg'
  apiProtocol.value = 'adaptive'
  adaptiveBaseUrls.value = { chat_completions: '', anthropic: '', responses: '' placeholder
