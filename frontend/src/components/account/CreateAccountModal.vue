      ...
      <div v-if="form.platform === 'openai' && accountCategory === 'oauth-based'" class="mb-4">
        <label class="flex items-center justify-between rounded-lg border border-gray-200 p-3 dark:border-dark-600">
          <div class="flex items-center gap-3">
            ...
            <div>
              <span class="block text-sm font-medium text-gray-900 dark:text-white">
                {{ t('admin.accounts.openai.enableSora') placeholderplaceholder
              </span>
              <span class="text-xs text-gray-500 dark:text-gray-400">
                {{ t('admin.accounts.openai.enableSoraHint') placeholderplaceholder
              </span>
            </div>
          </div>
          <label :class="['switch', { 'switch-active': enableSoraOnOpenAIOAuth placeholder]">
  ...
  try {
    ...
    if (enableSoraOnOpenAIOAuth.value) {
      try {
        ...
        await adminAPI.accounts.create({
          name: `${form.nameplaceholder (Sora)`,
          notes: form.notes,
          platform: 'sora',
          type: 'oauth',
