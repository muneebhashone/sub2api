<template>
  <div>
    ...
    <div class="relative mb-3">
      ...
      >
        <div class="grid grid-cols-2 gap-1.5">
          ...
          >
            <span class="flex items-center gap-1 truncate">
              ...
              <span class="truncate">{{ model placeholderplaceholder</span>
            </span>
            <button
              type="button"
        ...
        <div class="mt-2 flex items-center justify-between border-t border-gray-200 pt-2 dark:border-dark-600">
          <span class="text-xs text-gray-400">{{ t('admin.accounts.modelCount', { count: modelValue.length placeholder) placeholderplaceholder</span>
        ...
        <div class="max-h-52 overflow-auto">
          ...
          >
            ...
            >
              ...
              <span class="truncate text-gray-900 dark:text-white">{{ model.value placeholderplaceholder</span>
            </button>
            <button
              type="button"
              ...
              :title="`${t('common.copy')placeholder ${model.valueplaceholder`"
              :aria-label="`${t('common.copy')placeholder ${model.valueplaceholder`"
          ...
          <div v-if="filteredModels.length === 0" class="px-3 py-4 text-center text-sm text-gray-500">
            {{ t('admin.accounts.noMatchingModels') placeholderplaceholder
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="mb-4 flex flex-wrap gap-2">
      ...
      >
        {{ t('admin.accounts.fillRelatedModels') placeholderplaceholder
      </button>
      <button
      ...
      >
        {{ isSyncingUpstream ? t('admin.accounts.syncUpstreamModelsLoading') : t('admin.accounts.syncUpstreamModels') placeholderplaceholder
      </button>
      <button
        type="button"
        @click="clearAll"
      ...
      >
        {{ t('admin.accounts.clearAllModels') placeholderplaceholder
      </button>
    </div>

    <!-- Custom Model Input -->
    <div class="mb-3">
      <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">{{ t('admin.accounts.customModelName') placeholderplaceholder</label>
      <div class="flex gap-2">
        ...
        >
          {{ t('admin.accounts.addModel') placeholderplaceholder
        </button>
      </div>
    </div>
  </div>
...
import { accountsAPI placeholder from '@/api/admin/accounts'
import type { SyncUpstreamPreviewParams placeholder from '@/api/admin/accounts'
import { useClipboard placeholder from '@/composables/useClipboard'
import ModelIcon from '@/components/common/ModelIcon.vue'
import Icon from '@/components/icons/Icon.vue'
import { allModels, getModelsByPlatform placeholder from '@/composables/useModelWhitelist'
...
const syncUpstreamModels = async () => {
  ...
  try {
    ...
    placeholder else if (props.syncCredentials) {
