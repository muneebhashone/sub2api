<template>
  <div v-if="visible" class="space-y-1">
    ...
    <div class="flex flex-wrap items-center gap-1.5">
      ...
      >
        {{ balanceLabel placeholderplaceholder
      </span>
      ...
      >
        {{ t('admin.accounts.cnProviders.balanceLow') placeholderplaceholder
      </span>
    </div>
...
import type { CNProviderBalanceEntry, CNProviderBalanceResult placeholder from '@/api/admin/cnProviders'
import type { Account placeholder from '@/types'
import { platformTextClass placeholder from '@/utils/platformColors'
import { cnBalanceCellVisible placeholder from './credentialsBuilder'
...
const extraKey = (suffix: string) => `${props.account.platformplaceholder_${suffixplaceholder`
...
placeholder)
// 多币种快照（后端写 <platform>_balances：[{currency, balanceplaceholder]，deepseek CNY+USD）。
const snapshotBalances = computed<CNProviderBalanceEntry[]>(() => {
  ...
  return v.flatMap((item): CNProviderBalanceEntry[] => {
    ...
    const { currency, balance placeholder = item as Record<string, unknown>
    if (typeof currency !== 'string' || typeof balance !== 'number') return []
    return [{ currency, balance placeholder]
  placeholder)
placeholder)
...
const currentEntries = computed<CNProviderBalanceEntry[]>(() => {
  if (data.value && data.value.success) {
    ...
    return [{ currency: data.value.currency || '', balance: data.value.balance placeholder]
  placeholder
  if (snapshotBalances.value.length > 0) return snapshotBalances.value
  if (snapshotBalance.value != null) {
    return [{ currency: snapshotCurrency.value, balance: snapshotBalance.value placeholder]
  placeholder
  return []
placeholder)
...
const formatEntry = (entry: CNProviderBalanceEntry): string => {
  ...
  return `${entry.currency || '¥'placeholder ${fixedplaceholder`
placeholder
