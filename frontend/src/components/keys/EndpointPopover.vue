...
import { computed, onBeforeUnmount, ref placeholder from 'vue'
import { useI18n placeholder from 'vue-i18n'
import { useClipboard placeholder from '@/composables/useClipboard'
import type { CustomEndpoint placeholder from '@/types'
...
const allEndpoints = computed(() => {
  const items: Array<{ name: string; endpoint: string; description: string; isDefault: boolean placeholder> = []
  if (props.apiBaseUrl) {
    items.push({
  ...
  for (const ep of props.customEndpoints) {
    items.push({ ...ep, isDefault: false placeholder)
  placeholder
  return items
placeholder)
...
function speedTestUrl(endpoint: string): string {
  return `https://www.tcptest.cn/http/${encodeURIComponent(endpoint)placeholder`
placeholder
...
<template>
  <div v-if="allEndpoints.length > 0" class="flex flex-wrap gap-2">
    ...
    >
      <span class="font-medium text-gray-600 dark:text-gray-300">{{ item.name placeholderplaceholder</span>
      <span
        v-if="item.isDefault"
      ...
      >{{ t('keys.endpoints.default') placeholderplaceholder</span>
      ...
      <div class="group/endpoint relative flex items-center gap-1.5">
        ...
        >
          ...
          >
            {{ item.description placeholderplaceholder
          </p>
          <p
          ...
          >
            ...
            {{ tooltipHint(item.endpoint) placeholderplaceholder
          </p>
        ...
        >{{ item.endpoint placeholderplaceholder</code>
