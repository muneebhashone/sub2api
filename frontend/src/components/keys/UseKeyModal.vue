  ...
  >
    <div class="space-y-4">
      ...
      <p class="text-sm text-gray-600 dark:text-gray-400">
        {{ t('keys.useKeyModal.description') placeholderplaceholder
      </p>
      ...
      <div class="relative">
        <div class="bg-gray-900 dark:bg-dark-900 rounded-xl overflow-hidden">
          ...
          <div class="flex items-center justify-between px-4 py-2 bg-gray-800 dark:bg-dark-800 border-b border-gray-700 dark:border-dark-700">
            <span class="text-xs text-gray-400 font-mono">{{ activeTabConfig?.filename placeholderplaceholder</span>
            <button
              @click="copyConfig"
            ...
            >
              ...
              {{ copied ? t('keys.useKeyModal.copied') : t('keys.useKeyModal.copy') placeholderplaceholder
            </button>
          </div>
          <!-- Code Content -->
      ...
      <div class="flex items-start gap-3 p-3 rounded-lg bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800">
        ...
        <p class="text-sm text-blue-700 dark:text-blue-300">
          {{ t('keys.useKeyModal.note') placeholderplaceholder
        </p>
      </div>
    </div>
...
import { ref, computed, h placeholder from 'vue'
...
const tabs = [
  { id: 'unix' as const, label: 'macOS / Linux', icon: AppleIcon, filename: 'Terminal' placeholder,
  { id: 'cmd' as const, label: 'Windows CMD', icon: WindowsIcon, filename: 'Command Prompt' placeholder,
  { id: 'powershell' as const, label: 'PowerShell', icon: WindowsIcon, filename: 'PowerShell' placeholder
]
...
const configCode = computed(() => {
  ...
  switch (activeTab.value) {
    case 'unix':
      return `export ANTHROPIC_BASE_URL="${baseUrlplaceholder"
export ANTHROPIC_AUTH_TOKEN="${apiKeyplaceholder"`
    case 'cmd':
      return `set ANTHROPIC_BASE_URL=${baseUrlplaceholder
set ANTHROPIC_AUTH_TOKEN=${apiKeyplaceholder`
    case 'powershell':
      return `$env:ANTHROPIC_BASE_URL="${baseUrlplaceholder"
...
const highlightedCode = computed(() => {
  ...
  switch (activeTab.value) {
    case 'unix':
      return `${keyword('export')placeholder ${variable('ANTHROPIC_BASE_URL')placeholder${operator('=')placeholder${string(`"${baseUrlplaceholder"`)placeholder
${keyword('export')placeholder ${variable('ANTHROPIC_AUTH_TOKEN')placeholder${operator('=')placeholder${string(`"${apiKeyplaceholder"`)placeholder`
    case 'cmd':
      return `${keyword('set')placeholder ${variable('ANTHROPIC_BASE_URL')placeholder${operator('=')placeholder${baseUrlplaceholder
${keyword('set')placeholder ${variable('ANTHROPIC_AUTH_TOKEN')placeholder${operator('=')placeholder${apiKeyplaceholder`
    case 'powershell':
      return `${keyword('$env:')placeholder${variable('ANTHROPIC_BASE_URL')placeholder${operator('=')placeholder${string(`"${baseUrlplaceholder"`)placeholder
