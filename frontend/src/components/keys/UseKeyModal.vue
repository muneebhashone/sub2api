    ...
    <div class="space-y-4">
      ...
      <div v-if="!platform" class="flex items-start gap-3 p-4 rounded-lg bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800">
        ...
        <div>
          <p class="text-sm font-medium text-yellow-800 dark:text-yellow-200">
            {{ t('keys.useKeyModal.noGroupTitle') placeholderplaceholder
          </p>
          <p class="text-sm text-yellow-700 dark:text-yellow-300 mt-1">
            {{ t('keys.useKeyModal.noGroupDescription') placeholderplaceholder
          </p>
        </div>
      </div>
      ...
      <template v-else>
        ...
        <p class="text-sm text-gray-600 dark:text-gray-400">
          {{ platformDescription placeholderplaceholder
        </p>
        ...
        >
          <div class="mb-2">
            <p class="text-sm font-medium text-gray-900 dark:text-white">
              {{ t('keys.useKeyModal.openai.authModeTitle') placeholderplaceholder
            </p>
            <p class="mt-0.5 text-xs text-gray-500 dark:text-gray-400">
              {{ t('keys.useKeyModal.openai.authModeDescription') placeholderplaceholder
            </p>
          </div>
          <div
          ...
          >
            ...
            >
              {{ t('keys.useKeyModal.openai.authModeLegacy') placeholderplaceholder
            </button>
            <button
              type="button"
              role="radio"
            ...
            >
              {{ t('keys.useKeyModal.openai.authModeApiKey') placeholderplaceholder
            </button>
          </div>
          <div
          ...
          >
            ...
            <p>{{ t('keys.useKeyModal.openai.authModeApiKeyRestartNotice') placeholderplaceholder</p>
          </div>
        </div>
        ...
        <div class="space-y-4">
          ...
          >
            ...
            <p v-if="file.hint" class="text-xs text-amber-600 dark:text-amber-400 mb-1.5 flex items-center gap-1">
              ...
              {{ file.hint placeholderplaceholder
            </p>
            <div class="bg-gray-900 dark:bg-dark-900 rounded-xl overflow-hidden">
              <!-- Code Header -->
              <div class="flex items-center justify-between px-4 py-2 bg-gray-800 dark:bg-dark-800 border-b border-gray-700 dark:border-dark-700">
                <span class="min-w-0 truncate text-xs text-gray-400 font-mono">{{ file.path placeholderplaceholder</span>
                <button
                  type="button"
                ...
                >
                  ...
                  {{ copiedIndex === index ? t('keys.useKeyModal.copied') : t('keys.useKeyModal.copy') placeholderplaceholder
                </button>
              </div>
              <!-- Code Content -->
        ...
        <div v-if="showPlatformNote" class="flex items-start gap-3 p-3 rounded-lg bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800">
          ...
          <p class="text-sm text-blue-700 dark:text-blue-300">
            {{ platformNote placeholderplaceholder
          </p>
        </div>
      </template>
    </div>
...
import { ref, computed, h, watch, type Component placeholder from 'vue'
...
const AppleIcon = {
  render() {
    return h('svg', {
      ...
      class: 'w-4 h-4'
    placeholder, [
      h('path', { d: 'M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.81-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z' placeholder)
    ])
  placeholder
placeholder
...
const WindowsIcon = {
  render() {
    return h('svg', {
      ...
      class: 'w-4 h-4'
    placeholder, [
      h('path', { d: 'M3 12V6.75l6-1.32v6.48L3 12zm17-9v8.75l-10 .15V5.21L20 3zM3 13l6 .09v6.81l-6-1.15V13zm7 .25l10 .15V21l-10-1.91v-5.84z' placeholder)
    ])
  placeholder
placeholder
...
const shellTabs: TabConfig[] = [
  { id: 'unix', label: 'macOS / Linux', icon: AppleIcon placeholder,
  { id: 'cmd', label: 'Windows CMD', icon: WindowsIcon placeholder,
  { id: 'powershell', label: 'PowerShell', icon: WindowsIcon placeholder
]

// OpenAI tabs (2 OS types)
const openaiTabs: TabConfig[] = [
  { id: 'unix', label: 'macOS / Linux', icon: AppleIcon placeholder,
  { id: 'windows', label: 'Windows', icon: WindowsIcon placeholder
]
...
const wrapToken = (className: string, value: string) =>
  `<span class="${classNameplaceholder">${escapeHtml(value)placeholder</span>`
...
const currentFiles = computed((): FileConfig[] => {
  ...
  const ensureV1 = (value: string) => {
    ...
    return trimmed.endsWith('/v1') ? trimmed : `${trimmedplaceholder/v1`
  placeholder
  const apiBase = ensureV1(baseRoot)
  const antigravityBase = ensureV1(`${baseRootplaceholder/antigravity`)
  ...
  switch (activeTab.value) {
    case 'unix':
      ...
      content = Object.entries(environment)
        .map(([name, value]) => `export ${nameplaceholder="${valueplaceholder"`)
        .join('\n')
      break
    case 'cmd':
      path = 'Command Prompt'
      content = Object.entries(environment)
        .map(([name, value]) => `set ${nameplaceholder=${valueplaceholder`)
        .join('\n')
      break
    case 'powershell':
      path = 'PowerShell'
      content = Object.entries(environment)
        .map(([name, value]) => `$env:${nameplaceholder="${valueplaceholder"`)
        .join('\n')
      break
    default:
      path = 'Terminal'
...
function generateGeminiCliContent(baseUrl: string, apiKey: string): FileConfig {
  ...
  switch (activeTab.value) {
    case 'unix':
      path = 'Terminal'
      content = `export GOOGLE_GEMINI_BASE_URL="${baseUrlplaceholder"
export GEMINI_API_KEY="${apiKeyplaceholder"
export GEMINI_MODEL="${modelplaceholder"  # ${modelCommentplaceholder`
      highlighted = `${keyword('export')placeholder ${variable('GOOGLE_GEMINI_BASE_URL')placeholder${operator('=')placeholder${string(`"${baseUrlplaceholder"`)placeholder
${keyword('export')placeholder ${variable('GEMINI_API_KEY')placeholder${operator('=')placeholder${string(`"${apiKeyplaceholder"`)placeholder
${keyword('export')placeholder ${variable('GEMINI_MODEL')placeholder${operator('=')placeholder${string(`"${modelplaceholder"`)placeholder  ${comment(`# ${modelCommentplaceholder`)placeholder`
      break
    case 'cmd':
      path = 'Command Prompt'
      content = `set GOOGLE_GEMINI_BASE_URL=${baseUrlplaceholder
set GEMINI_API_KEY=${apiKeyplaceholder
set GEMINI_MODEL=${modelplaceholder`
      highlighted = `${keyword('set')placeholder ${variable('GOOGLE_GEMINI_BASE_URL')placeholder${operator('=')placeholder${string(baseUrl)placeholder
${keyword('set')placeholder ${variable('GEMINI_API_KEY')placeholder${operator('=')placeholder${string(apiKey)placeholder
${keyword('set')placeholder ${variable('GEMINI_MODEL')placeholder${operator('=')placeholder${string(model)placeholder
${comment(`REM ${modelCommentplaceholder`)placeholder`
      break
    case 'powershell':
      path = 'PowerShell'
      content = `$env:GOOGLE_GEMINI_BASE_URL="${baseUrlplaceholder"
$env:GEMINI_API_KEY="${apiKeyplaceholder"
$env:GEMINI_MODEL="${modelplaceholder"  # ${modelCommentplaceholder`
      highlighted = `${keyword('$env:')placeholder${variable('GOOGLE_GEMINI_BASE_URL')placeholder${operator('=')placeholder${string(`"${baseUrlplaceholder"`)placeholder
${keyword('$env:')placeholder${variable('GEMINI_API_KEY')placeholder${operator('=')placeholder${string(`"${apiKeyplaceholder"`)placeholder
${keyword('$env:')placeholder${variable('GEMINI_MODEL')placeholder${operator('=')placeholder${string(`"${modelplaceholder"`)placeholder  ${comment(`# ${modelCommentplaceholder`)placeholder`
      break
    default:
      path = 'Terminal'
      content = ''
...
wire_api = "responses"
${generateCodexProviderAuthConfig()placeholder
...
export XAI_API_KEY="${apiKeyplaceholder"`
  ...
  const modelsListUrl = `${baseUrl.replace(/\/+$/, '')placeholder/models`
...
[endpoints]
models_base_url = "${baseUrlplaceholder"              # inference base; model list defaults to {baseplaceholder/models
models_list_url = "${modelsListUrlplaceholder"        # optional override (env: GROK_MODELS_LIST_URL)
xai_api_base_url = "${baseUrlplaceholder"             # public xAI API base override for gateway routing
cli_chat_proxy_base_url = "${baseUrlplaceholder"      # CLI chat-proxy base (env: GROK_CLI_CHAT_PROXY_BASE_URL)
...
function generateOpenCodeConfig(platform: string, baseUrl: string, apiKey: string, pathLabel?: string): FileConfig {
  ...
  placeholder else if (platform === 'anthropic') {
    provider[platform].npm = '@ai-sdk/anthropic'
  placeholder else if (platform === 'antigravity-claude') {
    provider[platform].npm = '@ai-sdk/anthropic'
