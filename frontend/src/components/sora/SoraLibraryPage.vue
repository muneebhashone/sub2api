  ...
  <div class="sora-gallery-page">
    ...
    <div class="sora-gallery-filter-bar">
      <div class="sora-gallery-filters">
        ...
        >
          {{ f.label placeholderplaceholder
        </button>
      </div>
      <span class="sora-gallery-count">
        {{ t('sora.galleryCount', { count: filteredItems.length placeholder) placeholderplaceholder
      </span>
    </div>
      ...
      >
        <div class="sora-gallery-card-thumb">
          ...
          <div v-else class="sora-gallery-card-image sora-gallery-card-placeholder" :class="getGradientClass(item.id)">
            {{ item.media_type === 'video' ? '🎬' : '🎨' placeholderplaceholder
          </div>
          ...
          >
            {{ item.media_type === 'video' ? 'VIDEO' : 'IMAGE' placeholderplaceholder
          </span>
          ...
          <span v-if="item.media_type === 'video'" class="sora-gallery-card-duration">
            {{ formatDuration(item) placeholderplaceholder
          </span>
        </div>

        <!-- 卡片底部信息 -->
        <div class="sora-gallery-card-info">
          <div class="sora-gallery-card-model">{{ item.model placeholderplaceholder</div>
          <div class="sora-gallery-card-time">{{ formatTime(item.created_at) placeholderplaceholder</div>
        </div>
      </div>
    </div>
    ...
    <div v-else-if="!loading" class="sora-gallery-empty">
      ...
      <h2 class="sora-gallery-empty-title">{{ t('sora.galleryEmptyTitle') placeholderplaceholder</h2>
      <p class="sora-gallery-empty-desc">{{ t('sora.galleryEmptyDesc') placeholderplaceholder</p>
      <button class="sora-gallery-empty-btn" @click="emit('switchToGenerate')">
        {{ t('sora.startCreating') placeholderplaceholder
      </button>
    </div>
    ...
    <div v-if="hasMore && filteredItems.length > 0" class="sora-gallery-load-more">
      <button
        ...
        @click="loadMore"
      >
        {{ loading ? t('sora.loading') : t('sora.loadMore') placeholderplaceholder
      </button>
    </div>
...
import soraAPI, { type SoraGeneration placeholder from '@/api/sora'
...
const filters = computed(() => [
  { value: 'all', label: t('sora.filterAll') placeholder,
  { value: 'video', label: t('sora.filterVideo') placeholder,
  { value: 'image', label: t('sora.filterImage') placeholder
])
...
function formatTime(iso: string): string {
  ...
  if (diff < 3600000) return t('sora.minutesAgo', { n: Math.floor(diff / 60000) placeholder)
  if (diff < 86400000) return t('sora.hoursAgo', { n: Math.floor(diff / 3600000) placeholder)
...
function formatDuration(item: SoraGeneration): string {
  ...
  if (match) {
    ...
    return `0:${sec.toString().padStart(2, '0')placeholder`
  placeholder
  return '0:10'
placeholder
...
/* 渐变背景 */
.gradient-bg-1 { background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); placeholder
.gradient-bg-2 { background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%); placeholder
.gradient-bg-3 { background: linear-gradient(135deg, #4facfe 0%, #00f2fe 100%); placeholder
.gradient-bg-4 { background: linear-gradient(135deg, #43e97b 0%, #38f9d7 100%); placeholder
.gradient-bg-5 { background: linear-gradient(135deg, #fa709a 0%, #fee140 100%); placeholder
.gradient-bg-6 { background: linear-gradient(135deg, #a18cd1 0%, #fbc2eb 100%); placeholder
.gradient-bg-7 { background: linear-gradient(135deg, #fccb90 0%, #d57eeb 100%); placeholder
.gradient-bg-8 { background: linear-gradient(135deg, #e0c3fc 0%, #8ec5fc 100%); placeholder
