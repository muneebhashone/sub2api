  ...
  >
    ...
    <div class="sora-task-header">
      <div class="sora-task-status">
        ...
        <span class="sora-status-label" :class="statusLabelClass">{{ statusText placeholderplaceholder</span>
      </div>
      <div class="sora-task-header-right">
        <span class="sora-model-tag">{{ generation.model placeholderplaceholder</span>
        <button
          v-if="generation.status === 'pending' || generation.status === 'generating'"
          class="sora-cancel-btn"
          @click="emit('cancel', generation.id)"
        >
          ✕ {{ t('sora.cancel') placeholderplaceholder
        </button>
      </div>
    </div>

    <!-- 提示词 -->
    <div class="sora-task-prompt" :class="{ 'line-through': generation.status === 'cancelled' placeholder">
      {{ generation.prompt placeholderplaceholder
    </div>

    <!-- 错误分类（失败时） -->
    <div v-if="generation.status === 'failed' && generation.error_message" class="sora-task-error-category">
      ⛔ {{ t('sora.errorCategory') placeholderplaceholder
    </div>
    <div v-if="generation.status === 'failed' && generation.error_message" class="sora-task-error-message">
      {{ generation.error_message placeholderplaceholder
    </div>
    ...
    <div v-if="showProgress" class="sora-task-progress-wrapper">
      ...
      <div v-if="generation.status !== 'failed'" class="sora-task-progress-info">
        <span>{{ progressInfoText placeholderplaceholder</span>
        <span>{{ progressInfoRight placeholderplaceholder</span>
      </div>
    </div>
    ...
    <div v-if="showActions" class="sora-task-actions">
      ...
      <template v-if="generation.status === 'completed'">
        ...
        <span v-if="generation.storage_type !== 'upstream'" class="sora-saved-badge">
          ✓ {{ t('sora.savedToCloud') placeholderplaceholder
        </span>
        <!-- 保存到存储按钮（upstream 时） -->
        <button
        ...
        >
          📥 {{ t('sora.downloadLocal') placeholderplaceholder
        </a>
        <!-- 倒计时文本（upstream） -->
        <span v-if="isUpstream && !isExpired" class="sora-countdown-text">
          ⏱ {{ t('sora.upstreamCountdown', { time: countdownText placeholder) placeholderplaceholder {{ t('sora.canDownload') placeholderplaceholder
        </span>
        <span v-if="isUpstream && isExpired" class="sora-countdown-text expired">
          {{ t('sora.upstreamExpired') placeholderplaceholder
        </span>
      </template>
      ...
      <template v-if="generation.status === 'failed' || generation.status === 'cancelled'">
        <button class="sora-action-btn primary" @click="emit('retry', generation)">
          🔄 {{ generation.status === 'cancelled' ? t('sora.regenrate') : t('sora.retry') placeholderplaceholder
        </button>
        <button class="sora-action-btn secondary" @click="emit('delete', generation.id)">
          🗑 {{ t('sora.delete') placeholderplaceholder
        </button>
      </template>
    </div>
    ...
    <div v-if="isUpstream && !isExpired && generation.status === 'completed'" class="sora-countdown-bar-wrapper">
      <div class="sora-countdown-bar">
        <div class="sora-countdown-bar-fill" :style="{ width: countdownPercent + '%' placeholder" />
      </div>
    </div>
  </div>
</template>
...
const props = defineProps<{ generation: SoraGeneration placeholder>()
...
const progressWidth = computed(() => {
  ...
  if (s === 'generating') {
    ...
    return `${Math.round(progress)placeholder%`
  placeholder
  return '100%'
placeholder)
...
const progressInfoText = computed(() => {
  ...
  if (s === 'generating') {
    const created = new Date(props.generation.created_at).getTime()
    const elapsed = Date.now() - created
    return `${t('sora.waited')placeholder ${formatElapsed(elapsed)placeholder`
  placeholder
  return ''
placeholder)
...
function formatElapsed(ms: number): string {
  ...
  const sec = s % 60
  return `${mplaceholder:${sec.toString().padStart(2, '0')placeholder`
placeholder
...
const countdownText = computed(() => {
  ...
  const s = totalSec % 60
  return `${mplaceholder:${s.toString().padStart(2, '0')placeholder`
placeholder)
...
.sora-status-dot.queued { background: var(--sora-text-tertiary, #666); placeholder
.sora-status-dot.generating {
  background: var(--sora-warning, #F59E0B);
  animation: sora-pulse-dot 1.5s ease-in-out infinite;
placeholder
.sora-status-dot.completed { background: var(--sora-success, #10B981); placeholder
.sora-status-dot.failed { background: var(--sora-error, #EF4444); placeholder
.sora-status-dot.cancelled { background: var(--sora-text-tertiary, #666); placeholder
...
/* 状态标签 */
.sora-status-label.queued { color: var(--sora-text-secondary, #A0A0A0); placeholder
.sora-status-label.generating { color: var(--sora-warning, #F59E0B); placeholder
.sora-status-label.completed { color: var(--sora-success, #10B981); placeholder
.sora-status-label.failed { color: var(--sora-error, #EF4444); placeholder
.sora-status-label.cancelled { color: var(--sora-text-tertiary, #666); placeholder
...
@keyframes sora-progress-shimmer {
  0% { opacity: 1; placeholder
  50% { opacity: 0.6; placeholder
