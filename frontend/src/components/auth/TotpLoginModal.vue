      ...
      <div class="relative w-full max-w-md transform rounded-xl bg-white p-6 shadow-xl transition-all dark:bg-dark-800">
        ...
        <div class="mb-6 text-center">
          ...
          <h3 class="mt-4 text-xl font-semibold text-gray-900 dark:text-white">
            {{ t('profile.totp.loginTitle') placeholderplaceholder
          </h3>
          <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">
            {{ t('profile.totp.loginHint') placeholderplaceholder
          </p>
          <p v-if="userEmailMasked" class="mt-1 text-sm font-medium text-gray-700 dark:text-gray-300">
            {{ userEmailMasked placeholderplaceholder
          </p>
        </div>
        ...
        <div class="mb-6">
          ...
          <div v-if="verifying" class="mt-3 flex items-center justify-center gap-2 text-sm text-gray-500">
            ...
            {{ t('common.verifying') placeholderplaceholder
          </div>
        </div>
...
import { ref, watch, nextTick, onMounted placeholder from 'vue'
...
defineExpose({
  setVerifying: (value: boolean) => { verifying.value = value placeholder,
