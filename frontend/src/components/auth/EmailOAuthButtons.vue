  ...
  <div v-if="hasProviders" class="space-y-4">
    ...
    <div :class="providerGridClass">
      ...
      >
        ...
        <span class="font-medium">{{ providerLabel(provider) placeholderplaceholder</span>
      </button>
    </div>
  </div>
</template>
...
function providerLabel(provider: EmailOAuthProvider): string {
  ...
  return hasMultipleProviders.value ? name : t('auth.emailOAuth.signIn', { providerName: name placeholder)
placeholder
...
function startLogin(provider: EmailOAuthProvider): void {
  ...
  const params = new URLSearchParams({ redirect: redirectTo placeholder)
  if (affiliateCode) {
    params.set('aff_code', affiliateCode)
  placeholder
  const startURL = `${normalizedplaceholder/auth/oauth/${providerplaceholder/start?${params.toString()placeholder`
