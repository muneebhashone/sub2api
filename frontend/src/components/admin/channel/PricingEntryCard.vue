  ...
  <div class="rounded-lg border border-gray-200 bg-gray-50 p-3 dark:border-dark-600 dark:bg-dark-800">
    ...
    >
      ...
      <div v-if="collapsed" class="flex min-w-0 flex-1 items-center gap-2 overflow-hidden">
        ...
        >
          {{ billingModeLabel placeholderplaceholder
        </span>
      </div>
    ...
    <div
      ...
      :class="{ 'collapsible-content--collapsed': collapsed placeholder"
    >
      <div class="collapsible-inner">
          ...
          <div class="w-40">
            ...
            <Select
              ...
              @update:modelValue="emit('update', {
                ...
                intervals: [],
                time_pricing: { ...entry.time_pricing, periods: [] placeholder,
              placeholder)"
        ...
        <div v-if="entry.billing_mode === 'token'">
          ...
          <div class="mt-1 grid grid-cols-2 gap-2 sm:grid-cols-6">
            ...
            <div>
              <label class="text-xs text-gray-400">{{ t('admin.channels.form.imageInputPrice') placeholderplaceholder</label>
          ...
          <div v-if="enableTierMultipliers" class="mt-3 grid max-w-md grid-cols-2 gap-2">
            <div>
              <label class="text-xs text-gray-400">{{ t('admin.channels.form.fastMultiplier') placeholderplaceholder</label>
            ...
            <div>
              <label class="text-xs text-gray-400">{{ t('admin.channels.form.flexMultiplier') placeholderplaceholder</label>
          ...
          <TimePricingSection
            ...
            @update:model-value="emit('update', { ...entry, time_pricing: $event placeholder)"
          />
        </div>
        ...
        <div v-else-if="entry.billing_mode === 'image' || entry.billing_mode === 'video'">
          ...
          <label class="mt-3 block text-xs font-medium text-gray-500 dark:text-gray-400">
            {{ entry.billing_mode === 'video' ? t('admin.channels.form.defaultVideoPrice') : t('admin.channels.form.defaultImagePrice') placeholderplaceholder
            <span class="ml-1 font-normal text-gray-400">$</span>
          </label>
          <div class="mt-1 w-48">
            <input :value="entry.per_request_price" @input="emitField('per_request_price', ($event.target as HTMLInputElement).value)"
          ...
          <div class="mt-3 flex items-center justify-between">
            <label class="text-xs font-medium text-gray-500 dark:text-gray-400">
              {{ entry.billing_mode === 'video' ? t('admin.channels.form.videoTiers') : t('admin.channels.form.imageTiers') placeholderplaceholder
            </label>
...
import { perTokenToMTok, getPlatformTagClass placeholder from './types'
...
const billingModeOptions = computed(() => [
  ...
  { value: 'video', label: t('admin.channels.billingMode.video') placeholder
])
...
async function onModelsUpdate(newModels: string[]) {
  ...
  emit('update', { ...props.entry, models: newModels placeholder)
