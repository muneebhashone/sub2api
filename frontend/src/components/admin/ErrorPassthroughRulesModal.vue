<template>
  ...
  >
    <div class="space-y-4">
      ...
      <div class="flex items-center justify-between">
        <p class="text-sm text-gray-500 dark:text-gray-400">
          {{ t('admin.errorPassthrough.description') placeholderplaceholder
        </p>
        <button @click="showCreateModal = true" class="btn btn-primary btn-sm">
          <Icon name="plus" size="sm" class="mr-1" />
          {{ t('admin.errorPassthrough.createRule') placeholderplaceholder
        </button>
      </div>
      ...
      <div v-else-if="rules.length === 0" class="py-8 text-center">
        ...
        <h4 class="mb-1 text-sm font-medium text-gray-900 dark:text-white">
          {{ t('admin.errorPassthrough.noRules') placeholderplaceholder
        </h4>
        <p class="text-sm text-gray-500 dark:text-gray-400">
          {{ t('admin.errorPassthrough.createFirstRule') placeholderplaceholder
        </p>
      </div>
          ...
          <thead class="sticky top-0 bg-gray-50 dark:bg-dark-700">
            <tr>
              <th class="px-3 py-2 text-left text-xs font-medium uppercase text-gray-500 dark:text-gray-400">
                {{ t('admin.errorPassthrough.columns.priority') placeholderplaceholder
              </th>
              <th class="px-3 py-2 text-left text-xs font-medium uppercase text-gray-500 dark:text-gray-400">
                {{ t('admin.errorPassthrough.columns.name') placeholderplaceholder
              </th>
              <th class="px-3 py-2 text-left text-xs font-medium uppercase text-gray-500 dark:text-gray-400">
                {{ t('admin.errorPassthrough.columns.conditions') placeholderplaceholder
              </th>
              <th class="px-3 py-2 text-left text-xs font-medium uppercase text-gray-500 dark:text-gray-400">
                {{ t('admin.errorPassthrough.columns.platforms') placeholderplaceholder
              </th>
              <th class="px-3 py-2 text-left text-xs font-medium uppercase text-gray-500 dark:text-gray-400">
                {{ t('admin.errorPassthrough.columns.behavior') placeholderplaceholder
              </th>
              <th class="px-3 py-2 text-left text-xs font-medium uppercase text-gray-500 dark:text-gray-400">
                {{ t('admin.errorPassthrough.columns.status') placeholderplaceholder
              </th>
              <th class="px-3 py-2 text-left text-xs font-medium uppercase text-gray-500 dark:text-gray-400">
                {{ t('admin.errorPassthrough.columns.actions') placeholderplaceholder
              </th>
            </tr>
          </thead>
            ...
            <tr v-for="rule in rules" :key="rule.id" class="hover:bg-gray-50 dark:hover:bg-dark-700">
              <td class="whitespace-nowrap px-3 py-2">
                <span class="inline-flex h-5 w-5 items-center justify-center rounded bg-gray-100 text-xs font-medium text-gray-700 dark:bg-dark-600 dark:text-gray-300">
                  {{ rule.priority placeholderplaceholder
                </span>
              </td>
              <td class="px-3 py-2">
                <div class="font-medium text-gray-900 dark:text-white text-sm">{{ rule.name placeholderplaceholder</div>
                <div v-if="rule.description" class="mt-0.5 text-xs text-gray-500 dark:text-gray-400 max-w-xs truncate">
                  {{ rule.description placeholderplaceholder
                </div>
              </td>
              <td class="px-3 py-2">
                <div class="flex flex-wrap gap-1 max-w-48">
                  ...
                  >
                    +{{ rule.error_codes.length - 3 placeholderplaceholder
                  </span>
                  <span
                  ...
                  >
                    "{{ keyword.length > 10 ? keyword.substring(0, 10) + '...' : keyword placeholderplaceholder"
                  </span>
                  <span
                    v-if="rule.keywords.length > 1"
                    class="text-xs text-gray-500"
                  >
                    +{{ rule.keywords.length - 1 placeholderplaceholder
                  </span>
                </div>
                <div class="mt-0.5 text-xs text-gray-500 dark:text-gray-400">
                  {{ t('admin.errorPassthrough.matchMode.' + rule.match_mode) placeholderplaceholder
                </div>
              </td>
              <td class="px-3 py-2">
                <div v-if="rule.platforms.length === 0" class="text-xs text-gray-500 dark:text-gray-400">
                  {{ t('admin.errorPassthrough.allPlatforms') placeholderplaceholder
                </div>
                <div v-else class="flex flex-wrap gap-1">
                  ...
                  >
                    {{ platform placeholderplaceholder
                  </span>
                  <span v-if="rule.platforms.length > 2" class="text-xs text-gray-500">
                    +{{ rule.platforms.length - 2 placeholderplaceholder
                  </span>
                </div>
              </td>
              <td class="px-3 py-2">
                <div class="text-xs space-y-0.5">
                  <div class="flex items-center gap-1">
                    ...
                    <span class="text-gray-600 dark:text-gray-400">
                      {{ t('admin.errorPassthrough.code') placeholderplaceholder:
                      {{ rule.passthrough_code ? t('admin.errorPassthrough.passthrough') : (rule.response_code || '-') placeholderplaceholder
                    </span>
                  </div>
                  <div class="flex items-center gap-1">
                    <Icon
                    ...
                    />
                    <span class="text-gray-600 dark:text-gray-400">
                      {{ t('admin.errorPassthrough.body') placeholderplaceholder:
                      {{ rule.passthrough_body ? t('admin.errorPassthrough.passthrough') : t('admin.errorPassthrough.custom') placeholderplaceholder
                    </span>
                  </div>
                  <div v-if="rule.skip_monitoring" class="flex items-center gap-1">
                    ...
                    />
                    <span class="text-gray-600 dark:text-gray-400">
                      {{ t('admin.errorPassthrough.skipMonitoring') placeholderplaceholder
                    </span>
                  </div>
                </div>
              </td>
    ...
    >
      <form @submit.prevent="handleSubmit" class="space-y-4">
        ...
        <div class="grid grid-cols-2 gap-4">
          <div>
            <label class="input-label">{{ t('admin.errorPassthrough.form.name') placeholderplaceholder</label>
            <input
              v-model="form.name"
              type="text"
              required
          ...
          <div>
            <label class="input-label">{{ t('admin.errorPassthrough.form.priority') placeholderplaceholder</label>
            <input
              ...
              class="input"
            />
            <p class="input-hint">{{ t('admin.errorPassthrough.form.priorityHint') placeholderplaceholder</p>
          </div>
        </div>

        <div>
          <label class="input-label">{{ t('admin.errorPassthrough.form.description') placeholderplaceholder</label>
          <input
        ...
        <div class="rounded-lg border border-gray-200 p-3 dark:border-dark-600">
          <h4 class="mb-2 text-sm font-medium text-gray-900 dark:text-white">
            {{ t('admin.errorPassthrough.form.matchConditions') placeholderplaceholder
          </h4>

          <div class="grid grid-cols-2 gap-3">
            <div>
              <label class="input-label text-xs">{{ t('admin.errorPassthrough.form.errorCodes') placeholderplaceholder</label>
              <input
              ...
              />
              <p class="input-hint text-xs">{{ t('admin.errorPassthrough.form.errorCodesHint') placeholderplaceholder</p>
            </div>
            <div>
              <label class="input-label text-xs">{{ t('admin.errorPassthrough.form.keywords') placeholderplaceholder</label>
              <textarea
              ...
              />
              <p class="input-hint text-xs">{{ t('admin.errorPassthrough.form.keywordsHint') placeholderplaceholder</p>
            </div>
          </div>

          <div class="mt-3">
            <label class="input-label text-xs">{{ t('admin.errorPassthrough.form.matchMode') placeholderplaceholder</label>
            <div class="mt-1 space-y-2">
              ...
              >
                ...
                />
                <div class="flex-1">
                  <span class="text-xs font-medium text-gray-700 dark:text-gray-300">{{ option.label placeholderplaceholder</span>
                  <p class="text-xs text-gray-500 dark:text-gray-400">{{ option.description placeholderplaceholder</p>
                </div>
              </label>
            </div>
          </div>
          ...
          <div class="mt-3">
            <label class="input-label text-xs">{{ t('admin.errorPassthrough.form.platforms') placeholderplaceholder</label>
            <div class="flex flex-wrap gap-3">
              ...
              >
                ...
                />
                <span class="text-xs text-gray-700 dark:text-gray-300">{{ platform.label placeholderplaceholder</span>
              </label>
            </div>
            <p class="input-hint text-xs mt-1">{{ t('admin.errorPassthrough.form.platformsHint') placeholderplaceholder</p>
          </div>
        </div>
        ...
        <div class="rounded-lg border border-gray-200 p-3 dark:border-dark-600">
          <h4 class="mb-2 text-sm font-medium text-gray-900 dark:text-white">
            {{ t('admin.errorPassthrough.form.responseBehavior') placeholderplaceholder
          </h4>
          ...
          <div class="grid grid-cols-2 gap-3">
            <div>
              <label class="flex items-center gap-1.5">
                ...
                <span class="text-xs font-medium text-gray-700 dark:text-gray-300">
                  {{ t('admin.errorPassthrough.form.passthroughCode') placeholderplaceholder
                </span>
              </label>
              <div v-if="!form.passthrough_code" class="mt-2">
                <label class="input-label text-xs">{{ t('admin.errorPassthrough.form.responseCode') placeholderplaceholder</label>
                <input
            ...
            <div>
              <label class="flex items-center gap-1.5">
                ...
                />
                <span class="text-xs font-medium text-gray-700 dark:text-gray-300">
                  {{ t('admin.errorPassthrough.form.passthroughBody') placeholderplaceholder
                </span>
              </label>
              <div v-if="!form.passthrough_body" class="mt-2">
                <label class="input-label text-xs">{{ t('admin.errorPassthrough.form.customMessage') placeholderplaceholder</label>
                <input
        ...
        <div class="flex items-center gap-1.5">
          ...
          />
          <span class="text-xs font-medium text-gray-700 dark:text-gray-300">
            {{ t('admin.errorPassthrough.form.skipMonitoring') placeholderplaceholder
          </span>
        </div>
        <p class="input-hint text-xs -mt-3">{{ t('admin.errorPassthrough.form.skipMonitoringHint') placeholderplaceholder</p>
        ...
        <div class="flex items-center gap-1.5">
          ...
          />
          <span class="text-xs font-medium text-gray-700 dark:text-gray-300">
            {{ t('admin.errorPassthrough.form.enabled') placeholderplaceholder
          </span>
        </div>
      </form>
      ...
      <template #footer>
        <div class="flex justify-end gap-3">
          ...
          <button @click="handleSubmit" :disabled="submitting" class="btn btn-primary">
            ...
            {{ showEditModal ? t('common.update') : t('common.create') placeholderplaceholder
          </button>
        </div>
      </template>
    </BaseDialog>
    ...
    <ConfirmDialog
      ...
      :message="t('admin.errorPassthrough.deleteConfirm', { name: deletingRule?.name placeholder)"
...
const matchModeOptions = computed(() => [
  { value: 'any', label: t('admin.errorPassthrough.matchMode.any'), description: t('admin.errorPassthrough.matchMode.anyHint') placeholder,
