...
// --- apply to monitors ---
const confirmApply_ = reactive<{ show: boolean; tpl: ChannelMonitorTemplate | null placeholder>({
  show: false,
  tpl: null,
placeholder)
...
async function doApply() {
  ...
  if (!tpl) return
  try {
    const { affected placeholder = await adminAPI.channelMonitorTemplate.apply(tpl.id)
