<template>
  ...
  >
    <form id="channel-monitor-form" @submit.prevent="handleSubmit" class="space-y-5">
      ...
      <div>
        <label class="input-label">{{ t('admin.channelMonitor.form.checkMode') placeholderplaceholder</label>
        <div class="grid gap-3 sm:grid-cols-3" data-testid="monitor-check-mode">
          <button
            v-for="opt in checkModeOptions"
            :key="opt.value"
            type="button"
            :data-testid="`monitor-check-mode-${opt.valueplaceholder`"
      ...
      <div>
        ...
        <div class="grid grid-cols-2 gap-3 sm:grid-cols-4">
          <button
            ...
            :key="opt.value"
            type="button"
            :data-testid="`monitor-provider-${opt.valueplaceholder`"
      ...
      <div v-if="usesQuotaMode">
        <label class="input-label">
          {{ t('admin.channelMonitor.form.linkedAccount') placeholderplaceholder <span class="text-red-500">*</span>
        </label>
        ...
        <p class="mt-1 text-xs text-gray-400">{{ t('admin.channelMonitor.form.linkedAccountHint') placeholderplaceholder</p>
        <p v-if="form.provider === PROVIDER_OPENAI" class="mt-1 text-xs text-amber-600 dark:text-amber-400">
          {{ t('admin.channelMonitor.form.openAIQuotaProbeHint') placeholderplaceholder
        </p>
        <p v-if="accountHydrationFailed" class="mt-1 text-xs text-amber-600 dark:text-amber-400">
          {{ t('admin.channelMonitor.form.linkedAccountMissing') placeholderplaceholder
        </p>
        <p v-if="accountOptions.length === 0 && !accountsLoading && !accountSearchQuery" class="mt-1 text-xs text-amber-600 dark:text-amber-400">
          {{ t('admin.channelMonitor.form.linkedAccountEmpty') placeholderplaceholder
        </p>
      </div>
      ...
      <div>
        <label class="input-label">{{ t('admin.channelMonitor.form.jitterSeconds') placeholderplaceholder</label>
        <input v-model.number="form.jitter_seconds" type="number" min="0" :max="maxJitterSeconds" class="input" />
        <p class="mt-1 text-xs text-gray-400">{{ t('admin.channelMonitor.form.jitterSecondsHint') placeholderplaceholder</p>
      </div>
...
const templateOptions = computed(() => {
  ...
  return [
    ...
    ...items.map((t) => ({ value: String(t.id), label: templateOptionLabel(t) placeholder)),
  ]
placeholder)
...
function templateOptionLabel(tpl: ChannelMonitorTemplate): string {
  ...
  return `${tpl.nameplaceholder · ${t(labelKey)placeholder`
placeholder
...
const accountOptions = computed(() => {
  const opts = linkedAccounts.value.map((a) => ({
    value: String(a.id),
    label: `${a.nameplaceholder (#${a.idplaceholder)`,
  placeholder))
  const pinned = pinnedAccount.value
  if (pinned && !linkedAccounts.value.some((a) => a.id === pinned.id)) {
    opts.unshift({ value: String(pinned.id), label: `${pinned.nameplaceholder (#${pinned.idplaceholder)` placeholder)
  placeholder
  return opts
placeholder)
...
async function loadLinkedAccounts(search = '') {
  ...
  try {
    const res = await adminAPI.accounts.list(
      1,
      50,
      { platform: form.provider, ...(search ? { search placeholder : {placeholder) placeholder,
      { signal: controller.signal placeholder,
    )
    if (seq !== accountSearchSeq) return
    linkedAccounts.value = (res.items || []).map((a) => ({ id: a.id, name: a.name placeholder))
...
async function ensureSelectedAccountHydrated() {
  ...
  try {
    ...
    if (String(account.platform) !== form.provider) {
      ...
      return
    placeholder
    pinnedAccount.value = { id: account.id, name: account.name placeholder
  placeholder catch {
...
placeholder, { flush: 'sync' placeholder)
