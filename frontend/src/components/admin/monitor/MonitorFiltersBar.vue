  ...
  <div class="flex flex-col justify-between gap-4 lg:flex-row lg:items-start">
    ...
    <div class="flex w-full flex-shrink-0 flex-wrap items-center justify-end gap-3 lg:w-auto">
      ...
      >
        ...
        {{ t('admin.channelMonitor.template.manageButton') placeholderplaceholder
      </button>
      <button @click="$emit('create')" class="btn btn-primary">
        <Icon name="plus" size="md" class="mr-2" />
        {{ t('admin.channelMonitor.createButton') placeholderplaceholder
      </button>
    </div>
  </div>
</template>
...
const search = defineModel<string>('search', { required: true placeholder)
const provider = defineModel<Provider | ''>('provider', { required: true placeholder)
const enabled = defineModel<'' | 'true' | 'false'>('enabled', { required: true placeholder)

const { t placeholder = useI18n()

const providerFilterOptions = computed(() => [
  { value: '', label: t('admin.channelMonitor.allProviders') placeholder,
...
const enabledFilterOptions = computed(() => [
  { value: '', label: t('admin.channelMonitor.allStatus') placeholder,
  { value: 'true', label: t('admin.channelMonitor.onlyEnabled') placeholder,
  { value: 'false', label: t('admin.channelMonitor.onlyDisabled') placeholder,
])
</script>
