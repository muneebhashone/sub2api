...
function deferred<T>() {
  ...
  return { promise, resolve, reject placeholder
placeholder
...
describe('UsageFilters — user search dropdown', () => {
  ...
  it('(a) labels deleted users with the i18n badge and (b) sorts active users before deleted ones, (c) selection sets user_id', async () => {
    ...
    mockSearchUsers.mockResolvedValue([
      { id: 2, email: 'gone@test.com', deleted: true placeholder,
      { id: 1, email: 'active@test.com', deleted: false placeholder,
    ])
  ...
  it('keeps results from the latest user search when responses arrive out of order', async () => {
    const firstSearch = deferred<Array<{ id: number; email: string; deleted: boolean placeholder>>()
    const secondSearch = deferred<Array<{ id: number; email: string; deleted: boolean placeholder>>()
    mockSearchUsers
    ...
    secondSearch.resolve([{ id: 2, email: 'ab@test.com', deleted: false placeholder])
    await flushPromises()
    expect(wrapper.text()).toContain('ab@test.com')

    firstSearch.resolve([{ id: 1, email: 'a@test.com', deleted: false placeholder])
    await flushPromises()
    expect(wrapper.text()).toContain('ab@test.com')
  ...
  it('does not restore stale user results after the search is cleared', async () => {
    const pendingSearch = deferred<Array<{ id: number; email: string; deleted: boolean placeholder>>()
    ...
    pendingSearch.resolve([{ id: 3, email: 'stale@test.com', deleted: false placeholder])
    await flushPromises()
