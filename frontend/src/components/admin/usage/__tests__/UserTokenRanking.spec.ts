...
const item = (id: number, tokens: number) => ({
  user_id: id,
  email: `u${idplaceholder@test.com`,
  requests: 1,
  input_tokens: tokens,
  output_tokens: 0,
  cache_tokens: 0,
...
const mountRanking = (props: Record<string, unknown> = {placeholder) =>
  mount(UserTokenRanking, {
    props: {
      ...
      ...props,
    placeholder,
    global: { stubs: { Select: true, LoadingSpinner: true placeholder placeholder,
  placeholder)

describe('UserTokenRanking', () => {
  beforeEach(() => {
    getUserBreakdown.mockReset()
    getUserBreakdown.mockResolvedValue({ users: [item(1, 100), item(2, 50)] placeholder)
  placeholder)

  it('loads on mount with shared filters and emits select-user with id + email on row click', async () => {
    const wrapper = mountRanking({ filters: { group_id: 3 placeholder, model: 'claude-fable-5' placeholder)
    await flushPromises()
  ...
  it('reloads when shared filters change', async () => {
    ...
    await wrapper.setProps({ filters: { user_id: 9 placeholder placeholder)
    await flushPromises()

    expect(getUserBreakdown).toHaveBeenCalledTimes(2)
    expect(getUserBreakdown).toHaveBeenLastCalledWith(expect.objectContaining({ user_id: 9 placeholder))
  placeholder)
placeholder)
