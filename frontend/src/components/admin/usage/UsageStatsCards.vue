<template>
  <div class="grid grid-cols-2 gap-4 lg:grid-cols-4">
    <div class="card p-4 flex items-center gap-3">
      ...
      <div>
        <p class="text-xs font-medium text-gray-500">{{ t('usage.totalRequests') placeholderplaceholder</p>
        <p class="text-xl font-bold">{{ stats?.total_requests?.toLocaleString() || '0' placeholderplaceholder</p>
        <p class="text-xs text-gray-400">{{ t('usage.inSelectedRange') placeholderplaceholder</p>
      </div>
    </div>
    <div class="card p-4 flex items-center gap-3">
      ...
      <div>
        <p class="text-xs font-medium text-gray-500">{{ t('usage.totalTokens') placeholderplaceholder</p>
        <p class="text-xl font-bold">{{ formatTokens(stats?.total_tokens || 0) placeholderplaceholder</p>
        <p class="flex flex-wrap items-center gap-x-1 text-xs text-gray-500">
          <span>{{ t('usage.in') placeholderplaceholder: {{ formatTokens(stats?.total_input_tokens || 0) placeholderplaceholder</span>
          <span>/</span>
          <span>{{ t('usage.out') placeholderplaceholder: {{ formatTokens(stats?.total_output_tokens || 0) placeholderplaceholder</span>
          <span>/</span>
          <span class="group relative inline-flex cursor-help items-center gap-0.5" tabindex="0">
            <span>{{ cacheLabel() placeholderplaceholder: {{ formatTokens(stats?.total_cache_tokens || 0) placeholderplaceholder</span>
            <svg
            ...
            >
              <span class="mb-2 block font-medium text-gray-900 dark:text-white">
                {{ cacheDetailLabel() placeholderplaceholder
              </span>
              <span class="flex items-center justify-between gap-3">
                <span>{{ t('usage.cacheCreationTokensLabel') placeholderplaceholder</span>
                <span class="tabular-nums">
                  {{ formatTokens(stats?.total_cache_creation_tokens || 0) placeholderplaceholder
                </span>
              </span>
              <span class="mt-1 flex items-center justify-between gap-3">
                <span>{{ t('usage.cacheReadTokensLabel') placeholderplaceholder</span>
                <span class="tabular-nums">
                  {{ formatTokens(stats?.total_cache_read_tokens || 0) placeholderplaceholder
                </span>
              </span>
            </span>
          </span>
    ...
    <div class="card p-4 flex items-center gap-3">
      ...
      <div class="min-w-0 flex-1">
        <p class="text-xs font-medium text-gray-500">{{ t('usage.totalCost') placeholderplaceholder</p>
    ...
    <div class="card p-4 flex items-center gap-3">
      ...
      <div><p class="text-xs font-medium text-gray-500">{{ t('usage.avgDuration') placeholderplaceholder</p><p class="text-xl font-bold">{{ formatDuration(stats?.average_duration_ms || 0) placeholderplaceholder</p></div>
    </div>
  </div>
</template>
