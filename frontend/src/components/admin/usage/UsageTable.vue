      ...
      >
        ...
        <template #cell-tokens="{ row placeholder">
          ...
          <div v-if="row.image_count > 0 && row.billing_mode === BILLING_MODE_IMAGE" class="flex items-center gap-1.5">
            ...
            <span class="text-gray-400">({{ row.image_size || '2K' placeholderplaceholder)</span>
          </div>
          <!-- Token 请求 -->
        ...
        <template #cell-ip_address="{ row placeholder">
          <span v-if="row.ip_address" class="text-sm font-mono text-gray-600 dark:text-gray-400">{{ row.ip_address placeholderplaceholder</span>
          <span v-else class="text-sm text-gray-400 dark:text-gray-500">-</span>
        </template>
...
import { getBillingModeLabel, getBillingModeBadgeClass, BILLING_MODE_TOKEN, BILLING_MODE_IMAGE placeholder from '@/utils/billingMode'
