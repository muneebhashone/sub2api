  ...
  <div :class="flat ? '' : 'card overflow-hidden'">
    ...
    >
      <span v-if="pendingIpCount > 0" class="text-xs text-gray-500 dark:text-gray-400">
        {{ t('usage.ipGeo.pending', { count: pendingIpCount placeholder) placeholderplaceholder
      </span>
      <button
        type="button"
      ...
      >
        {{ ipGeoBatchLoading ? t('usage.ipGeo.batchFetching') : t('usage.ipGeo.batchFetch') placeholderplaceholder
      </button>
    </div>
      ...
      >
        <template #cell-user="{ row placeholder">
          <div class="text-sm">
            ...
            >
              {{ row.user.email placeholderplaceholder
            </button>
            ...
            <span v-if="row.user?.deleted_at" class="ml-1 inline-flex items-center rounded px-1 py-px text-[10px] font-medium leading-tight bg-rose-100 text-rose-600 ring-1 ring-inset ring-rose-200 dark:bg-rose-500/20 dark:text-rose-400 dark:ring-rose-500/30">
              {{ t('admin.usage.userDeletedBadge') placeholderplaceholder
            </span>
          ...
          <div class="space-y-0.5 text-xs">
            <div v-if="row.model_mapping_chain && row.model_mapping_chain.includes('→')" class="space-y-0.5">
              <div v-for="(step, i) in row.model_mapping_chain.split('→')" :key="i"
                   ...
                   :style="i > 0 ? `padding-left: ${i * 0.75placeholderrem` : ''">
                <span v-if="i > 0" class="mr-0.5">↳</span>{{ step placeholderplaceholder
              </div>
            </div>
            <div v-else-if="row.upstream_model && row.upstream_model !== row.model" class="space-y-0.5">
              ...
              <div class="break-all text-gray-500 dark:text-gray-400">
                <span class="mr-0.5">↳</span>{{ row.upstream_model placeholderplaceholder
              </div>
            </div>
            <span v-else class="font-medium text-gray-900 dark:text-white">{{ row.model placeholderplaceholder</span>
            <div
            ...
            >
              <span class="mr-1">↳ {{ t('usage.upstreamResponseModel') placeholderplaceholder:</span>{{ row.upstream_response_model placeholderplaceholder
              <span
              ...
              >
                {{ isLikelyModelVariant(row) ? t('usage.modelVariant') : t('usage.modelMismatch') placeholderplaceholder
              </span>
            </div>
          </div>
        </template>
        ...
        <template #cell-endpoint="{ row placeholder">
          <div class="max-w-[320px] space-y-1 text-xs">
            ...
            <div v-if="showUpstreamEndpoint" class="break-all text-gray-700 dark:text-gray-300">
              <span class="font-medium text-gray-500 dark:text-gray-400">{{ t('usage.upstream') placeholderplaceholder:</span>
              <span class="ml-1">{{ row.upstream_endpoint?.trim() || '-' placeholderplaceholder</span>
            </div>
          </div>
        </template>
            ...
            <div class="space-y-1 text-sm">
              ...
              <div v-if="hasImageInputTokens(row)" class="flex items-center gap-2">
                <div class="inline-flex items-center gap-1">
                  ...
                  <span class="font-medium text-fuchsia-600 dark:text-fuchsia-400">{{ row.image_input_tokens.toLocaleString() placeholderplaceholder</span>
                </div>
              </div>
        ...
        <template #cell-latency="{ row placeholder">
          <div class="flex items-stretch gap-2">
            ...
            <div class="grid grid-cols-[max-content_max-content] items-baseline gap-x-2 gap-y-0.5 text-xs">
              <span class="text-gray-400 dark:text-gray-500">{{ t('usage.latencyFirstToken') placeholderplaceholder</span>
              <span v-if="row.first_token_ms != null" class="font-medium tabular-nums" :class="LATENCY_TEXT_CLASSES[firstTokenSeverity(row.first_token_ms)]">{{ formatDuration(row.first_token_ms) placeholderplaceholder</span>
              <span v-else class="text-gray-400 dark:text-gray-500">-</span>
              <span class="text-gray-400 dark:text-gray-500">{{ t('usage.latencyDuration') placeholderplaceholder</span>
              <span class="font-medium tabular-nums" :class="LATENCY_TEXT_CLASSES[durationSeverity(row.duration_ms ?? 0)]">{{ formatDuration(row.duration_ms) placeholderplaceholder</span>
            </div>
          </div>
        </template>
        ...
        <template #cell-ip_address="{ row placeholder">
          <div v-if="row.ip_address">
            <span class="text-sm font-mono text-gray-600 dark:text-gray-400">{{ row.ip_address placeholderplaceholder</span>
        ...
        <div class="space-y-1.5">
          <div>
            ...
            <div v-if="tokenTooltipData && hasImageInputTokens(tokenTooltipData) && textInputTokens(tokenTooltipData) > 0" class="flex items-center justify-between gap-4">
              <span class="text-gray-400">{{ t('admin.usage.inputTokens') placeholderplaceholder</span>
              <span class="font-medium text-white">{{ textInputTokens(tokenTooltipData).toLocaleString() placeholderplaceholder</span>
            </div>
            <div v-if="tokenTooltipData && hasImageInputTokens(tokenTooltipData)" class="flex items-center justify-between gap-4">
              <span class="text-gray-400">{{ t('usage.imageInputTokens') placeholderplaceholder</span>
              <span class="font-medium text-fuchsia-300">{{ tokenTooltipData.image_input_tokens.toLocaleString() placeholderplaceholder</span>
            </div>
        ...
        <div class="space-y-1.5">
          ...
          <div class="mb-2 border-b border-gray-700 pb-1.5">
            ...
            <div v-if="tooltipData && hasImageInputCost(tooltipData)" class="flex items-center justify-between gap-4">
              <span class="text-gray-400">{{ t('usage.imageInputCost') placeholderplaceholder</span>
              <span class="font-medium text-fuchsia-300">${{ tooltipData.image_input_cost.toFixed(6) placeholderplaceholder</span>
            </div>
            ...
            <template v-if="tooltipData && !isImageUsage(tooltipData) && (!tooltipData.billing_mode || tooltipData.billing_mode === BILLING_MODE_TOKEN)">
              <div v-if="tooltipData && textInputTokens(tooltipData) > 0" class="flex items-center justify-between gap-4">
                ...
                <span class="font-medium text-sky-300">{{ formatTokenPricePerMillion(tooltipData.input_cost, textInputTokens(tooltipData)) placeholderplaceholder {{ t('usage.perMillionTokens') placeholderplaceholder</span>
              </div>
              <div v-if="tooltipData && hasImageInputTokens(tooltipData)" class="flex items-center justify-between gap-4">
                <span class="text-gray-400">{{ t('usage.imageInputTokenPrice') placeholderplaceholder</span>
                <span class="font-medium text-fuchsia-300">{{ formatTokenPricePerMillion(tooltipData.image_input_cost ?? 0, tooltipData.image_input_tokens) placeholderplaceholder {{ t('usage.perMillionTokens') placeholderplaceholder</span>
              </div>
          ...
          <div class="flex items-center justify-between gap-6">
            ...
            <span class="font-semibold text-blue-400">{{ formatMultiplier(tooltipData?.rate_multiplier || 1) placeholderplaceholderx</span>
          </div>
          <div class="flex items-center justify-between gap-6">
          ...
          <div class="flex items-center justify-between gap-6">
            <span class="text-gray-400">{{ t('usage.userBilled') placeholderplaceholder</span>
            <span class="font-semibold text-green-400">${{ tooltipData?.actual_cost?.toFixed(6) || '0.000000' placeholderplaceholder</span>
          </div>
          <!-- Account billing (separated from user billing) -->
          <template v-if="showAccountBilling">
            <div class="flex items-center justify-between gap-6 border-t border-gray-700 pt-1.5">
              <span class="text-gray-400">{{ t('usage.accountMultiplier') placeholderplaceholder</span>
              <span class="font-semibold text-blue-400">{{ formatMultiplier(tooltipData?.account_rate_multiplier ?? 1) placeholderplaceholderx</span>
            </div>
            <div class="flex items-center justify-between gap-6">
              <span class="text-gray-400">{{ t('usage.accountBilled') placeholderplaceholder</span>
              <span class="font-semibold text-green-400">
                ...
                placeholder).toFixed(6) placeholderplaceholder
              </span>
            </div>
          </template>
        </div>
...
import {
  ...
  durationSeverity,
  firstTokenSeverity,
placeholder from '@/utils/latencyHealth'
import {
  BILLING_MODE_TOKEN,
  getBillingModeLabel,
...
function accountBilled(row: { total_cost?: number | null; account_stats_cost?: number | null; account_rate_multiplier?: number | null placeholder): number {
...
import { fetchBatch, getEntry placeholder from '@/utils/ipGeoLookup'
...
const normalizeModelVariant = (model: string): string => model
  ...
  .toLowerCase()
  .replace(/-latest$/, '')
  .replace(/-\d{4placeholder-\d{2placeholder-\d{2placeholder$/, '')
  .replace(/-\d{8placeholder$/, '')
...
const modelAuditTitle = (row: AdminUsageLog): string => [
  `${t('usage.requestedModel')placeholder: ${row.model || '-'placeholder`,
  `${t('usage.sentUpstreamModel')placeholder: ${sentUpstreamModel(row) || '-'placeholder`,
  `${t('usage.upstreamResponseModel')placeholder: ${row.upstream_response_model || '-'placeholder`,
].join('\n')
...
const formatDuration = (ms: number | null | undefined): string => {
  ...
  if (ms < 60_000) return `${(ms / 1000).toFixed(2)placeholders`
  const totalSec = Math.round(ms / 1000)
  if (totalSec < 3600) return `${Math.floor(totalSec / 60)placeholderm ${totalSec % 60placeholders`
  return `${Math.floor(totalSec / 3600)placeholderh ${Math.floor((totalSec % 3600) / 60)placeholderm`
placeholder
