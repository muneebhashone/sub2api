    ...
    <div v-if="group" class="space-y-4">
      ...
      <div class="flex flex-wrap items-center gap-3 rounded-lg bg-gray-50 px-4 py-2.5 text-sm dark:bg-dark-700">
        <span class="inline-flex items-center gap-1.5" :class="platformColorClass">
          ...
          {{ t('admin.groups.platforms.' + group.platform) placeholderplaceholder
        </span>
      ...
      <div class="rounded-lg border border-gray-200 p-3 dark:border-dark-600">
        ...
        <div class="flex items-end gap-2">
          <div class="relative flex-1">
            ...
            >
              ...
              >
                <span class="text-gray-400">#{{ user.id placeholderplaceholder</span>
                <span class="text-gray-900 dark:text-white">{{ user.username || user.email placeholderplaceholder</span>
                <span v-if="user.username" class="text-xs text-gray-400">{{ user.email placeholderplaceholder</span>
              </button>
            </div>
          </div>
          <div class="w-24">
        ...
        <div v-if="localEntries.length > 0" class="mt-3 flex items-center gap-3 border-t border-gray-100 pt-3 dark:border-dark-600">
          <span class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('admin.groups.batchAdjust') placeholderplaceholder</span>
          <div class="flex items-center gap-1.5">
            ...
            >
              {{ t('admin.groups.applyMultiplier') placeholderplaceholder
            </button>
          </div>
          <div class="ml-auto">
            <button
              ...
              @click="clearAllLocal"
            >
              {{ t('admin.groups.clearAll') placeholderplaceholder
            </button>
          </div>
        </div>
      </div>
      ...
      <div v-else>
        <h4 class="mb-2 text-sm font-medium text-gray-700 dark:text-gray-300">
          {{ t('admin.groups.rateMultipliers') placeholderplaceholder ({{ localEntries.length placeholderplaceholder)
        </h4>
              ...
              <table class="w-full min-w-max text-sm">
                <thead class="sticky top-0 z-[1]">
                  <tr class="border-b border-gray-200 bg-gray-50 dark:border-dark-600 dark:bg-dark-700">
                    <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('admin.groups.columns.userEmail') placeholderplaceholder</th>
                    <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400">ID</th>
                    <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('admin.groups.columns.userName') placeholderplaceholder</th>
                    <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('admin.groups.columns.userNotes') placeholderplaceholder</th>
                    <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('admin.groups.columns.userStatus') placeholderplaceholder</th>
                    <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('admin.groups.columns.rateMultiplier') placeholderplaceholder</th>
                    <th v-if="showFinalRate" class="px-3 py-2 text-left text-xs font-medium text-primary-600 dark:text-primary-400">{{ t('admin.groups.finalRate') placeholderplaceholder</th>
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-dark-600">
                  ...
                  >
                    <td class="px-3 py-2 text-gray-600 dark:text-gray-400">{{ entry.user_email placeholderplaceholder</td>
                    <td class="whitespace-nowrap px-3 py-2 text-gray-400 dark:text-gray-500">{{ entry.user_id placeholderplaceholder</td>
                    <td class="whitespace-nowrap px-3 py-2 text-gray-900 dark:text-white">{{ entry.user_name || '-' placeholderplaceholder</td>
                    <td class="max-w-[160px] truncate px-3 py-2 text-gray-500 dark:text-gray-400" :title="entry.user_notes">{{ entry.user_notes || '-' placeholderplaceholder</td>
                    <td class="whitespace-nowrap px-3 py-2">
                      ...
                      >
                        {{ entry.user_status placeholderplaceholder
                      </span>
                    </td>
                    <td class="whitespace-nowrap px-3 py-2">
                      <input
                    ...
                    <td v-if="showFinalRate" class="whitespace-nowrap px-3 py-2 font-medium text-primary-600 dark:text-primary-400">
                      {{ computeFinalRate(entry.rate_multiplier) placeholderplaceholder
                    </td>
                    <td class="px-2 py-2">
                      <button
                        type="button"
      ...
      <div class="flex items-center gap-3 border-t border-gray-200 pt-4 dark:border-dark-600">
        ...
        <template v-if="isDirty">
          <span class="text-xs text-amber-600 dark:text-amber-400">{{ t('admin.groups.unsavedChanges') placeholderplaceholder</span>
          <button
            type="button"
            class="text-xs font-medium text-primary-600 hover:text-primary-700 dark:text-primary-400 dark:hover:text-primary-300"
            @click="handleCancel"
          >
            {{ t('admin.groups.revertChanges') placeholderplaceholder
          </button>
        </template>
        <!-- 右侧：关闭 / 保存 -->
...
interface LocalEntry extends GroupRateMultiplierEntry {placeholder
...
const cloneEntries = (entries: GroupRateMultiplierEntry[]): LocalEntry[] => {
  return entries.map(e => ({ ...e placeholder))
placeholder
