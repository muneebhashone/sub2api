<template>
  <div class="space-y-4">
    <div>
      <label :for="`${idPrefixplaceholder-max-effort`" class="input-label">
        {{ t("admin.groups.form.maxReasoningEffort") placeholderplaceholder
      </label>
      <Select
        :id="`${idPrefixplaceholder-max-effort`"
      ...
      />
      <p class="input-hint">{{ t("admin.groups.form.maxReasoningEffortHint") placeholderplaceholder</p>
    </div>

    <div class="border-t border-gray-200 pt-4 dark:border-dark-600">
      <div class="mb-3 flex items-center justify-between gap-3">
        <label class="input-label mb-0">
          {{ t("admin.groups.form.reasoningEffortMappings") placeholderplaceholder
        </label>
        <button
          type="button"
        ...
        >
          ...
          {{ t("admin.groups.form.addReasoningEffortMapping") placeholderplaceholder
        </button>
      </div>
      ...
      <div v-if="mappings.length > 0" class="space-y-2">
        ...
        >
          <div class="grid gap-3 md:grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)_auto] md:items-start">
            <div>
              <label :for="`${idPrefixplaceholder-${row.idplaceholder-from`" class="input-label">
                {{ t("admin.groups.form.reasoningEffortFrom") placeholderplaceholder
              </label>
              <Select
                :id="`${idPrefixplaceholder-${row.idplaceholder-from`"
                ...
                :aria-describedby="showValidation && validationErrors[row.id]?.from ? `${idPrefixplaceholder-${row.idplaceholder-from-error` : undefined"
                :searchable="false"
                clearable
              ...
              <p
                ...
                :id="`${idPrefixplaceholder-${row.idplaceholder-from-error`"
                class="mt-1 text-xs text-red-600 dark:text-red-400"
                role="alert"
              >
                {{ mappingErrorText(validationErrors[row.id]?.from) placeholderplaceholder
              </p>
            </div>
            ...
            <div>
              <label :for="`${idPrefixplaceholder-${row.idplaceholder-to`" class="input-label">
                {{ t("admin.groups.form.reasoningEffortTo") placeholderplaceholder
              </label>
              <Select
                :id="`${idPrefixplaceholder-${row.idplaceholder-to`"
                :model-value="row.to"
                :options="reasoningEffortOptions"
                ...
                :aria-describedby="showValidation && validationErrors[row.id]?.to ? `${idPrefixplaceholder-${row.idplaceholder-to-error` : undefined"
                :searchable="false"
                clearable
              ...
              <p
                ...
                :id="`${idPrefixplaceholder-${row.idplaceholder-to-error`"
                class="mt-1 text-xs text-red-600 dark:text-red-400"
                role="alert"
              >
                {{ mappingErrorText(validationErrors[row.id]?.to) placeholderplaceholder
              </p>
            </div>
...
placeholder from "@/views/admin/groupsReasoningEffort";
...
const updateMapping = (
  ...
  emit(
    ...
    props.mappings.map((row) =>
      row.id === id ? { ...row, [field]: asString(value) placeholder : row,
    ),
  );
placeholder;
...
): string => (code ? t(`admin.groups.form.${codeplaceholder`) : "");
...
defineExpose({ validate, resetValidation placeholder);
</script>
