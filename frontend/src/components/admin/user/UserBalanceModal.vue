  ...
  <BaseDialog :show="show" :title="operation === 'add' ? t('admin.users.deposit') : t('admin.users.withdraw')" width="narrow" @close="$emit('close')">
    <form v-if="user" id="balance-form" @submit.prevent="handleBalanceSubmit" class="space-y-5">
      <div class="flex items-center gap-3 rounded-xl bg-gray-50 p-4 dark:bg-dark-700">
        ...
        <div class="flex-1"><p class="font-medium text-gray-900 dark:text-gray-100">{{ user.email placeholderplaceholder</p><p class="text-sm text-gray-500 dark:text-gray-400">{{ t('admin.users.currentBalance') placeholderplaceholder: ${{ formatBalance(user.balance) placeholderplaceholder</p></div>
      </div>
      <div>
        ...
        <div class="relative flex gap-2">
          ...
          <button v-if="operation === 'subtract'" type="button" @click="fillAllBalance" class="btn btn-secondary whitespace-nowrap">{{ t('admin.users.withdrawAll') placeholderplaceholder</button>
        </div>
      </div>
      ...
      <div v-if="form.amount > 0" class="rounded-xl border border-blue-200 bg-blue-50 p-4 dark:border-blue-800 dark:bg-blue-950"><div class="flex items-center justify-between text-sm"><span class="text-gray-700 dark:text-gray-300">{{ t('admin.users.newBalance') placeholderplaceholder:</span><span class="font-bold text-gray-900 dark:text-gray-100">${{ formatBalance(calculateNewBalance()) placeholderplaceholder</span></div></div>
    </form>
    <template #footer>
...
const handleBalanceSubmit = async () => {
  ...
  placeholder finally { submitting.value = false placeholder
placeholder
</script>
