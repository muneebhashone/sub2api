<template>
  <div class="card p-4">
    <h3 class="mb-4 text-sm font-semibold text-gray-900 dark:text-white">
      {{ t('payment.admin.topUsers') placeholderplaceholder
    </h3>
    <div
      ...
      <div v-for="[currency, currencyUsers] in sortedUsers(props.users)" :key="currency" class="space-y-2">
        ...
        >
          ...
          <span class="text-sm font-medium text-gray-900 dark:text-white">
            {{ formatMoney(currency, user.amount) placeholderplaceholder
          </span>
        </div>
      </div>
    </div>
...
import type { TopUserPaymentStats placeholder from '@/types/payment'
