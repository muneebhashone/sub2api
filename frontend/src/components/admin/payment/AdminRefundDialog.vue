  ...
  >
    <form id="refund-form" @submit.prevent="handleSubmit" class="space-y-4">
      ...
      >
        <div class="flex items-center gap-2 text-sm font-medium text-violet-700 dark:text-violet-300">
          ...
          {{ t('payment.admin.refundRequestInfo') placeholderplaceholder
        </div>
        <div v-if="order?.refund_requested_at" class="mt-2 flex justify-between text-sm">
          <span class="text-violet-600 dark:text-violet-400">{{ t('payment.admin.refundRequestedAt') placeholderplaceholder</span>
          <span class="text-violet-800 dark:text-violet-200">{{ formatDateTime(order.refund_requested_at) placeholderplaceholder</span>
        </div>
        <div v-if="order?.refund_request_reason" class="mt-1 text-sm">
          <span class="text-violet-600 dark:text-violet-400">{{ t('payment.admin.refundRequestReason') placeholderplaceholder:</span>
          <span class="ml-1 text-violet-800 dark:text-violet-200">{{ order.refund_request_reason placeholderplaceholder</span>
        </div>
      </div>
      ...
      <div class="rounded-lg bg-gray-50 p-3 dark:bg-dark-700">
        <div class="flex justify-between text-sm">
          ...
          <span class="font-mono text-gray-900 dark:text-white">#{{ order?.id placeholderplaceholder</span>
        </div>
      ...
      <div>
        <div class="flex items-center gap-2">
          ...
          <label for="deduct-balance" class="text-sm text-gray-700 dark:text-gray-300">
            {{ t('payment.admin.deductBalance') placeholderplaceholder
          </label>
          <span class="text-xs text-gray-500 dark:text-gray-400">{{ t('payment.admin.deductBalanceHint') placeholderplaceholder</span>
        </div>
        ...
        >
          {{ t('payment.admin.insufficientBalance') placeholderplaceholder
        </div>
        ...
        >
          {{ t('payment.admin.noDeduction') placeholderplaceholder
        </div>
      </div>
      ...
      <!-- Reason -->
      <div>
        <label class="input-label">{{ t('payment.admin.refundReason') placeholderplaceholder</label>
        <textarea
          v-model="form.reason"
          rows="3"
          class="input"
      ...
      >
        {{ warning placeholderplaceholder
      </div>
      ...
      <div v-if="requireForce" class="flex items-center gap-2">
        ...
        <label for="force-refund" class="text-sm font-medium text-red-600 dark:text-red-400">
          {{ t('payment.admin.forceRefund') placeholderplaceholder
        </label>
      </div>
    </form>
    ...
    <template #footer>
      <div class="flex justify-end gap-3">
        ...
        >
          {{ submitting ? t('common.processing') : t('payment.admin.confirmRefund') placeholderplaceholder
        </button>
      </div>
    </template>
  </BaseDialog>
...
import { reactive, computed, watch placeholder from 'vue'
