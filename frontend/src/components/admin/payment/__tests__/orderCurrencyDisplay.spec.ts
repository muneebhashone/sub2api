...
describe('admin order currency display', () => {
  it('uses order currency for paid/base/fee amounts and USD for credited/refund amounts', () => {
    const wrapper = mount(AdminOrderDetail, {
      props: {
        show: true,
        order: orderFactory({ currency: 'CNY' placeholder),
      placeholder,
      global: {
        stubs: {
  ...
  it('renders payment currency consistently in the shared order table', () => {
    const wrapper = mount(OrderTable, {
      props: {
        orders: [
          orderFactory({ id: 1, currency: 'USD', amount: 100, pay_amount: 108 placeholder),
          orderFactory({ id: 2, currency: 'CNY', amount: 100, pay_amount: 108 placeholder),
        ],
        loading: false,
        showUser: true,
      placeholder,
