...
const { getAvailableModels, copyToClipboard placeholder = vi.hoisted(() => ({
      ...
      getReader: () => ({
        read: vi.fn().mockImplementation(async () => {
          if (index < chunks.length) {
            return { done: false, value: chunks[index++] placeholder
          placeholder
          return { done: true, value: undefined placeholder
        placeholder)
      placeholder)
    placeholder
  ...
  return mount(AccountTestModal, {
    ...
    global: {
      stubs: {
        ...
        Select: { template: '<div class="select-stub"></div>' placeholder,
        TextArea: {
          props: ['modelValue'],
...
describe('AccountTestModal', () => {
  beforeEach(() => {
    ...
    global.fetch = vi.fn().mockResolvedValue(
      createStreamResponse([
        'data: {"type":"test_start","model":"gemini-2.5-flash-image"placeholder\n',
        'data: {"type":"image","image_url":"data:image/png;base64,QUJD","mime_type":"image/png"placeholder\n',
        'data: {"type":"test_complete","success":trueplaceholder\n'
      ])
    ) as any
  placeholder)
  ...
  it('grok 账号测试默认选择 Grok 模型', async () => {
    getAvailableModels.mockResolvedValue([
      { id: 'grok-4.3', display_name: 'Grok 4.3' placeholder,
      { id: 'grok-build-0.1', display_name: 'Grok Build 0.1' placeholder
    ])
    global.fetch = vi.fn().mockResolvedValue(
      createStreamResponse([
        'data: {"type":"test_start","model":"grok-4.3"placeholder\n',
        'data: {"type":"content","text":"ok"placeholder\n',
        'data: {"type":"test_complete","success":trueplaceholder\n'
      ])
    ) as any
