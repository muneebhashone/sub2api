    ...
    <div class="space-y-4">
      ...
      <div class="flex items-center justify-between">
        <p class="text-sm text-gray-500 dark:text-gray-400">
          {{ t('admin.scheduledTests.title') placeholderplaceholder
        </p>
        <button
        ...
        >
          ...
          {{ t('admin.scheduledTests.addPlan') placeholderplaceholder
        </button>
      </div>
        ...
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <div>
            <label class="mb-1 block text-xs font-medium text-gray-600 dark:text-gray-400">
              {{ t('admin.scheduledTests.model') placeholderplaceholder
            </label>
            <Select
          ...
          <div>
            <label class="mb-1 flex items-center gap-1 text-xs font-medium text-gray-600 dark:text-gray-400">
              {{ t('admin.scheduledTests.cronExpression') placeholderplaceholder
              <HelpTooltip>
                <template #trigger>
                ...
                <div class="space-y-1.5">
                  <p class="font-medium">{{ t('admin.scheduledTests.cronTooltipTitle') placeholderplaceholder</p>
                  <p>{{ t('admin.scheduledTests.cronTooltipMeaning') placeholderplaceholder</p>
                  <p>{{ t('admin.scheduledTests.cronTooltipExampleEvery30Min') placeholderplaceholder</p>
                  <p>{{ t('admin.scheduledTests.cronTooltipExampleHourly') placeholderplaceholder</p>
                  <p>{{ t('admin.scheduledTests.cronTooltipExampleDaily') placeholderplaceholder</p>
                  <p>{{ t('admin.scheduledTests.cronTooltipExampleWeekly') placeholderplaceholder</p>
                  <p>{{ t('admin.scheduledTests.cronTooltipRange') placeholderplaceholder</p>
                </div>
              </HelpTooltip>
            </label>
            <Input
          ...
          <div>
            <label class="mb-1 flex items-center gap-1 text-xs font-medium text-gray-600 dark:text-gray-400">
              {{ t('admin.scheduledTests.maxResults') placeholderplaceholder
              <HelpTooltip>
                <template #trigger>
                  <span class="inline-flex h-4 w-4 cursor-help items-center justify-center rounded-full border border-gray-400/70 text-[10px] font-semibold text-gray-400 transition-colors hover:border-primary-500 hover:text-primary-600 dark:border-gray-500 dark:text-gray-500 dark:hover:border-primary-400 dark:hover:text-primary-400">
                    ?
                ...
                <div class="space-y-1.5">
                  <p class="font-medium">{{ t('admin.scheduledTests.maxResultsTooltipTitle') placeholderplaceholder</p>
                  <p>{{ t('admin.scheduledTests.maxResultsTooltipMeaning') placeholderplaceholder</p>
                  <p>{{ t('admin.scheduledTests.maxResultsTooltipBody') placeholderplaceholder</p>
                  <p>{{ t('admin.scheduledTests.maxResultsTooltipExample') placeholderplaceholder</p>
                  <p>{{ t('admin.scheduledTests.maxResultsTooltipRange') placeholderplaceholder</p>
                </div>
              </HelpTooltip>
            </label>
            <Input
          ...
          <div class="flex items-end">
            <label class="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
              ...
              {{ t('admin.scheduledTests.enabled') placeholderplaceholder
            </label>
          </div>
          <div class="flex items-end">
            <div>
              <label class="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
                ...
                {{ t('admin.scheduledTests.autoRecover') placeholderplaceholder
              </label>
              <p class="mt-0.5 text-xs text-gray-400 dark:text-gray-500">
                {{ t('admin.scheduledTests.autoRecoverHelp') placeholderplaceholder
              </p>
            </div>
          </div>
        </div>
      ...
      <div v-if="loading" class="flex items-center justify-center py-8">
        ...
        <span class="ml-2 text-sm text-gray-500">{{ t('common.loading') placeholderplaceholder...</span>
      </div>
      ...
      >
        ...
        <p class="text-sm text-gray-500 dark:text-gray-400">
          {{ t('admin.scheduledTests.noPlans') placeholderplaceholder
        </p>
      </div>
        ...
        >
          ...
          >
            <div class="flex flex-1 items-center gap-4">
              ...
              <div class="min-w-0">
                <div class="text-sm font-medium text-gray-900 dark:text-gray-100">
                  {{ plan.model_id placeholderplaceholder
                </div>
                <div class="mt-0.5 font-mono text-xs text-gray-500 dark:text-gray-400">
                  {{ plan.cron_expression placeholderplaceholder
                </div>
              </div>
              ...
              <div class="flex items-center gap-1.5" @click.stop>
                ...
                <span class="text-xs text-gray-500 dark:text-gray-400">
                  {{ plan.enabled ? t('admin.scheduledTests.enabled') : '' placeholderplaceholder
                </span>
              </div>
            ...
            <div class="flex items-center gap-3">
              ...
              <div v-if="plan.last_run_at" class="hidden text-right text-xs text-gray-500 dark:text-gray-400 sm:block">
                <div>{{ t('admin.scheduledTests.lastRun') placeholderplaceholder</div>
                <div>{{ formatDateTime(plan.last_run_at) placeholderplaceholder</div>
              </div>

              <!-- Next Run -->
              <div v-if="plan.next_run_at" class="hidden text-right text-xs text-gray-500 dark:text-gray-400 sm:block">
                <div>{{ t('admin.scheduledTests.nextRun') placeholderplaceholder</div>
                <div>{{ formatDateTime(plan.next_run_at) placeholderplaceholder</div>
              </div>
          ...
          >
            <div class="mb-2 text-xs font-medium text-gray-600 dark:text-gray-400">
              {{ t('admin.scheduledTests.editPlan') placeholderplaceholder
            </div>
            <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
              <div>
                <label class="mb-1 block text-xs font-medium text-gray-600 dark:text-gray-400">
          ...
          >
            <div class="mb-2 text-xs font-medium text-gray-600 dark:text-gray-400">
              {{ t('admin.scheduledTests.results') placeholderplaceholder
            </div>
            ...
            <div v-if="loadingResults" class="flex items-center justify-center py-4">
              ...
              <span class="ml-2 text-xs text-gray-500">{{ t('common.loading') placeholderplaceholder...</span>
            </div>
            ...
            >
              {{ t('admin.scheduledTests.noResults') placeholderplaceholder
            </div>
            ...
            <div v-else class="max-h-64 space-y-2 overflow-y-auto">
              ...
              >
                <div class="flex items-center justify-between">
                  <div class="flex items-center gap-2">
                    ...
                    <span v-if="result.latency_ms > 0" class="text-xs text-gray-500 dark:text-gray-400">
                      {{ result.latency_ms placeholderplaceholderms
                    </span>
                  </div>

                  <!-- Started At -->
                  <span class="text-xs text-gray-400">
                    {{ formatDateTime(result.started_at) placeholderplaceholder
                  </span>
                </div>
                ...
                <div v-if="result.error_message" class="mt-2">
                  ...
                  >
                    {{ t('admin.scheduledTests.errorMessage') placeholderplaceholder
                    <Icon
                      name="chevronDown"
                      size="sm"
                      :class="[
                  ...
                  >{{ result.error_message placeholderplaceholder</pre>
                </div>
                <div v-else-if="result.response_text" class="mt-2">
                  <div
                    ...
                    @click="toggleResultDetail(result.id)"
                  >
                    {{ t('admin.scheduledTests.responseText') placeholderplaceholder
                    <Icon
                      name="chevronDown"
                      size="sm"
                      :class="[
                  ...
                  >{{ result.response_text placeholderplaceholder</pre>
                </div>
              </div>
            </div>
          </div>
...
import type { ScheduledTestPlan, ScheduledTestResult placeholder from '@/types'
...
const handleToggleEnabled = async (plan: ScheduledTestPlan, enabled: boolean) => {
  try {
    const updated = await adminAPI.scheduledTests.update(plan.id, { enabled placeholder)
