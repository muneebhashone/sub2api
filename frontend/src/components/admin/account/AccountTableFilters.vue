...
const props = defineProps(['searchQuery', 'filters']); const emit = defineEmits(['update:searchQuery', 'update:filters', 'change']); const { t placeholder = useI18n()
...
const pOpts = computed(() => [{ value: '', label: t('admin.accounts.allPlatforms') placeholder, { value: 'anthropic', label: 'Anthropic' placeholder, { value: 'openai', label: 'OpenAI' placeholder, { value: 'gemini', label: 'Gemini' placeholder, { value: 'sora', label: 'Sora' placeholder, { value: 'antigravity', label: 'Antigravity' placeholder])
...
const sOpts = computed(() => [{ value: '', label: t('admin.accounts.allStatus') placeholder, { value: 'active', label: t('admin.accounts.status.active') placeholder, { value: 'inactive', label: t('admin.accounts.status.inactive') placeholder, { value: 'error', label: t('admin.accounts.status.error') placeholder])
</script>
