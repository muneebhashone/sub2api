    ...
    <div class="space-y-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="bg-white p-6 rounded-lg shadow">
          ...
          <div class="text-2xl font-bold text-indigo-600">${{ balance placeholderplaceholder</div>
        </div>
...
<template>
  <AuthLayout>
    ...
    <form @submit.prevent="handleSubmit" class="space-y-4">
      ...
      >
        {{ loading ? 'Logging in...' : 'Login' placeholderplaceholder
      </button>
    </form>
        ...
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="key in apiKeys" :key="key.id">
              <td class="px-6 py-4 whitespace-nowrap">{{ key.name placeholderplaceholder</td>
              <td class="px-6 py-4 font-mono text-sm">{{ key.key placeholderplaceholder</td>
              <td class="px-6 py-4">
                <span
              ...
              <td class="px-6 py-4 text-sm text-gray-500">
                {{ new Date(key.created_at).toLocaleDateString() placeholderplaceholder
              </td>
        ...
        <div class="p-6">
          <div class="space-y-4">
            <div v-for="user in users" :key="user.id" class="flex items-center justify-between border-b pb-4">
              <div>
                <div class="font-medium text-gray-900">{{ user.username placeholderplaceholder</div>
                <div class="text-sm text-gray-500">{{ user.email placeholderplaceholder</div>
              </div>
              <div class="flex items-center space-x-4">
                ...
                <span class="text-sm font-medium text-gray-700">
                  ${{ user.balance.toFixed(2) placeholderplaceholder
                </span>
...
async function handleChangePassword() {
  try {
    ...
    passwordForm.value = { old_password: '', new_password: '' placeholder;
  placeholder catch (error) {
