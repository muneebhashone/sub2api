...
const userNavItems = computed((): NavItem[] => {
  const items: NavItem[] = [
    ...
    ...(appStore.cachedPublicSettings?.sora_client_enabled
      ? [{ path: '/sora', label: t('nav.sora'), icon: SoraIcon placeholder]
      : []),
...
const adminNavItems = computed((): NavItem[] => {
  ...
  if (authStore.isSimpleMode) {
    ...
    filtered.push({ path: '/admin/backup', label: t('nav.backup'), icon: CloudArrowUpIcon placeholder)
  ...
  baseItems.push({ path: '/admin/backup', label: t('nav.backup'), icon: CloudArrowUpIcon placeholder)
