...
export const useAuthStore = defineStore('auth', () => {
  ...
  function checkAuth(): void {
    ...
    if (savedToken && savedUser) {
      ...
      placeholder catch (error) {
        ...
        clearAuth({ preservePendingAuthSession: true placeholder)
      placeholder
    placeholder
  placeholder
  ...
  async function refreshUser(): Promise<User> {
    ...
    try {
      ...
      placeholder
      const { run_mode: _run_mode, ...userData placeholder = response.data
      user.value = userData
   ...
   * Internal helper function
   */
  function clearAuth(options?: { preservePendingAuthSession?: boolean placeholder): void {
    // Stop auto-refresh
    stopAutoRefresh()
    // Stop token refresh
    stopTokenRefresh()
