...
import type { BillingMode placeholder from '@/constants/channel'
...
export async function getAvailable(options?: { signal?: AbortSignal placeholder): Promise<UserAvailableChannel[]> {
  const { data placeholder = await apiClient.get<UserAvailableChannel[]>('/channels/available', {
    signal: options?.signal
  placeholder)
  return data
placeholder

export const userChannelsAPI = { getAvailable placeholder
