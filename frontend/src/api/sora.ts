...
export function normalizeGenerationListResponse(payload: unknown): GenerationListResponse {
  const record = asRecord(payload)
  if (!record) {
    return { data: [], total: 0, page: 1 placeholder
  placeholder
...
export async function generate(req: GenerateRequest): Promise<GenerateResponse> {
  const { data placeholder = await apiClient.post<GenerateResponse>('/sora/generate', req)
  return data
placeholder
...
export async function listGenerations(params?: {
  ...
  storage_type?: string
  media_type?: string
placeholder): Promise<GenerationListResponse> {
  const { data placeholder = await apiClient.get<unknown>('/sora/generations', { params placeholder)
  return normalizeGenerationListResponse(data)
placeholder

/** 查询生成记录详情 */
export async function getGeneration(id: number): Promise<SoraGeneration> {
  const { data placeholder = await apiClient.get<SoraGeneration>(`/sora/generations/${idplaceholder`)
  return data
placeholder

/** 删除生成记录 */
export async function deleteGeneration(id: number): Promise<{ message: string placeholder> {
  const { data placeholder = await apiClient.delete<{ message: string placeholder>(`/sora/generations/${idplaceholder`)
  return data
placeholder

/** 取消生成任务 */
export async function cancelGeneration(id: number): Promise<{ message: string placeholder> {
  const { data placeholder = await apiClient.post<{ message: string placeholder>(`/sora/generations/${idplaceholder/cancel`)
  return data
placeholder
...
export async function saveToStorage(
  id: number
): Promise<{ message: string; object_key: string; object_keys?: string[] placeholder> {
  const { data placeholder = await apiClient.post<{ message: string; object_key: string; object_keys?: string[] placeholder>(
    `/sora/generations/${idplaceholder/save`
  )
  return data
placeholder
...
export async function getQuota(): Promise<QuotaInfo> {
  const { data placeholder = await apiClient.get<QuotaInfo>('/sora/quota')
  return data
placeholder

/** 获取可用模型家族列表 */
export async function getModels(): Promise<SoraModelFamily[]> {
  const { data placeholder = await apiClient.get<unknown>('/sora/models')
  return normalizeModelFamiliesResponse(data)
placeholder

/** 获取存储状态 */
export async function getStorageStatus(): Promise<StorageStatus> {
  const { data placeholder = await apiClient.get<StorageStatus>('/sora/storage-status')
  return data
placeholder
