...
export async function getStats(
  paramsOrPeriod: (UsageQueryParams & { period?: string; timezone?: string placeholder) | string = 'today',
  apiKeyId?: number
): Promise<UsageStatsResponse> {
  const params: Record<string, unknown> = typeof paramsOrPeriod === 'string'
    ? { period: paramsOrPeriod placeholder
    : { ...paramsOrPeriod placeholder
...
export async function getDashboardModels(params?: {
  ...
  billing_mode?: string | null
  timezone?: string
placeholder): Promise<ModelStatsResponse> {
...
export async function getDashboardSnapshotV2(
  ...
  const { data placeholder = await apiClient.get<UsageDashboardSnapshotV2Response>(
...
export async function listMyErrorRequests(
  ...
  const { data placeholder = await apiClient.get<PaginatedResponse<UserErrorRequest>>('/usage/errors', {
    params
  placeholder)
  return data
placeholder
...
export async function getMyErrorDetail(id: number): Promise<UserErrorRequestDetail> {
  const { data placeholder = await apiClient.get<UserErrorRequestDetail>(`/usage/errors/${idplaceholder`)
  return data
placeholder
