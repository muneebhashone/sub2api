...
export async function getConfig(): Promise<ContentModerationConfig> {
  const { data placeholder = await apiClient.get<ContentModerationConfig>('/admin/risk-control/config')
  return data
placeholder
...
export async function updateConfig(
  ...
  const { data placeholder = await apiClient.put<ContentModerationConfig>('/admin/risk-control/config', payload)
  return data
placeholder

export async function getStatus(): Promise<ContentModerationRuntimeStatus> {
  const { data placeholder = await apiClient.get<ContentModerationRuntimeStatus>('/admin/risk-control/status')
  return data
placeholder

export async function testAPIKeys(
  payload: TestContentModerationAPIKeysPayload = {placeholder
): Promise<TestContentModerationAPIKeysResponse> {
  const { data placeholder = await apiClient.post<TestContentModerationAPIKeysResponse>('/admin/risk-control/api-keys/test', payload)
  return data
placeholder

export async function listLogs(
  params: ListContentModerationLogsParams = {placeholder
): Promise<ContentModerationLogsResponse> {
  const { data placeholder = await apiClient.get<ContentModerationLogsResponse>('/admin/risk-control/logs', {
    params,
  placeholder)
  return data
placeholder
...
export async function unbanUser(userID: number): Promise<ContentModerationUnbanUserResponse> {
  const { data placeholder = await apiClient.post<ContentModerationUnbanUserResponse>(
    `/admin/risk-control/users/${userIDplaceholder/unban`
  )
  return data
placeholder

export async function deleteFlaggedHash(inputHash: string): Promise<DeleteFlaggedHashResponse> {
  const { data placeholder = await apiClient.delete<DeleteFlaggedHashResponse>('/admin/risk-control/hashes', {
    data: { input_hash: inputHash placeholder,
  placeholder)
  return data
placeholder

export async function clearFlaggedHashes(): Promise<ClearFlaggedHashesResponse> {
  const { data placeholder = await apiClient.delete<ClearFlaggedHashesResponse>('/admin/risk-control/hashes/all')
  return data
placeholder
