...
export function buildAuthSourceDefaultsState(
  ...
  return AUTH_SOURCE_TYPES.reduce((acc, source) => {
    ...
    acc[source] = {
      balance: Number(raw[`auth_source_default_${sourceplaceholder_balance`] ?? AUTH_SOURCE_DEFAULT_BALANCE),
      concurrency: Math.max(
        1,
        Number(raw[`auth_source_default_${sourceplaceholder_concurrency`] ?? AUTH_SOURCE_DEFAULT_CONCURRENCY)
      ),
      ...
      grant_on_signup: raw[`auth_source_default_${sourceplaceholder_grant_on_signup`] !== false,
      grant_on_first_bind: raw[`auth_source_default_${sourceplaceholder_grant_on_first_bind`] === true,
    placeholder
    return acc
...
export function appendAuthSourceDefaultsToUpdateRequest(
  ...
  for (const source of AUTH_SOURCE_TYPES) {
    ...
    target[`auth_source_default_${sourceplaceholder_balance`] = Number(current.balance) || 0
    ...
    target[`auth_source_default_${sourceplaceholder_subscriptions`] = normalizeDefaultSubscriptionSettings(
      current.subscriptions
    )
    target[`auth_source_default_${sourceplaceholder_grant_on_signup`] = current.grant_on_signup
    target[`auth_source_default_${sourceplaceholder_grant_on_first_bind`] = current.grant_on_first_bind
  placeholder
