...
export function normalizeAccountSchedulingThresholdsMap(
  ...
  const result = {placeholder as AccountSchedulingThresholdsMap
...
export function normalizePlatformQuotasMap(input?: DefaultPlatformQuotasMap | null): DefaultPlatformQuotasMap {
  const result: DefaultPlatformQuotasMap = {placeholder
...
export function sanitizePlatformQuotasMap(input?: DefaultPlatformQuotasMap | null): DefaultPlatformQuotasMap {
  ...
  for (const p of PLATFORMS) {
    const src = input?.[p]
    result[p] = { daily: clean(src?.daily), weekly: clean(src?.weekly), monthly: clean(src?.monthly) placeholder
  placeholder
  return result
placeholder
...
> = {
  alipay: [
    { value: "", labelZh: "未配置", labelEn: "Not configured" placeholder,
    {
...
const WECHAT_CONNECT_MODE_OPTIONS: WeChatConnectModeOption[] = [
  { value: "open", labelZh: "PC 应用", labelEn: "PC App" placeholder,
  {
    value: "mp",
    labelZh: "公众号",
...
export function buildAuthSourceDefaultsState(
  ...
  return AUTH_SOURCE_TYPES.reduce((acc, source) => {
    const subscriptions = raw[`auth_source_default_${sourceplaceholder_subscriptions`];
    acc[source] = {
      balance: Number(
        raw[`auth_source_default_${sourceplaceholder_balance`] ??
          AUTH_SOURCE_DEFAULT_BALANCE,
      ),
      concurrency: Math.max(
        1,
        Number(
          raw[`auth_source_default_${sourceplaceholder_concurrency`] ??
      ...
      grant_on_signup:
        raw[`auth_source_default_${sourceplaceholder_grant_on_signup`] === true,
      grant_on_first_bind:
        raw[`auth_source_default_${sourceplaceholder_grant_on_first_bind`] === true,
      platform_quotas: normalizePlatformQuotasMap(raw[`auth_source_default_${sourceplaceholder_platform_quotas`] as DefaultPlatformQuotasMap | undefined),
    placeholder;
    return acc;
  placeholder, {placeholder as AuthSourceDefaultsState);
placeholder
...
export function appendAuthSourceDefaultsToUpdateRequest(
  ...
  for (const source of AUTH_SOURCE_TYPES) {
    ...
    target[`auth_source_default_${sourceplaceholder_balance`] =
      Number(current.balance) || 0;
    target[`auth_source_default_${sourceplaceholder_concurrency`] = Math.max(
      1,
      Math.floor(
        Number(current.concurrency) || AUTH_SOURCE_DEFAULT_CONCURRENCY,
      ),
    );
    target[`auth_source_default_${sourceplaceholder_subscriptions`] =
      normalizeDefaultSubscriptionSettings(current.subscriptions);
    target[`auth_source_default_${sourceplaceholder_grant_on_signup`] =
      current.grant_on_signup;
    target[`auth_source_default_${sourceplaceholder_grant_on_first_bind`] =
      current.grant_on_first_bind;
    target[`auth_source_default_${sourceplaceholder_platform_quotas`] = sanitizePlatformQuotasMap(current.platform_quotas)
  placeholder
...
export function resolveWeChatConnectModeCapabilities(
  ...
  legacyMode: unknown,
): { openEnabled: boolean; mpEnabled: boolean; mobileEnabled: boolean placeholder {
  if (
  ...
  switch (normalizeWeChatConnectMode(legacyMode)) {
    case "mp":
      return { openEnabled: false, mpEnabled: true, mobileEnabled: false placeholder;
    case "mobile":
      return { openEnabled: false, mpEnabled: false, mobileEnabled: true placeholder;
    default:
      return { openEnabled: true, mpEnabled: false, mobileEnabled: false placeholder;
  placeholder
placeholder
...
export async function updateSettings(
  ...
  const { data placeholder = await apiClient.put<SystemSettings>(
    "/admin/settings",
    settings,
  );
  return data;
...
export async function getEmailTemplates(): Promise<EmailTemplateListResponse> {
  const { data placeholder = await apiClient.get<EmailTemplateListResponse>(
...
export async function getEmailTemplate(
  ...
  const { data placeholder = await apiClient.get<EmailTemplateDetail>(
    `/admin/settings/email-templates/${encodeURIComponent(event)placeholder/${encodeURIComponent(locale)placeholder`,
  );
  return data;
placeholder
...
export async function updateEmailTemplate(
  ...
  const { data placeholder = await apiClient.put<EmailTemplateDetail>(
    `/admin/settings/email-templates/${encodeURIComponent(event)placeholder/${encodeURIComponent(locale)placeholder`,
    request,
  );
  return data;
...
export async function restoreOfficialEmailTemplate(
  ...
  const { data placeholder = await apiClient.post<EmailTemplateDetail>(
    `/admin/settings/email-templates/${encodeURIComponent(event)placeholder/${encodeURIComponent(locale)placeholder/restore-official`,
  );
  return data;
placeholder
...
export async function previewEmailTemplate(
  ...
  const { data placeholder = await apiClient.post<EmailTemplatePreviewResponse>(
...
export async function getAdminApiKey(): Promise<AdminApiKeyStatus> {
  const { data placeholder = await apiClient.get<AdminApiKeyStatus>(
...
export async function regenerateAdminApiKey(): Promise<{ key: string placeholder> {
  const { data placeholder = await apiClient.post<{ key: string placeholder>(
...
export async function getOverloadCooldownSettings(): Promise<OverloadCooldownSettings> {
  const { data placeholder = await apiClient.get<OverloadCooldownSettings>(
...
export async function getRateLimit429CooldownSettings(): Promise<RateLimit429CooldownSettings> {
  const { data placeholder = await apiClient.get<RateLimit429CooldownSettings>(
...
export async function updateRateLimit429CooldownSettings(
  ...
  const { data placeholder = await apiClient.put<RateLimit429CooldownSettings>(
    "/admin/settings/rate-limit-429-cooldown",
    settings,
  );
  return data;
...
export async function getPanelRateLimitSettings(): Promise<PanelRateLimitSettings> {
  const { data placeholder = await apiClient.get<PanelRateLimitSettings>(
...
export async function updatePanelRateLimitSettings(
  ...
  const { data placeholder = await apiClient.put<PanelRateLimitSettings>(
    "/admin/settings/panel-rate-limit",
    settings,
  );
  return data;
...
export async function getStreamTimeoutSettings(): Promise<StreamTimeoutSettings> {
  const { data placeholder = await apiClient.get<StreamTimeoutSettings>(
...
export async function getRectifierSettings(): Promise<RectifierSettings> {
  const { data placeholder = await apiClient.get<RectifierSettings>(
...
export async function getBetaPolicySettings(): Promise<BetaPolicySettings> {
  const { data placeholder = await apiClient.get<BetaPolicySettings>(
