...
import { apiClient, buildGatewayUrl placeholder from '../client'
...
export async function getConcurrencyStats(platform?: string, groupId?: number | null): Promise<OpsConcurrencyStatsResponse> {
  const params: Record<string, any> = {placeholder
  if (platform) {
  ...
  const { data placeholder = await apiClient.get<OpsConcurrencyStatsResponse>('/admin/ops/concurrency', { params placeholder)
  return data
placeholder

export async function getUserConcurrencyStats(): Promise<OpsUserConcurrencyStatsResponse> {
  const { data placeholder = await apiClient.get<OpsUserConcurrencyStatsResponse>('/admin/ops/user-concurrency')
  return data
placeholder
...
export async function getAccountAvailabilityStats(platform?: string, groupId?: number | null): Promise<OpsAccountAvailabilityStatsResponse> {
  ...
  if (typeof groupId === 'number' && groupId > 0) {
    params.group_id = groupId
  placeholder
  const { data placeholder = await apiClient.get<OpsAccountAvailabilityStatsResponse>('/admin/ops/account-availability', { params placeholder)
  return data
placeholder
...
export interface SubscribeQPSOptions {
   ...
   */
  onReconnectScheduled?: (info: { attempt: number, delayMs: number placeholder) => void
  wsBaseUrl?: string
  /**
...
export function subscribeQPS(onMessage: (data: any) => void, options: SubscribeQPSOptions = {placeholder): () => void {
  ...
  const startStaleTimer = () => {
    ...
    staleTimer = setInterval(() => {
      ...
      if (ageMs > staleTimeoutMs) {
        ...
        ws.close()
      placeholder
    placeholder, staleCheckIntervalMs)
  placeholder
  ...
  const scheduleReconnect = () => {
    ...
    reconnectTimer = setTimeout(() => {
      reconnectAttempts++
      connect()
    placeholder, delay + jitter)
    options.onReconnectScheduled?.({ attempt: reconnectAttempts + 1, delayMs: delay + jitter placeholder)
  placeholder
  ...
  const connect = () => {
    ...
    const wsURL = wsBaseUrl
      ? new URL(`${window.location.protocol === 'https:' ? 'wss:' : 'ws:'placeholder//${wsBaseUrlplaceholder/api/v1/admin/ops/ws/qps`)
    ...
    if (rawToken) protocols.push(`jwt.${rawTokenplaceholder`)
...
export async function getThroughputTrend(
  ...
  const { data placeholder = await apiClient.get<OpsThroughputTrendResponse>('/admin/ops/dashboard/throughput-trend', {
    params,
    signal: options.signal
  placeholder)
  return data
...
export async function getLatencyHistogram(
  ...
  const { data placeholder = await apiClient.get<OpsLatencyHistogramResponse>('/admin/ops/dashboard/latency-histogram', {
    params,
    signal: options.signal
  placeholder)
  return data
...
export async function getErrorTrend(
  ...
  const { data placeholder = await apiClient.get<OpsErrorTrendResponse>('/admin/ops/dashboard/error-trend', {
    params,
    signal: options.signal
  placeholder)
  return data
...
export async function getErrorDistribution(
  ...
  const { data placeholder = await apiClient.get<OpsErrorDistributionResponse>('/admin/ops/dashboard/error-distribution', {
    params,
    signal: options.signal
  placeholder)
  return data
...
export async function getOpenAITokenStats(
  ...
  const { data placeholder = await apiClient.get<OpsOpenAITokenStatsResponse>('/admin/ops/dashboard/openai-token-stats', {
    params,
    signal: options.signal
  placeholder)
  return data
...
export async function getErrorLogDetail(id: number): Promise<OpsErrorDetail> {
  const { data placeholder = await apiClient.get<OpsErrorDetail>(`/admin/ops/errors/${idplaceholder`)
  return data
placeholder
...
export async function listRequestDetails(params: OpsRequestDetailsParams): Promise<OpsRequestDetailsResponse> {
  const { data placeholder = await apiClient.get<OpsRequestDetailsResponse>('/admin/ops/requests', { params placeholder)
  return data
placeholder

// Alert rules
export async function listAlertRules(): Promise<AlertRule[]> {
  const { data placeholder = await apiClient.get<AlertRule[]>('/admin/ops/alert-rules')
  return data
placeholder

export async function createAlertRule(rule: AlertRule): Promise<AlertRule> {
  const { data placeholder = await apiClient.post<AlertRule>('/admin/ops/alert-rules', rule)
  return data
placeholder

export async function updateAlertRule(id: number, rule: Partial<AlertRule>): Promise<AlertRule> {
  const { data placeholder = await apiClient.put<AlertRule>(`/admin/ops/alert-rules/${idplaceholder`, rule)
  return data
placeholder
...
export async function getEmailNotificationConfig(): Promise<EmailNotificationConfig> {
  const { data placeholder = await apiClient.get<EmailNotificationConfig>('/admin/ops/email-notification/config')
  return data
placeholder

export async function updateEmailNotificationConfig(config: EmailNotificationConfig): Promise<EmailNotificationConfig> {
  const { data placeholder = await apiClient.put<EmailNotificationConfig>('/admin/ops/email-notification/config', config)
  return data
placeholder

// Runtime settings (DB-backed)
export async function getAlertRuntimeSettings(): Promise<OpsAlertRuntimeSettings> {
  const { data placeholder = await apiClient.get<OpsAlertRuntimeSettings>('/admin/ops/runtime/alert')
  return data
placeholder

export async function updateAlertRuntimeSettings(config: OpsAlertRuntimeSettings): Promise<OpsAlertRuntimeSettings> {
  const { data placeholder = await apiClient.put<OpsAlertRuntimeSettings>('/admin/ops/runtime/alert', config)
  return data
placeholder
...
export async function updateAdvancedSettings(config: OpsAdvancedSettings): Promise<OpsAdvancedSettings> {
  const { data placeholder = await apiClient.put<OpsAdvancedSettings>('/admin/ops/advanced-settings', config)
  return data
placeholder
