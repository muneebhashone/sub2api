...
import { duplicate placeholder from '@/api/admin/groups'
...
describe('admin group duplicate API', () => {
  beforeEach(() => {
    ...
    post.mockReset()
    post.mockResolvedValue({ data: { id: 43, name: 'primary (Copy)', status: 'inactive' placeholder placeholder)
  ...
  it('sends a stable idempotency key with the duplicate request', async () => {
    ...
    expect(post).toHaveBeenCalledWith('/admin/groups/42/duplicate', undefined, {
      ...
      placeholder
    placeholder)
    expect(group).toEqual({ id: 43, name: 'primary (Copy)', status: 'inactive' placeholder)
  ...
  it('reuses the operation key after a page reload', async () => {
    ...
    const { duplicate: duplicateAfterReload placeholder = await import('@/api/admin/groups')
