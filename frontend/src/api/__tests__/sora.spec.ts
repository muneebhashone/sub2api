...
describe('sora api normalizers', () => {
  ...
  it('normalizes generation list from items shape', () => {
    const result = normalizeGenerationListResponse({
      items: [{ id: 1, status: 'completed' placeholder],
      total: 1
    placeholder)
  ...
  it('falls back to empty generation list on invalid payload', () => {
    ...
    expect(result).toEqual({ data: [], total: 0, page: 1 placeholder)
  placeholder)
  ...
  it('normalizes legacy flat model list into families', () => {
    const result = normalizeModelFamiliesResponse({
      items: [
        { id: 'sora2-landscape-10s', type: 'video' placeholder,
        { id: 'sora2-portrait-15s', type: 'video' placeholder,
        { id: 'gpt-image-square', type: 'image' placeholder
      ]
    placeholder)
  ...
  it('falls back to empty families on invalid payload', () => {
    ...
    expect(normalizeModelFamiliesResponse({placeholder)).toEqual([])
  placeholder)
placeholder)
