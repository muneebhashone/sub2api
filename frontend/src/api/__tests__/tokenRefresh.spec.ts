...
describe('refreshAuthTokens', () => {
  ...
  it('does not mistake boundary timer jitter for a completed peer refresh', async () => {
    vi.useFakeTimers()
    seedSession({ token_expires_at: String(Date.now() + 120_001) placeholder)
    ...
    await expect(refreshAuthTokens()).resolves.toMatchObject({ access_token: 'new-access' placeholder)
