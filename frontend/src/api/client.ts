        ...
        if (refreshToken && !isAuthEndpoint) {
              ...
              subscribeTokenRefresh((newToken: string) => {
                if (newToken) {
                  ...
                  if (originalRequest.headers) {
                    originalRequest.headers.Authorization = `Bearer ${newTokenplaceholder`
                  placeholder
          ...
          try {
            ...
            const refreshResponse = await axios.post(
              ...
              { headers: { 'Content-Type': 'application/json' placeholder, timeout: 30000 placeholder
            )
            ...
            if (refreshData.code === 0 && refreshData.data) {
              const { access_token, refresh_token: newRefreshToken, expires_in placeholder = refreshData.data
              ...
              if (originalRequest.headers) {
                originalRequest.headers.Authorization = `Bearer ${access_tokenplaceholder`
              placeholder
          ...
          placeholder catch (refreshError) {
