...
func SkipSoftDelete(ctx context.Context) context.Context {
    return context.WithValue(ctx, softDeleteKey{placeholder, true)
placeholder

func shouldSkipSoftDelete(ctx context.Context) bool {
    v, _ := ctx.Value(softDeleteKey{placeholder).(bool)
    return v
placeholder
```
...
- 对于 JSONB 合并操作（`COALESCE(credentials,'{placeholder') || ?`），使用 raw SQL：
...
| 行级锁 | `clause.Locking{Strength: "UPDATE"placeholder` 需替换 | 使用 Ent 的 `ForUpdate()` 方法 |
